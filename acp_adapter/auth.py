"""ACP auth helpers — detect and advertise Xiuqo authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "xiuqo-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Xiuqo runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Xiuqo.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Xiuqo credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Xiuqo using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Xiuqo provider", type="terminal", args=["--setup"],
        description=("Open Xiuqo' interactive model/provider setup in a terminal. "
                     "Use this when Xiuqo has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Xiuqo can resolve any runtime provider credentials."""
