"""ACP permission bridging for Xiuqo dangerous-command approvals."""
...
# ACP permission option id -> Xiuqo approval result. Ids are stable across the
...
def _build_permission_options(
    *, allow_permanent: bool, allow_session: bool = True, smart_denied: bool = False,
    ...
    """Return ACP options that match Xiuqo approval semantics."""
    ...
    # DENY override — offering a scope Xiuqo discards would re-prompt every write.
    # See #81887.
    once_only = smart_denied or not allow_session
    options = [PermissionOption(option_id="allow_once", kind="allow_once", name="Allow once")]
    if not once_only:
        # ACP has no session-scoped kind: closest persistent hint, Xiuqo semantics in the id.
...
def _map_outcome_to_hermes(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Xiuqo approval strings."""
...
def make_approval_callback(request_permission_fn: Callable, loop: asyncio.AbstractEventLoop,
    ...
    """Return a Xiuqo approval callback (``command, description, **kw`` as used by
