...
Additive Xiuqo extension under ACP ``_meta.xiuqo`` (unknown to other clients,
...
rotates the internal Xiuqo head, ``build_session_provenance`` exposes the
...
def build_session_provenance(
    ...
    """Build ``_meta.xiuqo.sessionProvenance`` for an ACP session.
...
def session_provenance_meta(
    ...
    """Return a ready ``_meta`` payload: ``{"xiuqo": {"sessionProvenance": ...}}``."""
    ...
    return None if prov is None else {"xiuqo": {"sessionProvenance": prov}}
