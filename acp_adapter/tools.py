"""ACP tool-call helpers for mapping xiuqo tools to ACP ToolKind and building content."""
...
# Xiuqo tool name -> ACP ToolKind (anything unlisted is "other").
...
def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a xiuqo tool, defaulting to 'other'."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    Some Xiuqo tools append a human hint after the payload (``{...}\\n\\n[Hint: ...]``),
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured Xiuqo tool result clearly failed.
...
def build_tool_title(tool_name: str, args: Args) -> str:
    ...
    every other Xiuqo surface, so ACP clients never show a different summary than the CLI/TUI;
...
def build_tool_start(tool_call_id: str, tool_name: str, arguments: Args, *, edit_diff: Any = None) -> ToolCallStart:
    """Create a ToolCallStart event for the given xiuqo tool invocation.
