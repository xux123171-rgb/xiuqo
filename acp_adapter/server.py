"""ACP agent server — exposes Xiuqo Agent via the Agent Client Protocol."""
...
from acp_adapter.commands import XIUQO_VERSION, SlashCommandsMixin, _estimate_tokens
...
def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
    ...
    if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
        ...
        kind = "standalone"
    if kind == "standalone":
        return {"xiuqo": {"compactionSummary": True}}
    if kind == "merged":
        return {"xiuqo": {"containsCompactionSummary": True}}
    return None
...
class HermesACPAgent(SlashCommandsMixin, acp.Agent):
    """ACP Agent implementation wrapping Xiuqo AIAgent."""
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        """Authenticated providers + models, from the shared Xiuqo inventory (same substrate
        as ``xiuqo model``/TUI/dashboard) so the selector isn't just the current curated list."""
    ...
    def _switch_model(
        ...
        Resolution goes through ``xiuqo_cli.model_switch.switch_model`` seeded with the live
        ...
        from xiuqo_cli.models import parse_model_input
    ...
    def _provenance_meta(
        ...
        """Best-effort ``_meta.xiuqo.sessionProvenance`` for an ACP session."""
        try:
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            agent.enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(agent, "enabled_toolsets", None) or ["xiuqo-acp"],
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        try:
            from xiuqo_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from xiuqo_cli.mcp_startup import join_mcp_discovery
    ...
    async def initialize(
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="xiuqo", version=XIUQO_VERSION),
    ...
    def _run_agent_turn(
        ...
        Interactive routing is a ``tools.approval`` contextvar, not ``XIUQO_INTERACTIVE`` in
        ...
        with contextlib.ExitStack() as stack:
            # XIUQO_SESSION_KEY scopes per-session caches (interactive sudo password) to this
            # session, not the reused thread. ``cwd`` pins what the system prompt reports as the
            # working directory — otherwise it advertises the Xiuqo workspace while tools are
            # rooted at the client's project and edits land outside it. ``cron_session=""`` masks
            # any leaked process-global XIUQO_CRON_SESSION.
            ...
            stack.callback(_restore_env, "XIUQO_SESSION_ID", os.environ.get("XIUQO_SESSION_ID"))
    ...
    async def prompt(self, prompt: list[PromptBlock], session_id: str, **kwargs: Any) -> PromptResponse:
        """Run Xiuqo on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Xiuqo has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
