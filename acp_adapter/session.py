"""ACP session manager — maps ACP sessions to Xiuqo AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.xiuqo/state.db``) so they
...
from xiuqo_constants import get_hermes_home, translate_cwd_for_wsl_backend, windows_path_to_wsl
...
def _translate_acp_cwd(cwd: str) -> str:
    ...
    when Xiuqo runs in WSL so agents, tools, and persisted sessions agree; no-op elsewhere."""
...
def _expand_acp_enabled_toolsets(toolsets: List[str] | None = None,
    ...
    names = [n for n in (toolsets or ["xiuqo-acp"]) if n]
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Xiuqo agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Xiuqo AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        the runtime provider config. ``db``: SessionDB; default lazily opens ``~/.xiuqo/state.db``."""
    ...
    def _get_db(self):
        ...
        minimal test env). ``XIUQO_HOME`` is resolved here, not via the import-time
    ...
    def _make_agent(self, *, session_id: str, cwd: str, model: str | None = None,
        ...
        kwargs = {
            ...
            "enabled_toolsets": _expand_acp_enabled_toolsets(["xiuqo-acp"], mcp_server_names=configured_mcp_servers),
