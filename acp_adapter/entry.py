"""CLI entry point for the xiuqo ACP adapter.

Loads ``~/.xiuqo/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: xiuqo acp / xiuqo-acp
"""

# IMPORTANT: xiuqo_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See xiuqo_bootstrap.py for full rationale.
try:
    import xiuqo_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Partial ``xiuqo update`` (git-reset landed, ``uv pip install -e .`` did not):
    # UTF-8 stdio setup is skipped on Windows; POSIX is unaffected.
    pass
else:
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing Xiuqo modules.
...
def _load_env() -> None:
    """Load .env from XIUQO_HOME (default ``~/.xiuqo``)."""
    from xiuqo_cli.env_loader import load_hermes_dotenv

    xiuqo_home = get_hermes_home()
    loaded = load_hermes_dotenv(xiuqo_home=xiuqo_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", xiuqo_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="xiuqo-acp", description="Run Xiuqo Agent as an ACP stdio server.")
    parser.add_argument("--version", action="store_true", help="Print Xiuqo version and exit")
    parser.add_argument("--check", action="store_true", help="Verify ACP dependencies and adapter imports, then exit")
    parser.add_argument("--setup", action="store_true",
                        help="Run interactive Xiuqo provider/model setup for ACP terminal auth")
    parser.add_argument("--setup-browser", action="store_true",
                        help="Install agent-browser + Playwright Chromium into ~/.xiuqo/node/ "
...
def _print_version() -> None:
    from xiuqo_cli import __version__ as xiuqo_version
...
def _run_check() -> None:
    ...
    print("Xiuqo ACP check OK")


def _run_setup() -> None:
    from xiuqo_cli.main import main as xiuqo_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "xiuqo", "model"]
        xiuqo_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting xiuqo ACP adapter")
    ...
    if os.environ.get("XIUQO_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
