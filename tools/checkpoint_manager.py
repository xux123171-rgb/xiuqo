...
config / ``--checkpoints`` flag.  One store under ``~/.xiuqo/checkpoints/`` so git dedupes
blobs across projects (pre-v2 one-repo-per-workdir re-stored ~40 MB each): ``store/`` bare
repo with per-project ``refs/xiuqo/<hash16>``, ``indexes/<hash16>``, ``projects/<hash16>.json``
...
def _resolve_checkpoint_base() -> Path:
    ...
    changed it, else live profile-scoped XIUQO_HOME — under the multiplexed gateway one process
...
_REFS_PREFIX, _LEGACY_PREFIX, _PRUNE_MARKER_NAME = "refs/xiuqo", "legacy-", ".last_prune"
...
DEFAULT_EXCLUDES = [
    ...
    ".git/", ".hg/", ".svn/", ".worktrees/",  # VCS + worktrees (Xiuqo convention — don't snapshot siblings)
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("XIUQO_CHECKPOINT_TIMEOUT", 30)))
...
_STORE_GIT_CONFIG = (("user.email", "xiuqo@local"), ("user.name", "Xiuqo Checkpoint"),
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file``/``patch`` produced, so restores can tell Xiuqo' writes from later user edits."""
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    users may want to recover; the archive falls under retention and ``xiuqo checkpoints
    ...
    logger.info("Migrated pre-v2 checkpoint repos to %s. "
                "Clear with `xiuqo checkpoints clear-legacy` when safe.", legacy_root)
    return legacy_root
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir() if base.exists() else ():
        ...
        try:
            if (child / "XIUQO_WORKDIR").exists():
                workdir = (child / "XIUQO_WORKDIR").read_text(encoding="utf-8").strip()
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Xiuqo just wrote (agent-write ledger), so safe-mode
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        """Classify files changed since ``commit_hash``: ``restore`` = still matching what Xiuqo
        last wrote (or deleted since); ``skipped`` = user-edited afterwards or never written by
        Xiuqo.  ``ledger_empty`` => no ledger, callers fall back to a full restore."""
        ...
        for rel in filter(None, names_out.split("\x00")):
            ...
            xiuqo_authored = recorded is not None and _hash_file(abs_path) in (None, recorded)
            out["restore" if xiuqo_authored else "skipped"].append(rel)
    ...
    def restore(self, working_dir: str, commit_hash: str, file_path: str = None,
        ...
        the user hand-edited after Xiuqo' last write untouched (agent-write ledger); the result
    ...
    def _apply_safe_restore_deletes(self, p: _ProjectRefs, commit_hash: str,
        ...
        from the checkpoint is Xiuqo-created (delete to restore) — unless ``max_file_size_mb`` kept
