...
def _capture_required_environment_variables(
    ...
    # hint. Interactive gateway surfaces (desktop app / TUI) set XIUQO_INTERACTIVE (same flag
    # tools/approval.py uses) and register a callback routing to a secure secret.request overlay.
    if _is_gateway_surface() and not env_var_enabled("XIUQO_INTERACTIVE"):
        try:
...
def _is_gateway_surface() -> bool:
    if env_var_enabled("XIUQO_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("XIUQO_SESSION_PLATFORM"))
