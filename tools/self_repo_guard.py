...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(xiuqo_home).expanduser() if xiuqo_home else Path.home() / ".xiuqo") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite Xiuqo's live source checkout "
        ...
        "is pushed. To change this checkout, stop Xiuqo, run the command externally, then restart "
        "Xiuqo.")
