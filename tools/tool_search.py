...
tool_search / tool_describe / tool_call. Invariants: core tools (``toolsets._XIUQO_CORE_TOOLS``)
...
def _config_from_loader(loader_name: str) -> ToolSearchConfig:
    """Tool-search config via ``xiuqo_cli.config.<loader_name>`` (defaults on any failure)."""
    try:
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_XIUQO_CORE_TOOLS)
    except Exception:
        return frozenset()


# Session-gated GUI toolsets: off ``_XIUQO_CORE_TOOLS`` so non-GUI clients never pay
