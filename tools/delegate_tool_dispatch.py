...
def _capture_origin() -> tuple[str, str, Any, Any, bool]:
    ...
    clobbers the XIUQO_SESSION_ID ContextVar/os.environ with the subagent's id.  The wake-
    ...
    with _quiet(None):
        ...
        _origin_ui_session_id = get_session_env("XIUQO_UI_SESSION_ID", "")
...
_SYNC_FALLBACK_NOTES = {
    "no_async": (
        ...
        "finite chat using -Q, --oneshot, or non-TTY stdio, `xiuqo -z`, a cron job, a Kanban "
...
def _resolve_async_wake_sid(origin_wake_sid: str, origin_session_history_delivery: bool = False) -> Optional[str]:
    ...
    if get_session_env("XIUQO_SINGLE_QUERY_SESSION") == "1":
        return None
...
def _resolve_async_session_key(parent_agent: Any, origin_ui_session_id: str) -> tuple[str, str]:
    ...
    platform conversation key (agent:main:...). The CLI has no bound approval contextvar and no XIUQO_SESSION_KEY, so
    ...
    with _quiet(None):
        ...
        origin_ui_session_id = get_session_env("XIUQO_UI_SESSION_ID", "") or origin_ui_session_id
