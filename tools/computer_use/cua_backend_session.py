...
_UNKNOWN_OUTCOME_MESSAGES = {
    "transport_outcome_unknown": (
        "cua-driver transport failed during {name}; the action outcome is unknown, so Xiuqo "
...
class _CuaDriverSession:
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            # Telemetry policy first (default: disabled), then strip Xiuqo secrets.
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                f"{getattr(self, '_startup_phase', 'unknown')}). Run `xiuqo computer-use doctor` and check "
