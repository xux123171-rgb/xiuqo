...
# Hard-blocked regardless of approval level (e.g. logout kills the session Xiuqo runs in). Alt is
...
# Per-Xiuqo-session cached backends (own cua-driver session, native target, refs, grant namespace).
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Xiuqo's approval bypass onto Cua's immutable mode; fails closed. Both identity namespaces are consulted
...
def _new_backend(permission_mode: str) -> ComputerUseBackend:
    backend_name = os.environ.get("XIUQO_COMPUTER_USE_BACKEND", "cua").lower()
    if backend_name in {"cua", "cua-driver", ""}:
        from tools.computer_use.cua_backend import CuaDriverBackend
        return CuaDriverBackend(permission_mode=permission_mode)
    if backend_name != "noop":
        raise RuntimeError(f"Unknown XIUQO_COMPUTER_USE_BACKEND={backend_name!r}")
...
def _shutdown_backend_atexit() -> None:
    ...
    the Xiuqo process that spawned it (#28152 item 3). #69903 kept the orphan from burning a core by
...
class _NoopBackend(ComputerUseBackend):  # pragma: no cover
    """Test/CI stub (XIUQO_COMPUTER_USE_BACKEND=noop). Records ``(name, kwargs)`` calls; returns trivial results."""
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({"error": f"computer_use backend unavailable: {e}",
                           "hint": "If the cua-driver binary is missing, run `xiuqo computer-use install`. "
...
def _cache_file(subdir: str, legacy: str, name: str, pattern: str = "", cap: int = 0):
    """Path for a new file under ``$XIUQO_HOME/<subdir>`` (dir created). With ``pattern``/``cap``, first unlinks the
    oldest matching files so at most ``cap - 1`` remain (best-effort)."""
    from xiuqo_constants import get_hermes_dir  # lazy so tests can patch get_hermes_dir
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Copy of the capture in Xiuqo' media cache so attachment surfaces can deliver it (None without an image)."""
...
def _should_route_through_aux_vision() -> bool:
    ...
    try:
        ...
        if (cached := _AUX_VISION_ROUTE_CACHE.get(key := (xiuqo_home_key(), str(provider), str(model)))) is not None:
            return cached
        stage = "decision"
...
def _route_capture_through_aux_vision(cap: CaptureResult, summary: str, *, visible_elements: Optional[List[UIElement]] = None,
    ...
    """Pre-analyse the capture via ``vision_analyze_tool`` (temp file under ``$XIUQO_HOME/cache/vision/``) and merge
...
def check_computer_use_requirements() -> bool:
    """macOS/Windows/Linux + cua-driver binary (or env override). `xiuqo computer-use doctor` names blocked checks."""
