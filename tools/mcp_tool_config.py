...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    from xiuqo_cli.env_loader import secret_source_names
    env = {
    ...
    for key in secret_source_names():
        ...
        if value is not None:
            env[key] = value
    for key in ("XIUQO_KANBAN_DB", "XIUQO_KANBAN_BOARD"):
...
def _node_fallback(command: str) -> str:
    ...
    xiuqo_home = os.path.expanduser(os.getenv("XIUQO_HOME", os.path.join(home, ".xiuqo")))
    # /usr/local/bin: canonical Node location (from-source Linux, Xiuqo Docker image, Intel Homebrew),
    # needed when a hand-authored env.PATH omits it — npx's shebang re-execs /usr/bin/env node.
    candidates = (os.path.join(xiuqo_home, "node", "bin", command), os.path.join(home, ".local", "bin", command),
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    extension (same precedence as ``xiuqo_constants._candidate_node_command_names``). ``windows``
...
def _npx_cached_bin(args: list) -> Optional[tuple]:
    ...
    for nothing (~48 MB private memory per MCP server, measured); Xiuqo already supervises the
