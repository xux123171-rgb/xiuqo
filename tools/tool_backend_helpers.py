...
def nous_tool_gateway_unavailable_message(capability: str = "the Nous Tool Gateway", *,
    ...
    try:
        ...
        pass
    return (f"{capability} is unavailable. Run `xiuqo model` to refresh your "
...
def _dotenv_value(env_var: str) -> str:
    """``.env`` value via ``xiuqo_cli.config.get_env_value`` (``""`` when unavailable)."""
    try:
...
def resolve_provider_secret(env_var: str, provider_id: str, config_value: str = "",
    ...
    ``xiuqo_cli.config.get_env_value``) -> credential pool for ``provider_id``. Under an
    ...
    Resolution order (fixes #68003 — keys added via ``xiuqo auth add <provider>`` were invisible to the
...
def resolve_openai_audio_api_key() -> str:
    ...
    Outside a multiplexed turn, ``OPENAI_API_KEY`` additionally falls back to the credential pool (``xiuqo
...
def read_selection(section: str) -> str | None:
    """THE single runtime read of the persisted `xiuqo tools` selection: ``"nous"`` (managed
...
# (xiuqo_cli.config.validate_config_structure) and selection_error() consult this map so the user learns
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (f"{section} is configured to use {selection_name} (set via xiuqo "
            f"tools), but {failure}. Run 'xiuqo tools' to change it.")
