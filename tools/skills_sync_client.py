...
tick ``maybe_pull_skills`` and ``xiuqo sync``. Lives under tools/ so it never imports the CLI at
...
cross-device state is the ``sync-manifest`` blob in the plane. Only ~/.xiuqo/skills/ skills qualify."""
...
# Configuration -- env-first so Xiuqo Cloud can enable sync via environment alone. Every knob:
# XIUQO_SYNC_<KEY> env -> config.yaml ``sync.<key>`` -> default (base_url = the sync plane, NOT
...
def resolve_sync_base_url() -> Optional[str]:
    """XIUQO_SYNC_BASE_URL -> ``sync.base_url`` -> production plane, without trailing slash
    (``/v1/sync/`` is appended by the client). None only if the default is blanked out."""
    env = os.getenv("XIUQO_SYNC_BASE_URL")
...
def sync_feature_enabled() -> bool:
    ...
    return _sync_config_bool("XIUQO_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """False (default): edits to an org skill stay LOCAL until ``xiuqo sync propose``. True: every
    edit is proposed right away (an admin still approves unless the editor is one)."""
    return _sync_config_bool("XIUQO_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False)


def sync_default_opt_in() -> bool:
    """False (default): opt-IN -- a skill syncs only after ``xiuqo sync enable`` or a plane manifest
    opting it in. True: opt-OUT -- every eligible skill syncs unless disabled (Xiuqo Cloud default)."""
    return _sync_config_bool("XIUQO_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)
...
def _rel_to_skills_dir(skill_dir: Path) -> Optional[Path]:
    """*skill_dir* relative to ~/.xiuqo/skills/, or None if outside/unresolvable."""
    try:
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """The skill's path relative to ~/.xiuqo/skills/ (posix), or None."""
...
def _all_local_skill_names() -> List[str]:
    """Every local skill name (dir under ~/.xiuqo/skills/ with SKILL.md; frontmatter ``name`` or dir name)."""
    names: List[str] = []
    root = _skills_dir()
    try:
...
def stable_device_id() -> str:
    """Per-device label at ~/.xiuqo/skills/.sync_device_id. An existing file always wins; else seeded
    from XIUQO_SYNC_DEVICE_NAME (first use only, for Xiuqo Cloud) or a friendly default, then persisted."""
    with suppress(OSError):
        val = _device_id_path().read_text(encoding="utf-8").strip()
        if val:
            return val
    val = (os.environ.get("XIUQO_SYNC_DEVICE_NAME") or "").strip() or _default_device_label()
    try:
...
# the bundled manifest (skills_sync.py) and the plane's `sync-manifest`. ~/.xiuqo/skills/.sync_state.
...
def push_skills(client: Optional[SyncClient] = None, *, skill_names: Optional[List[str]] = None,
                identity: Optional[Dict[str, Any]] = None, message: str = "xiuqo skill sync") -> Dict[str, Any]:
    ...
    if overlaps:
        ...
        return {"ok": False, "conflict": True, "conflict_ref": conflict_ref, "overlapping_skills": sorted(overlaps),
                "actual_head": actual_head, "message": (f"{len(overlaps)} skill(s) changed on both sides; wrote "
                                                        f"{conflict_ref}. Resolve out-of-band (xiuqo sync / NAS UI).")}
...
def maybe_push_skills(*, message: str = "xiuqo skill sync") -> Optional[Dict[str, Any]]:
...
def sync_status() -> Dict[str, Any]:
    """Snapshot for ``xiuqo sync status``; never raises. ``org_available`` False = not in a shared org."""
