...
GitHub releases to $XIUQO_HOME/bin/tirith in a background thread -- SHA-256 always verified,
...
from xiuqo_constants import get_hermes_home, get_hermes_home_override, xiuqo_home_key
...
def _cached_path() -> str | None:
    ...
    if get_hermes_home_override() is not None:
        return _resolved_path_by_home.get(xiuqo_home_key())
...
def _store_resolved(path: str) -> None:
    ...
    if get_hermes_home_override() is not None:
        _resolved_path_by_home[xiuqo_home_key()] = path
    else:
        _resolved_path = path
...
def _hermes_bin_dir() -> str:
    """$XIUQO_HOME/bin, created if needed."""
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $XIUQO_HOME/bin/tirith -> ``(installed_path,
...
def _find_local_tirith() -> str | None:
    """Cheap local lookup for the default "tirith": PATH, then $XIUQO_HOME/bin."""
    xiuqo_bin = os.path.join(_hermes_bin_dir(), "tirith")
    return shutil.which("tirith") or (xiuqo_bin if _is_executable(xiuqo_bin) else None)
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    $XIUQO_HOME/bin/tirith → install; failures cached in-process and on disk for 24h). On a miss
