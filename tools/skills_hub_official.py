"""Skills Hub official sources: repo-shipped optional skills and the centralized Xiuqo index."""
...
class OptionalSkillSource(SkillSource):
    ...
    activated by default — absent from the system prompt and not copied to ~/.xiuqo/skills/ at
    setup. Discoverable via the Skills Hub as source "official" with "builtin" trust."""

    SOURCE_ID = "official"
    TRUST_LEVEL = "builtin"
    OFFICIAL_REPO = "xux123171-rgb/xiuqo"
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # (metadata.xiuqo.upstream); install pulls the live content from there.
        try:
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``main``; rather than demanding ``xiuqo update`` first, resolve against the live repo.
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.xiuqo.upstream: {repo: owner/name, path: ...}`` out of SKILL.md content
        ...
        xiuqo_meta = meta_block.get("xiuqo") if isinstance(meta_block, dict) else None
        upstream = xiuqo_meta.get("upstream") if isinstance(xiuqo_meta, dict) else None
...
class HermesIndexSource(SkillSource):
    """Skill source backed by the centralized Xiuqo Skills Index: a JSON catalog on the docs site,
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        for github_id in filter(None, candidates):
            ...
            if bundle:
                bundle.source = entry.get("source", "xiuqo-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            ...
            source=entry.get("source", "xiuqo-index"), identifier=entry.get("identifier", ""),
