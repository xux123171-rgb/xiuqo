...
# prefix, the exact-name XIUQO_ allowlist, or (Windows) an OS-essential name.
# The broad "XIUQO_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (XIUQO_BASE_URL, XIUQO_KANBAN_DB, *_WEBHOOK).
# XIUQO_RPC_SOCKET / XIUQO_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. XIUQO_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports Xiuqo code loses the Kanban mutation guard
# while still inheriting XIUQO_HOME.
_XIUQO_CHILD_ALLOWED = frozenset({
    "XIUQO_HOME", "XIUQO_PROFILE", "XIUQO_CONFIG", "XIUQO_ENV", "XIUQO_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational XIUQO_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret XIUQO_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    for k, v in source_env.items():
        ...
        if (any(k.startswith(p) for p in _SAFE_ENV_PREFIXES)
                or k in _XIUQO_CHILD_ALLOWED
                or (is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS)):
            scrubbed[k] = v
        elif k.startswith("XIUQO_"):
            _dropped_hermes.append(k)
    if _dropped_hermes:
        logger.debug(
            "execute_code: dropped %d non-allowlisted XIUQO_* var(s) from the "
    ...
    # non-allowlisted XIUQO_* values in otherwise ordinary execution environments.
    if scoped.get(DELEGATED_CHILD_ENV_MARKER):
        for key in (DELEGATED_CHILD_ENV_MARKER, "XIUQO_KANBAN_DB", "XIUQO_KANBAN_BOARD"):
            if key in scoped:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    from xiuqo_constants import apply_subprocess_home_env
    child_env = _scrub_child_env(os.environ)
    child_env["XIUQO_RPC_SOCKET"] = rpc_endpoint
    child_env["XIUQO_RPC_TOKEN"] = rpc_token
    ...
    # Only TZ reaches the child; XIUQO_TIMEZONE is an internal setting (and under the multiplexed
    # gateway holds only the default profile's value — xiuqo_time resolves the routed profile's).
    from xiuqo_time import get_timezone_name

    _tz_name = get_timezone_name()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("XIUQO_TIMEZONE", None)
    apply_subprocess_home_env(child_env)
    # PYTHONPATH: the staging dir (xiuqo_tools.py) must always be importable even when project
    # mode changes CWD. Xiuqo's root is added ONLY when the child runs in Xiuqo's Python env —
    # exposing Xiuqo's site-packages to an external interpreter can mix incompatible compiled
    # extensions (3.12 NumPy under a 3.9 venv). Inherited Xiuqo-owned entries are stripped first.
    # Before re-injecting PYTHONPATH, strip Xiuqo-owned entries that leaked through _scrub_child_env
    # (PYTHONPATH is in _SAFE_ENV_PREFIXES so it passes the scrub). They are redundant for same-Xiuqo-
    ...
    if _uses_hermes_python_environment(child_python):
        ...
        # Surface once per interpreter so "import xiuqo_constants fails" is diagnosable.
        _external_env_logged.add(child_python)
        logger.info("execute_code: child interpreter %s is outside the Xiuqo "
                    "environment; xiuqo root omitted from PYTHONPATH", child_python)
    if _existing_pp:
...
# Interpreter paths already reported as outside the Xiuqo environment.
...
def _uses_hermes_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Xiuqo's active Python environment. Short-circuits when
    it IS the running interpreter (by path or realpath — covers ``uv run`` venvs) so no probe
    runs on the default strict path and a flaky probe can never drop the xiuqo root."""
