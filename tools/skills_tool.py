...
XIUQO_HOME = get_hermes_home()  # all skills live in ~/.xiuqo/skills/ (seeded from bundled)
...
def _skills_dir() -> Path:
    ...
    it, else live profile-scoped XIUQO_HOME (long-lived runtimes may import before profile set)."""
...
def load_env() -> Dict[str, str]:
    """Snapshot of XIUQO_HOME/.env for the post-skill secret-capture diff (same tokenizer that
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    """``~/.xiuqo/skills/mlops/axolotl/SKILL.md`` -> ``"mlops"``; active profile dir first
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    """Disabled in config? Platform precedence: explicit arg, ``XIUQO_PLATFORM``, session
    ``XIUQO_SESSION_PLATFORM``. A globally-disabled skill stays disabled on every platform
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("XIUQO_PLATFORM")
        if not resolved_platform:
            with suppress(Exception):
                from gateway.session_context import get_session_env
                resolved_platform = get_session_env("XIUQO_SESSION_PLATFORM") or ""
...
def _org_provenance_header(skill_dir: Path, active_skills_dir: Path):
    ...
    header = (
        ...
        "> `xiuqo sync propose` (or automatically, if your org enables it).\n\n")
    ...
    if skill_md is not None and project_dirs:
        ...
        if _under_any(skill_md, project_dirs) and is_quarantined_project_skill(skill_md):
            return _fail(
                ...
                "`xiuqo skills untrust`."), None, None
...
def _log_security_warnings(name: str, skill_md: Path, content: str, all_dirs, active_skills_dir):
    ...
    if not _under_any(skill_md, trusted_dirs):
        warnings.append(f"skill file is outside the trusted skills directory (~/.xiuqo/skills/): {skill_md}")
...
def skill_view(
    ...
    try:
        ...
        if _is_skill_disabled(resolved_name):
            return _fail(f"Skill '{resolved_name}' is disabled. Enable it with `xiuqo skills` or inspect the files directly on disk.")
        if file_path and skill_dir:
            return _serve_skill_file(
                skill_dir, file_path, name, list_available=True, mark_read=True,
                hint="Use a relative path within the skill directory")
        # tags/related_skills: metadata.xiuqo.* (agentskills.io) first, then top-level.
        metadata = frontmatter.get("metadata")
        xiuqo_meta = (metadata.get("xiuqo", {}) or {}) if isinstance(metadata, dict) else {}
        tags, related_skills = (
            _parse_tags(xiuqo_meta.get(k) or frontmatter.get(k, "")) for k in ("tags", "related_skills"))
