...
def xiuqo_xai_user_agent() -> str:
    """Return a stable Xiuqo-specific User-Agent for xAI HTTP calls."""
    try:
        from xiuqo_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Xiuqo-Agent/{__version__}"


def xiuqo_xai_default_headers() -> Dict[str, str]:
    """Default headers for OpenAI-SDK and raw HTTP clients talking to xAI (replaces the SDK User-Agent)."""
    return {"User-Agent": xiuqo_xai_user_agent()}
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Xiuqo home, then mark it seen."""
...
def _xai_base_url_override() -> str:
    """``XIUQO_XAI_BASE_URL`` then ``XAI_BASE_URL``, stripped; '' when unset."""
    from xiuqo_cli.config import get_env_value
    return str(get_env_value("XIUQO_XAI_BASE_URL") or get_env_value("XAI_BASE_URL") or "").strip().rstrip("/")
...
def resolve_xai_http_credentials(
    *, force_refresh: bool = False, api_key_hint: Optional[str] = None, prefer_api_key: bool = False,
    ...
    Default order: Xiuqo-managed xAI OAuth, then ``XAI_API_KEY`` (via ``get_env_value`` so
    ``~/.xiuqo/.env`` keys count). ``prefer_api_key=True`` inverts that for API-metered
    endpoints where the subscription OAuth bearer authorizes but misbehaves (x_search answers
    without citations, TTS 403s). Both branches honor ``XIUQO_XAI_BASE_URL``/``XAI_BASE_URL``
    ...
    Prefers Xiuqo-managed xAI OAuth credentials when available, then falls back to ``XAI_API_KEY`` resolved
    via ``xiuqo_cli.config.get_env_value`` so keys stored in ``~/.xiuqo/.env`` (the standard Xiuqo
    ...
    scoping is identical to the fallback branch, and the base URL honors ``XIUQO_XAI_BASE_URL`` /
