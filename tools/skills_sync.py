...
~/.xiuqo/skills/, tracking each synced skill's origin hash in .bundled_manifest (v2 "name:hash"
...
from xiuqo_constants import get_bundled_skills_dir, get_hermes_home, get_optional_skills_dir
...
# retarget XIUQO_HOME after import, and frozen constants would resolve (and for
...
# once under the launch XIUQO_HOME and later scope requests to a different profile via
# set_hermes_home_override(). See #65828.
_XIUQO_HOME_AT_IMPORT = XIUQO_HOME
...
def _hermes_home() -> Path:
    return _live(XIUQO_HOME, _XIUQO_HOME_AT_IMPORT, get_hermes_home)
...
# Written by `xiuqo profile create --no-skills` / installer `--no-skills`: sync seeds only
# essential skills. Mirrors xiuqo_cli.profiles.NO_BUNDLED_SKILLS_MARKER (no CLI import here).
NO_BUNDLED_SKILLS_MARKER = ".no-bundled-skills"


def _get_bundled_dir() -> Path:  # XIUQO_BUNDLED_SKILLS env first, then repo-relative
    ...
    for candidate in st.active_index.get(skill_name, []):
        ...
        if not _matches_origin_hash(candidate, origin_hash):  # moving a customized copy would edit user work
            st.say(
                ...
                f"Run `xiuqo skills reset {skill_name} --restore` to move to the new location.")
            continue
        try:
        ...
        if dest.exists():
            ...
            if _dir_hash(dest) == bundled_hash:
                ...
                st.say(
                    ...
                    f"by this name — yours was kept. Run `xiuqo skills reset {skill_name}` to "
...
def sync_skills(quiet: bool = False) -> dict:
    """Sync bundled skills into ~/.xiuqo/skills/ using the manifest; returns the per-category
...
def _rmtree_writable(path: Path) -> None:
    ...
    child of the active skills root (bad join / missing XIUQO_HOME / malicious manifest entry).
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.xiuqo/skills/ ...")
