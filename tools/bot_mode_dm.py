...
local → ``xiuqo -p <name> chat --in ~ -c "Bot Chat" --create-if-missing -Q
--query-file <tmp>``; peer → ``xiuqo peer dm <peer>[/<name>] < <tmp>``; both via
...
# delivery runner (``python bot_mode_dm.py --run-delivery …``); Xiuqo helpers import lazily.
...
_DM_DIR_NAME = "xiuqo-dm"
_DM_STALE_SECONDS = 24 * 60 * 60
_LIVE_WAIT_SECONDS = 300

# '<peer>/<agent>' — peer names are lowercase (``xiuqo peer`` normalizes them).
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'xiuqo' for the default agent), or "
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('xiuqo' → 'default')."""
...
def message_agent_tool(target: str = "", message: str = "", task_id: Optional[str] = None, agent: Any = None) -> str:
    ...
    if peer_match or raw_target.lower() in peers:
        ...
        # Pin the registry-owning profile: `xiuqo peer` resolves bot_peers via the profile-scoped
        ...
        return _start_delivery(["xiuqo", "-p", _self_profile_name(root), "peer", "dm", dm_target], content,
    ...
    return _start_delivery(["xiuqo", "-p", resolved, *BOT_CHAT_TURN_ARGS], content, f"@{_handle(resolved)}",
...
def cleanup_bot_dm_cache(max_age_hours: float = _DM_STALE_SECONDS / 3600, *, now: float | None = None) -> int:
    ...
    locations = [(temp_root, "xiuqo-dm-*.txt"), (temp_root, "xiuqo-relay-dm-*.txt")]
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    if stdin_file or len(argv) < 3 or cli not in ("xiuqo", "xiuqo.exe") or argv[1] != "-p":
...
def _run_local_turn(argv: list[str], dm_file: str, *, env: Optional[dict[str, str]] = None) -> int:
    ...
    reason = next((line.removeprefix("xiuqo-refusal-reason: ").strip()
                   for line in stderr_text.splitlines()
                   if line.startswith("xiuqo-refusal-reason: ")), None)
...
def _local_delivery_home(argv: list[str]) -> Path | None:
    cli = (argv[0] if argv else "").rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
    if len(argv) < 3 or cli not in ("xiuqo", "xiuqo.exe") or argv[1] != "-p":
        return None
...
def _run_delivery(argv: list[str], dm_file: str, *, stdin_file: bool,
    ...
    ``author`` rides to the child as XIUQO_TURN_AUTHOR; ``xiuqo peer dm`` forwards it in the request body.
