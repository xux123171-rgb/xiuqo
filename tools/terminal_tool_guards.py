...
_FOREGROUND_GUIDANCE = (
    (
        ...
        "notify_on_complete=true) so Xiuqo tracks the process, then run readiness "
...
def gateway_lifecycle_block(
    *,
    ...
    ``systemctl``/``launchctl``/``xiuqo gateway restart|stop|uninstall``
    targeting xiuqo-gateway would SIGTERM the gateway — and this very
    ...
    SUPERVISED-gateway probe, not the raw ``_XIUQO_GATEWAY`` marker: that
    marker leaks into every process that merely imports gateway.run (xiuqo
    serve, CLI, web server), which must still be able to restart the gateway;
    an unsupervised foreground ``xiuqo gateway run`` has no KeepAlive to turn
    ...
    if lifecycle_scan_root_within_budget(command) and contains_launchctl_submit_command(command):
        return _blocked_json(
            ...
            "or determine whether it is independent of Xiuqo. Perform authorized "
    ...
    if contains_gateway_lifecycle_command_or_referenced_script(
        ...
        return _blocked_json(
            ...
            "to child processes). Run `xiuqo gateway restart` from a "
