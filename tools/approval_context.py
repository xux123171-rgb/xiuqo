...
# Xiuqo session id (observability identity, distinct from the gateway routing session_key), forwarded to approval
...
# os.environ["XIUQO_INTERACTIVE"] races: one session's `finally` restore can clobber another's set mid-run, dropping
...
_hermes_interactive_ctx: contextvars.ContextVar[str | None] = _ctx("xiuqo_interactive", None)
...
def _is_interactive_cli() -> bool:
    ...
    return is_truthy_value(ctx_val) if ctx_val is not None else env_var_enabled("XIUQO_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("XIUQO_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    return _session_env("XIUQO_SESSION_PLATFORM")


def _is_cron_approval_context() -> bool:
    """True when the current approval decision is running inside cron."""
    return is_truthy_value(_session_env("XIUQO_CRON_SESSION"))
...
def _is_unattended_platform_approval_context() -> bool:
    ...
    Webhook, msgraph_webhook, and api_server sessions bind ``XIUQO_SESSION_PLATFORM`` like chat gateways
...
def _is_single_query_approval_context() -> bool:
    """True for a single-query (-q) session: ``xiuqo chat -q`` exports
    ``XIUQO_INTERACTIVE=1`` (so sudo password prompts work) but nobody is waiting
    ...
    return is_truthy_value(_session_env("XIUQO_SINGLE_QUERY_SESSION"))


def _is_gateway_approval_context() -> bool:
    """True inside a gateway/API session that can answer an approval.

    Legacy integrations set XIUQO_GATEWAY_SESSION; concurrent paths bind
    XIUQO_SESSION_PLATFORM via contextvars. Cron is NEVER a gateway approval
    ...
    if _is_cron_approval_context() or _is_unattended_platform_approval_context():
        return False
    return env_var_enabled("XIUQO_GATEWAY_SESSION") or bool(_get_session_platform())
...
def _should_fall_through_to_cli_approval(*, is_cli: bool, approval_callback, notify_cb) -> bool:
    ...
    ``XIUQO_EXEC_ASK`` (or a platform marker) can leak into an interactive CLI
