...
For custom/local-only tools do NOT edit core: create `~/.xiuqo/plugins/<name>/plugin.yaml` +
...
1. `tools/your_tool.py`:
   ...
   ```
2. `toolsets.py`: add the name to `_XIUQO_CORE_TOOLS` (all platforms) or a new toolset. **Required**
   — discovery registers the schema, but a tool is only exposed if a toolset names it.
   `_XIUQO_CORE_TOOLS` is the default bundle every platform's base toolset inherits, not dead code.
...
- **Paths in schema descriptions use `display_hermes_home()`** (schema is built at import, after
  `_apply_profile_override()` set `XIUQO_HOME`). **State files use `get_hermes_home()`**, never
  `Path.home()/.xiuqo`, so each profile gets its own state.
...
- New tools integrate with existing setup UX (`xiuqo tools`, `xiuqo setup`, auto-install) rather
  than a raw env var; secrets go in `OPTIONAL_ENV_VARS` (`xiuqo_cli/AGENTS.md`).
...
`xiuqo tools` (curses) or `tools.<platform>.enabled/disabled` in config.yaml. `browser_exec`
...
imports against a temp `XIUQO_HOME` (see `tests/tools/test_approval_config_readonly.py`).
