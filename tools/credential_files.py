...
from xiuqo_constants import get_hermes_dir, get_hermes_home
...
def _contained_host_path(rel: str, xiuqo_home: Path, abs_msg: str, traversal_msg: str) -> Optional[Path]:
    """Resolve *rel* under XIUQO_HOME, refusing absolute paths and escapes."""
    if os.path.isabs(rel):
        logger.warning(abs_msg, rel)
        return None
    host_path = xiuqo_home / rel
    from tools.path_security import validate_within_dir  # resolves symlinks and ``..`` before checking

    if containment_error := validate_within_dir(host_path, xiuqo_home):
        logger.warning(traversal_msg, rel, containment_error)
        return None
    return host_path.resolve()


def register_credential_file(relative_path: str, container_base: str = "/root/.xiuqo") -> bool:
    """Register a XIUQO_HOME-relative credential file for mounting; True if it exists and was registered.

    Rejects absolute paths and traversal out of XIUQO_HOME. Containment alone is not
    enough: XIUQO_HOME holds the MASTER stores (``.env``, ``auth.json``, ``mcp-tokens/``),
    ...
    resolved = _contained_host_path(
        ...
        "credential_files: rejected absolute path %r (must be relative to XIUQO_HOME)",
    ...
    if not resolved.is_file():
        ...
        return False
    # Master credential stores are never mountable, even though they sit inside XIUQO_HOME and therefore
...
def register_credential_files(entries: list, container_base: str = "/root/.xiuqo") -> List[str]:
    ...
    try:
        ...
        for item in cred_files if isinstance(cred_files, list) else []:
            ...
            if resolved_path is not None and resolved_path.is_file():
                result.append(_mount(resolved_path, f"/root/.xiuqo/{rel}"))
...
def get_skills_directory_mount(container_base: str = "/root/.xiuqo") -> list[Dict[str, str]]:
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = _safe_skills_tempdir = Path(tempfile.mkdtemp(prefix="xiuqo-skills-safe-"))
...
def iter_skills_files(container_base: str = "/root/.xiuqo") -> List[Dict[str, str]]:
...
# (new_subpath, old_name) pairs matching xiuqo_constants.get_hermes_dir().
...
def get_cache_directory_mounts(container_base: str = "/root/.xiuqo") -> List[Dict[str, str]]:
...
def map_cache_path_to_container(host_path: str, container_base: str = "/root/.xiuqo") -> Optional[str]:
...
def from_agent_visible_cache_path(container_path: str, container_base: str = "/root/.xiuqo") -> str:
...
# Backends whose file-sync lands under the remote home: ``~/.xiuqo`` is
...
def to_agent_visible_cache_path(host_path: str, container_base: str = "/root/.xiuqo") -> str:
    ...
    ``/root/.xiuqo``; ssh/daytona/vercel_sandbox under ``~/.xiuqo``; plugin backends declare
    ...
    * docker / modal — bind-mounted (docker) or per-file-synced (modal) at ``/root/.xiuqo`` (the
    *container_base* default). * ssh / daytona / vercel_sandbox — file-synced under the remote user's home;
    ``~/.xiuqo`` is shell-expanded by the remote shell, so tool commands resolve it regardless of the
...
def iter_cache_files(container_base: str = "/root/.xiuqo") -> List[Dict[str, str]]:
