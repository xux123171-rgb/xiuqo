...
The LLM writes a Python script that calls Xiuqo tools via RPC, collapsing
...
# Loopback TCP replaces AF_UNIX on Windows, so execute_code runs on every platform Xiuqo does.
...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        import hashlib
        from xiuqo_constants import get_hermes_dir
...
def _missing_xiuqo_tools_import_hint(m, enabled_tools) -> str:
    missing = m.group(1)
    if missing in {"json_parse", "shell_quote", "retry"}:
        return (f"Import helpers with `from xiuqo_tools import {missing}`. "
                "If that import failed, the generated module may be stale or another "
                "xiuqo_tools may be first on sys.path. Check xiuqo_tools.__file__ "
...
def generate_xiuqo_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """Source of the xiuqo_tools.py stub module for SANDBOX_ALLOWED_TOOLS ∩ *enabled_tools*.
...
"""Auto-generated Xiuqo tools RPC stubs."""
...
def _connect():
    ...
    XIUQO_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["XIUQO_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("XIUQO_RPC_TOKEN", ""),
    }) + "\\n"
    ...
    # re-accepts (XIUQO_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("XIUQO_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
"""Auto-generated Xiuqo tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("XIUQO_RPC_DIR") or os.path.join(tempfile.gettempdir(), "xiuqo_rpc")
_seq = 0
...
def _run_remote_per_call(env, env_type: str, code: str, effective_task_id: str,
    ...
    """Per-call script ship: stage xiuqo_tools.py + script.py in a fresh remote sandbox dir,
    serve file-RPC from a polling thread, run, clean up."""
    sandbox_dir = f"{_env_temp_dir(env)}/xiuqo_exec_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{sandbox_dir}/xiuqo_tools.py",
                             generate_xiuqo_tools_module(list(sandbox_tools), transport="file"))
        ...
        rpc_thread.start()
        env_prefix = (f"XIUQO_RPC_DIR={quoted_rpc_dir} XIUQO_RPC_TOKEN={shlex.quote(rpc_token)} "
...
def _execute_remote(code: str, task_id: Optional[str], enabled_tools: Optional[List[str]],
    ...
    try:
        ...
        try:
            # --- Session-kernel path (xiuqo#96873) ------------------- Same always-on model as
...
def execute_code(
    ...
    with RPC access to a subset of Xiuqo tools; returns the JSON result string. "Sandbox" means
    ...
    # execute_code is a straight bypass — the terminal() path refuses `launchctl bootout ai.xiuqo.gateway`,
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        cwd_note = (
            ...
            "(os.path.expanduser('~/.xiuqo/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
            ...
            "Xiuqo's own python (the common case — stdlib plus Xiuqo's "
    ...
    description = (
        "Run Python that calls Xiuqo tools programmatically. Use when you "
        ...
        f"Available via `from xiuqo_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        f"{cwd_note}\n\n"
        "Helpers require imports: `from xiuqo_tools import json_parse, shell_quote, retry`. "
        ...
        "parameters": {
            ...
            "properties": {
                "code": {"type": "string", "description": (
                    ...
                    f"`from xiuqo_tools import {import_str}` "
