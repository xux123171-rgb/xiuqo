...
_PRIVATE_BROWSER_SENTINEL = "_XIUQO_BU_PRIVATE_BROWSER"
...
# BU_NAME + daemon pid).
_OWN_TAB_PREAMBLE = """\
# xiuqo: pin this named session to its own tab (once per daemon process)
def _hermes_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "xiuqo-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
...
def _base_subprocess_env() -> dict:
    ...
    # (Xiuqo's venv) wins over its site-packages → wrong-ABI C-extensions and a crash.
    # PYTHONPATH/PYTHONHOME inherited from the agent process point at Xiuqo's venv site-packages, and a
    ...
    # #84841, #86006, #86104). Strip both — the CLI manages its own environment and never needs Xiuqo's
    # import path.
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        with contextlib.suppress(OSError):
            ...
            stamp.touch()
        return ("Browser Use CLI not found — using the built-in browser tools. Run `xiuqo tools` "
...
def _managed_bin_dir() -> str:
    """$XIUQO_HOME/bin — where install.sh puts uv/uvx and install_cli() links browser-use."""
    return str(Path(get_hermes_home()) / "bin")


def _find_cli() -> Optional[List[str]]:
    """Locate the browser-use CLI, or None when it can't be run. MANAGED-FIRST: Xiuqo' own ``$XIUQO_HOME/bin``
    copy always wins so every session drives one Xiuqo-controlled binary; PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    the binary into ``$XIUQO_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()`` resolves it for every profile.
...
def _resolve_lightpanda_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    """Point the harness at a Xiuqo-spawned ``lightpanda serve`` (``browser.engine: lightpanda`` and
    ...
    err = _export_session_cdp(
        ...
        lambda e: (f"Lightpanda could not be started: {e} Set browser.engine to auto "
                   "to use local Chrome, or switch backends via `xiuqo tools` → Browser Automation."),
...
def _resolve_managed_chromium_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    """Point the harness at Xiuqo' packaged Chromium, launched through agent-browser for this cache key —
    ...
    if not cdp:
        return (f"The local browser could not be started: {(res or {}).get('error') or 'agent-browser returned no CDP endpoint'} "
                "Run `xiuqo tools` → Browser Automation to (re)install Chromium, or switch backends.")
    _set_cdp_env(env, cdp)
...
def _resolve_backend_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    ...
    (4) the local engine — ``browser.engine: lightpanda`` or Xiuqo' packaged Chromium via agent-browser
    ...
    err = _export_session_cdp(
        ...
        lambda e: (f"Cloud browser provider {provider_name} failed to provide a session: {e}. "
                   "Fix the provider configuration or switch backends via `xiuqo tools` → Browser Automation."),
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    profile, xiuqo_cli.browser_connect) when consented. Two ways in: the effective backend is already local
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            ...
            "description": ("Drive the user's own local browser (a Xiuqo-managed copy of their real "
