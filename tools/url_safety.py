...
(TOCTOU) is closed for Xiuqo-owned httpx paths by ``create_ssrf_safe_[async_]client()``, which
...
def normalize_url_for_request(url: str) -> str:
    """ASCII-safe HTTP URL for Xiuqo-owned URL tools (IRI -> URI, e.g. ``https://wttr.in/Köln``).
...
def _global_allow_private_urls() -> bool:
    """True when the user has opted out of private-IP blocking. Priority: ``XIUQO_ALLOW_PRIVATE_URLS``
...
def _resolve_allow_private_urls() -> bool:
    ...
    env_val = os.getenv("XIUQO_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def _install_ssrf_guard_on_client(client: Any, *, is_async: bool = False) -> None:
    ...
    import contextvars
    var_name = "xiuqo_ssrf_async_origin_schemes" if is_async else "xiuqo_ssrf_origin_schemes"
...
def ssrf_safe_async_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("xiuqo_ssrf_async_origin_schemes")
...
def ssrf_safe_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("xiuqo_ssrf_origin_schemes")
