...
is ALWAYS host-side ``$XIUQO_HOME/cache/spillover/{id}.txt`` (works for sessions that never
...
def get_spillover_dir():
    """Return $XIUQO_HOME/cache/spillover as a Path (not created)."""
...
def _write_to_spillover(content: str, filename: str):
    """Write host-side to $XIUQO_HOME/cache/spillover; returns path str or None."""
    try:
...
def _resolve_storage_dir(env) -> str:
    ...
    return f"{temp_dir.rstrip('/') or '/'}/xiuqo-results" if temp_dir else STORAGE_DIR
...
HEREDOC_MARKER = "XIUQO_PERSIST_EOF"
# ---- END PLUGIN-COMPAT ----
