...
# Mirrors RELAY_DELIVER_TIMEOUT_MS in apps/desktop/src/plugins/xiuqo-bots/relay.ts; both test suites pin it.
...
# One turn in a profile's canonical Bot Chat: ``xiuqo -p <profile> *BOT_CHAT_TURN_ARGS``.
...
def _bot_mode_cfg(key: str, *, loader: str) -> Any:
    ...
    try:
        import xiuqo_cli.config as cfgmod
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    handle = str(row.get("handle") or "").strip().lstrip("@") or ("xiuqo" if profile == "default" else profile)
...
def _xiuqo_cli() -> str:
    """xiuqo CLI beside this interpreter, then ``shutil.which``, then the bare name
    (service contexts lack PATH, so a bare "xiuqo" died with ENOENT).

    The deliver RPC runs on the target gateway, whose process is the venv python — its bin/Scripts directory
    holds the matching ``xiuqo`` entrypoint. A bare ``"xiuqo"`` relies on PATH, which is exactly what
    ...
    """
    sibling = Path(sys.executable or "").parent / ("xiuqo.exe" if sys.platform == "win32" else "xiuqo")
    return str(sibling) if sibling.is_file() else shutil.which("xiuqo") or "xiuqo"
...
def local_delivery_command(profile: str, query_file: str) -> list[str]:
    ...
    return [_xiuqo_cli(), "-p", profile, *BOT_CHAT_TURN_ARGS, "--query-file", query_file]
...
def delivery_env(author: Optional[dict]) -> dict[str, str]:
    """Environment for one delivery turn's ``xiuqo`` child. The dispatcher's own XIUQO_TURN_AUTHOR is
...
# Deliveries are separate ``xiuqo`` subprocesses, so the lock is a per-profile
...
# target profile must never run their Bot Chat turns concurrently: deliveries spawn separate ``xiuqo``
