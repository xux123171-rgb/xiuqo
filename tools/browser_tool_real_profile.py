...
xiuqo-owned copy, launch the real browser binary on it, and attach agent-browser.
...
def _real_profile_daemon_env() -> dict:
    ...
    Chrome is launched by Xiuqo, not the daemon, so a self-exiting daemon would leave Chrome
...
def _real_profile_unsupported_reason(browser) -> Optional[str]:
    ...
    """
    from xiuqo_cli.browser_connect import UNSUPPORTED_CHANNEL
    if browser is None:
...
def _real_profile_snapshot_error(err: str) -> str:
    ...
    from xiuqo_cli.browser_connect import _PROFILE_LOCKED_PREFIX
    if err and err.startswith(_PROFILE_LOCKED_PREFIX):
        return (err[len(_PROFILE_LOCKED_PREFIX):] + " To close it (only after the user approves — it "
                "quits their browser and loses unsaved tabs), run: `xiuqo browser close-profile`, then retry.")
    return f"{_RP}{err}"
...
def _real_profile_cdp() -> tuple:
    ...
    if not _cloud._use_real_profile():
        ...
        try:
            from xiuqo_cli.browser_connect import cleanup_real_profile_snapshots
    ...
    from xiuqo_cli.browser_connect import (chromium_executable, detect_default_chromium,
    ...
    with _bt._real_profile_cdp_lock:
        ...
        # xiuqo process) holds the user-data-dir open — that corrupts the databases.
        ...
        # A Chrome from an earlier xiuqo process can still hold the copy dir after its attach
