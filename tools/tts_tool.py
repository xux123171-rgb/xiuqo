...
def _default_output_dir() -> str:
    ...
    XIUQO_HOME and later scope requests to a different profile via
    ``xiuqo_constants.set_hermes_home_override()`` — a frozen module constant keeps writing synthesized
    ...
    re-resolve from the live profile-scoped XIUQO_HOME on every call.
    """
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("xiuqo_cli.config not available, using default TTS config")
...
_BUILTIN_DISPATCH: Dict[str, tuple] = {
    ...
    "mistral": (lambda: _importable(_import_mistral_client), "Mistral Voxtral TTS", "_generate_mistral_tts",
                ...
                "Run `xiuqo setup` to install Mistral support."),
    ...
    "neutts": (lambda: _check_neutts_available(), "NeuTTS (local)", "_generate_neutts",
               ...
               "Run xiuqo setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."),
    "kittentts": (lambda: _importable(_import_kittentts), "KittenTTS (local, ~25MB)", "_generate_kittentts",
                  ...
                  "Run 'xiuqo setup tts' and choose KittenTTS, or install manually: "
    ...
    "piper": (lambda: _importable(_import_piper), "Piper (local)", "_generate_piper_tts",
              ...
              "Run 'xiuqo tools' and select Piper under TTS, or install manually: "
...
def _session_platform() -> tuple:
    ...
    platform = get_session_env("XIUQO_SESSION_PLATFORM", "").lower()
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": _output_path_description("the profile XIUQO_HOME")
            },
            "speed": {
                "type": "number",
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'xiuqo_xai_user_agent': ('tools.xai_http', 'xiuqo_xai_user_agent'),
