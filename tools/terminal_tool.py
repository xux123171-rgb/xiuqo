...
processes survive cleanup, idle reaping, or Xiuqo exit.
...
# display_hermes_home imported lazily at call site (stale-module safety during xiuqo update)
...
def _current_session_key() -> str:
    ...
    return get_session_env("XIUQO_SESSION_KEY", "")


def _current_session_profile() -> str:
    """Active session's Xiuqo profile name, or "" (same lookup discipline as
    :func:`_current_session_key`)."""
    from gateway.session_context import get_session_env

    return get_session_env("XIUQO_SESSION_PROFILE", "")
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps Exited containers labeled ``xiuqo=1`` for the current
    profile — leftovers of Xiuqo processes that died without firing
    ...
    Xiuqo processes sharing a profile) and the once-per-interpreter flag so
...
def _ensure_terminal_env_bridged() -> None:
    ...
    at startup; processes that skip those paths (``xiuqo serve``, Desktop
    ...
    env values (possibly stale from ``xiuqo setup``); env values for omitted keys
    ...
    context-local ``XIUQO_HOME`` override (multiplexed dashboard / gateway
    ...
    with _quiet("terminal config → env fallback bridge failed"):
        from xiuqo_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _resolve_config_cwd(env_type: str, mount_docker_cwd: bool) -> tuple:
    ...
    from xiuqo_cli.config import _is_ssh_remote_tilde_cwd
...
def terminal_tool(
    ...
    ``_host_local`` forces the local backend for Xiuqo-owned control-plane
