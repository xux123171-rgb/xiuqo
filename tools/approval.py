...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("XIUQO_YOLO_MODE", ""))
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode derives from Xiuqo YOLO. Lazy import so approval-only
...
def _permanent_set() -> set:
    ...
    profile (XIUQO_HOME override) → its own set, lazily loaded from ITS ``command_allowlist``: the
...
def _baseline_key() -> str:
    from xiuqo_constants import get_hermes_home_override, xiuqo_home_key
    return "" if get_hermes_home_override() is None else xiuqo_home_key()
...
def _unattended_contexts() -> list[_Unattended]:
    """Active unattended contexts in evaluation order: single-query first (``xiuqo chat -q``
    exports XIUQO_INTERACTIVE=1 but nobody answers); cron beats a platform marker because
...
def _presence(approval_callback=None) -> tuple:
    ...
    (-q) exports XIUQO_INTERACTIVE=1 but nobody answers prompts, and XIUQO_EXEC_ASK has no
    ...
    is_ask = env_var_enabled("XIUQO_EXEC_ASK")
    if _is_single_query_approval_context():
        ...
        for ctx in _unattended_contexts():
            ...
            if fail_closed_when_no_human:
                logger.warning("%s (pattern: %s): %s — no interactive user/gateway present; "
                               "BLOCKED (fail-closed). Set XIUQO_INTERACTIVE or "
                               "XIUQO_GATEWAY_SESSION to answer the prompt.", *log_args)
                return _blocked(no_human_block_message or (
                    ...
                    pattern_key=pattern_key, description=description)
        logger.warning("%s (pattern: %s): %s — set XIUQO_INTERACTIVE or "
                       "XIUQO_GATEWAY_SESSION to require approval.", *log_args)
        return _approved()
