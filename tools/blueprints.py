...
``metadata.xiuqo.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.xiuqo.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.xiuqo.blueprint`` is a mapping with a
    ...
    xiuqo = meta.get("xiuqo") if isinstance(meta, dict) else None
    blueprint = xiuqo.get("blueprint") if isinstance(xiuqo, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.xiuqo.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.xiuqo.blueprint`` block) ready for ``xiuqo skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"xiuqo": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
