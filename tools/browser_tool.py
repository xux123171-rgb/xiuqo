...
from xiuqo_cli.config import DEFAULT_CONFIG, cfg_get
...
# dependency could read every Xiuqo secret from process.env.
...
def _build_browser_env() -> dict:
    ...
    env = xiuqo_subprocess_env(inherit_credentials=False)
...
# than a concrete path (also compared in xiuqo_cli/tools_config.py and doctor.py).
...
# The config-derived ones are keyed by profile home (``xiuqo_home_key()``): the multiplexed
...
_REAL_PROFILE_SESSION = "xiuqo-real-profile"
...
# Owner Xiuqo home per session: the janitor is one process-global thread, so each
...
def _url_policy_error(url: str, *, auto_local: bool = False) -> Optional[dict]:
    ...
    URL protects nothing. Xiuqo' own secrets leaking into a URL are caught by ``_secret_url_error``."""
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        if not cfg_get(read_raw_config(), "browser", "record_sessions", default=False):
            return
        recordings_dir = xiuqo_home / "browser_recordings"
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_hermes_home_override': ('xiuqo_constants', 'get_hermes_home_override'),
    'xiuqo_home_key': ('xiuqo_constants', 'xiuqo_home_key'),
    ...
    'node_tool_runnable': ('xiuqo_constants', 'node_tool_runnable'),
