...
def _home_candidates() -> list:
    ...
    try:
        ...
        logger.debug("xiuqo home unavailable for lightpanda lookup: %s", e)
    return candidates
...
def find_lightpanda_binary() -> Optional[str]:
    ...
    fallbacks), then installer/agent-browser locations, then ``$XIUQO_HOME/bin``. No Windows build."""
    if os.name == "nt":
...
def _http_cache_dir() -> Path:
    """Filesystem HTTP cache shared by every Lightpanda this Xiuqo spawns.
    ...
    bounds it without Xiuqo managing eviction.
    """
...
def _cdp_ready(url: str, timeout: float = 0.2) -> bool:
    try:
        from xiuqo_cli.browser_connect import is_browser_debug_ready
    ...
    if not binary:
        if os.name == "nt":
            return None, ("browser.engine is 'lightpanda' but Lightpanda has no Windows "
                          "build. Set browser.engine to auto (or run Xiuqo under WSL2).")
...
def reap_orphaned_lightpanda() -> int:
    """Kill ``lightpanda serve`` processes whose owning Xiuqo is gone; return the count. A live owner is
