...
# `cronjob(action="run")` executes in-process (comfortably below XIUQO_AGENT_TIMEOUT).
# Mirrors the 10s cadence of tools/environments/base.py::touch_activity_if_due (delegate_task's heartbeat
# uses 30s) — comfortably below the 1800s default XIUQO_AGENT_TIMEOUT. See #76502.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0
# Hard ceiling: with XIUQO_CRON_TIMEOUT=0 a truly hung run would otherwise mask the
# gateway watchdog forever; past this the heartbeat stops and the watchdog regains authority.
# The child cron run has its own inactivity watchdog (XIUQO_CRON_TIMEOUT, default 600s) that bounds a
# wedged job, but with XIUQO_CRON_TIMEOUT=0 (explicit "unlimited") a truly hung run_one_job would otherwise
...
def _reap_stale_executions(job_name: str) -> None:
    ...
    one-shot `xiuqo cron run` that died mid-run). The ticker does this at startup; one-shot
    ...
    try:
        ...
        # process -- e.g. a PRIOR one-shot `xiuqo cron run` invocation whose dispatched runner died with
...
def _try_dispatch_background_run(
    ...
    # completion, so don't claim-and-dispatch (direct callers like `xiuqo cron run` exit right after).
    ...
    try:
        ...
        origin_ui_session_id = get_session_env("XIUQO_UI_SESSION_ID", "") or ""
    except Exception:
        pass
...
Jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered — cron runs are autonomous and cannot ask questions. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": _script_description("the profile XIUQO_HOME")
            },
            "monitor": {
                "type": "string",
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("XIUQO_INTERACTIVE")
        or env_var_enabled("XIUQO_GATEWAY_SESSION")
        or env_var_enabled("XIUQO_EXEC_ASK")
    )


# Agent-facing arguments forwarded verbatim to cronjob(). model / provider / base_url are
# intentionally NOT here: per-job inference pins are user-owned (dashboard, `xiuqo cron
