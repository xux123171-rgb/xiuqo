...
``~/.xiuqo/.env`` (Docker: see ``CAMOFOX_REWRITE_LOOPBACK_URLS`` below).
"""
...
from xiuqo_cli.config import cfg_get, load_config, read_raw_config
...
def _config_cdp_url() -> str:
    ...
    try:
        from xiuqo_cli.config import read_raw_config  # late-bound: tests patch the source module
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    share a user ID so Xiuqo uses the same profile), or None."""
...
def _save_screenshot(content: bytes) -> str:
    """Write PNG bytes under ``$XIUQO_HOME/browser_screenshots`` and return the path."""
