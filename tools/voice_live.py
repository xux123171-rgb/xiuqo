"""GPT-Live voice chat mode: the full-duplex voice frontend that delegates to Xiuqo.
...
delegates every real request to Xiuqo as its *client-delegation* backend. Xiuqo stays the
...
* the renderer turns each ``session.delegation.created`` into a normal ``prompt.submit`` on the
  ...
  ``session.commentary.append`` — Xiuqo' answer is what the voice speaks.
...
# backend (Xiuqo) carries the real instructions, tools and memory.
LIVE_PERSONA = (
    "You are Xiuqo, a calm and friendly voice assistant. Speak naturally at an unhurried pace. "
    ...
    "Delegation policy:\n"
    "Backend tools:\n"
    "- Xiuqo agent: a full AI agent with tools — it can run commands, read and edit files, "
