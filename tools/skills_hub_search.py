"""Skills Hub discovery: the centralized Xiuqo index fetch (cached, stale-
...
XIUQO_INDEX_URL = "https://hermes-agent.nousresearch.com/docs/api/skills-index.json"
XIUQO_INDEX_TTL = 6 * 3600  # 6 hours


def _hermes_index_cache_file() -> Path:
    from tools.skills_hub import _index_cache_dir
    return _index_cache_dir() / "xiuqo-index.json"


def _load_hermes_index() -> Optional[dict]:
    """Fetch the centralized skills index (docs site, rebuilt daily), cached
    locally for XIUQO_INDEX_TTL; on any failure serve the stale cache.
    ...
    cached = _read_json_if_fresh(cache_file, XIUQO_INDEX_TTL)
    if cached is not None:
        return cached
    data = None
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            resp = httpx.get(XIUQO_INDEX_URL, timeout=15, follow_redirects=True,
                             headers={"Accept-Encoding": accept_encoding})
            if resp.status_code != 200:
                logger.debug("Xiuqo index fetch returned %d", resp.status_code)
                return _load_stale_index_cache()
            data = resp.json()
            break
        except httpx.DecodingError as e:
            logger.debug("Xiuqo index decode failed (Accept-Encoding=%s): %s", accept_encoding, e)
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Xiuqo index fetch failed: %s", e)
            return _load_stale_index_cache()
...
def _select_active_sources(sources: List[SkillSource], source_filter: str) -> List[SkillSource]:
    ...
    index_available = effective == "all" and any(
        src.source_id() == "xiuqo-index" and getattr(src, "is_available", False) for src in sources
    )
