...
class MCPServerHealthMixin:
    ...
    def _make_logging_callback(self):
        """``logging_callback`` forwarding server ``notifications/message`` into Xiuqo logging (SDK default drops them).

        Routes MCP ``notifications/message`` log notifications from the server into Xiuqo' logging
        (agent.log via xiuqo_logging), tagged with the server name. Without this, the SDK's default
