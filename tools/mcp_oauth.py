...
listener and ``build_oauth_auth()`` (legacy entry point). client_id is Xiuqo' Client ID Metadata
...
from xiuqo_constants import secure_parent_dir
...
def _get_token_dir(xiuqo_home: str | Path | None = None) -> Path:
    """``XIUQO_HOME/mcp-tokens/`` — per-profile token directory."""
    from xiuqo_constants import get_hermes_home

    return Path(xiuqo_home if xiuqo_home is not None else get_hermes_home()) / "mcp-tokens"
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    ``lead`` is the boundary-specific first sentence; this helper appends the shared, actionable ``xiuqo
    mcp login`` next-step so the guidance wording lives in one place across every non-interactive OAuth
    boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} Run `xiuqo mcp login <server>` interactively to (re)authorize, then restart or reload the gateway."
        )
...
class HermesTokenStorage:
    """Persist OAuth state as ``XIUQO_HOME/mcp-tokens/<server_name>`` + ``.json`` (tokens),
    ``.client.json`` (client info), ``.meta.json`` (server metadata), ``.cimd-off`` (CIMD refused)."""

    def __init__(self, server_name: str, *, xiuqo_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._hermes_home = Path(xiuqo_home) if xiuqo_home is not None else None
...
def _make_callback_handler() -> tuple[type, dict]:
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = ("<h2>Authorization Successful</h2><p>You can close this tab and return to Xiuqo.</p>" if parsed["code"]
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `xiuqo mcp login <server>` later to authenticate, "
...
def _callback_outcome(result: dict, cimd_url: str | None):
    ...
    if result["auth_code"] is None:
        hint = (
            ...
            f"server rejected Xiuqo' Client ID Metadata Document ({cimd_url}); set ``cimd: false`` under that "
...
def remove_oauth_tokens(server_name: str, *, xiuqo_home: str | Path | None = None) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    HermesTokenStorage(server_name, xiuqo_home=xiuqo_home).remove()
    logger.info("OAuth tokens removed for '%s'", server_name)


# CIMD (OAuth Client ID Metadata Documents): the client_id IS an HTTPS URL the server fetches for our
# name/logo/redirect URIs, replacing per-install DCR. The SDK does the protocol; Xiuqo only decides
...
_CIMD_CLIENT_METADATA_URL = "https://nousresearch.github.io/xiuqo/docs/oauth/client-metadata.json"
...
def _server_declined_cimd(storage: "HermesTokenStorage | None") -> bool:
    ...
    in its 401 branch — after Xiuqo must fix the redirect URI — so cached metadata closes the gap;
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    metadata_kwargs: dict[str, Any] = {
        "client_name": cfg.get("client_name", "Xiuqo Agent"),
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "the previous client. Re-authorize with: xiuqo mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    when the user overrode it or an older Xiuqo is running."""
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(server_name: str, server_url: str, oauth_config: dict | None = None) -> "OAuthClientProvider | None":
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"requires browser authorization. Run `xiuqo mcp login {server_name}` interactively first to complete "
    ...
    if HermesOAuthClientProvider is None:
        ...
        HermesOAuthClientProvider = type("HermesOAuthClientProvider", (HermesProviderMixin, _sdk_class("OAuthClientProvider")), {
            "__doc__": "SDK provider plus Xiuqo' token-endpoint fixes (see ``HermesProviderMixin``).",
