...
_XIUQO_CONFIG_FILES = r'\.xiuqo/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.xiuqo/\.env|\~/\.xiuqo/\.env',
     "xiuqo_env_access", "critical", "exfiltration", "directly references Xiuqo secrets file"),
    ...
    # quarantine only acts on "dangerous") but high for Xiuqo/other config (setup docs routinely say
    ...
    (_prose_modify_re(_XIUQO_CONFIG_FILES),
     "xiuqo_config_mod", "high", "persistence", "modification language aimed at Xiuqo configuration files (verify intent)"),
    (_shell_write_re(_XIUQO_CONFIG_FILES),
     "xiuqo_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Xiuqo configuration files"),
    (r'\.xiuqo/config\.yaml|\.xiuqo/SOUL\.md',
     "xiuqo_config_ref", "low", "persistence", "references Xiuqo configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Xiuqo-native; `.clawhubignore` is honored for skills published through ClawHub.
