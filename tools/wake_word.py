"""Wake-word ("Hey Xiuqo") detection — hands-free session trigger.
...
# Minimum gap between two wake fires, so one "hey xiuqo" can't retrigger across
...
_DEFAULTS: Dict[str, Any] = {
    ...
    "provider": "openwakeword", "phrase": "hey xiuqo", "sensitivity": 0.6,
    "confirmation_frames": _DEFAULT_CONFIRMATION_FRAMES, "start_new_session": True,
}

# Bundled "hey xiuqo" model (tools/wakewords/) — the default; alias names resolve
# to it, not to an openWakeWord built-in.
_BUNDLED_MODEL_NAME = "hey_hermes"
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_hermes", "hey xiuqo", "xiuqo"})
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey xiuqo"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    with suppress(Exception):
        ...
        from xiuqo_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return ("Microphone delivers only silence. Grant the Xiuqo backend "
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    ladder = (
        ...
        (bool(missing), lambda: (f"Wake word needs {missing} configured — run `xiuqo tools` "
...
# ── Process-wide singleton (mirrors xiuqo_cli.voice's continuous API) ──
