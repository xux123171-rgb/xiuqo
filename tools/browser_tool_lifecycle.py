...
def _emergency_cleanup_all_sessions():
    ...
    xiuqo processes — every clean exit reaps accumulated orphans, not only
    ...
    # owned by other live xiuqo processes.
    _best_effort("Orphan reap on exit", _reap_orphaned_browser_sessions)


@contextlib.contextmanager
def _session_owner_scope(task_id: str):
    """Run under the Xiuqo home + secret scope owning ``task_id``'s session (no-op if unrecorded).
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record this xiuqo PID in ``<socket_dir>/<session>.owner_pid`` so the orphan
...
def _reap_socket_dir(socket_dir: str, session_name: str, tracked_names: set) -> bool:
    ...
    A live ``owner_pid`` means another xiuqo process owns it — leave it UNLESS untracked
...
def _reap_orphaned_browser_sessions():
    """Kill agent-browser daemons whose owning xiuqo process is gone (an unclean exit loses
...
def _update_session_activity(task_id: str):
    """Touch the activity timestamp and record the owning Xiuqo home on first sight (the
