...
One append-only log per child under ``<xiuqo_home>/cache/delegation/live/
...
def live_transcript_root() -> Path:
    """Root directory for live transcripts (profile-safe, never ~/.xiuqo)."""
...
class LiveTranscriptWriter:
    ...
    def __init__(self, delegation_id: str, task_index: int, goal: str,
        ...
        with _best_effort(f"init ({delegation_id} task {task_index})"):
            ...
            path.write_text(
                "=== Xiuqo subagent live transcript ===\n"
