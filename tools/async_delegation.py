...
def _connect() -> sqlite3.Connection:
    from xiuqo_cli.sqlite_util import open_db
    # Same state.db as xiuqo_state.SessionDB -- reuse its owner-only (0600)
    # hardening so this writer doesn't create/leave the file (and its WAL
    # sidecars) at the process umask. See xiuqo_state._secure_state_db_files.
    from xiuqo_state import _secure_state_db_files
...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    from xiuqo_state_repair import apply_durability_barriers
    from xiuqo_state_schema import reconcile_state_schema
...
def _transaction():
    from xiuqo_cli.sqlite_util import transaction
...
def _capture_routing_origin() -> Dict[str, Any]:
    ...
    try:
        ...
        return {k: v for k in _ROUTING_KEYS if (v := get_session_env(f"XIUQO_SESSION_{k.upper()}", ""))}
...
def _current_origin_session_id() -> str:
    """Raw session id of the ORIGINATING api_server request, or ``""``. ``XIUQO_SESSION_ID``
    ...
    request-scoped ``XIUQO_SESSION_CHAT_ID`` (raw X-Xiuqo-Session-Id on api_server) survives
    child construction; on push platforms chat_id is a chat, not a session => ``""``."""
    try:
        from gateway.session_context import get_session_env
        is_api = get_session_env("XIUQO_SESSION_PLATFORM", "") == "api_server"
        return (get_session_env("XIUQO_SESSION_CHAT_ID", "") or "") if is_api else ""
    except Exception:
        return ""
