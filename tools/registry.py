"""Central registry for all xiuqo tools: each tool file calls ``registry.register()``
...
# timescales, so results are cached ~30 s: env-var flips via ``xiuqo tools`` still land
...
_BROWSER_IDENTITY_KEYS = (
    "XIUQO_SESSION_ID",
    "XIUQO_BROWSER_CONTROL_PRINCIPAL",
    "XIUQO_BROWSER_CONTROL_TRANSPORT_FAMILY")
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    processes keep the process-wide cache; a multiplex gateway installs a Xiuqo-home override
...
def invalidate_check_fn_cache() -> None:
    """Drop all cached ``check_fn`` results (after config changes like ``xiuqo tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin overlays keyed by resolved XIUQO_HOME; a profile sees its overlay first.
    ...
    def _plugin_namespace_of_module(self, module_namespace: str) -> Optional[str]:
        ...
        if module_namespace.startswith("xiuqo_plugins."):
    ...
    def deregister(self, name: str, *, scope: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # Ownership binds to the plugin package root (``xiuqo_plugins.{name}``), not
                # the exact module: a submodule's handler is still the package's to remove.
                # A handler defined in ``xiuqo_plugins.pkg.handlers`` is still owned by the
                # ``xiuqo_plugins.pkg`` package — exact string equality would wrongly block root-module
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        absent). Probes use the ~30 s TTL cache so ``xiuqo tools enable`` lands quickly."""
        result = []
