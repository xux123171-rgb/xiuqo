...
_NEEDS_REAUTH_MSG = (
    "MCP server '{s}' requires re-authentication. Run `xiuqo mcp login {s}` (or delete the tokens file under "
    "~/.xiuqo/mcp-tokens/ and restart). Do NOT retry this tool — ask the user to re-authenticate.")
...
_STDIO_OUTCOME_UNCERTAIN_MSG = (
    ...
    "Xiuqo did not replay it. Do NOT retry automatically; inspect the external state first.")
...
def _render_content_blocks(result, server_name: str) -> Tuple[str, int]:
    ...
    # cache those via the gateway's image-cache helper so they flow through Xiuqo' MEDIA: tag convention
    ...
    # (gnanirahulnutakki), both too stale to cherry-pick. #10848's approach (integrate with Xiuqo' MEDIA
