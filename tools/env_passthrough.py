...
# Config-based allowlist, keyed by Xiuqo home: under gateway.multiplex_profiles one process serves
...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a Xiuqo-managed provider credential per
    ``_XIUQO_PROVIDER_ENV_BLOCKLIST`` or a dynamic Xiuqo-internal secret
    ...
    in the ``execute_code`` child); non-Xiuqo keys (TENOR_API_KEY, …) stay
    registerable. Fails closed when the blocklist cannot be imported."""
    try:
        from tools.environments.local_env_policy import (
            _XIUQO_PROVIDER_ENV_BLOCKLIST, _is_hermes_internal_secret)
        ...
        return True
    return _is_hermes_internal_secret(name) or name in _XIUQO_PROVIDER_ENV_BLOCKLIST


def register_env_passthrough(var_names: Iterable[str]) -> None:
    """Register env var names as allowed in sandboxed environments (typically a
    skill's ``required_environment_variables``). Xiuqo-managed provider credentials
    ...
    for name in _accepted((n.strip() for n in var_names), (
        "env passthrough: refusing to register Xiuqo provider "
        "credential %r (blocked by _XIUQO_PROVIDER_ENV_BLOCKLIST). "
...
def _accepted(names, refusal_msg: str):
    """Yield non-empty *names* that are not Xiuqo provider credentials; refused
...
def _load_config_passthrough() -> frozenset[str]:
    ...
    try:
        ...
        result.update(_accepted((i.strip() for i in items if isinstance(i, str)), (
            "env passthrough: refusing to register Xiuqo "
            "provider credential %r from config.yaml (blocked "
            "by _XIUQO_PROVIDER_ENV_BLOCKLIST). Operator "
