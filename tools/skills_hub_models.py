...
def _hermes_tags(fm: dict) -> Any:
    """``metadata.xiuqo.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    xiuqo_meta = metadata.get("xiuqo", {}) if isinstance(metadata, dict) else None
    return xiuqo_meta.get("tags", []) if isinstance(xiuqo_meta, dict) else []
