    ...
    if supervisor is None:
        return {
            ...
            "error": (
                ...
                "secrets. Start the browser through the Xiuqo-managed "
                "session and retry."
            ),
        }
    ...
    if not can_prompt_here():
        return json.dumps({"success": False, "error_type": "unlock_unavailable",
                           "error": (f"{backend.display_name} is locked and this session cannot prompt for the "
                                     "master password (headless/cron/API). Unlock it from an interactive Xiuqo "
    ...
    if prompt is None or not can_prompt_here():
        return json.dumps({"success": False, "error_type": "prompt_unavailable",
                           "error": (f"This session cannot ask the user for a login (headless/cron/API). Tell them to run "
                                     f"`xiuqo vault add` or use Desktop → Settings → Passwords & Logins for {origin}.")})
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    "To save a credential: run `xiuqo vault add` in a terminal, or "
...
BROWSER_VAULT_LIST_SCHEMA = {
    ...
    "description": (
        ...
        "Secret values are NEVER returned. Sources: the local Xiuqo vault plus any installed password manager "
...
BROWSER_VAULT_SAVE_LOGIN_SCHEMA = {
    ...
    "description": (
        ...
        "through a masked prompt in their UI, to save the login for this site. Xiuqo stores it encrypted, "
...
BROWSER_VAULT_ENTER_CODE_SCHEMA = {
    ...
    "parameters": {
        "type": "object",
        "properties": {"handle": {"type": "string", "description": "The login handle you just filled (lets Xiuqo generate the code when an authenticator key is saved)."}},
        "required": [],
    },
}
