...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    xiuqo_meta = meta.get("xiuqo") if isinstance(meta, dict) else None
    if not isinstance(xiuqo_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.xiuqo.{tags, related_skills}.")
    elif "tags" not in xiuqo_meta:
        yield _warn("missing-metadata", "metadata.xiuqo.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("xiuqo", "agent", "xiuqo agent") and (
        author != "Xiuqo Agent"):
        yield _warn("author-caps", f"author '{author}' should be 'Xiuqo Agent' (proper caps) "
