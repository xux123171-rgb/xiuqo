...
# ladders, parked-server self-probes and repeated `xiuqo mcp test` runs re-run the preflight
...
# The cache is also persisted under the Xiuqo home so separate processes and gateway
...
def _disk_cache_path() -> Optional[Path]:
    ...
    Uses ``xiuqo_constants.get_hermes_home()`` so the cache follows the
    active profile and is isolated across Xiuqo homes. The cache directory
    is created on demand. Returns ``None`` when Xiuqo home cannot be
...
def _query_osv(package: str, ecosystem: str, version: Optional[str] = None) -> list:
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "xiuqo-osv-check/1.0"},
        method="POST")
