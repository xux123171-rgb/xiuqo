...
def _get_hermes_config_resolved() -> str | None:
    """Resolved absolute path of the Xiuqo config file for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``XIUQO_HOME`` scope (#107327);
    ...
    try:
        ...
        # home rather than the active multiplex ``XIUQO_HOME`` — comparing
...
def _get_real_hermes_home() -> str | None:
    """Realpath of the authoritative Xiuqo home for the ACTIVE profile.
    ...
    try:
        ...
        # rather than ``_expand_tilde("~/.xiuqo")`` so the protected-instruction
        ...
        # the consumer: the ``~/.xiuqo`` exemption is skipped, so the gate runs.
        try:
            from xiuqo_constants import get_hermes_home
            return os.path.realpath(str(get_hermes_home()))
        except Exception:
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    xiuqo_config = _get_hermes_config_resolved()
    if xiuqo_config and xiuqo_config in candidates:
        return (
            f"Refusing to write to Xiuqo config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.xiuqo/config.yaml directly or use 'xiuqo config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .xiuqo tree).
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    try:
        from xiuqo_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.xiuqo itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.xiuqo`` component rule, which would match the home.
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .xiuqo config dirs (<repo>/.xiuqo/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.xiuqo.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".xiuqo":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Xiuqo mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.xiuqo" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    Xiuqo state (a write the host never reads). Not profile isolation — that guard was
