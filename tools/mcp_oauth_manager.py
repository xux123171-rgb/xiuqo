...
class HermesMCPOAuthProvider(HermesProviderMixin, *_SDK_BASES):
    ...
    async def _prefetch_oauth_metadata(self) -> None:
        ...
        # The SDK's httpx flavour, not Xiuqo': `create_oauth_metadata_request` returns *its* (httpx2) Request objects.
    ...
    async def _maybe_flag_poisoned_client(self, response: Any) -> None:
        ...
        is swallowed. The browser-side "Redirect URI Mismatch" case has no HTTP signal (``xiuqo mcp reauth``).
        ...
        try:
            ...
            # back into the same refusal (`xiuqo mcp login` clears the marker).
    ...
    async def async_auth_flow(self, request):  # type: ignore[override]
        try:  # pre-flow hook: reload from disk if it changed (non-fatal on error)
            await get_manager().invalidate_if_disk_changed(self._hermes_server_name, xiuqo_home=self._hermes_home)
...
_XIUQO_PROVIDER_CLS: Optional[type] = HermesMCPOAuthProvider if _SDK_BASES else None
...
class MCPOAuthManager:
    ...
    @staticmethod
    def _key(server_name: str, xiuqo_home: str | Path | None = None) -> tuple[str, str]:
    ...
    def _build_provider(self, server_name: str, entry: _ProviderEntry) -> Optional[Any]:
        ...
        if _XIUQO_PROVIDER_CLS is None:
        ...
        if get_dashboard_oauth_flow() is None and not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                f"Run `xiuqo mcp login {server_name}` interactively first to complete initial authorization.")
        return _XIUQO_PROVIDER_CLS(
    ...
    def remove(self, server_name: str, *, xiuqo_home: str | Path | None = None) -> _ProviderEntry | None:
        """Evict the provider from cache AND delete tokens from disk (``xiuqo mcp remove`` / forced re-auth)."""
        entry = self.evict(server_name, xiuqo_home=xiuqo_home)
        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, xiuqo_home=xiuqo_home)
        logger.info("MCP OAuth '%s': evicted from cache and removed from disk", server_name)
        return entry

    def restore_entry(self, server_name: str, entry: _ProviderEntry | None, *, xiuqo_home: str | Path | None = None) -> None:
        """Restore a provider entry removed for a failed reauthorization."""
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, xiuqo_home), entry)

    def evict(self, server_name: str, *, xiuqo_home: str | Path | None = None) -> _ProviderEntry | None:
        """Drop only the in-process provider, preserving persisted OAuth state."""
        with self._entries_lock:
            return self._entries.pop(self._key(server_name, xiuqo_home), None)

    async def invalidate_if_disk_changed(self, server_name: str, *, xiuqo_home: str | Path | None = None) -> bool:
        """Force the SDK provider to reload when the tokens file mtime changed (e.g. a cron refresh); True if so."""
        from tools.mcp_oauth import _get_token_dir, _safe_filename
        entry = self._entries.get(self._key(server_name, xiuqo_home))
        if entry is None or entry.provider is None:
            return False
        async with entry.lock:
            try:
                mtime_ns = (_get_token_dir(xiuqo_home) / f"{_safe_filename(server_name)}.json").stat().st_mtime_ns
            except OSError:
                return False
