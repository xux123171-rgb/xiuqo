...
``~/.xiuqo/skills/.curator_ledger.jsonl`` with before/after file manifests whose
contents are stored content-addressed (sha256-deduped) under
``~/.xiuqo/.curator_backups/blobs/``. JSONL, not the state DB: durable, greppable,
...
def _validate_entry_paths(entry: Dict[str, Any]) -> Optional[str]:
    """Every entry path must be under XIUQO_HOME — a hand-edited ledger must not
