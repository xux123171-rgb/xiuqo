...
under ``XIUQO_HOME`` and new sandboxes are restored from them on task reuse.
"""
...
def _ensure_vercel_sdk() -> None:
    ...
    # The SDK (>=0.7) ships default-on telemetry; Xiuqo policy is opt-in only, so disable it
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _remote_hermes_dir(self) -> str:
        return f"{self._remote_home.rstrip('/')}/.xiuqo"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        ...
        remote_tar = f"/tmp/.xiuqo_sync.{os.getpid()}.tar"
        sandbox = self._require_sandbox()
        try:
