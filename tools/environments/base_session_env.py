...
# backend serves many sessions, so a snapshot carrying the FIRST session's XIUQO_SESSION_ID
# would make every LATER session source a foreign identity. Every bridged name starts with
# one of these prefixes (or is XIUQO_UI_SESSION_ID); unit tests use this regex as the
...
# "default" environment), so ``export -p`` dumping the FIRST session's XIUQO_SESSION_ID into the snapshot
# makes every LATER session ``source`` that stale value and see a FOREIGN session's identity — overriding
# the correct per-command Popen env (issue: cross-session XIUQO_SESSION_ID leak via the shared snapshot).
# Stripping them from the snapshot is safe because they are re-injected on every command; a snapshot should
# only carry the user's own shell state (PATH, functions, exports they set), not Xiuqo' per-turn session
# identity. Used by unit tests as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (XIUQO_SESSION_|XIUQO_UI_SESSION_ID|XIUQO_CRON_AUTO_DELIVER_|"
    "XIUQO_CRON_SESSION|XIUQO_BROWSER_CONTROL_)")
...
def _cwd_marker(session_id: str) -> str:
    return f"__XIUQO_CWD_{session_id}__"
...
def _export_dump_excluding_session_vars(tmp_path: str, excluded_names: Iterable[str] = ()) -> str:
    ...
    ``curl … | bash #`` smuggled into a Matrix room/display name via ``XIUQO_SESSION_CHAT_NAME``) land in
    ...
    return (
        "{ ( unset ${!XIUQO_SESSION_*} ${!XIUQO_CRON_AUTO_DELIVER_*} "
        "${!XIUQO_BROWSER_CONTROL_*} "
        # AI_AGENT / XIUQO_AGENT are per-command attribution markers re-exported
        ...
        "AI_AGENT XIUQO_AGENT "
        f"XIUQO_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; ) || true; } "
        f"> {tmp_path}")
...
def _passthrough_save_restore(names: Iterable[str]) -> tuple[list[str], list[str]]:
    ...
    restore: list[str] = []
    for name in names:
        marker = f"_XIUQO_RUNTIME_PASSTHROUGH_{name}"
...
def _wrap_command_script(
    ...
    ``declare -x`` lines when sourcing. AI_AGENT/XIUQO_AGENT advertise the harness to remote
    ...
    parts += restore
    parts += [
        'export AI_AGENT="${AI_AGENT:-xiuqo}" XIUQO_AGENT="${XIUQO_AGENT:-true}"',
