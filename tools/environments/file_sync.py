...
_FORCE_SYNC_ENV = "XIUQO_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.xiuqo") -> list[tuple[str, str]]:
    """Enumerate all (host_path, remote_path) pairs to sync to a remote. Credential paths are
    remapped from the hardcoded /root/.xiuqo to *container_base* (remote home may differ)."""
    ...
    files = [
        (entry["host_path"], entry["container_path"].replace("/root/.xiuqo", container_base, 1))
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        per ``sync_interval`` unless *force* or ``XIUQO_FORCE_FILE_SYNC=1``. Transactional:
    ...
    def _sync_transaction(self, *, force: bool = False) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="xiuqo-sync-push-") as staging:
                staged_files = []
                pushed_hashes = {}
    ...
    def sync_back(self, xiuqo_home: Path | None = None) -> None:
        """Pull remote changes back to the host: download the remote ``.xiuqo/`` as a tar and
        ...
        with self._transaction_lock:
            self._sync_back_transaction(xiuqo_home=xiuqo_home)

    def _sync_back_transaction(self, xiuqo_home: Path | None = None) -> None:
        ...
        # to avoid retry storms against an uninitialized remote .xiuqo/.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (xiuqo_home or get_hermes_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="xiuqo-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str, file_mapping: list[tuple[str, str]] | None = None, *,
        ...
        substitution (``/root/.xiuqo/skills/b.md`` -> ``~/.xiuqo/skills/b.md``)."""
