"""Base class for all Xiuqo execution environment backends.
...
# (XIUQO_DEBUG_INTERRUPT=1). Off by default to avoid flooding gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("XIUQO_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    overlays/SIF cache). ``TERMINAL_SANDBOX_DIR`` overrides ``{XIUQO_HOME}/sandboxes``."""
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Xiuqo backends. Subclasses
    ...
    # True only when commands execute on the SAME host as the Xiuqo process
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/xiuqo-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/xiuqo-cwd-{self._session_id}.txt"
    ...
    def fetch_file(self, remote_path: str, local_dest: Path, *, max_bytes: int) -> None:
        ...
        import base64
        import binascii
        marker = f"__XIUQO_FETCH_{uuid.uuid4().hex[:12]}__"
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"XIUQO_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the ``__XIUQO_CWD_{session}__`` marker from ``result["output"]``, update
