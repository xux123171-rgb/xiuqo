"""Xiuqo-owned PYTHONPATH stripping for child processes. Launchers prepend the repo
root and the Xiuqo venv's site-packages so the backend can ``import tools``; leaked
into a child Python of a DIFFERENT version they load the backend's C extensions and
crash. Only entries proven Xiuqo-owned by *path provenance* are removed — never by a
...
def _build_hermes_repo_root_aliases(
    ...
    """Exact repo-root spellings emitted by Xiuqo launchers. Mirrors
    ``gateway_windows._preserve_hermes_home_path`` (physical path under the resolved
    XIUQO_HOME -> configured spelling) so a junction-backed install matches without
    treating arbitrary XIUQO_HOME descendants as Xiuqo-owned. A repo-level junction
...
def _get_hermes_site_packages(env: dict) -> list[Path]:
    """Exact site-packages dirs owned by the Xiuqo runtime (cached):
...
def _strip_hermes_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip Xiuqo-owned PYTHONPATH entries, then the runtime marker vars. Order is
...
def _strip_hermes_owned_pythonpath(env: dict) -> None:
    """Remove Xiuqo-owned PYTHONPATH entries: only exact matches of the repo root
    ...
    if stripped:
        logger.debug("Stripped Xiuqo-owned entries from PYTHONPATH: %s", stripped)
