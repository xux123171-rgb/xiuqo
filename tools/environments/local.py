...
from tools.environments.local_env_policy import (
    _ALWAYS_STRIP_KEYS, _XIUQO_PROVIDER_ENV_BLOCKLIST, _XIUQO_PROVIDER_ENV_FORCE_PREFIX,
...
# get_temp_dir() defaults to XIUQO_HOME/cache/terminal (real storage, not tmpfs), so
...
# Background artifacts come in triplets (xiuqo_bg_<id>.log/.pid/.exit). A live
...
_BG_GROUP_RE = re.compile(r"^(xiuqo_bg_[A-Za-z0-9_-]+)\.(log|pid|exit)$")


def _default_terminal_temp_dir() -> "Path | None":
    """Return XIUQO_HOME/cache/terminal, or None if unresolvable."""
    try:
...
def _apply_profile_home(env: dict) -> None:
    """Bridge the context-local XIUQO_HOME override, then the subprocess HOME contract."""
    from xiuqo_constants import apply_subprocess_home_env, get_hermes_home_override
    try:
        if value := get_hermes_home_override():
            env["XIUQO_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    """Bridge gateway session ContextVars (XIUQO_SESSION_*) into a child env.
...
def _filter_secret_env(
    ...
    """Copy *items* into *out*, dropping Xiuqo-managed secrets. ``_XIUQO_FORCE_<NAME>``
    ...
    for key, value in items.items():
        if key.startswith(_XIUQO_PROVIDER_ENV_FORCE_PREFIX):
            if not unwrap_force:
                continue
            key = key[len(_XIUQO_PROVIDER_ENV_FORCE_PREFIX):]
        ...
        if key in _XIUQO_PROVIDER_ENV_BLOCKLIST and not (passthrough or first_party):
            continue
...
def _finalize_child_env(env: dict) -> dict:
    ...
    bridging, Xiuqo-owned PYTHONPATH + venv-marker strip, MSYS defaults, delegate_task
...
def _scrubbed_env(parts, plugin_strip: frozenset, fix_path) -> dict:
    ...
    *fix_path* (always prepending the xiuqo install dir so bare ``xiuqo`` resolves
    ...
    # Keep bare ``xiuqo`` invocations available to child jobs even when the gateway was launched by a
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Xiuqo-managed secrets from a subprocess environment (background/PTY
...
def xiuqo_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    if not inherit_credentials:
        strip |= _XIUQO_PROVIDER_ENV_BLOCKLIST
    for key in list(env):
        if (key in strip or key.startswith(_XIUQO_PROVIDER_ENV_FORCE_PREFIX)
...
def build_subprocess_env(
    ...
    bridges XIUQO_HOME + HOME and ``extra`` is applied last so caller overrides win."""
...
def strip_launch_profile_env(env: dict, target_home: "str | Path | None" = None) -> dict:
    ...
    the secret scrub removes credentials but not settings (``XIUQO_MODEL``, ``TERMINAL_ENV``,
    ``XIUQO_LANGUAGE``...), so a standalone ``xiuqo -p X`` worker and a served one saw different
    ...
    from xiuqo_constants import get_hermes_home_override, get_process_hermes_home
    ...
    if Path(target).resolve() == launch_home.resolve():
        return env
    from xiuqo_cli.config import TERMINAL_CONFIG_ENV_MAP
...
def _windows_bash_candidates(custom: "str | None") -> list[str]:
    """Ordered bash.exe candidates on Windows: XIUQO_GIT_BASH_PATH, our portable Git
    under %LOCALAPPDATA%\\xiuqo\\git (PortableGit ``bin`` and MinGit ``usr\\bin``),
    ...
    roots = [
        lad and os.path.join(lad, "xiuqo", "git", "bin"),
        lad and os.path.join(lad, "xiuqo", "git", "usr", "bin"),
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("XIUQO_GIT_BASH_PATH")
    candidates = _windows_bash_candidates(custom)
    # First candidate that can actually start wins: a stale XIUQO_GIT_BASH_PATH
    ...
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "XIUQO_GIT_BASH_PATH=%s fails to start; using %s instead", custom, candidate)
            return candidate
    if candidates:
        probe_details = "\n".join(
        ...
        return candidates[0]
    raise RuntimeError(
        "Git Bash not found. Xiuqo Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set XIUQO_GIT_BASH_PATH to your bash.exe location.")
...
# Cached directory containing the ``xiuqo`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_XIUQO_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_hermes_bin_dir() -> str | None:
    """Directory holding the ``xiuqo`` console-script, or None (cached). A gateway
    launched by systemd/cron/a desktop launcher lacks the install dir on PATH and bare
    ``xiuqo`` exits 127. Order: ``which``; absolute ``sys.argv[0]`` naming a real
    xiuqo executable; ``sys.executable``'s dir if it holds the shim."""
    global _XIUQO_BIN_DIR
    if _XIUQO_BIN_DIR is not _SENTINEL:
        return _XIUQO_BIN_DIR  # type: ignore[return-value]
    which = shutil.which("xiuqo")
    argv0 = sys.argv[0] if sys.argv else ""
    base = os.path.basename(argv0).lower()
    exe_dir = os.path.dirname(sys.executable) if sys.executable else ""
    shim = "xiuqo.exe" if _IS_WINDOWS else "xiuqo"
    if which:
        candidate = os.path.dirname(which)
    elif (os.path.isabs(argv0) and (base == "xiuqo" or base.startswith("xiuqo."))
    ...
    _XIUQO_BIN_DIR = candidate if candidate and os.path.isdir(candidate) else None
    return _XIUQO_BIN_DIR


def _prepend_hermes_bin_dir(existing_path: str) -> str:
    """Prepend the xiuqo install dir to ``existing_path`` if missing."""
...
def _managed_runtime_path_entries() -> list[str]:
    """Existing Xiuqo-managed runtime dirs: ``$XIUQO_HOME/node`` (+``/bin``) and
    ``$XIUQO_HOME/bin`` (managed ``uv``). Per call, not cached: home is
    profile-scoped and a managed tree can appear mid-process."""
    try:
        from xiuqo_constants import get_hermes_home, iter_hermes_node_dirs
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    and ``wmic``. Xiuqo runs terminal commands through bash on Windows, so set the standard MSYS opt-out by
    default. Refs #56700.
...
# --- Xiuqo venv / repo-root detection (module-level, computed once) ---
...
# spellings — the Windows gateway launcher renders Xiuqo-owned paths under the
# configured XIUQO_HOME spelling (possibly a junction to another drive).
...
class LocalEnvironment(BaseEnvironment):
    ...
    # Commands run on the Xiuqo host itself — controller-side platform behavior
    ...
    def get_temp_dir(self) -> str:
        ...
        (Termux has no /tmp), ``XIUQO_HOME/cache/terminal`` (real storage: tmpfs /tmp
        fills under Xiuqo load; pruned by ``cleanup_terminal_temp_cache``), /tmp,
        ...
        so always the XIUQO_HOME cache dir with forward slashes (bash- and Python-valid)."""
        if _IS_WINDOWS:
            cache_dir = (_default_terminal_temp_dir()
                         or Path(tempfile.gettempdir()) / "xiuqo_terminal")
