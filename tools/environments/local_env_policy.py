"""Secret-scrub policy for Xiuqo child processes: pure data + predicates for which env
names are Xiuqo-managed credentials. The env *builders* applying it (``_make_run_env``,
``_sanitize_subprocess_env``, ``xiuqo_subprocess_env``) live in ``tools.environments.local``."""

import os

# Prefix a caller uses in ``extra_env`` to force a blocklisted var through.
_XIUQO_PROVIDER_ENV_FORCE_PREFIX = "_XIUQO_FORCE_"

# Xiuqo-managed AWS *inference* credentials for ``auth_type="aws_sdk"`` (Bedrock):
...
_STATIC_PROVIDER_ENV_BLOCKLIST = frozenset({
    ...
    "EMAIL_HOME_ADDRESS", "EMAIL_HOME_ADDRESS_NAME", "XIUQO_DASHBOARD_SESSION_TOKEN",
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Claude Code install, not Xiuqo: stripping it made agent-spawned ``claude`` CLIs
    ...
    # It is set and owned by the user's Claude Code install (subscription OAuth), not a Xiuqo-managed
    # inference credential — Claude subscription auth is not a working Xiuqo provider path. It arrives via
...
_XIUQO_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# ``_sanitize_subprocess_env``); execute_code, xiuqo_subprocess_env, docker and
...
# ``buzz`` (the gateway's ``XIUQO_SESSION_PLATFORM`` ContextVar; concurrency safe under a multi-session
...
# scope. Every other surface stays sealed — execute_code scrubbing, :func:`xiuqo_subprocess_env` (browser /
...
# the blocklist entirely because it is NOT a Xiuqo credential; these ARE Xiuqo-managed first-party
...
def _buzz_terminal_context_active() -> bool:
    ...
    try:
        ...
        return get_session_env("XIUQO_SESSION_PLATFORM", "").strip().lower() == "buzz"
    except Exception:
        return False
...
# ANOTHER project's deps into the Xiuqo venv (still reachable via PATH, so stripping
# is safe); PYTHONHOME redirects a child interpreter's stdlib to the Xiuqo venv
# (version-mismatch crashes). PYTHONPATH is handled separately (Xiuqo-owned entries only).
...
# dependencies into the Xiuqo venv path instead of the project's own ``.venv`` — silently clobbering the
# Xiuqo environment (e.g. a project pinned to a different Python version overwrites it and breaks the
# gateway). PYTHONHOME is included because a gateway-inherited value redirects the standard-library search
# of ANY child interpreter — including unrelated system/venv Pythons — to the Xiuqo venv's stdlib, which
# crashes with version-mismatch errors before a child script even imports a package (#75018). Xiuqo itself
...
# _strip_hermes_owned_pythonpath() which removes only Xiuqo-owned entries, preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_hermes_internal_secret(key: str) -> bool:
    """True for Xiuqo-internal secrets injected under *dynamic* names the static
...
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    "HASS_TOKEN", "EMAIL_PASSWORD", "XIUQO_DASHBOARD_SESSION_TOKEN",
