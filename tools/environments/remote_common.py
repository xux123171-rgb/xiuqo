...
from tools.environments.local_env_policy import _XIUQO_PROVIDER_ENV_BLOCKLIST, _is_hermes_internal_secret


def load_hermes_env_vars() -> dict[str, str]:
    """``~/.xiuqo/.env`` values, or ``{}`` — a broken .env must not fail command execution."""
    try:
...
def resolve_passthrough_env(explicit_forward: Iterable[str] = (),
                            xiuqo_env_loader: Callable[[], dict[str, str]] = load_hermes_env_vars,
    ...
    is filtered through the Xiuqo provider-credential blocklist and the dynamic internal-secret
    ...
    forward_keys = set(explicit_forward) | (implicit_forward - _XIUQO_PROVIDER_ENV_BLOCKLIST)
    xiuqo_env = xiuqo_env_loader() if forward_keys else {}
    exec_env: dict[str, str] = {}
    unset_names: set[str] = set()
    for key in sorted(forward_keys):
        value = os.getenv(key) or xiuqo_env.get(key)
