...
def lightpanda_engine_status() -> Tuple[bool, str]:
    ...
    precedence with config-only gates (no network I/O) for ``/browser status`` / ``xiuqo doctor``.
    """
    _bt = _origin()
    ...
    return True, "Browser Use mode: Xiuqo spawns `lightpanda serve` per session"
