...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return _fail("not_in_manifest", f"'{name}' is not a tracked bundled skill. Nothing to reset. "
                     f"(Hub-installed skills use `xiuqo skills uninstall`.)")
    ...
    if not restore:
        action, message = "manifest_cleared", (f"Cleared manifest entry for '{name}'. Future `xiuqo update` runs "
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Bundled skills ``xiuqo update`` keeps because the user edited them (same test the sync
...
def diff_bundled_skill(name: str) -> dict:
    ...
    if (bundled_src := bundled_by_name.get(name)) is None:
        return _fail(False, f"'{name}' is not a tracked bundled skill (no stock version to "
                     f"diff against). Hub-installed skills use `xiuqo skills inspect`.")
...
_OPT_OUT_MESSAGES = {  # (enabled, changed) -> message
    ...
    (False, True): "Opted back in. The next `xiuqo update` (or `xiuqo skills opt-in --sync`) will re-seed bundled skills.",
    (False, False): "Not opted out — no marker to remove."}


def set_bundled_skills_opt_out(enabled: bool) -> dict:
    """Toggle the .no-bundled-skills marker (on-disk half of ``xiuqo skills opt-out`` / ``opt-in``;
    ...
    try:
        if enabled:
            ...
            marker.write_text(
                "This profile opted out of bundled-skill seeding (`xiuqo skills opt-out`).\n"
