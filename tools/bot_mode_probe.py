...
When any profile carries ``ui_meta['xiuqo-bots']`` in profile.yaml (Bot-Mode-managed),
...
def _default_home() -> str:
    """Ambient process XIUQO_HOME (env, else the platform default) as a string."""
...
def _hermes_root(home: Path) -> Path:
    """Root ~/.xiuqo for both the default profile and named profiles."""
...
def _handle(name: str) -> str:
    # The mention middleware aliases the default profile as @xiuqo.
    return "xiuqo" if name == "default" else name
...
def _bots_meta(data: dict | None) -> dict | None:
    """The ``ui_meta['xiuqo-bots']`` block of a parsed profile.yaml, if a dict."""
    ui_meta = data.get("ui_meta") if data else None
    bots = ui_meta.get("xiuqo-bots") if isinstance(ui_meta, dict) else None
    return bots if isinstance(bots, dict) else None


def _is_bot_managed(profile_dir: Path) -> bool:
    return _bots_meta(_read_yaml_dict(profile_dir / "profile.yaml", "xiuqo-bots")) is not None
...
def _peers(root: Path) -> list[str]:
    """Registered peer gateway names (``xiuqo peer``) from config.yaml, read
...
def _peer_paragraph(root: Path) -> str:
    ...
    return (
        ...
        "for the peer's main agent). Run `xiuqo peer list` for the live "
        "peer list."
    )
...
def _build_section(home: Path) -> str:
    ...
    return (
        f"{_PROTOCOL_HEADING}\n"
        "This install runs Bot Mode: each Xiuqo profile is an agent teammate with "
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    the AGENT'S OWN resolved home (session-db derived), not ambient XIUQO_HOME — build threads
