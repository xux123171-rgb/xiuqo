#!/usr/bin/env python3
"""One parent-death supervisor per Xiuqo process, shared by all stdio MCP servers.

Why this exists
---------------
When Xiuqo dies without running its cleanup path (SIGKILL, OOM killer, a hard
crash), stdio MCP servers it spawned are reparented to init and keep running
forever.  macOS has no ``PR_SET_PDEATHSIG``, so something has to outlive Xiuqo
and reap them.

This module is deliberately standard-library-only and must not import anything
from ``tools/``: it runs after Xiuqo may already be dead, and pulling in
...
with a single supervisor per Xiuqo process:

* **Death detection is a blocking read on a pipe.**  Xiuqo holds the only write
  end.  When Xiuqo dies -- by any means, including SIGKILL -- the write end
...
Xiuqo never unregistered *is* the orphan set, so a clean Xiuqo shutdown --
...
an unrelated process owned by the same user.  If Xiuqo then dies ungracefully
...
Two things narrow the window.  Xiuqo prunes registrations whose group has no
...
exposure already exists in Xiuqo's own killpg-based orphan cleanup, which this
...
def _is_safe_target(pgid: int, *, own_pgid: int, parent_pgid: int) -> bool:
    ...
    Defensive only -- Xiuqo already filters non-MCP children before it
    ...
    Xiuqo process tree or the session.
    """
    if pgid <= 1:
        return False
...
def _serve(stream, *, own_pgid: int, parent_pgid: int) -> set[int]:
    ...
    stopped. Nothing in Xiuqo can produce that today, but this process is the
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Process group of the spawning Xiuqo process; never signalled.",
    )
