...
React + Ink terminal UI for Xiuqo. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `XIUQO_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `xiuqo setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.xiuqo/.xiuqo_history` or under `XIUQO_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `xiuqo setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/xiuqo-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external xiuqo setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.xiuqo/tui-parent.log
    ...
    types/
      xiuqo-ink.d.ts            type declarations for @xiuqo/ink
