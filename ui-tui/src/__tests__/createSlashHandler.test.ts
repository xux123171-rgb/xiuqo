...
// DASHBOARD_TUI_MODE resolves once at module load from XIUQO_TUI_DASHBOARD,
...
describe('createSlashHandler', () => {
  ...
  it('routes /status to live session.status instead of slash worker', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ output: 'Xiuqo TUI Status' }))
    ...
    expect(ctx.gateway.gw.request).not.toHaveBeenCalled()
    await vi.waitFor(() => {
      expect(ctx.transcript.page).toHaveBeenCalledWith('Xiuqo TUI Status', 'Status')
    })
  })
  ...
  it('falls through to command.dispatch for skill commands, sending the body but showing the invocation', async () => {
    const skillMessage =
      '[IMPORTANT: The user has invoked the "xiuqo-dev" skill, indicating they want you to follow its instructions.\n' +
    ...
    const h = createSlashHandler(ctx)
    expect(h('/xiuqo-dev')).toBe(true)
    await vi.waitFor(() => {
      expect(ctx.transcript.send).toHaveBeenCalledWith(skillMessage, true, '/xiuqo-dev')
    })
  ...
  it('/save forwards to session.save RPC and reports the returned file', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ file: '/tmp/xiuqo_conversation_test.json' }))
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('conversation saved to: /tmp/xiuqo_conversation_test.json')
    })
  })
