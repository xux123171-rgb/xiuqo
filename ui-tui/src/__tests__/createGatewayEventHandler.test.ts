...
describe('createGatewayEventHandler', () => {
  ...
  it('surfaces self-improvement review summaries as a persistent system line', () => {
    ...
    onEvent({
      payload: { text: "💾 Self-improvement review: Skill 'xiuqo-release' patched" },
      type: 'review.summary'
    } as any)

    expect(ctx.system.sys).toHaveBeenCalledWith("💾 Self-improvement review: Skill 'xiuqo-release' patched")
  })
  ...
  it('annotates gateway.start_timeout with stderr tail lines so users can diagnose without /logs', () => {
    ...
    onEvent({
      payload: {
        ...
        stderr_tail:
          '[startup] timed out\nModuleNotFoundError: No module named openai\nFileNotFoundError: ~/.xiuqo/config.yaml'
      },
  ...
  it('shows setup panel for missing provider startup error', () => {
    ...
    onEvent({
      payload: {
        message:
          'agent init failed: No LLM provider configured. Run `xiuqo model` to select a provider, or run `xiuqo setup` for first-time configuration.'
      },
      type: 'error'
    } as any)
  ...
  it('picks the polarity-matching paired palette from gateway.ready skins', async () => {
    ...
    vi.stubEnv('XIUQO_TUI_BACKGROUND', '')
    ...
    vi.stubEnv('XIUQO_TUI_BACKGROUND', '#ffffff')
    createGatewayEventHandler(buildCtx(appended))({ payload: skin, type: 'skin.changed' } as any)
  ...
  it('opens a fresh session before starting voice after wake detection', async () => {
    ...
    createGatewayEventHandler(ctx)({
      payload: { phrase: 'hey xiuqo', start_new_session: true },
      type: 'wake.detected'
    } as any)
  ...
  it('keeps the current session when wake detection disables session creation', async () => {
    ...
    createGatewayEventHandler(ctx)({
      payload: { phrase: 'hey xiuqo', start_new_session: false },
      type: 'wake.detected'
    } as any)
