import { contrastRatio, ensureContrast, mix, parseColor, relativeLuminance, toHex } from '@xiuqo/shared/color'
import type { SkinBranding, SkinColors } from '@xiuqo/shared/skin'
...
// ── Color math ───────────────────────────────────────────────────────
//
// Generic color computation lives in @xiuqo/shared/color (the primitives,
...
// Mirrors @xiuqo/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @xiuqo/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic Xiuqo navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Xiuqo look was never hand-authored: for years the
 ...
 *  answer when known (cached in XIUQO_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.XIUQO_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit XIUQO_TUI_THEME / COLORFGBG signals above still win,
...
// `XIUQO_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `XIUQO_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `XIUQO_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `XIUQO_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Xiuqo palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.XIUQO_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.XIUQO_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.XIUQO_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into XIUQO_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into XIUQO_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Xiuqo navy
  ...
  // the derived value already carries the identity seeds (e.g. Xiuqo navy).
  const activeRow =
