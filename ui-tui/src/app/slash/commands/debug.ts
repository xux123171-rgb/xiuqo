...
import { terminalBackgroundHex } from '@xiuqo/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $XIUQO_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see XIUQO_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['XIUQO_TUI_BACKGROUND', process.env.XIUQO_TUI_BACKGROUND ?? '(unset)'],
            ['XIUQO_TUI_THEME', process.env.XIUQO_TUI_THEME ?? '(unset)'],
