import type { RunExternalProcess } from '@xiuqo/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`xiuqo ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching xiuqo: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`xiuqo ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
