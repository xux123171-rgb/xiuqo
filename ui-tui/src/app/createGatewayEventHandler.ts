...
import { forceRedraw, onTerminalBackground, onTerminalForeground } from '@xiuqo/ink'
import { stripAnsi } from '@xiuqo/shared/ansi'
import { relativeLuminance } from '@xiuqo/shared/color'
import type { SubagentStatus, Usage } from '@xiuqo/shared/gateway-events'
...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.XIUQO_TUI_THEME : undefined

  writeBootTheme(theme, process.env.XIUQO_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to XIUQO_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit XIUQO_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the XIUQO_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.XIUQO_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.XIUQO_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a XIUQO_TUI_THEME the user
    ...
    delete process.env.XIUQO_TUI_THEME
  }
 ...
 * is cached into XIUQO_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * XIUQO_TUI_LIGHT / XIUQO_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.XIUQO_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.XIUQO_TUI_THEME || process.env.XIUQO_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.XIUQO_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.XIUQO_TUI_BACKGROUND ||
      process.env.XIUQO_TUI_THEME ||
      process.env.XIUQO_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.XIUQO_TUI_BACKGROUND || process.env.XIUQO_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.XIUQO_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: AnyGatewayEvent) => void {
  ...
  const scheduleStartupPrompt = () => {
    ...
    setTimeout(async () => {
      ...
      // launches, `xiuqo --tui -q "…"`) — submit LITERALLY, bypassing the
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // Arm "Hey Xiuqo" if this surface owns it (server gates on config).
    ...
    // forging a brand-new one.  Mirrors classic CLI's `xiuqo -c` /
    // `xiuqo --tui` muscle memory and addresses the audit's "session
  ...
  return (ev: AnyGatewayEvent) => {
    ...
    switch (ev.type) {
      ...
      case 'wake.detected': {
        // "Hey Xiuqo": optionally open a fresh session (start_new_session),
        ...
        void (async () => {
          ...
          if (wakeProfile && wakeProfile !== ownProfile) {
            sys(`wake phrase for profile '${wakeProfile}' — run: xiuqo -p ${wakeProfile} --tui`)
