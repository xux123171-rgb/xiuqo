import type { InputEvent, Key } from '@xiuqo/ink'
import * as Ink from '@xiuqo/ink'
 ...
 * but that accepts `key.meta` on macOS — and xiuqo-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.XIUQO_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which xiuqo-ink decodes as meta+'d'; without this branch it
