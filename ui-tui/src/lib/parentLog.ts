...
const logDir = join(process.env.XIUQO_HOME?.trim() || join(homedir(), '.xiuqo'), 'logs')
...
// a real ~/.xiuqo (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('xiuqo-tui: parent lifecycle log unavailable\n')
    }
  }
}
