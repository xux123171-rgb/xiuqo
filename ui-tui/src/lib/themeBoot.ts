 ...
 * `xiuqo-boot-background` / `xiuqo-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports XIUQO_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.xiuqo matches get_hermes_home()'s default.
const bootFilePath = () => join(process.env.XIUQO_HOME ?? join(homedir(), '.xiuqo'), 'tui-theme-boot.json')

// Never touch the user's real ~/.xiuqo from test runs (the TS suite has no
// XIUQO_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into XIUQO_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.XIUQO_TUI_THEME || env.XIUQO_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.XIUQO_TUI_BACKGROUND
  ) {
    env.XIUQO_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.XIUQO_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.XIUQO_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into XIUQO_TUI_THEME.
