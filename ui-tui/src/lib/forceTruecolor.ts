...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.XIUQO_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Xiuqo on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via XIUQO_TUI_TRUECOLOR=1.
