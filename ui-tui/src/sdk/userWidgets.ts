...
import { Box, Text } from '@xiuqo/ink'
...
/**
 * User widget apps — Xiuqo authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$XIUQO_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.xiuqo/plugins/`: files under XIUQO_HOME execute
...
const widgetsDir = () => join(process.env.XIUQO_HOME?.trim() || join(homedir(), '.xiuqo'), 'tui-widgets')
 ...
 *  every change, so a widget Xiuqo writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Xiuqo) ever writes must hot-load too — a 10s poll here read
