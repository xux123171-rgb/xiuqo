...
def _is_dispatcher_owned_worker() -> bool:
    """False when XIUQO_KANBAN_* is present but this execution does not own it
...
def _tool_defs_cache_key(
    ...
    return (
        ...
        bool(os.environ.get("XIUQO_KANBAN_TASK")), bool(skip_tool_search_assembly),
...
def _apply_toolset_selection(tools: set, names: List[str], quiet_mode: bool, *, disable: bool) -> None:
    ...
    for name in names:
        if validate_toolset(name):
            ...
            if disable and (name.startswith("xiuqo-") or (get_toolset(name) or {}).get("posture")):
                ...
                if not quiet_mode and name.startswith("xiuqo-") and name not in _WARNED_DISABLED_BUNDLES:
...
def _select_tool_names(enabled_toolsets: Optional[List[str]], disabled_toolsets: Optional[List[str]], quiet_mode: bool) -> set:
    ...
    if enabled_toolsets is not None:
        ...
        if (os.environ.get("XIUQO_KANBAN_TASK") and not _is_delegated_child_context()
    ...
    # toolset is stripped even when a composite (xiuqo-cli) re-enables it.
    # This ensures that even if a composite toolset (like xiuqo-cli) is enabled, any tools belonging to a
...
def _emit_post_tool_call_hook(
    *, function_name: str, function_args: Dict[str, Any], result: Any,
    ...
    if _post_tool_call_hook_suppressed.get():
        return
    try:
        from xiuqo_cli.lifecycle import has_hook, invoke_hook
