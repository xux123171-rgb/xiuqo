...
version: 0.1.0
author: Bihruze (Asahi0x), Xiuqo Agent
license: MIT
...
import upstream classes into Xiuqo itself.
...
```bash
mkdir -p ~/.xiuqo/cache/darwinian-evolver && cd ~/.xiuqo/cache/darwinian-evolver
...
```bash
cd ~/.xiuqo/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.xiuqo/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.xiuqo/skills/research/darwinian-evolver
DE_DIR=~/.xiuqo/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Xiuqo core.
   Custom driver scripts under `~/.xiuqo/skills/...` are user-side and fine.
