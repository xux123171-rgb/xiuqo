---
name: actual-setup
description: Set up Actual Computer (actual.inc) inference in Xiuqo.
version: 2.0.0
author: shl0ms + Xiuqo Agent
license: MIT
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a Xiuqo inference
...
- User has an `ac_` key and wants Xiuqo routed through their Actual cluster.
...
- Xiuqo has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current Xiuqo — the
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.xiuqo/.env`.
...
3. Select provider + model:
   ```bash
   xiuqo config set model.provider actual
   xiuqo config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   xiuqo chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point Xiuqo at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   built-in provider into local no-auth mode automatically — no key needed:
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.xiuqo/.env`, then:
   ```bash
   xiuqo config set model.provider actual
   xiuqo config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   xiuqo chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by Xiuqo since the first-class provider).**
   ...
   request still 400s this way on an old Xiuqo, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** Xiuqo' default toolset
   ...
   servers emit a bare `data: [DONE]` — Xiuqo reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current Xiuqo the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
xiuqo chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
xiuqo chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
