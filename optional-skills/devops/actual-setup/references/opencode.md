...
is typically only in `~/.xiuqo/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Xiuqo,
...
`xiuqo-custom-providers` skill, pitfall 2). Xiuqo hits this because it forwards
