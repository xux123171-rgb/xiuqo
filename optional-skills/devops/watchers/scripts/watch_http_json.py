...
Usage (via cron with --no-agent):
    ...
    xiuqo cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$XIUQO_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Xiuqo-Watcher/1.0"})
