...
Usage (via cron with --no-agent):
    ...
    xiuqo cron create xiuqo-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$XIUQO_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name xiuqo-issues --repo xux123171-rgb/xiuqo --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Xiuqo .env file
(``${XIUQO_HOME:-~/.xiuqo}/.env``) to avoid the 60 req/hr
