...
- smoke-test a server with the FastMCP CLI before wiring it into Xiuqo or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Xiuqo. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.xiuqo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.xiuqo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Xiuqo integration, either:

- configure the server in `~/.xiuqo/config.yaml` using the `native-mcp` skill, or
...
### Xiuqo cannot see the deployed server

The server-building part may be correct while the Xiuqo config is not. Load the `native-mcp` skill and configure the server in `~/.xiuqo/config.yaml`, then restart Xiuqo.
