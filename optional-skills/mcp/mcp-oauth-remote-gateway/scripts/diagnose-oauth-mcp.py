...
Usage:
  ...
  <server_name> matches the files in $XIUQO_HOME/mcp-tokens/<server>.json etc.
...
def _hermes_home():
    # Prefer Xiuqo' own resolver (profile-safe); fall back to env then ~/.xiuqo.
    try:
        from xiuqo_constants import get_hermes_home
        return str(get_hermes_home())
    except Exception:
        return os.environ.get("XIUQO_HOME") or os.path.expanduser("~/.xiuqo")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "xiuqo-diag", "version": "1.0"}},
        },
        headers={
