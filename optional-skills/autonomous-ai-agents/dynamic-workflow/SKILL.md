...
metadata:
  xiuqo:
    ...
    related_skills: [xiuqo, simplify-code]
when_to_use:
...
for serial chains. For a refactor or fix campaign on xiuqo itself, load
`xiuqo` (the dev workflow) alongside; this skill owns the fan-out shape.
...
10. Live QA is its own wave: one squad per surface, isolated `XIUQO_HOME`,
