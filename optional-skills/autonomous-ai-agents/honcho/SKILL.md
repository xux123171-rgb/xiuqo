---
name: honcho
description: Configure and troubleshoot Honcho memory for Xiuqo.
version: 2.0.0
author: Xiuqo Agent
license: MIT
...
# Honcho Memory for Xiuqo

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Xiuqo profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/xiuqo#running-honcho-locally-with-xiuqo

### Verify

```bash
xiuqo honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Xiuqo profile's AI peer
...
Honcho models conversations as interactions between **peers**. Xiuqo creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Xiuqo instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Xiuqo run |
| `global` | Single session across all directories |

Manual override: `xiuqo honcho map my-project-name`
...
Each Xiuqo profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
xiuqo profile create coder --clone
# creates host block xiuqo.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `xiuqo.coder` host block in `honcho.json`
...
```bash
xiuqo honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Xiuqo when Honcho memory is active.
...
Config file: `$XIUQO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `xiuqo honcho setup`. Ensure `memory.provider: honcho` is in `~/.xiuqo/config.yaml`.

### Memory not persisting across sessions
Check `xiuqo honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `xiuqo profile create <name> --clone`. For existing profiles: `xiuqo honcho sync`.
...
| Command | Description |
|---------|-------------|
| `xiuqo honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `xiuqo honcho status` | Show resolved config, connection test, peer info for active profile |
| `xiuqo honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `xiuqo honcho disable` | Disable Honcho for the active profile |
| `xiuqo honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `xiuqo honcho peers` | Show peer identities across all profiles |
| `xiuqo honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `xiuqo honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `xiuqo honcho sessions` | List known directory-to-session-name mappings |
| `xiuqo honcho map <name>` | Map current working directory to a Honcho session name |
| `xiuqo honcho identity` | Seed AI peer identity or show both peer representations |
| `xiuqo honcho sync` | Create host blocks for all Xiuqo profiles that don't have one yet |
| `xiuqo honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Xiuqo + Honcho |
| `xiuqo memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `xiuqo memory status` | Show active memory provider and config |
| `xiuqo memory off` | Disable external memory provider |
