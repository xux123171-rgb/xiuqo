...
version: 0.1.1
author: Matt Maximo (MattMaximo), Xiuqo Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  xiuqo:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, xiuqo]
---

# Grok Build CLI — Xiuqo Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Xiuqo terminal. Grok
...
1. Prepare stable input files first with Xiuqo tools (`read_file`,
...
2. **Don't conflate Xiuqo' xAI auth with the `grok` CLI's auth.** Xiuqo'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Xiuqo' xAI auth carries
   over.
