...
version: 0.1.0
author: Tim Koepsel (xzessmedia), Xiuqo Agent
license: MIT
platforms: [linux, macos]
metadata:
  xiuqo:
    tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
    related_skills: [claude-code, codex, opencode, xiuqo]
---
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Xiuqo.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Xiuqo-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `xiuqo` (Xiuqo subagents via `delegate_task`).
