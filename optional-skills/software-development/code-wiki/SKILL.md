...
Invoke through the `terminal` tool from the target repo's root, then use `read_file` / `search_files` / `write_file` to produce the wiki. Default output location is `~/.xiuqo/wikis/<repo-name>/`. Only write into the repo (`docs/wiki/`) when the user explicitly requests it.
...
```bash
OUTPUT_DIR="$HOME/.xiuqo/wikis/$REPO_NAME"
...
cat > "$OUTPUT_DIR/.codewiki-state.json" <<EOF
{
  ...
  "generator": "xiuqo code-wiki skill v0.1.0",
  "modules_documented": []
}
EOF
```
...
```
Generated wiki at ~/.xiuqo/wikis/<repo-name>/:
...
- **In-repo output without asking.** Default is `~/.xiuqo/wikis/`. Only write into the repo when the user explicitly requests it.
