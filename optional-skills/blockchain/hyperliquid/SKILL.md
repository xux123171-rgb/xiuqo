...
version: 0.1.0
author: Hugo Sequier (Hugo-SEQUIER), Xiuqo Agent
license: MIT
...
The script reads `${XIUQO_HOME:-~/.xiuqo}/.env` for two optional defaults:
...
Helper script: `~/.xiuqo/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python ~/.xiuqo/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${XIUQO_HOME:-~/.xiuqo}/.env`.
...
```bash
python ~/.xiuqo/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python ~/.xiuqo/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
