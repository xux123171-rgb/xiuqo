...
version: 0.1.0
author: Teknium (teknium1), Xiuqo Agent
license: MIT
...
Wraps [@stripe/link-cli](https://github.com/stripe/link-cli) so Xiuqo can complete purchases on the user's behalf using one-time-use virtual cards or Shared Payment Tokens (SPT). Every spend is gated by an in-app approval in the Link mobile/web app — Xiuqo cannot self-approve.
...
The Link account, payment method, and spend-approval app do NOT need to be set up before Xiuqo attempts to pay — the CLI walks the user through them on first run:
...
```
link-cli auth login --client-name "Xiuqo" --interval 5 --timeout 300
```
...
`@stripe/link-cli --mcp` exposes the same commands as MCP tools over stdio. To register it with Xiuqo' native MCP:

```
xiuqo mcp add stripe-link --command "npx" --args "@stripe/link-cli --mcp"
```

Then `xiuqo mcp list` should show `stripe-link`. The same approval rules apply — MCP doesn't bypass the Link app approval step.
