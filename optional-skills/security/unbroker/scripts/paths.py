...
All per-subject data lives under PDD_DATA_DIR (default: $XIUQO_HOME/unbroker),
which is the same trust boundary Xiuqo uses for .env and OAuth tokens.
"""
...
def xiuqo_home() -> Path:
    return Path(os.environ.get("XIUQO_HOME") or (Path.home() / ".xiuqo"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else xiuqo_home() / "unbroker"
