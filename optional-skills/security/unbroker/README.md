...
The design is **Xiuqo-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
xiuqo skills install official/security/unbroker
```

Then start a new Xiuqo session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Xiuqo home directory (`0600`, optionally
