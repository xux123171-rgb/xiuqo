...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("XIUQO_HOME", os.path.expanduser("~/.xiuqo")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("XIUQO_HOME", Path.home() / ".xiuqo")) / "skills" / "red-teaming" / "godmode"
...
XIUQO_HOME = Path(os.getenv("XIUQO_HOME", Path.home() / ".xiuqo"))
CONFIG_PATH = XIUQO_HOME / "config.yaml"
PREFILL_PATH = XIUQO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Xiuqo models — already uncensored, just needs clean prompt
    "xiuqo": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "xiuqo" in model_lower or "nous" in model_lower:
        return "xiuqo"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Xiuqo config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.xiuqo/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Xiuqo for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Xiuqo for changes to take effect.")
