...
version: 1.0.0
author: arceus77-7, enhanced by Xiuqo Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during Xiuqo terminal calls (desktop app flow only)
...
### Service Account (recommended for Xiuqo)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${XIUQO_HOME:-~/.xiuqo}/.env` (the skill will prompt for this on first load).
...
## Xiuqo Execution Pattern (desktop app flow)

Xiuqo terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/xiuqo-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/xiuqo-op.sock"
