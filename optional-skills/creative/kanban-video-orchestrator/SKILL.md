...
short to a music video to an ASCII loop — in a Xiuqo Kanban pipeline that
...
3. **Generates** a setup script that creates Xiuqo profiles, project workspace, and the initial kanban task
...
For mapping role → which Xiuqo skills + toolsets it loads, see
...
3. Creates each Xiuqo profile via `xiuqo profile create --clone`
...
7. Fires the initial `xiuqo kanban create` task assigned to the director
...
```bash
xiuqo kanban watch --tenant <project-tenant>     # live events
xiuqo kanban list  --tenant <project-tenant>     # board snapshot
xiuqo dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `xiuqo kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${XIUQO_HOME:-~/.xiuqo}/.env` or the user's secret store.
