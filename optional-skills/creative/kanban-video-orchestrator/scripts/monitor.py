#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `xiuqo kanban list` and
...
def xiuqo_available() -> bool:
    return shutil.which("xiuqo") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed xiuqo CLI."""
    try:
        out = subprocess.run(
            ["xiuqo", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `xiuqo kanban list`
    out = subprocess.run(
        ["xiuqo", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["xiuqo", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not xiuqo_available():
        print("ERROR: 'xiuqo' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
