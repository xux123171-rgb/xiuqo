...
xiuqo kanban watch --tenant <project-slug>

# Snapshot of the board
xiuqo kanban list --tenant <project-slug>
xiuqo kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
xiuqo kanban stats --tenant <project-slug>

# Visual dashboard (browser)
xiuqo dashboard

# Inspect a specific task (includes comments + events)
xiuqo kanban show <task-id>

# Follow a single task's event stream
xiuqo kanban tail <task-id>
```

Verify available subcommands with `xiuqo kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `xiuqo kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `xiuqo kanban block <id>` then `xiuqo kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `xiuqo kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `xiuqo kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `xiuqo profile list` (profile exists?), `xiuqo status` (gateway/dispatcher up?), and verify tenant. |
...
xiuqo kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
xiuqo kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(xiuqo kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
xiuqo kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
xiuqo kanban block <task-id>
xiuqo kanban archive <task-id>

# Diagnose what happened
xiuqo kanban show <task-id>      # task body, comments, recent events
xiuqo kanban tail <task-id>      # follow the live event stream
xiuqo kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `xiuqo kanban create` for the director
...
while true; do
    clear
    xiuqo kanban list --tenant <slug>
    echo "---"
    xiuqo kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `xiuqo kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `xiuqo profile list` and start
