...
Maps each role archetype to the Xiuqo skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public xiuqo
...
## Xiuqo skills relevant to video production

### Visual / rendering skills (`xiuqo/skills/creative/`)
...
| `touchdesigner-mcp` (via the `touchdesigner` plugin: `xiuqo plugins install touchdesigner`) | Control a running TouchDesigner instance — real-time visuals, audio-reactive installation art, VJ | Renderer for real-time/audio-reactive content; installation art; live performance |
...
### Design / pre-production skills (`xiuqo/skills/creative/`)
...
### Audio / media skills (`xiuqo/skills/creative/` + `skills/media/`)
...
These are **not** Xiuqo skills but external CLIs / APIs that profiles invoke.
...
## Built-in Xiuqo tools for media review

These are native Xiuqo tools — not invoked via terminal but through their own
...
key is present in `${XIUQO_HOME:-~/.xiuqo}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Xiuqo profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${XIUQO_HOME:-~/.xiuqo}/.env` → environment variable.
...
When a new Xiuqo-public video skill ships:
