...
Companion skill for the `unreal-engine` entry in the Xiuqo MCP catalog. The
...
Two halves, in this order: the editor side must be up before Xiuqo connects.
...
### One-time, Xiuqo side
    ...
    xiuqo mcp install unreal-engine
...
`~/.xiuqo/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Xiuqo — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Xiuqo connects
...
2. Start the Xiuqo session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Xiuqo
they appear as:
...
then happens at `xiuqo mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Xiuqo runs batched calls
...
- **Start order matters.** Editor + server up first, then the Xiuqo
...
- **Timeouts on long operations.** Xiuqo' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.xiuqo/config.yaml` for
