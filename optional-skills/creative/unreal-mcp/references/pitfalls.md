...
### 1. Start order: editor first, Xiuqo second

Xiuqo probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Xiuqo session.
...
restart the editor, restart the Xiuqo session.
...
server fails to bind (Output Log) or Xiuqo' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.xiuqo/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Xiuqo

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Xiuqo' connection
lives in `~/.xiuqo/config.yaml` via `xiuqo mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Xiuqo — just don't
mistake it for the Xiuqo setup step.
...
explicitly warns against overlapping calls. Xiuqo executes same-turn tool
...
### 9. Timeouts: Xiuqo gives up before Unreal does

Xiuqo' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Xiuqo runs elsewhere (SSH backend,
