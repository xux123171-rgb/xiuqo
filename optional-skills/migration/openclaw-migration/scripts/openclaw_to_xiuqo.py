#!/usr/bin/env python3
"""OpenClaw -> Xiuqo migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Xiuqo Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Xiuqo.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Xiuqo memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Xiuqo memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Xiuqo-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Xiuqo-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Xiuqo command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.xiuqo/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.xiuqo/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Xiuqo .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Xiuqo .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Xiuqo .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Xiuqo .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Xiuqo .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Xiuqo config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Xiuqo config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Xiuqo.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Xiuqo MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Xiuqo config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'xiuqo cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Xiuqo config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "full-providers": {
        ...
        "description": "Import custom model providers (baseUrl, apiType, headers) into Xiuqo custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Xiuqo config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Xiuqo config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Xiuqo config.yaml approvals section.",
    },
    "memory-backend": {
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `xiuqo config edit` (or move it aside), then "
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`xiuqo config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.xiuqo/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Xiuqo config.yaml")
    ensure_parent(path)
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Xiuqo in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Xiuqo`` (prose), but lowercase matches
# like ``openclaw`` → ``xiuqo`` (so filesystem paths like ``~/.openclaw``
# become ``~/.xiuqo`` — the real Xiuqo home — not the broken ``~/.Xiuqo``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Xiuqo'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Xiuqo'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Xiuqo'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Xiuqo`` but maps ``openclaw`` → ``xiuqo`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.xiuqo/config.yaml`` (the real Xiuqo home) instead of the broken
    ``~/.Xiuqo/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Xiuqo.
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION Xiuqo memory store (memories/MEMORY.md, USER.md).
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Xiuqo Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-xiuqo/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Xiuqo config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-xiuqo/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Xiuqo session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Xiuqo config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Xiuqo
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Xiuqo-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Xiuqo-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: xiuqo config set",
                        )
                    continue
        ...
        for oc_key, xiuqo_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and xiuqo_key not in secret_additions:
                secret_additions[xiuqo_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, xiuqo_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and xiuqo_key not in secret_additions:
                        secret_additions[xiuqo_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        xiuqo_config = load_yaml_file(destination)
        current_model = xiuqo_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = xiuqo_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                xiuqo_config["model"] = {"default": model_str}
            dump_yaml_file(destination, xiuqo_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Xiuqo still uses "edge"
        ...
        xiuqo_config = load_yaml_file(destination)
        existing_tts = xiuqo_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Xiuqo destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Xiuqo-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Xiuqo-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        xiuqo_cfg_path = self.target_root / "config.yaml"
        xiuqo_cfg = load_yaml_file(xiuqo_cfg_path)
        existing_mcp = xiuqo_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Xiuqo config")
                continue

            xiuqo_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                xiuqo_srv["command"] = srv["command"]
                if srv.get("args"):
                    xiuqo_srv["args"] = srv["args"]
                if srv.get("env"):
                    xiuqo_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    xiuqo_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                xiuqo_srv["url"] = srv["url"]
                if srv.get("headers"):
                    xiuqo_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    xiuqo_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                xiuqo_srv["enabled"] = False
            if srv.get("timeout"):
                xiuqo_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                xiuqo_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                xiuqo_srv["tools"] = {}
                if tools_cfg.get("include"):
                    xiuqo_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    xiuqo_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(xiuqo_cfg_path)
            xiuqo_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(xiuqo_cfg_path, xiuqo_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'xiuqo cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = xiuqo_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Xiuqo reasoning_effort
        ...
        if compaction:
            compression = xiuqo_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'xiuqo gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("XIUQO_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if session.get("reset") or session.get("resetTriggers") or session.get("reset_triggers"):
            self.record("session-config", "session reset timers", None, "skipped",
                        "Xiuqo conversations do not reset on idle or daily timers")
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        xiuqo_cfg_path = self.target_root / "config.yaml"
        xiuqo_cfg = load_yaml_file(xiuqo_cfg_path)
        custom_providers = xiuqo_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(xiuqo_cfg_path)
            xiuqo_cfg["custom_providers"] = custom_providers
            dump_yaml_file(xiuqo_cfg_path, xiuqo_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Xiuqo config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            xiuqo_cfg_path = self.target_root / "config.yaml"
            xiuqo_cfg = load_yaml_file(xiuqo_cfg_path)
            discord_hermes = xiuqo_cfg.get("discord") or {}
            changed = False
            ...
            if changed and self.execute:
                xiuqo_cfg["discord"] = discord_hermes
                dump_yaml_file(xiuqo_cfg_path, xiuqo_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        xiuqo_cfg_path = self.target_root / "config.yaml"
        xiuqo_cfg = load_yaml_file(xiuqo_cfg_path)
        browser_hermes = xiuqo_cfg.get("browser") or {}
        changed = False

        # Map fields that have Xiuqo equivalents
        ...
        if changed:
            xiuqo_cfg["browser"] = browser_hermes
            if self.execute:
                self.maybe_backup(xiuqo_cfg_path)
                dump_yaml_file(xiuqo_cfg_path, xiuqo_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            xiuqo_mode = mode_map.get(mode, "manual")
            xiuqo_cfg.setdefault("approvals", {})["mode"] = xiuqo_mode
            if self.execute:
                self.maybe_backup(xiuqo_cfg_path)
                dump_yaml_file(xiuqo_cfg_path, xiuqo_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{xiuqo_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Xiuqo Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Xiuqo. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Xiuqo Config Not Overwritten)",
                "",
                "These items already existed in your Xiuqo config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Xiuqo agent discovers these",
            "directories, it may read/write to them instead of the Xiuqo state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `xiuqo claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `xiuqo claw cleanup` to fix it.",
            "",
            "## Xiuqo-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `xiuqo claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `xiuqo setup` to configure any remaining settings",
            "- Run `xiuqo mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `xiuqo cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `xiuqo cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `xiuqo gateway install` if you need the gateway service",
            "- Review `~/.xiuqo/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Xiuqo Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("XIUQO_HOME") or str(Path.home() / ".xiuqo"), help="Xiuqo home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Xiuqo targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Xiuqo-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Xiuqo Migration   [{mode_label:>8s}]   ║")
    print("  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.xiuqo/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.xiuqo/config.yaml")
        print("    2. Run: xiuqo mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: xiuqo cron")
