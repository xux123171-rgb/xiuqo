...
description: Import an OpenClaw setup (memories, skills) into Xiuqo.
version: 1.0.0
author: Xiuqo Agent (Nous Research)
license: MIT
platforms: [linux, macos, windows]
metadata:
  xiuqo:
    tags: [Migration, OpenClaw, Xiuqo, Memory, Persona, Import]
    related_skills: [xiuqo]
---

# OpenClaw -> Xiuqo Migration

Use this skill when a user wants to move their OpenClaw setup into Xiuqo Agent with minimal manual cleanup.
...
```bash
xiuqo claw migrate              # Full interactive migration
xiuqo claw migrate --dry-run    # Preview what would be migrated
xiuqo claw migrate --preset user-data   # Migrate without secrets
xiuqo claw migrate --overwrite  # Overwrite existing conflicts
xiuqo claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `xiuqo setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the Xiuqo home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Xiuqo memory entries
- merge OpenClaw command approval patterns into Xiuqo `command_allowlist`
- migrate Xiuqo-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Xiuqo working-directory configuration
- copy OpenClaw skills into `~/.xiuqo/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Xiuqo workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.xiuqo/tts/`
- archive non-secret docs that do not have a direct Xiuqo destination
...
- `~/.xiuqo/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.xiuqo/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.xiuqo/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Xiuqo-compatible secrets, currently:
...
Xiuqo CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Xiuqo. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Xiuqo workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Xiuqo skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.xiuqo/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.xiuqo/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.xiuqo/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.xiuqo/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.xiuqo/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.xiuqo/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Xiuqo unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Xiuqo targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Xiuqo destination. Unsupported auth blobs must still be reported as skipped.
...
- Xiuqo persona state imported
- Xiuqo memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.xiuqo/skills/openclaw-imports/`
