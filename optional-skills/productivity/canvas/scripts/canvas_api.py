#!/usr/bin/env python3
"""Canvas LMS API CLI for Xiuqo Agent.
...
def _check_config():
    ...
    if missing:
        xiuqo_env = os.path.join(
            os.environ.get("XIUQO_HOME", os.path.expanduser("~/.xiuqo")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {xiuqo_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Xiuqo Agent"
    )
