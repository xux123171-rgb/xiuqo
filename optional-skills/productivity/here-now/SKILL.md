...
- Skill helper paths:
  - `${XIUQO_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${XIUQO_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${XIUQO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client xiuqo
```
...
```bash
PUBLISH="${XIUQO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client xiuqo
```
...
```bash
DRIVE="${XIUQO_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `xiuqo`)    |
