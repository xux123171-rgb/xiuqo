...
This optional skill gives Xiuqo practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${XIUQO_HOME:-~/.xiuqo}/.env`
...
It does **not** turn Xiuqo into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Xiuqo memory
...
### 1) "I want Xiuqo to own a real phone number"
Use **Twilio**.
...
- pairs well with Xiuqo `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${XIUQO_HOME:-~/.xiuqo}/.env`
...
```bash
SCRIPT="$(find ~/.xiuqo/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
xiuqo skills search telephony
xiuqo skills install official/productivity/telephony
```
...
Then save credentials into Xiuqo:
...
3. Buy it and save it into `${XIUQO_HOME:-~/.xiuqo}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Xiuqo calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Xiuqo's existing `text_to_speech` support.

Use this when:
- you want the call to use Xiuqo's configured TTS voice rather than Twilio `<Say>`
...
Recommended Xiuqo TTS -> Twilio Play workflow:

1. Generate the audio with Xiuqo `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Xiuqo to create the message audio with `text_to_speech`
...
Important note:
- Xiuqo TTS is great for prerecorded outbound messages
...
- Xiuqo STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Xiuqo memory.
...
- Do not store arbitrary third-party phone numbers in Xiuqo memory.
...
3. persist that number to `${XIUQO_HOME:-~/.xiuqo}/.env`
