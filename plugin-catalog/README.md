# Xiuqo Plugin Catalog

Curated, Nous-approved Xiuqo plugins. Each YAML file in this directory
(except `removed.yaml`) is one catalog entry, discoverable via
`xiuqo plugins catalog` / `xiuqo plugins search` and installable with
`xiuqo plugins install <name>`.
...
1. **Human-merged gate.** Entries are added *only* via a PR to the
   `xiuqo` repository, reviewed and merged by a maintainer. There is
...
3. **Pin maturity.** The pinned release should be **at least 2 weeks old**
   ...
   compromised release before Xiuqo ships a pointer to it.
