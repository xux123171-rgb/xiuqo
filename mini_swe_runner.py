#!/usr/bin/env python3
"""SWE Runner with Xiuqo Trajectory Format

Runs tool-calling agent tasks in Xiuqo-Agent's execution environments (local,
docker, modal) and writes trajectories in Xiuqo format (from/value pairs with
...
def create_environment(env_type: str = "local", image: str = "python:3.11-slim", cwd: str = "/tmp", timeout: int = 60, **kwargs):
    """Create a Xiuqo execution environment (``local`` ignores ``image``/``kwargs``)."""
    if env_type == "local":
...
class MiniSWERunner:
    """Tool-calling agent loop over a Xiuqo execution environment, emitting Xiuqo trajectories."""
    ...
    def _convert_to_hermes_format(self, messages: List[Dict[str, Any]], user_query: str) -> List[Dict[str, Any]]:
        """Convert the OpenAI-style message list to the Xiuqo trajectory format used by batch_runner.py."""
        system_msg = XIUQO_SYSTEM_PREFIX + f"<tools>\n{self._format_tools_for_system_message()}\n</tools>\n" + XIUQO_SYSTEM_SUFFIX
...
def main(
    ...
    """
    Run SWE tasks with Xiuqo trajectory format output.
    ...
    """
    print("🚀 Mini-SWE Runner with Xiuqo Trajectory Format")
    print("=" * 60)
