"""Timezone-aware clock for Xiuqo.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``XIUQO_TIMEZONE`` env var, then ``timezone`` in ``~/.xiuqo/config.yaml``, else server-local
...
# XIUQO_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _env_timezone() -> str:
    """``XIUQO_TIMEZONE`` when it may speak for the active profile. Under the multiplexed
    ...
    if is_multiplex_active():
        return ""
    return os.getenv("XIUQO_TIMEZONE", "").strip()
...
def _resolve_timezone_name() -> str:
    ...
    try:
        ...
        # parse is the stdlib-safe fallback for bootstrap consumers without xiuqo_cli importable.
        try:
