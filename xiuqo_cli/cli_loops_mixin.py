...
class CLILoopsMixin:
    ...
    def _cmd_version(self, cmd_original: str):
        from xiuqo_cli.main import _print_version_info
        _print_version_info(check_updates=True)

    def _cmd_reload(self, cmd_original: str):
        from xiuqo_cli.config import reload_env
        count = reload_env()
    ...
    def _cmd_plugins(self, cmd_original: str):
        ...
        try:
            # Discover from disk (bundled + user) like `xiuqo plugins list`, so
            ...
            if bundled_count:
                print(f"  (+{bundled_count} bundled — see: xiuqo plugins list)")
            print("  Enable/disable: xiuqo plugins enable/disable <name>")
    ...
    def _get_heartbeat_manager(self):
        ...
        def load():
            from xiuqo_cli.heartbeat import HeartbeatManager
    ...
    def _start_heartbeat_watchdog(self):
        ...
        from xiuqo_cli.heartbeat import POLL_SECONDS
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        _active_deleg = 0
        try:
            from xiuqo_cli.goals import count_active_delegations, gather_background_processes as _gather_bg
