...
def _resolve_busy_timeout_ms() -> int:
    ...
    return _kb._env_int("XIUQO_KANBAN_BUSY_TIMEOUT_MS", DEFAULT_BUSY_TIMEOUT_MS, minimum=1)
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``xiuqo gateway run --replace`` / ``gateway restart`` invoked from a shell
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from xiuqo_cli.sqlite_safe_read import read_header_bytes_preopen
...
def repair_db(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> RepairResult:
    ...
    ``xiuqo kanban repair`` can pick its exit code. ``OperationalError``
...
def _open_configured(path: Path, under_lock) -> tuple[sqlite3.Connection, Any]:
    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            # DELETE with one ERROR log (see xiuqo_state_wal._WAL_INCOMPAT_MARKERS).
...
def connect(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> sqlite3.Connection:
    ...
    :func:`kanban_db_path` (``XIUQO_KANBAN_DB`` -> ``XIUQO_KANBAN_BOARD`` ->
    ...
    # holder (e.g. an external `xiuqo kanban list` mid-integrity-probe) block
    ...
    with _cross_process_init_lock(path):
        ...
        # See #12508.
        from xiuqo_state import preflight_db_writability
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    ...
    if getattr(conn, "in_transaction", False):
        ...
        savepoint = f"xiuqo_nested_{secrets.token_hex(8)}"
