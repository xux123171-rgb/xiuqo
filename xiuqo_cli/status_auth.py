"""Credential sections of `xiuqo status`, run through ``status._SECTIONS`` with its shared context.
Origin helpers (``_row``, ``_first_env_value``, ...) are resolved through the ``xiuqo_cli.status``
...
_OAUTH_BLOCKS = (
    ...
    ("OpenAI Codex", "get_codex_auth_status", "xiuqo model", _FILE_REFRESH_ROWS),
    ...
    ("MiniMax OAuth", "get_minimax_oauth_auth_status", "xiuqo auth add minimax-oauth", (
        ("Region:", "region", None, True),
        ("Access exp:", "expires_at", None, None),
        ("Error:", "error", None, False))),
    ("xAI OAuth", "get_xai_oauth_auth_status", "xiuqo auth add xai-oauth", _FILE_REFRESH_ROWS))
...
def _render_auth_providers(ctx):
    ...
    try:
        # Read-only display: the refresh-free snapshot, so `xiuqo status` never performs an OAuth
    ...
    _status._row("Nous Portal", logged_in,
         ...
         else "not logged in (run: xiuqo portal)")
...
def _render_apikey_providers(ctx):
    ...
    for pname, env_vars in _APIKEY_PROVIDERS.items():
        ...
        _status._row(pname, configured, "configured" if configured else "not configured (run: xiuqo model)", 16, " ")
    ...
    if _status._effective_provider_label() == "LM Studio":
        from xiuqo_cli.models_local import probe_lmstudio_models
...
import xiuqo_cli.status as _status  # noqa: E402  (bottom: xiuqo_cli.status imports this module)
