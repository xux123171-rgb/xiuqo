"""Xiuqo update pipeline: dispatchers (``_cmd_update_impl``/``_cmd_update_check``) + git plumbing.

Each concern lives in ``update_cmd_<concern>.py`` and is re-imported here so
``xiuqo_cli.update_cmd.<name>`` keeps resolving (and stays monkeypatchable). Imports are one-way:
main -> update_cmd -> update_cmd_*; ``_m()`` resolves ``xiuqo_cli.main`` at call time.
"""
...
from xiuqo_cli.config import get_hermes_home  # noqa: F401  (re-exported; patched via update_cmd)
from xiuqo_cli.update_cmd_common import _best_effort
from xiuqo_constants import get_default_hermes_root, venv_python_path

# Re-exports: every split-module name stays reachable (and monkeypatchable) as update_cmd.<name>.
from xiuqo_cli.update_abort_recovery import (  # noqa: F401
...
from xiuqo_cli.update_cmd_windows import (  # noqa: F401
...
from xiuqo_cli.update_cmd_fleet import (  # noqa: F401
...
from xiuqo_cli.update_cmd_zip import (  # noqa: F401
...
from xiuqo_cli.update_cmd_stash import (  # noqa: F401
...
from xiuqo_cli.update_cmd_config import (  # noqa: F401
...
from xiuqo_cli.update_cmd_deps import (  # noqa: F401
...
from xiuqo_cli.update_cmd_git import (  # noqa: F401
...
from xiuqo_cli.update_cmd_maint import (  # noqa: F401
...
def _m():
    """Lazy ``xiuqo_cli.main`` handle: keeps main-side test patches effective, import one-way."""
    from xiuqo_cli import main
    return main
...
_UPDATE_CRITICAL_FILES = (
    "xiuqo_cli/main.py", "xiuqo_cli/config.py", "xiuqo_cli/__init__.py",
    "xiuqo_cli/web_server.py", "cli.py", "run_agent.py", "model_tools.py", "toolsets.py",
    "xiuqo_constants.py")
...
def _record_update_step(step: str, ok: bool, detail: str = "") -> None:
    ...
    with suppress(Exception):
        from xiuqo_cli.update_receipt import record_step
        record_step(step, ok, detail)


# A fetch whose transport dead-stalls (HTTP/2 to GitHub on some networks, a black-holed proxy)
# otherwise leaves `xiuqo update` on "Fetching updates..." forever (#93759, #95777). Five
...
def _validate_python_files_syntax(root, relpaths) -> tuple[bool, str | None, str | None]:
    ...
    import tempfile
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="xiuqo-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    from xiuqo_constants import get_hermes_home  # noqa: F811  (deliberate: constants variant)
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    See #87331.
    """
    print("✗ Cannot continue the update: live Xiuqo launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["xiuqo.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Xiuqo")
    print("  Desktop, a gateway, or another xiuqo REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `xiuqo` command to finish it automatically.")
...
def _format_concurrent_instances_message(matches: list[tuple[int, str]], scripts_dir: Path) -> str:
    """Explanation + remediation hint for the Windows concurrent-xiuqo.exe gate."""
    shim = scripts_dir / "xiuqo.exe"
    lines = [
        "✗ Another xiuqo.exe is running:",
        *(f"    PID {pid}  {name}" for pid, name in matches),
        "",
        f"  Updating now would fail to overwrite {shim} because",
        "  Windows blocks REPLACE on a running executable.",
        "",
        "  Close Xiuqo Desktop, exit any open `xiuqo` REPLs, and",
        "  stop the gateway (`xiuqo gateway stop`) before retrying.",
        ""]
    if matches:
        ...
        lines += [
            ...
            ""]
    lines += [
        "  Override with `xiuqo update --force` if you've already",
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from xiuqo_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _run_logged_subprocess(cmd, *, cwd=None, env=None):
    ...
    import codecs
    import io
    from xiuqo_cli._subprocess_compat import kill_process_tree, windows_hide_flags
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """``xiuqo update --check``: fetch and report without installing. ``branch_explicit`` is
    ...
    from xiuqo_cli.update_contract import evaluate_update_admission, record_refusal_receipt
    ...
    from xiuqo_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        from xiuqo_cli.gitlock import repair_broken_shallow_boundaries, prune_stale_shallow_grafts
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            return
        from xiuqo_cli.banner import _github_compare_behind
...
def _repair_venv_on_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, desktop_dir,
    ...
    _write_update_incomplete_marker()
    from xiuqo_cli.managed_uv import ensure_uv
    repair_uv = ensure_uv()
    ...
    if not healthy_after:
        ...
        print("  Close all Xiuqo windows/gateways and re-run: xiuqo update")
        return False
...
def _pip_install_prefix(uv_bin) -> tuple[list[str], dict | None]:
    ...
    if uv_bin:
        ...
        # resolves here.
        # See #83914.
        from xiuqo_cli.managed_uv import managed_python_env
...
def _repair_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, desktop_dir,
    ...
    from xiuqo_cli.managed_uv import ensure_uv, update_managed_uv
    runtime_repairs = []
    ...
    if handed_off_sync:
        print("→ Finishing the dependency install handed off by xiuqo.exe...")
    elif not healthy:
    ...
    if runtime_repaired is not None and not _m()._is_windows():
        ...
        print(
            "  Any running Xiuqo gateways, Desktop backends, or other "
...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    ...
    if not has_common_ancestor and pre_pull_sha:
        ...
        rescue_ref = (
            f"refs/xiuqo-update-backups/orphan-{branch}-"
...
def _rollback_if_pulled_syntax_error(git_cmd, pre_pull_sha) -> None:
    ...
    if pre_pull_sha:
        ...
        if rollback_result.returncode == 0:
            ...
            print("  Try ``xiuqo update`` again later once a fix lands.")
        else:
...
def _pull_updates(
    ...
    # critical-path file (PR #28452 incident: orphan merge-conflict markers in xiuqo_cli/config.py bricked
    # every user who ran ``xiuqo update`` for the 7 minutes between the bad commit and the fix landing).
...
class _UpdateOptions:
    """Resolved ``xiuqo update`` inputs (flags, config, pre-update snapshots)."""
...
def _begin_update_receipt_and_plan(args):
    ...
    pre-update plan (None if the probe failed); ``sys.exit(2)`` when a non-gateway xiuqo.exe
    holds the venv shim."""
    ...
    with _best_effort('Update receipt unavailable: %s'):
        ...
        from xiuqo_cli.update_receipt import begin_update_receipt
    ...
    # Plan phase (#91277 Phase 2): snapshot the pre-update fleet — every running Xiuqo runtime, its
    ...
    with _best_effort('Update plan phase failed: %s'):
        from xiuqo_cli.update_inventory import collect_runtime_inventory, record_plan_in_receipt
    ...
    # Windows: another xiuqo.exe holding the venv shim means WinError 32 spam and a
...
def _prepare_git_command() -> tuple[bool, list, bool]:
    ...
    if use_zip_update and sys.platform != "win32":
        ...
        print("  curl -fsSL https://raw.githubusercontent.com/xux123171-rgb/xiuqo/main/scripts/install.sh | bash")
        sys.exit(1)
...
def _verify_head_after_pull(
    ...
    # step re-detaches to the SHA). Before this guard, ``xiuqo update`` printed "✓ Code updated!" and
    # reinstalled deps + rebuilt the desktop app against the stale tree — no error, no warning, ``xiuqo
    ...
    if pre_pull_sha and post_pull_sha == pre_pull_sha:
        ...
        print(
            ...
            f"git -C {_m().PROJECT_ROOT} checkout {branch} && xiuqo update")
        _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
        sys.exit(1)
        ...
        if _called_process_error_is_python_dep_install(e):
            ...
            if _m()._is_windows():
                ...
                print(
                    ...
                    '"from xiuqo_cli.main import main; main()" update --yes')
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # Any venv python still running (typically the Desktop `xiuqo serve` backend) keeps .pyd
    ...
    # Self-lock deferral moved: the venv-holder sweep above excludes this process by design (a CLI `xiuqo
