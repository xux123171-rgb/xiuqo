#!/usr/bin/env python3
"""Skills Hub CLI — Unified interface for the Xiuqo Skills Hub."""
...
# serves everything from "xiuqo-index", so that entry MUST exceed the whole catalog or browse
...
_BROWSE_LIMITS = {
    "xiuqo-index": 1000000, "official": 200, "skills-sh": 200, "well-known": 50,
    "github": 200, "clawhub": 500, "lobehub": 500, "browse-sh": 500}
# Programmatic (TUI gateway) browse keeps its own, lower caps.
_BROWSE_API_LIMITS = {
    "xiuqo-index": 5000, "official": 100, "skills-sh": 100, "well-known": 25,
...
def _ident_col(style: str) -> tuple:
    ...
    # copy into `xiuqo skills install`.
...
def _existing_categories() -> List[str]:
    """Sorted category buckets under ``~/.xiuqo/skills/`` (children without their own SKILL.md)."""
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print("[bold]Category[/] "
                "[dim](optional — press Enter to install flat at ~/.xiuqo/skills/<name>/)[/]")
...
def do_search(query: str, source: str = "all", limit: int = 10, console: Optional[Console] = None,
    ...
    c.print(table)
    c.print("[dim]Use: xiuqo skills inspect <identifier> to preview, "
            "xiuqo skills install <identifier> to install "
            "(--json for scripting)[/]\n")
...
def _render_browse_page(c: Console, deduped, page_items, page: int, total_pages: int,
    ...
    c.print("[dim]Tip: 'xiuqo skills inspect <identifier>' to preview, "
            "'xiuqo skills install <identifier>' to install, "
            "'xiuqo skills search <query>' to search deeper[/]\n")
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    # xiuqo-index AND the external APIs at once would double-count every skill.
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if preview is not None:
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="xiuqo skills install <id> to install"))
    c.print()
...
def _resolve_url_bundle_name(c: Console, bundle, meta, identifier: str,
    ...
    if name_override and _is_valid_installed_skill_name(name_override):
        ...
        c.print(f"[bold red]Cannot install from URL:[/] {url}\n"
                ...
                f"  [bold]xiuqo skills install {url} --name <your-name>[/]\n\n"
...
def _announce_blueprint(c: Console, skill_name: str) -> None:
    """Offer an installed skill's ``metadata.xiuqo.blueprint`` via /suggestions — never
    ...
    try:
        ...
        if register_blueprint_suggestion(spec) is not None:
            ...
            c.print("[dim]You can still schedule it any time by asking the agent "
                    "or via[/] [bold]xiuqo cron add[/][dim].[/]\n")
...
def _print_fetch_failure(c: Console, sources, identifier: str, meta=None, source=None) -> None:
    ...
    if meta is not None and not rate_limited:
        ...
        c.print("[dim]Stale index entry: the skill was likely renamed or removed by "
                "its author. Try `xiuqo skills search` for an alternative.[/]\n")
        return
...
_INSTALL_PANELS = {
    "official": (
        ...
        "It ships with xiuqo but is not activated by default.\n"
...
def do_list(source_filter: str = "all", enabled_only: bool = False,
    ...
    profile's config — ``-p`` swaps XIUQO_HOME at process start, so no profile flag here."""
    from tools.skills_hub import HubLockFile, ensure_hub_dirs
...
def do_check(name: Optional[str] = None, console: Optional[Console] = None) -> None:
    ...
    if orphaned:
        c.print(f"[yellow]Orphaned:[/] {', '.join(orphaned)} — lock-file entries whose local "
                ...
                "remove the stale entry with: xiuqo skills uninstall <name>\n")
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    Mirrors the user-modified protection bundled skills already get from ``xiuqo update`` (ported from
    ...
    if skipped_local:
        ...
        c.print("[dim]Overwrite with: xiuqo skills update <name> --force[/]\n")
...
def do_list_modified(console: Optional[Console] = None, as_json: bool = False) -> None:
    """List bundled skills the user has edited (which `xiuqo update` keeps)."""
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `xiuqo update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   xiuqo skills diff <name>[/]")
    c.print("[dim]Resume updates: xiuqo skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: xiuqo skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    c.print()
    c.print(f"[dim]Revert with: xiuqo skills reset {name} --restore[/]\n")
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            _print_error(c, f"Repo required. Usage: xiuqo skills tap {action} owner/repo")
            return
    ...
    if target == "github":
        if not repo:
            _print_error(c, "--repo required for GitHub publish.\n"
                            "Usage: xiuqo skills publish <path> --to github --repo owner/repo")
            return
        auth = GitHubAuth()
...
def _github_publish(skill_path: Path, skill_name: str, target_repo: str, auth) -> tuple:
    ...
    try:
        resp = call("post", f"{target_repo}/pulls", json={
            ...
            "body": f"Submitting the `{skill_name}` skill via Xiuqo Skills Hub.\n\n"
                    f"This skill was scanned by the Xiuqo Skills Guard before submission.",
...
def _snapshot_cli(args) -> None:
    ...
    if snap_action == "export":
        ...
        _console.print("Usage: xiuqo skills snapshot [export|import]\n")


def _tap_cli(args) -> None:
    tap_action = getattr(args, "tap_action", None)
    if not tap_action:
        _console.print("Usage: xiuqo skills tap [list|add|remove]\n")
        return
    do_tap(tap_action, repo=getattr(args, "repo", "") or getattr(args, "name", ""))


# `xiuqo skills <action>` -> handler(args). Lambdas late-bind the do_* names so
# tests that patch("xiuqo_cli.skills_hub.do_install") still intercept.
_CLI_ACTIONS = {
...
def skills_command(args) -> None:
    """Router for `xiuqo skills <subcommand>` — called from xiuqo_cli/main.py."""
    handler = _CLI_ACTIONS.get(getattr(args, "skills_action", None))
    if handler is None:
        _console.print("Usage: xiuqo skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
        _console.print("Run 'xiuqo skills <command> --help' for details.\n")
        return
    handler(args)
