...
from xiuqo_cli.active_sessions import active_session_registry_snapshot, session_already_owned_message
...
def configure_tui_attachment(env: dict[str, str], session_id: str | None, *,
    ...
    if not session_id or env.get("XIUQO_TUI_GATEWAY_URL", "").strip():
        return
    url = discover_attach_url(session_id, registry_home=registry_home)
    if url is not None:
        env["XIUQO_TUI_GATEWAY_URL"] = url
