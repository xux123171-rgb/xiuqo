"""Migrate Xiuqo MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
MIGRATION_MARKER = (
    "# managed by xiuqo — `xiuqo codex-runtime migrate` regenerates this section")
MIGRATION_END_MARKER = "# end xiuqo managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Xiuqo config.")
...
# Xiuqo MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown Xiuqo keys.
_KNOWN_XIUQO_KEYS = {
...
# (xiuqo key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, xiuqo_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one Xiuqo MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. Xiuqo' ``transport: sse`` hint is
    informational only — codex auto-negotiates. ``enabled`` is emitted only when explicitly false
    (codex defaults to true).
    """
    if not isinstance(xiuqo_cfg, dict):
        return None, []
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if xiuqo_cfg.get("command"):
        if xiuqo_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(xiuqo_cfg["command"])
        if xiuqo_cfg.get("args"):
            out["args"] = [str(a) for a in xiuqo_cfg["args"]]
        if xiuqo_cfg.get("env"):
            out["env"] = _str_map(xiuqo_cfg["env"])
        if xiuqo_cfg.get("cwd"):
            out["cwd"] = str(xiuqo_cfg["cwd"])
    elif xiuqo_cfg.get("url"):
        out["url"] = str(xiuqo_cfg["url"])
        if xiuqo_cfg.get("headers"):
            out["http_headers"] = _str_map(xiuqo_cfg["headers"])
        if xiuqo_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for xiuqo_key, codex_key, note in _TIMEOUT_KEYS:
        if xiuqo_key in xiuqo_cfg:
            try:
                out[codex_key] = float(xiuqo_cfg[xiuqo_key])
            except (TypeError, ValueError):
                skipped.append(note)
    if xiuqo_cfg.get("enabled") is False:
        out["enabled"] = False
    for key in xiuqo_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_XIUQO_KEYS:
            skipped.append(f"{key} (unknown Xiuqo key)")
    return out, skipped
...
# env-var passthrough (XIUQO_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by Xiuqo)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(codex_home=str(codex_home) if codex_home else None) as client:
            client.initialize(client_name="xiuqo-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a XIUQO_HOME pointing there burned into
...
def _build_xiuqo_tools_mcp_entry() -> dict:
    """Codex stdio entry launching Xiuqo' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    XIUQO_HOME passes through only IF SET, read from os.environ (not get_hermes_home()): when
    unset the codex subprocess must inherit its launcher's runtime XIUQO_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched XIUQO_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched xiuqo finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # XIUQO_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_hermes_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever XIUQO_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's XIUQO_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("XIUQO_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    xiuqo_home = os.environ.get("XIUQO_HOME") or ""
    if xiuqo_home and not _looks_like_test_tempdir(xiuqo_home):
        env["XIUQO_HOME"] = xiuqo_home
    if os.environ.get("PYTHONPATH"):
        env["PYTHONPATH"] = os.environ["PYTHONPATH"]
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["XIUQO_QUIET"] = "1"
    env["XIUQO_REDACT_SECRETS"] = env.get("XIUQO_REDACT_SECRETS", "true")
    return {
        "command": sys.executable,
        "args": ["-m", "agent.transports.xiuqo_tools_mcp_server"],
        "env": env,
...
def migrate(
    xiuqo_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_xiuqo_tools: bool = True) -> MigrationReport:
    """Translate Xiuqo mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_xiuqo_tools`` registers Xiuqo' own tool surface
    (agent/transports/xiuqo_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    xiuqo_servers = (xiuqo_config or {}).get("mcp_servers") or {}
    if not isinstance(xiuqo_servers, dict):
        report.errors.append("mcp_servers in Xiuqo config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in xiuqo_servers.items():
        name = str(raw_name)
    ...
    if expose_xiuqo_tools:
        translated["xiuqo-tools"] = _build_xiuqo_tools_mcp_entry()
        if "xiuqo-tools" not in report.migrated:
            report.migrated.append("xiuqo-tools")
