"""External-tool checks for xiuqo doctor: terminal backends, git/rg, Node + agent-browser, npm audit, tool availability.
...
import subprocess
import sys
from xiuqo_cli.doctor_platform import _system_package_install_cmd
from xiuqo_cli.doctor_report import Finding, _fail_and_issue, check_bool, check_info, check_ok, check_warn, doctor_check
from xiuqo_cli.vercel_auth import describe_vercel_auth
from xiuqo_constants import agent_browser_runnable, is_termux as _is_termux
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    return (item.get("name") == "kanban" and not os.environ.get("XIUQO_KANBAN_TASK")
            and bool(tools) and all(str(tool).startswith("kanban_") for tool in tools))


def _doctor_tool_availability_detail(toolset: str) -> str:
    """Optional explanatory suffix for toolsets whose doctor status needs context."""
    if toolset == "kanban" and not os.environ.get("XIUQO_KANBAN_TASK"):
...
def _apply_doctor_tool_availability_overrides(available: list[str], unavailable: list[dict]) -> tuple[list[str], list[dict]]:
    ...
    from xiuqo_cli.doctor_state import _honcho_is_configured_for_doctor
...
_TOOLSET_SETUP_HINTS: dict[str, str] = {
    "image_gen": "(image generation unavailable — check the provider selection and its key or SDK with 'xiuqo tools')",
}
...
def _check_vercel_backend(issues: list[str]) -> None:
    ...
    _require(importlib.util.find_spec("vercel") is not None, ("vercel SDK", "(installed)"),
             ("vercel SDK not installed", "(pip install 'xiuqo[vercel]')"),
             "Install the Vercel optional dependency: pip install 'xiuqo[vercel]'", issues)
...
def _check_terminal_backend(should_fix: bool, f: Finding) -> None:
    ...
    try:
        from xiuqo_constants import is_container as _is_container
...
def _check_agent_browser(should_fix: bool) -> bool:
    ...
    Mirrors ``tools.browser_tool_install._find_agent_browser``'s own cascade (lazy npx or a global/Xiuqo-managed
    ...
    try:
        ...
        # (or a global/Xiuqo-managed install) at first use.
    ...
    if resolved and _is_npx_agent_browser_sentinel(resolved):
        ...
        if should_fix:
            # Can't tell whether npx's cache is warm — fire the same warm-up `xiuqo update` does.
    ...
    if resolved and agent_browser_runnable(resolved):
        ...
        return True
    if resolved:
        # Almost always a dangling global symlink left by npm postinstall after `xiuqo update` wiped node_modules.
...
def _check_lightpanda() -> None:
    ...
    if not used:
        ...
        check_info("Fix: pick Lightpanda in `xiuqo tools` → Browser Automation, or set browser.engine: auto")
...
def _check_npm_audit(should_fix: bool, f: Finding) -> None:
    ...
    XIUQO_HOME mirror rather than the (possibly read-only) Docker install tree, hence the shared resolver.
    """
    from xiuqo_cli.doctor import PROJECT_ROOT
    ...
    if npm_bin:
        try:
            ...
            # See #38772. The WhatsApp bridge may live under a writable XIUQO_HOME mirror instead of the
...
def _check_tool_availability(should_fix: bool, f: Finding) -> None:
    ...
    if api_disabled or any(status != "ok" for status, _, _ in web_rows):
        f.issues.append("Run 'xiuqo setup' to configure missing API keys for full tool access")
