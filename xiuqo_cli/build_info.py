"""Baked-in build metadata for Xiuqo Agent.

Source installs report their git revision live via ``git rev-parse`` (``xiuqo_cli/dump.py``,
``xiuqo_cli/banner.py``). Inside the published Docker image ``.dockerignore`` excludes ``.git``,
so those callsites fall back to the ``.xiuqo_build_sha`` file written by the Dockerfile.
"""
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".xiuqo_build_sha"
...
def get_code_identity(refresh: bool = False) -> dict:
    ...
    ``.xiuqo_build_sha`` for Docker images, else unknown. Cached per process — code identity
...
def get_build_sha(short: int = 8) -> Optional[str]:
    """Baked-in build SHA (Dockerfile ``XIUQO_GIT_SHA`` build-arg, full 40-char hash on one
