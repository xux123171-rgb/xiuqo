...
_DEFAULT_PLATFORM_TOOLSETS = {"cli": "xiuqo-cli"}
...
def _has_agent_browser() -> bool:
    ...
    from xiuqo_constants import agent_browser_runnable
    ...
    try:
        ...
        # Xiuqo-managed Node dirs ($XIUQO_HOME/node, prepended to PATH at runtime but usually absent
...
def _local_stt_backend_available() -> bool:
    ...
    if get_env_value("XIUQO_LOCAL_STT_COMMAND"):
        return True
    try:
...
def prompt_enable_tool_gateway(config: Dict[str, object], *, force_fresh: bool = True) -> set[str]:
    ...
    if not unconfigured and not has_direct:
        return set()
    try:
        from xiuqo_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# Inline Nous Portal login for the Tool Gateway picker (`xiuqo tools`)
