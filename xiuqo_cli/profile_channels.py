...
gateways fight over one bot (standalone) or blocks ``xiuqo gateway migrate --multiplex`` with a
...
def shared_channel_credentials(profile_dir: Path, source_dir: Path) -> List[str]:
    ...
    manager, no gateway config load, so ``xiuqo profile list`` can afford it per profile."""
...
def shared_credential_warning(profile: str, platforms: List[str], source: str = "default") -> str:
    return (
        ...
        f"only belong to one profile. Give '{profile}' its own bot (xiuqo -p {profile} setup, or the "
        f"dashboard Messaging page) or remove the token from '{profile}'; a multiplexed gateway parks "
        f"the duplicate and `xiuqo gateway migrate --multiplex` refuses until it is gone."
    )
...
def format_stripped_notice(profile: str, platforms: List[str], clone_flag: str = "--clone") -> List[str]:
    ...
    return [
        ...
        f"  Configure this profile's own bots:  xiuqo -p {profile} setup   (or the dashboard Messaging page)",
        f"  To copy the source's channels anyway:  xiuqo profile create {profile} {clone_flag} --clone-channels",
    ]
