...
def configure_windows_stdio() -> bool:
    ...
    ``XIUQO_DISABLE_WINDOWS_UTF8=1`` to opt out (forces the old cp1252 path for diagnosing
    ...
    if _CONFIGURED:
        return False
    if not is_windows() or os.environ.get("XIUQO_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
...
def _augment_path_with_known_tools() -> None:
    r"""Prepend Xiuqo-managed tool directories to ``PATH`` (no-op on POSIX / missing dirs).

    install.ps1 adds entries like ``%LOCALAPPDATA%\xiuqo\git\bin`` to the User PATH via
    ``SetEnvironmentVariable``, but already-running shells never see that broadcast, so a xiuqo
    ...
    # dir hosts xiuqo.exe + pip console scripts; WinGet\Links is where ``winget install`` drops
    # CLI shims (ripgrep lands there as rg.exe).
    candidate_dirs = [
        os.path.join(local_appdata, "xiuqo", "git", "cmd"),
        os.path.join(local_appdata, "xiuqo", "git", "bin"),
        os.path.join(local_appdata, "xiuqo", "git", "usr", "bin"),
        os.path.join(local_appdata, "xiuqo", "xiuqo", "venv", "Scripts"),
