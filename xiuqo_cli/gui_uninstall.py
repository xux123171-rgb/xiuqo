"""Xiuqo Desktop (Chat GUI) uninstaller: removes only GUI state — built Electron artifacts, the packaged
...
def desktop_userdata_dir() -> Path:
    """Electron ``app.getPath('userData')`` for an app named "Xiuqo" on each platform (GUI-only state)."""
    home = Path.home()
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Xiuqo"
    if sys.platform == "win32":
        return _env_dir("APPDATA", home / "AppData" / "Roaming") / "Xiuqo"
    return _env_dir("XDG_CONFIG_HOME", home / ".config") / "Xiuqo"


def source_built_gui_artifacts(xiuqo_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``xiuqo desktop`` inside the checkout (same ``xiuqo/`` layout
    ...
    return [desktop_dir / "dist", desktop_dir / "release", desktop_dir / "node_modules",
            agent_root / "node_modules", xiuqo_home / "desktop-build-stamp.json"]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Xiuqo" product."""
    home = Path.home()
    if sys.platform == "darwin":
        return [Path("/Applications/Xiuqo.app"), home / "Applications" / "Xiuqo.app"]
    if sys.platform == "win32":
        ...
        return [local_base / "Programs" / "Xiuqo", local_base / "xiuqo-desktop"] + (
            [Path(program_files) / "Xiuqo"] if program_files else [])
    ...
    from xiuqo_cli.linux_desktop_entry import desktop_entry_path
    ...
    return [desktop_entry_path(), data_base / "applications" / "Xiuqo.desktop"] + [
        icons / size / "apps" / "xiuqo.png"
        for size in ("scalable", "24x24", "32x32", "48x48", "256x256", "512x512", "1024x1024")]


def agent_is_installed(xiuqo_home: Path) -> bool:
    """True when a usable Python agent install exists under XIUQO_HOME (gates the desktop UI's options).
    Package source or a venv alone is enough — a source checkout without a venv is still "the agent is here"."""
    return any((xiuqo_home / "xiuqo" / sub).is_dir() for sub in ("xiuqo_cli", "venv", ".venv"))


def gui_is_installed(xiuqo_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    return any(p.exists() for p in (*source_built_gui_artifacts(xiuqo_home), *packaged_gui_app_paths(), desktop_userdata_dir()))


def gui_install_summary(xiuqo_home: "Path | None" = None) -> dict:
    """JSON-serializable snapshot of what's installed, for the desktop UI to render via IPC."""
    home: Path = xiuqo_home if xiuqo_home is not None else get_hermes_home()
    userdata = desktop_userdata_dir()
    return {"xiuqo_home": str(home), "agent_installed": agent_is_installed(home),
...
def uninstall_gui(xiuqo_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        # The desktop entry was removed above but the menu caches still list it; reindex so Xiuqo
        # disappears from the launcher.
        try:
            from xiuqo_cli.linux_desktop_entry import desktop_entry_path, refresh_desktop_databases
        ...
        log_info("If you installed the desktop via a .deb / .rpm package, remove it with your package manager "
                 "(e.g. 'sudo apt remove xiuqo' or 'sudo dnf remove xiuqo'). AppImage builds are a single "
