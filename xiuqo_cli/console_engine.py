"""Safe Xiuqo Console command engine."""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (prev.startswith("Run 'xiuqo doctor'") and last.startswith("Run 'xiuqo setup'")):
        return text.rstrip()
    lines = lines[:-2]
    _drop_trailing_blank(lines)
...
def _split_line(line: str) -> list[str]:
    ...
    # See #78293.
    # See #83934.
    from xiuqo_cli._subprocess_compat import split_command_line
    try:
...
def _clean_summary(text: str | None) -> str:
    ...
    return "" if summary.startswith("Run `xiuqo ") else summary
...
class _CliSurface:
    ...
    ``extracted``: ``builder(subparsers, <handler>=fn)``, fn from xiuqo_cli.main.
    ``registered``: ``register(subparsers.add_parser(root))``, optional module ``handler``.
    ``builder``: ``top = builder(subparsers)``, func from xiuqo_cli.main.
    ``adder``: self-wiring."""
    ...
    def build(self, root: str, *, live: bool) -> _ArgumentParser:
        ...
        parser = _ArgumentParser(prog="xiuqo", add_help=False)
        subparsers = parser.add_subparsers(dest="_console_command")
        module = importlib.import_module(self.module)
        entry = getattr(module, self.builder)
        main_handler = (
            (lambda: getattr(importlib.import_module("xiuqo_cli.main"), self.handler))
...
def _sub(module: str, builder: str, handler: str) -> _CliSurface:
    return _CliSurface("extracted", f"xiuqo_cli.subcommands.{module}", builder, handler)


def _reg(module: str, handler: str | None = None) -> _CliSurface:
    return _CliSurface("registered", f"xiuqo_cli.{module}", "register_cli", handler)
...
_CLI_FAMILIES: dict[str, tuple[_CliSurface, str]] = {
    ...
    "portal": (_CliSurface("adder", "xiuqo_cli.portal_cli", "add_parser"), "info, tools"),
    "project": (
        _CliSurface("builder", "xiuqo_cli.projects_cmd", "build_parser", "cmd_project"),
        "list, show, *create, *add-folder, *remove-folder, *rename, *set-primary, *use, "
        "*archive, *restore, *bind-board"),
    "kanban": (
        _CliSurface("builder", "xiuqo_cli.kanban", "build_parser", "cmd_kanban"),
...
_SEND_SURFACE = _CliSurface("adder", "xiuqo_cli.send_cmd", "register_send_subparser")
...
def _register_command_family(
    ...
    for child_path, mutating in _paths(paths):
        ...
        engine.register(
            full_path, usage, summaries.get(full_path) or f"Run `xiuqo {usage}`.", handler,
            mutating=mutating, confirmation=f"Run `xiuqo {usage}`?")
...
_BLOCKED_PAIRS = {
    ("config", "edit"): "`config edit` opens an editor and is not available in Xiuqo Console.",
    ("mcp", "serve"): "`mcp serve` starts a server and is not available in Xiuqo Console.",
    ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Xiuqo Console.",
    ("skills", "config"): "`skills config` is interactive and is not available in Xiuqo Console.",
    ("skills", "publish"): "`skills publish` is not available in Xiuqo Console.",
    ("portal", "login"): "`portal login` is interactive and is not available in Xiuqo Console.",
    ("portal", "open"): "`portal open` opens a browser and is not available in Xiuqo Console.",
    ("kanban", "tail"): "`kanban tail` streams output and is not available in Xiuqo Console.",
    ("kanban", "watch"): "`kanban watch` streams output and is not available in Xiuqo Console.",
    ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Xiuqo Console.",
    ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Xiuqo Console.",
    ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Xiuqo Console.",
    ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Xiuqo Console.",
    ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Xiuqo Console.",
    ("kanban", "gc"): "`kanban gc` is not available in Xiuqo Console.",
    ("sessions", "delete"): "`sessions delete` and `sessions prune` are not available in Xiuqo Console.",
    ("sessions", "prune"): "`sessions delete` and `sessions prune` are not available in Xiuqo Console.",
}


class HermesConsoleEngine:
    """Curated line-command executor for Xiuqo Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "xiuqo":
                tokens = tokens[1:]
            if not tokens:
                return ConsoleResult("ok", output=self.help_text())
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Xiuqo Console does not run shell syntax. Use one supported "
                    "Xiuqo command at a time.")
    ...
    def help_text(self, subject: str | None = None) -> str:
        ...
        lines = ["Xiuqo Console", "", "Supported commands:"]
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Xiuqo Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Xiuqo Console."
        if first in _BLOCKED_TOP:
            return f"`xiuqo {first}` is not available in Xiuqo Console."
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError(
                "auth add --type api-key requires --api-key in Xiuqo Console.")
...
_version = _simple_command(
    "version", "xiuqo_cli._startup_fast", "print_fast_version_info", check_updates=True)


def _status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "status")
    from xiuqo_cli.status import show_status
...
_doctor = _simple_command(
    "doctor", "xiuqo_cli.doctor", "run_doctor", lambda: (SimpleNamespace(fix=False, ack=None),))
_config_show = _simple_command("config show", "xiuqo_cli.config", "show_config")
_cron_status = _simple_command("cron status", "xiuqo_cli.cron", "cron_status")


def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Xiuqo Console.")
    ns = _parse(
    ...
    from xiuqo_cli.logs import list_logs, tail_log
    if ns.log_name == "list":
...
def _config_path(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from xiuqo_cli.config import get_config_path
...
def _config_migrate(_engine: HermesConsoleEngine, args: list[str]) -> None:
    ...
    from xiuqo_cli.config import migrate_config
...
def _guard_exports(db, session_ids: list[str]) -> None:
    ...
    from xiuqo_state import SessionExportTooLargeError, resolved_max_export_messages
...
def _sessions_repair(_engine: HermesConsoleEngine, args: list[str]) -> int | None:
    ...
    from xiuqo_state import DEFAULT_DB_PATH
    from xiuqo_state_repair import _db_opens_cleanly, repair_state_db_schema
...
def _cron_list(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from xiuqo_cli.cron import cron_list
...
def _cron_pause(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    return _cron_job_action(
        ...
        lambda ref: pause_job(ref, reason="paused from xiuqo console"))
...
_BUILTIN_COMMANDS = (
    (("status",), "status", "Show Xiuqo component status.", _status, ""),
    (("version",), "version", "Show Xiuqo version information.", _version, ""),
    (("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor, ""),
    (("logs",), "logs [name] [-n N]", "Show recent Xiuqo logs.", _logs, ""),
    ...
    (("config", "migrate"), "config migrate", "Update config with new options.", _config_migrate,
     "Update Xiuqo configuration with missing defaults?"),
...
def run_console_repl(
    *, stdin=None, stdout=None, stderr=None, interactive: bool | None = None) -> int:
    """Run the local ``xiuqo console`` REPL."""
    ...
    say("Xiuqo Console. Type `help` for commands, `exit` to quit.")
    while True:
        say("xiuqo> ", end="", flush=True)
        line = stdin.readline()
        if line == "":
            say("")
            return 0
