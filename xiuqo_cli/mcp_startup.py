...
# Discovery slot per profile home (``xiuqo_home_key()`` follows the context-local XIUQO_HOME
...
def _has_configured_mcp_servers() -> bool:
    ...
    try:
        ...
        if isinstance(raw_config.get("mcp_servers"), dict) and raw_config["mcp_servers"]:
            return True
        from xiuqo_cli.agent_plugins import has_enabled_agent_plugin_mcp
...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # the context-local XIUQO_HOME override (multi-profile dashboard/desktop backends, #67605)
...
def _resolve_discovery_timeout(explicit: "float | None", *, single_query: bool = False) -> float:
    ...
    try:
        from xiuqo_cli.config import load_config, DEFAULT_CONFIG
...
def defer_background_mcp_discovery(*, logger, thread_name: str, delay: float) -> None:
    ...
    with _mcp_discovery_lock:
        if xiuqo_home_key() in _mcp_discovery_started or _mcp_discovery_deferred is not None:
            return
...
def _current_home_thread() -> Optional[threading.Thread]:
    ...
    return _mcp_discovery_thread.get(xiuqo_home_key())
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``xiuqo -z``) can construct ``AIAgent`` before any
