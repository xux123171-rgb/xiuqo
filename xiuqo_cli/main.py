#!/usr/bin/env python3
"""Xiuqo CLI - Main entry point.

Usage:
    xiuqo                     # Interactive chat (default)
    xiuqo chat / gateway / setup / status / cron / doctor / update / ...
    xiuqo --version           # Show version and update status
    xiuqo <cmd> --help        # Per-command help
"""

# xiuqo_bootstrap must be the very first import — it sets up UTF-8 stdio on
# Windows (no-op on POSIX). Guarded: after a ``git pull`` / interrupted
# ``xiuqo update`` the editable install's ``.pth`` may not list it yet; crashing
# here would block ``xiuqo update``.
try:
    import xiuqo_bootstrap  # noqa: F401
...
from xiuqo_cli._subprocess_compat import suppress_platform_ver_console
...
# Inline path math so ``python xiuqo_cli/main.py`` (script mode: sys.path[0]
# is xiuqo_cli/, not the repo root) can import xiuqo_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from xiuqo_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below. A prior
# ``xiuqo update`` may have left a recovery marker with a core package wiped;
# the xiuqo_cli.config/env_loader imports further down would then crash before
...
# PID" class it exists for. ``xiuqo_startup_watchdog`` is stdlib-only so it
...
if _argv_is_gateway_run(sys.argv[1:]):
    try:
        from xiuqo_startup_watchdog import arm_startup_watchdog as _arm_sw
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from xiuqo_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Cosmetic: show 'xiuqo' instead of 'python3.xx' in ps/top/htop.

    Order: opt-in ``setproctitle`` dep; ctypes ``prctl(PR_SET_NAME)`` (Linux,
    15-char limit); ``pthread_setname_np`` (macOS — lldb/top only, not ``ps
    aux``); no-op on Windows (the .exe is already ``xiuqo.exe``). Never fatal.
    """
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("xiuqo")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"xiuqo", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"xiuqo")
    except Exception:
        pass
...
# xiuqo_cli.config is importable. Cached so early callers don't re-parse YAML.
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("XIUQO_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".xiuqo", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    Precedence: ``--cli`` wins, then ``--tui``/``XIUQO_TUI=1``, then a
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("XIUQO_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `resetTerminalModes()` is the later cousin. ``XIUQO_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("XIUQO_TUI_NO_EARLY_DISABLE") == "1":
        return
...
# ``xiuqo --version`` is answered before config/logging imports.
...
from xiuqo_cli.subcommands.cron import build_cron_parser
from xiuqo_cli.subcommands.sync import build_sync_parser
from xiuqo_cli.subcommands.gateway import build_gateway_parser
from xiuqo_cli.subcommands.profile import build_profile_parser
from xiuqo_cli.subcommands.model import build_model_parser
from xiuqo_cli.subcommands.setup import build_setup_parser

from xiuqo_cli.subcommands.whatsapp import build_whatsapp_parser, build_whatsapp_cloud_parser
from xiuqo_cli.subcommands.slack import build_slack_parser
from xiuqo_cli.subcommands.login import build_login_parser
from xiuqo_cli.subcommands.logout import build_logout_parser
from xiuqo_cli.subcommands.auth import build_auth_parser
from xiuqo_cli.subcommands.status import build_status_parser
from xiuqo_cli.subcommands.pause import build_pause_parser
from xiuqo_cli.subcommands.webhook import build_webhook_parser
from xiuqo_cli.subcommands.hooks import build_hooks_parser
from xiuqo_cli.subcommands.doctor import build_doctor_parser
from xiuqo_cli.subcommands.verify import build_verify_parser
from xiuqo_cli.subcommands.security import build_security_parser
from xiuqo_cli.subcommands.approvals import build_approvals_parser
from xiuqo_cli.subcommands.dump import build_dump_parser
from xiuqo_cli.subcommands.debug import build_debug_parser
from xiuqo_cli.subcommands.backup import build_backup_parser
from xiuqo_cli.subcommands.import_cmd import build_import_cmd_parser
from xiuqo_cli.subcommands.import_agent import build_import_agent_parser
from xiuqo_cli.subcommands.config import build_config_parser
from xiuqo_cli.subcommands.skin import build_skin_parser
from xiuqo_cli.subcommands.console import build_console_parser
from xiuqo_cli.subcommands.update import build_update_parser
from xiuqo_cli.subcommands.uninstall import build_uninstall_parser
from xiuqo_cli.subcommands.dashboard import build_dashboard_parser, build_serve_parser
from xiuqo_cli.subcommands.gui import build_gui_parser
from xiuqo_cli.subcommands.logs import build_logs_parser
from xiuqo_cli.subcommands.prompt_size import build_prompt_size_parser
from xiuqo_cli.subcommands.memory import build_memory_parser
from xiuqo_cli.subcommands.acp import build_acp_parser
from xiuqo_cli.subcommands.tools import build_tools_parser
from xiuqo_cli.subcommands.insights import build_insights_parser
from xiuqo_cli.subcommands.monitoring import build_monitoring_parser
from xiuqo_cli.subcommands.skills import build_skills_parser
from xiuqo_cli.subcommands.pairing import build_pairing_parser
from xiuqo_cli.subcommands.plugins import build_plugins_parser
from xiuqo_cli.subcommands.mcp import build_mcp_parser
from xiuqo_cli.subcommands.claw import build_claw_parser
from xiuqo_cli.subcommands.vault import build_vault_parser
from xiuqo_cli.subcommands.moa import build_moa_parser
from xiuqo_cli.subcommands.fallback import build_fallback_parser
from xiuqo_cli.subcommands.worktree import build_worktree_parser
from xiuqo_cli.subcommands.browser import build_browser_parser
from xiuqo_cli.subcommands.secrets import build_secrets_parser
from xiuqo_cli.subcommands.egress import build_egress_parser
from xiuqo_cli.subcommands.migrate import build_migrate_parser
from xiuqo_cli.subcommands.checkpoints import build_checkpoints_parser
from xiuqo_cli.subcommands.bundles import build_bundles_parser
from xiuqo_cli.subcommands.curator import build_curator_parser
from xiuqo_cli.subcommands.pets import build_pets_parser
from xiuqo_cli.subcommands.journey import build_journey_parser
from xiuqo_cli.subcommands.computer_use import build_computer_use_parser
from xiuqo_cli.subcommands.sessions import build_sessions_parser
from xiuqo_cli.subcommands.completion import build_completion_parser
...
def _require_tty(command_name: str) -> None:
    ...
    if not sys.stdin.isatty():
        print(
            f"Error: 'xiuqo {command_name}' requires an interactive terminal.\n"
...
# Profile override — MUST happen before any xiuqo module import: many modules
# cache XIUQO_HOME at import time. --profile/-p is pre-parsed from sys.argv,
# XIUQO_HOME set, and the flag stripped so argparse never sees it. Falls back
# to ~/.xiuqo/active_profile for the sticky default.
_PROFILE_NAME_RE = r"^[a-z0-9][a-z0-9_-]{0,63}$"  # mirrors xiuqo_cli.profiles._PROFILE_ID_RE


def _inside_mcp_add_args(argv: list, index: int) -> bool:
    """True once argv reaches `xiuqo mcp add ... --args <command argv>`.
    ...
    ``--profile``), not to Xiuqo' own profile selector.
    """
    try:
...
def _scan_profile_flag(argv: list) -> tuple:
    ...
    Historically the flag worked even after the subcommand (`xiuqo chat -p
    ...
    """
    from xiuqo_cli._parser import top_level_value_flag_sets
...
def _resolve_sudo_user_profile_env(name: str) -> str | None:
    """Resolve `sudo xiuqo -p <name>` against the invoking user's home.
    ...
    if name == "default":
        return None
    from xiuqo_constants import sudo_invoker_default_home
...
def _under_gateway_supervisor(argv: list) -> bool:
    ...
    ``-p <name>`` or pin XIUQO_HOME to the profile dir; a bare invocation
    means "the root XIUQO_HOME profile". If a supervised default-profile
    child read active_profile, switching the active profile (dashboard,
    ``xiuqo profile use``) would silently redirect the default gateway into
    ...
    XIUQO_SUPERVISED_CHILD (systemd unit / launchd plist / Windows task),
    XIUQO_S6_SUPERVISED_CHILD (legacy s6 container), INVOCATION_ID (systemd
    ...
    self-hosted CI runners), XIUQO_GATEWAY_EXTERNAL_SUPERVISOR (explicit
    opt-in). XPC_SERVICE_NAME is deliberately NOT consulted: interactive macOS
    terminals set it too.
    """
    if os.environ.get("XIUQO_SUPERVISED_CHILD") or os.environ.get("XIUQO_S6_SUPERVISED_CHILD"):
        return True
...
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set XIUQO_HOME before imports."""
    argv = sys.argv[1:]
    profile_name, consume, profile_index = _scan_profile_flag(argv)

    # XIUQO_HOME already set with no explicit flag: trust it only when it
    # points at a specific profile dir ("profiles" as immediate parent). If it
    # points at the xiuqo root (systemd hardcodes XIUQO_HOME=/root/.xiuqo)
    # we must still read active_profile — the user may have run
    # `xiuqo profile use` and the gateway should honour it (#22502).
    xiuqo_home_env = os.environ.get("XIUQO_HOME", "")
    if profile_name is None and xiuqo_home_env and Path(xiuqo_home_env).parent.name == "profiles":
        return
    ...
    if profile_name is None:
        return
    try:
        from xiuqo_cli.profiles import resolve_profile_env

        xiuqo_home = resolve_profile_env(profile_name)
    except FileNotFoundError as exc:
        xiuqo_home = _resolve_sudo_user_profile_env(profile_name)
        if not xiuqo_home:
        ...
        # A bug in profiles.py must NEVER prevent xiuqo from starting
        print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
        return
    os.environ["XIUQO_HOME"] = xiuqo_home
...
# Windows launcher self-heal — the ``xiuqo`` command is a COPY of the venv
# console script staged into the managed bin dir (outside the checkout, since
# ``xiuqo update``'s autostash once swept ``<checkout>\bin`` copies off disk;
...
# app's ``python -m xiuqo_cli.main`` spawn. Gates fail toward inaction. Sits
# AFTER the profile override on purpose — no xiuqo module may import before
...
# ``<checkout>\bin``, where ``xiuqo update``'s autostash (``git stash push --include-untracked``) swept
# them off disk; with the desktop updater's ``--keep-stash`` nothing restored them and ``xiuqo`` stopped
...
if sys.platform == "win32":
    try:
        from xiuqo_cli import _install_repair as _install_repair_mod

        _install_repair_mod.ensure_windows_bin_launchers(_bootstrap_root)
    except Exception:
        pass

# Load .env from ~/.xiuqo/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from xiuqo_cli.config import get_hermes_home
from xiuqo_cli.env_loader import load_hermes_dotenv
...
# Bridge security.redact_secrets → XIUQO_REDACT_SECRETS BEFORE xiuqo_logging
...
try:
    ...
    # serves xiuqo_logging, xiuqo_time and later raw reads: 3-4 config.yaml parses become one.
    # Managed overlay included: administrator-pinned redact_secrets / force_ipv4 win here too.
    from xiuqo_cli.config_effective import load_user_config_effective as _load_effective_early

    _cfg_path = get_hermes_home() / "config.yaml"
    if _cfg_path.exists():
        _early_cfg_raw = _load_effective_early(_cfg_path)
        if "XIUQO_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["XIUQO_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from xiuqo_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from xiuqo_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if xiuqo_constants not importable yet

import logging
import threading
from datetime import datetime

from xiuqo_cli import __version__, __release_date__

from xiuqo_cli.model_setup_flows import (
...
from xiuqo_cli.main_agent_cmds import (
    cmd_acp,
    cmd_insights,
    cmd_memory,
    cmd_monitoring,
...
from xiuqo_cli.main_platform_setup import (
    cmd_slack,
    cmd_sync,
    cmd_whatsapp,
    cmd_whatsapp_cloud,
)
from xiuqo_cli.main_dashboard import (
...
from xiuqo_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes,
)
from xiuqo_cli.main_provider_setup import (
...
from xiuqo_cli.main_install_repair import (
    _cleanup_quarantined_exes,
    _recover_from_interrupted_install,
)
from xiuqo_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError,
    _UPDATE_REEXEC_ENV,
...
from xiuqo_cli.main_desktop import (
    cmd_gui,
)
from xiuqo_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _desktop_build_needed,
    _desktop_dist_exists,
    _desktop_macos_relaunchable_fixup,
    _desktop_packaged_executable,
)
from xiuqo_cli.main_web_build import (
    _sweep_stale_bytecode_if_checkout_changed,
)
from xiuqo_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,
    _nixos_build_env,
    _record_bytecode_fingerprint,
    _run_npm_install_deterministic,
)
from xiuqo_cli.main_tui_launch import (
    _launch_tui,
...
def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("XIUQO_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("XIUQO_TERMUX_PREFETCH_UPDATES") == "1"


def _dotenv_has_provider_key(env_file: Path, provider_env_vars: set) -> bool:
    """True if ~/.xiuqo/.env assigns a non-empty value to any provider key."""
...
def _auth_store_logged_in(auth_file: Path, registry, strict_profile_scope: bool) -> bool:
    ...
    from xiuqo_cli.auth import get_auth_status
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    ...
    counts only once its identity exists, and the boot bootstrap (``xiuqo_cli.free_tier_bootstrap``)
    ...
    """
    from xiuqo_cli.config import DEFAULT_CONFIG, get_env_path, get_hermes_home, load_config
    from xiuqo_cli.auth import PROVIDER_REGISTRY, get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_model_name, dict):
            from xiuqo_cli.config import split_model_config_default
    ...
    # Claude Code OAuth credentials count only once Xiuqo is explicitly
    ...
    try:
        from xiuqo_cli.anon_auth import guest_enabled, has_guest
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from xiuqo_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_db():
    """Yield a ``SessionDB`` (lazy import, so test patches on ``xiuqo_state``
    ...
    db = None
    try:
        from xiuqo_state import SessionDB
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``xiuqo -c`` from repo A continues repo A's last session rather than the
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, xiuqo_bin
        cli_args: the original CLI arguments (everything after 'xiuqo')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    xiuqo_bin = container_info["xiuqo_bin"]
    ...
    if _probe_container(inspect_cmd, backend).returncode != 0:
        ...
        if not sudo_path:
            print(
                ...
                f"The container may be running under root. Try: sudo xiuqo {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if _probe_container(cmd_prefix[:2] + inspect_cmd, backend, via_sudo=True).returncode != 0:
            print(
                ...
                f"  }}];\n"
                f"\n"
                f"Or run: sudo xiuqo {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _create_titled_session(title: str) -> Optional[str]:
    ...
    try:
        from xiuqo_state import SessionDB
        from xiuqo_state_ids import new_session_id as mint_session_id
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'xiuqo sessions list' to see available sessions, or "
            ...
            try:
                from xiuqo_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _import_foreign_resume(args) -> None:
    ...
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex")):
        return
    from xiuqo_cli.foreign_sessions import import_foreign_session, pick_foreign_session
    ...
    print(f"  (later: xiuqo --resume {_imported_id})")
...
def _resolve_chat_session_args(args, use_tui: bool) -> None:
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'xiuqo sessions list' to see available sessions.")
            sys.exit(1)
...
def _warn_retired_xai_models() -> None:
    ...
    try:
        from xiuqo_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from xiuqo_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'xiuqo doctor' for details.\033[0m\n\n")
    except Exception:
        pass
...
def _start_chat_background_prefetch() -> None:
    ...
    The ONE exception is an unseeded ~/.xiuqo/skills: there the banner
    ...
    if _termux_should_prefetch_update_check():
        try:
            from xiuqo_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import xiuqo_cli.banner as _banner_mod
...
def _first_run_setup_guard(args) -> None:
    """No provider configured: offer `xiuqo setup` (TTY) or exit 1 with guidance."""
    print()
    print(
        "It looks like Xiuqo isn't configured yet -- no API keys or providers found."
    )
    print()
    print("  Run:  xiuqo setup")
    print()
    ...
    if reply in {"", "y", "yes"}:
        ...
        return
    print()
    print("You can run 'xiuqo setup' at any time to configure.")
    sys.exit(1)
...
def cmd_chat(args):
    ...
    from xiuqo_cli.free_tier_bootstrap import run_bootstrap
    ...
    if getattr(args, "ignore_rules", False):
        os.environ["XIUQO_IGNORE_RULES"] = "1"
    # --source: tag session source for filtering (e.g. 'tool' for integrations)
    if getattr(args, "source", None):
        os.environ["XIUQO_SESSION_SOURCE"] = args.source
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older xiuqo_cli.config) crash
        ...
        # cmd_chat, so this one catch covers `xiuqo` / `xiuqo chat`.
        from xiuqo_constants import emit_partial_update_hint
...
def cmd_gateway(args):
    ...
    from xiuqo_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from xiuqo_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def _forward_command(name: str, module: str, attr: str, *, forward_return: bool = False, doc: str = ""):
    """A ``xiuqo <cmd>`` handler that hands ``args`` to ``<module>.<attr>``.
...
cmd_setup = _forward_command("cmd_setup", "xiuqo_cli.setup", "run_setup_wizard", doc='Interactive setup wizard.')
cmd_login = _forward_command("cmd_login", "xiuqo_cli.auth", "login_command", doc='Authenticate Xiuqo CLI with a provider.')
cmd_logout = _forward_command("cmd_logout", "xiuqo_cli.auth", "logout_command", doc='Clear provider authentication.')
cmd_auth = _forward_command("cmd_auth", "xiuqo_cli.auth_commands", "auth_command", doc='Manage pooled credentials.')
cmd_status = _forward_command("cmd_status", "xiuqo_cli.status", "show_status", doc='Show status of all components.')
cmd_cron = _forward_command("cmd_cron", "xiuqo_cli.cron", "cron_command", forward_return=True, doc='Cron job management.')
cmd_webhook = _forward_command("cmd_webhook", "xiuqo_cli.webhook", "webhook_command", doc='Webhook subscription management.')
cmd_kanban = _forward_command("cmd_kanban", "xiuqo_cli.kanban", "kanban_command", forward_return=True, doc='Multi-profile collaboration board.')
cmd_project = _forward_command("cmd_project", "xiuqo_cli.projects_cmd", "projects_command", forward_return=True, doc='Manage projects (named, multi-folder workspaces).')
cmd_hooks = _forward_command("cmd_hooks", "xiuqo_cli.hooks", "hooks_command", doc='Shell-hook inspection and management.')
cmd_doctor = _forward_command("cmd_doctor", "xiuqo_cli.doctor", "run_doctor", doc='Check configuration and dependencies.')
cmd_dump = _forward_command("cmd_dump", "xiuqo_cli.dump", "run_dump", doc='Dump setup summary for support/debugging.')
cmd_debug = _forward_command("cmd_debug", "xiuqo_cli.debug", "run_debug", doc='Debug tools (share report, etc.).')
cmd_skin = _forward_command("cmd_skin", "xiuqo_cli.skin_cmd", "skin_command", doc='Skin management (list / use / set).')
cmd_import = _forward_command("cmd_import", "xiuqo_cli.backup", "run_import", doc='Restore a Xiuqo backup from a zip file.')
cmd_dashboard_register = _forward_command("cmd_dashboard_register", "xiuqo_cli.dashboard_register", "cmd_dashboard_register", doc='Register a self-hosted dashboard OAuth client with Nous Portal.')
cmd_gateway_enroll = _forward_command("cmd_gateway_enroll", "xiuqo_cli.gateway_enroll", "cmd_gateway_enroll", doc='Enroll a self-hosted gateway with a relay connector.')
cmd_prompt_size = _forward_command("cmd_prompt_size", "xiuqo_cli.prompt_size", "cmd_prompt_size", doc='Show a byte/char breakdown of the system prompt + tool schemas.')
cmd_pairing = _forward_command("cmd_pairing", "xiuqo_cli.pairing", "pairing_command")
cmd_plugins = _forward_command("cmd_plugins", "xiuqo_cli.plugins_cmd", "plugins_command")
cmd_mcp = _forward_command("cmd_mcp", "xiuqo_cli.mcp_config", "mcp_command")
cmd_claw = _forward_command("cmd_claw", "xiuqo_cli.claw", "claw_command")
cmd_import_agent = _forward_command("cmd_import_agent", "xiuqo_cli.agent_import", "import_agent_command")
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from xiuqo_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from xiuqo_cli.setup import run_setup_action_with_navigation
...
def _resolve_active_provider(config, model_cfg, effective_provider, custom_provider_map):
    ...
    """
    from xiuqo_cli.auth import AuthError, format_auth_error, resolve_provider
    from xiuqo_cli.config import get_compatible_custom_providers, get_env_value
    from xiuqo_cli.providers import custom_provider_aliases, resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            print(
                f"Warning: Unknown provider '{effective_provider}'. Check 'xiuqo model' for "
                "available providers, or run 'xiuqo doctor' to diagnose config "
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``xiuqo model``) and the setup wizard
    ...
    effective_provider = config_provider or os.getenv("XIUQO_INFERENCE_PROVIDER") or "auto"
    ...
    from xiuqo_cli.models import _PROVIDER_LABELS
    ...
    # xiuqo_cli.main keep intercepting.
    ...
    # "custom") leaves a stale OPENAI_BASE_URL in ~/.xiuqo/.env that poisons
...
# ``xiuqo_cli/update_cmd*.py`` files resolve these names via ``_m().<name>``
# on xiuqo_cli.main; importing update_cmd eagerly would cost every ``xiuqo``
...
def cmd_verify(args):
    ...
    from xiuqo_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `xiuqo security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from xiuqo_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `xiuqo approvals <subcmd>`."""
    from xiuqo_cli.approvals_suggest import approvals_command
...
def cmd_config(args):
    ...
    from xiuqo_cli.config import config_command
...
def cmd_backup(args):
    """Back up Xiuqo home directory to a zip file."""
    from xiuqo_cli import backup

    (backup.run_quick_backup if getattr(args, "quick", False) else backup.run_backup)(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    # Shared with the `xiuqo --version` pre-import fast path.
...
def cmd_uninstall(args):
    """Uninstall Xiuqo Agent (or just the Chat GUI with --gui).
    ...
    if getattr(args, "gui_summary", False):
        from xiuqo_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from xiuqo_cli.uninstall import run_gui_uninstall
    ...
    from xiuqo_cli.uninstall import run_uninstall
...
def _finalize_update_receipt(code: int, reason: str) -> None:
    ...
    try:
        ...
        from xiuqo_cli.update_receipt import finalize_pending_update_receipt
...
def _update_preflight_handled(args) -> bool:
    ...
    from xiuqo_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Xiuqo Agent")
        return True
    ...
    if getattr(args, "plan", False):
        # Read-only plan phase (#91277 Phase 2): inventory every running Xiuqo runtime across profiles, its
        # supervisor, and its running code version — without mutating anything. Safe on a live fleet.
        from xiuqo_cli.update_inventory import (
    ...
    from xiuqo_cli.update_contract import (
    ...
    if getattr(args, "check", False):
        ...
        from xiuqo_cli.update_cmd import _cmd_update_check
...
def cmd_update(args):
    """Update Xiuqo Agent: hangup protection + update lock around ``_cmd_update_impl``."""
    ...
    from xiuqo_cli.update_lock import (
    ...
    from xiuqo_cli.update_cmd import _cmd_update_impl
    ...
    try:
        ...
        from xiuqo_cli.update_receipt import COMMAND_BOUNDARY_STOP_REASON
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    ``xiuqo -c Pokemon Agent Dev`` → ``['-c', 'Pokemon Agent Dev']``; tokens
...
from xiuqo_cli.profile_cmd import cmd_profile
...
def _dashboard_lifecycle_flags(args, token_file) -> None:
    ...
    if getattr(args, "stop", False):
        if not _find_stale_dashboard_pids():
            print("No xiuqo dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `xiuqo update`;
        # it prints outcomes itself. Exit 1 only if every pid was unkillable.
        from xiuqo_cli.dashboard_procs import _kill_stale_dashboard_processes
...
def _dashboard_validate_serve_args(args, headless_backend, token_file):
    ...
    # `xiuqo serve` is headless/non-interactive: fail closed on a corrupt
    ...
    if token_file and not headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with xiuqo serve")
...
def _dashboard_sanitize_desktop_env(headless_backend) -> None:
    ...
    Desktop Electron spawns its backend with XIUQO_DESKTOP=1 plus
    XIUQO_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    XIUQO_SERVE_HEADLESS=1). A shell inheriting those then running
    `xiuqo dashboard` would serve the desktop renderer ("Desktop IPC bridge
    ...
    (XIUQO_DESKTOP=1) keeps its dist. Headless `serve` re-sets
    XIUQO_SERVE_HEADLESS itself.
    """
    if os.environ.get("XIUQO_DESKTOP") != "1":
        if _is_electron_packaged_web_dist(os.environ.get("XIUQO_WEB_DIST", "")):
            os.environ.pop("XIUQO_WEB_DIST", None)
    if not headless_backend:
        os.environ.pop("XIUQO_SERVE_HEADLESS", None)
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    # the same logs directory as every other Xiuqo surface.
    try:
        from xiuqo_logging import setup_logging as _setup_logging_gui
    ...
    try:
        ...
        # #61115, #65696.
        from xiuqo_cli.config import apply_terminal_config_to_env
    ...
    mcp_discovery_after_bind = headless_backend and os.environ.get("XIUQO_DESKTOP") == "1"
    if not mcp_discovery_after_bind:
        try:
            from xiuqo_cli.mcp_startup import start_background_mcp_discovery
...
def cmd_dashboard(args):
    ...
    from xiuqo_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from xiuqo_cli import completion
...
def cmd_logs(args):
    """View and filter Xiuqo log files."""
    from xiuqo_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Xiuqo command console."""
    from xiuqo_cli.console_engine import run_console_repl
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    """
    first = _first_positional_argv()  # None = bare ``xiuqo`` → chat
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    on import; ``discover_plugins()`` alone leaves ``xiuqo photon`` failing
    ...
    not import it, so the CLI registration never happens and ``xiuqo photon`` fails with argparse ``invalid
    choice`` (issue #54678).
    """
    if not command_name:
        return
...
def _is_tui_chat_launch(args) -> bool:
    if getattr(args, "tui", False) or os.environ.get("XIUQO_TUI") == "1":
        return True
    # The chat path decides TUI-vs-classic via _resolve_use_tui (--cli/--tui
    # flags, TTY gate, XIUQO_TUI env, display.interface config). Bare
    # `xiuqo`/`xiuqo chat` with a TUI display config was previously missed
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo chokepoint: XIUQO_YOLO_MODE must be set before any discovery
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from xiuqo_cli.plugins import start_background_plugin_discovery
    ...
    try:
        from xiuqo_cli.mcp_startup import set_mcp_server_filter
    ...
    if _run_inline_mcp_discovery:
        try:  # synchronous for entrypoints without a later bounded startup path
            from xiuqo_cli.mcp_startup import get_mcp_server_filter
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["XIUQO_SAFE_MODE"] = "1"
    os.environ["XIUQO_IGNORE_USER_CONFIG"] = "1"
    os.environ["XIUQO_IGNORE_RULES"] = "1"
...
def _light_chat_parser():
    ...
    from xiuqo_cli._parser import build_top_level_parser
...
def _try_fast_serve_launch() -> bool:
    ...
    """
    if os.environ.get("XIUQO_DISABLE_FAST_SERVE_LAUNCH") == "1":
        return False
    ...
    try:
        from xiuqo_cli.config import get_container_exec_info
...
def _try_fast_chat_launch() -> bool:
    ...
    """
    if os.environ.get("XIUQO_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("XIUQO_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["XIUQO_DEFER_AGENT_STARTUP"] = "1"
            os.environ["XIUQO_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["XIUQO_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `xiuqo --tui` is the hot path on phones and the TUI immediately execs
...
def _advertise_agent_env() -> None:
    ...
    (``xiuqo``) — matching is exact. ``XIUQO_AGENT`` is the
    Xiuqo-specific marker. setdefault: never clobber an outer harness.
    ...
    harness that spawned them. Xiuqo running inside another agent's terminal).
    """
    os.environ.setdefault("AI_AGENT", "xiuqo")
    os.environ.setdefault("XIUQO_AGENT", "true")
...
def _register_plugin_cli_commands(subparsers) -> None:
    ...
    try:
        ...
        from xiuqo_cli.plugins import discover_plugins, get_plugin_manager
...
def _cmd_sessions_lazy(args, **kwargs):
    """``xiuqo sessions`` handler; sessions_cmd imports only when the subcommand runs."""
    from xiuqo_cli.sessions_cmd import cmd_sessions

    return cmd_sessions(args, **kwargs)


def _build_cli_parser():
    """Build the full ``xiuqo`` argparse tree -> ``(parser, subparsers)``.

    Registration ORDER is the ``xiuqo --help`` order; keep it stable. Groups
    live in ``xiuqo_cli/subcommands/<group>.py`` with handlers injected so
    ...
    # OUTBOUND egress firewall; ``xiuqo proxy`` (gateway group) is the INBOUND one.
    ...
    from xiuqo_cli.send_cmd import register_send_subparser
    ...
    from xiuqo_cli.subcommands.peer import build_peer_parser
    build_peer_parser(subparsers)

    from xiuqo_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    from xiuqo_cli.kanban import build_parser as _build_kanban_parser
    _build_kanban_parser(subparsers).set_defaults(func=cmd_kanban)

    from xiuqo_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # "desktop" is canonical (Xiuqo-Setup.exe tells users to run it, so it
...
def _parse_cli_args(parser, subparsers, argv):
    ...
    routing; if that fails (``xiuqo -c model`` — 'model' is the session name)
...
def main():
    """Main entry point for xiuqo CLI."""
    _set_process_title()
    _advertise_agent_env()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from xiuqo_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``xiuqo.exe.old.*`` quarantine files from previous Windows
    ...
    # Self-heal a venv left half-built by an interrupted ``xiuqo update``, and
    ...
    # substring match is deliberately loose: over-matching (``xiuqo skills
    # install update``) only defers recovery one launch; under-matching
    # (``xiuqo -p work update``) would race. Never raises.
    # See #95294.
    if "update" not in sys.argv[1:]:
        try:
            ...
            pass
        try:
            from xiuqo_cli.update_cmd_fleet import _warn_pending_fleet_restart_on_startup
...
_PLUGIN_COMPAT_LAZY = {
    'line_input': ('xiuqo_cli.cli_output', 'line_input'),
}
