...
from xiuqo_cli.web_server_files import (
...
# points the managed root at XIUQO_HOME. Mirrors the two canonical guards
...
# trees are blocked wherever they sit under the root, no XIUQO_HOME resolution.
...
def _decode_data_url(data_url: str) -> tuple[bytes, str]:
    from xiuqo_cli.web_server import _MANAGED_FILE_MAX_BYTES
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    the returned gateway-visible path under ``XIUQO_HOME/images/`` (the same
...
async def stream_upload_to_path(
    ...
    *,
    ...
    """
    from xiuqo_cli.web_server import _MANAGED_FILE_MAX_BYTES, _UPLOAD_CHUNK_BYTES
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the Electron ``xiuqo:fs:writeText`` hardening: path validated by
    ...
    tmp = target.with_name(f".{target.name}.xiuqo-tmp-{os.getpid()}")
    try:
...
async def _fs_download_path(path: str, profile: Optional[str], session_id: Optional[str]) -> Path:
    if session_id is not None:
        from xiuqo_cli.web_routers.sessions import get_session_detail
    ...
    if profile is not None:
        from xiuqo_cli.web_server_cron import _cron_profile_home
...
async def fs_read_data_url(
    ...
    from xiuqo_cli.web_server import _FS_DATA_URL_MAX_BYTES
