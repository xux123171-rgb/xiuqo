...
from xiuqo_cli.web_server_chat import _ws_auth_ok, _ws_request_is_allowed
from xiuqo_cli.web_server_gateway import _split_text_for_speak_stream
from fastapi import HTTPException, WebSocket, WebSocketDisconnect
from xiuqo_cli.web_models import AudioTranscriptionRequest, TTSSpeakRequest, TTSLeaseRequest, VoiceLiveSessionRequest
    ...
    try:
        with http_failure("Desktop voice transcription failed", 500, "Transcription failed"):
            with tempfile.NamedTemporaryFile(
                prefix="xiuqo-desktop-voice-", suffix=_audio_extension_for_mime(mime_type),
                delete=False,
            ) as tmp:
...
async def create_voice_live_session(payload: VoiceLiveSessionRequest, profile: Optional[str] = None):
    ...
    renderer receives becomes a Xiuqo turn on the session it belongs to.
    """
