...
Extracted from ``xiuqo_cli.web_server``; app state and helpers are late-bound through
:mod:`xiuqo_cli.web_deps` (cycle-safe, monkeypatch-friendly).
"""
...
from xiuqo_cli.web_deps import LateState, late
from xiuqo_cli.web_server_gateway import _display_system_platform
...
from xiuqo_cli.config import get_config_path, get_env_path
from xiuqo_constants import get_process_hermes_home, profile_name_for_home
from xiuqo_cli.web_models import CuratorPause, LearningNodeRef, LearningNodeEdit, DebugShareRequest
from xiuqo_cli.web_routers._common import scoped_to_thread
...
_collect_profile_gateway_topology_cached = late("_collect_profile_gateway_topology_cached", "xiuqo_cli.web_server_gateway")
_config_profile_scope = late("_config_profile_scope", "xiuqo_cli.web_server_profiles")
_dashboard_local_update_managed_externally = late("_dashboard_local_update_managed_externally", "xiuqo_cli.web_server_files")
_load_configured_gateway_platforms = late("_load_configured_gateway_platforms", "xiuqo_cli.web_server_gateway")
_probe_gateway_health = late("_probe_gateway_health", "xiuqo_cli.web_server_gateway")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "xiuqo_cli.web_server_profiles")
_resolve_restart_drain_timeout = late("_resolve_restart_drain_timeout", "xiuqo_cli.web_server_lifecycle")
_spawn_hermes_action = late("_spawn_hermes_action", "xiuqo_cli.web_server_gateway")
_ssh_runtime_intact = late("_ssh_runtime_intact")
app = LateState("app")  # the FastAPI instance (app.state.*)
check_config_version = late("check_config_version", "xiuqo_cli.config")
get_hermes_home = late("get_hermes_home", "xiuqo_cli.config")
...
load_config = late("load_config", "xiuqo_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
_open_session_db_for_profile = late("_open_session_db_for_profile", "xiuqo_cli.web_server_sessions")
...
def _count_status_active_sessions() -> int:
    ...
    /api/status never routinely writes to state.db while another Xiuqo process uses it."""
    from xiuqo_state import _default_db_path
...
async def get_ssh_ownership(request: Request):
    from xiuqo_cli.web_server import _SSH_OWNER_NONCE
...
# Profile segment mirrors xiuqo_cli.profiles._PROFILE_ID_RE. Platform segment mirrors the
...
# status) are written to the per-profile home, not the process-level XIUQO_HOME (see issue #69143). Plain
...
async def _resolve_gateway_status(profile_dir: Optional[Path], health_url) -> Dict[str, Any]:
    ...
    if liveness.runtime is not None:
        ...
        # profile is the process's own home (a pooled ``xiuqo --profile X serve``).
...
def _auth_gate_status() -> Dict[str, Any]:
    ...
    auth_flows: list[str] = []
    try:
        from xiuqo_cli.dashboard_auth import (
...
def _nous_session_validity() -> str:
    ...
    try:
        from xiuqo_cli.auth import get_nous_session_validity
...
async def _component_health(gateway: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from xiuqo_cli.web_server import DASHBOARD_HEALTH
...
async def _advisory_pressure(status: Dict[str, Any], home: Path) -> None:
    ...
    try:
        from xiuqo_state import SessionDB as _SDB
...
async def get_status(profile: Optional[str] = None):
    ...
    """
    from xiuqo_cli.web_server import _GATEWAY_HEALTH_URL
    status_scope = None
    ...
    try:
        ...
        # Off-loop: on a cold Windows install the first import of xiuqo_cli.gateway blocks
        ...
        # Profile NAMES and ``gateway_mode`` are low-sensitivity product surface (Xiuqo Cloud
        ...
        if not auth["auth_required"]:
            status.update({
                "xiuqo_home": str(get_hermes_home()), "config_path": str(get_config_path()),
...
async def get_system_stats():
    ...
    disk/uptime when available). Non-sensitive: no env values, no paths beyond xiuqo home."""
    ...
    info: Dict[str, Any] = {
        **_display_system_platform(
        ...
        "xiuqo_version": __version__, "cpu_count": os.cpu_count()}
...
def _spawn_action(argv: list, name: str, prefix: str) -> dict:
    """Spawn a background ``xiuqo <argv>`` action; a spawn failure is ``500 "<prefix>: <exc>"``."""
    try:
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from xiuqo_cli.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from xiuqo_cli.nous_subscription import get_nous_subscription_features
...
async def get_logs(
    ...
    from xiuqo_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from xiuqo_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
