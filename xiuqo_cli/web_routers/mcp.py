...
from xiuqo_cli.web_server_mcp import _mcp_oauth_flows, _mcp_server_summary, _normalize_mcp_server_create
from xiuqo_cli.web_models import MCPCatalogInstall, MCPEnabledToggle, MCPServerCreate, MCPServersReplace
from xiuqo_cli.web_routers._common import (
...
_run_dashboard_mcp_oauth = late("_run_dashboard_mcp_oauth", "xiuqo_cli.web_server_mcp")
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from xiuqo_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from xiuqo_cli.mcp_config import _get_mcp_servers, _save_bearer_auth_token, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    from xiuqo_cli.mcp_config import _replace_mcp_servers
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from xiuqo_cli.mcp_config import _get_mcp_servers, _oauth_tokens_present, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # endpoint. The probe only needs XIUQO_HOME for .env + token resolution.
    ...
    try:  # probe blocks on a dedicated MCP event loop — keep it off the FastAPI loop
        ...
        from xiuqo_cli.mcp_config import redact_mcp_probe_text
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    with _mcp_oauth_flows_lock:
        ...
        if any(f.server_name == name and f.xiuqo_home == flow_home for f in live):
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to Xiuqo and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to Xiuqo for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to Xiuqo.</p>")
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    from xiuqo_cli import mcp_catalog
    from xiuqo_cli.config import validate_env_var_name_for_write
