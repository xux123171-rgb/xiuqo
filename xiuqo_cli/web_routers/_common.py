...
late-binding seam in :mod:`xiuqo_cli.web_deps` (web_server owns helpers/state;
...
log = logging.getLogger("xiuqo_cli.web_server")
...
def spawn_profile_action(
    ...
    """Spawn a background ``xiuqo -p <profile> <argv>`` action; a spawn
...
# instead of a 500. The file is never quarantined or renamed from here — that is `xiuqo doctor`'s job.
...
CORRUPT_STORE_DETAIL = {
    ...
    "message": "state.db corrupt — run `xiuqo doctor` (then `xiuqo doctor --fix` or `xiuqo sessions repair`).",
}
...
def corrupt_store_as_status(db_path):
    ...
    from xiuqo_state_errors import is_malformed_db_error
    ...
    try:
        ...
        if last is None or now - last >= _CORRUPT_STORE_WARN_INTERVAL_S:
            ...
            log.warning("state.db at %s is corrupt (%s); dashboard reads return a status payload until it is "
                        "repaired — run `xiuqo doctor`", db_path, exc)
        else:
