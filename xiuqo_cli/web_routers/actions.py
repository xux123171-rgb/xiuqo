"""Gateway restart/drain, Xiuqo update and background-action status dashboard routes.

Extracted from ``xiuqo_cli.web_server``; helpers/state that tests monkeypatch on
...
from xiuqo_cli.config import format_docker_update_message, recommended_update_command_for_method
from xiuqo_cli.web_deps import LateState, late
from xiuqo_cli.web_server_gateway import _ACTION_LOG_FILES
from xiuqo_cli.web_routers._common import http_failure
...
detect_install_method = late("detect_install_method", "xiuqo_cli.config")
get_hermes_home = late("get_hermes_home", "xiuqo_cli.config")
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "xiuqo_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "xiuqo_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "xiuqo_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "xiuqo_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import xiuqo_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== xiuqo-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "Xiuqo updates are managed outside this dashboard in containerized environments."
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== xiuqo update started "):
            last_start = index
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``xiuqo gateway restart`` in the background."""
...
async def gateway_migrate_plan():
    ...
    from xiuqo_cli.gateway_migrate import build_migration_plan
...
async def gateway_migrate():
    """Run ``xiuqo gateway migrate --multiplex --yes`` detached; the CLI re-runs the preflight and
    refuses (exit 1 into the action log) when blocked, so the UI should gate on the plan first."""
    from xiuqo_cli.web_server_gateway import _spawn_hermes_action
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    _record_completed_action("xiuqo-update", message, exit_code=1)
    return {
        "ok": False, "pid": None, "name": "xiuqo-update", "error": error, "message": message,
        "update_command": update_command,
    }


@router.post("/api/xiuqo/update")
async def update_hermes():
    """Kick off ``xiuqo update`` in the background."""
    ...
    existing = _ACTION_PROCS.get("xiuqo-update")
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "xiuqo-update", "already_running": True}
        action_id = _ACTION_IDS.get("xiuqo-update")
        if action_id:
            response["action_id"] = action_id
        return response

    action_id = secrets.token_hex(16)
    with http_failure("Failed to spawn xiuqo update", 500, "Failed to start update"):
        proc = _spawn_hermes_action(["update"], "xiuqo-update", env_overrides={"XIUQO_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "xiuqo-update", "action_id": action_id}


_NON_APPLYABLE_MESSAGES = {
    "docker": format_docker_update_message,
    "apt": lambda: "Xiuqo is managed by Termux APT; run `pkg upgrade xiuqo`.",
}


@router.get("/api/xiuqo/update/check")
async def check_hermes_update(force: bool = False):
    """Report whether a Xiuqo update is available, without applying it.
    ...
    try:
        from xiuqo_cli.banner import check_for_updates, upstream_commits_behind
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "xiuqo-update":
        ...
        if durable_update_action_id:
            marker = f"=== xiuqo-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``xiuqo update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``xiuqo update`` run since #91283, including
...
@status_router.get("/api/xiuqo/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `xiuqo update` run recorded).")
