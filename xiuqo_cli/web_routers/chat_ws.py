...
from xiuqo_cli.pty_session import RegistryFull
from xiuqo_cli.web_deps import LateState, late
from xiuqo_cli.web_server_chat import (
...
_active_session_file_for_channel = late("_active_session_file_for_channel", "xiuqo_cli.web_server_chat")
_profile_scope = late("_profile_scope", "xiuqo_cli.web_server_profiles")
_resolve_chat_argv_async = late("_resolve_chat_argv_async", "xiuqo_cli.web_server_chat")
_resolve_profile_dir = late("_resolve_profile_dir", "xiuqo_cli.web_server_profiles")
_ws_auth_reason = late("_ws_auth_reason", "xiuqo_cli.web_server_chat")
_ws_client_reason = late("_ws_client_reason", "xiuqo_cli.web_server_chat")
_ws_host_origin_reason = late("_ws_host_origin_reason", "xiuqo_cli.web_server_chat")
...
def _ws_auth_mode() -> str:
    ...
    from xiuqo_cli.web_server_chat import _LOOPBACK_HOSTS
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from xiuqo_cli.console_engine import HermesConsoleEngine
        ...
        try:
            ...
            if command_id == command_generation:
                ...
                await out.error_then_complete(
                    "Command timed out. Xiuqo Console returned to the prompt.", line, command_id, "timeout",
                )
        except Exception as exc:
            if command_id == command_generation:
                pending_confirmation = None
...
async def pty_ws(ws: WebSocket) -> None:
    from xiuqo_cli.web_server_chat import PTY_REGISTRY, PtyBridge, PtyUnavailableError, _PTY_BRIDGE_AVAILABLE, _RESIZE_RE
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Xiuqo inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("XIUQO_TUI_RESUME") or raw_resume
...
# (XIUQO_TUI_SIDECAR_URL from /api/pty's env) and writes every dispatcher emit
