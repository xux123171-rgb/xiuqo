...
from xiuqo_cli.web_routers._common import http_failure, scoped_to_thread
...
from xiuqo_cli.config import DEFAULT_CONFIG, OPTIONAL_ENV_VARS, read_raw_config, custom_endpoint_key_env, coerce_provider_id, find_provider_entry, redact_key, _deep_merge
from xiuqo_cli.web_models import ConfigUpdate, EnvVarUpdate, EnvVarDelete, EnvVarReveal, CustomEndpointUpdate
...
_channel_managed_env_keys = late("_channel_managed_env_keys", "xiuqo_cli.web_server_messaging")
...
async def get_egress_status():
    ...
    from xiuqo_cli.proxy_cli import format_status_text
...
async def update_config(
    ...
    def _run():
        ...
        # different XIUQO_HOME than this process's gateway sessions.
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (the ``xiuqo model`` universe), so the desktop Keys tab renders a card even
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # by the SAME provider identity the CLI `xiuqo model` picker uses.
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    rather than the process-level XIUQO_HOME (mirrors ``/api/config``).
    """
