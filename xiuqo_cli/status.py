...
from xiuqo_cli.auth import AuthError, resolve_provider
from xiuqo_cli.colors import Colors, color
from xiuqo_cli.config import get_env_path, get_env_value, get_hermes_home, load_config
from xiuqo_cli.models import provider_label
from xiuqo_cli.runtime_provider import resolve_requested_provider
from xiuqo_cli.vercel_auth import describe_vercel_auth
from xiuqo_cli.status_auth import (  # renderers wired into _SECTIONS below
    _render_api_keys, _render_apikey_providers, _render_auth_providers, _render_nous_gateway)
from xiuqo_constants import OPENROUTER_MODELS_URL
...
def _estop_status_line():
    """One-line pause banner for `xiuqo status`, or None when not paused."""
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{f' — reason: {reason}' if reason else ''}; `xiuqo resume` to lift)"
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'xiuqo[vercel]')")
        ...
        # (fail-soft — never break `xiuqo status`).
        try:
...
def _render_gateway(ctx):
    ...
    try:
        ...
        from xiuqo_cli.gateway_multiplex_served import multiplexer_served_secondaries
        ...
        if not snapshot.running and named_profile_served_by_running_multiplexer():
            ...
            _kv("Manage with:", "xiuqo gateway status   # from the default profile")
            return
        ...
        if snapshot.running and (served := multiplexer_served_secondaries()):
            ...
            from xiuqo_cli.gateway_multiplex_served import served_profile_ingress_urls
...
def _render_sessions(ctx):
    ...
    if gateway_rows:
        ...
        if freshest > 0:
            from xiuqo_cli.timefmt import relative_time
    ...
    try:
        from xiuqo_cli.active_sessions import (
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'xiuqo doctor' for detailed diagnostics", "  Run 'xiuqo setup' to configure"),
            Colors.DIM)
    print()


# Print order of `xiuqo status`; each renderer takes the shared _StatusContext.
_SECTIONS = (
...
def show_status(args):
    """Show status of all Xiuqo Agent components."""
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nous_portal_account_info': ('xiuqo_cli.nous_account', 'get_nous_portal_account_info'),
