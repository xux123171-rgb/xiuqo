...
from xiuqo_cli.session_schema_history import SCHEMA_HISTORY, reachable_physical_layouts

from xiuqo_state_ids import SESSION_ID_PATTERN  # timestamp prefix: strongest sentinel for schema-less rows
...
# 3.50.7 and 3.44.6 — the same version gate xiuqo_state applies to the
...
# caught exactly this shell converting a live Xiuqo state.db into two
# generations. A salvage shell must therefore be version-gated, not just
# capability-gated, before it is pointed at (a copy of) a Xiuqo database.
#
# The predicate lives in xiuqo_cli.sqlite_runtime (stdlib-only, shared with
# the installer/update gates) so the embedded runtime and the salvage shell
# can never disagree about which versions are safe.
from xiuqo_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _wal_reset_vulnerable  # noqa: E502

_WAL_RESET_VULNERABLE_GUIDANCE = (
    "salvage against a Xiuqo database with the WAL-reset bug "
...
def _cli_supports_recover(binary: str) -> bool:
    ...
    scratch_dir = tempfile.mkdtemp(prefix="xiuqo-recover-probe-")
...
def _type_conflicts(value: Any, declared: str) -> bool:
    ...
    non-numeric string in an INTEGER column, but Xiuqo never writes one, so
...
# The closed sets SessionDB writes (xiuqo_state.py: request/claim/complete/
