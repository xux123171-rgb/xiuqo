...
Lives under the shared Xiuqo root: ``default`` board DB at ``<root>/kanban.db`` (pre-boards
back-compat), other boards at ``<root>/kanban/boards/<slug>/``; a worker on one board never sees
another. Board resolution: ``board=`` arg > ``XIUQO_KANBAN_BOARD`` > ``XIUQO_KANBAN_DB`` (pins the
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    from xiuqo_constants import VALID_REASONING_EFFORTS
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("XIUQO_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    try:
        from xiuqo_cli.lifecycle import invoke_hook
...
def _kanban_observer_consumed(event: str) -> bool:
    ...
    try:
        from xiuqo_cli.lifecycle import has_hook
...
# ``heartbeat_claim`` or raise it via XIUQO_KANBAN_CLAIM_TTL_SECONDS.
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    """Explicit ``ttl_seconds`` > ``XIUQO_KANBAN_CLAIM_TTL_SECONDS`` > default."""
    if ttl_seconds is not None:
        return max(1, int(ttl_seconds))

    return _env_int("XIUQO_KANBAN_CLAIM_TTL_SECONDS", DEFAULT_CLAIM_TTL_SECONDS, minimum=1)
...
def _resolve_crash_grace_seconds() -> int:
    """``XIUQO_KANBAN_CRASH_GRACE_SECONDS`` (0 = immediate, for tests) else default."""
    return _env_int("XIUQO_KANBAN_CRASH_GRACE_SECONDS", DEFAULT_CRASH_GRACE_SECONDS)


def _resolve_rate_limit_cooldown_seconds() -> int:
    """``XIUQO_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` (0 = next tick, for tests) else default."""
    return _env_int("XIUQO_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", DEFAULT_RATE_LIMIT_COOLDOWN_SECONDS)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "xiuqo_kanban_current_board_override", default=None,
)
...
def kanban_home() -> Path:
    """``XIUQO_KANBAN_HOME`` else ``get_default_hermes_root()``. Shared across
    profiles BY DESIGN: resolving through the active profile's XIUQO_HOME would
    fork the board per profile and break the dispatcher/worker handoff."""
    override = os.environ.get("XIUQO_KANBAN_HOME", "").strip()
    if override:
...
def get_current_board() -> str:
    """Active slug: context override -> ``XIUQO_KANBAN_BOARD`` -> ``<root>/kanban/current``
    ...
    for candidate in (
        ...
        os.environ.get("XIUQO_KANBAN_BOARD", "").strip(),
    ):
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    """``kanban.db`` path: ``XIUQO_KANBAN_DB`` pins it (injected into workers);
    ``default`` -> ``<root>/kanban.db`` (back-compat), else the board dir."""
    return _board_path("XIUQO_KANBAN_DB", board, ("kanban.db",), "kanban.db")


def workspaces_root(board: Optional[str] = None) -> Path:
    """Per-board scratch workspace root (``XIUQO_KANBAN_WORKSPACES_ROOT`` wins);
    ``default`` keeps the legacy ``<root>/kanban/workspaces/``."""
    return _board_path("XIUQO_KANBAN_WORKSPACES_ROOT", board, ("kanban", "workspaces"), "workspaces")


def attachments_root(board: Optional[str] = None) -> Path:
    """Per-board attachments root (``XIUQO_KANBAN_ATTACHMENTS_ROOT`` wins). Workers
    read attachments by absolute path, so remote terminal backends must mount it."""
    return _board_path("XIUQO_KANBAN_ATTACHMENTS_ROOT", board, ("kanban", "attachments"), "attachments")
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    """Per-board worker log dir (logs follow the board so ``xiuqo kanban log``
...
class Task:
    ...
    session_id: Optional[str] = None         # originating XIUQO_SESSION_ID; NULL from CLI/dashboard
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (xiuqo_cli/projects_db). When set,
    ...
    -- inside an agent loop that propagated ``XIUQO_SESSION_ID``. NULL
...
def _resolve_project_link(
    ...
    from xiuqo_cli import projects_db as _pdb
...
def create_task(
    ...
    """
    from xiuqo_cli.kanban_db_graph import initial_task_state, inherit_creator_origin
    from xiuqo_cli.kanban_pr_acceptance import validate_contract
...
# Canonical sort-order mappings for ``xiuqo kanban list --sort``.
...
# dispatcher tick or a manual `xiuqo kanban recompute` (issue #22459).
...
def complete_task(
    ...
    if not _parents_satisfied(conn, task_id):
        return False
    from xiuqo_cli.kanban_pr_acceptance_store import prepare_acceptance, record_acceptance
...
def promote_task(
    ...
    if unsatisfied:
        return False, (
            ...
            f"`xiuqo kanban unlink <parent_id> {task_id}`)"
        )
...
def _ctx_comments(lines: list[str], comments: list[Comment], now: int) -> None:
    ...
    worker" framing stops an operator-controlled XIUQO_PROFILE like
    "xiuqo-system" being read as a system directive above an
    ...
    if omitted_note:
        lines.append(omitted_note)
    for c in shown:
        # Render author with explicit "comment from worker" framing so operator-controlled XIUQO_PROFILE
        # values like "xiuqo-system" or "operator" can't be misread by the next worker as a system
...
def list_profiles_on_disk() -> list[str]:
    ...
    directly to avoid importing ``xiuqo_cli.profiles`` at startup."""
    try:
        from xiuqo_constants import get_default_hermes_root
...
from xiuqo_cli.kanban_db_connect import (  # noqa: E402
    _INITIALIZED_PATHS,
    init_db,
    write_txn,
)
from xiuqo_cli.kanban_db_workspace import (  # noqa: E402
    _cleanup_workspace,
    _is_managed_scratch_path,
    _managed_scratch_path_info,
    _scratch_workspace,
)
from xiuqo_cli.kanban_db_dispatch import (  # noqa: E402
...
_PLUGIN_COMPAT_LAZY = {
    'DEFAULT_BUSY_TIMEOUT_MS': ('xiuqo_cli.kanban_db_connect', 'DEFAULT_BUSY_TIMEOUT_MS'),
    'DEFAULT_LOG_BACKUP_COUNT': ('xiuqo_cli.kanban_db_dispatch', 'DEFAULT_LOG_BACKUP_COUNT'),
    'DEFAULT_LOG_ROTATE_BYTES': ('xiuqo_cli.kanban_db_dispatch', 'DEFAULT_LOG_ROTATE_BYTES'),
    'DERIVED_MAX_IN_PROGRESS_CEILING': ('xiuqo_cli.kanban_db_dispatch', 'DERIVED_MAX_IN_PROGRESS_CEILING'),
    'DERIVED_MAX_IN_PROGRESS_FLOOR': ('xiuqo_cli.kanban_db_dispatch', 'DERIVED_MAX_IN_PROGRESS_FLOOR'),
    'KANBAN_TERMINAL_TIMEOUT_GRACE_SECONDS': ('xiuqo_cli.kanban_db_dispatch', 'KANBAN_TERMINAL_TIMEOUT_GRACE_SECONDS'),
    'KanbanDbCorruptError': ('xiuqo_cli.kanban_db_connect', 'KanbanDbCorruptError'),
    'MEMORY_GUARD_MB_PER_WORKER': ('xiuqo_cli.kanban_db_dispatch', 'MEMORY_GUARD_MB_PER_WORKER'),
    'RepairResult': ('xiuqo_cli.kanban_db_connect', 'RepairResult'),
    'add_notify_sub': ('xiuqo_cli.kanban_db_notify', 'add_notify_sub'),
    'advance_notify_cursor': ('xiuqo_cli.kanban_db_notify', 'advance_notify_cursor'),
    'check_respawn_guard': ('xiuqo_cli.kanban_db_dispatch', 'check_respawn_guard'),
    'claim_unseen_events_for_sub': ('xiuqo_cli.kanban_db_notify', 'claim_unseen_events_for_sub'),
    'configured_max_in_progress': ('xiuqo_cli.kanban_db_dispatch', 'configured_max_in_progress'),
    'connect': ('xiuqo_cli.kanban_db_connect', 'connect'),
    'connect_closing': ('xiuqo_cli.kanban_db_connect', 'connect_closing'),
    'count_notify_subs': ('xiuqo_cli.kanban_db_notify', 'count_notify_subs'),
    'count_running_tasks': ('xiuqo_cli.kanban_db_dispatch', 'count_running_tasks'),
    'count_running_tasks_other_boards': ('xiuqo_cli.kanban_db_dispatch', 'count_running_tasks_other_boards'),
    'derive_default_max_in_progress': ('xiuqo_cli.kanban_db_dispatch', 'derive_default_max_in_progress'),
    'detect_crashed_workers': ('xiuqo_cli.kanban_db_dispatch', 'detect_crashed_workers'),
    'detect_stale_running': ('xiuqo_cli.kanban_db_dispatch', 'detect_stale_running'),
    'dispatch_once': ('xiuqo_cli.kanban_db_dispatch', 'dispatch_once'),
    'enforce_max_runtime': ('xiuqo_cli.kanban_db_dispatch', 'enforce_max_runtime'),
    'has_spawnable_ready': ('xiuqo_cli.kanban_db_dispatch', 'has_spawnable_ready'),
    'has_spawnable_review': ('xiuqo_cli.kanban_db_dispatch', 'has_spawnable_review'),
    'heartbeat_worker': ('xiuqo_cli.kanban_db_dispatch', 'heartbeat_worker'),
    'list_notify_subs': ('xiuqo_cli.kanban_db_notify', 'list_notify_subs'),
    'purge_stale_done_notify_subs': ('xiuqo_cli.kanban_db_notify', 'purge_stale_done_notify_subs'),
    'reap_worker_zombies': ('xiuqo_cli.kanban_db_dispatch', 'reap_worker_zombies'),
    'reconcile_orphaned_running': ('xiuqo_cli.kanban_db_dispatch', 'reconcile_orphaned_running'),
    'remove_notify_sub': ('xiuqo_cli.kanban_db_notify', 'remove_notify_sub'),
    'repair_db': ('xiuqo_cli.kanban_db_connect', 'repair_db'),
    'resolve_max_in_progress': ('xiuqo_cli.kanban_db_dispatch', 'resolve_max_in_progress'),
    'resolve_workspace': ('xiuqo_cli.kanban_db_workspace', 'resolve_workspace'),
    'review_dispatch_enabled': ('xiuqo_cli.kanban_db_dispatch', 'review_dispatch_enabled'),
    'rewind_notify_cursor': ('xiuqo_cli.kanban_db_notify', 'rewind_notify_cursor'),
    'run_daemon': ('xiuqo_cli.kanban_db_dispatch', 'run_daemon'),
    'set_branch_name': ('xiuqo_cli.kanban_db_workspace', 'set_branch_name'),
    'set_workspace_path': ('xiuqo_cli.kanban_db_workspace', 'set_workspace_path'),
    'unseen_events_for_sub': ('xiuqo_cli.kanban_db_notify', 'unseen_events_for_sub'),
    'worker_log_rotation_config': ('xiuqo_cli.kanban_db_dispatch', 'worker_log_rotation_config'),
}
