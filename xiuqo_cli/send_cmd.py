"""CLI subcommand: ``xiuqo send`` — pipe text from shell scripts to any configured messaging platform
...
def _read_message_body(positional: Optional[str], file_path: Optional[str]) -> Optional[str]:
    ...
    if file_path:
        ...
        try:
            ...
            _fail(
                f"xiuqo send: {file_path} is not a text file. --file reads the "
                ...
                f'  xiuqo send --to telegram "MEDIA:{file_path}"\n'
                f'  xiuqo send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  xiuqo send --to telegram "[[as_document]] MEDIA:{file_path}"',
                _USAGE_EXIT)
        except OSError as exc:
            _fail(f"xiuqo send: cannot read {file_path}: {exc}", _USAGE_EXIT)
...
def _emit_result(result_json: str, *, json_mode: bool, quiet: bool) -> int:
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"xiuqo send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        return _fail(f"xiuqo send: failed to load channel directory: {exc}")
    try:
        raw = load_directory()
    except Exception as exc:
        return _fail(f"xiuqo send: failed to read channel directory: {exc}")
    ...
    if platform_filter:
        ...
        if not filtered:
            return _fail(
                f"xiuqo send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `xiuqo gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.xiuqo/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.xiuqo/.env`` AND bridge top-level ``config.yaml`` keys into
    ...
    if env_path.exists():
        try:
            from xiuqo_cli.env_loader import _load_dotenv_with_fallback
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "list_targets", False):  # --list short-circuits everything else
        # `xiuqo send --list telegram` lands "telegram" in the `message` positional.
        exit_code = _list_targets(getattr(args, "message", None), json_mode=getattr(args, "json", False))
        sys.exit(exit_code)
    target = (getattr(args, "to", None) or "").strip()
    if not target:
        _fail(
            "xiuqo send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  xiuqo send --to telegram \"hello\"\n"
            "  xiuqo send --to discord:#ops --file report.md\n"
            "  xiuqo send --list      # list available targets",
            _USAGE_EXIT)
    message = _read_message_body(getattr(args, "message", None), getattr(args, "file", None))
    if message is None or not message.strip():
        _fail(
            "xiuqo send: no message provided. Pass text as a positional "
    ...
    # Lazy import keeps `xiuqo send --help` fast (no tool registry / gateway config stack).
...
_SEND_ARGUMENTS = (
    ...
    (("-l", "--list"), dict(dest="list_targets", action="store_true", default=False,
                            help="List available targets. Optional positional filter: `xiuqo send --list telegram`.")),
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Xiuqo "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.xiuqo/.env + ~/.xiuqo/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  xiuqo send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | xiuqo send --to telegram:-1001234567890\n"
            "  xiuqo send --to discord:#ops --file /tmp/report.md\n"
            "  xiuqo send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  xiuqo send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  xiuqo send --list                  # all platforms\n"
            "  xiuqo send --list telegram         # filter by platform\n"
            "\n"
