...
The identity is created in exactly one place, at boot (``xiuqo_cli.free_tier_bootstrap``), and only
while ``XIUQO_GUEST_ONBOARDING=1`` (see ``guest_enabled``). The bootstrap mints an anonymous Nous
...
connectors: ``xiuqo auth upgrade`` in a terminal and ``/login`` inside a chat.
...
ANON_SECRET_ENV = "XIUQO_ANON_API_SECRET"
...
GUEST_ONBOARDING_ENV = "XIUQO_GUEST_ONBOARDING"
...
UPGRADE_HINT = "Run `xiuqo auth upgrade` to sign in with a Nous account, or /login inside a chat."
FREE_TIER_NOT_SIGNED_IN = (
    "You're not signed in. Free inference and connectors are always on. "
    "Run `xiuqo auth` to sign in with a Nous account.")
...
def current_nous_state() -> Optional[Dict[str, Any]]:
    ...
    from xiuqo_cli.auth import _load_auth_store, _load_provider_state
    try:
...
def apply_exchange_to_state(state: Dict[str, Any], exchanged: Dict[str, Any]) -> None:
    ...
    from xiuqo_cli.auth_nous import _validate_nous_inference_url_from_network
...
def _portal_base_url() -> str:
    from xiuqo_cli.auth_nous import _nous_portal_env_override
...
def _mint_locked(
    ...
    from xiuqo_cli.auth import _store_provider_state, _save_auth_store
    from xiuqo_cli.auth_nous import _write_shared_nous_state
...
def _mint_failed_for_profile() -> bool:
    ...
    if get_hermes_home_override() is None:
        return _mint_failed
    return xiuqo_home_key() in _mint_failed_homes
...
def _set_mint_failed(failed: bool) -> None:
    ...
    if get_hermes_home_override() is None:
        ...
        _mint_failed_homes.add(xiuqo_home_key())
    else:
        _mint_failed_homes.discard(xiuqo_home_key())
...
def _reconcile_and_provision(*, timeout_seconds: float, carries_inference: bool = True) -> Optional[Dict[str, Any]]:
    ...
    1. The shared store is the identity of record for this Xiuqo root. If it holds an identity
    ...
    from xiuqo_cli.auth_nous import (
...
def refresh_guest_state(state: Dict[str, Any], client: httpx.Client) -> None:
    ...
    from xiuqo_cli.auth import _nous_portal_base_url
...
def clear_dead_guest(reason: str, *, dead_token: Optional[str] = None) -> None:
    ...
    *dead_token* is None the profile's current guest is treated as the failed one.
    ...
    from xiuqo_cli.auth_nous import _clear_shared_nous_state, _nous_shared_store_lock, _read_shared_nous_state
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
_WELCOME_ROUTE_REFUSALS = (
    ...
    ("serves anonymous xiuqo agent accounts only", "named_on_welcome_host"),
...
_WELCOME_ROUTE_COPY = {
    "anon_on_paid_host": "The Nous free tier must use its own inference host ({host}); "
                         "Xiuqo is pointed at the paid one. Restart Xiuqo to re-read the route, "
...
_SIGNIN_TERMINAL = "Sign in with a Nous account for the full catalog: `xiuqo auth upgrade`."
...
def apply_model_switch(agent: Any) -> Optional[str]:
    ...
    map. The config write is the same one a sign-in completion uses, so ``xiuqo model`` and the
    ...
    try:
        ...
        if str(model_cfg.get("default") or "").strip() == requested:
            from xiuqo_cli.auth import _update_config_for_provider
...
# --- ``xiuqo auth upgrade``: sign the guest into a real Nous account, keeping its connectors ---------
#
...
def _account_state_from_token(
    ...
    from xiuqo_cli.auth import PROVIDER_REGISTRY, _coerce_ttl_seconds, _optional_base_url, _tls_state_from_verify
    from xiuqo_cli.auth_nous import _NOUS_EMPTY_AGENT_KEY_FIELDS, _iso_after, refresh_nous_oauth_from_state
    now = datetime.now(timezone.utc)
...
def settle_after_upgrade(account_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    default for the account's tier (:func:`xiuqo_cli.models.recommended_nous_default_model`, the
    ...
    Every sign-in completion (CLI ``xiuqo auth upgrade``, the desktop poller) calls this once, after
    ...
    model = current
    if on_welcome_model:
        from xiuqo_cli.models import recommended_nous_default_model
        try:
    ...
    try:
        ...
        # same write; the runtime's silent default applies until the user picks one with `xiuqo model`.
        _update_config_for_provider(
...
def _poll_for_token(*args, **kwargs) -> Dict[str, Any]:
    ...
    from xiuqo_cli.auth_device_flow import _poll_for_token as poll
...
def persist_nous_credentials(*args, **kwargs):
    ...
    from xiuqo_cli.auth_nous import persist_nous_credentials as persist
...
from xiuqo_cli.anon_sign_in import (  # noqa: E402
...
from xiuqo_cli.anon_sign_in_cli import (  # noqa: E402
