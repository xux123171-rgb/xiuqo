...
Split out of :mod:`xiuqo_cli.plugins`; validation warns and never fails a load.
"""
...
# and continue loading. ``capabilities``/``emits``/``listens``/``xiuqo``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "xiuqo", "depends",
    "requires_hermes",
}

# Highest manifest schema version this Xiuqo understands.
...
def _plugins_debug() -> bool:
    from xiuqo_cli import plugins as _origin
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Xiuqo "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `xiuqo plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `xiuqo plugins list`: ``disk-cleanup``
    # for a flat plugin, ``image_gen/openai`` for a category plugin. Empty -> name.
    key: str = ""
    # Xiuqo version requirement (``">=0.19"``, comma-separated clauses allowed). Unsatisfied plugins are
    ...
    # VALIDATED AND SURFACED ONLY — Xiuqo never auto-installs these (isolation design for the install seam
...
def running_hermes_version() -> str:
    """Installed ``xiuqo`` distribution version, else ``xiuqo_cli.__version__`` (source checkout)."""
    try:
        return importlib.metadata.version("xiuqo")
    except Exception:
...
def requires_hermes_error(manifest: "PluginManifest") -> Optional[str]:
    ...
    if version_satisfies(manifest.requires_hermes, current):
        return None
    return f"requires xiuqo {manifest.requires_hermes}, running {current}"
