# xiuqo_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles
...
`xiuqo_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
Skill slash commands (`agent/skill_commands.py`) scan `~/.xiuqo/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`xiuqo_cli/curses_ui.py`; example
`xiuqo_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`xiuqo_cli/commands.py`)
...
`xiuqo_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`xiuqo_cli/config.py`)
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`xiuqo tools/setup`, most subcommands, `xiuqo_cli/config.py`, merges `DEFAULT_CONFIG`);
  `xiuqo_cli/config_effective.py::load_user_config_effective()` (gateway runtime via
  `gateway/run.py::_load_gateway_config`, TUI gateway `_load_cfg`, cron, `xiuqo send`, doctor,
  `xiuqo_time`/`xiuqo_logging`: user file + managed overlay + `${VAR}` expansion + model-key
...
## Skin engine (`xiuqo_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.xiuqo/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`xiuqo update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `xiuqo update --plan`): read-only inventory — install kind, all
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `xiuqo-gateway*` unit /
  `ai.xiuqo.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.xiuqo/logs/update_receipts/`
...
`_apply_profile_override()` in `xiuqo_cli/main.py` sets `XIUQO_HOME` before any module import, so
...
tool registry overlays) key on `xiuqo_constants.xiuqo_home_key()`, never a single flag.
Migration from per-profile gateways: `xiuqo_cli/gateway_migrate.py` (`xiuqo gateway migrate
...
## Nous free tier (`xiuqo_cli/anon_auth.py`)
...
which never contains a raw exception, a URL or a `xiuqo` verb) and `.copy_terminal`, so no caller
...
contextvars. `upgrade_guest` (`xiuqo auth upgrade`), the CLI `/login` handler and the desktop
