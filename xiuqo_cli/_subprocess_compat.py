...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_XIUQO_GATEWAY_BREAKAWAY"
...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    Xiuqo's plumbing calls. ``core.sshCommand`` is pinned to ``ssh -o BatchMode=yes`` so the ssh
    ...
    Xiuqo shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def _text_names_hermes(text: str) -> bool:
    r"""True when *text* names Xiuqo at a path-segment / token boundary.

    A bare ``"xiuqo" in text`` substring test would also match unrelated processes whose paths
    merely contain the letters (``...\shermesa\...``) — the false-positive class this prevents.
    """
    return any(token.startswith(("xiuqo", ".xiuqo"))
               for token in re.split(r"[\\/\s=,;\"']+", text.lower()))


def _process_command_is_hermes(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Xiuqo code."""
    try:
        import psutil
...
def pid_is_hermes(pid: int, *, expected_start_time: int | None = None) -> bool:
    ...
    The PID must be valid, currently exist, and identify a Xiuqo process. When the caller captured
...
def bounded_probe_run(
    ...
    ``Get-CimInstance Win32_Process`` gateway scan hit exactly this during ``xiuqo update`` on slow-WMI
