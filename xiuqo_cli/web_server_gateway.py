...
from xiuqo_cli._subprocess_compat import windows_detach_flags
...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    """
    from xiuqo_cli.web_server import _GATEWAY_HEALTH_TIMEOUT, _GATEWAY_HEALTH_URL
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    """
    try:
        from xiuqo_cli.profiles import _check_gateway_running, profiles_to_serve
...
# ~/.xiuqo/logs/<action>.log which the dashboard tails.
...
def _dashboard_spawn_executable() -> str:
    ...
    """
    from xiuqo_cli.web_server import PROJECT_ROOT
...
def _profile_action_environment(
    ...
    """Environment for a detached ``xiuqo <subcommand>`` action.

    The dashboard loads its own profile's ``.env`` into process-global ``os.environ``. Copying
    that mapping verbatim into ``xiuqo -p <other> ...`` lets the named child see the dashboard
    ...
    Named-profile actions therefore start from Xiuqo' standard scrubbed subprocess env, then drop
    the profile-managed keys plus every key declared by the dashboard/default profile dotenv files
    and their hydrated secret sources, and pin ``XIUQO_HOME`` to the target profile. The child's
    ...
    if profile is None:
        ...
        from xiuqo_cli.env_loader import (
            _PROFILE_MANAGED_ENV_KEYS, _env_keys_defined_in_dotenv, get_secret_source_values,
        )
        from xiuqo_cli.web_server_profiles import _resolve_profile_dir
        from xiuqo_constants import apply_subprocess_home_env, get_default_hermes_root
        ...
        action_env["XIUQO_HOME"] = str(target_home)
        apply_subprocess_home_env(action_env)

    action_env["XIUQO_NONINTERACTIVE"] = "1"
    # The dashboard runs inside the gateway process, so os.environ carries _XIUQO_GATEWAY=1;
    ...
    action_env.pop("_XIUQO_GATEWAY", None)
    if env_overrides:
...
def _spawn_hermes_action(
    ...
    """Spawn ``xiuqo <subcommand>`` detached (via ``xiuqo_cli.main``) and record the handle."""
    from xiuqo_cli.web_server import PROJECT_ROOT
    _ACTION_LOG_DIR.mkdir(parents=True, exist_ok=True)
    log_file = open(_ACTION_LOG_DIR / _ACTION_LOG_FILES[name], "ab", buffering=0)
    log_file.write(f"\n=== {name} started {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n".encode())

    cmd = [_dashboard_spawn_executable(), "-m", "xiuqo_cli.main", *subcommand]
    # Named-profile actions get a scrubbed, pinned environment so the child cannot inherit the
    # dashboard profile's credentials; see _profile_action_environment (also drops _XIUQO_GATEWAY).
    ...
    action_id = (env_overrides or {}).get("XIUQO_ACTION_ID")
    if action_id:
...
def _own_profile_selector(profile: Optional[str]) -> Optional[str]:
    ...
    profile (a pooled Desktop ``xiuqo --profile X serve`` answers ``/api/gateway/*`` without
...
def _gateway_subcommand(profile: Optional[str], verb: str) -> List[str]:
    """``xiuqo [-p X] gateway <verb>`` argv for a dashboard lifecycle action. A profile served by the
    ...
    pooled ``--profile X serve`` would inherit X's ``XIUQO_HOME`` and hit the same exit-78 refusal."""
    from xiuqo_cli.web_server_profiles import _profile_cli_args
...
def multiplexed_profile_refusal(profile: Optional[str], verb: str) -> Optional[str]:
    ...
    The spawned ``xiuqo -p X gateway <verb>`` would only print exit-78 / "no gateway running for this
...
def _restart_gateway_after(profile: Optional[str], *, what: str, label: str) -> dict[str, Any]:
    ...
    from xiuqo_cli.web_server import _spawn_gateway_restart
    try:
