...
def _is_hermes_gateway_run_argv(command: Sequence[str]) -> bool:
    """True for Xiuqo ``gateway run`` argv this wrapper is allowed to upgrade.

    The wrapper is generic. Only historical/current Xiuqo gateway shapes get ``--external-
...
def _prepare_child_command(command: Sequence[str], environ: Mapping[str, str] | None = None) -> list[str]:
    ...
    plists put ``--external-supervisor`` on the inner ``gateway run`` so ``xiuqo update`` can see
