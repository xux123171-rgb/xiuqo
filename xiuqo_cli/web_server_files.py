...
_MANAGED_FILES_ROOT_ENV = "XIUQO_DASHBOARD_FILES_ROOT"
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("XIUQO_HOME", "").strip()
    if not raw:
        return False
    try:
...
def _dashboard_local_update_managed_externally() -> bool:
    """True when the dashboard should not offer ``xiuqo update``.

    Containerized dashboards are updated by the outer launcher/image — except a
    ``git`` install (bind-mounted checkout, e.g. the xiuqo-webui image), where
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # install still browses its home). Lock to /opt/data only when the Xiuqo
    # root actually IS /opt/data or XIUQO_DASHBOARD_FILES_ROOT is set.
    if _default_hermes_root_is_opt_data():
