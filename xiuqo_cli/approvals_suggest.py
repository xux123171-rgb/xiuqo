"""``xiuqo approvals suggest`` — mine approval history into allowlist proposals.

Xiuqo has no dedicated approval-decision ledger: ``always`` answers land in ``command_allowlist``
...
def normalize_command(command: str) -> str:
    """Fold user/xiuqo home prefixes and collapse whitespace."""
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  xiuqo approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.xiuqo/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``xiuqo approvals suggest``."""
    ...
    if apply_spec:
        ...
        if as_json:
            ...
            for pattern in applied:
                print(f"  + {pattern}")
            print(f"\ncommand_allowlist now has {len(merged)} entries (~/.xiuqo/config.yaml).")
        return 0
...
def approvals_command(args) -> int:
    """Dispatch ``xiuqo approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from xiuqo_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: xiuqo approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `xiuqo approvals <subcommand> -h` for details."
    )
    return 1
