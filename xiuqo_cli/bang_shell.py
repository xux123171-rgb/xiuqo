...
def parse_bang_command(text: str) -> str:
    ...
    (history expansion), not to Xiuqo.
    """
...
def bang_shell_enabled() -> bool:
    ...
    return not (env_var_enabled("XIUQO_GATEWAY_SESSION") or env_var_enabled("XIUQO_CRON_SESSION")
                or (os.getenv("XIUQO_SESSION_PLATFORM") or "").strip())
...
def _bang_env() -> dict:
    """Environment for a bang command with Xiuqo-managed secrets filtered.
...
def run_bang_command(command: str, *, cwd: Optional[str] = None, timeout: int = DEFAULT_TIMEOUT, writer=None) -> int:
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the Xiuqo session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:
