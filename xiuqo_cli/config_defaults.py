"""Default configuration data for Xiuqo Agent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from xiuqo_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["xiuqo-cli"],
    # journal_mode: SQLite journal mode for every Xiuqo DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `xiuqo -c`: each CLI session writes a breadcrumb under
        # $XIUQO_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `xiuqo
        ...
        # Xiuqo-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping Xiuqo (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env XIUQO_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `xiuqo doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`xiuqo status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # XIUQO_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else XIUQO_HOME/cache/terminal (auto-pruned
        ...
        # XIUQO_HOME/home; "real" = force real HOME; "profile" = force XIUQO_HOME/home when it
        ...
        # ["/home/user/.xiuqo/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled Xiuqo image,
        # which drops to `xiuqo` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `xiuqo tools`, not a ring
        # member). Set by the `xiuqo tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a Xiuqo-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by Xiuqo' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.xiuqo/browser-profile/ so credentials
        ...
        # runs `xiuqo browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `xiuqo chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.xiuqo/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `xiuqo checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .xiuqo.md,
    ...
    # Same bound for single-query mode (``xiuqo -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # Xiuqo' summarizer cannot shrink it. native = codex decides; xiuqo = Xiuqo' threshold
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `xiuqo model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # Interface bare `xiuqo`/`xiuqo chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / XIUQO_TUI=1 forces the TUI.
        "interface": "cli",
        # `xiuqo --tui` auto-resumes the most recent human-facing session (like `xiuqo -c`).
        # XIUQO_TUI_RESUME=<id> always wins.
        ...
        # `path`/`paths` = XIUQO_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/xiuqo`.
        ...
        # CLI/TUI/desktop, managed with `xiuqo pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: XIUQO_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than XIUQO_TUI_SESSION_TTL_S, default 6h) with
        ...
        # plugin. Env XIUQO_DASHBOARD_OAUTH_CLIENT_ID / XIUQO_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env XIUQO_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # XIUQO_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env XIUQO_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/xiuqo) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.xiuqo/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    "voice": {
        ...
        #   chained  — STT → Xiuqo turn → TTS (the stt.* / tts.* providers below)
        #   gpt-live — one full-duplex voice model (OpenAI GPT-Live) owns the mic and speaker and
        #              DELEGATES every real request to Xiuqo (any model / provider you have
    ...
    # "Hey Xiuqo" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    # plugins/context_engine/<name>/ or ~/.xiuqo/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.xiuqo/cache/delegation/ with a head+tail window + read_file offset footer, nothing
    ...
    "moa": {
        ...
        # <xiuqo_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.xiuqo/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # XIUQO_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.xiuqo/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `xiuqo skills trust` / `untrust`.
        "trusted_project_dirs": [],
        # Substitute ${XIUQO_SKILL_DIR} / ${XIUQO_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `xiuqo skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.xiuqo/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.xiuqo/.curator_backups/blobs/); powers `xiuqo
    ...
    # daemon. `xiuqo curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `xiuqo curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `xiuqo update` restoring them);
        ...
        "prune_builtins": True,
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `xiuqo curator purge`
        # delete older archived skills (never automatic; logged in the ledger).
        "archive_ttl_days": 0,
        # Before every real (non-dry-run) pass, snapshot ~/.xiuqo/skills/ to
        # ~/.xiuqo/skills/.curator_backups/<utc-iso>/skills.tar.gz (`xiuqo curator rollback`).
        "backup": {
            "enabled": True,
    ...
    # peerName, sessions, enabled); xiuqo-specific overrides only here.
    "honcho": {},
    ...
    "whatsapp": {
        # reply_prefix: None = built-in "☤ *Xiuqo Agent*" header; "" disables; \n allowed.
    },
    ...
    #   cron job, a -q session (XIUQO_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export XIUQO_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. XIUQO_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `xiuqo plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `xiuqo plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.xiuqo/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # XIUQO_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .xiuqo config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `xiuqo doctor --ack <id>`; remove by editing the list. Catalog:
        # xiuqo_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $XIUQO_HOME/plugins/ <name>/),
        ...
        # Env override: XIUQO_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: XIUQO_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: XIUQO_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: XIUQO_CRON_MEDIA_SEND_TIMEOUT.
    ...
    # promotes dependency-satisfied todos to ready, and fires `xiuqo -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `xiuqo kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with xiuqo's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core Xiuqo tools
    ...
    "logging": {  # File logging to ~/.xiuqo/logs/: agent.log captures INFO+, errors.log WARNING+.
    ...
    "model_catalog": {
        ...
        # CLI refetches on the next /model or `xiuqo model` once the cache is older. Network
    ...
    # defaults (200K context, tools on, vision/reasoning off) and get patched. Provider keys: Xiuqo
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "xiuqo-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to XIUQO_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to XIUQO_STARTUP_WATCHDOG / XIUQO_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.xiuqo/sessions/sessions.json mirror of the routing index
        ...
        # stamps the profile into session keys. Flip with `xiuqo gateway migrate --multiplex`
        # (records a rollback manifest; `--standalone` undoes it) or `xiuqo config set
        # gateway.multiplex_profiles true` + `xiuqo gateway restart`. GATEWAY_MULTIPLEX_PROFILES
        # in the environment overrides. Two profiles configuring the same bot token cannot be
        # served together — the duplicate adapter is parked; `xiuqo profile create --clone`
        ...
        # (XIUQO_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # XIUQO_GATEWAY_MAX_STARTS / XIUQO_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.xiuqo/.env, auth.json). True:
        # files must be under the Xiuqo cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to XIUQO_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the Xiuqo cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to XIUQO_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to XIUQO_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # XIUQO_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.xiuqo/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `xiuqo sessions prune`).
        ...
        # stay until `xiuqo sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `xiuqo update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.xiuqo/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to XIUQO_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`xiuqo sessions export`); checked
    ...
    "doctor": {
        # Per-probe timeout (seconds) for `xiuqo doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Passive version/banner checks only; explicit `xiuqo update --check` remains enabled.
        "check": True,
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <XIUQO_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``xiuqo backup`` zip in
        # <XIUQO_HOME>/backups/ (``xiuqo import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `xiuqo update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `xiuqo update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <XIUQO_HOME>/lsp/bin/ on
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `xiuqo tools`.
    "x_search": {
    ...
    # encrypted vault (`xiuqo vault add`, Desktop → Settings → Credential Vault) is always on.
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.xiuqo/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.xiuqo/cache/,
            ...
            # Auto-download bws into ~/.xiuqo/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; Xiuqo sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; Xiuqo also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.xiuqo/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # Xiuqo Desktop (Electron) launch options; only affect `xiuqo desktop`.
        ...
        # Bridged to XIUQO_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to XIUQO_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "nous": {
        ...
        "anthropic_wire": "chat",
        # Nous free tier: with no other provider configured, Xiuqo sets up a free Nous identity on
        # first use (inference on nous/welcome + connectors) and offers `/login` (terminal:
        # `xiuqo auth upgrade`) to sign in. false turns the free tier off entirely: nothing is set
    ...
    "local_runtime": {
        # Off = detection-only (Xiuqo still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by Xiuqo releases after validation.
        "tag": "b10679",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials Xiuqo mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'xiuqo model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        ...
        "read from the Xiuqo auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey Xiuqo' wake word engine (optional; "
    ...
    "XIUQO_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "XIUQO_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "XIUQO_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use Xiuqo, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @xiuqo:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. XIUQO_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #xiuqo)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: xiuqo-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'xiuqo' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote Xiuqo API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote Xiuqo API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote Xiuqo API server (proxy mode). Must "
    ...
    # XIUQO_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. XIUQO_TOOL_PROGRESS is unsupported.
    "XIUQO_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "XIUQO_EPHEMERAL_SYSTEM_PROMPT": _setting(
