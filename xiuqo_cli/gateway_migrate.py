"""``xiuqo gateway migrate --multiplex`` / ``--standalone``: move a per-profile-gateway install onto one
...
what the multiplexer would do at startup. ``xiuqo update`` calls :func:`maybe_auto_migrate_after_update`.
"""
...
MIGRATE_COMMAND = "xiuqo gateway migrate --multiplex"
...
class MigrationPlan:
    ...
    def eligible_for_migration(self) -> bool:
        ...
        This is the AUTO-migration (``xiuqo update``) bar; the explicit command also proceeds with
...
def _home_env(home: Path) -> Iterator[None]:
    """Run service-manager helpers as if ``home`` were the active XIUQO_HOME. Both the contextvar
    ...
    previous = os.environ.get("XIUQO_HOME")
    token = set_hermes_home_override(str(home))
    os.environ["XIUQO_HOME"] = str(home)
    xiuqo_constants._default_hermes_root_memo = None
    try:
        yield
    finally:
        ...
        if previous is None:
            ...
            os.environ["XIUQO_HOME"] = previous
        xiuqo_constants._default_hermes_root_memo = None
...
def _installed_service(home: Path) -> Optional[tuple[str, bool]]:
    ...
    from xiuqo_cli import gateway as gw
    with _home_env(home):
...
def _stop_gateway_process(home: Path) -> None:
    from xiuqo_cli.profiles import _stop_gateway_process
...
def _write_multiplex_flag(default_home: Path, value: bool) -> None:
    ...
    (same read-guard + nested-set + atomic write ``xiuqo config set`` uses; no raw YAML edits)."""
    from xiuqo_cli.config import _set_nested, _write_user_config, require_readable_config_before_write
    cfg_path = default_home / "config.yaml"
        ...
        for platform, platform_config in cfg.platforms.items():
            ...
            if platform_serves_profile_prefix(platform.value):
                ...
                plan.blockers.append(
                    ...
                    f"keep '{profile.name}' on a standalone gateway (xiuqo -p {profile.name} gateway start --force)."
                )
...
def format_plan(plan: MigrationPlan, *, dry_run: bool) -> list[str]:
    ...
    lines.append(f"  - record the previous state in {plan.default_home / MANIFEST_NAME} (rollback: xiuqo gateway migrate --standalone)")
    return lines + _plan_tail(plan)
...
def _no_manifest_lines(default_home: Path) -> list[str]:
    return [f"✗ No migration manifest at {_manifest_path(default_home)}; nothing to roll back.",
            "  To leave multiplex mode by hand: xiuqo config set gateway.multiplex_profiles false && xiuqo gateway restart"]
...
def format_update_warning(plan: MigrationPlan) -> list[str]:
    return [
        ...
        *[f"    • {b}" for b in plan.blockers],
        ...
        "  (`xiuqo update` will migrate automatically once nothing blocks it.)",
    ]
...
def apply_migration(plan: MigrationPlan, *, served_wait: float = _SERVED_WAIT_SECONDS) -> bool:
    ...
    if served is not None and expected <= set(served):
        _print(["", f"✓ Migrated: the default gateway now serves {len(served)} profiles: {', '.join(served)}",
                f"  Rollback any time with: xiuqo gateway migrate --standalone",
    ...
    _print(["", f"⚠ Migration applied, but the default gateway has not confirmed serving: {', '.join(missing)}",
            "  Check `xiuqo gateway status` and the gateway log; the flag and manifest are in place.",
            "  Rollback: xiuqo gateway migrate --standalone"])
    return False
...
def rollback_migration(default_home: Optional[Path] = None) -> bool:
    ...
    if default_gw.has_gateway:
        try:
            ...
            print("    The default gateway is still multiplexing; stop it by hand (xiuqo gateway stop) and re-run.")
    if ok:
...
def _host_supports_migration() -> Optional[str]:
    ...
    if gw.is_windows():
        return "Windows Scheduled Tasks are not migrated automatically; set gateway.multiplex_profiles true, stop the per-profile tasks, and `xiuqo gateway restart`."
    return None


def cmd_migrate(args) -> None:
    """``xiuqo gateway migrate [--multiplex|--standalone] [--dry-run] [--yes]``."""
...
def maybe_auto_migrate_after_update() -> None:
    """``xiuqo update`` hook: with >= 2 profiles, per-profile gateways present and multiplex off,
