...
_TASK_DESCRIPTION = "Xiuqo Agent Gateway - Messaging Platform Integration"
_TASK_LOGON_DELAY = "PT30S"
_TASK_RESTART_INTERVAL = "PT1M"
_TASK_RESTART_COUNT = 999

_GATEWAY_ENV = (("PYTHONIOENCODING", "utf-8"), ("XIUQO_GATEWAY_DETACHED", "1"), ("XIUQO_SUPERVISED_CHILD", "1"))
...
def _preserve_hermes_home_path(path: str | Path) -> str:
    r"""Render Xiuqo-owned paths under the configured XIUQO_HOME spelling.

    ``%LOCALAPPDATA%\xiuqo`` may be a symlink/junction to another drive; launcher files must not
    bake in the resolved target for paths under XIUQO_HOME.
    """
    candidate = Path(path)
    try:
        home = _hermes_home()
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Xiuqo profile."""
    from xiuqo_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "xiuqo_cli.main", *_current_profile_cli_args(), "gateway", command, *(extra_args or [])]
...
def _launch_elevated_install(force: bool = False, *, start_now: bool | None = None, start_on_login: bool | None = None) -> bool:
    ...
    overrides = {"XIUQO_GATEWAY_ELEVATED_HANDOFF": "1"}
    extra_args = ["--elevated-handoff"]
    if force:
        extra_args.append("--force")
    for choice, env_key, flag in (
        (start_now, "XIUQO_GATEWAY_INSTALL_START_NOW", "start-now"),
        (start_on_login, "XIUQO_GATEWAY_INSTALL_START_ON_LOGIN", "start-on-login"),
    ):
...
def get_task_name() -> str:
    ...
    _assert_windows()
    from xiuqo_cli.gateway import _profile_suffix  # local: avoids circular init during boot
...
def get_task_script_path() -> Path:
    """The generated ``gateway.cmd`` wrapper under ``<XIUQO_HOME>/gateway-service/`` (per-profile
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Stable cwd for detached/startup runs: anchor at XIUQO_HOME when it exists (mirrors the POSIX
...
def _gateway_run_argv(python_exe: str, profile_arg: str) -> list[str]:
    """``python -m xiuqo_cli.main [--profile X] gateway run`` — shared by every launcher renderer."""
    argv = [python_exe, "-m", "xiuqo_cli.main"]
    if profile_arg:
...
def _launcher_settings() -> tuple[str, str, str, str]:
    """Return (python_path, working_dir, xiuqo_home, profile_arg) for generated launchers."""
    from xiuqo_cli.gateway import PROJECT_ROOT, _profile_arg, get_python_path  # avoid circular init

    xiuqo_home = str(_hermes_home())
    return (
        _preserve_hermes_home_path(get_python_path()),
        _stable_gateway_working_dir(PROJECT_ROOT),
        xiuqo_home,
        _profile_arg(xiuqo_home),
    )
...
def _build_gateway_cmd_script(python_path: str, working_dir: str, xiuqo_home: str, profile_arg: str) -> str:
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, xiuqo_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f"env.Item({q('XIUQO_HOME')}) = {q(xiuqo_home)}",
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    _assert_windows()
    from xiuqo_cli.gateway import PROJECT_ROOT

    python_path, working_dir, xiuqo_home, profile_arg = _launcher_settings()
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    env_overlay = {"XIUQO_HOME": xiuqo_home, **dict(_GATEWAY_ENV), "VIRTUAL_ENV": _preserve_hermes_home_path(venv_dir)}
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    interpreter via ``_resolve_detached_python`` and supplies the stable cwd + env overlay (XIUQO_HOME,
    ...
    try:
        xiuqo_home = str(_hermes_home().resolve())
    except Exception:
        xiuqo_home = ""
    env_overlay: dict[str, str] = {"PYTHONIOENCODING": "utf-8", "XIUQO_GATEWAY_DETACHED": "1", "VIRTUAL_ENV": str(venv_dir)}
    if xiuqo_home:
        env_overlay["XIUQO_HOME"] = xiuqo_home
...
def _prompt_install_choices(start_now: bool | None = None, start_on_login: bool | None = None) -> tuple[bool, bool]:
    ...
    if start_now is None:
        start_now = _install_choice_from_env("XIUQO_GATEWAY_INSTALL_START_NOW")
    if start_on_login is None:
        start_on_login = _install_choice_from_env("XIUQO_GATEWAY_INSTALL_START_ON_LOGIN")
    if start_now is not None and start_on_login is not None:
        return start_now, start_on_login

    from xiuqo_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    if running_pids or start_now:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"xiuqo {profile_arg} gateway start" if profile_arg else "xiuqo gateway start"
...
def _offer_elevated_install(headline: str, force: bool, start_now: bool, start_on_login: bool) -> bool:
    ...
    if prompt_yes_no("  Open the UAC prompt now?", False):
        if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
            print("✓ Launched elevated Xiuqo gateway install prompt.")
            if start_now:
                print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
            else:
                print("  Approve the Windows UAC prompt, then run: xiuqo gateway status")
            return True
...
def install(
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: xiuqo gateway start")
        return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: xiuqo gateway start")
        _print_next_steps()
        return
...
def check_start_attestation(current_pids: list[int] | None = None) -> str | None:
    ...
    lines = [
        ...
        "shutdown record.",
        "  This usually means the shell that ran `xiuqo gateway start` was inside "
...
def _print_next_steps() -> None:
    print("\nNext steps:\n  xiuqo gateway status                      # Check status")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_gateway_command("uninstall"):
                    print("✓ Launched elevated Xiuqo gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: xiuqo gateway status")
                    return
...
def start() -> None:
    ...
    if not is_task_registered() and not is_startup_entry_installed():
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: xiuqo gateway install")
            return
        install(force=False)
        if not is_task_registered() and not is_startup_entry_installed():
            print("⚠ Gateway install did not complete in this process.")
            print("  If a UAC prompt opened, approve it, then run: xiuqo gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `xiuqo gateway status`."
        )
