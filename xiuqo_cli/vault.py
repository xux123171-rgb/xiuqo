"""``xiuqo vault`` — manage the local encrypted autofill vault.

Subcommands:
- ``xiuqo vault add``   interactive wizard; the password is read via
  getpass (never echoed, never accepted as argv). The login identifier is
  visible metadata and prompted normally.
- ``xiuqo vault list``  metadata — labels, kinds, identifiers, origins,
  handles. Passwords are never shown.
- ``xiuqo vault rm``    remove an item by handle/id.
...
def _cmd_list(args) -> None:
    ...
    `xiuqo vault list` unlock does not carry into a chat session — unlock there when asked)."""
    ...
    if not rows and not locked:
        c.print("[dim]Vault is empty. Add an item with `xiuqo vault add`.[/]")
        return
    if rows:
...
def _cmd_sources(args) -> None:
    ...
    for name, cls in classes.items():
        if name in enabled:
            ...
            status = "[dim]turned off[/] (`xiuqo vault sources --enable {name}` to use it)".format(name=name)
        else:
...
def register_cli(subparser) -> None:
    """Build the ``xiuqo vault`` argparse tree (called from main.py)."""
    ...
    p_rm.add_argument("handle", help="Item handle (see `xiuqo vault list`)")
