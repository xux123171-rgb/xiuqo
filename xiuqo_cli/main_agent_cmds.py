...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from xiuqo_cli.memory_setup import memory_command
        memory_command(args)
...
def cmd_acp(args):
    """Launch Xiuqo Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        ...
        from xiuqo_cli.tools_config import run_post_setup_command
...
def cmd_skills(args):
    ...
    if action == "config":
        ...
        from xiuqo_cli.skills_config import skills_command as skills_config_command
        skills_config_command(args)
    elif action in ("trust", "untrust"):
        _cmd_skills_trust(args)
    else:
        from xiuqo_cli.skills_hub import skills_command
        skills_command(args)


def _cmd_skills_trust(args):
    """``xiuqo skills trust|untrust [path]`` — manage ``skills.trusted_project_dirs``.
