...
from xiuqo_cli.web_models import CronJobCreate
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from xiuqo_cli.web_server_profiles import _fallback_profile_dicts
...
def _cron_default_profile() -> str:
    ...
    profile's job into ~/.xiuqo. ``custom`` (XIUQO_HOME outside the profiles tree) has no
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, XIUQO_HOME)."""
    from xiuqo_cli import profiles as profiles_mod
...
def _cron_store_scope(home: Path):
    """Point XIUQO_HOME and the cron.jobs store at one profile's home for the block.
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # expansion) — never a raw yaml.safe_load (tests/xiuqo_cli/test_config_read_guard.py).
        from xiuqo_constants import reset_hermes_home_override, set_hermes_home_override
