...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or xiuqo -c.",
    ...
    "Ctrl+Z suspends Xiuqo to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "xiuqo -c resumes your most recent CLI session. xiuqo -c \"project name\" resumes by title.",
    "xiuqo -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "xiuqo -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "xiuqo chat -t web,terminal enables only specific toolsets for a focused session.",
    "xiuqo chat -s github-pr-workflow preloads a skill at launch.",
    "xiuqo chat -q \"query\" runs a single non-interactive query and exits.",
    "xiuqo chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "xiuqo chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "xiuqo --yolo bypasses all dangerous command approval prompts for the entire session.",
    "xiuqo chat --source telegram tags the session for filtering in xiuqo sessions list.",
    "xiuqo -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "xiuqo doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "xiuqo dump outputs a compact setup summary — great for bug reports.",
    "xiuqo config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "xiuqo config edit opens config.yaml in your default editor.",
    "xiuqo config check scans for missing or stale configuration options.",
    "xiuqo sessions browse opens an interactive session picker with search.",
    "xiuqo sessions stats shows session counts by platform and database size.",
    "xiuqo sessions prune --older-than 30 cleans up old sessions.",
    "xiuqo skills search react --source skills-sh searches the skills.sh public directory.",
    "xiuqo skills check scans installed hub skills for upstream updates.",
    "xiuqo skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "xiuqo skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "xiuqo mcp add github --command npx adds MCP servers from the command line.",
    "xiuqo mcp serve runs Xiuqo itself as an MCP server for other agents.",
    "xiuqo auth add lets you add multiple API keys for credential pool rotation.",
    "xiuqo completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "xiuqo logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "xiuqo backup creates a zip backup of your entire Xiuqo home directory.",
    "xiuqo profile create coder creates an isolated profile that becomes its own command.",
    "xiuqo profile create work --clone copies your current config and keys to a new profile.",
    "xiuqo update syncs new bundled skills to ALL profiles automatically.",
    "xiuqo gateway install sets up Xiuqo as a system service (systemd/launchd).",
    "xiuqo memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "xiuqo webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: xiuqo tools disables unused tools, xiuqo skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "xiuqo models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    # --- Configuration ---
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as xiuqo -w).",
    ...
    "XIUQO_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Xiuqo tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'xiuqo profile create coder' creates the 'coder' command.",
    "xiuqo profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Xiuqo prints a resume command with session ID and stats.",
    "xiuqo sessions export backup.jsonl exports all sessions for backup or analysis.",
    "xiuqo -r SESSION_ID resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    "xiuqo skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .xiuqo/plans/ without executing anything.",
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: xiuqo cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Xiuqo runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "xiuqo gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Xiuqo via a local macOS server.",
    ...
    "Context files (.xiuqo.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Xiuqo loads project context from .xiuqo.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "xiuqo mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "xiuqo mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "xiuqo mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — xiuqo tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.xiuqo/checkpoints/ — your project's .git is never touched.",
    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "xiuqo chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "xiuqo plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "xiuqo acp runs Xiuqo as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "XIUQO_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "xiuqo auth add nous or xiuqo auth add openai-codex sets up OAuth-based providers.",
    ...
    "xiuqo status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.xiuqo/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Xiuqo your own.",
    ...
    "Before a gateway session reset, Xiuqo auto-flushes important facts to memory in the background.",
    ...
    "execute_code helpers require explicit imports: from xiuqo_tools import json_parse, shell_quote, retry.",
    ...
    "The skills audit log at ~/.xiuqo/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Xiuqo state via XIUQO_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "XIUQO_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in XIUQO_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.xiuqo/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'xiuqo profile create ls' would be rejected.",
    "xiuqo profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "xiuqo claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "XIUQO_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via xiuqo tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.xiuqo/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.xiuqo/skills/.hub/quarantine/ holds skills pending security review.",
    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Xiuqo auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Xiuqo config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Xiuqo is working.',
    ...
    '/reload reloads ~/.xiuqo/.env into the running session — pick up new API keys without restarting.',
    ...
    "XIUQO_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.xiuqo/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.xiuqo/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    # --- Curator ---
    'xiuqo curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "xiuqo curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'xiuqo curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'xiuqo auth reset <provider> [target] clears cooldowns and exhaustion flags on a whole credential pool, or on one credential by index, id, or label.',
    ...
    # --- TUI & Dashboard ---
    'XIUQO_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "XIUQO_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'xiuqo dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'xiuqo dashboard embeds the full Xiuqo TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.xiuqo/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.xiuqo/dashboard-plugins/ — no npm build required.',
    ...
    'XIUQO_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'XIUQO_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'XIUQO_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'XIUQO_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.xiuqo/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.xiuqo/skills/ so drop-in skills appear without restarting the session.',
    ...
    'xiuqo -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'xiuqo chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'xiuqo chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'xiuqo chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "xiuqo chat --source tool tags programmatic chats so they don't clutter xiuqo sessions list.",
    'xiuqo dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'xiuqo sessions rename <ID> "new title" renames any past session; xiuqo sessions delete <ID> removes one.',
    'xiuqo import restores a full Xiuqo backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'xiuqo fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'xiuqo pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'xiuqo setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'xiuqo status --deep runs the full health sweep across every component; plain xiuqo status is the quick view.',
    # --- Agent Behavior Env Vars ---
    'XIUQO_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'XIUQO_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.xiuqo/plugins/ — trust-gated by design.',
    "XIUQO_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'XIUQO_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'XIUQO_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'XIUQO_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'XIUQO_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'XIUQO_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'XIUQO_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',
    # --- Gateway Behavior Env Vars ---
    'XIUQO_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'XIUQO_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'XIUQO_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'XIUQO_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Xiuqo block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from xiuqo sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.xiuqo/dashboard-plugins/.',
]
...
# Task-oriented example prompts for the empty composer. Kept generic — Xiuqo is
