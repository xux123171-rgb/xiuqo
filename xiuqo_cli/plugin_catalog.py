"""Plugin catalog — curated, Nous-approved Xiuqo plugins shipped with the repo.
...
an installed Xiuqo sees new entries and removals without updating. Any fetch failure falls back to the
...
def fetch_live_catalog(*, force: bool = False) -> Optional[Dict[str, Any]]:
    ...
    ``XIUQO_HOME/cache`` for :data:`LIVE_CATALOG_TTL_SECONDS`. ``None`` on ANY failure — callers fall
...
def entry_capability_summary(entry: PluginCatalogEntry) -> str:
    ...
    if entry.requires_hermes:
        bits.append(f"Requires Xiuqo {entry.requires_hermes}.")
    return " ".join(bits)
