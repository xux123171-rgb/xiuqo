"""Per-profile view of ``xiuqo_cli.models``' module-level catalog slots.
...
files. In a multiplexed gateway every turn runs under a XIUQO_HOME override, so a single module slot
...
def profile_slot_get(module: Any, attr: str, default: Any = None) -> Any:
    ...
    return _SLOTS_BY_HOME.get((xiuqo_home_key(), attr), default)
...
def profile_slot_set(module: Any, attr: str, value: Any) -> None:
    if get_hermes_home_override() is None:
        ...
        _SLOTS_BY_HOME[(xiuqo_home_key(), attr)] = value
