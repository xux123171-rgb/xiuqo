"""Post-update maintenance for ``xiuqo update``: pre-update backup snapshot, state-db verify/restore, curator/FTS notices, FHS path guard, completion summary, stale-module purge.

Split out of ``update_cmd.py``, which re-imports every name so ``xiuqo_cli.update_cmd.<name>``
...
_UPDATE_RUNTIME_RELOAD_MODULES = "xiuqo_constants", "tools.environments.local", "tools.lazy_deps"
...
_STALE_PURGE_PREFIXES = "xiuqo_cli", "gateway", "tools", "tui_gateway", "agent"
...
_STALE_PURGE_PROTECTED = frozenset({"xiuqo_cli", "xiuqo_cli.main", "xiuqo_cli.xiuqo_logging"})
...
_STALE_PURGE_PROTECTED_PREFIX = "xiuqo_cli.update_"
...
def _purge_stale_hermes_modules() -> None:
    """Evict every cached Xiuqo module after the checkout changed in-place. Never raises.
    ...
    with _best_effort('Could not purge stale Xiuqo modules: %s'):
        importlib.invalidate_caches()
        ...
        if purged:
            logger.debug("Purged %d stale Xiuqo module(s) after checkout update", len(purged))
...
def _print_curator_first_run_notice() -> None:
    ...
    print("  Preview now:  xiuqo curator run --dry-run")
    print("  Pause it:     xiuqo curator pause")
...
def _print_fts_optimize_available_notice() -> None:
    ...
    if interrupted:
        ...
        print(
            "  A previous `xiuqo sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    xiuqo sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  xiuqo sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    if "\n" in summary:
        ...
        for line in summary.splitlines():
            print(f"  {line}")
        print("  (This message shows once per curator run. View anytime: xiuqo curator status)")
...
def _reload_process_scan_modules() -> None:
    ...
    ``_scan_dashboard_processes`` does a function-level ``from xiuqo_cli._subprocess_compat import
    ...
    """
    _reload_modules(
        ("xiuqo_cli._subprocess_compat", "xiuqo_cli.dashboard_procs"),
        modules=sys.modules,
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    See #83595.
    """
    from xiuqo_cli.update_cmd import _m, _reload_process_scan_modules
    if node_failures:
        print()
    ...
    print("    xiuqo dashboard --port <port>")
...
def _print_update_completion(message: str) -> None:
    ...
    See #47359, #58764.
    """
    from xiuqo_cli.update_cmd import _branch_head_suffix
    print(f"{message}{_branch_head_suffix()}")
    action_id = os.environ.get("XIUQO_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== xiuqo-update completed {action_id} ===")
...
def _post_update_sqlite_runtime_status():
    ...
    from xiuqo_cli.update_cmd import _m
    from xiuqo_constants import project_venv_dir
    from xiuqo_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next Xiuqo runtime."""
    from xiuqo_cli.update_cmd import _post_update_sqlite_runtime_status
    ...
    print("  Rebuild the Xiuqo venv with a uv-managed Python, restart Xiuqo, then verify with `xiuqo doctor`.")
    return False
...
def _print_update_summary(*, node_failures: list, desktop_build_ok: bool, pre_update_version: str | None) -> bool:
    ...
    See #88251.
    """
    from xiuqo_cli.update_cmd import _post_update_sqlite_runtime_status, _update_complete_message
    sqlite_runtime_ok, sqlite_info = _post_update_sqlite_runtime_status()
    if sqlite_info is None:
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not desktop_build_ok:
            print("  Run `xiuqo desktop` to retry the desktop rebuild.")
        if not sqlite_runtime_ok:
            print(
                "  The Python runtime remediation did not complete. Run `xiuqo "
                "update` again; if SQLite is unchanged, rebuild the Xiuqo venv "
                "with a uv-managed Python, restart Xiuqo, then verify with "
                "`xiuqo doctor`."
            )
    else:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from xiuqo_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    from xiuqo_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (xiuqo gateway stop), "
    ...
    try:
        ...
        print(
            ...
            "handles (or restart Xiuqo) and retry."
        )
        return False
...
def _verify_and_restore_one_state_db(home: Path, *, label: str) -> None:
    ...
    try:
        from xiuqo_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    with _best_effort('Sibling-profile state.db guard sweep failed: %s'):
        from xiuqo_cli.backup import _sibling_profile_homes
...
def _print_bundled_skills_sync_report() -> None:
    ...
    if result.get("user_modified"):
        ...
        print("    → see them: xiuqo skills list-modified  (diff/reset to resume updates)")
...
def _ensure_fhs_path_guard() -> None:
    ...
    no-op on non-Linux/non-root/non-FHS or when ``bash -i -c 'command -v xiuqo'`` resolves."""
    from xiuqo_cli.update_cmd import _m
    ...
    try:
        ...
        return
    # Only for FHS-layout installs (link at /usr/local/bin/xiuqo).
    fhs_link = Path("/usr/local/bin/xiuqo")
    ...
    try:
        probe = subprocess.run(
            ...
            # recovery path in `xiuqo gateway restart` (`systemd_restart()`) as of PR #20949.
            capture_output=True,
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal a ``xiuqo-acp`` launcher next to ``xiuqo`` (mirrors install.sh): ACP hosts
    resolve it on the login-shell PATH but the console script lives in the venv. The shim
    delegates to the sibling ``xiuqo acp``, correct for every layout.
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        xiuqo_cmd = bin_dir / "xiuqo"
        acp_cmd = bin_dir / "xiuqo-acp"
        try:
            if not (xiuqo_cmd.is_file() or xiuqo_cmd.is_symlink()):
                continue
...
def _run_quick_snapshots() -> Optional[str]:
    ...
    from xiuqo_cli.update_cmd import _record_update_step
    ...
    with _best_effort('Sibling profile snapshots failed: %s'):
        from xiuqo_cli.backup import create_pre_update_snapshots_all_profiles
        ...
        if _sibling_snaps:
            ...
            import xiuqo_cli.update_cmd_config as _cfg
...
def _run_full_backup() -> None:
    """Zip XIUQO_HOME under ``backups/`` (restorable via ``xiuqo import``). Never raises."""
    try:
    ...
    from xiuqo_cli.sizefmt import format_bytes
    # display_hermes_home so the user sees ~/.xiuqo/...
    try:
        from xiuqo_constants import get_hermes_home, display_hermes_home
    ...
    print(f"  Restore:  xiuqo import {out_path}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    ``full`` — quick snapshot PLUS a zip of XIUQO_HOME under ``backups/`` (``xiuqo import``).
...
def _sweep_bytecode_after_update(branch: str) -> None:
    ...
    # symbols in xiuqo_constants or lazy_deps. The dependency install above may also have regenerated
...
def _sync_profiles_after_update() -> None:
    ...
    # XIUQO_HOME, so sync_skills()'s module-level XIUQO_HOME cache can't skew it.
    with suppress(Exception):
        from xiuqo_cli.profiles import list_profiles, seed_profile_skills
    ...
    with suppress(Exception):
        # See #44792.
        from xiuqo_cli.profiles import backfill_profile_envs
...
def _refresh_cua_driver_after_update() -> None:
    ...
    if (
        ...
        from xiuqo_cli.tools_config import install_cua_driver
        print()
...
def _print_plugin_compat_notice() -> None:
    ...
    from xiuqo_cli.plugin_compat import compat_report, removal_in_effect, summary_lines
...
def _print_post_update_notices_and_self_heals() -> None:
    ...
    from xiuqo_cli.update_cmd import _m, _print_curator_first_run_notice, _print_curator_recent_run_notice
    ...
    def _migrate_windows_bin_path() -> None:
        ...
        from xiuqo_cli._install_repair import migrate_windows_bin_path
    ...
    for message, step in (
        ...
        ('xiuqo-acp launcher self-heal failed: %s', _ensure_acp_launcher),
...
def _run_post_update_maintenance(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, had_desktop_app_before_update, node_failures, desktop_build_ok,
    ...
    from xiuqo_cli.update_cmd import _check_and_apply_config_migration, _m
    ...
    if sys.platform == "darwin" and had_desktop_app_before_update:
        print()
        print(
            "  ℹ macOS: if Xiuqo re-prompts for permissions you already "
            "granted (toggle shows ON), the stored grant is stale — run "
            "`tccutil reset ScreenCapture com.nousresearch.xiuqo` (repeat "
    ...
    try:
        # See #95596.
        from xiuqo_cli.macos_tcc_anchor import ensure_tcc_anchor
        ensure_tcc_anchor()
    except Exception:
    ...
    with _best_effort('Model catalog seed during update failed: %s'):
        from xiuqo_cli.model_catalog import seed_cache_from_checkout
