...
from xiuqo_cli.commands import (
    COMMAND_REGISTRY, _is_gateway_available, _iter_plugin_command_entries, _resolve_config_gates)

# Logger name parity with the origin module (tests capture "xiuqo_cli.commands").
logger = logging.getLogger("xiuqo_cli.commands")
...
def discord_skill_commands_by_category(
    ...
    parity, external-dir skills are visible via ``xiuqo skills list`` and the agent's ``/skill-name``
...
# Canonical commands deliberately routed through ``/xiuqo <command>`` on Slack only: the registry
...
_SLACK_VIA_XIUQO_ONLY = frozenset({
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    _SLACK_VIA_XIUQO_ONLY are skipped. ``/xiuqo`` is always first for anything dropped."""
...
def slack_app_manifest(
    request_url: str = "https://xiuqo.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """name/alias -> "/command" for the Slack ``/xiuqo`` handler, plugin commands included."""
