"""Backup and import commands for xiuqo CLI."""
...
from xiuqo_state_dbfile import RETIRED_GENERATION_DIR_SUFFIX
from utils import (
...
# Where ``xiuqo backup --quick`` / ``/snapshot`` / the pre-update safety net write state
# snapshots (see ``create_quick_snapshot``); defined here because the exclusion set needs it.
_QUICK_SNAPSHOTS_DIR = "state-snapshots"

# Directory names to skip (matched against each path component). ``xiuqo`` only matches at
# the root (``_should_exclude``) so skill dirs like ``skills/.../xiuqo/`` survive. The
# dependency/cache entries matter: one plugin venv or pip/uv cache under XIUQO_HOME walked
...
_EXCLUDED_DIRS = {
    "xiuqo",     # the codebase repo — re-clone instead
...
# Xiuqo-managed runtime downloads (GGUF models, llama.cpp runtimes, managed Node): re-downloaded
# on demand and routinely tens to hundreds of GB. Matched ONLY at the root of XIUQO_HOME and at
...
# Files ``xiuqo import`` must never overwrite, matched by basename so root and named profiles are
...
# Reserved archive subtree for memory-provider state OUTSIDE XIUQO_HOME (e.g. ~/.honcho, via
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Xiuqo backup slot."""
...
@contextmanager
def _backup_operation_lock(xiuqo_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = xiuqo_home / ".backup.lock"
        ...
        while not acquired:
            try:
                ...
                if time.monotonic() >= deadline:
                    raise BackupInProgressError("another Xiuqo backup is already running")
                time.sleep(0.05)
        yield
    finally:
        if acquired:
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to xiuqo root) should be skipped."""
    parts = rel_path.parts
    if _in_excluded_root_dir(rel_path):
        return True
    # ``xiuqo`` only matches at the root level; nested same-named dirs are preserved.
    if any(p in _EXCLUDED_DIRS and (p != "xiuqo" or p == parts[0]) for p in parts):
        return True
    name = rel_path.name
    return name in _EXCLUDED_NAMES or name.startswith(_EXCLUDED_PREFIXES) or name.endswith(_EXCLUDED_SUFFIXES)


def _iter_backup_files(xiuqo_root: Path, out_path: Path, skipped_dirs: Optional[set] = None):
    ...
    excluded tree, the root-only ``xiuqo`` carve-out, root runtime trees, per-file rules),
    shared by ``xiuqo backup`` and the pre-update / pre-migration path so they can never drift.
    """
    for dirpath, dirnames, filenames in os.walk(xiuqo_root, followlinks=False):
        rel_dir = Path(dirpath).relative_to(xiuqo_root)
        is_root = rel_dir == Path(".")
        kept = [
            d for d in dirnames
            if (d not in _EXCLUDED_DIRS or (d == "xiuqo" and not is_root))
        ...
        for fname in filenames:
            rel = rel_dir / fname
            fpath = xiuqo_root / rel
            # zipfile.write() follows file symlinks, so skip links before any archive write can
            # copy data from outside XIUQO_HOME; never archive the output zip into itself.
...
# 30 GB state.db, reading as a hung ``xiuqo update``) is replaced by the O(1) header+schema probe.
...
def _unlink_move_restore_db(src: Path, dst: Path) -> bool:
    ...
    try:
        ...
        logger.error("Refusing unlink+move restore of %s: %s Close the in-process "
                     "database handles (or restart Xiuqo) and retry.", dst, exc2)
        return False
...
_RUN_BACKUP_PREFIX = "xiuqo-backup-"
...
def run_backup(args) -> None:
    """Create a zip backup of the Xiuqo home directory."""
    xiuqo_root = get_default_hermes_root()

    if not xiuqo_root.is_dir():
        print(f"Error: Xiuqo home directory not found at {xiuqo_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(xiuqo_root):
            _run_backup_locked(args, xiuqo_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, xiuqo_root: Path) -> None:
    ...
    files_to_add: list[tuple[Path, Path]] = list(_iter_backup_files(xiuqo_root, out_path, skipped_dirs))
    ...
    if errors:
        ...
        print(f"\nRestore with: xiuqo import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Xiuqo backup."""
    names = zf.namelist()
    if not names:
        return False, "zip archive is empty"
    # Telltale files a xiuqo home has — at the root or one level deep (zipped directory).
    if not any(Path(n).name in {"config.yaml", ".env", "state.db"} for n in names):
        return False, "zip does not appear to be a Xiuqo backup (no config.yaml, .env, or state databases found)"
    return True, ""
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1 and first_parts <= {".xiuqo", "xiuqo"}:
...
def _count_session_rows(path: Path) -> Optional[Tuple[int, int]]:
    ...
    ``None`` means "unknown" (missing, not a Xiuqo session store, unreadable) — never "zero":
...
def _confirm_import_overwrite(xiuqo_root: Path) -> bool:
    """Prompt before importing over an existing installation; True when import may proceed."""
    if not any((xiuqo_root / m).exists() for m in ("config.yaml", ".env")):
        return True
    print("\nWarning: Target directory already has Xiuqo configuration.\n"
...
def _import_members(
    zf: zipfile.ZipFile, members: List[str], prefix: str, xiuqo_root: Path, file_count: int
    ...
    for member in members:
        ...
        # NOT under XIUQO_HOME; provider configs commonly hold credentials, so tighten to 0600.
        ...
        if external:
            ...
            if rel.endswith(_SQLITE_SIDECAR_SUFFIXES):
                skipped_runtime.append(rel)
                continue
            target = xiuqo_root / rel
            root = xiuqo_root.resolve()
...
def run_import(args) -> None:
    """Restore a Xiuqo backup from a zip file."""
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if not args.force and not _confirm_import_overwrite(xiuqo_root):
            return
        ...
        restored, restored_external, errors, skipped_runtime, db_shrunk = _import_members(
            zf, members, prefix, xiuqo_root, file_count)
        elapsed = time.monotonic() - t0
        ...
        if db_shrunk:
            ...
            print("    Anything recorded after the backup was taken is not in it. "
                  "Recover from a newer backup or snapshot: xiuqo snapshot list")
        if skipped_runtime:
            _print_capped(f"\n  Preserved {len(skipped_runtime)} runtime state "
                          f"file(s) (kept this machine's, not the backup's):",
                          sorted(skipped_runtime), "    ")
        restored_profiles = _restore_profile_wrappers(xiuqo_root)
        print()
        if not (xiuqo_root / "xiuqo").is_dir():
            print("Note: The xiuqo codebase was not included in the backup.\n"
                  "  If this is a fresh install, run: xiuqo update")
        if restored_profiles:
            print("\nTo re-enable gateway services for profiles:")
            for pname in restored_profiles:
                print(f"  xiuqo -p {pname} gateway install")
        _revive_gateway_after_import(xiuqo_root)
        print("Done. Your Xiuqo configuration has been restored.")


def _restore_profile_wrappers(xiuqo_root: Path) -> List[str]:
    """Re-create shell wrapper scripts for restored named profiles; return the profile names seen."""
    profiles_dir = xiuqo_root / "profiles"
    ...
    except ImportError:  # xiuqo_cli.profiles unavailable (fresh install)
        if any(profiles_dir.iterdir()):
            print("\n  Profiles detected but aliases could not be created.\n"
                  "  Run: xiuqo profile list  (after installing xiuqo)")
    return [n for n, _ in restored_profiles]


def _revive_gateway_after_import(xiuqo_root: Path) -> None:
    ...
    if xiuqo_root != native_default and any(
        ...
        print("\nRestored into a non-default home; leaving the gateway service alone to avoid clashing "
              ...
              "To start a gateway for this home, run:  xiuqo gateway install")
        return
    try:
        from xiuqo_cli.gateway import ensure_gateway_service, _is_service_running
...
# --- Quick state snapshots (used by /snapshot slash command and xiuqo backup --quick) ---

# Critical state files (relative to XIUQO_HOME) for quick snapshots; everything else is
...
# outside state.db, so it is listed explicitly — ``xiuqo update`` snapshots this set (#15733).
_QUICK_STATE_FILES = (
    ...
    # schema-init re-creates an empty one (#52889). Skipped when outside XIUQO_HOME.
...
def _quick_snapshot_root(xiuqo_home: Optional[Path] = None) -> Path:
    home = xiuqo_home or get_hermes_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None, xiuqo_home: Optional[Path] = None, keep: Optional[int] = None,
...
def _create_quick_snapshot_locked(
    ...
    multi-GB ``state.db`` never stalls ``xiuqo update`` while the small files are always captured.
    """
    ...
    if failed_dbs:
        ...
        print(f"  ⚠ CRITICAL: could not snapshot DB file(s): {', '.join(failed_dbs)}\n"
              f"  ⚠ If sessions disappear after update, check {root} and run: xiuqo snapshot list")
...
def list_quick_snapshots(limit: int = 20, xiuqo_home: Optional[Path] = None) -> List[Dict[str, Any]]:
    """List existing quick state snapshots, most recent first."""
    results = []
    for d in _snapshot_dirs(_quick_snapshot_root(xiuqo_home)):
...
def restore_quick_snapshot(snapshot_id: str, xiuqo_home: Optional[Path] = None) -> bool:
...
def restore_cron_jobs_if_emptied(snapshot_id: str, xiuqo_home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    """Safety net for silent cron-job loss across ``xiuqo update``.
    ...
    for name, profile_home in _sibling_profile_homes(home):
        try:
            snap_id = create_quick_snapshot(
                label="pre-update", xiuqo_home=profile_home, keep=keep, max_file_size=max_file_size)
            if snap_id:
...
def restore_config_model_settings_if_rewritten(
    snapshot_id: str, xiuqo_home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    """Safety net for silent config.yaml model/MoA loss across ``xiuqo update``.
...
def _restore_all_sibling_profiles(
    ...
    """Run ``restore_fn(snap_id, xiuqo_home=...)`` for every sibling against ITS OWN
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_fn(snap_id, xiuqo_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(keep: int = _QUICK_DEFAULT_KEEP, xiuqo_home: Optional[Path] = None) -> int:
    """Remove oldest quick snapshots beyond the keep limit. Returns count deleted."""
    return _prune_oldest(_snapshot_dirs(_quick_snapshot_root(xiuqo_home)), keep, shutil.rmtree, "snapshot")


def run_quick_backup(args) -> None:
    """CLI entry point for xiuqo backup --quick."""
...
def _write_full_zip_backup(out_path: Path, xiuqo_root: Path) -> Optional[Path]:
    """Full zip snapshot of ``xiuqo_root`` to ``out_path`` under the backup slot (same rules as
    :func:`run_backup`); None when nothing to back up, another backup running, or write error."""
    try:
        with _backup_operation_lock(xiuqo_root):
            return _write_full_zip_backup_locked(out_path, xiuqo_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, xiuqo_root: Path) -> Optional[Path]:
    scan_started = time.monotonic()
    logger.info("automatic backup phase=scan status=started")
    try:
        files_to_add = list(_iter_backup_files(xiuqo_root, out_path))
    except OSError as exc:
...
def _create_prefixed_full_backup(
    xiuqo_home: Optional[Path], prefix: str, keep: int, what: str, prune_what: str) -> Optional[Path]:
    """Write ``<XIUQO_HOME>/backups/<prefix><timestamp>.zip`` and prune older same-prefix zips.
    Returns the path, or ``None`` if nothing to back up or the write failed. Never raises."""
    xiuqo_root = xiuqo_home or get_default_hermes_root()
    if not xiuqo_root.is_dir():
        return None
    backup_dir = xiuqo_root / _PRE_UPDATE_BACKUPS_DIR
    try:
    ...
    if _write_full_zip_backup(out_path, xiuqo_root) is None:
        return None
...
def create_pre_update_backup(
    xiuqo_home: Optional[Path] = None, keep: int = _PRE_UPDATE_DEFAULT_KEEP) -> Optional[Path]:
    """Full zip backup to ``backups/pre-update-<timestamp>.zip``, auto-pruned; ``None`` if nothing
    was found or the backup failed. Never raises — ``xiuqo update`` continues anyway."""
    return _create_prefixed_full_backup(xiuqo_home, _PRE_UPDATE_PREFIX, max(keep, 1), "pre-update", "backup")


def create_pre_migration_backup(
    xiuqo_home: Optional[Path] = None, keep: int = _PRE_MIGRATION_DEFAULT_KEEP) -> Optional[Path]:
    """Full zip backup to ``backups/pre-migration-<timestamp>.zip`` before ``xiuqo claw migrate``
    (same dir as update backups so listings/``xiuqo import`` find it); ``None`` if nothing was
    found or the write failed. Never raises."""
    return _create_prefixed_full_backup(
        xiuqo_home, _PRE_MIGRATION_PREFIX, max(keep, 0), "pre-migration", "pre-migration backup")
