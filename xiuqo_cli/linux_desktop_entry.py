"""Install and remove the Linux desktop entry (``xiuqo.desktop``).
...
DESKTOP_ENTRY_NAME = "xiuqo.desktop"
...
def _can_import_xiuqo_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``xiuqo_cli.main`` unaided.
...
def _running_interpreter_fallback() -> str:
    """The RUNNING interpreter — it has ``xiuqo_cli`` importable by definition."""
    return os.path.abspath(sys.executable)


def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``xiuqo desktop``.

    Prefer the real ``xiuqo`` launcher; fall back to ``<python> -m xiuqo_cli.main desktop``.
    """
    from xiuqo_cli.relaunch import resolve_hermes_bin
    ...
    if not _can_import_xiuqo_cli(Path(interpreter)):
        ...
        # The candidate interpreter cannot actually import xiuqo_cli.main (checked in isolated mode from a
    ...
    argv = [interpreter, "-m", "xiuqo_cli.main", "desktop"]
    if bin_path:
        resolved = Path(bin_path).resolve()
        # A Python launcher whose shebang points OUTSIDE the venv (e.g. the repo's `xiuqo` script
...
def _inside_checkout(candidate: str, checkout_root: Path, original_argv0: str) -> bool:
    ...
    try:
        ...
        return False
    # Anything shipped in the tree (e.g. the repo `xiuqo` script) is checkout-internal. Compare
    ...
    for root in {checkout_root, resolved_root}:
        if root is not None and (path == root or root in path.parents):
            return True
    # The `python -m xiuqo_cli.main` relaunch context surfaces the invoking interpreter as
...
def _resolve_hermes_bin_for_desktop_entry(
    ...
    Wraps :func:`xiuqo_cli.relaunch.resolve_hermes_bin` with one rule: an ``argv[0]`` inside this
    ...
    if resolve_fn is None:
        from xiuqo_cli.relaunch import resolve_hermes_bin as resolve_fn
    ...
    # An external primary (another install's /opt/.../bin/xiuqo, a venv console script) wins
    ...
    # /opt/.../bin/xiuqo from another install method, or a venv console script). It must be evaluated
    ...
    # argv[0] was checkout-internal AND PATH had no `xiuqo` — common in stripped systemd user
...
def _known_wrapper_candidates():
    ...
    (``/usr/local/bin``), and user (``~/.local/bin``). The wrapper is always named ``xiuqo``.
    """
    candidates = []
    prefix = os.environ.get("PREFIX")
    if prefix:
        candidates.append(Path(prefix) / "bin" / "xiuqo")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        candidates.append(Path("/usr/local/bin/xiuqo"))
    candidates.append(Path.home() / ".local" / "bin" / "xiuqo")
    return candidates
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Xiuqo\n"
        "GenericName=Xiuqo Desktop\n"
        "Comment=Launch Xiuqo Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Xiuqo\n"
    )
...
def _hicolor_icon_dest(subdir: str) -> Path:
    return _xdg_data_home() / "icons" / "hicolor" / subdir / "apps" / "xiuqo.png"
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    """Install the app icon into the user's hicolor tree so ``Icon=xiuqo`` resolves without an
...
def _launcher_entry_management_enabled() -> bool:
    ...
    rewrite: a hand-edited ``xiuqo.desktop`` is then left alone instead
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    ...
    icon_value = str(icon) if icon.is_file() else "xiuqo"
