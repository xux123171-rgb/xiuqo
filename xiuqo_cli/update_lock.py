"""Cross-process mutual exclusion for in-flight Xiuqo updates.
...
MARKER_NAME = ".xiuqo-update-in-progress"

# Set by an orchestrating updater (Tauri `xiuqo-setup --update`) to its own pid before
# spawning `xiuqo update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "XIUQO_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Xiuqo is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Xiuqo home (never the context-local profile override): the Rust
    updater resolves ``$XIUQO_HOME`` or the platform default and the desktop pins that same
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``xiuqo update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Xiuqo update is already running (PID {holder.pid}, "
