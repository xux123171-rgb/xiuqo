"""Dashboard process-hygiene helpers — extracted from ``xiuqo_cli/main.py``.

Helpers defined in ``xiuqo_cli.main_dashboard`` / ``xiuqo_cli.main_install_repair`` are imported at
...
_DASHBOARD_PATTERNS = tuple(
    ...
    for launcher in ("xiuqo", "xiuqo_cli.main", "xiuqo_cli/main.py"))
...
def _iter_process_table() -> list[tuple[int, str]]:
    ...
    if sys.platform == "win32":
        ...
        return rows
    # ps, not `pgrep -f "xiuqo.*dashboard"` (greedy regex; consistent with gateway pid scan).
...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str]]:
    ...
    ``xiuqo update`` (every API call 401s). *exclude_pids* (Desktop's XIUQO_DESKTOP_CHILD_PID
    backends) are never returned.

    *exclude_pids* is an optional set of PIDs that must never be returned. This is used by the Xiuqo
    Desktop Electron app to protect its own backend child process: when the desktop spawns ``xiuqo serve``
    as a backend and triggers an auto-update, the update must not kill the backend that the desktop itself
    manages. The desktop sets the environment variable ``XIUQO_DESKTOP_CHILD_PID`` on the spawned backend
    ...
    try:
        ...
        return []
    # Spawn-ledger augmentation: substring patterns miss profiled launches (`xiuqo --profile p
    ...
    with contextlib.suppress(Exception):
        ...
        from xiuqo_cli.process_identity import ledger_entries
...
def _hermes_home_for_pid(pid: int) -> str | None:
    """Best-effort ``XIUQO_HOME`` from *pid*'s environment (psutil, then /proc)."""
    with contextlib.suppress(Exception):
        import psutil
        if home := psutil.Process(pid).environ().get("XIUQO_HOME"):
            return home
    try:
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"XIUQO_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    ``xiuqo update`` multiplies listening backends because ``--port 0`` binds a fresh port.
...
def _profile_key_for_respawn(argv: list[str], xiuqo_home: str | None = None) -> str:
    """Stable owner key: ``XIUQO_HOME`` when known, else ``--profile`` / ``-p``.
    ...
    """
    if xiuqo_home:
        parts = _resolved_home(xiuqo_home).parts
        if len(parts) >= 2 and parts[-2] == "profiles" and parts[-1]:
            return f"profile:{parts[-1]}"
        return f"home:{_normalized_home_for_compare(xiuqo_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends ``(pid, argv, xiuqo_home)`` to respawn after update.

    Rules: never resurrect Desktop ``--port 0`` backends; never replay a backend from a
    **foreign** ``XIUQO_HOME`` (the argv-only respawn would come back on this install's home
    ...
    (``XIUQO_DESKTOP_CHILD_PID``) owns their lifecycle. These are also the PPID-1 orphans that previously
    ...
    Intentionally does **not** blanket-skip every PPID-1 process: a prior ``xiuqo update`` respawn detaches
    ...
    for _pid, argv, xiuqo_home in candidates:
        if not argv or _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and xiuqo_home and _normalized_home_for_compare(xiuqo_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        profile_key = _profile_key_for_respawn(argv, xiuqo_home)
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (``XIUQO_DESKTOP_CHILD_PID``, comma-separated)."""
    out: set[int] = set()
    for part in os.environ.get("XIUQO_DESKTOP_CHILD_PID", "").split(","):
    ...
    for pid in pids:
        try:
            ...
            if expected_start_time is None:
                ...
                failed.append((pid, "not xiuqo-owned or process identity changed"))
            else:
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``xiuqo dashboard`` / ``xiuqo serve`` processes (update end, ``--stop``).
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    e.g. ``xiuqo update``'s systemd fleet-restart loop, which restarts ``xiuqo-serve*`` units before this
    ...
    """
    from xiuqo_cli import main_dashboard as _dash

    if restart_managed and _dash._restart_managed_dashboard_service(reason):
        # The dashboard unit is handled but other backends (e.g. xiuqo-serve.service) are not:
        # mark the unit handled so the filter below drops its PIDs, and keep going.
        _dash_unit = getattr(_dash, "_DASHBOARD_SYSTEMD_UNIT", "xiuqo-dashboard.service")
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid] and (cmdline := _dash._dashboard_cmdline_for_pid(pid)):
                # Manual process: exact argv + XIUQO_HOME for the respawn and its profile cap.
                # Manually-started process: preserve its exact argv so we can respawn it after the update
                # (#40449, #68934). Snapshot XIUQO_HOME before the kill so per-profile caps still work
...
def _restart_killed_backends(
    ...
    # Two categories: Without this, a remote backend (xiuqo serve) under Restart=on-failure never comes
...
def _detect_concurrent_hermes_instances(
    ...
    Windows blocks DELETE/REPLACE on a running .exe, so a Desktop-spawned ``xiuqo.EXE`` makes
    ...
    *shim* ancestor (the setuptools launcher is a separate native process from its
    ...
    """
    from xiuqo_cli.main_install_repair import _hermes_exe_shims, _is_windows
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    """True for the Desktop-local shape ``xiuqo serve [--isolated] --host 127.0.0.1 --port 0``.
    ...
    """
    from xiuqo_cli.update_cmd_windows import _hermes_holder_subcommand
    # Canonical token matcher, never argv substrings: ``kanban --preserve-cache`` contains "serve" and
    # ``vim notes about xiuqo serve`` contains both markers — this predicate decides a kill.
...
# (apps/desktop/electron/remote-lifecycle.ts) for every ``xiuqo serve`` it spawns. Such a backend
...
def _hermes_home_dir() -> Path:
    """The process's Xiuqo home: remote-backend locks are a process-level asset, so a request scoped
    to another profile must still see the same lock dir."""
    from xiuqo_constants import get_process_hermes_home
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    if any(not isinstance(parsed.get(f), str) or len(parsed[f]) > 1024
        ...
        return False
    # Suffix-only check of logPath so a relocated XIUQO_HOME can't reject a legitimate backend.
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    """PIDs claimed by valid ``{xiuqo_home}/desktop-ssh/<ownershipId>/backend.lock.json`` records
...
def _reap_orphaned_desktop_local_serves(
    *, reason: str = "orphaned desktop-local xiuqo serve", signal_term=None, signal_kill=None,
    sleep_fn=None, lock_owned_pids_fn=None, process_age_seconds_fn=None) -> dict[str, list]:
    """Kill leftover Desktop-local ``xiuqo serve`` backends with no parent. Never raises.
    ...
    0/1; not self / parent / XIUQO_DESKTOP_CHILD_PID; not claimed by a valid
    ...
    after XIUQO_BACKEND_READY, so a live sibling mid-startup is briefly unowned).
    """
    import signal as _signal
    import time as _time
