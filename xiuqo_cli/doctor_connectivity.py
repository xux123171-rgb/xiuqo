"""API connectivity probes for ``xiuqo doctor`` (split out of ``doctor.py``).
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from xiuqo_cli.providers import normalize_provider as _normalize_provider
...
_OPENROUTER_STATUS = {
    ...
    402: ("(out of credits — payment required)",
          ...
          "Fix: run 'xiuqo config set model.provider <provider>' "
...
def _apikey_request(key: str, base_env, default_url) -> tuple:
    ...
    headers = {"Authorization": f"Bearer {key}", "User-Agent": _XIUQO_USER_AGENT}
