...
Entries are added only by merging a PR into xiuqo; presence in ``optional-mcps/`` = Nous
...
from xiuqo_constants import get_hermes_home, get_optional_mcps_dir
from xiuqo_cli._subprocess_compat import noninteractive_git_env
from xiuqo_cli.colors import Colors, color
from xiuqo_cli.config import load_config, save_config, get_env_value, save_env_value
from xiuqo_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # go through auth.env so they are prompted for and land in ~/.xiuqo/.env.
...
def _parse_auth(path: Path, raw: Any, name: str, http: bool) -> AuthSpec:
    ...
    if http and a_type == "api_key":
        ...
        from xiuqo_cli.mcp_config import _env_key_for_server
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Xiuqo understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    skipped too but surfaced via :func:`catalog_diagnostics` so UIs can say "update Xiuqo".
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    ``kind`` is ``future_manifest`` (newer than this Xiuqo) or ``invalid`` (malformed)."""
...
def remove_server(name: str) -> bool:
    ...
    from xiuqo_cli.mcp_config import _remove_mcp_server
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.xiuqo/mcp-installs/<name>`` and run bootstrap. Returns the dir."""
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    """Prompt for each env spec; secrets and non-secrets alike go to ~/.xiuqo/.env."""
...
def _build_server_config(entry: CatalogEntry, install_dir: Optional[Path]) -> dict:
    """Translate a manifest into the ``mcp_servers.<name>`` block format used by xiuqo_cli/mcp_config.py."""
    cfg: dict = {}
    t = entry.transport
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from xiuqo_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    if not server_cfg:
        return None
    try:
        from xiuqo_cli.mcp_config import _probe_single_server  # lazy: keep this module cheap
...
def _apply_tool_selection(
    ...
    *,
    ...
    else apply ``default_enabled``, else no filter; point the user at ``xiuqo mcp configure``.
    """
    print()
    name = entry.name
    configure_hint = f"`xiuqo mcp configure {name}`"
    ...
    if len(chosen_indices) == len(probed):
        ...
        # re-run `xiuqo mcp configure <name>` and unselect a tool (switches to include-mode).
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        # Provider-mediated OAuth relies on the existing `xiuqo auth <provider>` flow; surface
        ...
        _say(
            ...
            f"`xiuqo auth {entry.auth.provider}` if you have not "
    ...
    from xiuqo_cli.mcp_config import _save_mcp_server
    ...
    _say(
        ...
        f"Start a new Xiuqo session to load its tools."
    )
