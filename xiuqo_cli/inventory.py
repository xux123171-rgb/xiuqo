...
def _strip_aggregator_overlaps(rows: list[dict]) -> None:
    ...
    try:
        from xiuqo_cli.providers import is_routing_aggregator
    except Exception:
        return
...
def _apply_capabilities(rows: list[dict]) -> None:
    ...
    from xiuqo_cli.models import model_supports_fast_mode
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`xiuqo_cli.providers.custom_provider_aliases` — lets the frontend do a membership check instead.
    """
...
def _append_unconfigured_rows(
    ...
    from xiuqo_cli.models import CANONICAL_PROVIDERS, _model_requires_account_discovery
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else f"Configured provider is not authenticated; run `xiuqo model` to reactivate. {tail}"
            )
...
def _anthropic_oauth_credentials_present() -> bool:
    """True when the user explicitly authenticated Anthropic via OAuth (Xiuqo device flow or Claude Code
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's Xiuqo overlay declares it keyless."""
    try:
        from xiuqo_cli.providers import XIUQO_OVERLAYS
        overlay = XIUQO_OVERLAYS.get(slug)
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (f"paste {key_env} to activate" if auth_type == "api_key" and key_env
                          else f"run `xiuqo model` to configure ({auth_type})")
...
def _local_runtime_row(ctx: "ConfigContext") -> dict | None:
    ...
    try:
        from xiuqo_cli.local_runtime.bootstrap import staged_model_ids
        ...
        if not current:
            ...
            try:
                from xiuqo_cli.local_runtime.endpoint import _state_endpoint
...
def _prewarm_pricing_async(
    ...
    from xiuqo_cli.models_pricing import pricing_cache_scope
    ...
    endpoint_scope = tuple(sorted(
        ...
        for slug in slugs))
    prewarm_key = (xiuqo_home_key(), endpoint_scope)
    ...
    with _pricing_prewarm_lock:
        ...
        thread = Thread(target=copy_context().run, args=(_worker,),
                        name="xiuqo-picker-pricing-prewarm", daemon=True)
