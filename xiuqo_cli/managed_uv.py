"""Xiuqo-managed uv and Python runtime repair.

The Python backing the install is shared by every Xiuqo profile because the checkout's ``venv``
is shared. Runtime repair therefore uses an install-scoped store under
``<checkout>/.xiuqo-runtime/python``. A vulnerable interpreter is never reinstalled in place.
"""
...
from xiuqo_cli.sqlite_runtime import (
    SQLiteRuntimeInfo, isolated_interpreter_env, probe_sqlite_runtime)

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".xiuqo-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
_REPAIR_LOCK_NAME = "runtime-repair.lock"
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.nousresearch.xiuqo.managed-python"

_Provisioned = tuple[Path, Path, SQLiteRuntimeInfo]


def managed_uv_path() -> Path:
    """Path of Xiuqo' own uv binary (``$XIUQO_HOME/bin/uv[.exe]``); may not exist yet."""
...
def managed_python_env(
    ...
    """Return a sanitized environment for Xiuqo-private uv Python commands."""
    target = (
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print("    Sessions stay protected meanwhile: Xiuqo keeps databases "
              "out of WAL mode on this SQLite build. The next `xiuqo update` "
              "will retry.")
        return
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``xiuqo update`` daily; a blocking network
...
def _reload_xiuqo_constants():
    """Re-execute ``xiuqo_constants`` from disk (the imported one may predate venv_python_path)."""
    import xiuqo_constants
    return importlib.reload(xiuqo_constants)
...
def _attempt_install_generation(
    ...
    try:
        ...
        return reject("uv resolved Python outside the Xiuqo generation: %s", python)
...
def _install_safe_python_generation(
    ...
    # supported minor (e.g. 3.11 → 3.12) so the user isn't stuck on every `xiuqo update`. The
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("xiuqo_cli.main")
    ...
    if holders:
        ...
        return True, f"other Xiuqo processes still hold the venv (PID {pids})"
    return False, ""
...
def _windows_runtime_self_lock(live: Path) -> tuple[bool, str]:
    ...
    (``xiuqo update`` itself runs from the venv python), which is correct for the dependency-sync
    ...
    updater's own ``python.exe`` (or a waiting ``xiuqo.exe`` launcher ancestor) can never be renamed from
    ...
    # Belt-and-braces: the venv\Scripts\xiuqo.exe launcher stays mapped while it waits for this
...
def _repair_windows_preflight(
    ...
    if self_locked:
        ...
        for line in (
            ...
            # See #93032.
            "    Retrying `xiuqo update` from inside this venv cannot help: "
            ...
            f"      cd {root}",
            "      <system Python> -m xiuqo_cli.main update",
            "    Sessions stay protected meanwhile: Xiuqo keeps databases "
...
def _install_uv(target: Path) -> None:
    ...
    into ``$XIUQO_HOME/bin/`` instead of ``~/.local/bin/``.
    """
