...
tests/xiuqo_cli/test_commands_execute.py).
"""
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    label = profile_name
    try:
        from xiuqo_cli.profiles import format_profile_label, get_profile_dir, read_profile_meta
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: xiuqo bundles create <name> --skill <s1> --skill <s2>\n"
...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    from xiuqo_cli.commands import gateway_help_lines
