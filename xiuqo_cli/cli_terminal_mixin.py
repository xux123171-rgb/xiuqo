...
class CLITerminalMixin:
    ...
    def _recover_terminal_after_interrupt(self) -> None:
        ...
        if getattr(self, "_terminal_io_broken", False):
            return
        try:
            from xiuqo_cli.curses_ui import flush_stdin
            flush_stdin()
        except Exception:
            pass
        self._force_full_redraw()
    ...
    def _try_attach_clipboard_image(self) -> bool:
        """Save a clipboard image to ~/.xiuqo/images/ and attach it; True if attached."""
        from cli import datetime
        from xiuqo_cli.clipboard import save_clipboard_image
