...
``xiuqo_cli.models.<name>`` stays the stable import/monkeypatch surface:
...
from xiuqo_cli.urllib_security import open_credentialed_url
from xiuqo_cli.models_catalog_static import (
    CANONICAL_PROVIDERS,
    OPENROUTER_MODELS,
...
from xiuqo_cli.models_reasoning_caps import (
...
_XIUQO_USER_AGENT = f"xiuqo-cli/{_XIUQO_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from xiuqo_cli.config import get_custom_provider_tls_settings
...
def _openrouter_catalog_disk_ttl() -> float:
    ...
    from xiuqo_cli.model_catalog import refresh_interval_seconds
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        from xiuqo_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    try:
        from xiuqo_cli.model_catalog import get_default_model_from_cache
...
def recommended_nous_default_model() -> dict[str, Any]:
    ...
    sign-in completion in ``xiuqo_cli.anon_auth`` so both land on the same model.
    """
    from xiuqo_cli import models_pricing as mp
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    """True when ``supported_parameters`` advertises ``tools`` (xiuqo is tool-calling-first).
...
def fetch_openrouter_models(
    ...
    from xiuqo_cli.models_profile_cache import profile_slot_get, profile_slot_set
    ...
    try:
        from xiuqo_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id, _ in fallback:
        ...
        if live_item is None or not _openrouter_model_supports_tools(live_item):
            continue
        # Hide models that don't advertise tool-calling support — xiuqo requires it and surfacing
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    try:
        from xiuqo_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    from xiuqo_constants import AI_GATEWAY_BASE_URL
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from xiuqo_cli.providers import custom_provider_slug
...
def _provider_has_credentials(pid: str) -> bool:
    try:
        from xiuqo_cli.auth import get_auth_status, has_usable_secret
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    derived from :data:`CANONICAL_PROVIDERS` (shared with ``xiuqo model`` and ``/model``)."""
...
def _resolve_static_model_alias(
    ...
    try:
        from xiuqo_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_provider_for_model(
    ...
    from xiuqo_cli.models_detect import (
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``xiuqo_cli.auth.resolve_provider()`` to resolve it from credentials."""
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Xiuqo should expose the /fast toggle for this model."""
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from xiuqo_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    def _pool_token() -> str:
        from xiuqo_cli.auth import read_credential_pool
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from xiuqo_cli.model_search import model_alias_canonical
...
def _codex_catalog(normalized: str, force_refresh: bool) -> list[str]:
    from xiuqo_cli.codex_models import get_codex_model_ids
...
def _nous_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    try:
        from xiuqo_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
    ...
    # Portal models still surface without a Xiuqo release.
...
def _openai_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    # entries, so intersect with the curated agentic catalog so ``/model`` matches ``xiuqo model``.
    ...
    # pure-catalog helper (no extra live fetch) so we only accept models Xiuqo actually ships. (#46850)
    ...
    from xiuqo_cli.providers import is_official_openai_host
...
# every authed provider's /v1/models). One JSON file at $XIUQO_HOME/provider_models_cache.json;
...
def _spawn_swr_refresh(cache_key: str, refresh_fn=None) -> None:
    ...
    from xiuqo_constants import get_hermes_home_override, xiuqo_home_key
    inflight_key = cache_key if get_hermes_home_override() is None else (xiuqo_home_key(), cache_key)
...
def clear_provider_models_cache(provider: Optional[str] = None) -> None:
    ...
    ``/model --refresh`` and ``xiuqo model --refresh``."""
    try:
...
def opencode_zen_free_headers() -> dict:
    ...
    try:
        from xiuqo_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def probe_api_models(
    ...
    headers: dict[str, str] = {"User-Agent": _XIUQO_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"xiuqo/{_XIUQO_VERSION}"
    ...
    if isinstance(request_headers, dict):
        ...
        from xiuqo_cli.config import normalize_extra_headers
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=xiuqo"
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "User-Agent": _XIUQO_USER_AGENT}
    try:
...
def cached_fetch_api_models(
    ...
    from xiuqo_cli.model_switch_providers import _NativePickerModelList
...
_PLUGIN_COMPAT_LAZY = {
    'LMStudioLoadResult': ('xiuqo_cli.models_local', 'LMStudioLoadResult'),
    'PROVIDER_GROUPS': ('xiuqo_cli.models_catalog_static', 'PROVIDER_GROUPS'),
    'ProviderEntry': ('xiuqo_cli.models_catalog_static', 'ProviderEntry'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'compute_sale_discount': ('xiuqo_cli.models_pricing', 'compute_sale_discount'),
    'ensure_lmstudio_model_loaded': ('xiuqo_cli.models_local', 'ensure_lmstudio_model_loaded'),
    'fetch_ai_gateway_pricing': ('xiuqo_cli.models_pricing', 'fetch_ai_gateway_pricing'),
    'fetch_lmstudio_models': ('xiuqo_cli.models_local', 'fetch_lmstudio_models'),
    'fetch_models_with_pricing': ('xiuqo_cli.models_pricing', 'fetch_models_with_pricing'),
    'fetch_ollama_local_models': ('xiuqo_cli.models_local', 'fetch_ollama_local_models'),
    'get_cached_nous_inference_base_url': ('xiuqo_cli.models_pricing', 'get_cached_nous_inference_base_url'),
    'get_pricing_for_provider': ('xiuqo_cli.models_pricing', 'get_pricing_for_provider'),
    'group_providers': ('xiuqo_cli.models_catalog_static', 'group_providers'),
    'lmstudio_model_reasoning_options': ('xiuqo_cli.models_local', 'lmstudio_model_reasoning_options'),
    'nous_catalog_url': ('xiuqo_cli.models_reasoning_caps', 'nous_catalog_url'),
    'nous_model_reasoning_capabilities': ('xiuqo_cli.models_reasoning_caps', 'nous_model_reasoning_capabilities'),
    'nous_policy_allowed_ids': ('xiuqo_cli.models_pricing', 'nous_policy_allowed_ids'),
    'ollama_model_supports_thinking': ('xiuqo_cli.models_local', 'ollama_model_supports_thinking'),
    'openrouter_model_reasoning_capabilities': ('xiuqo_cli.models_reasoning_caps', 'openrouter_model_reasoning_capabilities'),
    'parse_openrouter_reasoning_capabilities': ('xiuqo_cli.models_reasoning_caps', 'parse_openrouter_reasoning_capabilities'),
    'peek_cached_pricing': ('xiuqo_cli.models_pricing', 'peek_cached_pricing'),
    'pricing_cache_scope': ('xiuqo_cli.models_pricing', 'pricing_cache_scope'),
    'probe_lmstudio_models': ('xiuqo_cli.models_local', 'probe_lmstudio_models'),
    'probe_ollama_local_models': ('xiuqo_cli.models_local', 'probe_ollama_local_models'),
    'provider_group_for_slug': ('xiuqo_cli.models_catalog_static', 'provider_group_for_slug'),
    'refresh_reasoning_caps_async': ('xiuqo_cli.models_reasoning_caps', 'refresh_reasoning_caps_async'),
    'restrict_to_nous_policy': ('xiuqo_cli.models_pricing', 'restrict_to_nous_policy'),
    'should_use_ollama_native_catalog': ('xiuqo_cli.models_local', 'should_use_ollama_native_catalog'),
    'url_origin': ('xiuqo_cli.urllib_security', 'url_origin'),
    'validate_requested_model': ('xiuqo_cli.models_validate', 'validate_requested_model'),
    'warm_nous_reasoning_caps_async': ('xiuqo_cli.models_reasoning_caps', 'warm_nous_reasoning_caps_async'),
    'warm_openrouter_reasoning_caps_async': ('xiuqo_cli.models_reasoning_caps', 'warm_openrouter_reasoning_caps_async'),
}
