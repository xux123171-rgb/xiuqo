...
cycle) and tests patch ``xiuqo_cli.config.load_config`` etc. at call time.
"""
...
def _ask(prompt: str, *, secret: bool = False, raw: bool = False, cancel_msg: str | None = "\nCancelled.",
    ...
    if secret:
        from xiuqo_cli.secret_prompt import masked_secret_prompt as fn
    else:
...
def _existing_api_key_for_model_flow(provider_id: str, pconfig) -> tuple[str, str]:
    ...
    from xiuqo_cli.auth import _resolve_api_key_provider_secret
...
def _commit_model_config(cfg: dict) -> None:
    ...
    from xiuqo_cli.auth import deactivate_provider
