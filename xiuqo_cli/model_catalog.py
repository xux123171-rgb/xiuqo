...
``~/.xiuqo/cache/model_catalog.json`` → master URL fetch; any fetch failure keeps the stale copy
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/xux123171-rgb/xiuqo/main/website/static/api/model-catalog.json",
)
...
def _fetch_manifest(url: str, timeout: float) -> dict[str, Any] | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": _XIUQO_USER_AGENT})
...
def refresh_catalogs() -> bool:
    ...
    try:
        from xiuqo_cli.models import fetch_nous_recommended_models, fetch_openrouter_models
...
def get_default_model_from_cache(provider: str) -> str | None:
    """The manifest's labeled default for ``provider`` (the model Xiuqo silently lands on when the
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    After ``xiuqo update`` that file IS the newest catalog, so the picker stays current even when
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``xiuqo model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime, _catalog_cache_source_path
    _catalog_cache = None
