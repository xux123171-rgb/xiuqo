...
def stored_session_route(session_meta, *, current_model, current_provider):
    ...
    if not stored_model:
        return None
    from xiuqo_state import SessionDB as _SessionDB
...
def _merge_preflight_warning(cli, result, custom_providers) -> None:
    ...
    if cli.agent is None:
        return
    try:
        from xiuqo_cli.context_switch_guard import merge_preflight_compression_warning
...
def _print_switch_summary(cli, result, old_model, *, one_turn: bool, strict_context: bool) -> None:
    ...
    from xiuqo_cli.model_switch import format_model_for_display, resolve_display_context_length
...
def _commit_model_switch(
    ...
    if persist_global:
        from xiuqo_cli.model_switch import persist_model_selection
...
def _show_model_picker(cli, ctx, force_refresh: bool) -> None:
    ...
    from cli import _cprint
    from xiuqo_cli.inventory import build_models_payload
...
class CLIModelSwitchMixin:
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            from xiuqo_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if opencode_provider_family(resolved_provider) is not None:
            from xiuqo_cli.models import normalize_opencode_model_id, opencode_model_api_mode
            return _adopt_with_mode(
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        if warning is None:
            return True
        choices = [
            ("once", "Switch anyway", "Use this model for the current Xiuqo session."),
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # Curated list (same as `xiuqo model` / gateway pickers); live catalog only when
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from cli import _cprint
        from xiuqo_cli.model_switch import parse_model_switch_args, resolve_persist_behavior
    ...
    def _cmd_moa(self, cmd_original: str):
        ...
        from xiuqo_cli.moa_config import moa_usage, normalize_moa_config
