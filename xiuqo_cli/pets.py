"""CLI subcommand: ``xiuqo pets <subcommand>``."""
...
def _cmd_list(args) -> int:
    ...
    if args.installed:
        ...
        if not pets:
            print("No pets installed. Try: xiuqo pets install boba")
            return 0
    ...
    print("\nInstall one with: xiuqo pets install <slug>")
    return 0
...
def _cmd_install(args) -> int:
    ...
    if args.select or not _has_active_pet():
        ...
        print(f"  Make it active with: xiuqo pets select {slug}")
    return 0
...
def _cmd_select(args) -> int:
    ...
    if not slug:
        pets = store.installed_pets()
        if not pets:
            _err("✗ no pets installed — run: xiuqo pets install boba")
            return 1
    ...
    if pet is None or not pet.exists:
        _err(f"✗ '{slug}' is not installed — run: xiuqo pets install {slug}")
        return 1
...
def _cmd_show(args) -> int:
    ...
    if pet is None:
        _err("✗ no pet to show — run: xiuqo pets install boba")
        return 1
...
def _cmd_doctor(args) -> int:
    ...
    ok = True
    if not pets:
        print("  → no pets installed. Run: xiuqo pets install boba")
        ok = False
    elif active is None:
        print("  → active pet unresolved. Run: xiuqo pets select <slug>")
        ok = False
    elif not enabled:
        print("  → pet display is disabled. Run: xiuqo pets select " + active.slug)
