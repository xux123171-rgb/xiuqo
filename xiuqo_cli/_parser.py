"""Top-level argparse construction for the xiuqo CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``xiuqo_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``XIUQO_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/xiuqo_cli/
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``xiuqo --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``xiuqo_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    xiuqo                        Start interactive chat
    xiuqo chat -q "Hello"        Single query mode
    xiuqo --tui                  Launch the modern TUI (or set display.interface: tui)
    xiuqo --cli                  Force the classic REPL (overrides display.interface: tui)
    xiuqo -c                     Resume the most recent session
    xiuqo -c "my project"        Resume a session by name (latest in lineage)
    xiuqo --resume <session_id>  Resume a specific session by ID
    xiuqo --resume latest        Resume the most recent session (same as -c)
    xiuqo --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    xiuqo setup                  Run setup wizard
    xiuqo logout                 Clear stored authentication
    xiuqo auth add <provider>    Add a pooled credential
    xiuqo auth list              List pooled credentials
    xiuqo auth remove <p> <t>    Remove pooled credential by index, id, or label
    xiuqo auth reset <p> [t]     Clear exhaustion status for a provider, or one credential
    xiuqo auth priority <p> <t> <n>  Move a pooled credential to priority n (0 = tried first)
    xiuqo auth refresh <p> [t]   Refresh a pooled OAuth credential and clear its cooldown
    xiuqo model                  Select default model
    xiuqo fallback [list]        Show fallback provider chain
    xiuqo fallback add           Add a fallback provider (same picker as `xiuqo model`)
    xiuqo fallback remove        Remove a fallback provider from the chain
    xiuqo config                 View configuration
    xiuqo config edit            Edit config in $EDITOR
    xiuqo config set model gpt-4 Set a config value
    xiuqo gateway                Run messaging gateway
    xiuqo -s xiuqo-dev,github-auth
    xiuqo -w                     Start in isolated git worktree
    xiuqo gateway install        Install gateway background service
    xiuqo sessions list          List past sessions
    xiuqo sessions browse        Interactive session picker
    xiuqo sessions rename ID T   Rename/title a session
    xiuqo logs                   View agent.log (last 50 lines)
    xiuqo logs -f                Follow agent.log in real time
    xiuqo logs errors            View errors.log
    xiuqo logs --since 1h        Lines from the last hour
    xiuqo debug share             Upload debug report for support
    xiuqo console                Open the safe Xiuqo command console
    xiuqo update                 Update to latest version
    xiuqo dashboard              Start web UI dashboard (port 9119)
    xiuqo dashboard --stop       Stop running dashboard processes
    xiuqo dashboard --status     List running dashboard processes
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via XIUQO_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `xiuqo setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to XIUQO_ACCEPT_HOOKS=1 or "
    ...
    inherited(parser, "--ignore-user-config", action="store_true", default=False,
              help="Ignore ~/.xiuqo/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)")
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``xiuqo -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/xiuqo_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with Xiuqo Agent")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also XIUQO_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--ignore-user-config", action="store_true", default=SUPPRESS,
              help="Ignore ~/.xiuqo/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.")
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Xiuqo itself.")
