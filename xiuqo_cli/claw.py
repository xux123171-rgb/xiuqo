"""xiuqo claw — OpenClaw migration commands."""
...
from xiuqo_cli.config import get_hermes_home, get_config_path, load_config, save_config
from xiuqo_constants import get_optional_skills_dir
from xiuqo_cli.setup import (Colors, color, print_header, print_info, print_success, print_error,
                              prompt_yes_no)
...
# `xiuqo claw migrate` flags/defaults. Secrets are never included implicitly: --migrate-secrets
...
def _print_banner(title: str) -> None:
    ...
    print()
    rule = "─" * 57
    for line in (f"┌{rule}┐", f"│          ☤ Xiuqo — {title:<35s}│", f"└{rule}┘"):
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    if running and _warn_running(
        ...
        ("Messaging platforms (Telegram, Discord, Slack) only allow one "
         ...
         "Xiuqo may try to use the same token, causing disconnects.",
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Warn if a Xiuqo gateway has connected platforms (token conflicts, e.g. Telegram 409)."""
    ...
    if connected and _warn_running(
        auto_yes, "Xiuqo gateway is running with active connections: " + ", ".join(connected), [],
        ("Migrating bot tokens while the gateway is active will cause "
         "conflicts (Telegram, Discord, and Slack only allow one active "
         "session per token).",
         "Recommendation: stop the gateway first with 'xiuqo gateway stop'."),
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: xiuqo claw <command> [options]\n\nCommands:\n"
              "  migrate          Migrate settings from OpenClaw to Xiuqo\n"
              "  cleanup          Archive leftover OpenClaw directories after migration\n\n"
              "Run 'xiuqo claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Xiuqo migration: preflight, preview, confirm, back up, apply."""
    ...
    if not opts.source_dir.is_dir():
        return _error_block(
            ...
            "You can specify a custom path: xiuqo claw migrate --source /path/to/.openclaw")
    ...
    opts.xiuqo_home = get_hermes_home()
    print()
    print_header("Migration Settings")
    _info(f"Source:      {opts.source_dir}", f"Target:      {opts.xiuqo_home}",
    ...
    if _confirm(opts.yes, "Proceed with migration?", default=True, declined="Migration cancelled.",
                non_tty=("Non-interactive session — preview only.",
                         "To execute, re-run with: xiuqo claw migrate --yes")):
        _apply_migration(run_migrator, opts)
    # Source directory is left untouched — archiving is `xiuqo claw cleanup`'s job.
...
def _load_migrator(script_path: Path, opts: SimpleNamespace) -> Optional[Callable[[bool], dict]]:
    ...
    return lambda execute: mod.Migrator(
        source_root=opts.source_dir.resolve(), target_root=opts.xiuqo_home.resolve(),
...
def _preview_migration(run_migrator: Callable[[bool], dict], opts: SimpleNamespace) -> bool:
    ...
    if conflicts > 0 and not opts.overwrite:
        _error_block(
            ...
            "Each conflict is an item whose target already exists in ~/.xiuqo/. "
...
def _apply_migration(run_migrator: Callable[[bool], dict], opts: SimpleNamespace) -> None:
    ...
    restorable with `xiuqo import`: one restore point before any mutation, pruned to the last 5."""
    backup_archive: Optional[Path] = None
    if not opts.no_backup:
        try:
            from xiuqo_cli.backup import create_pre_migration_backup
            from xiuqo_cli.sizefmt import format_bytes as _format_size
            backup_archive = create_pre_migration_backup(xiuqo_home=opts.xiuqo_home)
            if backup_archive:
                print()
                print_success(f"Pre-migration backup: {backup_archive} "
                              f"({_format_size(backup_archive.stat().st_size)})")
                print_info(f"Restore with: xiuqo import {backup_archive.name}")
        except Exception as e:
            return _error_block(
                f"Could not create pre-migration backup: {e}",
                "Re-run with --no-backup to skip, or free up disk space under the Xiuqo home.",
    ...
    try:
        ...
        if backup_archive:
            _info(f"A pre-migration backup is available at: {backup_archive}",
                  f"Restore with: xiuqo import {backup_archive.name}")
        return
...
def _cmd_cleanup(args):
    ...
    if running and not _warn_running(
        ...
        "Proceed anyway?",
        declined="Aborted. Stop OpenClaw first, then re-run: xiuqo claw cleanup",
        ...
        if dry_run:
            ...
            print_info(f"Would archive: {source_dir} → {archive_path}")
                      ...
                      non_tty=(f"Non-interactive session — would archive: {source_dir}",
                               "To execute, re-run with: xiuqo claw cleanup --yes")):
            try:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        _info("", "To execute the migration, run without --dry-run:",
              f"  xiuqo claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if any(i.get("kind") == "provider-keys" and i.get("status") == "skipped" for i in items):
            ...
            _info("", "To migrate API keys, re-run with:",
                  "  xiuqo claw migrate --migrate-secrets", "", "Or add your key manually:",
                  "  xiuqo config set OPENROUTER_API_KEY sk-or-v1-...")
