...
# ``xiuqo auth add openrouter --api-key sk-or-...`` must keep working with no ``--type``.
...
def _qwen_oauth_login(args) -> dict:
    from xiuqo_cli.auth_qwen import _mark_qwen_oauth_active
...
class _OAuthAddSpec:
    """Per-provider parameters for the generic ``xiuqo auth add <provider> --type oauth`` path."""
...
_OAUTH_ADD_SPECS: dict[str, _OAuthAddSpec] = {
    "anthropic": _OAuthAddSpec(
        ...
        source=f"{SOURCE_MANUAL}:xiuqo_pkce",
...
def _add_nous_oauth_credential(args, provider: str) -> PooledCredential:
    """``xiuqo auth add nous --type oauth``: shared-credential import, else device-code login."""
    ...
    # Codex-style auto-import: a shared Nous credential at <xiuqo-root>/shared/nous_auth.json
    # (written by any previous login) makes `xiuqo --profile <name> auth add nous --type oauth`
...
def _add_credential(args, provider: str, pool, requested_type: str) -> PooledCredential:
    ...
    if spec is None:
        raise SystemExit(f"`xiuqo auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def _report_priority(provider: str, pool, moved, requested: int, verb: str, prep: str) -> None:
    ...
    print(f'{verb} {provider} credential "{moved.label}" {prep} priority {moved.priority} '
          f"(#{moved.priority + 1} in `xiuqo auth list {provider}`)")
...
def auth_priority_command(args) -> None:
    """`xiuqo auth priority <provider> <target> <priority>`: reorder one pooled credential."""
    provider = _normalize_provider(getattr(args, "provider", ""))
    pool = load_pool(provider)
...
def _free_tier_lines() -> tuple[str, str]:
    ...
    from xiuqo_cli.anon_auth import FREE_TIER_LABEL, GUEST_MODEL, UPGRADE_HINT
...
def auth_remove_command(args) -> None:
    ...
    # Every credential source Xiuqo reads from (env vars, external OAuth files, auth.json blocks,
...
def auth_refresh_command(args) -> None:
    """`xiuqo auth refresh <provider> [target]`: force one pooled OAuth entry to refresh.
    ...
    if target is None or not str(target).strip():
        if len(entries) != 1:
            raise SystemExit(
                ...
                f"label (see `xiuqo auth list {provider}`).")
    ...
    if provider == "nous" and matched.source != "device_code":
        raise SystemExit(
            ...
            "Reauthenticate with `xiuqo auth add nous --type oauth`.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `xiuqo auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `xiuqo auth` is called bare."""
...
def auth_upgrade_command(args) -> None:
    """``xiuqo auth upgrade``: sign the free tier into a Nous account, keeping its connectors."""
    from xiuqo_cli.anon_auth import upgrade_guest
