"""Image-authored deployment provenance for immutable Xiuqo runtimes.

The image bakes ``/etc/xiuqo/image-provenance.json`` outside ``$XIUQO_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/xiuqo/image-provenance.json")
