"""MCP Server Management CLI — ``xiuqo mcp`` subcommand."""
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    Mirrors ``_load_mcp_config()`` in ``tools/mcp_tool.py``: load ``~/.xiuqo/.env`` into ``os.environ`` and
...
def _configure_http_auth(
    ...
    if needs_auth and (auth_type == "header" or not auth_type):
        ...
        if get_env_value(env_key):
            ...
            from xiuqo_cli.cli_output import prompt
...
def cmd_mcp_add(args):
    ...
    # --command uses dest="mcp_command" (see xiuqo_cli/main.py for why the dest is renamed).
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  xiuqo mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  xiuqo mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  xiuqo mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: xiuqo mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # an earlier `xiuqo mcp test`) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  xiuqo mcp add <name> --url <endpoint>')
        _info('  xiuqo mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict, *, flow: str | None = None) -> bool:
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `xiuqo mcp remove` + `xiuqo mcp add` to reconfigure auth.")
        return False
    ...
    # re-auth path in web_server.py. force_interactive_oauth: `xiuqo mcp login` is explicitly
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `xiuqo mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    if not name:
        ...
        _info("Usage: xiuqo mcp reauth <name>   |   xiuqo mcp reauth --all")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'xiuqo mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    cfg = _lookup_server(name, _get_mcp_servers(), "Available")
    if cfg is None:
...
_MCP_USAGE = (
    "xiuqo mcp                                    Open the catalog picker (default)",
    "xiuqo mcp catalog                            List Nous-approved MCPs",
    "xiuqo mcp install <name>                     Install a catalog MCP",
    "xiuqo mcp serve                              Run as MCP server",
    "xiuqo mcp add <name> --url <endpoint>        Add a custom MCP server",
    "xiuqo mcp add <name> --command <cmd>         Add a stdio server",
    "xiuqo mcp add <name> --preset <preset>       Add from a known preset",
    "xiuqo mcp remove <name>                      Remove a server",
    "xiuqo mcp list                               List configured servers",
    "xiuqo mcp test <name>                        Test connection",
    "xiuqo mcp configure <name>                   Toggle tools",
    "xiuqo mcp login <name>                       Re-authenticate OAuth",
    "xiuqo mcp reauth <name> | --all              Re-auth one or all OAuth servers",
)


def mcp_command(args):
    """Main dispatcher for ``xiuqo mcp`` subcommands."""
    ...
    if action in ("picker", "catalog", "install"):
        ...
        from xiuqo_cli import mcp_picker
    ...
    if handler:
        handler(args)
        return
    # No subcommand — drop the user into the catalog picker (same UX as `xiuqo plugin`).
    from xiuqo_cli.mcp_picker import run_picker
    run_picker()
