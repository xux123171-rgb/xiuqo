"""``xiuqo profile`` command — one handler per action, dispatched by ``PROFILE_ACTIONS``.

Imports from ``xiuqo_cli.profiles`` stay lazy (inside each handler) so tests can monkeypatch
...
def _render_distribution_plan(plan) -> None:
    ...
    from xiuqo_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.xiuqo_requires:
        print(f"  Requires: Xiuqo {mf.xiuqo_requires}")
...
def _profile_status(args):
    """Bare ``xiuqo profile`` — show current profile status."""
    from xiuqo_constants import display_hermes_home
    from xiuqo_cli.profiles import format_profile_label, get_active_profile_name, list_profiles
    ...
    if current is not None:
        ...
        if p.alias_path:
            print(f"Alias:          {p.alias_name or p.name} → xiuqo -p {p.name}")
    print()
...
def _shared_credential_warnings(profiles) -> list:
    ...
    from xiuqo_cli.profile_channels import shared_channel_credentials, shared_credential_warning
...
def _profile_use(args):
    from xiuqo_cli.profiles import set_active_profile
    name = args.profile_name
    try:
        set_active_profile(name)
        print("Switched to: default (~/.xiuqo)" if name == "default" else f"Switched to: {name}")
...
def _clone_channels_refusal(source_label: str) -> Optional[str]:
    ...
    from xiuqo_cli.gateway_multiplex_served import recorded_served_profiles
    from xiuqo_cli.profile_channels import channel_platforms_configured
    ...
    return (
        ...
        "(xiuqo -p <name> setup), or route its chats with gateway.profile_routes instead."
    )


def _print_channel_clone_notice(name: str, source_label: str, clone_channels: bool, clone_flag: str) -> None:
    from xiuqo_cli.profile_channels import (
...
def _profile_create(args):
    ...
    if not no_alias:
        ...
        if collision:
            ...
            print(f"  Choose a custom alias:  xiuqo profile alias {name} --name <custom>")
            print(f"  Or access via flag:     xiuqo -p {name} chat")
        else:
    ...
    if served is not None and normalize_profile_name(name) in {normalize_profile_name(p) for p in served}:
        ...
        print("  xiuqo gateway restart    Serve this profile from the running multiplexed gateway")
    else:
...
def _profile_delete(args):
    from xiuqo_cli.profiles import delete_profile
    try:
...
def _describe_target_dir(name: str) -> Path:
    ...
    from xiuqo_cli import profiles as _profiles_mod
    if _profiles_mod.normalize_profile_name(name) == "default":
        from xiuqo_constants import get_hermes_home as _hh
        return Path(_hh())
...
def _profile_describe(args):
    ...
    from xiuqo_cli import profile_describer as _pd
    if all_flag:
...
def _profile_show(args):
    ...
    if dist_name:
        ...
        print(f"  (run `xiuqo profile info {name}` for full manifest)")
    if alias_name:
        print(f"Alias:   {alias_name} → xiuqo -p {name}  ({_wrapper_path(alias_name)})")
    print()
...
def _profile_rename(args):
    from xiuqo_cli.profiles import normalize_profile_name, rename_profile
    try:
...
def _profile_export(args):
    from xiuqo_cli.profiles import export_profile, get_profile_export_path
    name = args.profile_name
    try:
...
def _profile_install(args):
    import tempfile
    from xiuqo_cli.profile_distribution import DistributionError, install_distribution, plan_install
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="xiuqo_dist_preview_") as tmp:
        ...
        if plan.has_cron:
            print(
                ...
                f"  Review them with:  xiuqo -p {plan.manifest.name} cron list"
            )
        print(f"\n  Use with:      xiuqo -p {plan.manifest.name} chat")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")


def _profile_update(args):
    from xiuqo_cli.profile_distribution import DistributionError, read_manifest, update_distribution
    from xiuqo_cli.profiles import get_profile_dir, normalize_profile_name
    try:
        ...
        if current is None:
            _die(
                ...
                "Only profiles installed via `xiuqo profile install` can be updated."
            )
        ...
        if plan.has_cron:
            print(f"  Cron files were refreshed.  Review with:  xiuqo -p {plan.manifest.name} cron list")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")
...
def _profile_info(args):
    from xiuqo_cli.profile_distribution import describe_distribution, DistributionError
    try:
...
# Order mirrors the original if/elif chain; None = bare ``xiuqo profile``.
PROFILE_ACTIONS = {
