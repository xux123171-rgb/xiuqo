...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: xiuqo gateway install\n"
                "                    sudo xiuqo gateway install --system  # Linux servers\n"
                "     Check status:  xiuqo cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        print(color("No scheduled jobs.\nCreate one with 'xiuqo cron create ...' "
...
def cron_tick():
    ...
    try:
        ...
        print("  Check `xiuqo cron status` and the gateway log for details.")
        return 1
    return 0
...
def cron_incidents(args) -> int:
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(color("✗ Incident ID required: xiuqo cron incidents ack <incident_id>", Colors.RED))
            return 1
    ...
    for inc in incidents:
        ...
        print()
    print(color(f"  {len(incidents)} incident(s)  |  ack one with: xiuqo cron incidents ack <id>",
                Colors.DIM))
    return 0


_PERMISSION_HINT = ("  Hint: jobs.json may be owned by another user (e.g. rewritten by a root "
                    "`docker exec xiuqo xiuqo cron ...`). Fix ownership to match the gateway "
...
def _print_ticker_health(pids: list) -> None:
    ...
    if hb_age is None:
        ...
        print("  Cron jobs will NOT fire until the ticker writes its first heartbeat.\n"
              "  If the gateway just started, wait ~60s and re-run `xiuqo cron status`.\n"
              "  If heartbeat never appears, restart: xiuqo gateway restart")
        ...
        print("  Cron jobs may NOT be firing. Restart: xiuqo gateway restart")
...
def cron_status():
    ...
    from xiuqo_cli.gateway import find_gateway_pids, named_profile_served_by_running_multiplexer
    print()
    ...
    if provider != "builtin":
        ...
        print(color("✓ Gateway is running via the default-profile multiplexer — it ticks this profile's jobs.",
                    Colors.GREEN))
        print(color("  Ticker health is reported by `xiuqo cron status` on the default profile.", Colors.DIM))
    else:
        ...
        if pids or gateway_alive_via_lock:
            ...
            print("\n  To enable automatic execution:\n"
                  "    xiuqo gateway install    # Install as a user service\n"
                  "    sudo xiuqo gateway install --system  "
                  "# Linux servers: boot-time system service\n"
                  "    xiuqo gateway            # Or run in foreground")
...
def _script_health_issue(script: str) -> Optional[str]:
    ...
    try:
        ...
        return f"script resolves outside XIUQO_HOME/scripts: {script!r}"
    if not path.exists():
...
def cron_doctor() -> int:
    ...
    for job, issues in findings:
        ...
        for issue in issues:
            print(f"    - {issue}")
    print()
    print(color("Next: fix the listed job config, then run `xiuqo cron doctor` again.", Colors.DIM))
    return 1
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # inherits XIUQO_SESSION_KEY) would be orphaned mid-LLM-call, leaving the execution row
        ...
        with contextlib.suppress(Exception):
            ...
            # gateway/desktop session env (XIUQO_SESSION_KEY); declare the channel stateless so
...
def cron_notepad(args) -> int:
    """Handle ``xiuqo cron notepad <job_id> [get|set|delete|list]`` (per-job durable KV).
    ...
    try:
        ...
        if key is None or (action == "set" and value is None):
            print(color(f"Usage: xiuqo cron notepad <job_id> {usage_args}", Colors.RED))
            return 1
        if action == "set":
...
def cron_command(args):
    ...
    print(f"Unknown cron command: {subcmd}\n"
          "Usage: xiuqo cron [list|create|edit|pause|resume|run|remove|resnap|status|runs|doctor|tick]")
    sys.exit(1)


def _cron_resnap(args) -> int:
    """Handle `xiuqo cron resnap [job_id] [--all]`."""
    ...
    if not job_id:
        ...
        print("Usage: xiuqo cron resnap <job_id> | xiuqo cron resnap --all")
        return 1
