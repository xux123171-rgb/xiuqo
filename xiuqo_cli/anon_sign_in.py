...
from xiuqo_cli.auth_constants import httpx
...
UPGRADE_UNAVAILABLE = "The free tier is not available right now; run `xiuqo auth add nous` to sign in."
UPGRADE_REASON_COPY = {
...
UPGRADE_NO_DEFAULT_TERMINAL = "No default model is set yet; run `xiuqo model` to pick one."
...
LOGIN_DM_ONLY = "Sign in from a direct message with Xiuqo."
LOGIN_BUSY_ELSEWHERE = "Another sign-in is already running on this Xiuqo. Try again in a few minutes."
LOGIN_NOT_ALLOWED = "Only an operator of this Xiuqo can sign it in."
...
# the in-chat form (never a raw exception, a URL or a ``xiuqo`` verb) and ``.copy_terminal`` the
...
def _outcome_state(outcome: Dict[str, Any], anon_token: str) -> SignInState:
    ...
    """
    from xiuqo_cli import anon_auth as _core
...
def run_sign_in(
    ...
    *client_factory* is the HTTP client seam, ``client_factory(timeout_seconds, verify)``.
    """
    from xiuqo_cli import anon_auth as _core
    from xiuqo_cli.auth import PROVIDER_REGISTRY, _resolve_verify
    from xiuqo_cli.auth_device_flow import _request_device_code
    from xiuqo_cli.auth_nous import _nous_http_client
