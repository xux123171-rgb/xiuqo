...
Every Xiuqo process that may need the free tier runs this once at boot (``xiuqo serve`` on a
...
(:func:`xiuqo_cli.anon_auth.guest_enabled`), resolves which provider carries inference, records
...
def start_background_bootstrap() -> threading.Thread:
    """``xiuqo serve`` entry: run on a daemon thread so a slow portal never delays the socket."""
