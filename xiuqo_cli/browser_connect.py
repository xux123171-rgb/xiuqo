"""Shared helpers for attaching Xiuqo to a local Chromium-family CDP port.
...
# the real profile into ``~/.xiuqo/browser-profile/`` and launch the user's real binary on
...
def real_profile_copy_dir(browser: str) -> str:
    """Return the xiuqo-owned snapshot dir for ``browser``'s real profile."""
...
def _secure_snapshot(path: str, *, contents: bool = False) -> None:
    ...
    NixOS group-share carve-out, XIUQO_UID/GID) as every Xiuqo secret dir — via ``_secure_dir``/
    ...
    ``XIUQO_HOME_MODE`` hatch. Best-effort; never blocks a launch."""
    try:
        from xiuqo_cli.config import _secure_dir, _secure_file
        if not contents:
            _secure_dir(path)
            return
...
_SNAPSHOT_DONE_MARKER = ".xiuqo-snapshot-complete"
...
def _locked_profile_error(browser: str) -> str:
    ...
    if _real_profile_autoclose():
        msg = (
            ...
            "yet. Xiuqo can close it for you (this quits the browser — you'll lose unsaved "
        ...
        msg = (
            ...
            "browser.use_real_profile off. (Enable browser.real_profile_autoclose to let Xiuqo "
...
def snapshot_real_profile(browser: str, src: str | None = None) -> tuple[str | None, str | None]:
    """Snapshot ``browser``'s real ACTIVE profile into the xiuqo copy dir; returns ``(dst, err)``.
