...
from xiuqo_cli.config import DEFAULT_CONFIG, get_process_hermes_home
from xiuqo_cli.web_models import MCPServerCreate
from xiuqo_cli.web_server_gateway import _ACTION_LOG_FILES
from xiuqo_cli.web_server_mcp import _normalize_mcp_server_create
...
def _hermes_home_scope(path) -> Any:
    ...
    time) to ``path`` for the block via the context-local XIUQO_HOME override."""
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    """Write MCP server entries into ``profile_dir``'s config.yaml (XIUQO_HOME-scoped).
...
def _profile_scope(profile: Optional[str]):
    ...
    is stale (test isolation, late XIUQO_HOME override). Yields the profile dir for a named
    ...
    with _config_profile_scope(profile) as scoped:
        ...
        with _SKILLS_PROFILE_LOCK:
            saved = [(m.XIUQO_HOME, m.SKILLS_DIR) for m in modules]
            for m in modules:
                m.XIUQO_HOME, m.SKILLS_DIR = profile_dir, profile_dir / "skills"
            try:
                yield scoped
            finally:
                for m, (home, skills_dir) in zip(modules, saved):
                    m.XIUQO_HOME, m.SKILLS_DIR = home, skills_dir


@contextmanager
def _config_profile_scope(profile: Optional[str]):
    """Await-safe, config-only profile scope: touches ONLY the task-local XIUQO_HOME
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    a fresh ``xiuqo`` subprocess whose ``_apply_profile_override()`` reads ``-p`` from argv —
