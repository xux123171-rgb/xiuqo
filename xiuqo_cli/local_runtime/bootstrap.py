...
from xiuqo_cli.local_runtime.gguf import SPLIT_PART_RE, model_id_from_stem

logger = logging.getLogger(__name__)

_SUPERVISOR = None  # process-wide singleton; one router per Xiuqo process
...
def _detect_gpu_vendor() -> str | None:
    ...
    from xiuqo_cli.local_runtime.hardware import _nvidia_smi_path
...
def _presets_stale() -> bool:
    ...
    with suppress(Exception):
        from xiuqo_cli.local_runtime.presets import read_preset_decisions
...
def _stop_state_server(state: dict) -> None:
    ...
    server."""
    from xiuqo_cli.local_runtime.endpoint import _pid_alive
...
def _generate_presets(mdir: Path, preset_path: Path) -> Path | None:
    ...
    """
    from xiuqo_cli.local_runtime.hardware import probe_budget
    from xiuqo_cli.local_runtime.presets import generate_presets
...
def ensure_local_runtime(config: dict, force: bool = False) -> "object | None":
    ...
    # Another Xiuqo process may already be supervising — reuse via state, but ONLY while its
