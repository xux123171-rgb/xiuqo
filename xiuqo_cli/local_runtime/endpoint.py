...
def _state_endpoint() -> dict | None:
    ...
    if not base_url:
        return None
    # Ownership proof: on the stable port a SECOND install (different XIUQO_HOME) can own
...
def resolve_llamacpp_endpoint(config: dict | None = None,
    ...
    from xiuqo_cli.local_runtime.detect import detect_server
...
def _boot_in_flight(config: dict | None) -> bool:
    ...
    with suppress(Exception):
        ...
        if not ((config or {}).get("local_runtime") or {}).get("enabled"):
            return False
        from xiuqo_cli.local_runtime.binaries import manifest_verified, runtimes_root
