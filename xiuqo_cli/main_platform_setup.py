...
from xiuqo_cli.model_setup_flows_common import _say
...
def _whatsapp_choose_mode(get_env_value, save_env_value):
    """Step 1 of ``xiuqo whatsapp``: ``"bot"`` / ``"self-chat"``, or None when cancelled."""
    current_mode = get_env_value("WHATSAPP_MODE") or ""
    if current_mode:
        mode_label = "separate bot number" if current_mode == "bot" else "personal number (self-chat)"
        print(f"\n✓ Mode: {mode_label}")
        return current_mode
    _say("", "How will you use WhatsApp with Xiuqo?", "",
...
def _whatsapp_allowed_users(wa_mode: str, get_env_value, save_env_value) -> None:
    """Step 3 of ``xiuqo whatsapp``: show / set WHATSAPP_ALLOWED_USERS."""
...
def _whatsapp_install_bridge(bridge_dir) -> bool:
    """Step 4 of ``xiuqo whatsapp``: ``npm install`` the bridge when needed. False = stop."""
...
def cmd_whatsapp(args):
    ...
    from xiuqo_cli.main import _require_tty, get_hermes_home
    ...
    # every `xiuqo gateway` would pay a 30s bridge timeout + indefinite retries. Set only after
    ...
    if (session_dir / "creds.json").exists():
        ...
        if _yes_no("\n  Re-pair? This will clear the existing session. [y/N] "):
            ...
            _say("\n✓ WhatsApp is configured and paired!", "  Start the gateway with: xiuqo gateway")
            return
    ...
    _say("", "  Or install as a service: xiuqo gateway install")


def cmd_whatsapp_cloud(args):
    """Set up WhatsApp Business Cloud API (official Meta integration) — complementary to the
    ``xiuqo whatsapp`` Baileys bridge wizard. See ``xiuqo_cli/setup_whatsapp_cloud.py``."""
    from xiuqo_cli.main import _require_tty
    _require_tty("whatsapp-cloud")
    from xiuqo_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def _sync_toggle(args, sub: str) -> int:
    ...
    if not is_curation_eligible(skill):
        _err(f"'{skill}' is not sync-eligible (bundled, hub-installed, "
             ...
             f"skills under ~/.xiuqo/skills/ can sync.")
        return 1
...
def _sync_status(ssc) -> int:
    ...
    if status.get("org_available"):
        ...
        if modified:
            _err(f"  {len(modified)} with local edits not yet shared: "
                 f"{', '.join(modified)}\n"
                 f"  Share them back with `xiuqo sync propose <skill>`. "
    ...
    if not status.get("logged_in"):
        ...
        _err("\nSync feature is off for this instance (set XIUQO_SYNC_ENABLED=1 "
             "or config.yaml sync.enabled: true). Sync is inert.")
    elif not status.get("base_url"):
        _err("\nNo sync base URL configured (config.yaml sync.base_url or XIUQO_SYNC_BASE_URL). Sync is inert.")
    return 0
...
_SYNC_GATED = {
    "pull": _sync_pull,
    "push": lambda ssc, identity: ssc.push_skills(identity=identity, message="xiuqo sync push"),
    "now": lambda ssc, identity: {"pull": ssc.pull_skills(identity=identity),
                                  "push": ssc.push_skills(identity=identity, message="xiuqo sync now")}}
...
def cmd_sync(args):
    ...
    if not ssc.resolve_sync_base_url():
        _err("sync inert: no sync base URL configured (config.yaml sync.base_url or XIUQO_SYNC_BASE_URL).")
        return 1
...
def cmd_slack(args):
    """``xiuqo slack <subcommand>``; ``manifest`` prints or writes a Slack app manifest with
    every gateway command registered as a first-class slash."""
    sub = getattr(args, "slack_command", None)
    if sub in {None, ""}:
        _err("usage: xiuqo slack <subcommand>\n"
             "\n"
             "subcommands:\n"
             "  manifest   Generate a Slack app manifest with every gateway\n"
             "             command registered as a native slash\n"
             "\n"
             "Run `xiuqo slack manifest -h` for details.")
        return 1

    if sub == "manifest":
        from xiuqo_cli.slack_cli import slack_manifest_command
