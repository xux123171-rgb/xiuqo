"""``xiuqo gateway enroll`` — enroll a self-hosted gateway with a relay connector.
    ...
    try:
        ...
        if exc.code == 401:
            message = (
                ...
                "token could not be verified — try `xiuqo auth add nous` and retry."
            )
        elif exc.code == 403:
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        _fail(
            "✗ `xiuqo gateway enroll` is not available in a managed/hosted install.\n"
    ...
    # Persist idempotently; save_env_value writes the sensitive values to ~/.xiuqo/.env and never
