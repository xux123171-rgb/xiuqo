"""``xiuqo plugins`` catalog surface: resolution, provenance sidecar, search/browse/info/validate,
catalog-aware update, plus the dashboard/TUI-facing catalog payload helpers.

Sibling of :mod:`xiuqo_cli.plugins_cmd` (the installer core, enable/disable state and console helpers
...
from xiuqo_cli.plugin_catalog import (
...
CATALOG_SIDECAR = ".xiuqo-catalog.json"
...
def looks_like_catalog_name(identifier: str) -> bool:
    ...
    from xiuqo_cli.plugins_cmd import _URL_SCHEMES
...
def raise_if_removed(*candidates: str) -> None:
    ...
    from xiuqo_cli.plugins_cmd import PluginOperationError
    for candidate in candidates:
        ...
        if removed is not None:
            ...
            raise PluginOperationError(
                f"Plugin '{removed.name}' was removed from the Xiuqo plugin catalog and is blocked from "
...
def resolve_catalog_name(identifier: str, console) -> PluginCatalogEntry:
    ...
    from xiuqo_cli.plugins_cmd import _fail
    entry = get_live_catalog_entry(identifier)
    if entry is None:
        _fail(console, (
            f"[red]Error:[/red] '{identifier}' is not in the Xiuqo plugin catalog and is not a Git URL or "
            "owner/repo shorthand. Browse entries with `xiuqo plugins search`."))
...
def write_catalog_sidecar(target: Path, entry: PluginCatalogEntry) -> None:
    """``.xiuqo-catalog.json`` inside the install dir — how ``update``/``list``/dashboards know the plugin
...
def install_catalog_entry(entry: PluginCatalogEntry, *, force: bool, ref: Optional[str] = None,
    ...
    from xiuqo_cli.plugins_cmd import _install_plugin_core
...
def repin_catalog_plugin(target: Path, sidecar: dict) -> tuple[str, bool]:
    ...
    from xiuqo_cli.plugins_cmd import PluginOperationError, _get_enabled_set, _save_enabled_set
    ...
    if entry is None:
        raise PluginOperationError(
            ...
            "See `xiuqo plugins info` and the removed blocklist.")
...
def cmd_update_catalog(name: str, target: Path, sidecar: dict, console) -> None:
    from xiuqo_cli.plugins_cmd import PluginOperationError, _fail
...
def _render_entries(entries: List[PluginCatalogEntry], console) -> None:
    from xiuqo_cli.plugins_cmd import _table
    table = _table(((("Name", "bold")), ("Tier", None), ("Description", None), ("Pinned", "dim"),
                    ("Capabilities", "dim")), title="Xiuqo Plugin Catalog (curated)")
    for e in entries:
    ...
    console.print(table)
    console.print()
    console.print("[dim]Details:[/dim] xiuqo plugins info <name>    [dim]Install:[/dim] xiuqo plugins install <name>")
...
def cmd_search(term: str = "", *, json_output: bool = False) -> None:
    ...
    from xiuqo_cli.plugins_cmd import _console
...
def cmd_info(name: str) -> None:
    ...
    from xiuqo_cli.plugins_cmd import _console, cmd_show
    ...
    rows = [("Repo", entry.repo), ("Subdir", entry.subdir), ("Pinned SHA", entry.sha),
            ("Maintainer", entry.maintainer), ("Requires", f"xiuqo {entry.requires_hermes}" if entry.requires_hermes else ""),
    ...
    if removed is not None:
        ...
        console.print()
    console.print(f"[dim]Install:[/dim]     xiuqo plugins install {entry.name}")
    console.print()
...
def cmd_validate(path: str, as_json: bool = False) -> None:
    ...
    from xiuqo_cli.plugin_validate import validate_plugin_dir
    from xiuqo_cli.plugins_cmd import _console
...
def installed_catalog_state(installed: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    equals the manifest name (``xiuqo-plugin-x`` vs ``x``), so installs are matched through the
