...
One read-only pass answering, BEFORE any mutation: which Xiuqo runtimes run on this machine, how
each is deployed, which ones this update touches, and how each restarts. Every collector is a
side-effect-free probe, so ``xiuqo update --plan`` is safe on a live fleet.
"""
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Xiuqo runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "xiuqo update"
...
def _detect_supervisor_for_pid(pid: int, service_pids: set, windows_service_pids: set | None = None) -> str:
    ...
    with suppress(Exception):
        from xiuqo_cli.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    return _MECHANISM_DESCRIPTIONS.get(mechanism) or (
        f"xiuqo -p {profile} gateway restart" if profile != "default" else "xiuqo gateway restart"
    )
...
def _collect_install_shape(plan: UpdatePlan) -> None:
    with _probe("Install-method probe"):
        from xiuqo_cli.config import detect_install_method, get_managed_system, recommended_update_command_for_method
        ...
        with _probe("Image provenance probe"):
            # See #91277.
            from xiuqo_cli.image_provenance import read_image_provenance
...
def _supervisor_classifier() -> Callable[[int], str]:
    ...
    with _probe("Service-PID probe"):
        from xiuqo_cli.gateway import _get_service_pids
    ...
    with _probe("Windows SCM service-ownership probe"):
        from xiuqo_cli.gateway import find_windows_gateway_services
...
def _collect_gateway_runtimes(plan: UpdatePlan, profile_homes: list, seen: set[int]) -> None:
    ...
    with _probe("Gateway-state inventory"):
        ...
        from xiuqo_cli.update_receipt import _socket_identity
    ...
    with _probe("PID-file gateway inventory"):
        from xiuqo_cli.gateway import find_profile_gateway_processes
...
def _collect_ledger_runtimes(plan: UpdatePlan, seen: set[int]) -> None:
    ...
    (a manual `xiuqo serve --host <ip>` for a remote Desktop, a long-lived `xiuqo dashboard`).
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Xiuqo services: none detected — code swap only.")
        return
...
def _serve_unit_matches_profile(profile: str, unit: object) -> bool:
    """Does *unit* name a ``xiuqo-serve*``/``xiuqo-dashboard*`` unit for *profile*? (OWN vocabulary;
    the gateway's ``xiuqo-gateway*`` names never cover serve/dashboard runtimes.)

    Exact names only — ``work`` must not claim ``xiuqo-serve-workbench`` — and a scope prefix
    (``user/xiuqo-serve``) is tolerated because the restart phase records scope-qualified identities in
    some lists. See #100479.
    """
    name = str(unit).removesuffix(".service").rsplit("/", 1)[-1]
    suffix = "" if profile == "default" else f"-{profile}"
    return name in {f"xiuqo-serve{suffix}", f"xiuqo-dashboard{suffix}"}
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    Never substring-match: ``foo`` must not claim ``xiuqo-gateway-foobar.service``.
    Launchd labels are ``ai.xiuqo.gateway`` / ``ai.xiuqo.gateway-<profile>`` — they do
    not contain the substring ``xiuqo-gateway``, so a successful macOS kickstart must
    still credit the planned default gateway. A scope prefix (``user/xiuqo-gateway``,
    ``gui/501/ai.xiuqo.gateway``) is stripped the same way serve units are.
    """
    name = str(service).removesuffix(".service").rsplit("/", 1)[-1]
    if profile == "default":
        return name in {"xiuqo-gateway", "ai.xiuqo.gateway", "gateway", "gateway-default"}
    return name in {f"xiuqo-gateway-{profile}", f"ai.xiuqo.gateway-{profile}", f"gateway-{profile}"}
...
def match_runtime_outcomes(
    ...
    See #91277.
    They never borrow the gateway's outcome: ``relaunched_profiles`` and ``xiuqo-gateway*`` name a
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    if any(o.get("kind") not in _SERVE_KINDS for o in missed):
        print("      xiuqo gateway restart                # active profile")
        print("      xiuqo -p <profile> gateway restart   # named profile")
    if any(o.get("kind") in _SERVE_KINDS for o in missed):
        ...
        # See #100479.
        print("      systemctl --user restart xiuqo-serve.service   # unit-managed serve")
        print("      relaunch `xiuqo serve` / `xiuqo dashboard` / the Desktop app")
    return True
...
def record_plan_in_receipt(plan: UpdatePlan) -> None:
    ...
    try:
        import xiuqo_cli.update_receipt as ur
