"""``xiuqo logs`` — view and filter Xiuqo log files.

``xiuqo logs [name] [-n N] [-f] [--level L] [--session S] [--component C] [--since 1h]``;
``xiuqo logs list`` shows the available files.
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print("(Logs are created when Xiuqo runs — try 'xiuqo chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'xiuqo chat' to generate logs)")
