"""Audit log for dashboard-auth events: ``$XIUQO_HOME/logs/dashboard-auth.log``, one JSON object
per line. Token-like fields are stripped before serialisation so refresh tokens / JWTs never
reach disk. Minimal import surface (no ``xiuqo_constants`` at import time) so early-loading
