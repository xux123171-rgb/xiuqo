...
All HttpOnly, ``SameSite=Lax`` unless noted, Path = proxy prefix or /: ``xiuqo_session_at``
(access token; Max-Age = token TTL), ``xiuqo_session_rt`` (rotating refresh token; written only
when the provider returned one, always cleared on logout/expiry), ``xiuqo_session_provider``
(non-secret routing hint so an RT is not handed to the wrong provider), ``xiuqo_session_pkce``
...
cookies set on such a 302, crbug 40508226), ``xiuqo_sso_attempt`` (auto-SSO loop guard, 60 s).
...
SESSION_AT_COOKIE = "xiuqo_session_at"
SESSION_RT_COOKIE = "xiuqo_session_rt"
SESSION_PROVIDER_COOKIE = "xiuqo_session_provider"
PKCE_COOKIE = "xiuqo_session_pkce"
SSO_ATTEMPT_COOKIE = "xiuqo_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    """``Path=/xiuqo`` under a proxy prefix (no leak to sibling apps), else ``/``."""
...
def set_session_cookies(
    ...
    Nous Portal issues a 24h rotating refresh token (xiuqo #37247); a provider that omits it returns
