"""Offline, non-destructive recovery for a damaged Xiuqo session database.
...
from xiuqo_state_common import FTS_STORAGE_VERSION, SCHEMA_VERSION
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    guard keeps this path consistent with ``xiuqo_state_repair._backup_db_file``.
    ...
    ``xiuqo_cli.sqlite_safe_read`` exists to prevent (see #71724). Holding the lock means no connection can
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(prefix="xiuqo-session-recovery-", dir=str(work_root))
    try:
        snapshot_source, copied = _copy_source_bundle(source, Path(temp_dir.name))
        if _source_fingerprint(source) != before:
            raise SessionRecoverySafetyError(
                "The source database bundle changed while it was being copied. Stop every Xiuqo process using this "
                "profile and retry. This includes the interactive `xiuqo` CLI session this command may have been "
                "launched from: a running parent CLI writes session bookkeeping (compression ticks, context "
                "tracking) to state.db in the background and counts as a Xiuqo process even after the gateway is "
                "stopped. Run the recovery from a fresh shell with no `xiuqo` session open, or point --source at an "
...
def _salvage_rowid_bounds(source: sqlite3.Connection, table: str) -> dict[str, Any]:
    ...
    # aggregate lets the planner answer from any covering index (every Xiuqo table has at
...
def _lost_and_found_plausibility_errors(
    ...
    """
    from xiuqo_cli.session_lost_and_found import _EPOCH_LOW, STUB_TITLE_PREFIX
