"""Xiuqo Plugin System — discovers, loads, and manages plugins.

Sources, later overriding earlier on key collision: bundled ``<repo>/plugins/<name>/`` (``memory/``
and ``context_engine/`` have their own discovery), user ``~/.xiuqo/plugins/<name>/``, project
``./.xiuqo/plugins/<name>/`` (opt-in via ``XIUQO_ENABLE_PROJECT_PLUGINS``), and pip packages in
the ``xiuqo_agent.plugins`` entry-point group. A directory plugin needs a ``plugin.yaml`` manifest
...
from xiuqo_constants import get_hermes_home, xiuqo_home_key
from registration_lifecycle import replacement_coordinator
from utils import env_var_enabled
from xiuqo_cli.config import load_config_readonly
from xiuqo_cli.middleware import VALID_MIDDLEWARE
from xiuqo_cli.plugin_capabilities import plugin_capability_granted
from xiuqo_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV, legacy_relay_plugin_keys
# Sibling modules' names are re-exported here (origin) so plugins and tests keep one import path.
from xiuqo_cli.plugins_manifest import (  # noqa: F401 — re-exported
...
from xiuqo_cli.plugins_discovery import (  # noqa: F401 — re-exported
...
from xiuqo_cli.plugins_loader import (
...
from xiuqo_cli.plugins_dispatch import (  # noqa: F401 — re-exported
    DEFAULT_SYSTEM_PROMPT_SECTION_MAX_CHARS, XIUQO_EVENT_NAMESPACE, MAX_SYSTEM_PROMPT_SECTION_CHARS,
...
from xiuqo_cli.plugins_ledger import PluginLedgerMixin, PluginRegistration
from xiuqo_cli.plugins_state import (
...
def get_bundled_plugins_dir() -> Path:
    """Bundled ``plugins/`` dir: ``XIUQO_BUNDLED_PLUGINS`` (Nix wrapper / packaged installs, read-only
    store paths) first, else the in-repo path."""
    env_override = os.getenv("XIUQO_BUNDLED_PLUGINS")
    if env_override:
...
# ``XIUQO_PLUGINS_DEBUG=1`` tees verbose discovery logs to stderr in addition to agent.log. Read
# once at import; tests flip it mid-process via ``_install_plugin_debug_handler(force=True)``.
_PLUGINS_DEBUG = env_var_enabled("XIUQO_PLUGINS_DEBUG")
_DEBUG_HANDLER_INSTALLED = False


def _install_plugin_debug_handler(force: bool = False) -> None:
    """When XIUQO_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG (once per process)."""
    ...
    logger.debug("XIUQO_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled")
...
VALID_HOOKS: Set[str] = {
    ...
    "pre_transcription",
    # Kanban task observers (xiuqo_cli.kanban_db), fired AFTER the DB commit so a slow plugin never
    ...
    # | "idle", result: xiuqo_cli.kanban_db.DispatchResult (spawned, reclaimed, promoted,
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        segments = self._segments(key)
        from xiuqo_cli import config as config_mod
    ...
    def platform_actions(self):
        ...
        from xiuqo_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        """Active profile name (``"default"``, the ``~/.xiuqo/profiles/<name>`` id, or ``"custom"``),
        derived from ``XIUQO_HOME`` — not ``_cli_ref``, which is None outside the interactive CLI —
    ...
    def register_approval_transport(self, name: str, present_fn: Callable) -> None:
        ...
        from xiuqo_cli.approval_transport import RegisteredApprovalTransport
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Xiuqo core) are trusted by default — an override there is a
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (``xiuqo <name> ...``). *setup_fn* receives the argparse
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        """Register a :class:`xiuqo_cli.dashboard_auth.DashboardAuthProvider` for the dashboard
        auth gate (non-loopback bind without ``--insecure``). Wrong type / duplicate name warn and
        are ignored, never raised."""
        from xiuqo_cli.dashboard_auth import DashboardAuthProvider
        from xiuqo_cli.dashboard_auth.registry import register_global_provider, unregister_global_provider
    ...
    def register_auxiliary_task(
        ...
        from xiuqo_cli.main_provider_setup import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        the subscriber count scheduled. Any ``':'`` in the name (``xiuqo:x`` is reserved for core,
        ...
        if ":" in event:
            logger.warning("Plugin '%s' tried to emit namespaced/reserved event '%s' — a plugin may only emit "
                           ...
                           plugin_key, event, plugin_key, XIUQO_EVENT_NAMESPACE)
            raise ValueError(f"Plugin '{plugin_key}' may not emit '{event}': emit only the bare event name; "
                             f"the namespace is forced to '{plugin_key}:' and the '{XIUQO_EVENT_NAMESPACE}:' "
    ...
    def register_skill(
        ...
        Not in ``~/.xiuqo/skills/`` nor ``<available_skills>`` — explicit loads only. Raises
...
_SCOPED_PROVIDER_REGISTRARS: Tuple[Tuple[str, str, str, str, str, str, Dict[str, Any]], ...] = (
    ...
    ("register_secret_source", "secret_source", "agent.secret_sources.registry",
     ...
     "(after ``~/.xiuqo/.env``, before credentials are read) when ``secrets.<name>`` is enabled. The "
...
class PluginManager(PluginLoaderMixin, PluginDispatchMixin, PluginLedgerMixin):
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        self.scope_key = scope_key or xiuqo_home_key()
        ...
        # is keyed per (xiuqo_home, plugin_id) and every inverse is identity-conditional — one
        ...
        # and contributed tool names (so `xiuqo plugins list` still attributes them).
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if env_var_enabled("XIUQO_SAFE_MODE"):
                logger.info("XIUQO_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            load_hermes_dotenv(xiuqo_home=home)
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning("Removed Xiuqo plugin %s is still listed in plugins.enabled; "
    ...
    def _refresh_plugin_compat_report(self, manifests: List[PluginManifest]) -> None:
        """Refresh XIUQO_HOME/.plugin-compat-report.json from this discovery pass (xiuqo_cli.plugin_compat).
        ...
        """
        try:
            from xiuqo_cli.plugin_compat import compat_report
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        if _env_enabled("XIUQO_SAFE_MODE"):
            return False
        ...
        for lookup_key, manifest in {manifest_key(m): m for m in self._collect_directory_manifests()}.items():
            ...
            try:  # lazy: this is a startup probe, keep agent_plugins unimported unless needed
                from xiuqo_cli.agent_plugins import _discover_mcp
...
# Resolved Xiuqo home -> PluginManager. A process can switch profiles via
...
def _plugin_home_key() -> Path:
    """Resolved active Xiuqo home — the key for per-profile plugin managers (plugins capture the
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Xiuqo profile/home (cached per resolved home; a
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=xiuqo_home_key(current_home))
...
def _reset_plugin_managers_for_tests() -> None:
    ...
    try:
        from xiuqo_cli.dashboard_auth.registry import clear_providers
        clear_providers()
    except Exception:
...
def _get_pre_tool_call_directive_details(
    ...
    from xiuqo_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    # async hook under a running loop reads the default XIUQO_HOME and get_secret raises.
    threading.Thread(target=contextvars.copy_context().run, args=(_runner,),
                     name="xiuqo-plugin-command-await", daemon=True).start()
...
def get_plugin_toolsets() -> List[tuple]:
    """Plugin toolsets as ``(key, label, description)`` tuples for the ``xiuqo tools`` TUI."""
    manager = get_plugin_manager()
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``xiuqo:<event>``) to subscriber callbacks in
...
_PLUGIN_COMPAT_LAZY = {
    'CAPABILITY_REGISTRY': ('xiuqo_cli.plugin_capabilities', 'CAPABILITY_REGISTRY'),
    'ENTRY_POINT_CAPABILITIES_GROUP': ('xiuqo_cli.plugins_discovery', 'ENTRY_POINT_CAPABILITIES_GROUP'),
    'LEGACY_RELAY_PLUGIN_KEYS': ('xiuqo_cli.relay_plugin_cutover', 'LEGACY_RELAY_PLUGIN_KEYS'),
    'MAX_SYSTEM_PROMPT_SECTIONS': ('xiuqo_cli.plugins_dispatch', 'MAX_SYSTEM_PROMPT_SECTIONS'),
    'OBSERVER_SCHEMA_VERSION': ('xiuqo_cli.middleware', 'OBSERVER_SCHEMA_VERSION'),
    'VALID_CAPABILITY_IDS': ('xiuqo_cli.plugin_capabilities', 'VALID_CAPABILITY_IDS'),
    'cfg_get': ('xiuqo_cli.config', 'cfg_get'),
    'fast_safe_load': ('utils', 'fast_safe_load'),
    'format_system_prompt_section': ('xiuqo_cli.plugins_dispatch', 'format_system_prompt_section'),
    'reset_hermes_home_override': ('xiuqo_constants', 'reset_hermes_home_override'),
    'set_hermes_home_override': ('xiuqo_constants', 'set_hermes_home_override'),
}
