...
:mod:`xiuqo_cli.runtime_provider`; origin-internal collaborators
...
from xiuqo_cli.providers import custom_provider_aliases, custom_provider_slug
from agent.secret_scope import get_secret_str
from utils import base_url_hostname

logger = logging.getLogger("xiuqo_cli.runtime_provider")

_LLAMACPP_ALIASES = ("llamacpp", "llama.cpp", "llama-cpp")


def _rp():
    """Origin module, late-bound so test patches on ``xiuqo_cli.runtime_provider.*`` apply."""
    import xiuqo_cli.runtime_provider as origin
    return origin
...
def canonical_custom_identity(*, base_url: Optional[str] = None, config_provider: Optional[str] = None,
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("XIUQO_INFERENCE_PROVIDER", "").strip()
...
def _resolve_named_custom_runtime(*, requested_provider: str, explicit_api_key: Optional[str] = None,
    ...
    if family is not None and not custom_provider.get("api_mode"):
        from xiuqo_cli.models import normalize_opencode_base_url, opencode_model_api_mode
