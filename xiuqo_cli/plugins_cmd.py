"""``xiuqo plugins`` CLI subcommand — install, update, remove, and list plugins."""
...
from xiuqo_cli._subprocess_compat import noninteractive_git_env
from xiuqo_cli.cli_output import line_input
from xiuqo_cli.config import cfg_get
from xiuqo_cli.plugin_capabilities import _child_dict
...
def _read_manifest(plugin_dir: Path) -> dict:
    ...
    if manifest_file is None:
        if not _has_portable_manifest(plugin_dir):
            return {}
        try:
            from xiuqo_cli.agent_plugins import read_agent_plugin_manifest
...
def _missing_env_specs(manifest: dict) -> list[dict]:
    ...
    normalised to dicts; nameless entries dropped) whose variable is unset in ``~/.xiuqo/.env``."""
    env_specs = [
...
def _print_python_dependencies(manifest: dict, console) -> None:
    """Print declared ``python_dependencies`` with an install hint — Xiuqo never auto-installs
...
def _check_manifest_version(manifest: dict, plugin_name: str) -> None:
    ...
    if mv_int > _SUPPORTED_MANIFEST_VERSION:
        ...
        raise PluginOperationError(
            ...
            f"Run {recommended_update_command()} to update Xiuqo.",
        ) from None
    ...
    with tempfile.TemporaryDirectory(prefix=".install-", dir=plugins_dir) as tmp:
        ...
        if target.exists() and not force:
            raise PluginOperationError(
                ...
                f"or run `xiuqo plugins update {plugin_name}`.")
...
def cmd_install(
    ...
    a ``.xiuqo-catalog.json`` sidecar; URLs/shorthand are flagged as custom (unreviewed) sources. Every
    ...
    *enable* None prompts "Enable now? [y/N]"; True/False skip the prompt.
    """
    from xiuqo_cli import plugins_cmd_catalog as catalog
    console = _console()
    entry = None
    if catalog.looks_like_catalog_name(identifier):
        ...
        console.print("[yellow]Warning:[/yellow] custom (unreviewed) source — not from the Xiuqo catalog.")
    if allow_removed:
        console.print(
    ...
    if not _looks_like_plugin_dir(target):
        console.print(
            ...
            f"plugin.json, or __init__.py. It may not be a valid Xiuqo plugin.")
    ...
    if enable:
        ...
        console.print(
            ...
            f"Run `xiuqo plugins enable {installed_name}` to activate.[/dim]")
    ...
    console.print("[dim]  xiuqo gateway restart[/dim]")
    console.print()
...
def cmd_update(name: str) -> None:
    ...
    try:
        output = _pull_plugin_update(
            ...
            lambda rec: (
                ...
                f"`xiuqo plugins install {escape(str(rec.get('source', '<source>')))} --force "
    ...
    if declared_caps:
        from xiuqo_cli.plugin_capabilities import declared_set_changed, pending_capabilities
...
def _rescan_after_update(target: Path, name: str, console) -> None:
    ...
    if scan_result.verdict == "dangerous":
        ...
        console.print(
            ...
            f"findings, then re-enable with `xiuqo plugins enable {name}` "
            f"if you trust them.")
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``xiuqo setup`` / ``xiuqo plugins disable basic`` can park it there while
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    """
    from xiuqo_cli.relay_plugin_cutover import LEGACY_RELAY_PLUGIN_KEYS, RELAY_PLUGINS_CONFIG_ENV
    console = _console()
    ...
    def _refuse_legacy_relay(plugin: str) -> None:
        if plugin in LEGACY_RELAY_PLUGIN_KEYS:
            _fail(console, (
                ...
                f"by Xiuqo core; configure {RELAY_PLUGINS_CONFIG_ENV} instead."))
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from xiuqo_cli.plugin_capabilities import parse_declared_capabilities
...
def _declared_capabilities_for_key(key: str) -> list:
    ...
    if entry[3] == "entrypoint":
        from xiuqo_cli.plugins import discover_entrypoint_manifests
...
def _run_capability_consent(console, plugin_id: str, declared: list, *, context: str = "install") -> bool:
    ...
    """
    from xiuqo_cli.plugin_capabilities import CAPABILITY_REGISTRY, pending_capabilities, record_consent
    ...
    if not _is_tty():
        console.print(
            ...
            f"`xiuqo plugins capabilities {plugin_id}` to review and "
            f"`xiuqo plugins enable {plugin_id}` to grant interactively.")
        return False
    ...
    console.print(
        ...
        f"`xiuqo plugins enable {plugin_id}` to grant later.[/dim]")
    return False


def cmd_capabilities(name: Optional[str] = None) -> None:
    """``xiuqo plugins capabilities [<id>]`` — declared vs granted."""
    from xiuqo_cli.plugin_capabilities import (
        CAPABILITY_REGISTRY,
...
def _resolve_tool_override_grant(console, key: str, allow_tool_override: Optional[bool]) -> None:
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`xiuqo plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]")
...
def _discover_all_plugins() -> list:
    ...
    from xiuqo_cli.plugins import discover_entrypoint_manifests, get_bundled_plugins_dir
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``xiuqo plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] xiuqo plugins install owner/repo")
        return
    ...
    for line in removed_lines:
        console.print(line)
    console.print()
    console.print("[dim]Compact view:[/dim] xiuqo plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] xiuqo plugins")
    console.print("[dim]Enable/disable:[/dim] xiuqo plugins enable/disable <name>")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        ...
        pass
    try:
        from xiuqo_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        _fail(console, "[dim]List installed plugins:[/dim] xiuqo plugins list")
...
def _run_composite_ui(curses, plugin_keys, plugin_labels, plugin_selected, disabled, categories, console):
    ...
    from xiuqo_cli.curses_ui import _addnstr, flush_stdin
...
def dashboard_install_plugin(
    ...
    if catalog_name:
        ...
        if entry is None:
            return {"ok": False, "error": f"'{catalog_name}' is not in the Xiuqo plugin catalog."}
        identifier = entry.install_identifier
    else:
        warnings.append("Custom (unreviewed) source — not from the Xiuqo catalog.")
    try:
...
def _get_plugin_toolset_key(name: str) -> Optional[str]:
    ...
    def _from_manifest_on_disk() -> Optional[str]:
        from xiuqo_cli.plugins import get_bundled_plugins_dir
        return next((
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.xiuqo/plugins/<name>`` if it exists."""
    try:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.xiuqo/plugins/<name>``."""
    from xiuqo_cli import plugins_cmd_catalog as catalog
    ...
    try:
        ...
        msg = _pull_plugin_update(
            ...
            lambda rec: (
                ...
                f"run `xiuqo plugins install {rec.get('source', '<source>')} --force "
...
def _run_plugin_git(
    ...
    if auth_url:
        from xiuqo_cli.git_credentials import with_git_auth
...
def _autostash_dirty_tree(git_exe: str, target: Path) -> tuple[bool, str]:
    ...
    push = _run_plugin_git(
        git_exe, target, "stash", "push", "--include-untracked", "-m", "xiuqo-plugin-update-autostash")
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    marketplace now succeeds when its checkout has local changes"), and the same autostash approach ``xiuqo
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.xiuqo/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from xiuqo_cli.plugin_dev import doctor_plugin
...
def _catalog():
    from xiuqo_cli import plugins_cmd_catalog
    return plugins_cmd_catalog


def _action_pack(args):
    from xiuqo_cli.plugin_packs import pack_command
    pack_command(args)


def cmd_compat(args: Any | None = None) -> None:
    """``xiuqo plugins compat`` — which installed plugins import paths scheduled for removal, and where."""
    import sys
    from pathlib import Path
    from xiuqo_cli.plugin_compat import (
...
def plugins_command(args) -> None:
    """Dispatch xiuqo plugins subcommands."""
