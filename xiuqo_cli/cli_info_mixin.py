...
from xiuqo_constants import is_termux as _is_termux_environment
from rich.markup import escape as _escape
from utils import base_url_hostname

from xiuqo_cli.cli_modal_mixin import _gated_confirm
...
class CLIInfoMixin:
    ...
    def show_banner(self):
        ...
        from xiuqo_cli.banner import build_welcome_banner
        self.console.clear()
        ctx_len = None
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Xiuqo needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        from xiuqo_cli.model_switch import is_nous_hermes_non_agentic
        if is_nous_hermes_non_agentic(getattr(self, "model", "") or ""):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Hermes 3 & 4 models are NOT agentic and are not "
                "designed for use with Xiuqo Agent.[/]")
            self._console_print(
        ...
        # `xiuqo skills trust`. Never raises.
        try:
            ...
            if _proj_dirs:
                ...
                if _untrusted is not None:
                    ...
                    self._console_print(
                        ...
                        f"loaded — run `xiuqo skills trust` to enable them.[/]")
        except Exception:
    ...
    def show_help(self, arg: str = ""):
        ...
        from xiuqo_cli.commands import COMMANDS_BY_CATEGORY, HELP_SESSION_SUBGROUPS
        ...
        try:
            from xiuqo_cli.skin_engine import get_active_help_header
    ...
    def handle_bang_shell(self, text: str) -> bool:
        ...
        (tests/xiuqo_cli/test_bang_shell_mode.py). Returns False when the text is not a bang command or
        bang mode is disabled for this context (gateway/cron), so the caller routes normally.
        """
        from cli import _rich_text_from_ansi
        from xiuqo_cli.bang_shell import (
    ...
    def _print_random_tip(self) -> None:
        ...
        try:
            from xiuqo_cli.tips import get_random_tip
            _tip = get_random_tip()
            try:
    ...
    def _toggle_verbose(self):
        ...
        if getattr(self, "_focus_view_enabled", False):
            ...
            try:
                from xiuqo_cli.focus_view import FOCUS_CONFIG_KEY
    ...
    def _usage_reset(self, force: bool = False):
        ...
        if str(self._agent_or_self("provider") or "").strip().lower() != "openai-codex":
            ...
            print("  Switch with `/model` or `xiuqo auth` first.")
            return
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from xiuqo_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.xiuqo/skills/ and queue a note for the next user turn.
