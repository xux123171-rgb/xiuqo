...
Pure leaf: imports nothing from ``xiuqo_cli.auth`` so the per-provider modules
...
# httpx is imported lazily (~30ms) because xiuqo_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("xiuqo_cli.auth.httpx.Client")
...
DEFAULT_NOUS_CLIENT_ID = "xiuqo-cli"
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from xiuqo_cli import __version__ as _XIUQO_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _XIUQO_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"xiuqo-cli/{_XIUQO_CLI_VERSION}"
