...
from xiuqo_cli.platforms import PLATFORMS
from xiuqo_cli.toolset_scope import toolset_allowed_for_platform

_NO_TOOLS = "the agent will have no tools on this platform. Run `xiuqo tools` to reconfigure."


def _platform_default_toolset(platform: object) -> str:
    info = PLATFORMS.get(platform)
    return info.default_toolset if info is not None else f"xiuqo-{platform}"
...
def _platform_default_is_valid(
    ...
    # xiuqo-<platform> name is not in TOOLSETS.
    try:
...
def validate_platform_toolsets(
    ...
    Reports: a toolset name ``is_valid_toolset`` rejects (suggesting ``xiuqo-<platform>`` when that
    ...
    for platform, raw in platform_toolsets.items():
        ...
        if not isinstance(raw, list):
            ...
            warnings.append(
                ...
                f"{fallback_detail}. Run `xiuqo tools` to configure explicitly.")
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `xiuqo tools` to reconfigure.")
    return warnings
