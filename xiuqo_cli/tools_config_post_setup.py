"""Post-setup install hooks and installed-state predicates for `xiuqo tools` provider rows."""
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Xiuqo-managed ``$XIUQO_HOME/bin`` copy does."""
...
def _post_setup_lightpanda() -> None:
    ...
    if lightpanda_bin:
        ...
        if os.name == "nt":
            _print_info("    Lightpanda has no native Windows build; run Xiuqo under WSL2.")
...
def _post_setup_agent_browser(post_setup_key: str) -> None:
    ...
    global/Xiuqo-managed install), so there is no ``npm install`` step here."""
    ...
    try:
        ...
        # PATH/node_modules probes above. Mirror the rung xiuqo_cli.doctor uses so this probe can't diverge
        ...
        # agent-browser is no longer a root package.json dependency (#43564) — it resolves lazily via npx
        # (or a global/Xiuqo-managed install) instead of a local `npm install`, so there's no node_modules/
    ...
    # Reuse the runtime resolution cascade (PATH -> Homebrew/Xiuqo-managed node -> npx) rather than
    # a bare shutil.which — Xiuqo-managed-Node-only setups resolve agent-browser/npx only that way.
    try:
...
_PIP_POST_SETUP_HOOKS: dict = {
    "faster_whisper": _pip_hook(
        ...
        on_install=("Model sizes: tiny, base (default), small, medium, large-v3",
                    "Change via stt.local.model in ~/.xiuqo/config.yaml")),
    "kittentts": _pip_hook(
    ...
    "piper": _pip_hook(
        ...
        always=("Default voice: en_US-lessac-medium (downloaded on first TTS call)",
                ...
                "Switch voices by setting tts.piper.voice in ~/.xiuqo/config.yaml")),
    "ddgs": _pip_hook(
...
def _post_setup_spotify() -> None:
    # Full `xiuqo auth spotify` flow: no client_id yet → interactive wizard (persists to ~/.xiuqo/.env)
    # then PKCE; existing app → OAuth only.
    from types import SimpleNamespace
    try:
        from xiuqo_cli.auth import login_spotify_command
    except Exception as exc:
        _print_warning(f"    Could not load Spotify auth: {exc}")
        _info_lines("Run manually: xiuqo auth spotify")
        return
    ...
    try:
        ...
        _info_lines("Run later: xiuqo auth spotify")
    except Exception as exc:
...
def _post_setup_langfuse() -> None:
    ...
    try:
        from xiuqo_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
        ...
        _info_lines("Run manually: xiuqo plugins enable observability/langfuse")
    _info_lines("Restart Xiuqo for tracing to take effect.", "Verify: xiuqo plugins list")
...
def _post_setup_xai_grok() -> None:
    ...
    try:
        from xiuqo_cli.setup import prompt_choice, prompt as _setup_prompt
        from xiuqo_cli.setup_tts import _run_xai_oauth_login_from_setup
        from xiuqo_cli.config import save_env_value
    except Exception as exc:
        _print_warning(f"    Could not load setup helpers: {exc}")
        _info_lines("Run later: xiuqo auth add xai-oauth   (or set XAI_API_KEY)")
        return
    ...
    idx = prompt_choice(
        ...
        choices=["Sign in with xAI Grok OAuth (SuperGrok / Premium+) — browser login",
                 ...
                 "Skip — configure later via `xiuqo auth add xai-oauth`"])
    if idx == 0:
        if _run_xai_oauth_login_from_setup():
            _print_success("    Logged in — xAI will use these OAuth credentials")
        else:
            _print_warning("    xAI Grok OAuth login did not complete. Run later: xiuqo auth add xai-oauth")
    elif idx == 1:
        ...
        if api_key:
            ...
            _print_warning("    No API key provided. Run later: xiuqo auth add xai-oauth")
    else:
...
def run_post_setup_command(args) -> int:
    """``xiuqo tools post-setup <key>`` — non-interactive runner the dashboard spawns so the GUI can drive
    backend setup without re-implementing install logic. Exit code: 0 ok, 2 unknown key."""
    key = getattr(args, "post_setup_key", None)
    if not key:
        _print_error("Usage: xiuqo tools post-setup <key>")
        return 2
...
# Python deps installed via ``xiuqo tools`` aren't in the managed runtime's locked ``all`` sync, so a
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``xiuqo tools`` Python dependencies present in this runtime."""
    return [
...
def _agent_browser_installed() -> bool:
    ...
    *and* the Chromium build it drives (or the Lightpanda engine, which needs no Chromium), so "Run
    ...
    from xiuqo_cli.nous_subscription import _local_browser_runnable
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from xiuqo_cli.nous_subscription import _has_agent_browser
