"""xiuqo memory setup|status — configure memory provider plugins."""
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``xiuqo memory setup`` — if the update-time refresh only reinstalled the declared ``hindsight-client``,
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    no-ops when nothing drifted) — how ``xiuqo update`` heals a provider after a venv rebuild.
    ...
    satisfied ranges. This is how ``xiuqo update`` heals the active memory provider after a venv
    ...
    # install_specs routes to the durable target on sealed hosted images (XIUQO_LAZY_INSTALL_TARGET)
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'xiuqo memory setup' to see available providers.\n")
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.xiuqo/plugins/ and try again.\n")
        return
...
def _write_env_vars(
    env_writes: dict, xiuqo_home: str | os.PathLike[str] | None = None) -> None:
    """Persist memory-provider env vars through the canonical ``.env`` writer.

    ``save_env_value`` applies the shared gate (name regex, ``LD_PRELOAD``/``PYTHONPATH``/``XIUQO_HOME``
    denylist, CR/LF stripping, atomic 0o600 writes). ``ValueError`` is reported and skipped so one
    bad key doesn't sink the batch; filesystem errors propagate. ``xiuqo_home`` is applied via the
    ...
    token = set_hermes_home_override(xiuqo_home) if xiuqo_home is not None else None
    try:
                ...
                if required_fields:
                    ...
                    for f in required_fields:
                        ...
                        if url and not is_set:
                            line += f"  → {url}"
                        print(line)
                print("  Note: systemd/gateway services do not inherit ~/.xiuqo/.env —")
                print("        set any variables above in the service environment.")
        else:
            print("\n  Plugin:    NOT installed ✗")
            print(f"  Install the '{provider_name}' memory plugin to ~/.xiuqo/plugins/")
