...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None) -> list[int]:
    ...
    from xiuqo_cli.dashboard_procs import _scan_dashboard_processes
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    for candidate in ("dashboard", "serve"):
        patterns = (f"xiuqo {candidate}", f"xiuqo_cli.main {candidate}", f"xiuqo_cli/main.py {candidate}")
...
def _restart_managed_dashboard_service(reason: str, unit: str = _DASHBOARD_SYSTEMD_UNIT) -> bool:
    ...
    # User manager first (Xiuqo installs Linux services in the user scope by
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    """The systemd service unit name *pid* belongs to (``xiuqo-serve.service``), or None.
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Respawn manually-started dashboards after ``xiuqo update``, detached, logging to
...
class _UpdateOutputStream:
    """stdout/stderr wrapper for ``xiuqo update``: mirrors to ``logs/update.log`` and, once the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    try:
        ...
        # xiuqo_cli.config.get_hermes_home to simulate setup failure.
        from xiuqo_cli.config import get_hermes_home as _get_hermes_home
        ...
        log_file.write(f"\n=== xiuqo update started {_dt.datetime.now().isoformat(timespec='seconds')} ===\n")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No xiuqo dashboard or serve processes running.")
        return 0

    print(f"{len(live)} xiuqo dashboard/serve process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from xiuqo_cli.web_server import should_require_dashboard_auth
    ...
    print("    [2] OAuth via Nous Portal (run `xiuqo dashboard register`)\n    [3] Cancel\n")
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    xiuqo dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "XIUQO_DASHBOARD_OAUTH_CLIENT_ID into ~/.xiuqo/.env for you.\n"
    ...
    try:
        ...
        from xiuqo_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
        cfg = load_config()
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from xiuqo_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under the account's $HOME/.xiuqo/
    # desktop-ssh, independent of XIUQO_HOME and the active profile. Anchor
    ...
    # See #69551.
    token_root = Path.home() / ".xiuqo" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    A standalone ``xiuqo dashboard`` inheriting that ``XIUQO_WEB_DIST`` would
...
def _route_named_profile_dashboard(
    ...
    (XIUQO_DESKTOP=1) stay per-profile. Returns normally when no routing applies.
    """
    try:
    ...
    if (
        ...
        or os.environ.get("XIUQO_DESKTOP") == "1"
    ):
        return
    ...
    reexec_argv = [
        sys.executable, "-m", "xiuqo_cli.main",
        "-p", "default",
    ...
    # XIUQO_HOME is pinned to the machine root below — the factory must not
    # re-inject a profile home.
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=False)
    # Pin the child to the machine ROOT, resolved explicitly rather than by
    # dropping XIUQO_HOME: in the Docker layout the root is /opt/data, and an
    # unset XIUQO_HOME would fall back to $HOME/.xiuqo = /opt/data/.xiuqo — an
    ...
    try:
        ...
        env["XIUQO_HOME"] = str(get_default_hermes_root())
    except Exception:
        env.pop("XIUQO_HOME", None)  # prior behaviour rather than blocking the reroute
...
def _resolve_dashboard_web_dist(args, _headless_backend: bool) -> None:
    ...
    ``serve`` sets XIUQO_SERVE_HEADLESS so mount_spa() stays off. Otherwise build
    unless XIUQO_WEB_DIST / --skip-build promise a dist — then verify index.html
    (else the server serves 404s). --skip-build on the default location gets ONE
    recovery build; a caller-managed XIUQO_WEB_DIST can't be populated.
    """
    from xiuqo_cli.main import PROJECT_ROOT
    from xiuqo_cli.main_web_build import _build_web_ui
    skip_build = getattr(args, "skip_build", False)
    if _headless_backend:
        os.environ["XIUQO_SERVE_HEADLESS"] = "1"  # set before web_server import
    elif "XIUQO_WEB_DIST" not in os.environ and not skip_build:
        ...
        _dist_root = (
            ...
            Path(os.environ["XIUQO_WEB_DIST"])
            if "XIUQO_WEB_DIST" in os.environ
            else PROJECT_ROOT / "xiuqo_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            # Only the default dist location is recoverable (desktop launches with
            # --build-mode skip after a wipe of web_dist); a custom XIUQO_WEB_DIST
            ...
            _recoverable = "XIUQO_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # XIUQO_WEB_DIST without --skip-build: the env var points at a
        # caller-managed dist, so validate it like the --skip-build branch.
        # XIUQO_WEB_DIST is set without --skip-build: the build is skipped (the env var points at a
        ...
        # path).
        _dist_root = Path(os.environ["XIUQO_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ XIUQO_WEB_DIST is set but no web dist found at: {_dist_root}")
            print(_PRE_BUILD_HINT)
            print("  Or unset XIUQO_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # web_server reads XIUQO_WEB_DIST raw at import (no expanduser), so a
        # validated "~/dist" would otherwise pass here and still 404 there.
        os.environ["XIUQO_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from XIUQO_WEB_DIST: {_dist_root}")
