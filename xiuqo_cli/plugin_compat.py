...
The decomposition (PR #102117) moved most of Xiuqo's internals into ``<stem>_<topic>`` sibling modules.
...
Surfaces that read from here: the CLI banner, ``xiuqo plugins compat``, ``xiuqo doctor``, the post-update
...
def disable_reason(manifest, *, today: Optional[_dt.date] = None) -> Optional[str]:
    ...
    if not hits:
        return None
    return (f"uses {len(hits)} import path(s) removed on {COMPAT_REMOVAL}; run `xiuqo plugins compat` "
...
def summary_lines(report: Dict[str, List[Hit]], *, today: Optional[_dt.date] = None) -> List[str]:
    ...
    if removal_in_effect(today) and allow_deprecated_imports():
        ...
        tail = "Update the plugin(s); the old paths no longer exist. Details: xiuqo plugins compat"
    elif removal_in_effect(today):
        head = (f"{n} plugin{'s' if n != 1 else ''} DISABLED: they import paths removed on {COMPAT_REMOVAL}: {names}")
        tail = f"Update the plugin(s) or set plugins.{ALLOW_KEY}: true to force-load. Details: xiuqo plugins compat"
    else:
        ...
        tail = "Check for plugin updates or notify the author before then. Details: xiuqo plugins compat"
    return [head, tail]
...
def warn_once(facade: str, name: str, target_module: str, target_name: str) -> None:
    ...
    msg = (f"xiuqo plugin compat: `{facade}.{name}` moved to `{new}`. The old path is kept only for external "
