...
Toolsets = explicit --toolsets, else the user's "cli" toolsets from `xiuqo tools`. Rules /
memory / AGENTS.md / preloaded skills = same as a normal chat turn. Approvals are auto-bypassed
(XIUQO_YOLO_MODE=1). Model/provider mirror `xiuqo chat`: both optional; only --model → auto-detect
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"xiuqo -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in _ALL_TOOLSETS for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "xiuqo -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"xiuqo -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "xiuqo -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )
    if not valid:
        return None, "xiuqo -z: --toolsets did not contain any valid toolsets.\n"
    return valid, None
...
def run_oneshot(
    ...
    Model/provider fall back to ``XIUQO_INFERENCE_MODEL`` and config.yaml. ``usage_file`` gets a
    ...
    env_model_early = os.getenv("XIUQO_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "xiuqo -z: --provider requires --model (or XIUQO_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"xiuqo -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        if result.get("failed") or result.get("partial"):
            return 2
        real_stderr.write("xiuqo -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
    return 0
...
def _resolve_model_and_provider(cfg: dict, model: Optional[str], provider: Optional[str]) -> _ModelChoice:
    ...
    env_model = os.getenv("XIUQO_INFERENCE_MODEL", "").strip()
    ...
    try:
        from xiuqo_cli import model_switch as _ms
    ...
    if direct is None:
        ...
        current_provider = cfg_provider or os.getenv("XIUQO_INFERENCE_PROVIDER", "").strip().lower() or "auto"
...
def _load_resume_target(session_db, resume: Optional[str]) -> tuple[Optional[str], list, Optional[dict]]:
    ...
    nothing but is recorded under the requested session — ``xiuqo -z "hello" -c <title>
...
def _apply_stored_session_runtime(
    ...
    if explicit_model:
        return choice
    from xiuqo_cli.cli_model_switch_mixin import stored_session_route
...
def _run_agent(
    ...
    from xiuqo_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        agent = AIAgent(
            ...
            # XIUQO_INTERACTIVE (never set), hook approval via XIUQO_ACCEPT_HOOKS=1, dangerous
            # commands via XIUQO_YOLO_MODE=1, skill secret capture degrades gracefully.
