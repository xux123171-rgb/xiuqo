...
resolved through the module object so test patches on ``xiuqo_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from xiuqo_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'xiuqo setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'xiuqo setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'xiuqo tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("xiuqo setup", "          Re-run the full wizard"), ("xiuqo setup model", "    Change model/provider"),
    ("xiuqo setup terminal", " Change terminal backend"), ("xiuqo setup gateway", "  Configure messaging"),
    ("xiuqo setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("xiuqo config", "         View current settings"), ("xiuqo config edit", "    Open config in your editor"),
    ("xiuqo config set <key> <value>", ""))
_READY_ROWS = (
    ("xiuqo", "              Start chatting"), ("xiuqo gateway", "      Start messaging gateway"),
    ("xiuqo doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'xiuqo setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `xiuqo tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'xiuqo setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via xiuqo auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, xiuqo_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — Xiuqo cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    xiuqo model            (pick any provider/model)",
              "    xiuqo setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'xiuqo setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {xiuqo_home}/cron/, sessions/, logs/")):
...
import xiuqo_cli.setup as _setup  # noqa: E402  (bottom: xiuqo_cli.setup imports this module)
