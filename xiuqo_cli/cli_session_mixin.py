...
def _reset_model_to_config_default(cli, silent: bool) -> None:
    ...
    if not _config_model or _config_model == getattr(cli, "model", None):
        return
    try:
        from xiuqo_cli.model_switch import switch_model as _switch_model
...
class CLISessionMixin:
    ...
    def _show_session_status(self):
        ...
        from xiuqo_cli.status_report import build_status_fields, status_lines
        session_meta = {}
        if self._session_db:
        ...
        lines = ["Xiuqo CLI Status", "", *status_lines(fields, "session_id", "path", "title", "model")]
        try:
            from agent.i18n import t
            from xiuqo_cli.auth import resolve_provider
            from xiuqo_cli.anon_auth import guest_carries_inference
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from xiuqo_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from xiuqo_cli.timefmt import relative_time as _relative_time
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            _cli_visible_print(f"\n  [Xiuqo #{visible_index}]{_ts_suffix(msg)}")
    ...
    def _notify_session_boundary(self, event_type: str) -> None:
        ...
        with contextlib.suppress(Exception):
            from xiuqo_cli.lifecycle import finalize_session, invoke_hook
            ...
            if self._session_db:
                with contextlib.suppress(Exception):
                    ...
                    self._session_db.create_session(
                        ...
                        source=os.environ.get("XIUQO_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self, cmd: str = "/save"):
        ...
        """
        from cli import datetime
        from xiuqo_cli.session_export import (
        ...
        if filename:
            ...
            path = saved_dir / f"xiuqo_conversation_{timestamp}.{fmt}"
        ...
        try:
            ...
            # tools/subprocesses on this thread resolve XIUQO_SESSION_ID to the child id after an
            # out-of-place rotation (idempotent when no rotation happened).
            if self.session_id:
                print(f"       Resume the live session with: xiuqo --resume {self.session_id}")
        except Exception as e:
    ...
    def _write_terminal_breadcrumb(self) -> None:
        """Record this terminal's live session for bare ``xiuqo -c``. Called whenever
        ...
        with contextlib.suppress(Exception):
            from xiuqo_cli.terminal_breadcrumbs import write_breadcrumb
    ...
    def _toggle_yolo(self):
        ...
        ``XIUQO_YOLO_MODE``: that env var is frozen into ``tools.approval._YOLO_MODE_FROZEN``
        ...
        if _YOLO_MODE_FROZEN:
            _cprint(
                ...
                " for this process (started with --yolo / XIUQO_YOLO_MODE)."
    ...
    def _print_exit_summary(self, clear_screen: bool = True):
        ...
        if not msg_count:
            try:
                from xiuqo_cli.skin_engine import get_active_goodbye
        ...
        # the hint ("default"/"custom" use the standard XIUQO_HOME).
        try:
        ...
        print(f"  xiuqo --resume {self.session_id}{profile_flag}")
        if session_title:
            print(f"  xiuqo -c \"{session_title}\"{profile_flag}")
        print()
