...
def _root() -> Path:
    ...
    # XIUQO_HOME) cannot move the reader away from the helper's token.
...
def process_state(pid: int, creation_time_ns: int, xiuqo_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(xiuqo_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is the xiuqo exe or (normally) the base Python, whose path varies by venv/uv
    ...
    launches_module = (
        argv[1:3] == ["-m", "xiuqo_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "xiuqo_cli.main" in argv[2]))
...
def terminate_owned(pid: int, creation_time_ns: int, xiuqo_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, xiuqo_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    process that both owns the port and is the one we lock. xiuqo_cli's parent is prepended
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the xiuqo_cli package")
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    bootstrap = (
        ...
        "runpy.run_module('xiuqo_cli.main',run_name='__main__',alter_sys=True)")
...
def inspect_hermes(xiuqo_path: str) -> dict[str, Any]:
    path = os.path.abspath(xiuqo_path)
    if not os.path.isabs(xiuqo_path) or not os.path.isfile(path):
        raise ValueError("Xiuqo path is not an executable file")
