...
Mixed into :class:`xiuqo_cli.plugins.PluginManager`. ``_resolve_hook_callback_timeout`` stays on
...
from xiuqo_cli.middleware import OBSERVER_SCHEMA_VERSION
...
PLUGIN_SECTIONS_START = "<!-- xiuqo-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- xiuqo-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- xiuqo-plugin-section-chars:{len(content)} -->\n\n{content}")
...
# Reserved event namespace prefix — only core may publish ``xiuqo:<event>``.
XIUQO_EVENT_NAMESPACE = "xiuqo"
...
class PluginDispatchMixin:
    ...
    def _invoke_hook_callback(callback: Callable, payload: Dict[str, Any]) -> Any:
        ...
        """
        from xiuqo_cli.plugins import resolve_plugin_command_result
        try:
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        """
        from xiuqo_cli.plugins import _resolve_hook_callback_timeout
    ...
    def _run_hook_callback_bounded(
        ...
        thread = threading.Thread(target=_runner, name=f"xiuqo-hook-{callback_name}"[:40], daemon=True)
        try:
            thread.start()
    ...
    def _ensure_event_worker_locked(self) -> None:
        ...
        worker = threading.Thread(
            target=self._event_worker_loop, args=(self._event_queue,), name="xiuqo-plugin-events",
            daemon=True,
        )
