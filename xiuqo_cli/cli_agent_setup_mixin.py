...
def _single_query_clarify_callback(question: str, choices=None, multi_select=False) -> str:
    """Headless clarify answer for ``xiuqo chat -q``.
...
class CLIAgentSetupMixin:
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not callable(api_key) and not (isinstance(api_key, str) and api_key):
            if _keyless_custom_base(base_url):
                ...
                print("   Run 'xiuqo model' to choose a provider, or "
                      "'xiuqo setup' for first-time setup.")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: xiuqo setup")
            return False
        ...
        # is just the provider slug/display name (`xiuqo chat --model <provider-name>`
        ...
        # Still empty (e.g. `xiuqo auth add` without `xiuqo model`): fall back to the
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        startup, TTY). Runs the same flow as ``xiuqo model`` so onboarding has a single
        ...
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'xiuqo model' or 'xiuqo setup' any time.")
            return False
        try:
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'xiuqo model' to retry.")
        return False
    ...
    def _load_resumed_history_late(self) -> bool:
        ...
        # stderr so stdout stays machine-readable for `$(xiuqo chat -Q --resume ...)`.
        ...
        if not session_meta:
            hint = "Use a session ID from a previous CLI run (xiuqo sessions list)."
            if _quiet_mode:
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        try:
            ...
            from xiuqo_constants import partial_update_hint
    ...
    def _preload_resumed_session(self) -> bool:
        ...
        if not session_meta:
            ...
            self._console_print("[dim]Use a session ID from a previous CLI run (xiuqo sessions list).[/]")
            return False
        session_meta = self._follow_compression_chain(
            session_meta,
    ...
    def _display_resumed_history(self):
        ...
        role_styles = {
            ...
            "assistant": ("  ◆ Xiuqo: ", f"dim bold {_assistant_label_c}", "dim", " " * 12),
            "assistant_last": ("  ◆ Xiuqo: ", f"bold {_assistant_label_c}", "", " " * 12),  # full, non-dim
        }
        lines = Text()
        if skipped:
