...
The provider list shown by ``xiuqo model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`xiuqo_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `xiuqo model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``xiuqo model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("xiuqo_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("xiuqo_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    XIUQO_OVERLAYS = _safe_import("xiuqo_cli.providers", "XIUQO_OVERLAYS", {})
    try:
