"""``xiuqo plugins validate`` — admission checks for a plugin directory.
...
scratch child process (with a throwaway ``XIUQO_HOME``), so a crashing or
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "XIUQO_VALIDATE_JSON:"
...
def _requires_hermes_spec_valid(spec: str) -> bool:
    ...
    Unlike :func:`xiuqo_cli.plugins_manifest.version_satisfies` (permissive at load
    ...
    """
    from xiuqo_cli.plugins_manifest import _VERSION_COMPARATOR_RE, _version_tuple
...
# from xiuqo so a hostile plugin only sees a bare interpreter.
_PROBE_SCRIPT = r"""
...
def _run_capability_probe(plugin_dir: Path) -> Tuple[Optional[dict], str]:
    ...
    *error* is a human-readable failure description otherwise.
    """
    with tempfile.TemporaryDirectory(prefix="xiuqo-validate-") as scratch:
        env = dict(os.environ)
        env["XIUQO_HOME"] = scratch
        try:
