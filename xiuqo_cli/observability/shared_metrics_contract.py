"""Bounded product contract for the first Xiuqo shared-metrics slice."""
...
SCHEMA_KEY = "xiuqo.metrics.schema_version"
SCHEMA_VERSION = "xiuqo.metrics.event.v2"
MODEL_CALL_SCOPE = "xiuqo.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "xiuqo.task_run"
TOOL_CALL_SCOPE = "xiuqo.tool_call"
CLIENT_ACTIVE_MARK = "xiuqo.client.active"
TOOL_APPROVAL_MARK = "xiuqo.tool_approval"
SKILL_LIFECYCLE_MARK = "xiuqo.skill.lifecycle"
SKILL_LOAD_MARK = "xiuqo.skill.load"
SUBSCRIBER_NAME = "xiuqo.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "xiuqo.client.active"
LEGACY_MODEL_CALL_METRIC = "xiuqo.model_call.count"
MODEL_ROUTE_METRIC = "xiuqo.model_route.count"
TASK_STARTED_METRIC = "xiuqo.task_run.started"
TASK_FINISHED_METRIC = "xiuqo.task_run.finished"
TOOL_CALL_METRIC = "xiuqo.tool_call.count"
TOOL_APPROVAL_METRIC = "xiuqo.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "xiuqo.skill.lifecycle.count"
SKILL_LOAD_METRIC = "xiuqo.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "xiuqo_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Xiuqo installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    xiuqo_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(xiuqo_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "xiuqo_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("xiuqo_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Xiuqo logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "xiuqo.call_role"
    )
    call_role = (metadata or {}).get("xiuqo.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from xiuqo_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Xiuqo terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "xiuqo-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Xiuqo registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Xiuqo tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Xiuqo."""
