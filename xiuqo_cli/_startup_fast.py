...
Imported by ``xiuqo_cli/main.py`` BEFORE its heavy import wall (config, argparse tree, logging,
providers). Everything here must stay **stdlib-only and cheap** (os/sys file probes; no yaml, no
xiuqo_cli.config, no argparse). Exists so version-printing stops being reimplemented as
...
def active_profile_may_override_home(xiuqo_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect XIUQO_HOME?"""
    active = (_read_text(os.path.join(xiuqo_root, "active_profile")) or "").strip()
    return bool(active and active != "default")


def _default_home() -> str:
    return os.path.join(os.path.expanduser("~"), ".xiuqo")


def _resolved_home() -> str:
    return os.environ.get("XIUQO_HOME", "").strip() or _default_home()
...
def container_mode_may_be_active() -> bool:
    ...
    be active".
    """
    if os.environ.get("XIUQO_DEV") == "1" or is_container_startup_environment():
        return False
    xiuqo_home = os.environ.get("XIUQO_HOME", "").strip()
    if xiuqo_home:
        if os.path.exists(os.path.join(xiuqo_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(xiuqo_home)))
        return parent_name != "profiles" and active_profile_may_override_home(xiuqo_home)
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``xiuqo --version`` output (also used by /version).
    ...
    try:
        ...
        from xiuqo_cli import __release_date__, __version__

        print(f"Xiuqo Agent v{__version__} ({__release_date__})")
    ...
    try:
        from xiuqo_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``xiuqo --version`` before the heavy import wall.
    ...
    XIUQO_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("XIUQO_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
