"""Configuration management for Xiuqo Agent: config.yaml / .env loading, saving,
validation, migration, and the ``xiuqo config`` command."""
...
from xiuqo_cli import managed_scope
from xiuqo_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from xiuqo_cli.secret_prompt import masked_secret_prompt
# Re-export from xiuqo_constants — canonical definition lives there.
from xiuqo_constants import get_hermes_home, get_process_hermes_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    ...
    "refuse-write": (
        ...
        "Fix the YAML (xiuqo config edit) and retry.")}
...
def _warn_config_parse_failure(
    ...
    from xiuqo_cli.config_backups import backup_config
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  xiuqo config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# interpreter init (PYTHON*, NODE_*: Xiuqo restarts through them), PATH (fix tool lookup
...
# and Xiuqo runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``XIUQO_*`` overall is NOT blocked — many integration credentials use that prefix
# (XIUQO_LANGFUSE_PUBLIC_KEY, XIUQO_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Xiuqo runtime location
    "XIUQO_HOME", "XIUQO_PROFILE", "XIUQO_CONFIG", "XIUQO_ENV",
    "XIUQO_CONFIG_PATH", "XIUQO_ENV_PATH",
    ...
    "XIUQO_COPILOT_ACP_COMMAND", "XIUQO_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "XIUQO_YOLO_MODE", "XIUQO_ACCEPT_HOOKS", "XIUQO_REDACT_SECRETS",
    "XIUQO_INTERACTIVE", "XIUQO_EXEC_ASK", "XIUQO_GATEWAY_SESSION",
    "XIUQO_CRON_SESSION", "XIUQO_SINGLE_QUERY_SESSION",
    "XIUQO_SESSION_KEY", "XIUQO_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Xiuqo runtime location and security policy (XIUQO_HOME, XIUQO_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.xiuqo/.env "
            "directly.")
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean XIUQO_TOOL_PROGRESS variant is unsupported (its only
    # consumer, the v3->4 migration, is below the v12 support floor); doctor flags it as ignored.
    "XIUQO_TOOL_PROGRESS_MODE",
    ...
    "XIUQO_LANGFUSE_ENV", "XIUQO_LANGFUSE_RELEASE", "XIUQO_LANGFUSE_SAMPLE_RATE",
    "XIUQO_LANGFUSE_MAX_CHARS", "XIUQO_LANGFUSE_CAPTURE", "XIUQO_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "XIUQO_ACP_AUTH_METHOD", "XIUQO_ACP_AUTO_APPROVE", "XIUQO_COPILOT_ACP_COMMAND",
    "XIUQO_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# XIUQO_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
# Explicit opt-out (``XIUQO_MANAGED=false``): without this a bool-shaped value became a package
# manager literally named "false" and is_managed() blocked `xiuqo update` (#12864).
_MANAGED_FALSE_VALUES = frozenset({"false", "0", "no", "off"})


def get_managed_system() -> Optional[str]:
    """Return the package manager owning this install, if any.
    Signals: XIUQO_MANAGED env var (systemd service) or a ``.managed`` marker file in
    XIUQO_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("XIUQO_MANAGED", "").strip().lower() or None
...
def is_managed() -> bool:
    """Check if Xiuqo is running in package-manager-managed mode."""
...
_NIX_UPDATE_MSG = (
    "Update Xiuqo through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Xiuqo was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$XIUQO_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because XIUQO_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``xiuqo update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    ``/opt/xiuqo`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of xiuqo_cli/), NOT of $XIUQO_HOME,
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``xiuqo_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    ...
    "apt": "pkg upgrade xiuqo",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "xiuqo update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (xiuqo_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Xiuqo.
_DOCKER_UPDATE_MESSAGE = """\
✗ ``xiuqo update`` doesn't apply inside the Docker container.

Xiuqo Agent runs as a published image (nousresearch/hermes-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/hermes-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate xiuqo
...
Notes:
  ...
  • Your config and session history live under ``$XIUQO_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``xiuqo update`` inside Docker."""
    return _DOCKER_UPDATE_MESSAGE


def format_managed_message(action: str = "modify this Xiuqo installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Xiuqo installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Xiuqo.")
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from XIUQO_HOME/.container-mode.
    ...
    already inside the container, or when XIUQO_DEV=1 is set. Only FileNotFoundError is
    swallowed; other errors (permissions, malformed data) propagate."""
    if os.environ.get("XIUQO_DEV") == "1":
        return None
    ...
    return {
        ...
        "container_name": info.get("container_name", "xiuqo"),
        "exec_user": info.get("exec_user", "xiuqo"),
        "xiuqo_bin": info.get("xiuqo_bin", "/data/current-package/bin/xiuqo")}


# ---- Config paths / XIUQO_HOME skeleton ----
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / XIUQO_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("XIUQO_IGNORE_USER_CONFIG") == "1":
        return
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read XIUQO_UID / XIUQO_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns XIUQO_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Xiuqo commonly set these to map the in-container user to a host user so
    ...
    return _env_int("XIUQO_UID"), _env_int("XIUQO_GID")


def _chown_to_hermes_uid(path) -> None:
    """Chown ``path`` to ``XIUQO_UID:XIUQO_GID`` when set; EPERM/ENOENT are non-fatal (the
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply XIUQO_UID/GID ownership. No-op when managed;
    in a container only an explicit XIUQO_HOME_MODE is applied. XIUQO_HOME_MODE (e.g. 0701)
    overrides the mode so a web server can traverse XIUQO_HOME to a served subdirectory without
    directory listings.

    Also applies ``XIUQO_UID``/``XIUQO_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    if is_managed():
        return
    explicit_mode = os.environ.get("XIUQO_HOME_MODE", "").strip()
    ...
    # start (#10757). An explicit XIUQO_HOME_MODE is the operator's choice and is still applied.
...
def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or XIUQO_CONTAINER / XIUQO_SKIP_CHMOD opt-out).
    ...
    if (os.environ.get("XIUQO_CONTAINER") or os.environ.get("XIUQO_SKIP_CHMOD")
...
def ensure_hermes_home():
    """Ensure the ~/.xiuqo directory skeleton exists with secure permissions.
    ...
    from xiuqo_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _XIUQO_HOME_ENSURED and home.is_dir():
        return
    from xiuqo_cli.config_home import initialize_home
    initialize_home(home, _XIUQO_HOME_SUBDIRS, _XIUQO_HOME_ENSURED)


# ---- Config loading/saving ----

from xiuqo_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from xiuqo_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use xiuqo_cli.config.<name>)
...
# Back-compat re-exports — :mod:`xiuqo_cli.personality` owns personality/overlay semantics.
from xiuqo_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``xiuqo config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `xiuqo config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    return (
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `xiuqo update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (xiuqo_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # xiuqo_cli/tools_config.py toolset-save flow
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'xiuqo tools' and pick a different Web Search & Extract provider")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            from xiuqo_cli.config_home import config_load_issue
    ...
    if cp and not config.get("model"):
        _issue(issues, "warning",
               "custom_providers defined but no 'model' section — Xiuqo won't know which provider to use",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'xiuqo doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from xiuqo_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ xiuqo config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from xiuqo_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from xiuqo_cli.toolset_validation import validate_platform_toolsets
        from xiuqo_cli.toolset_scope import toolset_allowed_for_platform
...
def _offer_list(heading: str, items: List[str], question: str) -> bool:
    ...
    if not _ask_yes_no(question):
        print("  Set later with: xiuqo config set <key> <value>")
        return False
    print()
    return True
...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        logger.warning(
            "Config ref %r: %s is not set (check ~/.xiuqo/.env); "
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``xiuqo config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``xiuqo status``/``dump``) read ``name`` and
...
# the XIUQO_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Xiuqo host
...
def _last_known_good_fallback(config_path: Path, path_key: str, cache_sig, exc: Exception) -> Optional[Dict[str, Any]]:
    ...
    if lkg is None:
        # Fresh process (CLI restart, `xiuqo config get`): nothing loaded yet in this process, so
        ...
        from xiuqo_cli.config_backups import load_newest_good_backup
...
#   openai-codex (OAuth — xiuqo auth) — OpenAI Codex
#   nous         (OAuth — xiuqo auth) — Nous Portal
...
def save_config(
    ...
    """Save configuration to ~/.xiuqo/config.yaml.
...
def load_env() -> Dict[str, str]:
    """Load ~/.xiuqo/.env as a dict (memoised; ``get_env_value()`` runs hundreds of times per
...
def sanitize_env_file() -> int:
    """Rewrite ~/.xiuqo/.env with normalized line formatting; returns the number of changed lines."""
    env_path = get_env_path()
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.xiuqo/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``XIUQO_CUSTOM_`` prefix keeps the
    ...
    return f"XIUQO_CUSTOM_{slug}_API_KEY" if slug else "XIUQO_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.xiuqo/.env and os.environ; True if it was found and removed."""
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from xiuqo_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    """Re-read ~/.xiuqo/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Xiuqo (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.xiuqo/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Xiuqo-managed credential preferring ``~/.xiuqo/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale XIUQO_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("XIUQO_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale XIUQO_MAX_ITERATIONS={env_ghost} "
                    f"(run 'xiuqo doctor --fix' to remove)", Colors.YELLOW))
...
def _show_display_section(config: Dict[str, Any]) -> None:
    ...
    try:
        from xiuqo_cli.personality import active_personality_name
...
def show_config():
    ...
    print(color("│              ☤ Xiuqo Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from xiuqo_cli.auth import get_anthropic_key
    ...
    print(color("  xiuqo config edit     # Edit config file", Colors.DIM))
    print(color("  xiuqo config set <key> <value>", Colors.DIM))
    print(color("  xiuqo setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``XIUQO_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("XIUQO_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "`xiuqo cron edit <job_id> --provider <provider> --model <model>` or set a fleet default "
        "with `xiuqo config set cron.model <model>`.")
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``xiuqo config set`` string to bool/None/int/float/list/dict.
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``xiuqo config set platforms.telegram.streaming false`` landed on a key
...
def _guard_section_overwrite(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> str:
    ...
    err += [
        ...
        f"    xiuqo config set {key}.<sub-key> <value>",
        "  Or use --force to replace the entire section:",
        f"    xiuqo config set --force {key} {value!r}"]
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Xiuqo may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `xiuqo config
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from xiuqo_cli.credential_lifecycle import remove_provider_env_credential
...
_USAGE_GET = ("Usage: xiuqo config get <key> [--json]", [
    "xiuqo config get model", "xiuqo config get terminal.backend",
    "xiuqo config get skills.config --json"], None)
_USAGE_SET = ("Usage: xiuqo config set [--force] <key> <value>", [
    "xiuqo config set model anthropic/claude-sonnet-4", "xiuqo config set terminal.backend docker",
    "xiuqo config set OPENROUTER_API_KEY sk-or-..."], [
    "", "  --force: skip the unknown-key notice for unrecognized keys,",
    "           and allow a scalar to replace a whole mapping section"])
_USAGE_UNSET = ("Usage: xiuqo config unset <key>", [
    "xiuqo config unset model", "xiuqo config unset terminal.backend",
    "xiuqo config unset OPENROUTER_API_KEY"], None)
...
def _cmd_config_check(args):
    ...
    if missing_config:
        ...
        print("    Run 'xiuqo config migrate' to add them")
...
_CONFIG_USAGE = """Available commands:
  xiuqo config           Show current configuration
  xiuqo config edit      Open config in editor
  xiuqo config get <key>          Print a resolved config value
  xiuqo config set <key> <value>   Set a config value
  xiuqo config unset <key>        Remove a config value
  xiuqo config check     Check for missing/outdated config
  xiuqo config migrate   Update config with new options
  xiuqo config path      Show config file path
  xiuqo config env-path  Show .env file path"""
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``xiuqo config`` UI without the core knowing they exist.
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``xiuqo_cli/`` — i.e. the git checkout for source
    installs, ``/opt/xiuqo`` inside the published image. It is a property of
    the running interpreter, NOT of ``$XIUQO_HOME``, which is why a
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$XIUQO_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/xiuqo`` in the published image, which instead bakes the stamp at
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('xiuqo_cli.route_identity', 'normalize_route_base_url'),
}
