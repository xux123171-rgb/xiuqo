...
Wraps the same config layer the CLI uses (xiuqo_cli.mcp_config); stdio ``env``
...
def _normalize_mcp_server_create(body: MCPServerCreate) -> tuple[str, Dict[str, Any], Optional[str]]:
    ...
    """
    from xiuqo_cli.mcp_config import _bearer_auth_headers, _strip_bearer_prefix
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.xiuqo_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        home_token = set_hermes_home_override(flow.xiuqo_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.xiuqo_home)))
        try:
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                previous_entry = None
                try:
                    previous_entry = manager.remove(flow.server_name, xiuqo_home=flow.xiuqo_home)
                    ...
                    manager.restore_entry(flow.server_name, previous_entry, xiuqo_home=flow.xiuqo_home)
                    raise
        finally:
