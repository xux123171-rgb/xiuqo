"""``xiuqo debug`` debug tools for Xiuqo Agent."""
...
_REDACTION_BANNER = (
    "[xiuqo debug share: log content redacted at upload time. "
...
# ``xiuqo debug`` sweeps on entry (CLI-only users). Replaced a fork-and-sleep subprocess that
...
def _save_pending(entries: list[dict]) -> None:
    try:
        ...
        pass  # non-fatal — worst case the user runs ``xiuqo debug delete`` manually
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `xiuqo debug share` "
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from xiuqo_cli.logs import LOG_FILES
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Xiuqo Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``xiuqo dump`` and return its stdout as a string."""
    from xiuqo_cli.dump import run_dump
    capture = io.StringIO()
...
def collect_debug_report(
    *, log_lines: int = 200, dump_text: str = "",
    ...
    ``dump_text`` is pre-captured dump output; when empty, ``xiuqo dump`` is run internally.
    """
    buf = io.StringIO()
...
_NOUS_BUNDLE_FORMAT = "xiuqo-debug-share/1"
...
def build_debug_share(
    ...
    Shared by ``xiuqo debug share`` and the dashboard ``POST /api/ops/debug-share``. Blocking
    ...
    if redact:
        logger.info(
            "xiuqo debug share: applied force-mode redaction to log snapshots before upload")
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `xiuqo debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    if dpaste_urls:
        ...
        print(f"⚠️  {len(dpaste_urls)} of {len(result.urls)} upload(s) fell back to "
              ...
              "deleted with `xiuqo debug delete`.\n"
              "\nShare these links with the Xiuqo team for support.")
    else:
        print(f"\n⏱  Pastes will auto-delete in {result.auto_delete_seconds // 3600} hours.\n"
              "To delete now:  xiuqo debug delete <url>\n"
              "\nShare these links with the Xiuqo team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Xiuqo version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """``xiuqo debug share --nous``: gzip the same bundle into the Nous envelope → Nous-S3."""
    from xiuqo_cli.diagnostics_upload import share_to_nous
    ...
    bundle = collect_share_bundle(log_lines=log_lines, redact=redact)
    if redact:
        logger.info("xiuqo debug share --nous: applied force-mode redaction before upload")
    ...
    try:
        ...
        print(f"\nNous upload failed: {exc}\n"
              ...
              "Run `xiuqo debug share --local` to print the report instead, "
              "or `xiuqo debug share` to upload to a public paste service.\n", file=sys.stderr)
        sys.exit(1)
    ...
    print("\nShare this private link with the Nous team — only Nous staff "
          ...
          "  GitHub Issues        https://github.com/xux123171-rgb/xiuqo/issues\n"
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: xiuqo debug delete <url> [<url> ...]\n"
              "  Deletes paste.rs pastes uploaded by 'xiuqo debug share'.")
        return
    for url in urls:
        try:
            if delete_paste(url):
