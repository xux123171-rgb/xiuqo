...
Scope is deliberately narrow: Xiuqo only upgrades an npm inside its **own** managed Node tree
(``$XIUQO_HOME/node``), installing in place with ``--prefix`` so ``bin/npm`` keeps resolving to
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Xiuqo-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: ``~/.local/bin/npm`` → ``$XIUQO_HOME/node/bin/npm`` →
...
def upgrade_managed_npm(npm: str, npm_range: str, *, prefix: Path, quiet: bool = False) -> bool:
    ...
    into ``$XIUQO_HOME/node/etc/npmrc`` (so global installs land on PATH), and without the override
    the "upgrade" would install a second npm elsewhere while the managed one stayed stale.
    """
    if not quiet:
        print(f"→ Upgrading Xiuqo-managed npm to satisfy {npm_range}…", flush=True)
    ...
    if managed_node_tree_in_use():
        if not quiet:
            print(
                "  ⚠ deferred: the Xiuqo-managed Node.js tree is in use by a "
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="xiuqo-npm-upgrade-") as tmp:
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Xiuqo could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision (or reuse) the managed tree under ``$XIUQO_HOME/node`` and return a satisfying npm.
    ...
    if not quiet:
        print(
            "→ Provisioning a Xiuqo-managed Node.js runtime "
...
def maybe_repair_npm_engine(npm: str | None, output: str, *, quiet: bool = False) -> str | None:
    ...
    if prefix is not None:
        # Xiuqo owns this npm — upgrade in place. Only an npm-range failure is fixable this way.
