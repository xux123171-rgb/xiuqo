"""``xiuqo project`` CLI — manage first-class, multi-folder Projects."""
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    p_create.add_argument("name", help="Human name, e.g. 'Xiuqo Agent'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``xiuqo project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print("usage: xiuqo project <action> [options]\nRun 'xiuqo project --help' for the full list.", file=sys.stderr)
        return 0
...
def _cmd_list(args, conn):
    ...
    if not projs:
        return "No projects yet. Create one with `xiuqo project create <name>`."
    for p in projs:
...
def _cmd_set_primary(args, conn, proj):
    if not pdb.set_primary(conn, proj.id, args.path):
        return _err(f"'{args.path}' is not a folder of {proj.slug}; add it first with `xiuqo project add-folder`.")
