...
def active_session_limit_message(
    ...
    return (
        f"Xiuqo is at the active session limit ({active_count}/{max_sessions})."
...
def format_refusal_stderr(message: str) -> str:
    ...
    return f"xiuqo-refusal-reason: {reason}\n{message}" if reason else str(message)
...
class ActiveSessionLease:
    ...
    enabled: bool = True
    released: bool = False
    # Pinned at acquisition: a lease taken under the root XIUQO_HOME must release
    ...
    with _FileLock(lock_path):
        ...
        if loaded is None:
            return None, ActiveSessionRefusal(
                "Xiuqo could not read the active-session registry at "
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    # running under a profile XIUQO_HOME override.
    # See #85431.
