"""Directory initialization and storage diagnostics for the active Xiuqo home."""
...
def _ensure_directory(path: Path, *, create: bool, secure: bool) -> None:
    from xiuqo_cli.config import _secure_dir
    ...
    try:
        ...
        raise HomeInitializationError(
            f"Cannot initialize Xiuqo directory {path}: {exc}. "
            ...
            "Xiuqo has not replaced the link or created its missing target."
        ) from exc


def initialize_home(home: Path, subdirs: tuple[str, ...], ensured: set[str]) -> None:
    from xiuqo_cli.config import _ensure_default_soul_md, is_managed
    ...
    try:
        ...
        try:
            ...
            raise HomeInitializationError(
                f"Cannot initialize Xiuqo home {home}: {exc}. "
...
def config_load_issue(exc: Exception):
    from xiuqo_cli.config import ConfigIssue

    if isinstance(exc, (HomeInitializationError, OSError)):
        return ConfigIssue(
            "error", f"Xiuqo storage is unavailable: {exc}",
            "Check the reported path, link target, mount and permissions; keep config.yaml unchanged.",
        )
    return ConfigIssue("error", "Could not load config.yaml", "Run 'xiuqo setup' to create a valid config")
