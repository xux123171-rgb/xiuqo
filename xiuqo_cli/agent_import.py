"""xiuqo import-agent — import Claude Code / Codex CLI setups into Xiuqo.
...
# Entry delimiter of the Xiuqo memory store (memories/MEMORY.md) and the openclaw script.
ENTRY_DELIMITER = "\n§\n"
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not path.exists():
        return {}
    fix_hint = "Fix it with `xiuqo config edit` (or move it aside), then re-run the import."
...
def _translate_mcp_server(name: str, srv: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
    """Map one Claude/Codex MCP server entry to Xiuqo shape; returns (server, stripped secret paths)."""
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record("slash-commands", commands_dir, None, "skipped",
                        "Claude slash commands have no direct Xiuqo equivalent — "
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → XIUQO_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``xiuqo import-agent`` (invoked from xiuqo_cli.main)."""
    from xiuqo_cli.config import get_config_path, load_config, save_config
    from xiuqo_constants import get_hermes_home
    from xiuqo_cli.setup import (Colors, color, print_header, print_info, print_success,
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: xiuqo import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: xiuqo import-agent claude-code   or   xiuqo import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ☤ Xiuqo — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))
    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info(f"Specify a custom path: xiuqo import-agent {agent} --source /path/to/{_AGENT_DEFAULT_DIRS[agent]}")
        return
    ...
    print_info(f"Target:      {xiuqo_home}")
    print_info(f"Overwrite:   {'yes' if overwrite else 'no (skip conflicts)'}")
    print_info("Secrets:     never imported — run 'xiuqo setup' for credentials")
    ...
    def run_import(execute: bool, phase: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            return AgentImporter(agent, source_dir.resolve(), xiuqo_home.resolve(),
    ...
    if not args.yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: xiuqo import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'xiuqo setup' "
               "to configure providers, or add them to ~/.xiuqo/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from xiuqo_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped := report.get("stripped_secrets"):
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'xiuqo setup' or ~/.xiuqo/.env.")
        print()
