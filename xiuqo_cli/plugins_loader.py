...
dependency/config-schema warnings. Mixed into :class:`xiuqo_cli.plugins.PluginManager`.

Origin-internal names (``PluginContext``, ``LoadedPlugin``, ``_PLUGINS_DEBUG`` …) are imported lazily
through ``xiuqo_cli.plugins`` so tests that patch them on the origin keep working.
"""
...
from xiuqo_constants import get_hermes_home, reset_hermes_home_override, set_hermes_home_override
from registration_lifecycle import replacement_coordinator
from xiuqo_cli.plugins_discovery import ENTRY_POINTS_GROUP, _select_entry_point_group
from xiuqo_cli.plugins_manifest import PluginManifest, manifest_key, validate_config_schema
from xiuqo_cli.plugins_state import _plugin_settings_entry

if TYPE_CHECKING:  # pragma: no cover
    from xiuqo_cli.plugins import LoadedPlugin

logger = logging.getLogger("xiuqo_cli.plugins")

_NS_PARENT = "xiuqo_plugins"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Xiuqo home."""
...
class PluginLoaderMixin:
    ...
    def _register_deferred_platform_tools(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        would miss them in ``xiuqo tools`` / ``platform_toolsets``. Opt-in is explicit via ``provides_tools``;
        ...
        returns ``[]``, the toolset is missing from the ``xiuqo tools`` checklist, and even an explicit
        ...
        """
        from xiuqo_cli.plugins import PluginContext, _PLUGINS_DEBUG
        ...
        try:
            ...
            # Tools registered before the raise are live: credit them or `xiuqo plugins list` under-reports
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        See #64165.
        python_dependencies is a declaration seam ONLY: Xiuqo validates and prints the requirements with an
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Xiuqo does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        from xiuqo_cli.plugins import LoadedPlugin, PluginContext, _PLUGINS_DEBUG
        ...
        from xiuqo_cli.plugins_manifest import requires_hermes_error
        ...
        # skipped with a clear reason instead of dying on ImportError mid-register (xiuqo_cli.plugin_compat).
        from xiuqo_cli.plugin_compat import disable_reason
        ...
        try:
            ...
            if register_fn is None:
                ...
                loaded.enabled = True
                from xiuqo_cli.plugins_ledger import _hook_source_of
    ...
    def _track_tool_override_policy(self, manifest: PluginManifest, module_name: str) -> None:
        ...
        from xiuqo_cli.plugins import PluginContext
    ...
    def _attribute_registrations(
        ...
        # Discovery-time tools predate registration_start; credit them back or `xiuqo plugins list`
    ...
    def _load_portable_plugin(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        lookup_key = manifest_key(manifest)
        try:
            from xiuqo_cli.agent_plugins import load_agent_plugin
    ...
    def _directory_module_name(self, manifest: PluginManifest) -> str:
        """Profile-safe import namespace for a directory plugin: the bare ``xiuqo_plugins.<slug>`` for the
    ...
    def _load_directory_module(
        ...
        """Import a directory plugin as ``xiuqo_plugins.<slug>`` (slug from ``manifest.key`` so
        ...
        # Evict stale entries for this slug (same slug cached from another Xiuqo home, or an earlier force
