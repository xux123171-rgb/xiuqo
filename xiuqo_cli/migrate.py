"""CLI handlers for ``xiuqo migrate ...``.

Currently exposes only ``xiuqo migrate xai`` — diagnoses and (with --apply) rewrites references to
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``xiuqo migrate <subtype>``."""
    if getattr(args, "migrate_type", None) == "xai":
        return cmd_migrate_xai(args)

    print("usage: xiuqo migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `xiuqo migrate xai --apply` to rewrite "
    ...
    print()
    print(color("Run `xiuqo doctor` to confirm no retired xAI models remain.", Colors.DIM))
    return 0
