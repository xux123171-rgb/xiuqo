"""Helpers for loading Xiuqo .env files consistently across entrypoints."""
...
# env-var name → source label ("bitwarden", …) for externally injected credentials; setup / `xiuqo
...
# XIUQO_HOME paths already pulled external secrets for: load_hermes_dotenv() runs at import time from
...
# Behavioral routing keys a parent Xiuqo process injects into child env that silently redirect a profile
...
def get_secret_source_values(xiuqo_home: str | os.PathLike) -> dict[str, str]:
    """Return the external-secret value snapshot for ``xiuqo_home``."""
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(str(Path(xiuqo_home).resolve()), {}))


def hydrate_profile_secret_sources(xiuqo_home: str | os.PathLike) -> dict[str, str]:
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(xiuqo_home))
...
def reset_secret_source_cache(xiuqo_home: str | os.PathLike | None = None) -> None:
    """Forget applied homes so the next load re-pulls (tests, long-running processes after config edits).

    ``xiuqo_home`` limits the reset to ONE home: a multiplex gateway keeps every profile's snapshot in
    ...
    (#102041)."""
    if xiuqo_home is None:
        _APPLIED_HOMES.clear()
        _SECRET_SOURCES.clear()
        _SECRET_SOURCE_VALUES_BY_HOME.clear()
        return
    home_key = str(Path(xiuqo_home).resolve())
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `xiuqo setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from xiuqo_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    xiuqo_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    load_external_secrets: bool = True,
) -> list[Path]:
    """Load Xiuqo env files: ``~/.xiuqo/.env`` overrides stale shell exports; project ``.env`` is a dev
    ...
    home_path = Path(xiuqo_home) if xiuqo_home else get_process_hermes_home()
    ...
    if is_multiplex_active() and get_hermes_home_override() is not None:
        ...
        if load_external_secrets:
            from xiuqo_cli import _early_recovery
    ...
    # Windows (#73381, #86735). 2. A fresh ``xiuqo update`` retry just completed a deferred dependency
    ...
    # ~/.xiuqo/.env win on every reload and flip the backend mid-session in long-lived processes.
    ...
    # with override=True — so a stale TERMINAL_ENV=docker left in ~/.xiuqo/.env (e.g. written by an older
    # `xiuqo setup` before the user switched terminal.backend in config.yaml) silently wins again on every
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    tokens), BEFORE Xiuqo reads credentials; failures never block startup. Precedence/conflicts/provenance
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # populating it lets main.py's early bridge and xiuqo_logging reuse one parse instead of 3-4.
...
def _process_hermes_home() -> Path:
    """The XIUQO_HOME the running process was launched under.
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    ``xiuqo_constants.get_process_hermes_home()`` is the override-immune
