...
in :func:`run_migrations`. Every write goes through ``xiuqo_cli.config._persist_migration`` so a
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_hermes_home()}/config.yaml and run `xiuqo setup` to "
        f"regenerate, or manually set _config_version: {SUPPORT_FLOOR_VERSION} "
        "after reviewing the changelog.")


def _cfg():
    """Return the live ``xiuqo_cli.config`` module (lazy, cycle-free, monkeypatch-friendly)."""
...
def _installed_user_plugins(disabled: set) -> List[str]:
    """Names of plugins under ``$XIUQO_HOME/plugins/`` with a manifest, minus *disabled*."""
    _c = _cfg()
    found: List[str] = []
    try:
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    message = (
        ...
        "Use `xiuqo plugins enable <name>` to activate.")
    _commit(
        config, results, quiet,
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # section; deep-merge made it work but users could not see/edit it and `xiuqo curator status`
        ...
        for label, added in (("curator", added_curator), ("auxiliary.curator", added_aux)):
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added)}) — edit via `xiuqo config set`")
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from xiuqo_cli.relay_plugin_cutover import legacy_relay_plugin_keys
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if changed:
        _commit(
            ...
            "lists — video generation now lives under `xiuqo tools` → "
...
MIGRATIONS: Tuple[Tuple[int, Callable[[Dict[str, Any], bool], None]], ...] = (
    ...
    (44, _rewrite_stale_default(
        ...
        message=(
            ...
            "skills/.archive/ (recoverable with `xiuqo curator restore`). Set it back to 90 to keep the old window."))),
)
