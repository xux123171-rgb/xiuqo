"""Desktop (Electron) app: build/stamp, stage-and-swap pack, exe integrity gate, macOS signing/TCC, Linux sandbox, launch (xiuqo gui/desktop).
...
from xiuqo_cli.main_tui_launch import _npm_lifecycle_env
...
def _desktop_stamp_path() -> Path:
    """Path of the desktop build stamp under $XIUQO_HOME."""
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Xiuqo.app/Contents/MacOS/Xiuqo → …/Contents/Resources
    resources = (
...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live ``apps/desktop/release`` or a
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Xiuqo.app/Contents/MacOS/Xiuqo"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / d / "Xiuqo.exe" for d in ("win-unpacked", "win-ia32-unpacked", "win-arm64-unpacked")
        ]
    else:
        candidates = [
            release_dir / d / n for d in ("linux-unpacked", "linux-arm64-unpacked") for n in ("xiuqo", "Xiuqo")
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # mtime can hand a wrong-architecture Xiuqo.exe to the launcher. Prefer
        ...
        # wrong-architecture Xiuqo.exe to the launcher, which Windows rejects with "This app can't run on
        # your computer" (#69179).
...
# PLACE: before-pack.mjs wipes ``release/<platform>- unpacked`` (or the mac ``Xiuqo.app``) and the Electron
# unpack + asar + rename then rebuild it. Any failure after that wipe — corrupt cached zip, blocked
# download, missing dep, disk full — leaves the user with NO app, and ``xiuqo update`` used to report
...
# (Desktop → xiuqo-setup --update → `xiuqo update` → `xiuqo desktop --build-only` → relaunch) rebuilds
# Xiuqo.exe on the end user's machine and used to verify only that the file EXISTS before declaring
...
# Xiuqo.exe is caught BEFORE the updater replaces the working app, and the previous build can be restored
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    emulated process arch) and rejected a correctly-built ARM64 ``Xiuqo.exe``. Binding
...
def _windows_native_machine() -> str:
    ...
    chain runs an x64 xiuqo-setup.exe (and thus x64 Python) on Windows-on-ARM devices, where
...
def _ensure_desktop_exe_launchable(desktop_dir: Path, packaged_executable: Optional[Path]) -> tuple:
    ...
    print(f"✗ The built Xiuqo.exe failed its integrity check: {error}\n    at: {packaged_executable}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Xiuqo.exe from backup.")
        print("    Your existing version was kept and still works. Run `xiuqo desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `xiuqo desktop --force-build` to rebuild, or re-run the Xiuqo")
...
def _redownload_electron_dist(project_root: Path, env: dict, *, mirror: Optional[str] = None) -> bool:
    ...
    if not installer.is_file():
        return False
    from xiuqo_constants import find_node_executable, with_hermes_node_path
...
def _desktop_macos_relaunchable_fixup(
    ...
    "Xiuqo is damaged" and loses every grant. Clear quarantine xattrs, then sign
    ...
    if exe is None:
        return True
    # exe = .../Xiuqo.app/Contents/MacOS/Xiuqo  ->  app bundle = .../Xiuqo.app
    app = exe.parents[2]
...
def _macos_create_signing_identity(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="xiuqo-tcc-"))
    try:
...
def _desktop_macos_setup_tcc_identity(identity: str = "Xiuqo Local Signing") -> bool:
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.nousresearch.xiuqo"
    )
    return True
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Xiuqo Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Xiuqo Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Xiuqo Desktop (Linux only, best-effort).

    ``Exec`` and ``Icon`` are absolute so the entry works outside a login shell.
    ``xiuqo uninstall --gui`` removes it.
    """
    from xiuqo_cli.main import PROJECT_ROOT
    try:
        from xiuqo_cli.linux_desktop_entry import install_desktop_entry, is_supported
        if not is_supported():
            return
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    from xiuqo_cli.main import PROJECT_ROOT
    from xiuqo_cli.main_web_build import _run_npm_install_deterministic
...
def _run_desktop_pack_with_recovery(
    ...
    if build_result.returncode != 0 and staging_dir is not None and _staged_exe() is None:
        ...
        if restored:
            ...
            for p in purged:
                print(f"    - {p}")
            # The purge can't remove a win-unpacked tree whose Xiuqo.exe is
...
def _promote_staged_desktop_app(desktop_dir: Path, staging_dir: Path) -> Path:
    ...
    # Xiuqo.exe Windows cannot load. Verified on the STAGED exe, so a failure
...
def _build_desktop_app(desktop_dir: Path, *, source_mode: bool, npm: str, env: dict) -> Optional[Path]:
    ...
    if not source_mode:
        ...
        # A running desktop instance holds Xiuqo.exe locked on Windows, so the
    ...
    if build_result.returncode != 0:
        ...
        if sys.platform == "win32":
            print("  If this says \"Access is denied\" on Xiuqo.exe, close any")
            print("  running Xiuqo desktop window and retry.")
        print("  If the log shows Electron download retries, rebuild via a mirror:")
        print("    ELECTRON_MIRROR=<mirror-base-url> xiuqo desktop --force-build")
...
def _prefer_wsl_d3d12(env: dict) -> None:
    ...
    from xiuqo_constants import is_wsl
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    for attr, key in (
        ("fake_boot", "XIUQO_DESKTOP_BOOT_FAKE"), ("ignore_existing", "XIUQO_DESKTOP_IGNORE_EXISTING")):
        if getattr(args, attr, False):
            env[key] = "1"
    if getattr(args, "xiuqo_root", None):
        env["XIUQO_DESKTOP_XIUQO_ROOT"] = str(Path(args.xiuqo_root).expanduser().resolve())
    cwd = getattr(args, "cwd", None)
    env["XIUQO_DESKTOP_CWD"] = str(Path(cwd).expanduser().resolve()) if cwd else os.getcwd()

    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options())
    if config_disable_gpu != "auto" and "XIUQO_DESKTOP_DISABLE_GPU" not in os.environ:
        env["XIUQO_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if sys.platform == "linux" and "XIUQO_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
            config_password_store if config_password_store != "auto" else _detect_linux_password_store()
        )
        if password_store:
            env["XIUQO_DESKTOP_PASSWORD_STORE"] = password_store
...
def cmd_gui(args: argparse.Namespace):
    ...
    from xiuqo_cli.main import PROJECT_ROOT
    from xiuqo_cli.main_install_repair import _resolve_node_runtime_npm
    desktop_dir = PROJECT_ROOT / "apps" / "desktop"
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Xiuqo Local Signing"
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  xiuqo gui")
            sys.exit(1)
    ...
    if source_mode:
        print("→ Launching Xiuqo Desktop from source build...")
    ...
    if not source_mode:
        print(f"→ Launching packaged Xiuqo Desktop: {' '.join(launch_command)}")
