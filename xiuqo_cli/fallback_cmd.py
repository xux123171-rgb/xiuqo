"""xiuqo fallback — manage the fallback provider chain (tried in order when the primary fails).

Subcommands: ``list`` (default), ``add`` (same picker as `xiuqo model`), ``remove``, ``clear``.
"""
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if chain is None:
        print("  Add one with:  xiuqo fallback add\n")
        return
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `xiuqo model`, then append the selection to the chain."""
    from xiuqo_cli.main import _require_tty, select_provider_and_model
    from xiuqo_cli.config import load_config, save_config
    ...
    print("\n  Adding a fallback provider.  The picker below is the same one used by\n"
          "  `xiuqo model` — select the provider + model you want as a fallback.\n")
    ...
    print("  Run `xiuqo fallback list` to view, or `xiuqo fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``xiuqo fallback [subcommand]``."""
