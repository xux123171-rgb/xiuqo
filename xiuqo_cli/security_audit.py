"""On-demand supply-chain audit for Xiuqo Agent installs.
...
def _discover_plugins(xiuqo_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.xiuqo/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, xiuqo_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = xiuqo_home or Path(get_hermes_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `xiuqo security audit`."""
    ...
    components = _discover_components(xiuqo_home=home, **skips)
    ...
    try:
        findings = run_audit(xiuqo_home=home, components=components)
