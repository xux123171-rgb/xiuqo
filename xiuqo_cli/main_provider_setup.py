...
Split out of ``xiuqo_cli/main.py``. Names that still live in main are imported lazily at call time.
"""

import contextlib

from typing import Optional
from xiuqo_cli.model_setup_flows_common import _ask, _ensure_dict_section, _print_numbered, _radiolist, _say
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.xiuqo/.env unless the active provider is 'custom' — a
    leftover value routes provider:auto auxiliary clients to the old custom endpoint."""
    from xiuqo_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    (:meth:`xiuqo_cli.plugins.PluginContext.register_auxiliary_task`)."""
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        _say("", "  Auxiliary models — side-task routing", "",
             ...
             "  Xiuqo only falls back to a lightweight backend (OpenRouter,",
...
def _aux_select_for_task(task: str) -> None:
    ...
    from xiuqo_cli.config import load_config
    from xiuqo_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    with contextlib.suppress(Exception):
        from xiuqo_cli.setup import _curses_prompt_choice
...
_CUSTOM_API_MODES = (
    ("", "Auto-detect", "Use Xiuqo URL heuristics; best for standard OpenAI-compatible endpoints.",
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    from xiuqo_cli.runtime_provider import _detect_api_mode_for_url
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "", existing_source: str = "") -> tuple:
    """API-key entry for ``xiuqo setup`` / ``xiuqo model``: first-time entry, or [K]eep / [R]eplace /
    ...
    from xiuqo_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(f"  API key cleared.  Re-run `xiuqo setup` to configure {pconfig.name} again.")
        return "", True
...
def _stepfun_base_url_for_region(region: str) -> str:
    from xiuqo_cli.auth import STEPFUN_STEP_PLAN_CN_BASE_URL, STEPFUN_STEP_PLAN_INTL_BASE_URL
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    from xiuqo_cli.config import save_anthropic_oauth_token, use_anthropic_claude_code_credentials
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (is_claude_code_token_valid(creds) or bool(creds.get("refreshToken"))):
            ...
            from xiuqo_constants import display_hermes_home as _dhh_fn
            print(f"    Xiuqo will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
    ...
    from xiuqo_cli.config import get_compatible_custom_providers, read_raw_config
...
def _build_provider_picker_rows(config: dict, active: str, provider_labels: dict[str, str],
    ...
    """Rows for the ``xiuqo model`` provider picker plus the pre-selected index. Canonical providers
    ...
    from xiuqo_cli.models import CANONICAL_PROVIDERS, _PROVIDER_ALIASES
    from xiuqo_cli.models_catalog_static import group_providers, provider_group_for_slug
