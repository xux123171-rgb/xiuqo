...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    env = os.getenv("XIUQO_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
...
# Keyed by xiuqo_home_key() so a multiplex profile never bills through a sibling's token.
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError("Not logged into Nous Portal — run `xiuqo portal` to log in.", status=401, error="invalid_token")
    if exc is not None:
        err.__cause__ = exc
    return err
