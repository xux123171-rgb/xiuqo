"""Single source of truth for provider identity in Xiuqo Agent."""
...
# -- Xiuqo overlay: metadata models.dev doesn't provide ----------------------

@dataclass(frozen=True)
class HermesOverlay:
    """Xiuqo-specific provider metadata layered on top of models.dev."""
...
XIUQO_OVERLAYS: Dict[str, HermesOverlay] = {
    ...
    "qwen-oauth": HermesOverlay(auth_type="oauth_external", base_url_override="https://portal.qwen.ai/v1",
                                base_url_env_var="XIUQO_QWEN_BASE_URL"),
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "xiuqo", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    """Look up a built-in provider by id or alias: models.dev catalog merged with the Xiuqo overlay;
    Xiuqo-only overlay (nous, openai-codex, …); plugin provider profiles with a concrete endpoint."""
    canonical = normalize_provider(name)
    mdev_info = _models_dev_info(canonical, allow_network)
    overlay = XIUQO_OVERLAYS.get(canonical)
    ...
    if overlay is not None:
        return _overlay_pdef(canonical, overlay, _LABEL_OVERRIDES.get(canonical, canonical), overlay.extra_env_vars,
                             overlay.base_url_override, "", "xiuqo")
    # Plugin-registered profiles (plugins/model-providers/<name>/) absent from models.dev and
    # XIUQO_OVERLAYS would otherwise be "Unknown provider" in /model, --provider and model-switch
...
def _lossy_alias_registry_pdef(raw: str, canonical: str) -> Optional[ProviderDef]:
    """Exact Xiuqo registry ids win over LOSSY alias collapsing (kimi-coding-cn must stay distinct
    ...
    try:
        from xiuqo_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
...
def _llamacpp_pdef() -> Optional[ProviderDef]:
    ...
    try:
        from xiuqo_cli.local_runtime.endpoint import resolve_llamacpp_endpoint
