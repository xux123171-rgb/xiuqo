"""Shell completion script generation for xiuqo CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# Xiuqo Agent bash completion
# Add to ~/.bashrc:
#   eval "$(xiuqo completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.xiuqo/profiles"
...
complete -F _hermes_completion xiuqo
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef xiuqo
# Xiuqo Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(xiuqo completion zsh)"

_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.xiuqo/profiles" ]]; then
        profiles+=($HOME/.xiuqo/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'xiuqo command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Xiuqo Agent fish completion",
        "# Add to your config:",
        "#   xiuqo completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.xiuqo/profiles",
        "        for d in $HOME/.xiuqo/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c xiuqo -f -s p -l profile"
        " -d 'Profile name' -xa '(__hermes_profiles)'",
        "",
        "# Top-level subcommands"]
    for cmd, info in subcommands:
        lines.append(
            f"complete -c xiuqo -f "
