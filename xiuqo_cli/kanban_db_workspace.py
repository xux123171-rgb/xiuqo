...
def _managed_scratch_path_info(p: Path) -> tuple[bool, Optional[str]]:
    ...
    override = os.environ.get("XIUQO_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    root (``XIUQO_KANBAN_WORKSPACES_ROOT``, ``<kanban_home>/kanban/workspaces``,
    ...
    Xiuqo' own DB and metadata. :func:`_cleanup_workspace` refuses
...
def _cleanup_worktree_workspace(
    ...
    try:
        from xiuqo_cli.worktree_ops import _worktree_has_unpushed_commits, _worktree_is_dirty
    except Exception:
