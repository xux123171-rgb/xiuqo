"""``xiuqo slack manifest`` — generate the Slack app manifest JSON that registers every gateway
...
def _build_full_manifest(
    ...
    Other sections (OAuth scopes, socket mode) are sensible Xiuqo defaults, tweakable in the Slack
    UI after pasting.
    """
    from xiuqo_cli.commands_platforms import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Xiuqo in threads and DMs."}
        ...
        features["agent_view"] = {"agent_description": "Chat with Xiuqo in Slack Messages."}
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Xiuqo agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    """Print or write a Slack app manifest JSON (flags documented in ``xiuqo_cli/main.py``)."""
    name = getattr(args, "name", None) or "Xiuqo"
    description = getattr(args, "description", None) or "Your Xiuqo agent on Slack"
    ...
    def fail(msg: str) -> int:
        print(f"xiuqo slack manifest: {msg}", file=sys.stderr)
        return 2
    ...
    print(
        "\nNext steps:\n"
        "  1. Open https://api.slack.com/apps and pick your Xiuqo app\n"
