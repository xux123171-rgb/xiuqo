...
Imports of xiuqo_cli.auth / config / models stay lazy (tests patch them at call time).
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    from xiuqo_cli.auth import _resolve_api_key_provider_secret, ProviderConfig
    from xiuqo_cli.config import save_env_value
    from xiuqo_cli.models import _PROVIDER_MODELS
    mantle_base_url = f"https://bedrock-mantle.{region}.api.aws/v1"

    # Check env var and credential pool (keys added via `xiuqo auth`)
