...
def pairing_command(args):
    """Handle xiuqo pairing subcommands."""
    ...
    if handler is None:
        print("Usage: xiuqo pairing {list|approve|revoke|clear-pending}")
        print("Run 'xiuqo pairing --help' for details.")
    else:
        handler()
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: xiuqo pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(f"  To reset sooner, delete the '_lockout:{platform}' entry from ~/.xiuqo/platforms/pairing/_rate_limits.json\n")
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'xiuqo pairing list' to see pending requests.\n")
