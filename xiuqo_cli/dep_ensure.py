...
from xiuqo_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import xiuqo_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $XIUQO_HOME/node is not on PATH, so
    ...
    "browser": lambda: (agent_browser_runnable(shutil.which("agent-browser")) or _has_system_browser()
                        or _has_xiuqo_agent_browser() or _has_npx_agent_browser()),
...
def _has_xiuqo_agent_browser() -> bool:
    from xiuqo_constants import get_hermes_home  # late import: tests patch xiuqo_constants.get_hermes_home
    home = get_hermes_home()
    if _IS_WINDOWS:  # npm -g --prefix puts .cmd shims directly in the prefix dir
        return (home / "node" / "agent-browser.cmd").is_file()
    # install.sh installs into $XIUQO_HOME/node/bin/ via npm -g --prefix; legacy git clones used node_modules/.bin/.
...
def ensure_dependency(dep: str, interactive: bool = True) -> bool:
    ...
    run_env = {**xiuqo_subprocess_env(inherit_credentials=False), "IS_INTERACTIVE": "false"}
