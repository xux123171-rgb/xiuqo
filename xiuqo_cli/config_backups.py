...
``<XIUQO_HOME>/backups/config/`` — ``backups/`` is already excluded from full backups, so they
...
``xiuqo setup`` runs or a gateway restarting against broken YAML cannot litter the home dir.
"""
