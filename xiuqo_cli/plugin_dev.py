"""Runtime-backed validation behind ``xiuqo plugins doctor``: every manifest/import/registration
...
def _doctor_runtime(plugin_path: Path):
    ...
    XIUQO_HOME with outbound socket connects blocked.
    """
    stack = ExitStack()
    try:
        # The temp dir enters the stack FIRST so any failure below (ENOSPC / KeyboardInterrupt in
        # copytree) removes it instead of stranding a xiuqo-plugin-doctor-* directory.
        home = Path(stack.enter_context(tempfile.TemporaryDirectory(prefix="xiuqo-plugin-doctor-")))
        ...
        stack.enter_context(patch.dict(os.environ, {
            "XIUQO_HOME": str(home),
            "XIUQO_BUNDLED_PLUGINS": str(bundled),
            "XIUQO_ENABLE_PROJECT_PLUGINS": "0",
        }, clear=False))
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(f"Xiuqo discovery found no valid plugin manifest under {copied}")
        if len(manifests) != 1:
...
def _is_plugin_module(name: str) -> bool:
    return name == "xiuqo_plugins" or name.startswith("xiuqo_plugins.")
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    if _is_plugin_id(raw):
        ...
        try:
            ...
            pass
        candidates.append(Path.cwd() / ".xiuqo" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    import re as _re
    from xiuqo_cli.plugins import SUPPORTED_MANIFEST_VERSION
    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Xiuqo supports "
    ...
    if missing:
        report.warning(
            ...
            + " — Xiuqo never auto-installs plugin dependencies; install manually: pip install "
            + " ".join(f"'{m}'" for m in missing))

    schema = getattr(manifest, "config_schema", {}) or {}
    if schema:
        from xiuqo_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Xiuqo' real scanner and registration path."""
    try:
