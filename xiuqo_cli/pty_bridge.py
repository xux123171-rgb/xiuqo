"""PTY bridge for `xiuqo dashboard` chat tab.
...
``xiuqo_cli.web_server``. POSIX-only: depends on ``fcntl``, ``termios`` and ``ptyprocess`` (native
...
# ui-tui/packages/xiuqo-ink/src/ink/termio/host.ts — keep the two in sync.
PTY_HOST_ENV = "XIUQO_PTY_HOST"
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError("Pseudo-terminals are unavailable on this platform. "
                                          "Xiuqo Agent supports Windows only via WSL.")
