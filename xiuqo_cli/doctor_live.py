"""``xiuqo doctor --live`` — opt-in bounded real-call tool-backend probes.

Opt-in only: these probes make real (cheap, metadata/read-only) network calls and may spend a
trivial amount of quota. They run ONLY when the user passes ``xiuqo doctor --live``.
"""
...
from xiuqo_cli.doctor import _section, check_info
from xiuqo_cli.doctor_report import check_fail, check_ok, check_warn
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from xiuqo_cli.doctor import XIUQO_HOME, PROJECT_ROOT
        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (XIUQO_HOME / "node" / "bin", XIUQO_HOME / "node", XIUQO_HOME / "node_modules" / ".bin"):
    ...
    # probes above. Mirror the rung xiuqo_cli.doctor uses so this probe can't diverge from it, including
