"""xiuqo webhook — manage dynamic webhook subscriptions from the CLI."""
...
def _setup_hint() -> str:
  ...
  Then start the gateway: xiuqo gateway run
"""


def webhook_command(args):
    """Entry point for 'xiuqo webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)
    if not sub:
        print("Usage: xiuqo webhook {subscribe|list|remove|test}")
        print("Run 'xiuqo webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (xiuqo gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: xiuqo webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from xiuqo webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (xiuqo gateway run)")
