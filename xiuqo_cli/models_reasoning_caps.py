...
Split out of ``xiuqo_cli.models``. OpenRouter and
...
``xiuqo_cli.models`` — tests reset them there — and are read/written by attribute name.
...
# that must never block on HTTP — so without a disk copy, `xiuqo -p`, a cron job, or a freshly
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._XIUQO_USER_AGENT})
...
class _CapsSource:
    """One catalog's cache slots on ``xiuqo_cli.models`` plus how to name its URL.
    ...
    def get(self, slot: str):
        ...
        from xiuqo_cli.models_profile_cache import profile_slot_get
    ...
    def set(self, slot: str, value) -> None:
        if not self.per_profile:
            ...
            return
        from xiuqo_cli.models_profile_cache import profile_slot_set
