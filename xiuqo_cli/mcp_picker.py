"""MCP picker — interactive `xiuqo mcp picker` (also the default `xiuqo mcp`)."""
...
from xiuqo_cli.colors import Colors
from xiuqo_cli.cli_output import prompt_yes_no
from xiuqo_cli.curses_ui import curses_single_select
from xiuqo_cli.mcp_catalog import (
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    _say(
        ...
        "Start a new Xiuqo session for changes to take effect."
    )
...
def _configure_tools(name: str) -> None:
    ...
    from xiuqo_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    """Plain-text catalog dump: `xiuqo mcp catalog` output and the non-curses fallback."""
    print()
    if not rows:
    ...
    print()
    _say("  Install: xiuqo mcp install <name>    Picker: xiuqo mcp", Colors.DIM)
    # Manifest-version warnings: the user's Xiuqo is too old to install everything listed.
    future = [d for d in catalog_diagnostics() if d[1] == "future_manifest"]
    if future:
        print()
        for name, _, _msg in future:
            _say(f"  ⚠ '{name}' requires a newer Xiuqo — run `xiuqo update` to install this entry.", Colors.YELLOW)
        print()
    print()


def show_catalog() -> None:
    """`xiuqo mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`xiuqo mcp picker` (and default `xiuqo mcp`) — interactive selector; re-renders after each
    action until ESC/q."""
    while True:
        rows = _build_rows()
...
def install_by_name(identifier: str) -> int:
    """`xiuqo mcp install <name>` — non-interactive entry-point."""
    from xiuqo_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        _say(
            f"  ✗ '{identifier}' is not in the catalog. Run `xiuqo mcp catalog` to see available entries.",
            Colors.RED,
        )
        return 1
