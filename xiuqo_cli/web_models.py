"""Pydantic request/response models for the Xiuqo dashboard web server."""
...
class ImportRequest(BaseModel):
    archive: str
    # --force: the spawned `xiuqo import` has stdin=DEVNULL, so its "Continue? [y/N]" prompt would
...
class ProfileCreate(BaseModel):
    ...
    # Installed async via `xiuqo -p <name> skills install` (skills_hub.SKILLS_DIR is import-time-bound,
    # so XIUQO_HOME can't redirect it); PIDs go back for the UI to poll.
    hub_skills: List[str] = []
...
class ProfileExport(BaseModel):
    ...
    output: str = ""  # archive path; empty → a staging path under XIUQO_HOME
