"""Implementation of the ``xiuqo bundles`` CLI subcommand."""
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  xiuqo bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``xiuqo bundles`` argparse tree (called from xiuqo_cli/main.py, which owns the
    top-level subparser)."""
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `xiuqo bundles list`")
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``xiuqo bundles <subcommand>`` to the right handler."""
