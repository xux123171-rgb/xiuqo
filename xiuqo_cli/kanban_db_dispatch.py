...
Split out of ``xiuqo_cli.kanban_db``; origin-resident helpers are reached
...
if TYPE_CHECKING:
    from xiuqo_cli.kanban_db import Task
...
# ``XIUQO_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``.
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a Xiuqo profile. Expected steady-state
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``xiuqo kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``xiuqo_cli.profiles.profile_exists``, or ``None`` when it cannot be
    imported (local import avoids a cycle; callers fall back to trusting the
    assignee)."""
    try:
        from xiuqo_cli.profiles import profile_exists
    except Exception:
        return None
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real Xiuqo profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (Xiuqo ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``XIUQO_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``xiuqo -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``xiuqo kanban tail`` shows why the task looks stuck.
...
def _module_hermes_argv() -> list[str]:
    """Interpreter-bound Xiuqo CLI invocation (``xiuqo_cli.main`` is the
    console-script target — there is no top-level ``xiuqo`` package)."""
    return [sys.executable, "-m", "xiuqo_cli.main"]


def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Xiuqo shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """argv for a resolved Xiuqo executable path. Windows batch shims
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``xiuqo`` invocation as argv for ``Popen``: ``$XIUQO_BIN``
    (path-like -> absolute; bare names keep PATH semantics, never a
    same-directory file), then ``which("xiuqo")`` (Windows: safe PATH search,
    batch shims fall back to the module form), then ``sys.executable -m
    xiuqo_cli.main`` for shim-less environments (cron, systemd ``User=``,
    launchd). Mirrors ``gateway.run._resolve_hermes_bin``; local because
    ``xiuqo_cli`` sits below ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("XIUQO_BIN", "").strip()
    if env_bin:
    ...
    xiuqo_bin = _safe_which_no_cwd("xiuqo") if _kb._IS_WINDOWS else shutil.which("xiuqo")
    if xiuqo_bin:
        return _hermes_path_argv(xiuqo_bin)
    return _module_hermes_argv()
...
def _resolve_worker_cli_toolsets(xiuqo_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``XIUQO_KANBAN_TASK`` is set.
    """
    if not xiuqo_home:
        return None
    try:
        ...
        token = set_hermes_home_override(xiuqo_home)
        ...
        secret_token = (
            set_secret_scope(build_profile_secret_scope(Path(xiuqo_home)))
        ...
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for XIUQO_HOME=%r (%s)",
            xiuqo_home,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, xiuqo_home: Optional[str]) -> list[str]:
    """Build the ``xiuqo -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_hermes_argv(),
        ...
        "--cli",
        # Workers run under a profile-scoped XIUQO_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(xiuqo_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `xiuqo kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    """Fire-and-forget ``xiuqo -p <profile> chat -q ...`` subprocess.
    ...
    ``XIUQO_KANBAN_DB`` / ``XIUQO_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from xiuqo_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    # Inject XIUQO_HOME so the worker reads the profile-scoped config.yaml:
    # without it the child's get_hermes_home() falls back to the DEFAULT
    # profile root because `xiuqo -p` applies its override before
    # xiuqo_constants is imported.
    try:
        env["XIUQO_HOME"] = resolve_profile_env(profile_arg)
        ...
        strip_launch_profile_env(env, env["XIUQO_HOME"])
    except FileNotFoundError:
        # No profile dir (isolated test fixtures) — the CLI resolves it from
        # XIUQO_PROFILE (set below) instead.
        pass
    if task.tenant:
        env["XIUQO_TENANT"] = task.tenant
    env["XIUQO_KANBAN_TASK"] = task.id
    env["XIUQO_KANBAN_WORKSPACE"] = workspace
    ...
    env["XIUQO_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["XIUQO_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["XIUQO_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["XIUQO_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["XIUQO_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["XIUQO_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # match after `xiuqo -p` rewrites XIUQO_HOME (symlink / Docker layouts).
    env["XIUQO_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["XIUQO_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["XIUQO_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["XIUQO_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads XIUQO_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["XIUQO_PROFILE"] = profile_arg
    ...
    # `--cli` is the highest-precedence TUI override; dropping XIUQO_TUI covers
    # older xiuqo builds on PATH that predate the flag's precedence.
    env.pop("XIUQO_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("XIUQO_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`xiuqo` executable not found on PATH. "
            "Install Xiuqo Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``xiuqo kanban dispatch`` — the standalone
...
from xiuqo_cli import kanban_db as _kb  # noqa: E402
from xiuqo_cli import kanban_db_connect as _kbc  # noqa: E402
from xiuqo_cli import kanban_db_workspace as _kbw  # noqa: E402
