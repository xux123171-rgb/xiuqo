"""Git plumbing for ``xiuqo update``: fork/upstream sync, trampoline-git detection, lockfile/EOL churn cleanup, orphan rescue refs, parked-branch assessment, fetch-failure classification.
...
_BAR = "=" * 68
_UPSTREAM_ADD_CMD = "git remote add upstream https://github.com/xux123171-rgb/xiuqo.git"
...
def _git_stdout(git_cmd, args, cwd, **kw) -> Optional[str]:
    ...
    from xiuqo_cli.update_cmd import _git_run
...
def _prune_orphan_rescue_refs(
    ...
    """Expire old orphan rescue refs (``refs/xiuqo-update-backups/orphan-<branch>-<ts>-<sha>``).
    ...
    with suppress(OSError):
        prefix = f"refs/xiuqo-update-backups/orphan-{branch}-"
...
def _print_parked_branch_skip_warning(git_cmd: list[str], cwd: Path, current_branch: str, target_branch: str, reason: str) -> None:
    ...
    print(
        ...
        f"    git -C {cwd} status\n"
        f"    git -C {cwd} checkout {target_branch} && xiuqo update\n"
...
OFFICIAL_REPO_URLS = {
    "https://github.com/xux123171-rgb/xiuqo.git",
    "git@github.com:xux123171-rgb/xiuqo.git",
    "https://github.com/xux123171-rgb/xiuqo",
    "git@github.com:xux123171-rgb/xiuqo",
}
OFFICIAL_REPO_URL = "https://github.com/xux123171-rgb/xiuqo.git"
...
def _offer_upstream_remote(git_cmd: list[str], cwd: Path, *, assume_yes: bool, input_fn) -> bool:
    ...
    from xiuqo_cli.update_cmd import _add_upstream_remote, _mark_skip_upstream_prompt
    print(
        "\nℹ Your fork is not tracking the official Xiuqo repository.\n"
        "  This means you may miss updates from xux123171-rgb/xiuqo.\n"
    )
    ...
    if not _add_upstream_remote(git_cmd, cwd):
        ...
        return False
    print("  ✓ Added upstream: https://github.com/xux123171-rgb/xiuqo.git")
    return True
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path, *, assume_yes: bool = False, input_fn=None) -> bool:
    ...
    See #97052.
    """
    from xiuqo_cli.update_cmd import _count_commits_between, _has_upstream_remote, _no_prompt_git_kwargs, _should_skip_upstream_prompt
...
def _portable_git_candidates() -> list:
    ...
    profile-scoped XIUQO_HOME), then profile home as a fallback for custom layouts.

    The Xiuqo-managed PortableGit tree lives under the SHARED root (``<root>/git/...``), not the
    profile-scoped XIUQO_HOME (``<root>/profiles/<name>``), so a profile-scoped ``xiuqo update`` must look
    there (monerostar review, #87876).
    """
    from xiuqo_cli.update_cmd import get_default_hermes_root, get_hermes_home
    candidates = []
    with suppress(Exception):
...
def _ensure_non_trampoline_git(git_cmd: list) -> list:
    ...
    from xiuqo_cli.update_cmd import _locate_real_git
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    on update; install.ps1 pins ``false`` but older checkouts never got it and only ``xiuqo update`` can
    ...
    reuses its build-pinned ``install.ps1`` forever — so ``xiuqo update``, which ships with the checkout
