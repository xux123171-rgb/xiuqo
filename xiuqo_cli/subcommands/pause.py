"""``xiuqo pause`` / ``xiuqo resume`` — the global emergency stop.

``pause`` writes the ESTOP sentinel at ``$XIUQO_HOME/ESTOP``; cron, kanban and new gateway
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "Xiuqo paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `xiuqo resume` to lift the pause.")
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  Xiuqo resumed — dispatch picks up on the next tick.")
    else:
        print(f"Xiuqo is not paused (no sentinel at {sentinel_path()}).")
    return 0
