"""``xiuqo peer`` — bot-to-bot DMs across machines/gateways.

A *peer* is another Xiuqo gateway running the ``api_server`` platform; its stock
...
cross-machine twin of ``xiuqo -p <bot> chat --in ~ -c "Bot Chat"``. ``run``/``status``
/``stop`` do the same turn through the async Runs API. Peer labels/URLs live in
config.yaml (``bot_peers``); the key lives in ``~/.xiuqo/.env`` as
``XIUQO_PEER_<NAME>_KEY``. ``<peer>/<profile>`` targets the ``/p/<profile>/`` mirror.
"""
...
def _peer_key_env(name: str) -> str:
    return f"XIUQO_PEER_{name.upper().replace('-', '_')}_KEY"
...
def _request(
    ...
    request_headers = {
        ...
        "User-Agent": "xiuqo-peer-dm"}
    if headers:
        request_headers.update(headers)
    req = urllib.request.Request(url, data=data, method=method, headers=request_headers)
    # The peer URL is user-registered (``xiuqo peer add``); a redirect to a
    ...
    try:
        ...
        if exc.code == 400 and "title" in detail.lower():
            ...
            raise RuntimeError(
                ...
                f"(HTTP 400: {detail}). Update the peer's xiuqo, or unhide the "
                f"session there: PATCH /api/sessions/<id> {{\"hidden\": false}}.") from exc
        raise
    # Real api_server wraps the row: {"object": "xiuqo.session", "session": {...}}.
...
def _parse_target(target: str) -> tuple[str, str | None]:
    ...
    if not peer:
        raise ValueError("Peer name required (xiuqo peer dm <peer>[/<agent>] ...)")
...
def _resolve_peer_target(target: str) -> tuple[str, str | None, dict, str]:
    ...
    if not isinstance(peer, dict) or not peer.get("url"):
        raise LookupError(f"No peer named '{peer_name}'. Run: xiuqo peer list")
    key = _peer_secret(peer_name)
    if not key:
        raise PermissionError(
            f"No API key for peer '{peer_name}'. Set it: xiuqo peer add {peer_name} "
            f"--url <url> --key <key> (or add {_peer_key_env(peer_name)}=<key> to ~/.xiuqo/.env)")
...
def _peer_add(args) -> int:
    ...
    if key:
        ...
        print(f"Peer '{name}' saved ({url}) — key stored as {_peer_key_env(name)} in ~/.xiuqo/.env")
    else:
        print(
            f"Peer '{name}' saved ({url}). No key given — set the peer's API_SERVER_KEY with:\n"
            f"  xiuqo peer add {name} --url {url} --key <key>\n"
            f"  (or add {_peer_key_env(name)}=<key> to ~/.xiuqo/.env)")
    return 0
...
def _peer_list(args) -> int:
    peers = _load_peers()
    if not peers:
        print("No peers registered. Add one: xiuqo peer add <name> --url http://host:port --key <API_SERVER_KEY>")
        return 0
...
def _turn_body(message: str, *, message_key: str, **extra) -> dict:
    """Request body for one turn. ``author`` is added only when a dispatcher set XIUQO_TURN_AUTHOR."""
...
def cmd_peer(args) -> int:
    ...
    if action not in {"dm", "run", "status", "stop"}:
        print("Unknown peer action. See: xiuqo peer --help", file=sys.stderr)
        return 2
    try:
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer", help="Bot-to-bot DMs across machines (peer Xiuqo gateways)",
        description="Register other Xiuqo gateways as peers and message their agents. "
            "'xiuqo peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'xiuqo -p <bot> chat'. "
            "The peer must run the api_server platform; its API_SERVER_KEY is "
            "stored locally as a credential in ~/.xiuqo/.env.",
        epilog=(
            "Examples:\n"
            "  xiuqo peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>\n"
            "  xiuqo peer list\n"
            '  xiuqo peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  xiuqo peer dm spark/researcher "..."   # named profile on a multiplexed peer\n'
            "  xiuqo peer run spark --idempotency-key ticket-123 < long-task.txt\n"
            "  xiuqo peer status spark run_abc123\n"
            "  xiuqo peer stop spark run_abc123\n"
            "  xiuqo peer remove spark\n"
            "\n"
    ...
    add_p.add_argument("--key", default="", help="The peer's API_SERVER_KEY (stored in ~/.xiuqo/.env)")
    ...
    def _remote(name: str, help: str, *, run_id: bool):
        ...
        if run_id:
            sp.add_argument("run_id", help="Run ID returned by 'xiuqo peer run'")
        else:
