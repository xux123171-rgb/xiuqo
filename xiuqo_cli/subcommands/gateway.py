"""``xiuqo gateway`` and ``xiuqo proxy`` subcommand parsers."""
...
from xiuqo_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    _flag(gateway_run, "--no-supervise",
        help="Inside the s6-overlay Docker image, normally `gateway run` is "
            ...
            "if XIUQO_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy", help="Remove legacy xiuqo.service units from pre-rename installs",
        description="Stop, disable, and remove legacy Xiuqo gateway unit files "
            "(e.g. xiuqo.service) left over from older installs. Profile "
            "units (xiuqo-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser("enroll",
        ...
        description="Redeem a single-use enrollment token with a relay connector. "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.xiuqo/.env. "
            "Requires being logged in (xiuqo setup). Not available in managed installs.")
    ...
    gateway_enroll.add_argument("--wake-url", dest="wake_url", default=None,
        help="Phase 5 §5.2 wake URL: a reachable URL the connector pokes "
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.xiuqo/.env and forwarded at provision. "
    ...
    proxy_start.add_argument("--provider", default="nous",
        help="Upstream provider: nous or xai (default: nous). See `xiuqo proxy providers`.")
