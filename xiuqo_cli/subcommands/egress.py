"""``xiuqo egress`` subcommand parser."""
...
def build_egress_parser(subparsers) -> None:
    ...
    # OUTBOUND egress firewall (iron-proxy); `xiuqo proxy` is the separate INBOUND
    ...
    from xiuqo_cli import proxy_cli as _proxy_cli
    _proxy_cli.register_cli(egress_parser)

    def _dispatch_egress(args):  # noqa: ANN001
        # dest='egress_command' stays disjoint from ``xiuqo proxy`` (dest='proxy_command').
