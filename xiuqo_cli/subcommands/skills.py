"""``xiuqo skills`` subcommand parser."""

from __future__ import annotations

from typing import Callable

from xiuqo_cli.subcommands._shared import add_json_flag, add_yes_flag
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_trust = skills_subparsers.add_parser("trust",
        help="Trust a project so its repo-local skills (./.xiuqo/skills, ./.agents/skills) load")
    ...
    skills_reset = skills_subparsers.add_parser("reset",
        ...
        description="Clear a bundled skill's entry from the sync manifest (~/.xiuqo/skills/.bundled_manifest) "
            "so future 'xiuqo update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified", help="List bundled skills you've edited (which `xiuqo update` keeps)",
        description="Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `xiuqo update` reports as user-modified and skips. "
            "Use `xiuqo skills diff <name>` to see changes and `xiuqo skills reset "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description="Write the .no-bundled-skills marker so the installer, "
            "`xiuqo update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description="Remove the .no-bundled-skills marker so bundled skills are seeded "
            "again on the next `xiuqo update`. Pass --sync to re-seed now.")
