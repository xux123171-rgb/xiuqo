...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Xiuqo Agent. Use 'xiuqo debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    xiuqo debug share              Upload debug report (asks for confirmation)
    xiuqo debug share --yes        Skip confirmation (for scripts/CI)
    xiuqo debug share --lines 500  Include more log lines
    xiuqo debug share --expire 30  Keep dpaste.com fallback pastes for 30 days
    xiuqo debug share --local      Print report locally (no upload)
    xiuqo debug share --no-redact  Disable upload-time secret redaction
    xiuqo debug share --nous       Upload to Nous-internal storage (private)
    xiuqo debug delete <url>       Delete a previously uploaded paste
""")
    ...
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'xiuqo debug share'")
