"""``xiuqo update`` subcommand parser."""
...
def build_update_parser(subparsers, *, cmd_update: Callable) -> None:
    ...
    update_parser = subparsers.add_parser(
        "update", help="Update Xiuqo Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        help="Show the update plan and exit without changing anything: install "
            "kind (git/docker/nix), every running Xiuqo service across all "
    ...
    update_parser.add_argument(
        ...
        help="Force a FULL pre-update backup (quick state snapshot + XIUQO_HOME zip) for this run, regardless of updates.pre_update_backup",
    )
    update_parser.add_argument(
        "--yes", "-y", action="store_true", default=False,
        help="Run without blocking on prompts: accepts the config-migration and stash-restore prompts, skips the fork-upstream prompt without adding a remote. API-key entry is skipped; run 'xiuqo config migrate' separately for those.",
    )
    update_parser.add_argument(
    ...
    update_parser.add_argument(
        ...
        help="Update against this branch instead of the default (main). "
            "If the local checkout is on a different branch, xiuqo will "
    ...
    update_parser.add_argument(
        ...
        help="Windows: proceed with the update even when another xiuqo.exe is detected. The concurrent process will likely cause WinError 32 warnings. Does NOT bypass the venv-process guard (see --force-venv).",
    )
    update_parser.add_argument(
