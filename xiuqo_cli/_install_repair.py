...
Callers: ``_early_recovery.recover_if_needed`` (stdlib-only, runs BEFORE ``xiuqo_cli.main``'s
third-party imports so a pending update completes while no native extension is mapped yet) and
``xiuqo_cli.main_install_repair._recover_core_update_marker_locked`` (the post-import recovery path).
...
from xiuqo_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (xiuqo_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``xiuqo acp`` speaks JSON-RPC on stdout; an inherited-fd install child writing there would
...
def _venv_scripts_dir(root: Path) -> Path | None:
    """Project venv Scripts/bin dir, when present (xiuqo_constants is stdlib-only)."""
    # xiuqo_constants is stdlib-only, so the canonical layout helpers are safe to use from this
...
#: Xiuqo root's ``bin``, next to uv.exe). Keep in lockstep with scripts/install.ps1.
_WINDOWS_BIN_LAUNCHERS = ("xiuqo", "xiuqo-acp")
...
def _default_hermes_root() -> Path | None:
    """The DEFAULT Xiuqo root (not ``get_hermes_home()``, which under ``xiuqo -p <name>`` is
...
def ensure_windows_bin_launchers(
    ...
    r"""Re-stage the Windows ``xiuqo`` launchers when they vanish.

    On Windows, ``xiuqo`` resolves through launchers derived from the venv console scripts — never
    ...
    The canonical launcher home is the managed binary dir — the default Xiuqo root's ``bin``
    (``%LOCALAPPDATA%\\xiuqo\\bin``, next to the managed uv) — which lives OUTSIDE the git checkout so no
    git operation can ever touch it. It is a per-machine dir shared by every profile: ``get_hermes_home()``
    would point inside ``profiles\\<name>`` under ``xiuqo -p``, so the anchor here is
    :func:`xiuqo_constants.get_default_hermes_root`. See #83797.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
    ...
    targets: list[Path] = []
    # Runs at every xiuqo_cli.main process start, so the healthy path must stay a few stat calls.
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print("  ✓ Restored xiuqo launcher(s): " + ", ".join(restored), file=sys.stderr)
    return restored
...
def migrate_windows_bin_path(
    ...
    """One-time PATH migration to the ``XIUQO_HOME\\bin`` launcher layout (``xiuqo update`` tail).
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    if kept != entries:
        try:
            ...
            return False
        with contextlib.suppress(OSError, ValueError):
            print(f"  ✓ xiuqo launchers now resolve from {home_bin} "
...
def _quarantine_running_hermes_exe(
    ...
    """Rename live xiuqo*.exe shims aside so the installer can rewrite them.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    (#75584). Warnings land on stderr — this module runs in the early-recovery path and ``xiuqo acp``
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    try:
        ...
        # the shims renamed aside and `xiuqo` gone from PATH. Restore only renames back when the
