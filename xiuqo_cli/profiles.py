"""Profile management for multiple isolated Xiuqo instances."""
...
from xiuqo_cli.archive_safe import archive_root_dirs, make_targz, normalize_archive_parts, safe_extract_targz
from xiuqo_constants import clear_named_profile_deleted, mark_named_profile_deleted, named_profile_is_deleted
...
# stay visible; containers persist HOME state here). See xiuqo_constants.get_subprocess_home().
...
# (``~/.xiuqo``): git checkout (+ ~3 GB venv), worktrees, sibling profiles, shared bins,
...
_CLONE_ALL_DEFAULT_EXCLUDE_ROOT: frozenset[str] = frozenset({
    "xiuqo", ".worktrees", "profiles", "bin", "node_modules",
})

# Per-profile history excluded from --clone-all for ANY source: SQLite session store
# (+wal/shm, can reach many GB), session dirs, `xiuqo backup` archives, quick-backup
...
# Marker written by `xiuqo profile create --no-skills`. When present at a profile root,
# seed_profile_skills() callers (fresh-create, `xiuqo update` all-profile sync, the
...
_PLACEHOLDER_ENV = (
    "# Per-profile secrets for this Xiuqo profile.\n"
...
# Allow-list for ``export_profile("default")``: when XIUQO_HOME equals the cwd
# (Docker/custom deployments) the default home holds arbitrary user files that must NOT
# be bundled. Only known Xiuqo profile artifacts at the root survive; sensitive runtime
...
# when introduced in ``xiuqo_constants``.
# See #58394.
...
_RESERVED_NAMES = frozenset({"xiuqo", "default", "test", "tmp", "root", "sudo"})

# Xiuqo subcommands that cannot be used as profile names/aliases
_XIUQO_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    """Named-profiles root, anchored to the xiuqo root (NOT the current XIUQO_HOME, which
    may itself be a profile) so ``coder profile list`` sees all profiles."""
    return _get_default_hermes_home() / "profiles"


def _get_default_hermes_home() -> Path:
    """Default (pre-profile) XIUQO_HOME: ``~/.xiuqo``, or XIUQO_HOME itself in
...
def _is_our_wrapper(path: Path) -> bool:
    """True when *path* reads as a Xiuqo-generated wrapper (contains ``xiuqo -p``)."""
    try:
        return "xiuqo -p" in path.read_text(encoding="utf-8")
    except Exception:
        return False


def _missing_profile_error(canon: str) -> FileNotFoundError:
    return FileNotFoundError(f"Profile '{canon}' does not exist. Create it with: xiuqo profile create {canon}")
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its XIUQO_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if canon in _XIUQO_SUBCOMMANDS:
        return f"'{canon}' conflicts with a xiuqo subcommand"
    try:
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    try:
        if sys.platform == "win32":
            ...
            xiuqo_exe = shutil.which("xiuqo") or "xiuqo"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(xiuqo_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    if not (profile_dir / "config.yaml").exists():
        return
    # Creation must not fail over an unmigratable old config; `xiuqo doctor --fix` surfaces
    # the detailed error in the target profile.
    with contextlib.suppress(Exception):
        from xiuqo_constants import reset_hermes_home_override, set_hermes_home_override
        from xiuqo_cli.config import check_config_version, migrate_config
...
# wrappers are a few hundred bytes with the ``xiuqo -p X`` needle near the top; the wrapper
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    if config_path.exists():
        return
    with contextlib.suppress(Exception):  # creation must not fail over this; `xiuqo model` sets it later
        import yaml
        from xiuqo_constants import get_hermes_home
        from xiuqo_cli.config import read_user_config_raw
...
def _check_gateway_running(profile_dir: Path) -> bool:
    """Gateway liveness for a profile dir, never mutating XIUQO_HOME.
...
def _served_by_running_multiplexer(profile_name: str) -> bool:
    ...
    """
    try:
        from xiuqo_cli.gateway import named_profile_served_by_running_multiplexer
...
# Deliberately tiny and separate from ``config.yaml`` (user-facing Xiuqo config, ~5000
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """``(profile_name, xiuqo_home)`` pairs a gateway should serve — the single chokepoint
...
def _clone_all_into(source_dir: Path, profile_dir: Path, canon: str) -> None:
    ...
    from xiuqo_cli.auth import strip_cloned_single_use_oauth_grants
...
def create_profile(
    ...
    makes two gateways fight over one bot (``xiuqo_cli.profile_channels``; callers list what
    was left behind with ``channel_platforms_configured(source_dir)``).
    ``no_skills`` creates an empty profile and writes a marker so ``xiuqo update`` skips
    ...
    canon = _canon_valid(name)
    if canon == "default":
        raise ValueError("Cannot create a profile named 'default' — it is the built-in profile (~/.xiuqo).")
    profile_dir = get_profile_dir(canon)
    ...
    if source_dir is not None and not clone_channels:
        from xiuqo_cli.profile_channels import strip_channel_settings
    ...
    # profile-scoped env writes (dashboard Channels/Keys pages, `xiuqo -p <name> auth add`)
    ...
    with contextlib.suppress(Exception):  # best-effort — don't fail profile creation over this
        from xiuqo_cli.default_soul import DEFAULT_SOUL_MD
        _seed_file_if_missing(profile_dir / "SOUL.md", DEFAULT_SOUL_MD)

    # Opt-out marker read by seed_profile_skills() and `xiuqo update`'s all-profile sync
    # (the feature still works via the empty skills/ dir if this fails).
    if no_skills:
        _seed_file_if_missing(
            profile_dir / NO_BUNDLED_SKILLS_MARKER,
            "This profile opted out of bundled-skill seeding (`xiuqo profile create --no-skills`).\n"
    ...
    if description and description.strip():
        with contextlib.suppress(Exception):  # non-fatal — `xiuqo profile describe` works later
    ...
    # `xiuqo -p <profile> gateway start` supervises via `s6-svc -u` instead of a bare
...
def _notify_multiplexer(canon: str) -> None:
    from xiuqo_cli.gateway_multiplex_served import notify_multiplexer_profiles_changed
    notify_multiplexer_profiles_changed(canon)


def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    """Seed bundled skills into a profile via subprocess (sync_skills() caches XIUQO_HOME at
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "XIUQO_HOME": str(profile_dir)},
...
_XIUQO_ARGV_MARKERS = ("xiuqo_cli.main", "xiuqo-gateway", "tui_gateway")
...
# `findOnPath('xiuqo')`) spawns the shim by handing the interpreter its path — then the
# OS-reported argv[0] is the interpreter, not "xiuqo".
...
# argv[1] is matched against exact names, not ``startswith("xiuqo")``: with a bare
# interpreter argv[0], argv[1] can be ANY user script ("xiuqo-notes.py").
_XIUQO_CONSOLE_SCRIPT_NAMES = frozenset({"xiuqo", "xiuqo", "xiuqo-acp"})


def _is_hermes_argv(argv: list) -> bool:
    """True for a Xiuqo process: entrypoint marker in argv, executable named ``xiuqo*``,
    or a python interpreter directly exec'ing a known ``xiuqo`` console-script shim."""
    joined = " ".join(argv)
    exe_name = os.path.basename(argv[0]).lower()
    if any(marker in joined for marker in _XIUQO_ARGV_MARKERS) or exe_name.startswith("xiuqo"):
        return True
    if len(argv) >= 2 and _PYTHON_INTERPRETER_RE.match(exe_name):
        script_name = os.path.basename(str(argv[1])).lower()
        return script_name.rsplit(".", 1)[0] in _XIUQO_CONSOLE_SCRIPT_NAMES
    return False
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Xiuqo *backends* bound to this profile (``gateway.pid`` only tracks
    ...
    # Never terminate ourselves or a parent (`xiuqo -p <canon> profile delete` runs under
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Bound to THIS profile by selector flag, or by XIUQO_HOME pointing at its dir.
            ...
            if not bound:
                with contextlib.suppress(Exception):  # environ() can raise AccessDenied even same-user
                    env_home = (proc.environ() or {}).get("XIUQO_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    canon = normalize_profile_name(name)
    if canon == "default":
        raise ValueError("Cannot delete the default profile (~/.xiuqo).\nTo remove everything, use: xiuqo uninstall")
    ...
    with contextlib.suppress(Exception):
        from xiuqo_state_registry import close_all_under as _close_session_dbs_under
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    Port selection: each supervised profile gateway loads its own ``XIUQO_HOME`` and binds the port
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # XIUQO_HOME is set temporarily so _profile_suffix resolves the service name.
    old_home = os.environ.get("XIUQO_HOME")
    try:
        os.environ["XIUQO_HOME"] = str(profile_dir)
        from xiuqo_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        os.environ.pop("XIUQO_HOME", None)
        if old_home is not None:
            os.environ["XIUQO_HOME"] = old_home
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal: the record's xiuqo_home stamp names the gateway's TRUE
        ...
        if recorded_gateway_home_conflicts(data, expected_home=profile_dir):
            print(
                f"✗ Refusing to stop PID {pid}: its recorded XIUQO_HOME "
...
def get_active_profile_name() -> str:
    """Profile name inferred from XIUQO_HOME: ``"default"`` when unset or ``~/.xiuqo``, the
    name under ``~/.xiuqo/profiles/<name>``, ``"custom"`` for any other path."""
    from xiuqo_constants import get_hermes_home
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    (not cwd) so the check holds when XIUQO_HOME sits in a checkout but the process runs
...
def _profile_export_directory() -> Path:
    ...
    # A custom deployment may point XIUQO_HOME at its source checkout: use a sibling store,
    # falling back to the OS temp dir only when the user's home itself is a checkout (dotfiles
    # repo). Per-uid temp name: a fixed /tmp/xiuqo-profile-exports is a predictable shared
    # path another local user could pre-create (or symlink) first.
    uid_suffix = f"-{os.getuid()}" if hasattr(os, "getuid") else ""
    candidates = (
        Path.home() / ".xiuqo-profile-exports", Path(tempfile.gettempdir()) / f"xiuqo-profile-exports{uid_suffix}"
    )
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in ``_DEFAULT_EXPORT_INCLUDE_ROOT``
    ...
    XIUQO_HOME equals the cwd) is excluded. Blacklisting was tried first and proved unable to anticipate
    every non-Xiuqo file the user may have lying alongside XIUQO_HOME (#58394). * **Universal exclusions
...
def _scrub_export_secrets(staged: Path) -> None:
    """Force-redact secret-shaped strings in a staged export tree (same pass as ``xiuqo
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    # The default profile IS ~/.xiuqo (dir name ".xiuqo"), so both paths stage a filtered
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: xiuqo profile import <archive> --name <name>"
        )
    ...
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.xiuqo). "
            "Specify a different name: xiuqo profile import <archive> --name <name>"
        )
    profile_dir = get_profile_dir(canon)
    if profile_dir.exists():
        raise FileExistsError(f"Profile '{canon}' already exists at {profile_dir}")
    _get_profiles_root().mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="xiuqo_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    legacy_old_host = f"xiuqo.{old_name}"
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            block["aiPeer"] = old_name  # source_host is ``hermes_<old>`` or legacy ``xiuqo.<old>``
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a XIUQO_HOME path string. Called early in the CLI entry
    point, before xiuqo modules are imported, to set XIUQO_HOME.

    When XIUQO_HOME is already set, the configured spelling IS the launch root (it may be a
    junction/symlink alias of the platform default). Keep that spelling so profile re-home does not destroy
    the launcher's lexical provenance -- the subprocess sanitizer needs it to match Xiuqo-owned PYTHONPATH
    ...
    env_home = os.environ.get("XIUQO_HOME", "").strip()
    if env_home:
