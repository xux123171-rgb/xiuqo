"""``xiuqo doctor`` — diagnose (and with --fix, repair) a Xiuqo install.
...
from xiuqo_cli.config import get_env_path, get_hermes_home, get_project_root
...
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.xiuqo or ~/.xiuqo/profiles/coder)

# Load environment variables from ~/.xiuqo/.env so API key checks work
_env_path = get_env_path()
load_hermes_dotenv(xiuqo_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from xiuqo_cli.colors import Colors, color
from xiuqo_cli.doctor_report import Finding, _section, check_bool, check_info, doctor_check, warn_on_error
from xiuqo_cli.doctor_connectivity import _has_healthy_oauth_fallback_for_apikey_provider, build_probes, run_probes
from xiuqo_cli.doctor_tools import _safe_which

from xiuqo_cli.doctor_config import (
    _check_config_drift,
    _check_config_file,
    _check_env_file,
    _check_mcp_security,
...
from xiuqo_cli.doctor_platform import (
    _check_certificates,
...
from xiuqo_cli.doctor_tools import (
    _check_git_and_rg,
...
from xiuqo_cli.doctor_state import (
...
def _check_auth_providers(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("Auth provider status", "(could not check: {e})"):
        from xiuqo_cli.auth import get_nous_auth_status_local, get_codex_auth_status, get_minimax_oauth_auth_status
        _login_row("Nous Portal auth", get_nous_auth_status_local())
        # Native OAuth is Xiuqo' own device-code flow; the Codex CLI only imports existing ~/.codex/auth.json
    ...
    with warn_on_error(""):  # xAI OAuth separately, so an import failure cannot disrupt the rows already printed
        from xiuqo_cli.auth import get_xai_oauth_auth_status
...
def _ack_advisory(ack_target: str) -> None:
    """`xiuqo doctor --ack <id>`: persist the ack and return without running diagnostics."""
    from xiuqo_cli.security_advisories import ADVISORIES, ack_advisory
    ...
    if ack_advisory(ack_target):
        ...
        print(color(f"  ✗ Failed to persist ack for {ack_target}. Check ~/.xiuqo/config.yaml is writable.", Colors.RED))
        sys.exit(1)
...
def _print_summary(should_fix: bool, total: Finding) -> None:
    ...
    if should_fix and total.fixed > 0:
        ...
        print(numbered)
        if not should_fix:
            print(color("  Tip: run 'xiuqo doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
...
def run_doctor(args):
    ...
    os.environ.setdefault("XIUQO_INTERACTIVE", "1")
    ...
    with warn_on_error(""):
        from xiuqo_cli.doctor_live import maybe_run_live_checks
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'OPENROUTER_MODELS_URL': ('xiuqo_constants', 'OPENROUTER_MODELS_URL'),
    'STATE_DB_SIZE_WARN_BYTES': ('xiuqo_cli.doctor_state', 'STATE_DB_SIZE_WARN_BYTES'),
    'agent_browser_runnable': ('xiuqo_constants', 'agent_browser_runnable'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'check_certificates': ('xiuqo_cli.doctor_platform', 'check_certificates'),
    'check_macos_full_disk_access': ('xiuqo_cli.doctor_platform', 'check_macos_full_disk_access'),
    'check_macos_tcc_anchor': ('xiuqo_cli.doctor_platform', 'check_macos_tcc_anchor'),
    'check_macos_tcc_grants': ('xiuqo_cli.doctor_platform', 'check_macos_tcc_grants'),
    'collect_deprecated_config_keys': ('xiuqo_cli.doctor_config', 'collect_deprecated_config_keys'),
    'collect_deprecated_env_vars': ('xiuqo_cli.doctor_config', 'collect_deprecated_env_vars'),
    'collect_relay_plugin_cutover_findings': ('xiuqo_cli.doctor_config', 'collect_relay_plugin_cutover_findings'),
    'describe_vercel_auth': ('xiuqo_cli.vercel_auth', 'describe_vercel_auth'),
    'detect_install_method': ('xiuqo_cli.config', 'detect_install_method'),
    'is_nix_install_method': ('xiuqo_cli.config', 'is_nix_install_method'),
    'managed_scope_check': ('xiuqo_cli.doctor_config', 'managed_scope_check'),
    'recommended_update_command_for_method': ('xiuqo_cli.config', 'recommended_update_command_for_method'),
    'report_deprecated_config_and_env': ('xiuqo_cli.doctor_config', 'report_deprecated_config_and_env'),
}
