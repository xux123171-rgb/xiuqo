...
def _strip_hermes_path_lines(content: str) -> str:
    """Drop the ``# Xiuqo Agent`` marker (+ its PATH line) and any xiuqo PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# Xiuqo Agent' in line or '# xiuqo' in line:
            skip_next = True
            continue
        if skip_next and ('xiuqo' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'xiuqo' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove Xiuqo PATH entries from shell configuration files."""
    removed_from = []
...
def remove_wrapper_script():
    """Remove the xiuqo wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a xiuqo_cli / xiuqo reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('xiuqo_cli' in content or 'xiuqo' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("xiuqo", "xiuqo-acp", "xiuqo"))
...
def remove_node_symlinks(xiuqo_home: Path) -> list:
    ...
    node_dir = (xiuqo_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from xiuqo_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_launchd_gateway() -> bool:
    ...
    from xiuqo_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from xiuqo_cli import gateway_windows as gw
...
# XIUQO_HOME / XIUQO_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\xiuqo\git\{cmd,bin,usr\bin}, ...\xiuqo\node), PortableGit + Node copies
...
def _hermes_path_markers(xiuqo_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Xiuqo-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(xiuqo_home).rstrip("\\/")
    subs = ("xiuqo", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(xiuqo_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Xiuqo-owned entries from User-scope PATH in the registry (see ``_hermes_path_markers``)."""
    markers = tuple(m.lower() for m in _hermes_path_markers(xiuqo_home, include_managed_bin=include_managed_bin))
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete XIUQO_HOME and XIUQO_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("XIUQO_HOME", "XIUQO_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(xiuqo_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``xiuqo_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (xiuqo_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``xiuqo`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from xiuqo_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
...
def _is_default_hermes_home(xiuqo_home: Path) -> bool:
    """Return True when ``xiuqo_home`` points at the default (non-profile) root."""
    try:
        from xiuqo_constants import get_default_hermes_root
        return xiuqo_home.resolve() == get_default_hermes_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``xiuqo -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current XIUQO_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m xiuqo_cli.main` (the `xiuqo` wrapper may be half-gone).
    xiuqo_invocation = [sys.executable, "-m", "xiuqo_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                xiuqo_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's XIUQO_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``xiuqo uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from xiuqo_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    xiuqo_home = get_hermes_home()
    summary = gui_install_summary(xiuqo_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ☤ Xiuqo Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No Xiuqo Chat GUI installation was found.")
        print(f"  Checked: {xiuqo_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Xiuqo agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(xiuqo_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Xiuqo agent at {xiuqo_home / 'xiuqo'}")
        print(f"  • Your config, sessions, and secrets under {xiuqo_home}")
        print()
    ...
    print()
    uninstall_gui(xiuqo_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Xiuqo agent is still installed. Run 'xiuqo' to use the CLI,")
    print("or 'xiuqo uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.xiuqo/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, xiuqo_home=xiuqo_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie XIUQO_HOMEs and systemd units behind.
    is_default_profile = _is_default_hermes_home(xiuqo_home)
    ...
    print()
    _print_box("│            ☤ Xiuqo Agent Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {xiuqo_home / 'config.yaml'}")
    print(f"  Secrets: {xiuqo_home / '.env'}")
    print(f"  Data:    {xiuqo_home / 'cron/'}, {xiuqo_home / 'sessions/'}, {xiuqo_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, XIUQO_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Xiuqo data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Xiuqo code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, xiuqo_home=xiuqo_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, xiuqo_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Xiuqo PATH entries from shell configs / Windows User PATH")
    print("  • Xiuqo wrapper scripts and Xiuqo-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Xiuqo config/data: {xiuqo_home}")
    else:
        print(f"  • Xiuqo config/data: {xiuqo_home}")
        profiles = _discover_named_profiles() if _is_default_hermes_home(xiuqo_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$XIUQO_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: xiuqo_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; xiuqo\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_hermes_home(xiuqo_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(xiuqo_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Xiuqo-owned PATH entries in User environment"),
        (windows, "Removing XIUQO_HOME / XIUQO_GIT_BASH_PATH User env vars...",
         remove_hermes_env_vars_windows, "Removed User env var: {}", "No Xiuqo-set User env vars to remove"),
        (True, "Removing xiuqo command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows xiuqo launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows xiuqo launchers found"),
        (True, "Removing Xiuqo-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(xiuqo_home), "Removed {}", "No Xiuqo-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE XIUQO_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from xiuqo_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(xiuqo_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(xiuqo_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.xiuqo/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(xiuqo_home)
    else:
        log_info(f"Keeping configuration and data in {xiuqo_home}")
    ...
    print()
    print("Thank you for using Xiuqo Agent! ☤")
    print()


_REINSTALL_HINT = {
    True: "  iex (irm https://raw.githubusercontent.com/xux123171-rgb/xiuqo/main/scripts/install.ps1)",
    False: "  curl -fsSL https://raw.githubusercontent.com/xux123171-rgb/xiuqo/main/scripts/install.sh | bash"}
...
def main(argv=None) -> int:
    """``python -m xiuqo_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``xiuqo_constants`` + ``xiuqo_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m xiuqo_cli.uninstall")
    parser.add_argument(
