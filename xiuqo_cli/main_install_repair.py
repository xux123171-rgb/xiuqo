...
# Install-scoped breadcrumbs live next to the venv (not under $XIUQO_HOME)
...
def _stdout_to_stderr():
    ...
    a protocol stream (``xiuqo acp`` speaks JSON-RPC on stdout) must never get install noise."""
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``xiuqo update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    from xiuqo_cli.main import PROJECT_ROOT
    print(
        "⚠ A previous `xiuqo update` was interrupted mid-install — "
        "finishing dependency installation now...")

    # Windows: a ``xiuqo.exe`` launch has the launcher as an ancestor; the quarantined full
    ...
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from xiuqo.exe; applying package-only first aid, "
    ...
    try:
        from xiuqo_cli import _install_repair as _ir
        ...
        manual = (
            "  Xiuqo is still running from the launcher that needs "
            "replacing. Close other Xiuqo windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\xiuqo.exe`` holds itself open (no ``FILE_SHARE_DELETE``) for the whole
    ...
    with the project venv's own shims so a foreign ``xiuqo.exe`` never matches.
...
def _windows_running_hermes_launcher_locked() -> bool:
    """True when a venv ``xiuqo*.exe`` shim is this process or an ancestor (best-effort)."""
    return _windows_shim_in_process_chain() is not None


# Set on the re-exec'd child so it can never spawn another one.
_UPDATE_REEXEC_ENV = "XIUQO_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    ``.update_exit_code``. The child re-runs ``xiuqo update`` so the sync and its tail happen
    ...
    user's own console.
    ``venv\\Scripts\\xiuqo.exe`` is a launcher that runs the interpreter with the shim as its script and
    ...
    cmd = [str(python_exe), "-m", "xiuqo_cli.main", *sys.argv[1:]]
...
def _run_install_with_heartbeat(
    ...
    try:
        ...
        # is imported when `xiuqo update` starts, so an update running
...
def _venv_scripts_dir() -> Path | None:
    ...
    from xiuqo_cli.main import PROJECT_ROOT
    from xiuqo_constants import project_venv_dir, venv_bin_dir
...
def _quarantine_running_hermes_exe(
    ...
    """Pre-empt the Windows file lock on the running ``xiuqo.exe``.
    ...
    transient; Xiuqo Desktop backend child: until closed) — retry with backoff, then warn
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Xiuqo Desktop, exit other `xiuqo` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `xiuqo update`.")
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Xiuqo versions queued for our shims: ``MOVEFILE_DELAY_UNTIL_REBOOT``
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    critical: a failed quarantine only aborts an update; a failed restore leaves no ``xiuqo``
...
class ShimQuarantineError(RuntimeError):
    """A live ``xiuqo*.exe`` shim could not be renamed aside. Raised by
...
def _run_quarantined_install(
    ...
    """Run an editable install, quarantining the running ``xiuqo.exe`` first.

    Every editable install rewrites the entry-point shims; on Windows the live ``xiuqo.exe``
    can be neither deleted nor overwritten, so without quarantine ``xiuqo`` drops off PATH.
...
# rewrites no entry points, which would otherwise leave the shims quarantined aside and `xiuqo` missing
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``xiuqo.exe.old.*`` from updates.

    Called early on every invocation. Two cases an unconditional ``unlink()`` gets wrong:
    (1) orphan rescue — ``xiuqo.exe`` missing while ``xiuqo.exe.old.*`` exists means the .old
...
def _run_package_only_install(cmd: list[str], *, env: dict[str, str] | None = None) -> None:
    ...
    ``xiuqo.exe``, and the quarantine path would rename shims uv then never recreates.
...
def _repair_venv_via_import_probes(
    ...
    ``xiuqo.exe``. Never raises. Returns ``"healthy"``, ``"repaired"``, ``"failed"``
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `xiuqo update`:")
...
def _interpreter_scripts_dir() -> Path | None:
    ...
    See #76105.
    """
    from xiuqo_constants import venv_bin_dir
...
def _install_python_dependencies_with_optional_fallback(
    ...
    attempt quarantines the live ``xiuqo*.exe`` shims first. When ``env`` carries a
    ...
    if pin_python:
        ...
        # dir, not PROJECT_ROOT/venv; quarantining the wrong dir leaves xiuqo.exe locked.
...
def _verify_console_scripts_installed(
    ...
    On Windows ``uv pip install -e .`` can register ``xiuqo.exe`` in the wheel RECORD while the
    file never lands (live shim locked, launcher write skipped), so ``xiuqo`` drops off PATH
    after a "successful" install. Missing shims get ``--reinstall -e .`` under quarantine.

    The symptom is ``xiuqo.exe`` and ``xiuqo-acp.exe`` present but ``xiuqo.exe`` missing, so
    ``xiuqo`` drops off PATH even though the install reported success (issue #52931).
    """
    if not _is_windows():
        return
    scripts_dir = _venv_scripts_dir()
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            "  ⚠ Entry point repair failed; try `xiuqo update --force` after "
            "closing other xiuqo processes.")):
        return
    _report_still_missing(
        _missing(), "Workaround: python -m xiuqo_cli.main <command>",
...
def _verify_core_dependencies_installed(
    ...
    # ``xiuqo update``; the install prefix/env encode which environment we targeted.
    venv_python = _resolve_install_target_python(install_cmd_prefix, env)
    if venv_python is None:
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg="  ⚠ Repair install failed; check `xiuqo update` output above."):
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            ...
            "Run `xiuqo update --force` after closing other xiuqo processes.")):
        return
    _report_still_missing(
        _missing_deps(), "Run `xiuqo update --force` after closing other xiuqo processes.",
