...
def _session_db_read_probe_statements() -> tuple:
    ...
    from xiuqo_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from xiuqo_state import SessionDB, is_malformed_schema_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    ``xiuqo serve`` runs neither CLI nor gateway startup hooks, so this
