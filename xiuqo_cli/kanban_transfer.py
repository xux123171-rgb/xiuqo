...
Backs ``xiuqo kanban export|import``, the ``/boards/{slug}/export`` and
...
from xiuqo_cli.archive_safe import (
    archive_root_dirs,
    copy_regular_files,
    make_targz,
    safe_extract_targz,
...
ARCHIVE_FORMAT = "xiuqo-kanban-board"
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manifest = {
            ...
            "xiuqo_version": str(xiuqo_version),
...
def _read_manifest(root: Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise ValueError("archive is not a Xiuqo kanban board export (no manifest.json)")
    try:
    ...
    if not isinstance(manifest, dict) or manifest.get("format") != ARCHIVE_FORMAT:
        raise ValueError(
            "archive is not a Xiuqo kanban board export "
    ...
    if not isinstance(version, int) or version > ARCHIVE_FORMAT_VERSION:
        raise ValueError(
            f"archive format version {version!r} is newer than this Xiuqo "
            f"understands (max {ARCHIVE_FORMAT_VERSION}) — update Xiuqo and retry"
        )
    return manifest
    ...
    *,
    ...
    return {
        ...
        "source": {
            ...
            "xiuqo_version": manifest.get("xiuqo_version"),
        },
        "counts": counts,
