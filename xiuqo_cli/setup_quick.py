"""Streamlined setup flows: the Nous Portal one-shot (`xiuqo portal`), first-time quick setup,
Blank Slate setup and the `--quick` missing-items pass. Names from setup.py are imported lazily
per function so test patches on ``xiuqo_cli.setup`` take effect."""
...
logger = logging.getLogger("xiuqo_cli.setup")
...
def _blank_slate_done(config: dict, xiuqo_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from xiuqo_cli.setup import _info, _print_setup_summary, print_success
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         xiuqo skills opt-in --sync",
          "  Add MCP servers:     xiuqo mcp add", *extra, "  Tune agent settings: xiuqo setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from xiuqo_cli.setup import load_config
...
def _run_nous_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    ...
    single source of truth shared with ``xiuqo model``. False when cancelled or failed (the
    message is already printed)."""
    from xiuqo_cli.setup import _info
    try:
        from xiuqo_cli.model_setup_flows import _model_flow_nous
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Nous Portal setup (``xiuqo setup --portal`` / ``xiuqo portal``)."""
    from xiuqo_cli.setup import _info, _print_banner, print_error, print_info, print_success
    _print_banner("│     ☤ Xiuqo Setup — Nous Portal (one-shot)             │")
    ...
    def _on_error(exc: Exception) -> None:
        ...
        print_info("  You can retry later with `xiuqo portal`.")

    if not _run_nous_flow(config, context="`xiuqo portal`", cancel_exc=(KeyboardInterrupt, EOFError, SystemExit),
                          cancel_lines=(None, "  Setup cancelled.", "  You can retry later with `xiuqo portal`."),
    ...
    _info("  Run `xiuqo portal info` to inspect routing.", "  Run `xiuqo` to start chatting.")


def _run_first_time_quick_setup(config: dict, xiuqo_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice("Connect a messaging platform? (Telegram, Discord, etc.)", [
        "Set up messaging now (recommended)", "Skip — set up later with 'xiuqo setup gateway'",
    ], 0)
    if gateway_choice == 0:
        ...
        # platforms come alive as soon as tokens are added later (e.g. via `xiuqo import`).
        from xiuqo_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    print()
    print_success("Setup complete! You're ready to go.")
    _info(None, "  Configure all settings:    xiuqo setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  xiuqo setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and Xiuqo.app if you use Desktop), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every Xiuqo update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``xiuqo`` skill
    ...
    try:
        ...
        from xiuqo_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
        all_keys = {k for k, _, _ in CONFIGURABLE_TOOLSETS}
        all_keys.update(_get_plugin_toolset_keys())
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "xiuqo-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("xiuqo-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``xiuqo setup agent``."""
...
def _run_blank_slate_setup(config: dict, xiuqo_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `xiuqo` skill is always kept so the agent",
          "can help you drive and configure Xiuqo itself.", None)
    ...
    if path != 0:
        _blank_slate_walkthrough(config, xiuqo_home)
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `xiuqo update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, xiuqo_home, "  Enable tools:        xiuqo tools", "  Enable plugins:      xiuqo plugins",
                      intro="Enable anything later, on demand:")


def _blank_slate_walkthrough(config: dict, xiuqo_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `xiuqo`",
              "skill). A .no-bundled-skills marker keeps future",
              "`xiuqo update` runs from re-injecting them. Opt back in any",
              "time with `xiuqo skills opt-in --sync`.")
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `xiuqo tools`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `xiuqo plugins list` / `xiuqo plugins install`.",
         "No plugins enabled. Add later with `xiuqo plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `xiuqo mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `xiuqo mcp add`."),
    ):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, xiuqo_home, "  Enable more tools:   xiuqo tools")


def _run_quick_setup(config: dict, xiuqo_home):
    ...
    from xiuqo_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'xiuqo setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Xiuqo to messaging apps to chat from anywhere.",
              "You can configure these later with 'xiuqo setup gateway'.")
