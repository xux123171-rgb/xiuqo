...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with xiuqo)
2. User plugins: ``$XIUQO_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``xiuqo_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$XIUQO_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$XIUQO_HOME/plugins/model-providers/`` if it exists."""
    try:
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$XIUQO_HOME/plugins/`` if it exists.

    This is where ``xiuqo plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple XIUQO_HOME profiles don't alias each other.
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``xiuqo_agent.plugins``
    entry-point group so they self-register.

    A distribution ships::

        [project.entry-points."xiuqo_agent.plugins"]
    ...
    * **Provider targets only.** The ``xiuqo_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$XIUQO_HOME``) keep their
    ...
    try:
        from xiuqo_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$XIUQO_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``xiuqo plugins install`` at
          ``$XIUQO_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``xiuqo_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$XIUQO_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with xiuqo.
    ...
    # 2. User plugins — under $XIUQO_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``xiuqo plugins install`` / the plugin index.
    #     Those clone into $XIUQO_HOME/plugins/<name>/ — flat, NOT under
