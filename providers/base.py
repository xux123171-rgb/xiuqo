...
def _profile_user_agent() -> str:
    """Return a ``xiuqo-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from xiuqo_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"xiuqo-cli/{_ver}"
    except Exception:
        return "xiuqo-cli"
...
class ProviderProfile:
    ...
    # describe how to launch it; xiuqo_cli/auth.py's
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.xiuqo/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # xiuqo-cli UA so the catalog endpoint is reachable.
