"""
Xiuqo MCP Server — expose messaging conversations as MCP tools (`xiuqo mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Xiuqo-specific
channels_list. Client config: {"mcpServers": {"xiuqo": {"command": "xiuqo", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("xiuqo.mcp_serve")
...
def _hermes_home() -> Path:
    try:
        ...
        return Path(os.environ.get("XIUQO_HOME", Path.home() / ".xiuqo"))
...
class EventBridge:
    ...
    event queue with waiter support (the Xiuqo analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Xiuqo MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("xiuqo", instructions=(
        "Xiuqo Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Xiuqo MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
