...
an unlinked SQLite DB/WAL/SHM generation.  ``xiuqo_state`` supplies only the
...
_XIUQO_EXECUTABLES = frozenset({"xiuqo", "xiuqo", "xiuqo-acp"})
_XIUQO_PYTHON_MODULES = frozenset({"acp_adapter", "xiuqo_cli.main"})
_XIUQO_PYTHON_SCRIPTS = frozenset({"xiuqo_cli/main.py", "run_agent.py"})
...
def _looks_like_hermes(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Xiuqo execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _XIUQO_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _XIUQO_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _XIUQO_PYTHON_SCRIPTS
    )
...
def _argv_scoped_to_other_home(argv: Sequence[str], db_path: Path) -> bool:
    ...
    ``state.db`` lives at the XIUQO_HOME root, so an absolute-path token
    containing a ``/.xiuqo`` segment (or naming a ``state.db``/WAL/SHM under
    some other parent) identifies that token's own Xiuqo home.  When at least
    ...
    of ours (issue #92401: a second gateway under /home/demo/.xiuqo deferred
    ...
    for token in argv:
        ...
        if path_token is not None:
            ...
            if normalized in ours or normalized.startswith(this_home + os.sep):
                return False
            if "/.xiuqo" in normalized or normalized.endswith("/.xiuqo"):
