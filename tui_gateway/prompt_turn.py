...
class _TurnScopes:
    ...
    home: Any = None  # per-turn XIUQO_HOME override for a resumed remote profile
    secret: Any = None
    terminal: Any = None
...
def _route_turn_images(agent, prompt: Any, images: list[str]) -> Any:
    ...
    try:
        ...
        from xiuqo_cli.config import load_config as _tui_load_config
    ...
    try:
        ...
        def _thinking_should_play() -> bool:
            if is_audio_output_active():
                return False
            try:
                from xiuqo_cli.voice import is_continuous_active
...
def _after_complete_turn(sid: str, session: dict, st: _TurnRun, raw: Any) -> None:
    ...
    if st.tts_queue is None and isinstance(raw, str) and raw.strip() and _voice_tts_enabled():
        try:
            ...
            logger.warning("voice TTS skipped: xiuqo_cli.voice unavailable")
...
def _finish_turn(sid: str, session: dict, st: _TurnRun) -> None:
    ...
    if isinstance(run_kwargs, dict):
        run_kwargs.clear()
    try:  # while the profile XIUQO_HOME override is still active (session's own config)
