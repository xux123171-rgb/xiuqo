...
def _session_home(session: dict) -> Path:
    """The XIUQO_HOME the session's durable state lives in (profile-aware)."""
...
def _maybe_schedule_auto_continue(sid: str, session: dict, session_key: str) -> dict | None:
    ...
    def kickoff() -> None:
        ...
        # Ownership admission BEFORE message.start: a sibling backend sharing this XIUQO_HOME may have written the
