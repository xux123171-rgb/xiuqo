...
def _profile_build_scope(profile_home):
    """Bind XIUQO_HOME + secret scope for an agent build (home alone leaves get_secret() on the LAUNCH .env)."""
    if not profile_home:
        yield
        return
...
def _billing_call(rid, fn, extra: dict | None = None) -> dict:
    ...
    from xiuqo_cli.nous_billing import BillingError
    try:
        return _ok(rid, fn())
...
def _persist_branch(db, new_key: str, parent_key: str, title: str, history: list, *, source, cwd, profile_name,
    ...
    try:
        ...
        from xiuqo_state_errors import is_disk_full_error
...
def _(rid, params: dict) -> dict:
    ...
    # ``profile`` (app-global remote mode): stored so the build and every turn re-bind XIUQO_HOME.
...
def _resume_guard(ctx: _Resume) -> dict | None:
    ...
    from xiuqo_state import SessionResumeTooLargeError, resolved_max_resume_messages
    ...
    with _session_resume_lock:
        ...
        try:
            ...
            if (session := _sessions.get(sid)) is not None:
                ...
                # Each turn re-binds XIUQO_HOME (mid-turn memory/skills reads); lease claimed lazily on turn 1.
...
def _(rid, params: dict) -> dict:
    ...
    if not (raw := _str_param(params, "cwd")):
        return _err(rid, 4016, "cwd required")
    from xiuqo_constants import translate_cwd_for_wsl_backend
...
def _(rid, params: dict) -> dict:
    ...
    # the WS-detached sentinel: detached is attachable until grace-reap, and ``xiuqo --tui`` rides stdio.
...
def _(rid, params: dict, slug: str) -> dict:
    ...
    from agent.pet import store
    from xiuqo_cli.pets import _clear_active_if
...
def _pet_config_followup(what: str, fn, *args) -> None:
    """Best-effort ``xiuqo_cli.pets`` active-slug update after a store op that already succeeded."""
    try:
        fn(*args)
    except Exception as exc:  # noqa: BLE001
...
def _(rid, params: dict, slug: str) -> dict:
    ...
    if new_slug != slug:
        from xiuqo_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    from xiuqo_cli.pets import _set_enabled
    _set_enabled(False)
...
def _(rid, params: dict) -> dict:
    ...
    from xiuqo_cli.pets import set_pet_scale
...
def _(rid, params: dict) -> dict:
    ...
    from xiuqo_cli.nous_billing import post_subscription_preview
...
def _billing_route(name: str, call, *, invalid=None, message: str = "", error: str = "invalid_request",
    ...
    """Portal write route on ``xiuqo_cli.nous_billing`` (lazy; tests patch its functions): ``invalid(params)``
...
def _(rid, params: dict) -> dict:
    ...
    def call():
        from xiuqo_cli.auth import step_up_nous_billing_scope
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    lines = [
        "Xiuqo TUI Status", "", *status_lines(fields, "session_id", "path"),
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    path = saved_dir / f"xiuqo_conversation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with session["history_lock"]:
        messages = list(session.get("history", []))
