...
# Stop a ``utils/`` (or ``proxy/``, ``ui/``) package in the launch directory from shadowing Xiuqo's own
...
# retry loop. ``xiuqo_bootstrap`` lives at the repo root (no collision risk), so importing it first is safe.
# ``xiuqo_bootstrap`` lives at the repo root, so importing it is safe before the guard runs (its name won't
...
_WATCHDOG_POLL_S = max(0.05, env_float("XIUQO_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, env_float("XIUQO_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _prepare_slash_worker_runtime() -> None:
    ...
    own process — the parent ``xiuqo serve`` discovery thread does not populate this registry.

    See #61891.
    """
    from xiuqo_cli.mcp_startup import start_background_mcp_discovery, wait_for_mcp_discovery
...
def main():
    ...
    args = p.parse_args()
    os.environ["XIUQO_SESSION_KEY"] = args.session_key
    os.environ["XIUQO_INTERACTIVE"] = "1"
