...
activate with `xiuqo --tui` or `XIUQO_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
themselves come from `xiuqo_cli/commands.py` (`xiuqo_cli/AGENTS.md`).
...
npm run dev       # watch mode (rebuilds xiuqo-ink + tsx --watch)
npm start         # production
npm run build     # full build (xiuqo-ink + tsc)
