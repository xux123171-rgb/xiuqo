...
def _set_model(rid, params, key, value, session):
    ...
    if session:
        from xiuqo_cli.model_switch import parse_model_switch_args
...
def _set_focus(rid, params, key, value, session):
    ...
    from xiuqo_cli.focus_view import FOCUS_TOOL_PROGRESS_MODE, normalize_tool_progress_mode, resolve_focus_arg
    d_f = _display_cfg()
...
def _set_yolo(rid, params, key, value, session):
    ...
    if scope == "global":
        ...
        enable = _BOOL_WORDS.get(raw, not is_truthy_value(os.environ.get("XIUQO_YOLO_MODE")))
        if enable:
...
def _set_personality(rid, params, key, value, session):
    ...
    # Persists via xiuqo_cli.personality (single owner), never the user-owned system prompt.
    from xiuqo_cli.personality import persist_personality
