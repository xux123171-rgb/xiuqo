...
# ── Voice state: XIUQO_VOICE / XIUQO_VOICE_TTS are runtime-only env flags (never config.yaml)
...
def _voice_mode_enabled() -> bool:
    return os.environ.get("XIUQO_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    return os.environ.get("XIUQO_VOICE_TTS", "").strip() == "1"
...
def _end_voice_chat(*, stop_loop: bool, stop_tts: bool) -> None:
    ...
    os.environ["XIUQO_VOICE"] = os.environ["XIUQO_VOICE_TTS"] = "0"
    if stop_loop:
        with contextlib.suppress(Exception):
            from xiuqo_cli.voice import stop_continuous
            stop_continuous()
    if stop_tts:
        with contextlib.suppress(Exception):
...
def _speak_text_with_barge(text: str) -> None:
    ...
    from xiuqo_cli.voice import speak_text
    stop, done = threading.Event(), threading.Event()
    with _fd_listener_lock:
...
# ── Wake word ("Hey Xiuqo"): process-global detector (one mic). The first eligible transport
...
def _(rid, params: dict) -> dict:
    ...
    from xiuqo_cli.active_sessions import PER_SESSION_EXCLUSIVE_SUBMIT
...
def _voice_toggle_mode(rid, params: dict) -> dict:
    ...
    os.environ["XIUQO_VOICE"] = "1" if enabled else "0"
    stop_hint = ""
    if enabled:
...
def _set_voice_tts(on: bool) -> None:
    ...
    os.environ["XIUQO_VOICE_TTS"] = "1" if on else "0"
    if not on:
        _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from xiuqo_cli.voice import start_continuous
        ...
        with contextlib.suppress(Exception):
            from xiuqo_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        import xiuqo_cli.voice  # noqa: F401  (a missing module must answer 5026, not die in a thread)
    except Exception as e:
