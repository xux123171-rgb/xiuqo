...
def _kb_board_key(_kb, board_meta) -> tuple[str, str]:
    """(slug, resolved DB identity) — multiple slugs can point at one DB when XIUQO_KANBAN_DB pins it."""
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    auto-subscribes with ``chat_id=XIUQO_SESSION_KEY``; no "tui" messaging adapter exists, so this poller is the
    ...
    try:
        ...
        try:
            ...
            return []
    # dict keyed by resolved DB identity: first slug per DB wins (a pinned XIUQO_KANBAN_DB aliases slugs).
    unique = {}
...
def _wire_desktop_sinks() -> None:
    ...
    tools pass the turn's ``XIUQO_UI_SESSION_ID`` as ``sid``. `_emit` is thread-safe."""
