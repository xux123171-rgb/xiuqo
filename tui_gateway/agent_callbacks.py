...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from xiuqo_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via xiuqo_cli.personality (single owner)."""
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    from xiuqo_cli.personality import normalize_personality_name, render_personality_prompt
...
def _prompt_text(value) -> str:
    """Normalize config prompt values from YAML for AIAgent (xiuqo_cli.personality owns this)."""
    from xiuqo_cli.personality import prompt_text
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    ...
    if env_val := os.environ.get("XIUQO_TUI_MAX_TURNS"):
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("XIUQO_TUI_SKILLS", "")
