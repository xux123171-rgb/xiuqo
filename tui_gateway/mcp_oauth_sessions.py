...
land. Reuses ``xiuqo mcp login``'s probe under ``force_interactive_oauth`` plus
...
def _start_loopback_listener(flow) -> "http.server.HTTPServer":
    ...
    class _Handler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):  # noqa: N802 — stdlib naming
            ...
            body = b"<h1>Authorization received</h1><p>You can close this tab and return to Xiuqo.</p>"
            status = 200
            try:
...
def _probe_with_rollback(
    server_name: str, cfg: dict, xiuqo_home: str, flow, reconnect_live: bool) -> None:
    """Run the OAuth probe; on ANY failure restore the prior token file + manager entry."""
    from xiuqo_cli.mcp_config import _oauth_tokens_present, _probe_single_server, _save_mcp_server
    ...
    previous_entry = None
    try:
        previous_entry = manager.remove(server_name, xiuqo_home=xiuqo_home)
        ...
        manager.restore_entry(server_name, previous_entry, xiuqo_home=xiuqo_home)
        raise


def _worker(
        session_id: str, xiuqo_home: str, server_name: str, cfg: dict, reconnect_live: bool) -> None:
    ...
    try:
        ...
        home_token = set_hermes_home_override(xiuqo_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(xiuqo_home)))
        try:
            with force_interactive_oauth(), dashboard_oauth_flow(flow):
                _probe_with_rollback(server_name, cfg, xiuqo_home, flow, reconnect_live)
        finally:
...
def start_flow(
    xiuqo_home: str, server_name: str, cfg: dict, *, reconnect_live: bool = False,
    ...
    with _sessions_lock:
        ...
        if any(r["server_name"] == server_name and r["xiuqo_home"] == xiuqo_home for r in active):
            raise RuntimeError(f"MCP OAuth for '{server_name}' is already in progress")

    session_id = secrets.token_urlsafe(24)
    flow = DashboardOAuthFlow(
        flow_id=session_id, server_name=server_name, profile=None, xiuqo_home=xiuqo_home,
    ...
    rec = {
        "session_id": session_id, "server_name": server_name, "xiuqo_home": xiuqo_home,
        "flow": flow, "httpd": httpd, "created_at": time.time()}
    with _sessions_lock:
        _sessions[session_id] = rec
    threading.Thread(
        target=_worker, args=(session_id, xiuqo_home, server_name, dict(cfg), reconnect_live),
...
def _lookup(
    session_id: str, server_name: str, xiuqo_home: Optional[str] = None,
    ...
    if xiuqo_home_key(rec["xiuqo_home"]) != xiuqo_home_key(xiuqo_home):
...
def cancel_flow(session_id: str, server_name: str, xiuqo_home: str) -> Dict[str, Any]:
    """Cancel only the owning profile's flow and release its callback waiter."""
    rec, err = _lookup(session_id, server_name, xiuqo_home)
    if rec is None:
