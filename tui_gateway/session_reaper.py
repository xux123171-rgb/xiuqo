...
def _flush_sessions_before_exit(budget_s: float | None = None) -> int:
    ...
    hung SQLite write can't block exit past ``XIUQO_TUI_EXIT_FLUSH_BUDGET_S`` (default 5s). Running sessions
    ...
    worker = threading.Thread(target=_run, daemon=True, name="xiuqo-exit-flush")
    worker.start()
    worker.join(budget)
...
def _transport_is_dead(transport) -> bool:
    ...
    # standalone `xiuqo --tui` and must NOT count as dead.
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from xiuqo_cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from xiuqo_cli.active_sessions import coerce_max_concurrent_sessions
        cfg = _load_cfg() or {}
...
_HEARTBEAT_REFRESH_S = max(0.0, env_float("XIUQO_GATEWAY_HEARTBEAT_REFRESH_S", 60.0))
...
def _start_backend_heartbeat_refresher() -> None:
    ...
    threading.Thread(target=_loop, name="xiuqo-gateway-heartbeat", daemon=True).start()
