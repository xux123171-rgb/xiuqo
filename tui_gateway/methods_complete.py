...
def _profile_mention_items(prefix: str) -> list[dict]:
    ...
    is also offered as 'xiuqo' when no real profile claims that name."""
    out: list[dict] = []
    try:
        ...
        if "xiuqo".startswith(prefix.lower()) and "xiuqo" not in seen:
            out.append(_item("@xiuqo", "agent profile (primary)"))
    except Exception:
        return []
    return out
...
def _(rid, params: dict) -> dict:
    ...
    if not text.startswith("/"):
        return _ok(rid, {"items": []})
    from xiuqo_cli.commands_completion import SlashCommandCompleter
...
@_catch(5033)
def _(rid, params: dict) -> dict:
    from xiuqo_cli.inventory import build_model_options_payload
...
def _(rid, params: dict) -> dict:
    ...
    if pconfig.auth_type != "api_key":
        return _err(rid, 4003, f"{pconfig.name} uses {pconfig.auth_type} auth — run `xiuqo model` to configure")
    if not pconfig.api_key_env_vars:
        return _err(rid, 4004, f"no env var defined for {pconfig.name}")
    # Save the key to ~/.xiuqo/.env via the unified credential lifecycle so any stale config.yaml mirror of
    # the previous key (model.api_key, custom_providers[*].api_key) is rotated in the same action (#62269).
    env_var = pconfig.api_key_env_vars[0]
    from xiuqo_cli.credential_lifecycle import save_provider_env_credential  # also rotates stale config.yaml mirrors
...
def _(rid, params: dict) -> dict:
    ...
    from xiuqo_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
