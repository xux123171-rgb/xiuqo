...
The dashboard's `/api/pty` spawns `xiuqo --tui`, which spawns ``tui_gateway.entry`` — three
...
class WsPublisherTransport:
    ...
    def __init__(self, url: str, *, connect_timeout: float = 2.0) -> None:
        ...
        try:
            ...
            self._dead = True
            return
        self._worker = threading.Thread(target=self._drain, name="xiuqo-ws-pub", daemon=True)
        self._worker.start()
