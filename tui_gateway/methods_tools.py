...
def _profile_scoped_rpc(
    ...
    """Wrap a handler body with the optional ``profile`` XIUQO_HOME scope. Order: ``required``
    ...
    def deco(body):
        def handler(rid, params: dict) -> dict:
            ...
            if profile := _str_arg(params, "profile") if scoped else "":
                try:
                    profile_dir = _tools_mod("xiuqo_cli.profiles").get_profile_dir(profile)
                    if not profile_dir or not profile_dir.is_dir():
                        return _err(rid, 4064, f"profile '{profile}' not found")
                    token = _tools_mod("xiuqo_constants").set_hermes_home_override(str(profile_dir))
...
def _scoped_rpc(name: str, fail_code: int = 5024, **kw):
    """``@method(name)`` + ``_profile_scoped_rpc`` (optional ``profile`` XIUQO_HOME scope)."""
...
def _mcp_named_server(rid, params):
    ...
    name, servers = _str_arg(params, "name"), _tools_mod("xiuqo_cli.mcp_config")._get_mcp_servers()
...
def _capture_run_kwargs(timeout: int) -> dict:
    ...
    return dict(
        ...
        stdin=subprocess.DEVNULL, creationflags=_tools_mod("xiuqo_cli._subprocess_compat").windows_hide_flags())
...
_SIMPLE_RPCS = {
    ...
    # Re-read ``~/.xiuqo/.env`` (CLI ``/reload`` parity); built agents keep their pool, ``/new`` resolves fresh.
    "reload.env": (5015, lambda params: {"updated": int(_tools_mod("xiuqo_cli.config").reload_env())}),
    "plugins.list": (5032, lambda params: {"plugins": [
        {"name": n, "version": getattr(i, "version", "?"), "enabled": getattr(i, "enabled", True)}
        for n, i in _tools_mod("xiuqo_cli.plugins").get_plugin_manager()._plugins.items()]}),
...
def _mcp_reload_confirm_required() -> bool:
    ...
    try:
        cfg = _tools_mod("xiuqo_cli.config").load_config()
...
def _catalog_registry(cat: _Catalog) -> None:
    commands = _tools_mod("xiuqo_cli.commands")
...
def _catalog_plugin_commands(cat: _Catalog) -> None:
    plugin_cmds = _tools_mod("xiuqo_cli.plugins").get_plugin_commands() or {}
    if plugin_cmds:
...
def _(rid, params: dict) -> dict:
    ...
    return _ok(rid, {
        "pairs": cat.pairs, "sub": {k: v[:] for k, v in _tools_mod("xiuqo_cli.commands").SUBCOMMANDS.items()},
        "canon": cat.canon,
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m xiuqo_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    return _captured_exec(
        rid, [sys.executable, "-m", "xiuqo_cli.main", *argv], min(int(params.get("timeout", 240)), 600),
        ...
        env=xiuqo_subprocess_env(inherit_credentials=True))


@_rpc("command.resolve", 5012)
def _(rid, params: dict) -> dict:
    r = _tools_mod("xiuqo_cli.commands").resolve_command(params.get("name", ""))
    if r:
...
def _plugin_command_handler(name: str):
    try:
        return _tools_mod("xiuqo_cli.plugins").get_plugin_command_handler(name)
    except Exception:
        return None


def _run_plugin_command(handler, arg: str) -> str:
    return str(_tools_mod("xiuqo_cli.plugins").resolve_plugin_command_result(handler(arg)) or "")


def _is_profile_skill_command(session: dict, base: str) -> bool:
    """True when ``/base`` is a skill command of the session's profile (XIUQO_HOME bound to it so
    get_skill_commands() sees its skills.external_dirs; nothing upstream binds it). False on failure."""
    try:
        hc = _tools_mod("xiuqo_constants")
...
def _bundle_key_for(name: str):
    ...
    try:
        if _tools_mod("xiuqo_cli.commands").resolve_command(name) is None:
...
_cmd_init = _prompt_builtin("xiuqo_cli.init_command", "build_init_prompt_for_cwd", kw="extra")
...
def _cmd_moa(rid, params, session, name, arg):
    ...
    try:
        moa = _tools_mod("xiuqo_cli.moa_config")
        if not arg:
...
def _cmd_focus(rid, params, session, name, arg):
    ...
    fv = _tools_mod("xiuqo_cli.focus_view")
...
def _cmd_goal(rid, params, session, name, arg):
    with _session_profile_runtime_scope(session or {}):
        sid_key, goals, err = _session_key_or_err(rid, session, "xiuqo_cli.goals", "goals")
        if err:
            return err
        try:
...
def _cmd_loop(rid, params, session, name, arg):
    sid_key, loops, err = _session_key_or_err(rid, session, "xiuqo_cli.loops", "loops")
    if err:
        return err
...
def _(rid, params: dict) -> dict:
    cfg = _load_cfg()
    api_key = _tools_mod("agent.secret_scope").get_secret("XIUQO_API_KEY", "") or cfg.get("api_key", "")
    masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
    base_url = os.environ.get("XIUQO_BASE_URL", "") or cfg.get("base_url", "")
    sections = [
        {"title": "Model", "rows": [
...
def _configure_session_tools(rid, params: dict, sid: str, session) -> dict:
    ...
    hc, tc = _tools_mod("xiuqo_cli.config"), _tools_mod("xiuqo_cli.tools_config")
    cfg = hc.load_config()
...
def _(rid, params: dict) -> dict:
    """cronjob() keys off XIUQO_HOME, so ``profile`` reaches a per-profile cron store."""
...
def _skills_install(rid, params, query):
    ...
    _tools_mod("xiuqo_cli.skills_hub").do_install(query, skip_confirm=True, console=quiet)
    return _ok(rid, {"installed": True, "name": query})


def _skills_browse(rid, params, query):
    pg = int(params.get("page", 0) or 0) or (int(query) if query.isdigit() else 1)
    browse = _tools_mod("xiuqo_cli.skills_hub").browse_skills
    return _ok(rid, browse(page=pg, page_size=int(params.get("page_size", 20))))


_SKILLS_ACTIONS = {
    "list": lambda rid, params, query: _ok(rid, {"skills": _tools_mod("xiuqo_cli.banner").get_available_skills()}),
    "search": _skills_search, "install": _skills_install, "browse": _skills_browse,
    "inspect": lambda rid, params, query: _ok(
        rid, {"info": _tools_mod("xiuqo_cli.skills_hub").inspect_skill(query) or {}})}
...
# Gateway mirrors of the dashboard REST surface (xiuqo_cli/web_routers/mcp.py) so a
# desktop plugin can manage MCP servers for ANY profile. Persistence: xiuqo_cli/mcp_config.py.
@_scoped_rpc("mcp.catalog")
def _(rid, params: dict) -> dict:
    """``{servers: [{name, description, installed, enabled, requires: [env keys], transport}]}`` per profile."""
    mcp_catalog = _tools_mod("xiuqo_cli.mcp_catalog")
    out = []
...
def _(rid, params: dict) -> dict:
    ...
    enabled, tools}]}``"""
    servers = _tools_mod("xiuqo_cli.mcp_config")._get_mcp_servers()
...
def _(rid, params: dict) -> dict:
    ...
    import time
    hc = _tools_mod("xiuqo_constants")
    configured = _tools_mod("xiuqo_cli.mcp_config")._get_mcp_servers()
    include_runtime = (_tools_mod("agent.secret_scope").is_multiplex_active()
                       or hc.xiuqo_home_key() == hc.xiuqo_home_key(hc.get_process_hermes_home()))
...
def _(rid, params: dict) -> dict:
    ...
    mc = _tools_mod("xiuqo_cli.mcp_config")
...
def _(rid, params: dict) -> dict:
    ...
    hc, mc = _tools_mod("xiuqo_cli.config"), _tools_mod("xiuqo_cli.mcp_config")
...
def _(rid, params: dict) -> dict:
    ...
    if not _tools_mod("xiuqo_cli.mcp_config")._remove_mcp_server(name):
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        cfg["auth"] = "oauth"
        xiuqo_home = str(_tools_mod("xiuqo_constants").get_hermes_home().expanduser().resolve(strict=False))
        result = _tools_mod("tui_gateway.mcp_oauth_sessions").start_flow(
            xiuqo_home, name, cfg, client_redirect_uri=client_redirect_uri)
...
def _(rid, params: dict) -> dict:
    ...
    home = str(_tools_mod("xiuqo_constants").get_hermes_home().expanduser().resolve(strict=False))
...
def _plugin_rows() -> list[dict]:
    pc = _tools_mod("xiuqo_cli.plugins_cmd")
    cat = _tools_mod("xiuqo_cli.plugins_cmd_catalog")
...
def _plugins_toggle(rid, params):
    ...
    toggle = _tools_mod("xiuqo_cli.plugins_cmd").dashboard_set_agent_plugin_enabled
...
def _plugins_install(rid, params):
    ...
    result = _tools_mod("xiuqo_cli.plugins_cmd").dashboard_install_plugin(
...
def _plugins_update(rid, params):
    ...
    pc, cat = _tools_mod("xiuqo_cli.plugins_cmd"), _tools_mod("xiuqo_cli.plugins_cmd_catalog")
...
def _(rid, params: dict) -> dict:
    """TUI Plugins Hub backend (shares primitives with ``xiuqo plugins`` / the dashboard):
