...
def _relay_root() -> Path:
    ...
    every XIUQO_HOME, including non-``profiles/`` subdirs of ``~/.xiuqo``."""
...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery) -> dict:
    ...
    GATEWAY via the one-turn ``xiuqo -p <profile> chat -c "Bot Chat"`` transport local DMs use →
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "xiuqo" else profile
        ...
        fd, tmp = tempfile.mkstemp(prefix="xiuqo-relay-dm-", suffix=".txt", text=True)
        try:
