...
from xiuqo_constants import display_hermes_home as _display_hermes_home
...
def _cfg_get_provider(params):
    from xiuqo_cli.models import list_available_providers, normalize_provider
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's XIUQO_HOME + ``.env`` secret scope (ContextVars: concurrent checks
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from xiuqo_cli.free_tier_bootstrap import wait_for_record
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No Xiuqo provider is configured.", source)
            ...
            from xiuqo_cli.anon_auth import route_is_welcome_host
...
def _(rid, params: dict) -> dict:
    ...
    force-redaction pipeline as ``xiuqo debug share --nous``; redaction is NOT client-controllable
    ...
    try:
        from xiuqo_cli.debug import _redact_log_text, build_nous_bundle, collect_share_bundle
