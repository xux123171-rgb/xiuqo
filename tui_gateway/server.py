...
from tools.environments.local import xiuqo_subprocess_env
...
load_hermes_dotenv(xiuqo_home=_hermes_home, project_env=Path(__file__).parent.parent / ".env")
...
with contextlib.suppress(Exception):
    from xiuqo_cli.banner import prefetch_update_check
...
_SLASH_WORKER_TIMEOUT_S = max(5.0, env_float("XIUQO_TUI_SLASH_TIMEOUT_S", 45.0))
...
def _resolve_ws_orphan_reap_grace() -> float:
    ...
    return _ws_orphan_setting("XIUQO_TUI_WS_ORPHAN_REAP_GRACE_S", "ws_orphan_reap_grace_s", 20.0)
...
_WS_ORPHAN_ACTIVITY_STALE_S = _ws_orphan_setting("XIUQO_TUI_WS_ORPHAN_ACTIVITY_STALE_S", "ws_orphan_activity_stale_s", 600.0)
...
_rpc_pool_workers = max(2, env_int("XIUQO_TUI_RPC_POOL_WORKERS", 8))
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = contextvars.ContextVar(
    "xiuqo_gateway_runtime_session_record", default=None)
...
_current_rpc_method: contextvars.ContextVar[str] = contextvars.ContextVar("xiuqo_gateway_rpc_method", default="")
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    ...
    ~/.local/bin to PATH so slash_worker children resolve Xiuqo-managed CLIs under the Desktop's minimal PATH."""
    managed_bin = ""
    with contextlib.suppress(Exception):
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Xiuqo agent → needs provider credentials. Tier-1 secrets
        ...
        # launch XIUQO_HOME (#40677). The override goes through the build_subprocess_env factory's `extra`
        # (applied last, always wins) instead of a hand-rolled env["XIUQO_HOME"] assignment.
        ...
        env = _prepend_tool_paths(build_subprocess_env(
            xiuqo_subprocess_env(inherit_credentials=True), scrub_secrets=False,
            inherit_profile_home=False, extra={"XIUQO_HOME": str(profile_home)} if profile_home else None))
        ...
        # Prepend the Xiuqo venv bin dir and the user-local bin dir to PATH so slash_worker child processes
        # can resolve Xiuqo-managed CLIs (browser-use, uvx) even when the parent gateway was launched with
...
_SESSION_TTL_S = max(0.0, env_float("XIUQO_TUI_SESSION_TTL_S", float(6 * 3600)))
...
_EXIT_FLUSH_BUDGET_S = max(0.0, env_float("XIUQO_TUI_EXIT_FLUSH_BUDGET_S", 5.0))
_INCREMENTAL_FLUSH_INTERVAL_S = max(0.0, env_float("XIUQO_TUI_SESSION_FLUSH_INTERVAL_S", _REAPER_SCAN_S))
...
def _canonical_profile_request(name: str) -> str:
    ...
    installation details — unless a real named profile of that name exists (``xiuqo`` is a legal
    id), in which case it wins; other unknown names keep failing closed in ``_profile_home``.
    """
    if name.casefold() in {".xiuqo", "xiuqo"}:
...
# backend, so calls carry ``profile`` → open that profile's db and bind its XIUQO_HOME (ContextVar
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s XIUQO_HOME around a handler (pets/projects resolve via
...
def _bind_build_profile_scopes(profile_home: str) -> "_TurnScopes":
    """Bind a session profile's XIUQO_HOME / secret / terminal scopes for an agent build. Fail-open per
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # Global-remote: bind the session profile's XIUQO_HOME and hand the agent that profile's db —
...
def _set_session_context(session_key: str, cwd: str | None = None, *, ui_session_id: str = "") -> list:
    with contextlib.suppress(Exception):
        ...
        # Live conversation id for subprocess XIUQO_SESSION_ID: an explicitly empty contextvar is authoritative
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ.update(XIUQO_GATEWAY_SESSION="1", XIUQO_EXEC_ASK="1", XIUQO_INTERACTIVE="1")
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps({
    "success": False,
    "error": ("No Xiuqo Desktop window answered the tour request. The tour is driven by the desktop app's "
              "renderer, which updates separately from this backend, so an app build older than the tour tool "
              "has nothing listening. Update the Xiuqo Desktop app and start a new session. Do not retry tour "
              "in this session.")})
...
def _env_model_seed() -> str:
    """The launch-scoped model seed (``xiuqo --tui -m``, hosted provisioning); "" when unset."""
    return (os.environ.get("XIUQO_MODEL", "") or os.environ.get("XIUQO_INFERENCE_MODEL", "")).strip()
...
def _resolve_model() -> str:
    ...
    with contextlib.suppress(Exception):
        from xiuqo_cli.models import get_preferred_silent_default_model
        return get_preferred_silent_default_model()
    return "z-ai/glm-5.2"


def _resolve_session_platform() -> str:
    """``XIUQO_DESKTOP=1`` without ``XIUQO_DESKTOP_TERMINAL`` → "desktop" (chat panel; the agent then
    suggests TUI-only slash commands), else "tui" (embedded terminal pane or standalone ``xiuqo --tui``)."""
    desktop = is_truthy_value(os.environ.get("XIUQO_DESKTOP"))
    return "desktop" if desktop and not is_truthy_value(os.environ.get("XIUQO_DESKTOP_TERMINAL")) else "tui"
...
def _config_model_target() -> tuple[str, str]:
    """(model, provider) selected by config.yaml — and ONLY config: the XIUQO_MODEL launch seed fed into
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    if explicit_provider := os.environ.get("XIUQO_TUI_PROVIDER", "").strip():
        return model, explicit_provider
    if not (explicit_model := _env_model_seed()):
        return model, None
    with contextlib.suppress(Exception):
        from xiuqo_cli.models import detect_static_provider_for_model
        cfg = _load_cfg().get("model") or {}
        current_provider = ((str(cfg.get("provider") or "").strip().lower() if isinstance(cfg, dict) else "")
                            or os.environ.get("XIUQO_INFERENCE_PROVIDER", "").strip().lower() or "auto")
...
from xiuqo_state import _BARE_BILLING_PROVIDERS


def _is_routable_provider(provider: str) -> bool:
    with contextlib.suppress(Exception):
        from xiuqo_cli.runtime_provider import is_routable_provider
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    if provider and not _is_routable_provider(provider):
        healed = None
        try:
            from xiuqo_cli.runtime_provider import canonical_custom_identity
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    if live is None or not hasattr(live[0], "_build_system_prompt") or not hasattr(live[2], "update_system_prompt"):
        return
    agent, session_key, db = live
    # Re-bind the session's profile XIUQO_HOME (the build's finally reset it → root profile's SOUL.md/skills)
...
def _load_reasoning_config(model: str = "") -> dict | None:
    """Via the shared chokepoint :func:`xiuqo_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("XIUQO_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    """Toolsets that exist because of the CLIENT (both off ``_XIUQO_CORE_TOOLS``; this is the one gate).
    ``platform`` is the SESSION's source, never a process env var: the desktop may drive a URL/cloud
    backend where ``XIUQO_DESKTOP`` is unset (AGENTS.md surface rule)."""
...
def _resolve_explicit_toolsets(explicit: list[str], validate_toolset) -> list[str] | None | bool:
    """Resolve a XIUQO_TUI_TOOLSETS pin: list, None for "all", False when nothing was valid."""
    ...
    if any(name in {"all", "*"} for name in built_in):
        if ignored := [name for name in explicit if name not in {"all", "*"}]:
            _tui_notice(f"[tui] XIUQO_TUI_TOOLSETS=all enables every toolset; ignoring additional entries: {', '.join(ignored)}")
        return None
    if not unresolved:
        return built_in
    try:  # (enabled, disabled) MCP server names from raw config; both empty on any failure
        ...
        from xiuqo_cli.tools_config import _parse_enabled_flag
    ...
    if unknown:
        _tui_notice(f"[tui] ignoring unknown XIUQO_TUI_TOOLSETS entries: {', '.join(unknown)}")
    if disabled:
        _tui_notice("[tui] ignoring disabled MCP servers in XIUQO_TUI_TOOLSETS "
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    """The agent's toolsets for this session (None = all): an explicit XIUQO_TUI_TOOLSETS pin; else the
    ...
    explicit = [item.strip() for item in os.environ.get("XIUQO_TUI_TOOLSETS", "").split(",") if item.strip()]
    ...
    if explicit and validate_toolset is not None:
        ...
        if resolved is not False:
            return resolved
        fallback_notice = "[tui] no valid XIUQO_TUI_TOOLSETS entries; using configured CLI toolsets"
    try:
        from xiuqo_cli.config import load_config
        ...
        if fallback_notice is not None:
            _tui_notice("[tui] no valid XIUQO_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets")
        return None
...
def _get_usage(agent) -> dict:
    ...
    # Dev-only live credits-spent readout, gated on XIUQO_DEV_CREDITS so the payload stays clean otherwise.
    if is_truthy_value(os.environ.get("XIUQO_DEV_CREDITS")):
        with contextlib.suppress(Exception):
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            with contextlib.suppress(Exception):
                from xiuqo_cli.personality import available_personalities
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from xiuqo_cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if live_agent:
        ...
        with contextlib.suppress(Exception):
            from xiuqo_cli.banner import get_available_skills
    ...
    with contextlib.suppress(Exception):
        from xiuqo_cli.banner import get_update_result
        from xiuqo_cli.config import recommended_update_command
...
def _startup_system_prompt(cfg: dict, task_id: str) -> str:
    """Config ephemeral system prompt + XIUQO_TUI_SKILLS preload block. Hard-fails only when EVERY requested
    skill is missing (cli.py parity): a typo'd name must not auto-block the Kanban task."""
    from xiuqo_cli.config import resolve_ephemeral_system_prompt_from_config
    ...
    if missing_skills:
        ...
        logger.warning("Unknown skill(s) requested, skipping: %s. Continuing with: %s. "
                       "List available skills with `xiuqo skills list`.", missing_display, ", ".join(loaded_skills))
    if skills_prompt:
...
def _make_agent(
    ...
    for _mod in ("xiuqo_cli.mcp_startup", "tui_gateway.entry"):
        with contextlib.suppress(Exception):
    ...
    ignore_rules = is_truthy_value(os.environ.get("XIUQO_IGNORE_RULES"))
    with _sessions_lock:
        session = _sessions.get(sid)
    agent = AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("XIUQO_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("XIUQO_TUI_PASS_SESSION_ID")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            # Profile-scoped XIUQO_HOME (None = launch); SessionBranch copies the parent's (same state.db).
...
def _load_resume_transcript(db, stored_id: str) -> tuple[list, list, list]:
    ...
    from xiuqo_state import SessionResumeTooLargeError
    prefix_fits = True
...
try:
    _PET_REFERENCE_MAX_BYTES = max(1, int(os.environ.get("XIUQO_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)))
except (TypeError, ValueError):
...
_CLI_EXEC_BLOCKED = {
    ("setup",): "`xiuqo setup` needs a full terminal — run it outside the TUI",
    ("gateway",): "`xiuqo gateway` is long-running — run it in another terminal",
    ("sessions", "browse"): "`xiuqo sessions browse` is interactive — use /resume here, or run browse in another terminal",
    ("config", "edit"): "`xiuqo config edit` needs $EDITOR in a real terminal",
}
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `xiuqo` is interactive — use `/xiuqo chat -q …` or run `xiuqo` in another terminal"
...
def _resolve_name(name: str) -> str:
    with contextlib.suppress(Exception):
        from xiuqo_cli.commands import resolve_command
