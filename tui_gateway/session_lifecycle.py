...
_SESSION_OWNERSHIP_UNAVAILABLE = "Xiuqo could not safely reserve this session. Try again."
...
def _other_runtime_lease_guard(session_id: str, session: dict):
    ...
    try:
        from xiuqo_cli.active_sessions import active_session_liveness_guard, release_active_session_liveness_guard
    except Exception as exc:
...
def _transfer_active_session_slot(sid: str, session: dict, *, new_session_id: str) -> bool:
    ...
    if lease is None:
        return True
    try:
        from xiuqo_cli.active_sessions import transfer_active_session
...
def _interrupt_session_turn(sid: str, session: dict, *, request_id: str | None = None) -> bool:
    ...
    if should_interrupt:
        ...
        try:
            from xiuqo_cli.plugins import invoke_hook as _invoke_hook
            _invoke_hook(
    ...
    for sid, session in clientless:
        ...
        with _session_resume_lock, _sessions_lock:
            ...
            if current.get("close_on_disconnect"):
                ...
                # `xiuqo --tui` keeps real _stdio. UNLESS another window (pop-out viewer) still shows the session:
