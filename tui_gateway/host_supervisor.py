...
class HostSupervisor:
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        self._hello = {}
        env = {**xiuqo_subprocess_env(inherit_credentials=True), **os.environ, **(self.env or {})}
        env["XIUQO_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
        root = str(_repo_root())
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("xiuqo_home") or "")
        if got_home and got_home != self.expected_hermes_home:
            raise RuntimeError(
                f"compute host XIUQO_HOME mismatch: {got_home} != {self.expected_hermes_home}")
