...
def _legacy_group_fence_error(rid, session, params):
    ...
    if hosted or peer:
        ...
        return _err(
            rid, 4122, f"This room is managed by {owner}. Update Xiuqo Desktop to continue it.")
    return None
...
def _parse_truncation_params(rid, sid, session, params, history):
    ...
    return None, None, _err(
        ...
        "(update your Xiuqo client if a rewind was intended)")
...
def _(rid, params: dict) -> dict:
    from xiuqo_cli.input_sanitize import sanitize_user_prompt_text
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from xiuqo_cli.clipboard import has_clipboard_image, save_clipboard_image
...
def _spawn_side_agent(
    ...
    def run():
        ...
        # Bug #50233: ephemeral agent threads don't inherit the session's XIUQO_HOME override (the
        ...
        # Bug #50233: ephemeral preview-restart agent threads don't inherit the session's XIUQO_HOME
...
_PREVIEW_RESTART_RULES = (
    "Restart exactly the app intended for the Preview URL, not Xiuqo Desktop itself.",
    ...
    "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Xiuqo/Desktop process cwd.",
    ...
    "Before declaring success, verify the Preview URL responds with the intended app, not Xiuqo Desktop. If it serves Xiuqo/Desktop UI or another unrelated app, stop that process and report failure.",
