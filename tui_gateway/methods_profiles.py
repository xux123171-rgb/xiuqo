...
only have the ws door), on the same `xiuqo_cli.profiles` primitives. Bodies are rebound onto
...
def _pin_profile_model(profile_dir, provider, model) -> None:
    _lazy("xiuqo_cli.web_routers.profiles", "_write_profile_model")(profile_dir, provider, model)
...
def _resurrect_recoverable_canonical(db, profile_path, session_id):
    ...
    try:
        ...
        try:
            ...
            _best_effort(lambda: _lazy("xiuqo_state_registry", "release_or_close")(wdb))
    except Exception:
        return False
...
def _canonical_session_row(db, profile_path):
    ...
    and its click target describe the same session (xiuqo#88200) with no client-side pointer
    involved.
    """
    try:
...
def _latest_profile_session_rows(db):
    ...
    (xiuqo#90268). Workers heartbeat ``last_activity_at`` every ≤60s while running (#72016), so a
...
def _profile_session_fields(row, profile_path):
    ...
    if _try(db_path.exists, False):
        db = _try(lambda: _lazy("xiuqo_state", "SessionDB")(db_path=db_path, read_only=True), None)
    try:
...
def _(rid, params: dict) -> dict:
    """List Xiuqo profiles. ``include_sessions`` (default true) adds ``last_session`` /
...
def _mirror_voice_sections(path) -> bool:
    ...
    try:
        from xiuqo_cli.config import load_config_readonly, read_user_config_raw, save_config
...
def _inherit_launch_model(path) -> bool:
    ...
    from xiuqo_cli.config import load_config_readonly, read_user_config_raw
    with _hermes_home_scope(path):
...
def _mirror_launch_credentials(path, params: dict) -> dict:
    ...
    if mirrored["env"] and not is_truthy_value(params.get("clone_channels", False)):
        ...
        _best_effort(lambda: _lazy("xiuqo_cli.profile_channels", "strip_channel_env_file")(path / ".env"))
    if not share_auth:  # a copy forks token state: the first refresh in either store strands the other
        ...
        if mirrored["auth"]:
            ...
            _best_effort(lambda: _lazy("xiuqo_cli.auth", "strip_cloned_single_use_oauth_grants")(path))
...
def _describe_toolsets(cfg):
    """``(toolsets, pinned_set)`` as the `xiuqo tools` checklist presents them (the raw registry
    ...
    default_off = _try(lambda: _lazy("xiuqo_cli.tools_config", "_DEFAULT_OFF_TOOLSETS"), set())
    toolsets_out = []
...
def _(rid, params: dict) -> dict:
    ...
    with _hermes_home_scope(profile_dir):
        ...
        from xiuqo_cli.skills_config import get_disabled_skills
        cfg = load_config() or {}
        ...
        meta = _try(lambda: _lazy("xiuqo_cli.profiles", "read_profile_meta")(profile_dir), {})
        return _ok(rid, {
...
def _configure_model(profile_dir, params, applied):
    ...
    if not is_truthy_value(params.get("confirm_expensive_model", False)):
        warn = _lazy("xiuqo_cli.model_selection_guards", "combined_selection_warning")
...
def _configure_cfg_sections(profile_dir, params, applied) -> None:
    ...
    if want_mcp:  # launch catalog read BEFORE the home override flips config resolution
        load_launch = _lazy("xiuqo_cli.config", "load_config_readonly")
    ...
    with _hermes_home_scope(profile_dir):
        ...
        if isinstance(params.get("disabled_skills"), list):
            try:
                from xiuqo_cli.skills_config import save_disabled_skills
...
def _(rid, params: dict) -> dict:
    ...
    if isinstance(params.get("description"), str):
        write_meta = _lazy("xiuqo_cli.profiles", "write_profile_meta")
