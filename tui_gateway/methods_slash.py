...
def _format_live_review_output(sid: str, session: Optional[dict], arg: str) -> str:
    ...
    # (XIUQO_UI_SESSION_ID + steer authority), or delegate_task registers the reviewer with no owner
...
def _format_live_history_output(sid: str, session: dict, arg: str) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = {"user": "You", "assistant": "Xiuqo"}.get(role, role.title())
...
def _format_live_help_output(sid: str, session: dict, arg: str) -> str:
    try:
        from xiuqo_cli.commands import COMMANDS_BY_CATEGORY
...
def _mirror_personality(sid, session, agent, arg) -> None:
    if arg and agent:
        ...
        from xiuqo_cli.personality import persist_personality  # single owner: no surface drift
