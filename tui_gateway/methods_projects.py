...
def _is_repo_junk(root: str) -> bool:
    ...
    XIUQO_HOME. User-created projects pointing there are still honored."""
    if not root:
        return True
    from xiuqo_constants import get_hermes_home
    real = os.path.realpath(root)
    xiuqo_home = os.path.realpath(str(get_hermes_home()))
    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == xiuqo_home
        or real.startswith(xiuqo_home + os.sep))


def _is_session_cwd_junk(cwd: str) -> bool:
    """A non-git cwd that stays in flat Recents. A DESCENDANT of XIUQO_HOME may be an
    intentional prose/data workspace, so only XIUQO_HOME itself is excluded here."""
    if not cwd:
        return True
    from xiuqo_constants import get_hermes_home
    real = os.path.normcase(os.path.realpath(cwd))
    xiuqo_home = os.path.normcase(os.path.realpath(str(get_hermes_home())))
    return real in _non_workspace_dirs() or real == xiuqo_home
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    host must scan its own disk so repos with zero Xiuqo sessions still appear in the sidebar (#81723).
