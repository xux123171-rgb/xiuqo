...
def _hosted_room_turn_timeout_seconds() -> float:
    try:
        agent_timeout = float(os.getenv("XIUQO_AGENT_TIMEOUT", "1800"))
...
class HostedRoomService:
    ...
    def local_profiles(self) -> tuple[str, ...]:
        from xiuqo_constants import named_profile_is_deleted

        profiles, profiles_dir = {"default"}, self.root / "profiles"
        if profiles_dir.is_dir():
            # ``profiles/.deleted/`` is the tombstone dir `xiuqo profile delete` leaves behind, not a
