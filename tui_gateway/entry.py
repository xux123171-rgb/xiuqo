...
# Stop a ``utils/``-style package in the launch directory from shadowing Xiuqo's own
# top-level modules; ``xiuqo_bootstrap``'s name can't collide, so importing it first is safe.
import xiuqo_bootstrap

xiuqo_bootstrap.harden_import_path()
...
# xiuqo_cli.mcp_startup (current path). The wait/in-flight/join helpers consult both.
...
def _install_sidecar_publisher() -> None:
    ...
    url = os.environ.get("XIUQO_TUI_SIDECAR_URL")
    if not url:
        return
...
# strand the process; ``XIUQO_TUI_GATEWAY_SHUTDOWN_GRACE_S`` overrides.
_DEFAULT_SHUTDOWN_GRACE_S = 1.0


def _shutdown_grace_seconds() -> float:
    value = env_float("XIUQO_TUI_GATEWAY_SHUTDOWN_GRACE_S", _DEFAULT_SHUTDOWN_GRACE_S)
    return value if value > 0 else _DEFAULT_SHUTDOWN_GRACE_S


def _mcp_startup_call(name: str, *args, default=None, log=None, **kwargs):
    """Call ``xiuqo_cli.mcp_startup.<name>`` (lazy import); ``default`` on any failure,
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    # Discovery is spawned via the shared owner (ensure_mcp_discovery_started → xiuqo_cli.mcp_startup);
    ...
    # CALLER's profile context (agent build binds the session profile's XIUQO_HOME first), so a launch
...
def mcp_discovery_in_flight() -> bool:
    ...
    There are two independent discovery-thread owners by surface: the stdio ``xiuqo --tui`` path spawns ITS
    thread here (``_mcp_discovery_thread``), while the desktop app + dashboard WebSocket sidecar
    (``tui_gateway/ws.py``) and ``xiuqo dashboard`` spawn theirs via
    ``xiuqo_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh scheduler imports this
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    ``xiuqo_cli.mcp_startup`` thread used by the desktop/dashboard surfaces. See #51587.
    """
    entry_done = True
    thread = _mcp_discovery_thread
...
def _has_configured_mcp_servers() -> bool:
    ...
    from xiuqo_cli.mcp_startup import _has_configured_mcp_servers as configured
    return configured()
...
def ensure_mcp_discovery_started() -> None:
    ...
    session profile's XIUQO_HOME.

    WebSocket/Desktop entrypoints can accept sessions without running ``main()``, so the agent-build path
    (``server._start_agent_build``) also calls it AFTER binding the session profile's XIUQO_HOME override —
    the shared owner in ``xiuqo_cli.mcp_startup`` captures the caller's context-local override and
    propagates it into the discovery thread, so discovery reads the SELECTED profile's ``mcp_servers``, not
    the launch profile's. The discovery slot in ``xiuqo_cli.mcp_startup`` is keyed by profile home, so
...
def main():
    ...
    # Live-apply skins Xiuqo activates mid-conversation.
