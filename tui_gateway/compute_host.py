...
class ComputeHost:
    ...
    def __init__(
        ...
        self._heartbeat_secs = (
            ...
            else float(os.environ.get("XIUQO_COMPUTE_HOST_HEARTBEAT_SECS") or "15"))
    ...
    def _run_real_turn(self, frame: dict[str, Any]) -> None:
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                import xiuqo_undo
                xiuqo_undo.on_user_message_appended(session["session_key"])
            with contextlib.suppress(Exception):
    ...
    def _build_server_session(self, server: Any, frame: dict[str, Any], sid: str) -> dict:
        ...
        try:
            if profile_home:
                from xiuqo_constants import set_hermes_home_override
            ...
            if home_token is not None:
                with contextlib.suppress(Exception):
                    from xiuqo_constants import reset_hermes_home_override
...
def _default_workers() -> int:
    try:
        return max(2, int(os.environ.get("XIUQO_TUI_RPC_POOL_WORKERS") or "8"))
    except (TypeError, ValueError):
        return 8


def run_host(stdin: Any = None, stdout: Any = None) -> None:
    os.environ["XIUQO_COMPUTE_HOST_CHILD"] = "1"
    ...
    host.emit({
        ...
        "xiuqo_home": os.environ.get("XIUQO_HOME", "")})
