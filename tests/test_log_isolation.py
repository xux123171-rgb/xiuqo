"""The test suite must never write into the operator's real Xiuqo logs.

`xiuqo_cli/main.py` calls `setup_logging()` at module scope, which resolves
...
the whole pytest session's logging to `<XIUQO_HOME>/logs/agent.log`.

If XIUQO_HOME is not already sandboxed at that moment, that is the
...
`tests/conftest.py` sets XIUQO_HOME at module scope for that reason - this
...
def _all_file_destinations() -> list[str]:
    ...
    try:
        ...
        listener = getattr(xiuqo_logging, "_queue_listener", None)
        if listener is not None:
            collect(getattr(listener, "handlers", ()))
    except Exception:
        pass
...
class TestLogIsolation:
    def test_hermes_home_is_sandboxed_before_imports(self):
        ...
        # fixture has sandboxed XIUQO_HOME, so reading it here would pass even
        # with the conftest block deleted. Assert the value captured at conftest
        # import, which is the moment that actually matters.
        from tests.conftest import XIUQO_HOME_AT_CONFTEST_IMPORT as home

        assert home, "conftest must set XIUQO_HOME before test modules import"
        assert Path(home).resolve() != _real_hermes_home().resolve(), (
            f"XIUQO_HOME pointed at the operator's real home ({home}) when "
