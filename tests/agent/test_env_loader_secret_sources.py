"""Tests for the secret-source tracking in ``xiuqo_cli.env_loader``.

These cover the small public surface that lets `xiuqo model` / `xiuqo setup`
...
from xiuqo_cli import env_loader  # noqa: E402
...
def test_single_profile_scoped_load_keeps_override_behavior(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("XIUQO_TEST_SHARED_ADAPTER_CONFIG", raising=False)
    other_home = tmp_path / "other"
    other_home.mkdir()
    (other_home / ".env").write_text("XIUQO_TEST_SHARED_ADAPTER_CONFIG=second\n")

    was_active = secret_scope.is_multiplex_active()
    secret_scope.set_multiplex_active(False)
    home_token = set_hermes_home_override(other_home)
    try:
        loaded = env_loader.load_hermes_dotenv(xiuqo_home=other_home)
    finally:
        secret_scope.set_multiplex_active(was_active)
        reset_hermes_home_override(home_token)

    try:
        assert os.environ.get("XIUQO_TEST_SHARED_ADAPTER_CONFIG") == "second"
        assert (other_home / ".env") in loaded
    finally:
        os.environ.pop("XIUQO_TEST_SHARED_ADAPTER_CONFIG", None)
...
def test_multiplex_dotenv_load_hydrates_sources_without_global_env(
    ...
    try:
        assert env_loader.load_hermes_dotenv(xiuqo_home=tmp_path) == []
    finally:
        secret_scope.set_multiplex_active(was_active)
        reset_hermes_home_override(home_token)
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    ...
    hot modules (cli.py, xiuqo_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per XIUQO_HOME per process.
    """
    ...
    # cli.py, xiuqo_cli/main.py, run_agent.py, trajectory_compressor.py,
...
def test_env_shadowed_reapply_keeps_home_snapshot(tmp_path, monkeypatch, _fresh_registry):
    ...
    env_loader.load_hermes_dotenv(xiuqo_home=home)
