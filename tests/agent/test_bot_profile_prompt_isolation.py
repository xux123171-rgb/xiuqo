...
thread that did not bind the XIUQO_HOME ContextVar.
...
so an unbound build thread fell back to ``~/.xiuqo`` (default) and leaked
default's full skills index + "Active Xiuqo profile: default" into a bot's
...
def test_skills_prompt_scoped_to_override_not_ambient_home(tmp_path, monkeypatch):
    """An explicit skills_dir_override wins over ambient XIUQO_HOME, on a
...
def test_profile_name_correct_on_bound_profile_session(tmp_path, monkeypatch):
    ...
    xiuqo ROOT (get_default_hermes_root)."""
    from agent import system_prompt

    bot_home = tmp_path / "profiles" / "mybot"
    bot_home.mkdir(parents=True)

    # Bound session: XIUQO_HOME env points at the profile dir itself.
    monkeypatch.setenv("XIUQO_HOME", str(bot_home))
