...
def curator_env(tmp_path, monkeypatch):
    """Isolated XIUQO_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".xiuqo"
    ...
    # save_state() resolves the state path from XIUQO_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.xiuqo once monkeypatch
    ...
    # thread here, while XIUQO_HOME is still pinned to this test's tmp home
...
def test_cli_pin_refuses_bundled_skill(curator_env, capsys):
    from xiuqo_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `xiuqo model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from xiuqo_cli.main_provider_setup import _AUX_TASKS
    from xiuqo_cli.web_server_config import _AUX_TASK_SLOTS
    ...
    # 2. xiuqo_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. xiuqo_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
...
def test_review_fork_restricts_toolsets_to_skills_only(curator_env, monkeypatch):
    ...
    ``xiuqo curator rollback`` restored a hollow skill. Removing the toolset
...
def test_review_fork_toolset_surface_excludes_execution_tools():
    ...
    already-stripped package, and ``xiuqo curator rollback`` restores a
...
def test_review_prompt_does_not_steer_terminal_writes():
    ...
    ``mkdir -p ~/.xiuqo/skills/<umbrella>/references/ && mv ...`` its
