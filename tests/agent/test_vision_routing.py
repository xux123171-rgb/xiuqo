...
def isolated_home(monkeypatch):
    """Temp XIUQO_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="xiuqo_test_31179_")
    xiuqo_home = os.path.join(test_home, ".xiuqo")
    os.makedirs(xiuqo_home)
...
def _fresh_modules():
    """Drop cached xiuqo modules so each test reloads against current env."""
    _drop_reload_targets()
