"""Regression tests for the codex_app_server → Xiuqo UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_xiuqo_tools_mcp_server_emits_bare_tool_name(self):
        """The xiuqo-tools MCP server wraps Xiuqo' own tools for codex;
        ...
        mcp.xiuqo-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "xiuqo-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "xiuqo-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "xiuqo agent docs"
        assert calls[0].args[3] == {"query": "xiuqo agent docs"}
