...
def test_locked_manager_is_reported_not_prompted_when_headless(fake_bw, monkeypatch):
    ...
    monkeypatch.setenv("XIUQO_CRON_SESSION", "1")  # headless: nobody can answer a prompt
...
def test_unlock_uses_vendor_passwordenv_contract_then_fill_routes_by_prefix(fake_bw):
    ...
    assert "XIUQO_BW_MASTER" not in os.environ, "master password env var is child-only"

    # Tokens are profile-scoped: another XIUQO_HOME sees the manager locked and cannot lock ours.
    other = str(exe.parent / "other-profile")
    with patch.dict(os.environ, {"XIUQO_HOME": other}):
