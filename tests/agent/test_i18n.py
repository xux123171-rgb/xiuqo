...
def test_default_when_nothing_set(monkeypatch):
    ...
    monkeypatch.delenv("XIUQO_LANGUAGE", raising=False)
...
def test_language_is_per_profile_under_multiplex(monkeypatch, tmp_path):
    """XIUQO_LANGUAGE in the DEFAULT profile's environ must not leak into a secondary profile's
    ...
    monkeypatch.setenv("XIUQO_LANGUAGE", "zh")  # default profile's .env, bridged into environ
    i18n.reset_language_cache()
    ...
    try:
        ...
        monkeypatch.setenv("XIUQO_HOME", str(prof_b))
...
def test_locales_dir_env_override_ignored_when_missing(tmp_path, monkeypatch):
    """A bogus XIUQO_BUNDLED_LOCALES falls through to source/wheel resolution
    instead of returning a path that doesn't exist."""
    monkeypatch.setenv("XIUQO_BUNDLED_LOCALES", str(tmp_path / "does-not-exist"))
