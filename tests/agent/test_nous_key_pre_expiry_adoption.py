...
def test_key_far_from_expiry_is_left_alone_without_touching_the_store():
    ...
    with patch("xiuqo_cli.auth.resolve_nous_runtime_credentials", side_effect=AssertionError("must not hit the store")):
...
def test_key_inside_the_skew_adopts_the_stores_fresh_key_without_forcing_a_refresh():
    ...
    with patch("xiuqo_cli.auth.resolve_nous_runtime_credentials", side_effect=resolve):
...
def test_a_fresh_key_for_a_different_account_is_never_adopted():
    ...
    with patch("xiuqo_cli.auth.resolve_nous_runtime_credentials", return_value={"api_key": other, "base_url": agent.base_url}):
        assert agent._adopt_nous_key_before_expiry() is False
...
def test_same_key_back_from_the_store_is_not_readopted():
    ...
    agent = _Agent(key)
    with patch("xiuqo_cli.auth.resolve_nous_runtime_credentials", return_value={"api_key": key, "base_url": agent.base_url}):
        assert agent._adopt_nous_key_before_expiry() is False
    assert agent.adopted == []
...
def test_keepalive_thread_starts_when_an_agent_routes_to_nous(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("XIUQO_HOME", str(tmp_path / "hh"))
    started = []
    monkeypatch.setattr("xiuqo_cli.nous_auth_keepalive.start_nous_auth_keepalive", lambda: started.append(1))
