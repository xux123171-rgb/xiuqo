...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"xiuqo-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "xiuqo-dev", "declared": True},
        },
    )
...
# `xiuqo curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `xiuqo curator pin <umbrella>` in the rename block so
