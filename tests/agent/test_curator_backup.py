...
def backup_env(monkeypatch, tmp_path):
    """Isolate XIUQO_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".xiuqo"
    home.mkdir()
    (home / "skills").mkdir()
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd xiuqo_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under XIUQO_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level XIUQO_DIR picks up the tmp HOME."""
    import xiuqo_constants
    importlib.reload(xiuqo_constants)
