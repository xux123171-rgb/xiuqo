"""Tests for XIUQO_HOME credential-file read blocking in file_safety.

Regression for https://github.com/xux123171-rgb/xiuqo/issues/17656 —
``read_file`` was previously only sandboxed against ``XIUQO_HOME`` itself,
...
for the credential stores while leaving arbitrary ``XIUQO_HOME`` files
...
def test_arbitrary_hermes_home_file_not_blocked(fake_home):
    """Non-credential files inside XIUQO_HOME stay readable."""
...
def test_identically_named_hermes_files_outside_home_not_blocked(
    ...
    """Xiuqo-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside XIUQO_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside XIUQO_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside XIUQO_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, XIUQO_HOME = <root>/profiles/<name>, but
