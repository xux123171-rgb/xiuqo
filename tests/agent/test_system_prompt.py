...
class TestNamedProfileHintIntegration:
    ...
    ``XIUQO_HOME`` and no resolver mocks.
    """
    ...
    def test_real_hermes_home_under_profiles_renders_correct_paths(
        ...
        assert "Active Xiuqo profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """XIUQO_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".xiuqo"
        ...
        assert "Active Xiuqo profile: default." in prompt
...
def test_coding_prompt_orders_shared_context_before_workspace(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "XIUQO_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "XIUQO_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_hermes_home", lambda: Path("/xiuqo"))

    # Production renders this as str(get_hermes_home()) + "/profiles/<name>/",
    # and str(Path("/xiuqo")) is platform-dependent (backslash on Windows) —
    # build the expectation the same way instead of hardcoding "/xiuqo".
    _home_str = str(Path("/xiuqo"))
    expected_profile = (
    ...
    with (
        ...
        patch("xiuqo_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("xiuqo_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under XIUQO_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from xiuqo_cli import config as _cfgmod
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("xiuqo_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from xiuqo_time import now as xiuqo_now
        sid = xiuqo_now().strftime("%Y%m%d_%H%M%S_fresh")
