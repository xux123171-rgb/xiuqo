...
conversation-affinity hint Xiuqo sends — ``prompt_cache_key`` on both
...
bucket it warmed. Xiuqo cannot infer the logical conversation from the id's
syntax (#79017's failure class), but it does not have to: the host declares
it through ``gateway_session_key`` (the ``X-Xiuqo-Session-Key`` /
...
class TestDeclaredConversationScope:
    ...
    def test_scope_never_carries_the_raw_key(self, db):
        ...
        A session id is a Xiuqo-internal token; a session KEY embeds the
...
class TestPeerIdentityIsSourceQualified:
    ...
    ``X-Xiuqo-Session-Key`` accepts any authenticated caller-supplied string,
...
class TestSourceOverrideDomain:
    ...
    ``XIUQO_SESSION_SOURCE``. Under an override both sides of a ``/new``
    ...
    def test_the_pre_row_source_matches_persistence(self, db, monkeypatch):
        ...
        monkeypatch.setenv("XIUQO_SESSION_SOURCE", "override-src")
