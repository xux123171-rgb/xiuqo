...
``~/.xiuqo/.anthropic_oauth.json`` for ``xiuqo_pkce`` ones.  Those singletons
...
def xiuqo_home(tmp_path, monkeypatch):
    """Real on-disk XIUQO_HOME so ``load_pool()`` re-reads what we persisted."""
    home = tmp_path / "xiuqo"
...
def test_hermes_oauth_writer_raises_instead_of_swallowing(xiuqo_home, monkeypatch):
...
def test_pool_hermes_pkce_fails_closed_and_reload_cannot_resurrect(
    ...
    reloaded = [
        e for e in load_pool("anthropic").entries() if e.source == "xiuqo_pkce"
    ]
    assert reloaded
    assert reloaded[0].refresh_token == _STALE_REFRESH
    assert reloaded[0].last_status == STATUS_DEAD
