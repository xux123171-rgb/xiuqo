...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("xiuqo_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated XIUQO_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="xiuqo_api_content_")
    os.makedirs(os.path.join(test_home, ".xiuqo"))
    prev_home = os.environ.get("XIUQO_HOME")
    os.environ["XIUQO_HOME"] = os.path.join(test_home, ".xiuqo")
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["XIUQO_HOME"] = prev_home
