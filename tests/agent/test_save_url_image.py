...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate XIUQO_HOME under tmp_path."""
    monkeypatch.setenv("XIUQO_HOME", str(tmp_path / ".xiuqo"))
    (tmp_path / ".xiuqo").mkdir()

    # Force the constants/image cache helpers to re-read XIUQO_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("xiuqo_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_hermes_home_cache(self, http_server):
        ...
        # The cache directory must be under XIUQO_HOME — gateway cleanup
