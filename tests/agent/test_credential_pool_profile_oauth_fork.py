...
Real imports, real temp XIUQO_HOME root + named profile, real auth.json I/O.
...
def fleet(tmp_path, monkeypatch):
    """Root XIUQO_HOME with an expired-but-refreshable Anthropic pool row."""
    ...
    # against $HOME/.xiuqo/auth.json; our root is elsewhere, so writes go.
    import xiuqo_constants
    xiuqo_constants._default_hermes_root_memo = None  # type: ignore[attr-defined]
    ...
    store = {
        ...
        "credential_pool": {
            "anthropic": [{
                ...
                "priority": 0, "source": "manual:xiuqo_pkce",
    ...
    import xiuqo_cli.auth as _auth_mod
...
def _profile(fleet, name, **kw):
    from xiuqo_cli.profiles import create_profile
...
def test_strip_helper_fails_closed_when_root_store_cannot_be_resolved(fleet, monkeypatch):
    ...
    monkeypatch.setattr(
        xiuqo_constants, "get_default_hermes_root",
...
def test_borrower_prune_never_deletes_root_singleton_grant(fleet, tmp_path):
    """Root's xiuqo_pkce row is seeded from ROOT's .anthropic_oauth.json; a
    ...
    assert [e.source for e in root_rows] == ["xiuqo_pkce"]

    kid = _profile(fleet, "kid")
    fleet["use"](kid)
    pool = load_pool("anthropic")
    assert [e.source for e in pool.entries()] == ["xiuqo_pkce"], "borrowed root grant was pruned"
    assert fleet["rows"](root) and fleet["rows"](root)[0]["source"] == "xiuqo_pkce"
...
def test_profile_auth_add_owns_only_its_own_rows(fleet):
    ...
    pool.add_entry(PooledCredential(
        ...
        priority=0, source="manual:xiuqo_pkce", access_token="sk-ant-oat01-MINE",
...
def test_heal_consolidates_existing_forks_to_the_live_copy(fleet, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="xiuqo_cli.auth"):
        fleet["use"](forge)
        sel = load_pool("anthropic").select()
...
def test_heal_is_idempotent_and_logs_once(fleet, caplog):
    import logging
    from agent.credential_pool import load_pool
    from xiuqo_cli.auth import consume_oauth_heal_notices, heal_forked_single_use_oauth_grants
...
def test_heal_never_deletes_the_only_surviving_copy(fleet):
    """Root lost its grant (user ran `xiuqo auth remove` at root); the profile's
...
def test_heal_preserves_independent_grants_for_same_account(fleet, shape, claims):
    ...
    import base64
    from xiuqo_cli.auth import heal_forked_single_use_oauth_grants
...
def test_heal_pkce_singleton_shape_commits_live_pair_to_root_singleton(fleet):
    """`xiuqo auth` PKCE shape: root + profile each have .anthropic_oauth.json +
    a xiuqo_pkce-seeded row; the profile's copy is the rotated (live) one."""
    from agent.credential_pool import load_pool
    ...
    fleet["use"](root)
    load_pool("anthropic")  # seeds root's xiuqo_pkce row from the singleton
...
def test_heal_skips_profile_auth_json_symlinked_to_the_root_store(fleet):
    """#101356: `ln -s ~/.xiuqo/auth.json <profile>/auth.json` shares ONE store.
...
# `xiuqo` invocation re-paid its two nested EXCLUSIVE auth-store locks to
...
def _new_process(auth_mod):
    """Simulate a fresh `xiuqo` invocation: in-memory state gone, disk kept."""
    auth_mod._oauth_heal_clean_marks.clear()
    auth_mod._global_auth_store_cache = None
...
def _count_locks(monkeypatch):
    ...
    The heal imports the lock from ``xiuqo_cli.auth`` inside the function, so
...
def test_clean_mark_persists_so_a_fresh_process_takes_no_auth_lock(fleet, monkeypatch):
    import xiuqo_cli.auth as auth_mod
    from xiuqo_cli import auth_oauth_grants as grants
