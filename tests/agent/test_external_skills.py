...
def xiuqo_home(tmp_path):
    """Create a minimal XIUQO_HOME with config."""
    home = tmp_path / ".xiuqo"
    home.mkdir()
    (home / "skills").mkdir()
    return home
...
class TestGetExternalSkillsDirs:
    def test_empty_config(self, xiuqo_home):
        (xiuqo_home / "config.yaml").write_text("skills:\n  external_dirs: []\n")
    ...
    def test_valid_dir_returned(self, xiuqo_home, external_skills_dir):
...
class TestGetAllSkillsDirs:
    def test_local_always_first(self, xiuqo_home, external_skills_dir):
        (xiuqo_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        with patch.dict(os.environ, {"XIUQO_HOME": str(xiuqo_home)}):
        ...
        assert result[0] == xiuqo_home / "skills"
        assert result[1] == external_skills_dir.resolve()


class TestExternalSkillsInFindAll:
    def test_external_skills_found(self, xiuqo_home, external_skills_dir):
        (xiuqo_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        local_skills = xiuqo_home / "skills"
        with (
            patch.dict(os.environ, {"XIUQO_HOME": str(xiuqo_home)}),
    ...
    def test_local_takes_precedence(self, xiuqo_home, external_skills_dir):
...
class TestExternalSkillView:
    def test_skill_view_finds_external(self, xiuqo_home, external_skills_dir):
        (xiuqo_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        local_skills = xiuqo_home / "skills"
