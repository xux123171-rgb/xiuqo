...
@pytest.fixture
def xiuqo_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.xiuqo/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".xiuqo"
    home.mkdir()
...
def test_cache_invalidates_on_mtime_change(xiuqo_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = xiuqo_home_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different XIUQO_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".xiuqo"
