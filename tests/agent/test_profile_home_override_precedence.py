...
1. MULTIPLEX INVERSION (@kshitijk4poor): the messaging gateway hands every
   ...
   via the XIUQO_HOME ContextVar (copy_context into the worker). A bound
...
def test_full_prompt_scoped_to_bot_on_bare_thread(tmp_path, monkeypatch):
    ...
    assert "Active Xiuqo profile: mybot" in prompt
    assert "Active Xiuqo profile: default" not in prompt
