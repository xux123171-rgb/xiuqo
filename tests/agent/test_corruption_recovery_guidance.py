...
3. Improved "corrupt" cause wording in _format_turn_completion_explanation
   with the full recovery path (xiuqo doctor, sqlite3 .recover, backups)
"""
...
def test_format_turn_completion_corrupt_includes_recovery_options():
    ...
    assert "xiuqo doctor" in explanation
...
def test_gateway_corruption_banner_backups_dir_follows_hermes_home(monkeypatch, tmp_path):
    """The gateway broadcast's step 3 must name the live backups dir, not ~/.xiuqo (#104250).

    Pre-update backups live at ``<xiuqo_root>/backups`` (``xiuqo_cli/backup.py``); a
    custom-XIUQO_HOME gateway must not be told to restore from a directory that never
    held its backups.
    """
    import asyncio
    ...
    custom_home = tmp_path / "custom-xiuqo-home"
    monkeypatch.setenv("XIUQO_HOME", str(custom_home / "profiles" / "research"))
    ...
    assert "~/.xiuqo/backups" not in sent[0]
...
def test_format_turn_completion_corrupt_never_names_the_live_db():
    ...
    assert 'sqlite3 ~/.xiuqo/state.db ".recover"' not in explanation
    # The replacement guidance names the safe command.
    assert "xiuqo sessions recover --source" in explanation
...
def test_corrupt_guidance_pins_the_failing_profile(tmp_path, monkeypatch):
    """#105887: every `xiuqo ...` command in the recovery guidance (turn explainer, gateway
    home-channel notice, exhausted-repair diagnostic) carries the active profile selector and
    names that profile's state.db. A bare `xiuqo` follows the sticky ``active_profile`` file,
    so with another profile active the operator would repair the wrong database."""
    import asyncio

    import gateway.run as gateway_run
    from xiuqo_state import _default_db_path
    from xiuqo_state_repair import _persistent_repair_exhausted_error
    from run_agent import AIAgent
    ...
    commands = [line.strip() for line in explanation.splitlines() if "xiuqo " in line]
    assert commands and all("xiuqo -p research " in line for line in commands), commands
    # The conftest pins xiuqo_state.DEFAULT_DB_PATH, so the store named is whatever the
    ...
    monkeypatch.setattr(runner, "_send_home_channel_message", _capture_send)
    asyncio.run(runner._send_session_db_warning_notifications())
    notice_commands = [line.strip() for line in sent[0].splitlines() if "xiuqo " in line]
    assert notice_commands and all("xiuqo -p research " in line for line in notice_commands), notice_commands
    assert f"--source {_default_db_path()} " in sent[0]

    exhausted = _persistent_repair_exhausted_error(home / "state.db")
    assert "`xiuqo -p research sessions recover --source" in exhausted
