...
def _make_agent(max_iterations: int = 10, config: dict | None = None) -> AIAgent:
    with (
        ...
        patch("xiuqo_cli.config.load_config", return_value=config or {}),
...
def test_explanation_persistence_turn_lease_cause_is_specific():
    ...
    assert "xiuqo doctor" not in lower
...
def test_explanation_persistence_corrupt_backups_dir_follows_hermes_home(monkeypatch, tmp_path):
    """Step 3 must name the backups dir under the ACTIVE home, not ~/.xiuqo (#104250).

    Pre-update backups live at ``<xiuqo_root>/backups`` (``xiuqo_cli/backup.py``), so a
    custom-XIUQO_HOME deployment told to restore from ``~/.xiuqo/backups/`` is misdirected
    ...
    assert "~/.xiuqo/backups" not in out
...
def test_deleted_wal_cause_is_enumerated_and_points_to_retired_capture():
    from xiuqo_state_errors import PERSISTENCE_ERROR_CAUSES
...
def test_classify_persistence_error_categories():
    ...
    from xiuqo_state import classify_persistence_error
...
def test_classify_persistence_error_reuses_disk_full_markers():
    """The disk bucket delegates to xiuqo_state_errors.is_disk_full_error, so
...
def test_classify_persistence_error_compression_busy_is_distinct():
    ...
    from xiuqo_state import SessionCompressionInProgressError
    from xiuqo_state_errors import CompressionSessionBusyError
    from xiuqo_state import classify_persistence_error
...
def test_classify_persistence_error_turn_lease_lost_is_distinct():
    from xiuqo_state import classify_persistence_error
    from xiuqo_state_errors import SessionTurnLeaseLostError
...
def test_classify_persistence_error_fts_provenance_order():
    ...
    from xiuqo_state import SessionDB, classify_persistence_error
    from xiuqo_state_errors import SQLITE_CORRUPT_VTAB, is_fts_scoped_corruption_error
...
def test_explainer_enabled_by_default():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("XIUQO_TURN_COMPLETION_EXPLAINER", None)
...
def test_explainer_disabled_via_env():
    agent = _make_agent()
    with patch.dict(
        os.environ, {"XIUQO_TURN_COMPLETION_EXPLAINER": "0"}, clear=False
    ):
...
def test_explainer_config_read_once_then_cached():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("XIUQO_TURN_COMPLETION_EXPLAINER", None)
        with patch("xiuqo_cli.config.load_config", counting_load):
...
def test_classify_persistence_error_quarantined_handle_is_corrupt() -> None:
    ...
    from xiuqo_state import StateDbCorruptError, classify_persistence_error
