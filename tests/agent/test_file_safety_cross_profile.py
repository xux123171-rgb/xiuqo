...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Xiuqo root with two profiles, monkeypatch the
...
def _set_active_home(monkeypatch, xiuqo_home: Path):
...
class TestResolveActiveProfileName:
    ...
    def test_falls_back_to_default_on_resolution_failure(self, fake_hermes, monkeypatch):
        """If XIUQO_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs
