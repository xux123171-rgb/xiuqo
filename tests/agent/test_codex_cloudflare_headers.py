...
The official Codex endpoint must receive Xiuqo' own harness identity, rather
...
These tests pin:
- the required Xiuqo originator
- the versioned Xiuqo User-Agent
...
class TestPrimaryClientWiring:
    ...
    def test_apply_client_headers_on_base_url_change(self):
        ...
        with patch("agent.process_bootstrap.OpenAI") as mock_openai:
            ...
            assert headers.get("originator") == "xiuqo"
            assert headers.get("ChatGPT-Account-ID") == "acct-rotation"
            assert headers.get("User-Agent") == f"XiuqoAgent/{__version__}"
