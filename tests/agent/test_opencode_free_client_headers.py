...
def test_opencode_free_sends_hermes_attribution(mock_openai):
    """Keyless requests still identify as Xiuqo (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "Xiuqo Agent"
    assert str(headers.get("User-Agent", "")).startswith("XiuqoAgent/")
