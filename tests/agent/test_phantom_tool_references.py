...
Covers:
  * XIUQO_AGENT_HELP_GUIDANCE degrades to the docs-only variant when the
...
class TestXiuqoAgentHelpGuidance:
    def test_skill_variant_used_when_skill_view_present(self):
        from agent.prompt_builder import XIUQO_AGENT_HELP_GUIDANCE
        assert "skill_view(name='xiuqo')" in XIUQO_AGENT_HELP_GUIDANCE

    def test_no_skills_variant_has_no_skill_view_reference(self):
        from agent.prompt_builder import XIUQO_AGENT_HELP_GUIDANCE_NO_SKILLS
        assert "skill_view" not in XIUQO_AGENT_HELP_GUIDANCE_NO_SKILLS
        assert "hermes-agent.nousresearch.com/docs" in XIUQO_AGENT_HELP_GUIDANCE_NO_SKILLS
...
class TestEssentialOnlySync:
    def test_opted_out_sync_seeds_only_essential(self, monkeypatch, tmp_path):
        """A profile with .no-bundled-skills still gets the xiuqo skill."""
        ...
        assert result["copied"] == ["xiuqo"]
        assert (home / "skills" / "autonomous-ai-agents" / "xiuqo" / "SKILL.md").exists()
