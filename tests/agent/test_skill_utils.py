...
class TestParseConfigStringList:
    """#86661: `xiuqo config set` and JSON-mode editor saves store lists as
    ...
    def test_python_literal_array_string_parses(self):
        # `xiuqo config set` can persist single-quoted Python-literal forms.
...
def test_skill_config_home_vars_use_subprocess_home(tmp_path, monkeypatch):
    ...
    xiuqo_home = tmp_path / "da\\ta"
    subprocess_home = xiuqo_home / "home"
    subprocess_home.mkdir(parents=True)
    (xiuqo_home / "config.yaml").write_text("", encoding="utf-8")
    monkeypatch.setenv("XIUQO_HOME", str(xiuqo_home))
    monkeypatch.setenv("HOME", str(xiuqo_home))
