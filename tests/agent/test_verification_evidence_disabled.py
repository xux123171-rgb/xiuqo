...
def project(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("XIUQO_VERIFY_ON_STOP", raising=False)
...
def test_disabled_guard_never_touches_the_ledger(project, tmp_path):
    db = tmp_path / ".xiuqo" / "verification_evidence.db"
...
def test_enabled_guard_records_into_the_ledger(project, tmp_path, monkeypatch):
    ...
    assert Path(tmp_path / ".xiuqo" / "verification_evidence.db").exists()
