...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    xiuqo_state = ModuleType("xiuqo_state")
    xiuqo_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "xiuqo_state", xiuqo_state)
    xiuqo_state_registry = ModuleType("xiuqo_state_registry")
    xiuqo_state_registry.acquire = lambda db_path=None: sentinel_db
    monkeypatch.setitem(sys.modules, "xiuqo_state_registry", xiuqo_state_registry)
    ...
    result = json.loads(agent._invoke_tool(
        "session_search",
        {"query": "Xiuqo", "detail": "full"},
        "task-id",
    ))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Xiuqo"
...
def test_sequential_session_search_forwards_detail(monkeypatch):
    ...
    tool_call = SimpleNamespace(
        ...
        function=SimpleNamespace(
            name="session_search",
            arguments=json.dumps({"query": "Xiuqo", "detail": "full"}),
        ),
    )
