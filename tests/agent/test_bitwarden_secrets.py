...
binary download are exercised manually by `xiuqo secrets bitwarden
...
def xiuqo_home(tmp_path, monkeypatch):
    """Point Xiuqo at an isolated home directory."""
    home = tmp_path / ".xiuqo"
    home.mkdir()
    monkeypatch.setenv("XIUQO_HOME", str(home))
    # Some modules cache get_hermes_home; clear if needed.
    import xiuqo_constants
    if hasattr(xiuqo_constants, "_XIUQO_HOME_CACHE"):
        xiuqo_constants._XIUQO_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(xiuqo_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from xiuqo_cli.env_loader import _apply_external_secret_sources
