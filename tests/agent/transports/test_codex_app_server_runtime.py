...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Xiuqo passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("XIUQO_HOME", "/users/alice/.xiuqo/profiles/backend-worker")
        monkeypatch.setenv("XIUQO_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "XIUQO_KANBAN_DB",
            "/users/alice/.xiuqo/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.xiuqo/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through xiuqo_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Xiuqo secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        ...
        for var in (
            ...
            "XIUQO_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
