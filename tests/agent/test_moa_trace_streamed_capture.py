...
path via ``xiuqo chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp XIUQO_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point XIUQO_HOME at a temp dir and turn moa.save_traces on."""
    xiuqo_home = tmp_path / ".xiuqo"
    xiuqo_home.mkdir()
    monkeypatch.setenv("XIUQO_HOME", str(xiuqo_home))

    # save_moa_turn reads config via xiuqo_cli.config.load_config; stub it to
