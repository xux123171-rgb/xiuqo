...
`xiuqo_cli/runtime_provider.py`), and the echo flag must still take effect for
it. A future refactor keying the flag on provider *name* would reintroduce that
miss; this test is the tripwire.

Uses a temp XIUQO_HOME + real `load_config_readonly` (the config cache is
...
def _write_home(tmp_path, monkeypatch, reasoning_echo: bool):
    """Point XIUQO_HOME at a temp profile declaring a named custom provider."""
    home = tmp_path / "xiuqo"
    home.mkdir()
    lines = [
        "model:",
    ...
    try:
        from xiuqo_cli import config as _cfg
