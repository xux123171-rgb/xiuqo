...
def xiuqo_home(tmp_path, monkeypatch):
    """Real on-disk XIUQO_HOME so ``load_pool()`` re-reads what it persisted."""
    home = tmp_path / "xiuqo"
    ...
    monkeypatch.setattr(
        "xiuqo_cli.auth.is_provider_explicitly_configured", lambda pid: True
    )
    return home
...
def test_persisted_claude_code_row_carries_no_token_material(
    ...
    rows = [r for r in _persisted_rows(xiuqo_home) if r.get("source") == "claude_code"]
    assert len(rows) == 1
...
def test_pool_store_sync_never_adopts_a_borrowed_row(xiuqo_home, claude_credentials):
...
def test_refresh_from_persisted_sanitized_row_keeps_the_full_pair(
    xiuqo_home, claude_credentials, monkeypatch
):
...
def test_empty_oauth_entry_is_never_leased(xiuqo_home, claude_credentials):
...
def test_hermes_pkce_row_still_syncs_from_the_pool_store(monkeypatch):
    ...
    ``xiuqo_pkce`` *is* pool-owned, so its persisted row keeps its tokens and
