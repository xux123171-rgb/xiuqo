...
def test_create_job_no_agent_requires_script(xiuqo_env):
...
def test_update_job_roundtrips_no_agent_flag(xiuqo_env):
    from cron.jobs import create_job, update_job, get_job

    script_path = xiuqo_env / "scripts" / "w.sh"
...
def test_cronjob_tool_create_no_agent_without_script_errors(xiuqo_env):
...
def test_run_job_no_agent_success_returns_script_stdout(xiuqo_env):
    """Happy path: script exits 0 with output, delivered verbatim."""
    from cron.jobs import create_job
    from cron.scheduler import run_job

    script_path = xiuqo_env / "scripts" / "alert.sh"
...
def test_run_job_no_agent_reloads_dotenv_before_script(xiuqo_env, monkeypatch):
    ...
    def fake_load(*, xiuqo_home=None, project_env=None):
        loaded_homes.append(xiuqo_home)
        return []

    monkeypatch.setattr(env_loader, "load_hermes_dotenv", fake_load)

    script_path = xiuqo_env / "scripts" / "probe.sh"
    script_path.write_text('#!/bin/bash\necho "ok"\n')
...
def test_timed_out_no_agent_script_delivery_is_not_mislabeled_as_provider_failure(
    ...
    (xiuqo_env / "scripts" / "slow.py").write_text("import time; time.sleep(999)\n")
    job = create_job(
        prompt=None,
        schedule="every 5m",
        script="slow.py",
...
def test_agent_provider_timeout_delivery_keeps_fallback_guidance(xiuqo_env, monkeypatch):
...
def test_run_job_script_path_traversal_still_blocked(xiuqo_env):
...
def test_run_job_script_nul_path_fails_cleanly(xiuqo_env):
...
def test_run_job_script_nul_rejected_before_any_path_call(xiuqo_env, monkeypatch):
...
def test_run_job_script_accepts_pathlike_script_path(xiuqo_env):
    ...
    script = xiuqo_env / "scripts" / "probe.py"
...
@pytest.mark.parametrize(
    "error",
    [
        "Script timed out after 900s: /home/u/.xiuqo/scripts/nightly.sh",
