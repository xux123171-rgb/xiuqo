...
def test_standalone_fallback_pool_keeps_profile_scope(tmp_path, monkeypatch):
    ...
    from xiuqo_constants import get_hermes_home, set_hermes_home_override
...
def test_desktop_ticker_gates_on_profile_gateway_running(tmp_path, monkeypatch, profile_count):
    ...
    running = {homes[-1][1]}
    monkeypatch.setattr(
        "xiuqo_cli.profiles.profiles_to_serve", lambda multiplex=False: list(homes)
    )
    monkeypatch.setattr(
        "xiuqo_cli.profiles._check_gateway_running", lambda home: home in running
    )
    monkeypatch.setattr("xiuqo_cli.profiles._served_by_running_multiplexer", lambda name: False)
    captured = {}
    ...
    monkeypatch.setattr("xiuqo_logging.enable_profile_log_routing", lambda homes: None)
