...
kind) on every recurring dispatch so `xiuqo cron list` and
`xiuqo cron status` can surface late catch-ups.
"""
...
class TestDueScanDispatchStamp:
    def test_catch_up_beyond_grace_stamped_and_persisted(self, cron_store):
        ...
        # Persisted, so a separate `xiuqo cron list` process can read it.
