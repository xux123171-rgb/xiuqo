...
builds must resolve exactly as it would under a standalone ``xiuqo -p <name> gateway run``.
"""
...
def two_homes(tmp_path, monkeypatch):
    ...
    (root / ".env").write_text(
        "XIUQO_CRON_TIMEOUT=111\nXIUQO_MODEL=default-model\nTERMINAL_ENV=docker\n"
        "TERMINAL_DOCKER_IMAGE=default-only-image\nXIUQO_LANGUAGE=en\n")
    (alpha / ".env").write_text("XIUQO_CRON_TIMEOUT=222\nXIUQO_LANGUAGE=zh\n")
    ...
    for key, val in (("XIUQO_CRON_TIMEOUT", "111"), ("XIUQO_MODEL", "default-model"),
...
def test_cron_env_settings_resolve_from_the_served_profile(two_homes):
    """``XIUQO_CRON_TIMEOUT`` / ``XIUQO_MODEL`` are alpha's (or absent) both with the fire-time
    ...
    try:
        ...
        try:
            ...
            assert captured["target_model"] == ""  # alpha has no XIUQO_MODEL; default's must not leak
...
def test_child_env_for_served_profile_drops_launch_profile_settings(two_homes):
    ...
    try:
        ...
        reset_secret_scope(token)
    for key in ("XIUQO_MODEL", "TERMINAL_ENV", "TERMINAL_DOCKER_IMAGE", "XIUQO_LANGUAGE", "XIUQO_CRON_TIMEOUT"):
        assert key not in env, key
    assert env["XIUQO_HOME"] == str(alpha)
    assert "PATH" in env
    ...
    assert env["XIUQO_MODEL"] == "default-model"
