...
``xiuqo_time`` under the process's own startup profile, then ticks OTHER
...
def test_foreign_process_tick_persists_owning_profile_offset(
    ...
    monkeypatch.setenv("XIUQO_HOME", str(backend_home))

    # Backend process resolves its own timezone first (process startup).
    assert xiuqo_time.now().utcoffset() == datetime.now(
        ZoneInfo("UTC")
    ).utcoffset()
