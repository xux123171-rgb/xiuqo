...
These exercise the real store against a temp XIUQO_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated XIUQO_HOME so jobs.json doesn't touch the real store."""
...
def test_fresh_claim_from_a_dead_same_host_owner_is_reclaimable(temp_home):
    ...
    ``xiuqo cron run`` killed mid-flight must not block the next manual run for the whole TTL
