...
Drives the REAL ``tick()`` against a throwaway XIUQO_HOME with a ``no_agent`` script job that
...
def slot_env(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("XIUQO_MACHINE_ID", raising=False)

    import cron.executions as E
    import cron.jobs as J
    import cron.scheduler as S

    monkeypatch.setattr(J, "XIUQO_DIR", home)
    ...
    def crash_before_dispatch() -> None:
        ...
        env = dict(os.environ, XIUQO_HOME=str(home))
