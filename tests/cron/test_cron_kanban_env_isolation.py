...
normal ``xiuqo chat -q`` CLI agent (its default toolset includes ``cronjob``)
running with ``XIUQO_KANBAN_TASK`` legitimately set in its own environment,
...
``$XIUQO_KANBAN_TASK`` — letting an unrelated cron job close the worker's task
...
def worker_env(monkeypatch):
    ...
    monkeypatch.setenv("XIUQO_KANBAN_TASK", "t_worker_real_task")
    monkeypatch.setenv("XIUQO_KANBAN_WORKSPACE", "/tmp/ws")
    monkeypatch.setenv("XIUQO_KANBAN_RUN_ID", "42")
    monkeypatch.setenv("XIUQO_KANBAN_CLAIM_LOCK", "lock-abc")
    monkeypatch.setenv("XIUQO_KANBAN_BOARD", "team-alpha")
...
class TestRunJobKanbanIsolation:
    ...
    def test_environment_is_left_untouched(self, monkeypatch, worker_env):
        ...
        before = {
            k: v for k, v in os.environ.items() if k.startswith("XIUQO_KANBAN_")
        }
    ...
    def test_context_reset_even_when_job_raises(self, monkeypatch, worker_env):
        ...
        assert os.environ.get("XIUQO_KANBAN_BOARD") == "team-alpha"
...
def test_dispatcher_grants_only_the_assigned_worker_scope(tmp_path, monkeypatch):
    ...
    from xiuqo_cli.kanban_db_dispatch import _default_spawn

    monkeypatch.setenv("HOME", str(tmp_path))
    db = tmp_path / "board.db"
    monkeypatch.setenv("XIUQO_KANBAN_DB", str(db))
    conn = connect(db)
    ...
    worker.chmod(0o700)
    monkeypatch.setenv("XIUQO_BIN", str(worker))
    # Building a new worker under an existing task must replace, not inherit, its scope.
    monkeypatch.setenv("XIUQO_KANBAN_TASK", "prior-task")
    ...
    assert os.environ["XIUQO_KANBAN_TASK"] == "prior-task"
    conn.close()
