...
resolve_runtime_provider against a temp XIUQO_HOME) and the create_job snapshot capture.
"""
...
def _run(job, tmp_path, *, current_provider="openrouter", current_model=None, cron_model=None,
    ...
    with patch("cron.scheduler._hermes_home", tmp_path), \
         ...
         patch("xiuqo_cli.runtime_provider.resolve_runtime_provider", side_effect=_resolve), \
...
class TestSnapshotIsTheEffectivePin:
    ...
    def test_missing_model_guides_to_user_owned_cli(self, tmp_path, monkeypatch):
        ...
        assert "xiuqo cron edit pin-test --model <name>" in error
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("xiuqo_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
