...
CronScheduler plan, .xiuqo/plans/cron-scheduler-provider-interface.md). They
...
No production code is exercised beyond the two ticker entry points:
  ...
  - xiuqo_cli/web_server.py::_start_desktop_cron_ticker  (desktop fallback)
...
def test_desktop_ticker_calls_tick_then_stops():
    ...
    no adapters/loop."""
    from xiuqo_cli.web_server import _start_desktop_cron_ticker
...
def test_multiplex_ticker_reenumerates_profiles_each_cycle(tmp_path):
    ...
    def _tick(*args, **kwargs):
        ...
        if len(ticked) == 1:  # "xiuqo profile create gamma" happens between two cycles
