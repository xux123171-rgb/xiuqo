...
class TestParseSchedule:
    ...
    def test_naive_iso_anchors_to_configured_tz_not_server_local(self, monkeypatch):
        """A naive ISO timestamp must be interpreted in the CONFIGURED Xiuqo
...
class TestNaiveScheduleTimezoneDivergence:
    ...
    become due even when the configured Xiuqo timezone differs from the
...
class TestGetDueJobs:
    ...
    def test_run_claim_heartbeat_keeps_long_run_claimed_past_ttl(
        ...
        monkeypatch.delenv("XIUQO_CRON_TIMEOUT", raising=False)
...
class TestLateEnvRepointScopesStore:
    """A XIUQO_HOME set AFTER cron.jobs import must scope the store even
    ...
    def test_use_cron_store_override_still_wins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIUQO_HOME", str(tmp_path / "env-home"))
    ...
    def test_public_io_after_late_env_repoint_leaves_old_file_untouched(
        ...
        called after a post-import XIUQO_HOME repoint must operate on the NEW
        ...
        monkeypatch.setattr(jobs, "XIUQO_DIR", sim_old_home)
        ...
        monkeypatch.setenv("XIUQO_HOME", str(new_home))
...
class TestJobsJsonIdKeyedMap:
    ...
    or hand edit — Xiuqo' own save_jobs() only ever writes a list) made
    ...
    required`` and took down ``xiuqo cron list``, the ``cronjob(action=
    ...
    def test_list_jobs_survives_id_keyed_map(self, tmp_cron_dir):
        """The reported traceback path (xiuqo cron list / cronjob list tool)."""
        import json
