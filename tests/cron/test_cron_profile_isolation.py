...
profile's XIUQO_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.xiuqo/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active XIUQO_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever XIUQO_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    XIUQO_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import xiuqo_constants

    monkeypatch.setattr(
        xiuqo_constants, "_get_platform_default_hermes_home", lambda: root
    )
    monkeypatch.setenv("XIUQO_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile XIUQO_HOME (<root>/profiles/<name>), the cron store
    ...
    assert xiuqo_constants.get_hermes_home().resolve() == profile_home.resolve()
    assert xiuqo_constants.get_default_hermes_root().resolve() == root.resolve()

    # cron/jobs.py computes XIUQO_DIR from get_hermes_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.XIUQO_DIR.resolve() == profile_home.resolve()
        assert (
