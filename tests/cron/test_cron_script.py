...
- Path resolution (absolute, relative to XIUQO_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp XIUQO_HOME."""
    xiuqo_home = tmp_path / ".xiuqo"
    xiuqo_home.mkdir()
    (xiuqo_home / "cron").mkdir()
    (xiuqo_home / "cron" / "output").mkdir()
    (xiuqo_home / "scripts").mkdir()
    monkeypatch.setenv("XIUQO_HOME", str(xiuqo_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "XIUQO_DIR", xiuqo_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", xiuqo_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", xiuqo_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", xiuqo_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Xiuqo provider env (SECURITY.md §2.3)."""
        from tools.environments.local_env_policy import _XIUQO_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler_script import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_XIUQO_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.xiuqo/scripts/ must be rejected."""
        from cron.scheduler_script import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("XIUQO_SESSION_PLATFORM") is None
        assert os.environ.get("XIUQO_SESSION_CHAT_ID") is None
        assert os.environ.get("XIUQO_SESSION_CHAT_NAME") is None
...
class TestScriptTimeoutTreeKill:
    ...
    def test_timeout_leaves_no_setsid_grandchild(self, cron_env, monkeypatch):
        ...
        monkeypatch.setenv("XIUQO_CRON_SCRIPT_TIMEOUT", "2")
