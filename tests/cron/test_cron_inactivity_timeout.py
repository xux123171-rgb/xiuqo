...
- Unlimited timeout (XIUQO_CRON_TIMEOUT=0)
- Backward compat: XIUQO_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """XIUQO_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """XIUQO_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("XIUQO_CRON_TIMEOUT", "1200")
        raw = os.getenv("XIUQO_CRON_TIMEOUT", "").strip()
...
class TestSysPathOrdering:
    ...
    def test_xiuqo_time_importable(self):
        """xiuqo_time should be importable when cron.scheduler loads."""
