...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("xiuqo_cli.config.get_config_path", return_value=config_path), \
             patch("xiuqo_cli.config.ensure_hermes_home"):
    ...
    def test_save_env_value_sets_0600(self):
        ...
        with patch("xiuqo_cli.config.get_env_path", return_value=env_path), \
             patch("xiuqo_cli.config.ensure_hermes_home"):
    ...
    def test_ensure_hermes_home_sets_0700(self):
        home = Path(self.tmpdir) / ".xiuqo"
        with patch("xiuqo_cli.config.get_hermes_home", return_value=home):
...
class TestSecureHelpers(unittest.TestCase):
    ...
    def test_secure_dir_preserves_operator_mode_in_container(self):
        ...
        an explicit XIUQO_HOME_MODE is still honored (#10757)."""
        from cron.jobs import _secure_dir
        with tempfile.TemporaryDirectory() as tmp:
            d = Path(tmp) / "cron"
            d.mkdir()
            os.chmod(d, 0o755)
            with patch.dict(os.environ, {"XIUQO_CONTAINER": "1"}, clear=False):
                os.environ.pop("XIUQO_HOME_MODE", None)
                _secure_dir(d)
                self.assertEqual(stat.S_IMODE(os.stat(d).st_mode), 0o755)
                os.environ["XIUQO_HOME_MODE"] = "0701"
                _secure_dir(d)
