"""
Tests for timezone support (xiuqo_time module + integration points).
...
def _reset_xiuqo_time_cache():
    """Reset the xiuqo_time module cache."""
    xiuqo_time.reset_cache()


# =========================================================================
# xiuqo_time.now() — core helper
# =========================================================================
...
class TestHermesTimeNow:
    ...
    def setup_method(self):
        _reset_xiuqo_time_cache()

    def teardown_method(self):
        _reset_xiuqo_time_cache()
        os.environ.pop("XIUQO_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["XIUQO_TIMEZONE"] = "Asia/Kolkata"
        result = xiuqo_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["XIUQO_TIMEZONE"] = "UTC"
        result = xiuqo_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["XIUQO_TIMEZONE"] = "America/New_York"
        result = xiuqo_time.now()
        assert result.tzinfo is not None
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["XIUQO_TIMEZONE"] = "Europe/London"
        tz = xiuqo_time.get_timezone()
        assert isinstance(tz, ZoneInfo)
        assert str(tz) == "Europe/London"

    def test_cache_isolated_by_active_profile_config(self, tmp_path, monkeypatch):
        """Switching XIUQO_HOME must not reuse another profile's timezone."""
        ...
        monkeypatch.delenv("XIUQO_TIMEZONE", raising=False)

        monkeypatch.setenv("XIUQO_HOME", str(first_home))
        assert str(xiuqo_time.get_timezone()) == "Asia/Tokyo"

        # Multiplexed profile runtime scopes switch XIUQO_HOME in one process.
        monkeypatch.setenv("XIUQO_HOME", str(second_home))
        assert str(xiuqo_time.get_timezone()) == "America/New_York"
    ...
    def test_multiplex_prefers_routed_profile_config_over_env(self, tmp_path, monkeypatch):
        """Under the multiplexed gateway XIUQO_TIMEZONE holds only the DEFAULT profile's value
        ...
        monkeypatch.setenv("XIUQO_TIMEZONE", "America/New_York")

        # Single-profile process: env stays authoritative.
        assert xiuqo_time.get_timezone_name() == "America/New_York"

        set_multiplex_active(True)
        try:
            assert xiuqo_time.get_timezone_name() == "America/New_York"  # default profile turn
            token = set_hermes_home_override(str(routed_home))
            try:
                assert xiuqo_time.get_timezone_name() == "Asia/Tokyo"
                assert str(xiuqo_time.get_timezone()) == "Asia/Tokyo"
            finally:
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When XIUQO_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) XIUQO_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("XIUQO_TIMEZONE=" + os.environ.get("XIUQO_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "XIUQO_TIMEZONE=NOT_SET" in result["output"], (
            "XIUQO_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When XIUQO_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("XIUQO_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=xiuqo_tz) which shifted
        absolute time when system-local tz != Xiuqo tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Xiuqo tz
        ...
        # Use a Xiuqo timezone far behind UTC so that the numeric wall time
        ...
        os.environ["XIUQO_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_xiuqo_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Xiuqo timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["XIUQO_TIMEZONE"] = "US/Eastern"
        _reset_xiuqo_time_cache()
