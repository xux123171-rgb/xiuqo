...
still be registered under the parent Desktop/TUI session (XIUQO_UI_SESSION_ID + exact steer
...
def test_slash_review_dispatches_under_the_parent_session_identity(monkeypatch):
    ...
    def fake_start_review(agent, snapshot, prompt):
        ...
        seen["ui_session_id"] = get_session_env("XIUQO_UI_SESSION_ID", "")
    ...
    try:
        ...
        from gateway.session_context import get_session_env
        assert get_session_env("XIUQO_UI_SESSION_ID", "") == ""  # scope cleared after dispatch
