...
by surface:
  ...
  * ``tui_gateway.entry`` — the stdio ``xiuqo --tui`` path.
  * ``xiuqo_cli.mcp_startup`` — the desktop app + dashboard WebSocket sidecar
    (``tui_gateway/ws.py``) and ``xiuqo dashboard``.
...
``xiuqo_cli.mcp_startup``), so the scheduler bailed immediately and a slow MCP
...
import xiuqo_cli.mcp_startup as startup
...
def test_entry_in_flight_sees_startup_thread(clean_discovery_globals):
    """Desktop/dashboard surface: discovery thread lives on xiuqo_cli.mcp_startup.

    The entry-level in-flight check must report True so the late-refresh
    scheduler does not bail (the #51587 bug).
    """
    stop = threading.Event()
    startup._mcp_discovery_thread[xiuqo_home_key()] = _alive_thread(stop)
    try:
