...
def test_parentless_seed_survives_a_restart_and_hides_its_runbook(monkeypatch, tmp_path):
    ...
    try:
        result = _create({
            "cols": 96, "source": "desktop", "title": "Welcome to Xiuqo",
            "messages": [
                {"role": "user", "content": "Private setup runbook", "display_kind": "hidden"},
                {"role": "assistant", "content": "Welcome to Xiuqo"},
        ...
        assert db.get_session(key)["title"] == "Welcome to Xiuqo"
        rows = db.get_messages_as_conversation(key)
        assert [r["content"] for r in rows] == ["Private setup runbook", "Welcome to Xiuqo", "Second question"]
