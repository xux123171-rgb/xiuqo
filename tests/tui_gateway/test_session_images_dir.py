...
before ``prompt.submit`` installs the session's profile XIUQO_HOME override, so
...
def test_profile_home_session_writes_under_profile(tmp_path):
    ...
    profile_home = tmp_path / ".xiuqo" / "profiles" / "coder"
...
def test_two_profiles_are_isolated(tmp_path):
    ...
    home_a = tmp_path / ".xiuqo" / "profiles" / "a"
    home_b = tmp_path / ".xiuqo" / "profiles" / "b"
