...
from xiuqo_cli.dashboard_auth.ws_tickets import INTERNAL_PROVIDER, INTERNAL_USER_ID
...
def home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XIUQO_HOME", str(h))
    return h
...
def test_outbox_drain_returns_each_envelope_once(home):
    ...
    env = bot_relay.enqueue_envelope(
        home, target=target, message="m", sender_profile="default", sender_handle="xiuqo"
    )
...
def test_deliver_validates_profile_and_runs_transport(home, monkeypatch):
    ...
    assert argv[0].rsplit("\\", 1)[-1].rsplit("/", 1)[-1] in ("xiuqo", "xiuqo.exe")
    assert "Bot Chat" in argv and "--query-file" in argv

    # 'xiuqo' alias resolves to default
    _result(srv._methods["bot_relay.deliver"](2, {"profile": "xiuqo", "message": "x"}))
...
def test_deliver_lands_in_live_bot_chat_instead_of_subprocess(home, monkeypatch):
    ...
    def _fake_run(argv, *a, **k):
        ...
        # daemon thread; only the relay's `xiuqo` CLI spawn is under test.
...
def test_deliver_write_failure_still_removes_tempfile(home, monkeypatch, tmp_path):
    ...
    assert not glob.glob(str(tmp_path / "xiuqo-relay-dm-*")), "tempfile leaked"
...
def test_deliver_child_env_carries_the_envelope_sender_on_every_attempt(home, monkeypatch, fake_runs, sender, expected):
    """XIUQO_TURN_AUTHOR on the child comes from the envelope's sender fields alone: the retry gets the same
    ...
    assert all(e["XIUQO_RELAY_TEST_MARKER"] == "kept" for e in envs)
...
def test_gateway_drains_the_mailbox_the_tools_write_to(tmp_path, monkeypatch, subdir):
    """Both ends of the relay mailbox derive the install root from XIUQO_HOME with ONE formula.
    ...
    target = {"profile": "scout", "handle": "scout", "connection_id": "cloud-1",
              "connection_label": "", "title": "", "description": ""}
    env = bot_relay.enqueue_envelope(
        writer_root, target=target, message="m", sender_profile="default", sender_handle="xiuqo")
