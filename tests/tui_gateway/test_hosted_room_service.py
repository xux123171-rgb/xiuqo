...
def test_create_send_drive_publish_and_replay_without_client_transport(tmp_path: Path):
    ...
    room = service.create_room(
        ...
        name="Release room",
        members=[
            {"member_id": "default", "profile": "default", "handle": "xiuqo"},
...
def test_same_thread_followup_migrates_and_delivers_committed_peer_reply(
    ...
    service.send(
        room_id="room-1",
        event_id="user-2",
        payload={"text": "@xiuqo continue", "thread_id": "thread-1"},
    )
    ...
    assert "User (user): @xiuqo continue" in prompt
...
def test_active_same_thread_followup_waits_for_current_task(tmp_path: Path):
    ...
    service.send(
        room_id="room-1",
        event_id="user-2",
        payload={"text": "@xiuqo follow up", "thread_id": "thread-1"},
    )
    ...
    assert service.stop(timeout=1.0)
    assert "User (user): @xiuqo follow up" in service.rpc.prompts[1][1]
...
def test_service_derives_room_deadline_from_agent_timeout(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("XIUQO_AGENT_TIMEOUT", "90")
...
def test_local_profiles_skips_delete_tombstones_and_dot_dirs(tmp_path: Path):
    """`xiuqo profile delete` leaves ``profiles/.deleted/<name>``; neither the tombstone dir nor a
    tombstoned profile is a roster member (#106847: ``.deleted`` failed validate_roster every cycle)."""
    from xiuqo_constants import mark_named_profile_deleted
