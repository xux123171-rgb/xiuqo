...
``XIUQO_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
@pytest.fixture
def xiuqo_root(tmp_path, monkeypatch):
    """A temp XIUQO_HOME root with two named profiles: 'work' and 'other'.

    Pointing XIUQO_HOME at a dir outside ~/.xiuqo makes it the profile ROOT
...
def test_add_lands_in_named_profile_only(xiuqo_root):
    root = xiuqo_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(xiuqo_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_status_is_profile_scoped_and_credential_safe(xiuqo_root):
    _result(
        _call(
            "mcp.servers.add",
            {"profile": "work", "name": "svc-a", "config": {"command": "svc-a-bin"}},
...
def test_status_does_not_mix_launch_runtime_into_another_profile(xiuqo_root):
...
def test_status_includes_named_profile_runtime_in_multiplex(xiuqo_root):
    ...
    work_token = set_hermes_home_override(xiuqo_root / "profiles" / "work")
    try:
        work_scope = xiuqo_home_key()
    finally:
...
def test_set_api_key_writes_env_and_header_to_right_profile(xiuqo_root):
    root = xiuqo_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(xiuqo_root):
    root = xiuqo_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(xiuqo_root):
    root = xiuqo_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(xiuqo_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(xiuqo_root):
    resp = _call("mcp.servers.add", {"profile": "work", "name": "empty", "config": {}})
    assert "error" in resp
    assert resp["error"]["code"] == 4063


def test_default_profile_add_when_profile_omitted(xiuqo_root):
    root = xiuqo_root
    _result(
        _call(
            "mcp.servers.add",
    ...
    # Omitted profile → launch/default profile == XIUQO_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
