...
The stdio ``xiuqo --tui`` path used to spawn its own discovery thread and
``wait_for_mcp_discovery`` only ever joined that local handle. Now the spawn
goes through ``xiuqo_cli.mcp_startup.start_background_mcp_discovery`` (single
...
def test_wait_falls_through_to_shared_owner(monkeypatch):
    ...
    thread.start()
    monkeypatch.setattr(mcp_startup, "_mcp_discovery_thread", {xiuqo_home_key(): thread})
