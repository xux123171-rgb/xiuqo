...
request is delegated to Xiuqo as a normal turn on the open session. Two contracts
...
* the gateway never hands the OpenAI key to the renderer — ``POST /v1/live/sessions``
  ...
  client delegation so Xiuqo (any model) is the backend;
...
class TestSessionCreation:
    def test_client_delegation_and_key_stay_server_side(self, monkeypatch):
        ...
        (Xiuqo is the backend) and authenticates with the resolved key; the client only ever
