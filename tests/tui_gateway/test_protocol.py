...
def server():
    ...
    # e.g. xiuqo_cli.active_sessions would bind the mocked get_hermes_home
    ...
    with patch.dict("sys.modules", {
        "xiuqo_constants": MagicMock(get_hermes_home=MagicMock(return_value="/tmp/xiuqo_test")),
        "xiuqo_cli.env_loader": MagicMock(),
        "xiuqo_cli.banner": MagicMock(),
        "xiuqo_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `XIUQO_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("XIUQO_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("XIUQO_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_idle_reaper_rearms_missing_ws_orphan_timer(server, monkeypatch, tmp_path, closed_transport):
    ...
    successor = subprocess.run(
        [
            ...
            "-c",
            (
                "from xiuqo_cli.active_sessions import try_acquire_active_session; "
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/xiuqo-dev": {"name": "xiuqo-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "xiuqo-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient XIUQO_HOME
    ...
    # The gateway's own XIUQO_HOME (the test-isolation tempdir, no
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `xiuqo config set` does)
    ...
    import xiuqo_cli.skin_engine as skin_engine
    ...
    # Activate midnight, as `xiuqo config set display.skin midnight` would.
