"""Desktop/TUI ``plugins.manage`` honours a ``ref`` pin exactly like ``xiuqo plugins install --ref``."""
...
def test_plugins_manage_install_threads_ref_to_headless_install():
    sha = "a" * 40
    with patch("xiuqo_cli.plugins_cmd.dashboard_install_plugin", return_value={"ok": True}) as install:
...
def test_plugins_manage_list_reports_ref_pin(monkeypatch, tmp_path):
    sha = "b" * 40
    pc = methods_tools._tools_mod("xiuqo_cli.plugins_cmd")
    ...
    monkeypatch.setattr(methods_tools._tools_mod("xiuqo_cli.plugins_cmd_catalog"), "catalog_pins", dict)
