...
@pytest.fixture()
def server(xiuqo_home):
    with patch.dict("sys.modules", {"xiuqo_cli.env_loader": MagicMock(), "xiuqo_cli.banner": MagicMock()}):
...
def _arm_due(key: str):
    from xiuqo_cli.heartbeat import HeartbeatManager, save_heartbeat
...
def test_notification_poller_fires_due_heartbeat_when_idle(server, session):
    ...
    from xiuqo_cli.heartbeat import load_heartbeat
...
def test_heartbeat_tick_defers_when_busy_or_not_due(server, session, running, due):
    ...
    if not due:
        ...
        from xiuqo_cli.heartbeat import save_heartbeat
...
def test_abandon_fire_never_overwrites_a_concurrent_pause(xiuqo_home):
    from xiuqo_cli.heartbeat import HeartbeatManager, load_heartbeat
