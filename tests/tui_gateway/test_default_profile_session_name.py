...
def test_default_home_aliases_are_reported_as_default(tmp_path, monkeypatch):
    ...
    for alias in (default_home.name, "xiuqo"):
    ...
    # "xiuqo" is a legal profile id: a REAL named profile of that name is never swallowed by the alias.
    (default_home / "profiles" / "xiuqo").mkdir(parents=True)
    assert server._response_profile_name("xiuqo") == "xiuqo"
