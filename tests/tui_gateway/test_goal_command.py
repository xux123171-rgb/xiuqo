...
def xiuqo_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves XIUQO_HOME.
...
def server(xiuqo_home, monkeypatch):
    ...
    # Pin config resolution to the isolated XIUQO_HOME. Sibling test
...
def test_moa_bare_returns_usage(server, session, xiuqo_home):
    _write_moa_config(xiuqo_home, """
moa:
