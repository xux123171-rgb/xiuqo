...
guard is covered in ``tests/xiuqo_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``xiuqo_cli.model_switch_providers.prewarm_picker_cache_async``
...
import xiuqo_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from xiuqo_cli.model_switch,
