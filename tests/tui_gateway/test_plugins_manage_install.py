...
def test_plugins_manage_install_success():
    ...
    with patch(
        "xiuqo_cli.plugins_cmd.dashboard_install_plugin",
...
def test_plugins_manage_update_requires_catalog_sidecar(tmp_path, monkeypatch):
    ...
    import xiuqo_cli.plugins_cmd as plugins_cmd
...
def test_plugins_manage_list_reports_desktop_half(tmp_path):
    ...
    with patch("xiuqo_cli.plugins_cmd._discover_all_plugins", return_value=rows), \
         ...
         patch("xiuqo_cli.plugins_cmd_catalog.catalog_pins", return_value={}):
