...
class _RecordingDB:
    """Stand-in for ``xiuqo_state.SessionDB`` that counts ``close()`` calls.
...
def profile_dbs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("xiuqo_state_registry.acquire", _factory)
    monkeypatch.setattr(
    ...
    # off the real agent/secret/XIUQO_HOME machinery.
