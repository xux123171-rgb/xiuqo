...
def live_home(monkeypatch):
    """A REAL isolated XIUQO_HOME with a config.yaml + state.db on disk."""
    tmp = Path(tempfile.mkdtemp(prefix="xiuqo-live-staleprov-"))
    home = tmp / ".xiuqo"
    ...
    # xiuqo_constants caches the resolved home at first read — the env var
    ...
    import xiuqo_cli.config as hconfig
    ...
    # conftest also re-pins xiuqo_state.DEFAULT_DB_PATH per test; pin it to
    # THIS home so server._get_db() opens the same real state.db we seed.
    monkeypatch.setattr(xiuqo_state, "DEFAULT_DB_PATH", home / "state.db")
