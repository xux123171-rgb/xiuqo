...
Relay runs its finalizer as soon as the provider stream ends — concurrently with Xiuqo'
...
def _stream_through_relay(tmp_path, monkeypatch, response_body: bytes, *, finalize_before):
    ...
    Returns ``(xiuqo_result, relay_llm_end_event)``."""
...
def test_openai_stream_final_tool_call_delta_reaches_relay_parent_event(tmp_path, monkeypatch):
    ...
    xiuqo_call = result.choices[0].message.tool_calls[0]
    assert (xiuqo_call.function.name, xiuqo_call.function.arguments) == ("read_file", '{"path": "/tmp/x"}')
