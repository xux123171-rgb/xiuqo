...
class TestConfigGate:
    def test_default_enabled(self):
        with mock.patch("xiuqo_cli.config.load_config", return_value={}):
            assert tier1_advisory_enabled()

    def test_disabled_via_config(self):
        with mock.patch("xiuqo_cli.config.load_config",
    ...
    def test_config_error_defaults_enabled(self):
        with mock.patch("xiuqo_cli.config.load_config", side_effect=RuntimeError):
            assert tier1_advisory_enabled()
...
class TestInstallPathHelper:
    ...
    def test_helper_never_raises_on_scanner_error(self, tmp_path):
        from xiuqo_cli.skills_hub import _print_tier1_advisory
