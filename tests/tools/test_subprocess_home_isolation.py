...
Xiuqo state stays profile-scoped through XIUQO_HOME. Host subprocesses should
...
See: https://github.com/xux123171-rgb/xiuqo/issues/25114
See: https://github.com/xux123171-rgb/xiuqo/issues/36144
See: https://github.com/xux123171-rgb/xiuqo/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for xiuqo_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(xiuqo_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("XIUQO_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        xiuqo_home = real_home / ".xiuqo" / "profiles" / "coder"
        profile_home = xiuqo_home / "home"
        profile_home.mkdir(parents=True)
        monkeypatch.setenv("HOME", str(real_home))
        monkeypatch.setenv("XIUQO_HOME", str(xiuqo_home))
        from xiuqo_constants import get_subprocess_home
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIUQO_REAL_HOME", str(real_home))

        from xiuqo_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIUQO_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("XIUQO_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["XIUQO_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(xiuqo_home / "home")
        assert result["XIUQO_REAL_HOME"] == str(real_home)
