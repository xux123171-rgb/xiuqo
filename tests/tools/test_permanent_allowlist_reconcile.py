...
So a hand edit made while a Xiuqo process is live was undone by the next
...
def fake_config(monkeypatch):
    ...
    monkeypatch.setattr("xiuqo_cli.config.load_config", _load, raising=False)
    monkeypatch.setattr("xiuqo_cli.config.save_config", _save, raising=False)
...
def test_a_second_process_writing_first_does_not_lose_this_ones_approval(fake_config):
    """Two live Xiuqo processes. Whoever writes second must not drop the first."""
...
def test_save_failure_is_logged_not_raised(fake_config, monkeypatch, caplog):
    ...
    monkeypatch.setattr("xiuqo_cli.config.load_config", _boom, raising=False)
    approval.approve_permanent("docker *")
