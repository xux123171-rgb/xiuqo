...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "xiuqo_cli"}
    ...
    try:
        ...
        _reset_modules(("tools", "agent", "xiuqo_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    xiuqo_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["xiuqo_cli"] = xiuqo_cli
    sys.modules["xiuqo_cli.config"] = types.SimpleNamespace(
        get_hermes_home=lambda: Path(tempfile.gettempdir()) / "xiuqo-home",
    )
...
def test_managed_modal_rejects_host_credential_passthrough():
    _install_fake_tools_package(
        credential_mounts=[{
            ...
            "container_path": "/root/.xiuqo/token.json",
        }]
    )
    managed_modal = _load_tool_module("tools.environments.managed_modal", "environments/managed_modal.py")
