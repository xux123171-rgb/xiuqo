...
Under CIMD the ``client_id`` is the HTTPS URL of a document Xiuqo publishes,
...
the conditions under which Xiuqo is allowed to present it.
...
def test_cached_metadata_decides_whether_to_pin(
    ...
    long after Xiuqo fixes the redirect URI. Metadata cached by an earlier
...
def test_sdk_chooses_cimd_only_when_the_server_advertises_it(
    ...
    """Closes the loop on the handoff: feed what Xiuqo configured into the
...
def test_reauthorizing_clears_the_rejection(tmp_path, monkeypatch, private_ports):
    """`xiuqo mcp login` wipes stored state, so a fixed document is retried."""
    monkeypatch.setenv("XIUQO_HOME", str(tmp_path))
    storage = HermesTokenStorage("srv")
...
def test_timeout_on_a_cimd_flow_names_the_document_and_the_escape_hatch(monkeypatch):
    ...
    symptom Xiuqo sees is the callback timing out."""
