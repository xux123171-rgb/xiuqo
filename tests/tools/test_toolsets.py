...
class TestToolsetConsistency:
    ...
    def test_hermes_platforms_share_core_tools(self):
        """All xiuqo-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on xiuqo-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["xiuqo-cli", "xiuqo-telegram", "xiuqo-discord", "xiuqo-whatsapp", "xiuqo-slack", "xiuqo-signal", "xiuqo-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_hermes_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("xiuqo-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("xiuqo-cli")
        second = resolve_toolset("xiuqo-cli")
        ...
        assert (
            "xiuqo-cli", True, registry_id, generation, registry.current_scope_key()
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("xiuqo-cli", include_registry=False)
        second = resolve_toolset("xiuqo-cli", include_registry=False)
        assert first == second
