...
def test_cannot_message_self(tmp_path):
    ...
    result = json.loads(
        bot_mode_dm.message_agent_tool(target="xiuqo", message="hi", agent=agent)
    )
    assert "error" in result
...
def test_local_delivery_command_and_ack(tmp_path, monkeypatch):
    ...
    assert _runner_author(command) == {"id": "bot:default", "name": "xiuqo", "is_bot": True}

    # attribution prefix applied server-side; body verbatim inside the file
    content = Path(dm_file).read_text(encoding="utf-8")
    assert content.startswith("Message from 🤖 xiuqo (@xiuqo): ")
...
def test_peer_delivery_command_pins_registry_profile_for_secondary_bots(
    ...
    profile (#93935). `xiuqo peer` resolves bot_peers through
    ...
    assert transport_argv == ["xiuqo", "-p", "default", "peer", "dm", "spark"]
...
def test_peer_delivery_command(tmp_path, monkeypatch):
    ...
    mode, _dm_file, transport_argv = _runner_parts(calls[0]["command"])
    assert mode == "stdin"
    assert transport_argv == ["xiuqo", "-p", "default", "peer", "dm", "spark/researcher"]
    # the peer child reads the author from its env and forwards it in the request body
    assert _runner_author(calls[0]["command"]) == {"id": "bot:eri-mac.local/default", "name": "xiuqo", "is_bot": True}
...
def test_named_profile_sender_prefix(tmp_path, monkeypatch):
    """A named-profile bot signs with its own handle, not @xiuqo."""
    calls = _capture_spawn(monkeypatch)
    home = _managed_home(tmp_path, teammates=("researcher", "coder"))
...
def test_delivery_command_author_json_survives_quoting_and_windows_slash_rewrite(tmp_path, monkeypatch):
    ...
    author = {"id": "bot:default", "name": "xiuqo", "is_bot": True}
    command = bot_mode_dm._delivery_command(["xiuqo", "-p", "x"], str(tmp_path / "dm.txt"),
    ...
    assert _runner_parts(command) == ("query-file", str(tmp_path / "dm.txt"), ["xiuqo", "-p", "x"])

    monkeypatch.setattr(sys, "platform", "win32")
    command = bot_mode_dm._delivery_command(["xiuqo", "-p", "x"], "C:\\Users\\me\\dm.txt",
    ...
    command = bot_mode_dm._delivery_command(["xiuqo"], "dm.txt", stdin_file=True)
...
def test_live_dm_admitted_before_waiter_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XIUQO_HOME", str(tmp_path / "wrong-home"))
    ...
    assert record["message"] == "Message from 🤖 xiuqo (@xiuqo): hello"
    assert record["author"] == {"id": "bot:default", "name": "xiuqo", "is_bot": True}
...
def test_delivery_runner_unlinks_when_child_launch_raises(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="child launch failed"):
        bot_mode_dm._run_delivery(["xiuqo"], str(dm_file), stdin_file=False)
    assert not dm_file.exists()
...
def test_query_file_delivery_closes_stdin_for_initial_attempt_and_retry(
    ...
    returncode = bot_mode_dm._run_delivery(
        ["xiuqo", "-p", "researcher"], str(dm_file), stdin_file=False
    )
...
def test_delivery_main_child_env_carries_only_the_argv_author(tmp_path, monkeypatch, mode, author):
    """The ``--author`` payload becomes XIUQO_TURN_AUTHOR on the child. Without it the runner drops the
    ...
    monkeypatch.setattr(subprocess, "run", fake_run)
    monkeypatch.setenv("XIUQO_DM_TEST_MARKER", "kept")
    ...
    returncode = bot_mode_dm._delivery_main(
        ["--run-delivery", *author_args, mode, str(dm_file), "xiuqo", "-p", "researcher"])

    assert returncode == 0
    [(argv, kwargs)] = calls
    assert argv[:3] == ["xiuqo", "-p", "researcher"]
    assert kwargs["env"]["XIUQO_DM_TEST_MARKER"] == "kept"
...
def test_real_delivery_command_round_trip_carries_author(tmp_path):
    """Through a real subprocess, the runner argv built by ``_delivery_command`` sets XIUQO_TURN_AUTHOR on the child."""
    dm_file = tmp_path / "message.txt"
    dm_file.write_text("secret", encoding="utf-8")
    observed = tmp_path / "observed.txt"
    child = tmp_path / "child.py"
    child.write_text(
        ...
        "pathlib.Path(sys.argv[1]).write_text(os.environ.get('XIUQO_TURN_AUTHOR', 'unset'), encoding='utf-8')\n",
        encoding="utf-8",
    )
    author = {"id": "bot:default", "name": "xiuqo", "is_bot": True}
...
def test_sweeper_removes_only_stale_dm_files(tmp_path, monkeypatch):
    ...
    legacy_stale = tmp_path / "xiuqo-dm-stale.txt"
