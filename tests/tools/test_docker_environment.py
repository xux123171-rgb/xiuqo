...
def test_non_persistent_cleanup_removes_container(monkeypatch):
    ...
    docker rm so containers don't leak across xiuqo processes.
...
def _make_execute_only_env(forward_env=None):
    ...
    env._snapshot_path = "/tmp/xiuqo-snap-test123.sh"
    env._cwd_file = "/tmp/xiuqo-cwd-test123.txt"
    env._cwd_marker = "__XIUQO_CWD_test123__"
...
def test_init_env_args_uses_hermes_dotenv_for_allowlisted_env(monkeypatch):
    ...
    # Use a var that is NOT in _XIUQO_PROVIDER_ENV_BLOCKLIST (GITHUB_TOKEN
...
def test_init_env_args_uses_hermes_dotenv_for_empty_shell_env(monkeypatch):
    ...
    MY_SECRET=`, clobbering the correct value sitting in ~/.xiuqo/.env.
    """
...
def test_egress_node_options_overrides_conflicting_ca_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: ([], {"_XIUQO_EGRESS_NODE_OPTIONS_APPEND": "--use-openssl-ca"}, []),
    )
    calls = _mock_subprocess_run(monkeypatch)
...
def test_security_args_include_setuid_setgid_for_privdrop(monkeypatch):
    ...
    (e.g. via ``s6-setuidgid`` in the bundled Xiuqo image, or ``gosu``/``su``
...
def test_snap_compat_drops_only_init_and_no_new_privileges(monkeypatch):
    ...
    def strip(argv):  # everything except the two flags and the random container name
        return [a for a in argv if a not in ("--init", "--security-opt", "no-new-privileges") and not a.startswith("xiuqo-")]
...
def test_run_command_tags_xiuqo_agent_label(monkeypatch):
    """Every container xiuqo starts must carry the xiuqo=1 label
    so the orphan reaper (and external operators) can identify them with a
    single ``docker ps --filter label=xiuqo=1`` call. Regression test
    ...
    assert "xiuqo=1" in labels, (
        f"xiuqo=1 label missing; got labels: {sorted(labels)}"
    )
...
def test_run_command_sanitizes_unsafe_task_id(monkeypatch):
    ...
    assert "xiuqo-task-id=task_with_weird_chars" in labels, (
...
def test_shared_container_key_replaces_profile_identity(monkeypatch):
    ...
    assert a._labels["xiuqo-profile"] == b._labels["xiuqo-profile"]
    assert a._labels["xiuqo-profile"] != "research"
    assert a._labels["xiuqo-profile"].startswith("team_workspace-")
...
def test_distinct_shared_keys_never_collide(monkeypatch):
    ...
    assert c._labels["xiuqo-profile"] != d._labels["xiuqo-profile"]
    # Both stay within Docker's 63-char label-value bound.
    assert len(c._labels["xiuqo-profile"]) <= 63
    assert len(d._labels["xiuqo-profile"]) <= 63
...
def test_empty_shared_container_key_preserves_profile_isolation(monkeypatch):
    ...
    assert env._labels["xiuqo-profile"] == "research"
...
def test_reuse_attaches_to_running_container_without_docker_run(monkeypatch):
    ...
    issue #20561 root cause: every Xiuqo process spawning a new container
...
def test_egress_enabled_does_not_reuse_pre_egress_container(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            ["-v", "/tmp/ca:/etc/ssl/certs/xiuqo-egress-ca.crt:ro"],
    ...
    def _run(cmd, **kwargs):
        ...
        if isinstance(cmd, list) and len(cmd) >= 2:
            ...
            if sub == "ps":
                ...
                assert any(str(part).startswith("label=xiuqo-egress=") for part in cmd)
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
            if sub == "run":
                return subprocess.CompletedProcess(cmd, 0, stdout="fresh-cid\n", stderr="")
        return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
        ...
        if isinstance(cmd, list) and len(cmd) >= 2:
            ...
            if sub == "ps":
                if "--format" in cmd:
                    ...
                    assert any(
                        str(part) == "label=xiuqo-egress=off" for part in cmd
...
def test_extra_args_proxy_override_refuses_under_egress(monkeypatch):
    """docker_extra_args are appended after Xiuqo args, so egress enforcement
...
def test_reuse_starts_stopped_container_before_attaching(monkeypatch):
    ...
    ``docker start`` before the new Xiuqo process uses it. Without this
...
def test_failed_docker_run_cleans_up_orphaned_container(monkeypatch):
    ...
    assert rm_cmd[3].startswith("xiuqo-"), "should remove the container by its generated name"
...
def test_cleanup_with_persist_is_noop_for_container(monkeypatch):
    ...
    Xiuqo startup, not on graceful exit. Issue #20561 — the first iteration
...
def test_credential_mount_skipped_when_source_is_directory(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(corrupted_dir), "container_path": "/root/.xiuqo/google_token.json"},
    ]
    monkeypatch.setattr(
...
def test_credential_mount_skipped_when_source_missing(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(missing_path), "container_path": "/root/.xiuqo/deleted_token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_s6_image_skips_docker_init_and_mounts_run_exec(monkeypatch):
    ...
    _make_dummy_env(image="xiuqo:latest")
