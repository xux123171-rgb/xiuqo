...
Verifies that Xiuqo-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Xiuqo secrets.

See: https://github.com/xux123171-rgb/xiuqo/issues/1002
See: https://github.com/xux123171-rgb/xiuqo/issues/1264
"""
...
from tools.environments.local_env_policy import (
    _XIUQO_PROVIDER_ENV_BLOCKLIST,
    _XIUQO_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.xiuqo/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Xiuqo inference secret
        ...
        models`` run inside a Xiuqo terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _XIUQO_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        Xiuqo-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "XIUQO_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
    ...
    def test_bare_hermes_resolves_from_sanitized_subprocess_path(self):
        """Cron children can resolve Xiuqo even when the gateway PATH cannot."""
...
class TestTerminalFirstPartyPlatformEnv:
    ...
    (and the other BUZZ_* vars) are stripped by _XIUQO_PROVIDER_ENV_BLOCKLIST
    ...
    children then get the BUZZ_* vars; execute_code, xiuqo_subprocess_env,
    ...
    def test_buzz_vars_stripped_without_buzz_context(self, monkeypatch):
        ...
        monkeypatch.delenv("XIUQO_SESSION_PLATFORM", raising=False)
        buzz_vars = {
            "BUZZ_PRIVATE_KEY": "nsec1faketestkey",
            "BUZZ_AUTH_TAG": '["tag","data","kind","sig"]',
            "BUZZ_RELAY_URL": "https://mycommunity.communities.buzz.xyz",
    ...
    def test_buzz_vars_stay_in_blocklist(self):
        ...
        xiuqo_subprocess_env, env_passthrough registration)."""
...
class TestTerminalFirstPartySnapshotIsolation:
    ...
    an exclusion, profile A's BUZZ_PRIVATE_KEY lands in xiuqo-snap-<id>.sh
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _XIUQO_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_XIUQO_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_XIUQO_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_XIUQO_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_XIUQO_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestActiveVenvMarkerStripping:
    ...
    into the Xiuqo venv path instead of the project's own ``.venv`` —
    silently clobbering the Xiuqo environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Xiuqo venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.xiuqo/xiuqo/venv",
        })
        assert "VIRTUAL_ENV" not in result_env

    def test_conda_prefix_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "CONDA_PREFIX": "/opt/conda/envs/xiuqo",
        })
...
def _physical_repo_root(tmp_path: Path) -> Path:
    ...
    physical_root = tmp_path / "physical-home" / "xiuqo"
    physical_root.mkdir(parents=True)
    return physical_root


class TestPythonpathSelectiveStrip:
    """PYTHONPATH Xiuqo-owned entry stripping (#74817).

    The Desktop Electron app injects the Xiuqo repo root and the Xiuqo
    ...
    entries Xiuqo itself owns (repo root, own venv site-packages),
    ...
    def test_owned_entries_stripped_matrix(self):
        """Exact Xiuqo-owned entries are removed; everything else survives
        ...
        cannot silently pass), duplicate Xiuqo entries, all-owned input
        (PYTHONPATH key removed), and mixed user/Xiuqo ordering with an
    ...
    def test_non_owned_entries_preserved(self, user_pp):
        """Anything not proven Xiuqo-owned is preserved byte-for-byte.
    ...
    def test_non_owned_runtime_shaped_entries_preserved(self):
        ...
        different interpreter version, a descendant of the Xiuqo venv
    ...
    def test_windows_backslash_paths(self):
        """Windows-style backslash paths are handled for Xiuqo-owned entries.
        ...
        Xiuqo-owned — the critical invariant is that user Windows paths
        (including site-packages paths for another Python version) are
        never destroyed.  On a real Windows host, Path splits on
        backslashes and Xiuqo venv site-packages entries are stripped
        by the same Xiuqo-owned check (covered by the Windows-only test
        below).
        """
        from tools.environments.local_pythonpath import _strip_hermes_owned_pythonpath
        import sys
        ...
        pyver = f"python{sys.version_info[0]}.{sys.version_info[1]}"
        xiuqo_win = f"C:\\\\Users\\\\u\\\\.xiuqo\\\\xiuqo\\\\venv\\\\lib\\\\{pyver}\\\\site-packages"
        user_win = "D:\\\\user\\\\lib"
        env = {
            "PYTHONPATH": ";".join([xiuqo_win, user_win]),
        }
        ...
        # the Xiuqo-owned check cannot match a backslash path here.
        assert xiuqo_win in entries
        assert user_win in entries

    @pytest.mark.windows_only
    def test_windows_hermes_owned_paths_stripped(self):
        """On Windows, a Xiuqo venv site-packages entry written with
        backslashes is stripped by the same Xiuqo-owned check, while a
        ...
        _strip_hermes_owned_pythonpath(env)
        entries = env["PYTHONPATH"].split(";")
        assert xiuqo_win not in entries
        assert user_win in entries
    ...
    def test_base_python_sanitizer_uses_validated_separate_runtime_venv(self, tmp_path, monkeypatch):
        ...
        This deliberately uses a synthetic Xiuqo venv separate from the test
        runner: sys.prefix represents base Python, while validated VIRTUAL_ENV
        identifies ``<repo>/venv`` as the Xiuqo runtime producer contract.
        """
        import tools.environments.local as local
        from tools.environments import local_pythonpath

        repo_root = tmp_path / "xiuqo"
    ...
    def test_builders_strip_hermes_venv_pythonpath(self, builder):
        ...
        Xiuqo venv site-packages is stripped, user entries survive.
        """
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod.xiuqo_subprocess_env()
    ...
    def test_scrub_child_env_strips_hermes_venv_pythonpath(self):
        """execute_code's _scrub_child_env path: after scrubbing, Xiuqo venv
    ...
    def test_execute_code_composition_strips_inherited_hermes_entries(self, same_env):
        ...
        Seeds a contaminated inherited PYTHONPATH (Xiuqo repo root + Xiuqo
        ...
        * inherited Xiuqo venv site-packages never survive into the sandbox;
        ...
        * user entries survive after the controlled entries.
        ...
        xiuqo_root = str(Path(cet.__file__).resolve().parents[1])
        venv_sp = str(_running_venv_site_packages())
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {
                 "PYTHONPATH": os.pathsep.join(
                     [xiuqo_root, venv_sp, user_a, user_b]),
             }):
        ...
        norm_root = os.path.normcase(xiuqo_root)
        ...
        assert norm_venv not in norm_parts, \
            "inherited Xiuqo venv site-packages must be stripped"
    ...
    def test_configured_home_alias_matches_launcher_output(self, tmp_path, monkeypatch):
        ...
        import tools.environments.local as local
        from tools.environments import local_pythonpath
        from xiuqo_cli.gateway_windows import _preserve_hermes_home_path
        ...
        monkeypatch.setenv("XIUQO_HOME", str(configured_home))
        ...
        assert launcher_entry == configured_home / "xiuqo"
    ...
    def test_profile_rehome_keeps_junction_lexical_alias(self, tmp_path, monkeypatch):
        ...
        The desktop/CLI spawn children with XIUQO_HOME and PYTHONPATH in the
        configured (junction) spelling, but --profile / sticky active_profile
        re-home XIUQO_HOME through resolve_profile_env() before the
        ...
        physical_home = tmp_path / "physical-home"
        physical_root = physical_home / "xiuqo"
        physical_root.mkdir(parents=True)
        ...
        monkeypatch.setenv("XIUQO_HOME", str(configured_home))
        lexical_root = configured_home / "xiuqo"
        ...
        # The sanitizer now runs under the re-homed (profile) XIUQO_HOME.
        aliases = local_pythonpath._build_hermes_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
    ...
    def test_repo_level_junction_recovers_lexical_alias(self, tmp_path, monkeypatch):
        ...
        (e.g. D:\\xiuqo\\xiuqo -> C:\\...\\xiuqo) while the
        ...
        # repo-level link: <configured-home>/xiuqo -> physical repo
        try:
            _make_directory_link(configured_home / "xiuqo", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")
    ...
    def test_same_named_non_owned_directories_preserved(self, tmp_path, monkeypatch):
        ...
        physical_root = _physical_repo_root(tmp_path)
        configured_home = tmp_path / "configured-home"
        (configured_home / "xiuqo").mkdir(parents=True)
        unrelated = tmp_path / "user-tools" / "xiuqo"
        ...
        for lookalike in (configured_home / "xiuqo", unrelated):
    ...
    def test_profile_home_with_repo_level_junction(self, tmp_path, monkeypatch):
        ...
        is <root>/profiles/<name> while the repo is a link at <root>/xiuqo.
        ...
        try:
            _make_directory_link(configured_root / "xiuqo", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")

        configured_home = configured_root / "profiles" / "coder"
        lexical_root = configured_root / "xiuqo"
        aliases = local_pythonpath._build_hermes_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        ...
        assert any(local_pythonpath._same_path(a, lexical_root) for a in aliases)
        assert not any(local_pythonpath._same_path(a, configured_home / "xiuqo") for a in aliases)
...
class TestPythonhomeSanitized:
    """PYTHONHOME must not leak from the Xiuqo runtime into subprocesses.
    ...
    inherits it redirects its stdlib search to the Xiuqo venv and crashes
    ...
    def test_builders_strip_pythonhome(self, builder):
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod._sanitize_subprocess_env(dict(os.environ))
            elif builder == "xiuqo_subprocess_env":
                result = local_mod.xiuqo_subprocess_env()
            else:
    ...
    def test_build_subprocess_env_no_scrub_preserves_pythonhome(self):
        ...
        base = {
            ...
            "HOME": "/home/user",
            "PYTHONHOME": "/opt/xiuqo-venv",
            "VIRTUAL_ENV": "/opt/xiuqo-venv",
            "SERVICE_TOKEN": "s3cr3t",
        }
        result = build_subprocess_env(base, scrub_secrets=False)
        assert result.get("PYTHONHOME") == "/opt/xiuqo-venv"
        assert result.get("VIRTUAL_ENV") == "/opt/xiuqo-venv"
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_XIUQO_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not Xiuqo (#55878).
        """
        from xiuqo_cli.auth import PROVIDER_REGISTRY
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                if var in exempt:
                    continue
                assert var in _XIUQO_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _XIUQO_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Xiuqo-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _XIUQO_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Xiuqo, and blocklisting
        ...
        leaked_block = general_chain & _XIUQO_PROVIDER_ENV_BLOCKLIST
    ...
    def test_claude_code_oauth_token_is_inheritable(self):
        ...
        assert "CLAUDE_CODE_OAUTH_TOKEN" not in _XIUQO_PROVIDER_ENV_BLOCKLIST
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        from xiuqo_cli.config import OPTIONAL_ENV_VARS

        for name, metadata in OPTIONAL_ENV_VARS.items():
            category = metadata.get("category")
            if category in {"tool", "messaging"}:
                assert name in _XIUQO_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_hermes_bin_injection(self):
        ...
        xiuqo-install-dir prepend (a separate concern, covered by
        TestHermesBinDirOnPath) so a real ``xiuqo`` on the test runner's PATH
        doesn't shift the asserted PATH layout."""
        from tools.environments import local as local_mod
        saved = local_mod._XIUQO_BIN_DIR
        local_mod._XIUQO_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._XIUQO_BIN_DIR = saved
...
class TestHermesBinDirOnPath:
    """The xiuqo install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``xiuqo`` via the terminal tool must work
    even when the gateway was launched without the xiuqo install dir on
    ...
    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._XIUQO_BIN_DIR = local_mod._SENTINEL

    def test_resolves_via_which(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/xiuqo/bin/xiuqo" if name == "xiuqo" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/xiuqo/bin")
        assert local_mod._resolve_hermes_bin_dir() == "/opt/xiuqo/bin"


    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._XIUQO_BIN_DIR = None
        assert local_mod._prepend_hermes_bin_dir("/usr/bin:/bin") == "/usr/bin:/bin"

    def test_make_run_env_injects_hermes_bin_dir(self):
        """A gateway env missing the xiuqo dir gets it back in the subshell PATH.
        ...
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._XIUQO_BIN_DIR = "/opt/xiuqo/bin"
        with patch.dict(
            os.environ,
            {"PATH": os.pathsep.join(["/usr/bin", "/bin"])},
            clear=True,
        ...
        assert entries[0] == "/opt/xiuqo/bin"
        assert "/usr/bin" in entries


class TestHermesInternalDynamicSecrets:
    """Dynamically-named Xiuqo secrets injected at gateway/CLI startup must
    not leak into terminal subprocesses.

    The static ``_XIUQO_PROVIDER_ENV_BLOCKLIST`` is name-based and derived
    ...
    ``xiuqo_subprocess_env``, Docker forward filter, ``env_passthrough``)
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _XIUQO_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _XIUQO_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _XIUQO_PROVIDER_ENV_BLOCKLIST
