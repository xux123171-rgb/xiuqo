...
code paths Xiuqo takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``xiuqo_cli.stdio.configure_windows_stdio`` wiring.
    ...
    - respect XIUQO_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("xiuqo_cli.stdio", None)
        # Fresh import now; tests import from xiuqo_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import xiuqo_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("xiuqo_cli.stdio", None)

    def test_no_op_on_posix(self, monkeypatch):
        from xiuqo_cli import stdio
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("xiuqo_cli.kanban_db_dispatch", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "xiuqo_cli" / "web_server_chat.py").read_text(encoding="utf-8")
    ...
    def test_pty_handler_checks_availability_flag(self):
        ...
        root = Path(__file__).resolve().parents[2]
        source = (root / "xiuqo_cli" / "web_routers" / "chat_ws.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, xiuqo_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "xiuqo_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call xiuqo_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """xiuqo_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from xiuqo_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from xiuqo_cli._subprocess_compat import resolve_node_command
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self):
        ...
        Without it, the gateway-respawn watcher spawned by ``xiuqo update``
        (which runs under xiuqo-setup.exe, itself a grandchild of the
...
# ---------------------------------------------------------------------------
# xiuqo_cli/kanban_db_dispatch.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "xiuqo_cli" / "kanban_db_dispatch.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the XIUQO_RPC_SOCKET format.
    """
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / xiuqo_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "xiuqo_cli/tools_config.py",
            "xiuqo_cli/doctor.py",
...
class TestGitBashPathNormalization:
    ...
    def test_posix_noop(self):
        ...
        from xiuqo_cli.worktree_ops import _normalize_git_bash_path
        if sys.platform != "win32":
    ...
    def test_windows_translation(self):
        ...
        """
        from xiuqo_cli import worktree_ops as cli_mod
...
class TestWorktreeSymlinkFallback:
    ...
    def test_source_has_symlink_fallback(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "xiuqo_cli" / "worktree_ops.py").read_text(encoding="utf-8")
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_xiuqo_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "xiuqo_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "xiuqo_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        xiuqo_cli._subprocess_compat; the watcher must call that helper from
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "xiuqo_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from xiuqo_cli._subprocess_compat import" in block
    ...
    def test_watcher_threads_hidden_console_spec_into_respawn(self):
        ...
        The spec supplies the stable cwd + env overlay (XIUQO_HOME,
        ...
        assert '_popen_kwargs["env"]' in block, (
            ...
            "XIUQO_HOME) from the restart spec."
        )
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import xiuqo_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "xiuqo_cli.main", "gateway", "run"]
    ...
    def test_windows_keeps_console_python_and_preserves_tail(self):
        ...
        it up — a pre-import so the lazy ``xiuqo_cli.gateway`` import didn't
        ...
        with mock.patch.object(
            gw, "_stable_gateway_working_dir", return_value="C:/xiuqo"
        ), mock.patch(
            "xiuqo_cli.config.get_hermes_home", return_value="C:/xiuqo"
        ):
            new_argv, cwd, env = gw.windowless_gateway_restart_spec(list(argv))
...
class TestGatewayRunRestartWatcherOuterPopenFallback:
    ...
    def test_outer_watcher_retries_without_breakaway_on_oserror(self, monkeypatch):
        ...
        monkeypatch.setattr(gr, "_resolve_hermes_bin", lambda: ["xiuqo"])
        ...
        assert "_XIUQO_GATEWAY" not in kw1["env"]
    ...
    def test_outer_watcher_dual_failure_warns_without_leaking_secrets(
        ...
        monkeypatch.setenv("XIUQO_TEST_SECRET", secret)
        ...
        assert secret in (kwargs_used.get("env") or {}).get("XIUQO_TEST_SECRET", "")
