...
Real config files in a temp XIUQO_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated XIUQO_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".xiuqo"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "XIUQO_LOCAL_STT_LANGUAGE",
    ):
