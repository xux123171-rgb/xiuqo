...
(``XIUQO_HOME=<profile>``) would — never the launch profile's values frozen into process caches or
...
def two_homes(tmp_path, monkeypatch):
    """Launch home A (XIUQO_HOME) and routed profile B, differing in every setting under test."""
    a = tmp_path / ".xiuqo"
    b = a / "profiles" / "b"
...
def test_routed_local_profile_cwd_matches_standalone_gateway(tmp_path, two_homes):
    ...
    wherever ``xiuqo gateway`` happened to be launched from."""
...
def test_terminal_backend_consumers_read_the_routed_scope(two_homes):
    ...
    def observe():
        return (
            ...
            credential_files.to_agent_visible_cache_path("/host/.xiuqo/x.png", "/root/.xiuqo"),
            sorted(Path(m["host_path"]).name for m in credential_files.get_credential_file_mounts()),
        )

    with _profile_runtime_scope(a):
        assert observe() == (True, None, "/host/.xiuqo/x.png", ["cred_a.txt"])
    with _profile_runtime_scope(b):
        local, cache_base, translated, mounts = observe()
    assert local is False
    assert cache_base == "/root/.xiuqo"
    assert translated != "/host/.xiuqo/x.png" or mounts == ["cred_b.txt"]
