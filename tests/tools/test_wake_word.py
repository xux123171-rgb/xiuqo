"""Tests for tools.wake_word — the "Hey Xiuqo" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey xiuqo"}) == "hey xiuqo"
    assert ww.wake_phrase({}) == "hey xiuqo"
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "xiuqo_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey xiuqo"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey xiuqo"
...
def test_bundled_hey_hermes_model_ships_on_disk():
    # The "hey xiuqo" wake word works out of the box only if the model is
