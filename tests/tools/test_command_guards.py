...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects XIUQO_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("XIUQO_INTERACTIVE", "XIUQO_GATEWAY_SESSION", "XIUQO_EXEC_ASK", "XIUQO_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["XIUQO_INTERACTIVE"] = "1"
        session_key = os.getenv("XIUQO_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No XIUQO_INTERACTIVE or XIUQO_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
