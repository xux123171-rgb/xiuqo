...
``xiuqo_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``XIUQO_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # XIUQO_* handling (#27303): only the operational allowlist passes;
        # every other XIUQO_* is dropped (the broad prefix was removed).
        "XIUQO_HOME": "/home/alice/.xiuqo",        # allowlisted → kept
        "XIUQO_PROFILE": "default",                 # allowlisted → kept
        "XIUQO_INTERACTIVE": "1",                   # not allowlisted → dropped
        "XIUQO_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "XIUQO_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``xiuqo_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``xiuqo_tools.py`` and ``script.py`` writes in
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        import tempfile, ast
        stub = generate_xiuqo_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_xiuqo_tools_module(["terminal"], transport="uds")
...
# let the child import xiuqo_tools, a user script that printed non-ASCII
