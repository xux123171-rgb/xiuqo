...
def test_output_path_rejects_hermes_oauth_store(tmp_path, monkeypatch):
    ...
    xiuqo_home.mkdir()
    monkeypatch.setattr(file_safety, "_hermes_home_path", lambda: xiuqo_home)
    monkeypatch.setattr(file_safety, "_hermes_root_path", lambda: xiuqo_home)

    target = xiuqo_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    xiuqo_home = tmp_path / "xiuqo-home"
    token_dir = xiuqo_home / "mcp-tokens"
