...
from xiuqo_constants import xiuqo_home_key, reset_hermes_home_override, set_hermes_home_override
...
def two_profiles(tmp_path, monkeypatch):
    ...
    def enter(which):
        ...
        return xiuqo_home_key(homes[which])

    yield enter
    for tool_name in list(registry.get_tool_names_for_toolset("mcp-x")):
        for home in homes.values():
            registry.deregister(tool_name, scope=xiuqo_home_key(home))
