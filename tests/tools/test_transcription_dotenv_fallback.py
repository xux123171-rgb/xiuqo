...
``~/.xiuqo/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.xiuqo/.env`` would be told
    ...
    def test_auto_detect_sees_dotenv_groq(self):
        ...
        with patch.object(tt, "_HAS_FASTER_WHISPER", False), \
             ...
             patch("xiuqo_cli.config.load_env",
...
class TestTranscribeCallSitesReadDotenv:
    ...
    def test_transcribe_groq_forwards_dotenv_key(self):
        ...
        with patch("xiuqo_cli.config.get_env_value", return_value="groq-dotenv-key"), \
             patch.object(tt, "_HAS_OPENAI", True), \
...
class TestEndToEndRegressionGuard:
    """End-to-end probe: patch ``xiuqo_cli.config.load_env`` to simulate
    ``~/.xiuqo/.env`` carrying the key while ``os.environ`` does not.
    ...
    def test_xai_key_only_in_dotenv_before_fix(self, monkeypatch):
        ...
        with patch("xiuqo_cli.config.load_env",
