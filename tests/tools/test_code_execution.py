...
UDS socket lifecycle, xiuqo_tools generation, timeout enforcement,
...
class TestHermesToolsGeneration(unittest.TestCase):
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_xiuqo_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"xiuqo_rpc\")", src)
        self.assertNotIn('os.environ.get("XIUQO_RPC_DIR", "/tmp/xiuqo_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_xiuqo_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        mkdir_cmd = next(cmd for cmd, _, _ in env.commands
                         if "mkdir -p" in cmd and "xiuqo_exec_" in cmd)
        run_cmd = next(cmd for cmd, _, _ in env.commands if "python3 script.py" in cmd)
        cleanup_cmd = next(cmd for cmd, _, _ in env.commands
                           if "rm -rf" in cmd and "xiuqo_exec_" in cmd)
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/xiuqo_exec_", mkdir_cmd)
        self.assertIn("XIUQO_RPC_DIR=/data/data/com.termux/files/usr/tmp/xiuqo_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/xiuqo_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/xiuqo_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """XIUQO_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"XIUQO_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import xiuqo_constants; print(xiuqo_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("xiuqo_constants.py", result["output"])

    def test_single_tool_call(self):
        """Script calls terminal and prints the result."""
        code = """
from xiuqo_tools import terminal
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from xiuqo_tools import shell_quote
...
"""
    ...
    def test_json_parse_helper_bom(self):
        ...
        code = """
from xiuqo_tools import json_parse
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_hermes_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("XIUQO_RPC_SOCKET", child_env)
...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_does_not_import_interactive_cli(self):
        ...
        with patch.dict("sys.modules", {"cli": mock_cli}), \
             patch("xiuqo_cli.config.load_config_readonly", return_value={}):
            result = _load_config()
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching XIUQO_RPC_TOKEN must be rejected before
    ...
    def test_generated_module_sends_token(self):
        """The generated xiuqo_tools module reads XIUQO_RPC_TOKEN and sends it."""
        src = generate_xiuqo_tools_module(["terminal"], transport="uds")
        self.assertIn("XIUQO_RPC_TOKEN", src)
