...
def _make_socket_dir(tmpdir, session_name, pid=None, owner_pid=None):
    ...
    Args:
        ...
        owner_pid: owning xiuqo PID to write to <session>.owner_pid
...
class TestReapOrphanedBrowserSessions:
    ...
    def test_real_profile_attach_daemon_is_reaped_when_owner_is_dead(self, fake_tmpdir):
        """#100855: the shared ``xiuqo-real-profile`` attach daemon is not ``<prefix>_<hex>``
        ...
        def _pid_exists(pid):
            return pid == 4242  # daemon alive, owning xiuqo gone
...
class TestOwnerPidCrossProcess:
    ...
    The owner_pid file records which xiuqo process owns a daemon so that
    concurrent xiuqo processes don't reap each other's active browser
...
class TestLeakedDaemonWithLiveOwner:
    ...
    long-lived xiuqo process, pinning ~5 CPU cores and driving load to 100+.
...
class TestPeriodicOrphanReap:
    ...
    boot — which is exactly what happens in a xiuqo process that stays up
    for days.
    """
