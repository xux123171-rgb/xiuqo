...
class TestWriteDenyExactPaths:
    ...
    def test_hermes_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.xiuqo/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "xiuqo_root"
        ...
        # Sanity check: XIUQO_HOME does point to the profile dir, not the root.
        from xiuqo_constants import get_hermes_home, get_default_hermes_root
