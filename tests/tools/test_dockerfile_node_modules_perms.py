...
Older images made /opt/xiuqo/ui-tui and /opt/xiuqo/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV XIUQO_TUI_DIR=/opt/xiuqo/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R xiuqo:xiuqo /opt/xiuqo/ui-tui" not in text
    assert "chown -R xiuqo:xiuqo /opt/xiuqo/node_modules" not in text
