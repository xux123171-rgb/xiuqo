...
Xiuqo exposes ``terminal.temp_dir`` (mirrored to ``TERMINAL_TEMP_DIR``) so
...
def test_default_is_hermes_cache_not_tmp(tmp_path, monkeypatch):
    ...
    XIUQO_HOME (cache/terminal), NOT tmpfs /tmp."""
    import xiuqo_constants  # noqa: F401 — resolves XIUQO_HOME per call
    ...
    assert result == str(tmp_path / ".xiuqo" / "cache" / "terminal")
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "xiuqo_bg_dead1.log").exists()
