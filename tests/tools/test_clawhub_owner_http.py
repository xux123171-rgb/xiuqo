...
def registry(*, fallback=False, mismatch=False):
    ...
    prior = os.environ.get("XIUQO_ALLOW_PRIVATE_URLS")
    os.environ["XIUQO_ALLOW_PRIVATE_URLS"] = "true"
    _reset_allow_private_cache()
    try:
        yield source, requests
    finally:
        if prior is None:
            os.environ.pop("XIUQO_ALLOW_PRIVATE_URLS", None)
        else:
            os.environ["XIUQO_ALLOW_PRIVATE_URLS"] = prior
        _reset_allow_private_cache()
        server.shutdown()
        server.server_close()
