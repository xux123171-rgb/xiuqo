...
TTS provider tools must resolve API keys from ``~/.xiuqo/.env`` (via
``xiuqo_cli.config.get_env_value``) and not only from ``os.environ`` —
...
already addressed for ``agent/credential_pool`` and ``xiuqo_cli/auth``.
"""
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.xiuqo/.env`` carries the
    key, the provider must find it. These per-provider tests model that
    dotenv-backed lookup by mocking ``xiuqo_cli.config.get_env_value`` directly;
    the separate regression-guard tests cover the lower-level
    ``xiuqo_cli.config.load_env`` integration. Before the fix, ``os.getenv``
    ...
    def test_elevenlabs_reads_dotenv_key(self, tmp_path):
        ...
        with patch("xiuqo_cli.config.get_env_value", return_value="el-dotenv-key"), \
    ...
    def test_xai_reads_dotenv_key(self, tmp_path):
        ...
        canonical ``xiuqo_cli.config.get_env_value`` — the dotenv contract from #17140.
        """
        from tools import tts_tool
        ...
        with patch("xiuqo_cli.config.get_env_value", return_value="xai-dotenv-key"), \
    ...
    def test_gemini_reads_dotenv_key(self, tmp_path):
        ...
        with patch("xiuqo_cli.config.get_env_value", side_effect=fake_get_env_value), \
             patch("requests.post", side_effect=fake_post):
...
class TestRegressionGuard:
    ...
    ``xiuqo_cli.config.load_env`` to simulate ``~/.xiuqo/.env`` carrying the
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.xiuqo/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
