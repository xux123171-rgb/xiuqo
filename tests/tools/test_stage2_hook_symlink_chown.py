...
def _run_helper(
    ...
    log_path: Path,
    *,
    xiuqo_home: Path | None = None,
) -> subprocess.CompletedProcess[str]:
    shell = shutil.which("sh")
    if shell is None:
        pytest.skip("sh not available")
    xiuqo_home = target if xiuqo_home is None else xiuqo_home
    script = (
        "set -eu\n"
        f'XIUQO_HOME="{xiuqo_home}"\n'
...
def test_chown_helper_refuses_symlinked_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    real_home.mkdir()
    symlinked_home = tmp_path / "xiuqo-home"
    try:
...
def test_chown_helper_refuses_target_under_symlinked_home(
    ...
    proc = _run_helper(
        ...
        linked_home / "cron",
        log_path,
        xiuqo_home=linked_home,
    )

    assert proc.returncode == 0, proc.stderr
    assert not log_path.exists(), "must not chown through a symlinked XIUQO_HOME"
    assert "refusing recursive chown through symlinked path" in proc.stdout


def test_stage2_uses_symlink_safe_helper_for_hermes_home_trees(stage2_text: str) -> None:
    assert 'chown_hermes_tree "$XIUQO_HOME/$sub"' in stage2_text
    assert 'chown_hermes_tree "$XIUQO_HOME/profiles"' in stage2_text
    assert 'chown_hermes_tree "$XIUQO_HOME/cron"' in stage2_text
    assert 'chown -R xiuqo:xiuqo "$XIUQO_HOME/$sub"' not in stage2_text
    assert 'chown -R xiuqo:xiuqo "$XIUQO_HOME/profiles"' not in stage2_text
    assert 'chown -R xiuqo:xiuqo "$XIUQO_HOME/cron"' not in stage2_text


def test_stage2_skips_top_level_chown_for_symlinked_hermes_home(
    stage2_text: str,
) -> None:
    assert 'refuse_symlinked_path "chown" "$XIUQO_HOME"' in stage2_text
...
def test_stage2_skips_recursive_repairs_when_tree_is_already_owned(
    ...
    assert 'if [ -e "$XIUQO_HOME/$sub" ] && tree_has_non_hermes_owner "$XIUQO_HOME/$sub"; then' in stage2_text
    assert 'if [ -d "$XIUQO_HOME/profiles" ] && tree_has_non_hermes_owner "$XIUQO_HOME/profiles"; then' in stage2_text
    # Sibling every-boot chown blocks carry the same warm-boot gate.
    assert 'if [ -d "$XIUQO_HOME/cron" ] && tree_has_non_hermes_owner "$XIUQO_HOME/cron"; then' in stage2_text
    assert 'if [ -d "$XIUQO_HOME/platforms/pairing" ] && tree_has_non_hermes_owner "$XIUQO_HOME/platforms/pairing"; then' in stage2_text
    assert 'if [ -d "$XIUQO_HOME/pairing" ] && tree_has_non_hermes_owner "$XIUQO_HOME/pairing"; then' in stage2_text
