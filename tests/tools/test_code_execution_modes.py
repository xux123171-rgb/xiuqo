...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def test_strict_mode_runs_in_tmpdir(self):
        ...
        path stages in xiuqo_sandbox_*, the session kernel in
        xiuqo_kernel_* — either satisfies strict mode's isolation promise."""
        result = self._run("import os; print(os.getcwd())", mode="strict")
        self.assertEqual(result["status"], "success")
        cwd = result["output"].strip()
        self.assertTrue(
            "xiuqo_sandbox_" in cwd or "xiuqo_kernel_" in cwd,
    ...
    def test_project_mode_interpreter_is_venv_python(self):
        ...
        # The xiuqo venv is always active during tests, so this also
    ...
    def test_project_mode_can_still_import_xiuqo_tools(self):
        """Regression: xiuqo_tools still importable from non-tmpdir CWD.

        This is the PYTHONPATH fix — without it, switching to session CWD
        breaks `from xiuqo_tools import terminal`.
        """
        import tempfile
    ...
    def test_strict_mode_can_still_import_xiuqo_tools(self):
...
class TestPythonEnvironmentPrefix(unittest.TestCase):
    ...
    def test_failure_is_not_cached(self):
        ...
        A sticky cached failure would silently drop the xiuqo root from
...
class TestUsesHermesPythonEnvironment(unittest.TestCase):
    ...
    def test_true_for_current_interpreter_without_probe(self):
        ...
        must never drop the xiuqo root for the interpreter Xiuqo itself runs.
        """
        with patch("subprocess.run",
                   side_effect=subprocess.TimeoutExpired(cmd=[], timeout=5)) as mock_run:
            self.assertTrue(_uses_hermes_python_environment(sys.executable))
    ...
    def test_true_when_prefix_matches_sys_prefix(self):
        xiuqo_prefix = os.path.realpath(sys.prefix)
        with patch("tools.code_execution_env._python_environment_prefix",
                   return_value=xiuqo_prefix):
            self.assertTrue(_uses_hermes_python_environment("/same/env/python"))


# ---------------------------------------------------------------------------
# PYTHONPATH composition — xiuqo root included only for same-env interpreters
# ---------------------------------------------------------------------------

class TestPythonPathComposition(unittest.TestCase):
    """Verify xiuqo root inclusion in PYTHONPATH depends on env match.
    ...
    def _capture_pythonpath(self, same_env: bool) -> tuple:
        ...
        def _fake_popen(cmd, **kwargs):
            ...
            # [python, <staging_dir>/xiuqo_kernel_runner.py] (session
    ...
    def test_hermes_root_included_when_same_env(self):
        """When interpreter is in the Xiuqo env, xiuqo root is in PYTHONPATH."""
        ...
        self.assertIn(self._hermes_root(), parts,
                      "xiuqo root must be in PYTHONPATH for same-env interpreters")

    def test_hermes_root_excluded_when_external_env(self):
        """When interpreter is external, xiuqo root must NOT be in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=False)
        parts = pythonpath.split(os.pathsep)
        self.assertNotIn(self._hermes_root(), parts,
                         "xiuqo root must not leak into an external interpreter's PYTHONPATH")
