...
def test_no_scrub_inherit_profile_home_bridges_context_override(tmp_path):
    ...
    assert env["XIUQO_HOME"] == str(tmp_path)
...
def test_e2e_child_sees_hermes_home_and_no_planted_secret(tmp_path, monkeypatch):
    """A real child spawned with a factory-built env must see XIUQO_HOME
    ...
    code = (
        "import os, json; "
        "print(json.dumps({'home': os.environ.get('XIUQO_HOME'), "
    ...
    assert result["home"] == str(xiuqo_home)
...
# ---------------------------------------------------------------------------
# E2E regression (#93082): cron/no_agent children keep bare `xiuqo` on PATH
# ---------------------------------------------------------------------------
...
def test_e2e_scrubbed_env_resolves_bare_hermes_under_minimal_parent_path(monkeypatch):
    ...
    minimal PATH (no xiuqo console-script dir) must still hand cron job
    children an env whose PATH resolves bare ``xiuqo``.
    ...
    if not bin_dir or not os.path.isfile(
        os.path.join(bin_dir, "xiuqo.exe" if os.name == "nt" else "xiuqo")
    ):
        pytest.skip("no real xiuqo console-script install available")

    # Simulate the service-manager minimal PATH: xiuqo dir absent.
    minimal_path = os.pathsep.join(["/usr/bin", "/bin"])
    monkeypatch.setenv("PATH", minimal_path)
    assert shutil.which("xiuqo", path=minimal_path) is None

    env = build_subprocess_env(scrub_secrets=True)  # cron _run_job_script path

    resolved = shutil.which("xiuqo", path=env.get("PATH", ""))
    assert resolved is not None, (
        f"bare 'xiuqo' must resolve from the child PATH {env.get('PATH')!r}"
    )
