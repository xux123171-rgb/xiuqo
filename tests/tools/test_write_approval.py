...
and the shared slash-command handlers (xiuqo_cli/write_approval_commands.py).
...
@pytest.fixture
def xiuqo_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="xiuqo_wa_test_")
    home = os.path.join(d, ".xiuqo")
    os.makedirs(home)
    monkeypatch.setenv("XIUQO_HOME", home)
    yield home
    shutil.rmtree(d, ignore_errors=True)


def _set_approval(subsystem, enabled):
    import xiuqo_cli.config as cfg
    c = cfg.load_config()
...
def test_default_gate_is_off(xiuqo_home):
...
def test_invalid_subsystem_is_off(xiuqo_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(xiuqo_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(xiuqo_home, capsys):
...
def test_load_on_disk_store_honors_configured_limits_and_permissions(xiuqo_home, monkeypatch):
    ...
    monkeypatch.setattr("xiuqo_cli.config.load_config", _boom)
...
def test_handle_approve_all(xiuqo_home):
...
def test_handle_approval_on(xiuqo_home):
    from xiuqo_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(xiuqo_home):
    from xiuqo_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_memory_inline_approve_writes(xiuqo_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(xiuqo_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_invalid_params_rejected_before_staging(xiuqo_home):
