...
Keys added via ``xiuqo auth add <provider>`` live in the credential pool /
auth store and used to be invisible to the voice tools, which only read
``os.environ`` + ``~/.xiuqo/.env`` via ``get_env_value``. The shared
...
def _no_dotenv(monkeypatch):
    """Keep the developer's real ~/.xiuqo/.env out of these tests."""
