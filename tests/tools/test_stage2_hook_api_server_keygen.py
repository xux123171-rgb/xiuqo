...
was gated on ``[ -f "$XIUQO_HOME/.env" ]`` while the first-boot seed that
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Xiuqo loads $XIUQO_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/xiuqo/.env.example -> $XIUQO_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $XIUQO_HOME/.env from it on first boot"
    )
