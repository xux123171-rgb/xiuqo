...
- _seed_from_env reads API keys from ~/.xiuqo/.env when not in os.environ
- _resolve_api_key_provider_secret falls back to credential_pool when env vars are empty
- ~/.xiuqo/.env takes priority over os.environ for Xiuqo-managed credentials
...
def isolated_hermes_home(tmp_path, monkeypatch):
    """Point XIUQO_HOME at a temp dir and clear known API key env vars.
...
def _write_env_file(home: Path, **kwargs) -> None:
    """Write key=value pairs to ~/.xiuqo/.env."""
    lines = [f"{k}={v}" for k, v in kwargs.items()]
    (home / ".env").write_text("\n".join(lines) + "\n")


class TestCredentialPoolSeedsFromDotEnv:
    """_seed_from_env must read keys from ~/.xiuqo/.env, not just os.environ.
    ...
    def test_dotenv_wins_over_stale_os_environ(self, isolated_hermes_home, monkeypatch):
        """Regression for #20591: a fresh key rotated into ~/.xiuqo/.env must
...
class TestAuthResolvesFromDotEnv:
    """_resolve_api_key_provider_secret must also read from ~/.xiuqo/.env."""
    ...
    def test_dotenv_wins_over_stale_os_environ_on_resolve(
        ...
        """Regression for #20591: when both ~/.xiuqo/.env and os.environ define
    ...
    def test_get_anthropic_key_prefers_dotenv_over_stale_os_environ(
        ...
        prefer ~/.xiuqo/.env over a stale shell export. This path resolves
