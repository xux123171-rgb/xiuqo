...
def fake_hermes(tmp_path, monkeypatch):
    """Build a two-profile Xiuqo layout and point XIUQO_HOME at
    the xiuqo-security profile (matching the original-incident shape).
    """
    root = tmp_path / "fake-xiuqo"
    (root / "skills" / "shared-skill").mkdir(parents=True)
    (root / "skills" / "shared-skill" / "SKILL.md").write_text(
        "---\nname: shared-skill\ndescription: default copy.\n---\n"
    )

    sec_home = root / "profiles" / "xiuqo-security"
    (sec_home / "skills").mkdir(parents=True)

    coder_home = root / "profiles" / "coder"
    (coder_home / "skills").mkdir(parents=True)

    monkeypatch.setenv("XIUQO_HOME", str(sec_home))

    import xiuqo_constants
    monkeypatch.setattr(xiuqo_constants, "get_default_hermes_root", lambda: root)
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        # Re-import the module so SKILLS_DIR picks up XIUQO_HOME (set in
        ...
        assert "not found in active profile 'xiuqo-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.xiuqo/profiles/<name>/."""
        # Don't set XIUQO_HOME — falls back to default.
        import agent.file_safety as fs
    ...
    def test_named_profile_line_in_prompt_text(self, fake_hermes):
        """When active profile is 'xiuqo-security', the prompt warns
        ...
        assert "Active Xiuqo profile" in src
        assert "cross_profile=True" not in src  # guard retired
        assert "~/.xiuqo/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Xiuqo profile: default" in src
        assert "Active Xiuqo profile: {active_profile}" in src
