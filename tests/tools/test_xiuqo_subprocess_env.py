"""Tests for xiuqo_subprocess_env() — the centralized credential-safe env
...
full credential environment. Two tiers:
  ...
  * Tier 2 (_XIUQO_PROVIDER_ENV_BLOCKLIST): LLM provider/tool keys — stripped
...
from tools.environments.local_env_policy import _ALWAYS_STRIP_KEYS, _XIUQO_PROVIDER_ENV_FORCE_PREFIX
...
_TIER1_SAMPLE = {
    ...
    "XIUQO_DASHBOARD_SESSION_TOKEN": "dash-secret",
}
...
def _build(extra=None, *, inherit_credentials=False):
    ...
    with patch.dict(os.environ, env, clear=True):
        return xiuqo_subprocess_env(inherit_credentials=inherit_credentials)
...
class TestStripByDefault:
    ...
    def test_buzz_platform_vars_stripped_by_default(self):
        ...
        for var in buzz_sample:
            assert var not in result, f"{var} leaked via xiuqo_subprocess_env"
...
class TestDelegatedChildMarker:
    def test_delegated_child_context_scrubs_parent_kanban_keys_and_sets_marker(self):
        ...
        with patch.dict(
            ...
            {
                **_SAFE_SAMPLE,
                "XIUQO_KANBAN_TASK": "t_parent",
                "XIUQO_KANBAN_RUN_ID": "123",
                "XIUQO_KANBAN_DB": "/tmp/parent-kanban.db",
                "XIUQO_KANBAN_WORKSPACE": "/tmp/parent-workspace",
            },
            clear=True,
        ):
        ...
        assert env["XIUQO_KANBAN_DB"] == "/tmp/parent-kanban.db"
        assert env["XIUQO_KANBAN_WORKSPACE"] == "/tmp/parent-workspace"
