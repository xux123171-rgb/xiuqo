...
class TestDefaults:
    ...
    def test_get_limits_returns_defaults_when_load_config_raises(self):
        ...
        with patch("xiuqo_cli.config.load_config", side_effect=_boom):
...
class TestDefaultConfigHasSection:
    """The DEFAULT_CONFIG in xiuqo_cli.config must expose tool_output so
    that ``xiuqo setup`` and default installs stay in sync with the
    helpers here."""
