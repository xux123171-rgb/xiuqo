...
2. ``local_delivery_command`` hardcoded ``"xiuqo"``, relying on PATH —
   ...
   basename so resolved absolute paths (and ``xiuqo.exe``) still take
...
def test_waiter_windows_path_compiles_after_backslash_folding():
    ...
    code = _waiter_code("C:\\Users\\joshu\\.xiuqo")
...
def test_waiter_raw_prefix_keeps_injection_defense():
    ...
    code = _waiter_code(Path("/tmp/xiuqo-home"), inj)
...
def test_local_delivery_uses_shutil_which_when_no_sibling(tmp_path, monkeypatch):
    ...
    which_hit = str(tmp_path / "usr-local-bin" / "xiuqo")
    monkeypatch.setattr(
        bot_relay.shutil, "which", lambda name: which_hit if name == "xiuqo" else None
    )
...
def test_delivery_lock_recognizes_resolved_cli_paths(tmp_path, monkeypatch):
    ...
    now that argv[0] may be a resolved absolute path (or xiuqo.exe)."""
    acquired = []
    ...
    with bot_mode_dm._delivery_lock(
        [str(tmp_path / "venv" / "bin" / "xiuqo"), "-p", "ops", "chat"],
        stdin_file=False,
    ):
        pass
    with bot_mode_dm._delivery_lock(["xiuqo", "-p", "ops", "chat"], stdin_file=False):
        pass
    with bot_mode_dm._delivery_lock(
        ["C:\\venv\\Scripts\\xiuqo.exe", "-p", "ops", "chat"], stdin_file=False
    ):
        pass
