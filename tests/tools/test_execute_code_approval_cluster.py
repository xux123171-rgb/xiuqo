...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad XIUQO_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: XIUQO_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_hermes_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "XIUQO_HOME": "/h", "XIUQO_PROFILE": "p",
        "XIUQO_CONFIG": "/c.yaml", "XIUQO_ENV": "/e",
        "XIUQO_DELEGATED_CHILD_CONTEXT": "1",
        # other XIUQO_* → dropped (broad prefix removed)
        "XIUQO_BASE_URL": "https://x", "XIUQO_INTERACTIVE": "1",
    ...
    for kept in (
        ...
        "XIUQO_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "XIUQO_BASE_URL", "XIUQO_INTERACTIVE", "XIUQO_KANBAN_TASK",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped XIUQO_* vars."""
    import logging
