...
def _worker_board(tmp_path, monkeypatch):
    ...
    for key, value in {
        "XIUQO_KANBAN_DB": str(db), "XIUQO_KANBAN_BOARD": "default",
        "XIUQO_KANBAN_TASK": own, "XIUQO_KANBAN_RUN_ID": str(task.current_run_id),
        "XIUQO_KANBAN_CLAIM_LOCK": task.claim_lock, "HOME": str(tmp_path),
    }.items():
...
def test_terminal_descendants_cannot_mutate_even_after_task_is_removed(tmp_path, monkeypatch):
    ...
    script.write_text(
        ...
        " 'db': os.getenv('XIUQO_KANBAN_DB'), 'board': os.getenv('XIUQO_KANBAN_BOARD')}\n"
        ...
        "os.environ.pop('XIUQO_KANBAN_TASK', None)\n"
        f"p = subprocess.run([sys.executable, '-m', 'xiuqo_cli.main', 'kanban', 'complete', foreign, '--result', 'must refuse'], cwd={str(ROOT)!r}, capture_output=True, text=True, stdin=subprocess.DEVNULL, timeout=45)\n"
    ...
    child_envs = [
        ...
        _build_safe_env({"XIUQO_HOME": os.environ["XIUQO_HOME"]}),
    ]
...
def test_worker_cli_cannot_use_foreign_task_to_drop_run_scope(tmp_path, monkeypatch):
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "xiuqo_cli.main", "kanban", "complete", foreign, "--result", "no"],
    ...
    for arguments in (["attach", foreign, str(attachment)], ["unblock", foreign]):
        proc = subprocess.run([sys.executable, "-m", "xiuqo_cli.main", "kanban", *arguments],
                              cwd=ROOT, env=dict(os.environ), stdin=subprocess.DEVNULL,
                              capture_output=True, text=True, timeout=45)
