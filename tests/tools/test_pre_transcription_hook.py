...
PROMPT = "Xiuqo, Teknium, Nous Research, kanban"
...
def _fake_hooks(monkeypatch, results):
    ...
    monkeypatch.setattr("xiuqo_cli.plugins.invoke_hook", _invoke)
    return captured
...
def _no_hooks(monkeypatch):
    ...
    monkeypatch.setattr("xiuqo_cli.plugins.has_hook", lambda name: False)
    monkeypatch.setattr("xiuqo_cli.plugins.invoke_hook", _boom)
...
class TestUnsupportedBackends:
    def test_xai_logs_debug_and_proceeds_without_prompt(
        ...
        monkeypatch.setattr(
            "tools.xai_http.xiuqo_xai_user_agent", lambda: "test-ua",
        )
...
def test_real_fixture_plugins_thread_prompt_in_registration_order(
    ...
    xiuqo_home = Path(os.environ["XIUQO_HOME"])
    plugin_dir = xiuqo_home / "plugins" / "stt_vocab"
