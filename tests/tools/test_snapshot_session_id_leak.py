"""Cross-session XIUQO_SESSION_ID leak via the shared bash snapshot.
...
``XIUQO_SESSION_ID`` into the snapshot, so every LATER session ``source``d that
stale value and its ``echo $XIUQO_SESSION_ID`` reported a FOREIGN session's id
— overriding the correct per-command Popen env injected by
``_inject_session_context_env``.

The fix strips the per-session bridged vars (XIUQO_SESSION_* / UI /
...
def test_export_snippet_shape():
    ...
    assert "${!XIUQO_SESSION_*}" in snippet
    assert "${!XIUQO_CRON_AUTO_DELIVER_*}" in snippet
    assert "${!XIUQO_BROWSER_CONTROL_*}" in snippet
    assert "XIUQO_UI_SESSION_ID" in snippet
    ...
    try:
        def run_as(sid):
            ...
            def worker():
                ...
                out["r"] = env.execute('echo "[$XIUQO_SESSION_ID]"')
        ...
        if os.path.exists(snap):
            with open(snap) as f:
                assert "XIUQO_SESSION_ID" not in f.read()
    finally:
        env.cleanup()
