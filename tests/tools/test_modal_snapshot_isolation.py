...
def _restore_tool_modules():
    original_hermes_home = os.environ.get("XIUQO_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_hermes_home is None:
            ...
            os.environ["XIUQO_HOME"] = original_hermes_home
        _reset_modules(("tools", "xiuqo_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    xiuqo_cli = types.ModuleType("xiuqo_cli")
    xiuqo_cli.__path__ = [str(REPO_ROOT / "xiuqo_cli")]  # type: ignore[attr-defined]
    sys.modules["xiuqo_cli"] = xiuqo_cli
    xiuqo_home = tmp_path / "xiuqo-home"
    os.environ["XIUQO_HOME"] = str(xiuqo_home)
    sys.modules["xiuqo_cli.config"] = types.SimpleNamespace(
        get_hermes_home=lambda: xiuqo_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="xiuqo", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": xiuqo_home / "modal_snapshots.json",
