...
def test_saved_opt_in_roundtrip_reaches_schema_and_board(surface, tmp_path, monkeypatch):
    ...
    from xiuqo_cli.tools_config import _apply_toolset_change, _get_platform_tools
    ...
    if surface == "http":
        ...
        from xiuqo_cli.web_routers.tools import router
    ...
    try:
        ...
        from xiuqo_cli.kanban_db_connect import connect_closing
        from xiuqo_cli.kanban_db import get_task
...
def test_selection_is_scoped_and_preserves_worker_and_deny_boundaries(legacy, tmp_path, monkeypatch):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        named, broad = list(pool.map(_names, [["kanban"], ["xiuqo-cli"]]))
    ...
    monkeypatch.setenv("XIUQO_KANBAN_TASK", "t_worker")
    worker = _names(["file"])
