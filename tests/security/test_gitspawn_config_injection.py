...
``[diff "x"] command=/textconv=`` driver. Xiuqo gathers workspace context by
...
context-gathering git path Xiuqo runs neutralizes every sink. They use a real
...
def test_goals_fingerprint_is_safe(malicious_repo):
    from xiuqo_cli.goals import workspace_fingerprint
    repo, marker = malicious_repo
