...
COMMON_XIUQO_TOOLS = ["read_file", "search_files", "terminal", "patch", "write_file", "process"]


class TestToolKindMap:
    def test_all_xiuqo_tools_have_kind(self):
        """Every common xiuqo tool should appear in TOOL_KIND_MAP."""
        for tool in COMMON_XIUQO_TOOLS:
...
class TestBuildToolTitle:
    ...
    def test_execute_code_title_includes_first_code_line(self):
        ...
        assert "from xiuqo_tools import terminal" in title
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("web_search", {"query": "xiuqo agent acp"}),
            ("execute_code", {"code": "\nfrom xiuqo_tools import terminal\nprint('done')"}),
