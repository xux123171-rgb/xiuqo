...
Uses a throwaway XIUQO_HOME so it never touches ~/.xiuqo. No real names.
"""
...
def _fresh_home():
    """Point XIUQO_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_hermes_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["XIUQO_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER XIUQO_HOME is set.
...
def main():
    ...
    print(f"XIUQO_HOME (throwaway): {HOME}")
