...
(``run_agent``, ``xiuqo_state``, ``toolsets``...) reach the wheel through
...
``ModuleNotFoundError`` on ``import xiuqo_state``. This test pins the two halves of that
...
PACKAGES = ("agent", "tools", "xiuqo_cli", "gateway", "tui_gateway", "cron", "acp_adapter", "plugins", "providers")
...
def test_every_root_module_imported_by_packaged_code_is_shipped():
    ...
    assert "xiuqo_state" in shipped and "setup" not in shipped
