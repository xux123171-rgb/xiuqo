...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"xiuqo[termux]"' in text
    assert '"xiuqo[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"xiuqo[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
