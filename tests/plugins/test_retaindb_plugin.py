...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure XIUQO_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", xiuqo_home=str(tmp_path / ".xiuqo"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIUQO_HOME", str(xiuqo_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", xiuqo_home=str(xiuqo_home))
        return p
