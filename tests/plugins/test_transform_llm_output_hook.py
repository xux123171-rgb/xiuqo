...
from xiuqo_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(xiuqo_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <xiuqo_home>/plugins/<name> and opt it in."""
    plugin_dir = xiuqo_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XIUQO_HOME", str(tmp_path / "xiuqo_empty"))
