...
class TestProfileKeyIsolationWarning:
    ...
    def test_keyless_profile_block_warns_when_default_has_key(self, tmp_path, monkeypatch, caplog):
        ...
        config_path.write_text(json.dumps({
            'hosts': {
                'xiuqo': {'apiKey': 'shared-key'},
                'xiuqo_coder': {'baseUrl': 'http://192.168.1.50:8000'},
            },
        }))
        with caplog.at_level(logging.WARNING, logger='plugins.memory.honcho.client'):
            cfg = HonchoClientConfig.from_global_config(
                host='xiuqo_coder', config_path=config_path,
            )
    ...
    def test_no_warning_for_default_host(self, tmp_path, monkeypatch, caplog):
        ...
        config_path = tmp_path / 'config.json'
        config_path.write_text(json.dumps({
            'hosts': {'xiuqo': {'baseUrl': 'http://localhost:8000'}},
        }))
        with caplog.at_level(logging.WARNING, logger='plugins.memory.honcho.client'):
            HonchoClientConfig.from_global_config(
                host='xiuqo', config_path=config_path,
            )
        assert not any('NOT inherited' in r.message for r in caplog.records)
...
def test_save_config_holds_the_refresh_locks_so_a_rotation_survives(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "honcho.json"
    config_path.write_text(json.dumps({"hosts": {"xiuqo": {"apiKey": "hch-at-old", "oauth": {"refreshToken": "hch-rt-old"}}}}))
    rotated = oauth.OAuthCredential("hch-at-new", "hch-rt-new", 10_000, "xiuqo-desktop", "http://localhost:8000/oauth/token")
    ...
    def rotate():
        ...
        with oauth._refresh_lock, oauth._config_refresh_lock(config_path):
            oauth._persist_credential(config_path, "xiuqo", rotated)
        rotation_done.set()
    ...
    data = json.loads(config_path.read_text())
    assert data["hosts"]["xiuqo"]["apiKey"] == "hch-at-new" and data["logging"] is True
