...
XIUQO_HOME override (``xiuqo_constants.set_hermes_home_override``).
...
def homes(tmp_path, monkeypatch):
    """Profile A (launch home, ``XIUQO_HOME``) and profile B with different config/.env values."""
    root = tmp_path / ".xiuqo"
...
def test_router_efforts_cache_and_base_url_follow_the_active_profile(homes, monkeypatch):
    ...
    profile's .env."""
    import xiuqo_cli.urllib_security as urllib_security
...
def test_memory_provider_skill_prune_only_touches_the_active_home(homes, monkeypatch):
    ...
    from xiuqo_cli.plugins import _reset_plugin_managers_for_tests, get_plugin_manager
    ...
    try:
        for home in (a, b):
            with scoped(home):
                ...
                provider.initialize(session_id=f"s-{home.name}", xiuqo_home=str(home))
