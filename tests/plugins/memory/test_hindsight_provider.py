...
from xiuqo_cli.memory_setup import _CANCELLED
...
def _provider_for_mode(tmp_path, monkeypatch, mode: str):
    ...
    provider.initialize(session_id="test-session", xiuqo_home=str(tmp_path), platform="cli")
    return provider
...
def provider(tmp_path, monkeypatch):
    ...
    p.initialize(session_id="test-session", xiuqo_home=str(tmp_path), platform="cli")
...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:xiuqo, agent:fakeassistantname") == [
...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Xiuqo Agent and the User"
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        xiuqo_home = tmp_path / "xiuqo-home"
        user_home = tmp_path / "user-home"
        user_home.mkdir()
        monkeypatch.setenv("HOME", str(user_home))
        monkeypatch.setattr("plugins.memory.hindsight.get_hermes_home", lambda: xiuqo_home)

        save_config = MagicMock()
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("xiuqo_cli.memory_setup._curses_select", lambda *args, **kwargs: _CANCELLED)
        ...
        monkeypatch.setattr("xiuqo_cli.config.save_config", save_config)

        provider = HindsightMemoryProvider()
        provider.post_setup(str(xiuqo_home), {"memory": {"provider": "builtin"}})

        save_config.assert_not_called()
        which.assert_not_called()
        run.assert_not_called()
        assert not (xiuqo_home / ".env").exists()
        assert not (xiuqo_home / "hindsight" / "config.json").exists()
        assert not (user_home / ".hindsight" / "profiles" / "xiuqo.env").exists()
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("xiuqo_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "sk-local-test")
        saved_configs = []
        monkeypatch.setattr("xiuqo_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(xiuqo_home), {"memory": {}})
        ...
        profile_env = user_home / ".hindsight" / "profiles" / "xiuqo.env"
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config, monkeypatch):
        ...
        item = call_kwargs["items"][0]
        assert item["context"] == "conversation between Xiuqo Agent and the User"
        ...
        assert item["metadata"]["source"] == "xiuqo"
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        p1.initialize(session_id="resumed-session", xiuqo_home=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", xiuqo_home=str(tmp_path), platform="cli")
...
class TestRetainIndicator:
    ...
    def test_status_callback_wired_from_initialize(self, tmp_path, monkeypatch):
        ...
        p.initialize(session_id="s", xiuqo_home=str(tmp_path), status_callback=cb)
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("xiuqo") == "xiuqo"
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "xiuqo-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            session_id="s1",
            xiuqo_home=str(tmp_path),
            platform="cli",
            agent_identity="coder",
            agent_workspace="xiuqo",
        )
        assert p._bank_id == "xiuqo-coder"
        assert p._bank_id_template == "xiuqo-{profile}"
...
def test_load_config_corrupt_profile_file_falls_through_to_env(tmp_path, monkeypatch):
    """A corrupt $XIUQO_HOME/hindsight/config.json is not the config: the loader falls through
    ...
    assert cfg["banks"]["xiuqo"]["bankId"] == "from-env"
...
class TestPostSetupEnvEncoding:
    def _run_cloud_post_setup(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("xiuqo_cli.memory_setup._curses_select",
                            lambda *a, **kw: 0)  # cloud mode
        monkeypatch.setattr("xiuqo_cli.config.save_config", lambda c: None)
...
class TestClientAutoUpgradeRoutesThroughLazyDeps:
    ...
    def _init_with_outdated_client(self, tmp_path, monkeypatch, outcome):
        ...
        provider = HindsightMemoryProvider()
        provider.initialize(session_id="s", xiuqo_home=str(tmp_path), platform="cli")
        return calls
...
class TestMultiplexBackgroundScope:
    ...
    def test_writer_thread_resolves_profile_secret(self, scoped_embedded):
        ...
        p = HindsightMemoryProvider()
        p._mode = "local_embedded"
        p._config = {"profile": "xiuqo", "llm_provider": "openai", "llm_model": "m"}
        p._ensure_writer()
    ...
    def test_daemon_start_thread_resolves_profile_secret(self, scoped_embedded):
        created, home = scoped_embedded
        p = HindsightMemoryProvider()
        p.initialize(session_id="s1", xiuqo_home=str(home), platform="cli")
