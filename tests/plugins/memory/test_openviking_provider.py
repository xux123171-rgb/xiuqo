...
_EXPECTED_USER_AGENT = f"openviking-memory-xiuqo/{_XIUQO_VERSION}"
...
def test_post_setup_existing_profile_picker_validates_and_links_saved_profile(
    ...
    from xiuqo_cli import memory_setup
    ...
    OpenVikingMemoryProvider().post_setup(str(xiuqo_home), config)
...
def test_start_local_openviking_server_strips_pythonpath_from_child_env(monkeypatch):
    """The spawned server must not inherit Xiuqo's PYTHONPATH (#78153).

    Inheriting it makes openviking-server import packages from the Xiuqo
    venv instead of its own, and on Windows locks Xiuqo venv DLLs so the
    venv cannot be rebuilt during `xiuqo update`.
    """
    popen_calls = []
    ...
    monkeypatch.setattr(openviking_module.shutil, "which", lambda name: "/usr/local/bin/openviking-server")
    monkeypatch.setattr(openviking_module.subprocess, "Popen", fake_popen)
    monkeypatch.setenv("PYTHONPATH", "/opt/xiuqo/.venv/Lib/site-packages")
    monkeypatch.setenv("XIUQO_PROFILE", "test-profile")
    ...
    assert child_env.get("XIUQO_PROFILE") == "test-profile"
...
def test_https_local_endpoint_is_not_runtime_autostart_eligible(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory is temporarily unavailable; Xiuqo will retry on a later access or when "
        "the config changes. "
...
def test_runtime_does_not_autostart_when_local_server_reports_unhealthy(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory is temporarily unavailable; Xiuqo will retry on a later access "
...
def test_tool_add_resource_rejects_hermes_credential_file_upload(tmp_path, monkeypatch):
    ...
    xiuqo_home.mkdir()
    auth_json = xiuqo_home / "auth.json"
    auth_json.write_text('{"OPENROUTER_API_KEY":"sk-test-secret"}', encoding="utf-8")
    monkeypatch.setattr(fs, "_hermes_home_path", lambda: xiuqo_home)
...
def test_viking_client_delete_uses_identity_headers(monkeypatch):
    ...
    assert captured["kwargs"]["headers"]["X-OpenViking-Actor-Peer"] == "xiuqo"
...
# ---------------------------------------------------------------------------
# on_session_switch — flush + commit + rotate behavior (xiuqo#28296)
# ---------------------------------------------------------------------------
...
def test_sync_turn_captures_session_id_before_worker_runs():
    ...
    assert captured_payloads == [{
        "messages": [
            ...
            {"role": "assistant", "parts": [{"type": "text", "text": "a"}], "peer_id": "xiuqo"},
        ]
    }]
...
def test_session_needs_commit_guard_wins_over_stale_turn_count():
    """Regression for xiuqo#28296 review (M3): once a session is marked
...
class TestOpenVikingEnvWriter:
    ...
    def test_bom_prefixed_env_updates_in_place(self, tmp_path):
        ...
        # The stale value must be gone, not left as a duplicate. Xiuqo and
