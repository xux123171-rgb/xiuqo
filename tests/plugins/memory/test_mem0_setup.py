...
def _inject_fake_xiuqo_cli(monkeypatch):
    """Inject fake xiuqo_cli modules so yaml/curses aren't required."""
    fake_config_mod = types.ModuleType("xiuqo_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("xiuqo_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_xiuqo_cli = types.ModuleType("xiuqo_cli")
    fake_xiuqo_cli.config = fake_config_mod
    fake_xiuqo_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "xiuqo_cli", fake_xiuqo_cli)
    monkeypatch.setitem(sys.modules, "xiuqo_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "xiuqo_cli.memory_setup", fake_setup_mod)
...
class TestPostSetup:
    ...
    def test_platform_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["xiuqo", "--mode", "platform", "--api-key", "sk-test"])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_hermes_home", lambda: tmp_path)
        _inject_fake_xiuqo_cli(monkeypatch)
        config = {"memory": {}}
    ...
    def test_selfhosted_flag_mode(self, tmp_path, monkeypatch):
        ...
        assert mem0_json["user_id"] == "xiuqo-user"
...
def test_discovery_loaded_setup_module_exposes_post_setup(monkeypatch):
    """`xiuqo memory setup mem0` reaches the wizard when the package is first imported by plugin
