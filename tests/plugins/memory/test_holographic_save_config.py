...
def test_save_config_merges_into_existing_yaml(tmp_path, monkeypatch):
    ...
    assert raw["plugins"]["xiuqo-memory-store"] == {"db_path": "custom.db", "hrr_dim": "512"}
...
def test_save_config_respects_managed_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("xiuqo_cli.config.is_managed", lambda: True)
    monkeypatch.setattr("xiuqo_cli.config.managed_error", lambda *_a, **_k: None)
