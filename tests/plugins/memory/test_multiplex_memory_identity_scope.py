...
OPENVIKING_* / HINDSIGHT_BANK_ID / XIUQO_HONCHO_HOST, the provider must fall back to its own default
...
_DEFAULT_ENV = {
    ...
    "XIUQO_HONCHO_HOST": "host-default", "HONCHO_BASE_URL": "https://honcho.default",
...
def test_secondary_profile_memory_identity_never_inherits_default_environ(secondary_profile):
    ...
    assert (cfg["agent_id"], cfg["host"], cfg["mode"]) == ("xiuqo", "", "platform")
    ...
    provider.initialize("s1", xiuqo_home=str(secondary_profile))
    assert provider._client.project == "xiuqo-b"
    ...
    assert (hcfg["banks"]["xiuqo"]["bankId"], hcfg["mode"]) == ("xiuqo", "cloud")
