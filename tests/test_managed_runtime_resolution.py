"""Guard: Xiuqo-owned subprocesses must not resolve managed runtimes by bare PATH.

Xiuqo installs runtimes for itself — ``uv`` at ``$XIUQO_HOME/bin/uv``, Node at
``$XIUQO_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Xiuqo's own
...
* when a system copy also exists, the one Xiuqo does not own wins — which is
...
# Runtimes Xiuqo provisions into XIUQO_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Xiuqo-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("xiuqo_cli/update_cmd_deps.py", "uv"): (
    ...
    ("xiuqo_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (xiuqo_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Xiuqo-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$XIUQO_HOME/bin (uv) and $XIUQO_HOME/node are not on an "
        ...
        "  node/npm -> xiuqo_constants.find_node_executable()\n"
        "  PATH env -> xiuqo_constants.iter_hermes_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(xiuqo_constants, helper))


def test_managed_uv_helpers_exist():
    from xiuqo_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
