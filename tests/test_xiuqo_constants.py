"""Tests for xiuqo_constants module."""
...
from xiuqo_constants import (
    ...
    xiuqo_managed_node_tree_present,
...
class TestGetDefaultHermesRoot:
    ...
    def test_no_hermes_home_returns_native(self, tmp_path, monkeypatch):
        """When XIUQO_HOME is not set, returns ~/.xiuqo."""
        monkeypatch.delenv("XIUQO_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_hermes_root() == tmp_path / ".xiuqo"
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (XIUQO_HOME=<root>/profiles/<name>),
    ...
    def test_no_hermes_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\xiuqo, not ~/.xiuqo."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("XIUQO_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")

        assert get_default_hermes_root() == local_appdata / "xiuqo"

    def test_result_memoised_until_env_or_home_changes(self, tmp_path, monkeypatch):
        """Repeated calls reuse the memo; XIUQO_HOME / home changes invalidate.

        get_default_hermes_root() resolves XIUQO_HOME against the native
        ...
        (native home, XIUQO_HOME) compared for free each call.
        """
        # XIUQO_HOME set to a Docker-profile path: every call resolves the
        ...
        monkeypatch.setattr(
            xiuqo_constants, "_default_hermes_root_memo", None, raising=False
        )
        ...
        # XIUQO_HOME change invalidates the memo (fresh resolution).
        other_profile = docker_root / "profiles" / "writer"
        other_profile.mkdir(parents=True)
        monkeypatch.setenv("XIUQO_HOME", str(other_profile))
        before = resolve_calls["n"]
        assert get_default_hermes_root() == docker_root
        assert resolve_calls["n"] > before, (
            "XIUQO_HOME change must force a fresh resolution"
        )
...
class TestGetHermesHome:
    ...
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When XIUQO_HOME is unset on Windows, use %LOCALAPPDATA%\\xiuqo."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("XIUQO_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(xiuqo_constants, "_profile_fallback_warned", False)

        assert get_hermes_home() == local_appdata / "xiuqo"
...
class TestHermesManagedNode:
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIUQO_HOME", str(home))
        monkeypatch.setattr(xiuqo_constants, "node_tool_runnable", lambda path: True)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(xiuqo_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(xiuqo_constants, "heal_hermes_managed_node", lambda: False)
        monkeypatch.setattr(
            xiuqo_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert xiuqo_managed_node_tree_present() is True
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Xiuqo-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(xiuqo_constants, "heal_hermes_managed_node", _heal)
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = xiuqo_constants._XIUQO_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
        ...
        resolved = xiuqo_constants.find_hermes_node_executable("node")
        assert heal_called["value"] is True
    ...
    def test_outdated_managed_node_survives_failed_heal(self, tmp_path, monkeypatch):
        ...
        assert xiuqo_constants.find_hermes_node_executable("node") == str(old_node)
    ...
    def test_target_major_managed_node_does_not_heal(self, tmp_path, monkeypatch):
        ...
        assert xiuqo_constants.find_hermes_node_executable("node") == str(node)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(xiuqo_constants, "_container_detected", None)
    ...
    def test_cgroup_v2_fallback_inspects_only_the_root_mount(self, tmp_path):
        ...
        from xiuqo_constants import _root_mount_has_marker
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(xiuqo_constants, "_container_detected", True)
        assert is_container() is True
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from xiuqo_constants import resolve_per_model_reasoning_effort
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".xiuqo"
    ...
    def test_install_tree_skipped(self, monkeypatch):
        ...
        Regression test for #93050: secure_parent_dir() chmod'd /opt/xiuqo to
        0700 because it has 3 path parts and passed the ``< 3`` guard, locking
        out UID 10000 (xiuqo user) from traversing the install dir.
        """
        install_root = Path(xiuqo_constants.__file__).resolve().parent

        # Directly under the install root (e.g. /opt/xiuqo/auth.json)
    ...
    def test_install_tree_siblings_still_hardened(self, monkeypatch):
        ...
        # Prefix-named sibling of the install root (/opt/xiuqo-data/...).
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import xiuqo_cli._subprocess_compat as subprocess_compat
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert xiuqo_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert xiuqo_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert xiuqo_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert xiuqo_constants.windows_path_to_wsl("/home/alex") is None
        assert xiuqo_constants.windows_path_to_wsl("relative\\dir") is None
    ...
    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        ...
        assert xiuqo_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert xiuqo_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert xiuqo_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
...
class TestManagedNodeTreeInUse:
    ...
    def _install_fake_psutil(self, monkeypatch, procs):
        ...
        monkeypatch.setattr(xiuqo_constants.sys, "platform", "win32")
    ...
    def test_always_false_off_windows(self, tmp_path, monkeypatch):
        monkeypatch.setattr(xiuqo_constants.sys, "platform", "darwin")
        monkeypatch.setenv("XIUQO_HOME", str(tmp_path))
        assert xiuqo_constants.managed_node_tree_in_use() is False
    ...
    def test_exe_under_node_dir_counts(self, tmp_path, monkeypatch):
        ...
        assert xiuqo_constants.managed_node_tree_in_use() is True
...
class TestWindowsHealStageSwap:
    ...
    def _stub_env(
        ...
        monkeypatch.setenv("XIUQO_HOME", str(home))
        monkeypatch.setenv(
            "XIUQO_NODE_TARGET_MAJOR",
            str(xiuqo_constants._XIUQO_NODE_TARGET_MAJOR),
        )
        monkeypatch.setattr(xiuqo_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(
            xiuqo_constants, "_managed_node_in_use_notice_printed", False
        )
        monkeypatch.setattr(
            xiuqo_constants,
        ...
        monkeypatch.setattr(
            xiuqo_constants, "node_tool_runnable", lambda path: True
        )
    ...
    def test_in_use_defers_without_touching_tree(self, tmp_path, monkeypatch):
        ...
        zip_name, zip_bytes = _make_node_zip(xiuqo_constants._XIUQO_NODE_TARGET_MAJOR)
        ...
        result = xiuqo_constants._heal_managed_node_windows()
...
class TestHealAttemptFlagSemantics:
    ...
    def test_deferral_keeps_flag_clear_and_retries(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(xiuqo_constants, "_heal_managed_node_windows", fake_heal)

        assert heal_hermes_managed_node() is False
        assert xiuqo_constants._managed_node_heal_attempted is False
    ...
    def test_real_failure_records_attempt(self, tmp_path, monkeypatch):
        ...
        assert heal_hermes_managed_node() is False
        assert xiuqo_constants._managed_node_heal_attempted is True
