"""Tests for xiuqo_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
# Use whatever RotatingFileHandler class xiuqo_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. xiuqo_logging
...
from xiuqo_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    xiuqo_logging._install_session_record_factory()
    yield
    ...
    xiuqo_logging._logging_initialized = False
    xiuqo_logging.clear_session_context()


@pytest.fixture
def xiuqo_home(tmp_path, monkeypatch):
    """Provide an isolated XIUQO_HOME for logging tests.
    ...
    """
    home = Path(os.environ["XIUQO_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, xiuqo_home):
        log_dir = xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home)
        assert log_dir == xiuqo_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, xiuqo_home):
        xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in xiuqo_logging._queued_file_handlers
    ...
    def test_idempotent_no_duplicate_handlers(self, xiuqo_home):
        xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home)
        xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, xiuqo_home):
        xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home)

        test_logger = logging.getLogger("test_xiuqo_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        xiuqo_logging.flush_log_queue()

        agent_log = xiuqo_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_profile_routing_follows_context_home(self, xiuqo_home, tmp_path):
        ...
        profile_home.mkdir()
        xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home)
        assert xiuqo_logging.enable_profile_log_routing(
            [xiuqo_home, profile_home]
        ) is True
        ...
        assert "profile-routed cron record" not in (
            xiuqo_home / "logs" / "agent.log"
        ).read_text()




    def test_explicit_params_override_config(self, xiuqo_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (xiuqo_home / "config.yaml").write_text(yaml.dump(config))

        xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, xiuqo_home):
        xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, xiuqo_home):
        xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, xiuqo_home):
        ...
        gw_log = xiuqo_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, xiuqo_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, xiuqo_home):
        xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, xiuqo_home):
        xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home, mode="gui")

        logging.getLogger("xiuqo_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        xiuqo_logging.flush_log_queue()

        gui_log = xiuqo_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, xiuqo_home):
        """When session context is set, log lines include [session_id]."""
        xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home)
        xiuqo_logging.set_session_context("abc123")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = xiuqo_logging._ComponentFilter(("gateway",))
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, xiuqo_home):
        xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home)
        xiuqo_logging.setup_verbose_logging()
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        xiuqo_logging._add_rotating_handler(
            log_path,
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in xiuqo_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)]
        ...
        xiuqo_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("xiuqo_cli.config.is_managed", return_value=True):
                xiuqo_logging._add_rotating_handler(
                    log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = xiuqo_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert xiuqo_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not xiuqo_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, xiuqo_home):
        level, max_size, backup = xiuqo_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, xiuqo_home):
        import yaml
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> xiuqo_logging._ManagedRotatingFileHandler:
        handler = xiuqo_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that xiuqo_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        xiuqo_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home, mode="gateway")
        gw_path = xiuqo_home / "logs" / "gateway.log"
        rotated = xiuqo_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = xiuqo_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, xiuqo_home):
        xiuqo_logging.setup_logging(xiuqo_home=xiuqo_home)
        root = logging.getLogger()
        ...
        assert any(
            ...
            for h in xiuqo_logging._queued_file_handlers
        )
