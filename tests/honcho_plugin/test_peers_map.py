"""Tests for ``xiuqo honcho peers map``: account discovery, the resolution preview, and alias writes."""
...
def _cfg(root=None, **host):
    return {"apiKey": "***", **(root or {}), "hosts": {"xiuqo": host}}
...
def _run_map(monkeypatch, tmp_path, *, answers, cfg, db_rows=(), ws_peers=None, workspaces=None):
    ...
    # One profile per host block: "xiuqo" is default, "xiuqo.<name>" is profile <name>.
    profiles = [("default" if k == "xiuqo" else k.removeprefix("xiuqo."), k, v) for k, v in cfg["hosts"].items()]
    ...
    for name, value in {
        ...
        "_peers_map_client": lambda workspace=None: (client, SimpleNamespace(workspace_id="xiuqo") if client else None),
...
class TestCmdPeersMap:
    @pytest.mark.parametrize("cfg, answers, kwargs", [
        ...
        (_cfg(), ["111", "eri", ""], {}),
        (_cfg(peerName="eri"), ["1", "p1", ""], dict(db_rows=[_row("111")], ws_peers=["eri", "xiuqo"])),
    ...
    def test_writes_alias_to_host_block(self, monkeypatch, tmp_path, cfg, answers, kwargs):
        ...
        assert written["cfg"]["hosts"]["xiuqo"]["userPeerAliases"] == {"111": "eri"}
    ...
    def test_dash_clears_alias(self, monkeypatch, tmp_path, aliases, expected_host):
        ...
        assert written["cfg"]["hosts"]["xiuqo"] == expected_host
    ...
    def test_cleared_host_map_does_not_resurrect_root_aliases(self, monkeypatch, tmp_path, capsys):
        ...
        assert written["cfg"]["hosts"]["xiuqo"]["userPeerAliases"] == {}
        assert "root aliases no longer apply to [xiuqo]" in capsys.readouterr().out

        path = tmp_path / "honcho.json"
        path.write_text(json.dumps(written["cfg"]))
        assert HonchoClientConfig.from_global_config(host="xiuqo", config_path=path).user_peer_aliases == {}

    @pytest.mark.parametrize("cfg, answers, kwargs", [
        (_cfg(), [""], {}),
        (_cfg(userPeerAliases={"111": "eri"}), ["111", "eri", ""], {}),
        (_cfg(pinUserPeer=True, peerName="eri"), ["n"], {}),
        (_cfg(peerName="eri"), ["w", "2", "n", ""], dict(ws_peers=["eri"], workspaces=["xiuqo", "cosmania-dex"])),
    ...
    @pytest.mark.parametrize("cfg, answers, kwargs, expected", [
        ...
        ({"apiKey": "***", "userPeerAliases": {},
          "hosts": {"xiuqo": {"workspace": "xiuqo"}, "xiuqo.dreamer": {"workspace": "dreamland"}}},
         ["222", "tek", "", "all"], {}, ["also apply in workspace 'dreamland'"]),
        ({"apiKey": "***", "hosts": {"xiuqo": {"peerName": "eri", "userPeerAliases": {"111": "eri"}},
                                     "xiuqo.dreamer": {"peerName": "eri", "userPeerAliases": {"111": "bob"}}}},
...
class TestWorkspaceSwitch:
    def test_browse_and_confirmed_switch_writes_workspace(self, monkeypatch, tmp_path):
        written = _run_map(
            ...
            cfg=_cfg(peerName="eri"), ws_peers=["eri"], workspaces=["xiuqo", "cosmania-dex"],
        )
        assert written["cfg"]["hosts"]["xiuqo"]["workspace"] == "cosmania-dex"
    ...
    def test_browse_client_is_built_for_the_browsed_workspace(self, monkeypatch):
        ...
        seen = []
        base = client_mod.HonchoClientConfig(host="xiuqo", workspace_id="xiuqo", api_key="k")
        ...
        assert own.workspace_id == "xiuqo"
        assert browsed.workspace_id == "cosmania-dex"
        assert [c.workspace_id for c in seen] == ["xiuqo", "cosmania-dex"]
...
class TestSaveScope:
    @pytest.mark.parametrize("hosts, answers", [
        ({"xiuqo": {}}, ["222", "tek", ""]),
        ({"xiuqo": {}, "xiuqo.dreamer": {}}, ["222", "tek", "", "all"]),
    ...
    def test_root_sourced_aliases_write_back_to_root(self, monkeypatch, tmp_path, hosts, answers):
        ...
        assert "userPeerAliases" not in written["cfg"]["hosts"]["xiuqo"]

    def test_scope_this_forks_host_block(self, monkeypatch, tmp_path):
        cfg = {"apiKey": "***", "userPeerAliases": {"111": "eri"}, "hosts": {"xiuqo": {}, "xiuqo.dreamer": {}}}
        written = _run_map(monkeypatch, tmp_path, answers=["222", "tek", "", "this"], cfg=cfg)
        assert written["cfg"]["hosts"]["xiuqo"]["userPeerAliases"] == {"111": "eri", "222": "tek"}
...
def test_classify_workspace_peers_labels_from_local_config(monkeypatch):
    monkeypatch.setattr(honcho_cli, "_host_key", lambda: "xiuqo")
    rows = [
        ("default", "xiuqo", {"peerName": "eri", "aiPeer": "hermetika"}),
        ("dreamer", "xiuqo.dreamer", {"aiPeer": "dreamer-ai"}),
    ]
