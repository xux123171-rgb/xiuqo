...
class TestResolveApiKey:
    ...
    def test_returns_api_key_from_root(self, monkeypatch):
        ...
        monkeypatch.setattr(honcho_cli, "_host_key", lambda: "xiuqo")
...
class TestCmdSetupLocalJwt:
    ...
    def test_local_setup_stores_jwt_under_host_block(self, monkeypatch, tmp_path):
        ...
        host_block = (cfg.get("hosts") or {}).get("xiuqo") or {}
...
class TestCmdStatus:
    ...
    def test_auth_line_detects_oauth_grant(self, monkeypatch, capsys, tmp_path):
        ...
        out = capsys.readouterr().out
        assert "Auth:           OAuth (xiuqo" in out
...
class TestCloneHonchoForProfile:
    ...
    def test_user_peer_aliases_carry_into_cloned_profile(self, monkeypatch, tmp_path):
        ...
        assert ok is True
        new_block = written["cfg"]["hosts"]["xiuqo_coder"]
...
class TestSetupWizardDeploymentShape:
    ...
    calls, asserting the resulting xiuqo_host block so the tree's routing
    ...
    def _run_setup(self, monkeypatch, tmp_path, *, answers, initial_cfg=None,
        ...
        monkeypatch.setattr(
            "xiuqo_cli.config.load_config", lambda: {"memory": {}}, raising=False,
        )
        monkeypatch.setattr(
            "xiuqo_cli.config.save_config", lambda c: None, raising=False,
        )
    ...
    def test_unpin_steers_to_pooled_by_default(self, monkeypatch, tmp_path):
        ...
        initial_cfg = {
            "apiKey": "***",
            "hosts": {"xiuqo": {"pinPeerName": True, "peerName": "eri"}},
        }
        answers = [
            "cloud",           # deployment
            "",                # api key (keep)
    ...
    def test_mapping_step_points_at_peers_map(self, monkeypatch, tmp_path, capsys):
        self._run_setup(monkeypatch, tmp_path, answers=["cloud", "", "eri", "hermetika", "xiuqo", "s"])
        assert "xiuqo honcho peers map" in capsys.readouterr().out
    ...
    def test_host_pin_user_peer_true_is_detected_as_single(self, monkeypatch, tmp_path):
        ...
        initial_cfg = {
            "apiKey": "***",
            "hosts": {"xiuqo": {"pinUserPeer": True, "peerName": "eri"}},
        }
        ...
        answers = ["cloud", "", "eri", "hermetika", "xiuqo"]
        host = self._run_setup(monkeypatch, tmp_path, answers=answers, initial_cfg=initial_cfg)
    ...
    @pytest.mark.parametrize("initial_cfg, expected_pin", [
        (None, True),
        ({"apiKey": "***", "hosts": {"xiuqo": {}}}, True),
        ({"apiKey": "***", "hosts": {"xiuqo": {"pinUserPeer": False, "peerName": "eri"}}}, False),
        ({"apiKey": "***", "hosts": {"xiuqo": {"enabled": True, "workspace": "xiuqo", "peerName": "eri"}}}, False),
        ({"apiKey": "***", "enabled": True, "workspace": "xiuqo", "peerName": "eri"}, False),
    ...
    def test_no_gateway_connected_skips_mapping_when_declined(self, monkeypatch, tmp_path):
        ...
        answers = ["cloud", "", "eri", "hermetika", "xiuqo", "n"]
...
class TestCloneCarriesPinUserPeer:
    ...
    def test_clone_inherits_host_pin_user_peer(self, monkeypatch, tmp_path):
        ...
        assert ok is True
        new_block = written["cfg"]["hosts"]["xiuqo_partner"]
        assert new_block["pinUserPeer"] is True
...
class TestCmdSetupDeviceFlow:
    ...
    def _run_setup(self, monkeypatch, tmp_path, *, answers, device_available=True,
        ...
        cred = OAuthCredential(
            ...
            client_id="xiuqo", token_endpoint="http://x/oauth/token",
    ...
    def test_device_choice_runs_flow_and_stores_grant(self, monkeypatch, tmp_path):
        ...
        assert calls[0]["source"] == "xiuqo-cli"
        host = cfg["hosts"]["xiuqo"]
    ...
    def test_headless_defaults_to_device(self, monkeypatch, tmp_path):
        ...
        assert cfg["hosts"]["xiuqo"]["apiKey"] == "hch-at-x"
    ...
    def test_denied_device_flow_aborts_without_grant(self, monkeypatch, tmp_path):
        ...
        assert len(calls) == 1
        assert "apiKey" not in cfg.get("hosts", {}).get("xiuqo", {})
...
class TestWriteRefusesUnparseableStore:
    ...
    def test_command_prints_one_sentence_asks_nothing_and_writes_nothing(self, monkeypatch, tmp_path, capsys, run):
        ...
        honcho_cli = _point_cli_at(monkeypatch, cfg_path, _host_key=lambda: "xiuqo_coder",
    ...
    def test_apikey_answer(self, monkeypatch, tmp_path, capsys, grant, root_key, answer, ok, host_key, root_after, shown):
        ...
        if grant:
            host = {"apiKey": "hch-at-dead", "oauth": {"refreshToken": "hch-rt-dead", "expiresAt": 1,
                                                       "clientId": "xiuqo", "tokenEndpoint": "https://api.honcho.dev/oauth/token"}}
        cfg = {"hosts": {"xiuqo": host}, **({"apiKey": root_key} if root_key else {})}
...
_OAUTH_DEFAULT = {"peerName": "eri", "hosts": {"xiuqo": {
    "enabled": True, "apiKey": "hch-at-live", "workspace": "xiuqo", "peerName": "eri", "oauth": {"refreshToken": "hch-rt-live"},
}}}
_KEYLESS_DEFAULT = {"hosts": {"xiuqo": {"workspace": "xiuqo"}}}
...
class TestEnabledRequiresACredential:
    ...
    def _env(self, monkeypatch, tmp_path, cfg, *, env_key=None, host="xiuqo_dreamer", profile="dreamer"):
        import copy
    ...
    @pytest.mark.parametrize("cfg, env_key, enabled", [
        ...
        ({"hosts": {"xiuqo": {"enabled": True, "apiKey": "hch-v3-hostonly", "workspace": "xiuqo"}}}, None, False),
    ...
    def test_clone_is_enabled_only_by_an_on_disk_credential(self, monkeypatch, tmp_path, cfg, env_key, enabled):
        ...
        block = written["cfg"]["hosts"]["xiuqo_dreamer"]
        assert block.get("enabled") is (True if enabled else None)
        assert "apiKey" not in block and "oauth" not in block
        assert block["aiPeer"] == "dreamer" and block["workspace"] == "xiuqo"

    @pytest.mark.parametrize("cfg, env_key, profile, expect, enabled", [
        ({"hosts": {"xiuqo_dreamer": {"workspace": "xiuqo"}}}, "hch-v3-from-env", "dreamer", ["setup"], False),
        (_OAUTH_DEFAULT, None, "dreamer",
         ["stays disabled", "xiuqo honcho setup --target-profile dreamer", "hosts.xiuqo_dreamer"], False),
        ({"hosts": {"xiuqo_dreamer": {"enabled": True, "aiPeer": "dreamer", "workspace": "xiuqo"}}}, None, "dreamer",
         ["stays disabled"], False),
        ({"hosts": {}}, None, "default", ["Run 'xiuqo honcho setup' to sign in"], False),
    ...
    def test_enable_writes_enabled_only_for_an_on_disk_credential(self, monkeypatch, tmp_path, capsys,
        ...
        host = "xiuqo" if profile == "default" else "xiuqo_dreamer"
...
class TestWriteConfigMergesOntoDisk:
    ...
    def _rotate_on_disk(self, cfg_path):
        ...
        disk["hosts"]["xiuqo"].update(apiKey="hch-at-new", oauth={"refreshToken": "hch-rt-new"})
        cfg_path.write_text(json.dumps(disk))

    def test_untouched_keys_take_disk_and_the_commands_edits_apply(self, monkeypatch, tmp_path):
        disk = {"apiKey": "root", "hosts": {"xiuqo": {"apiKey": "hch-at-old", "oauth": {"refreshToken": "hch-rt-old"},
        ...
        cfg["hosts"]["xiuqo"]["recallMode"] = "tools"
        cfg["hosts"]["xiuqo"].pop("runtimePeerPrefix")
        cfg["dialecticCadence"] = 3
        honcho_cli._write_config(cfg)
        out = json.loads(cfg_path.read_text())
        assert out["hosts"]["xiuqo"] == {"apiKey": "hch-at-new", "oauth": {"refreshToken": "hch-rt-new"}, "recallMode": "tools"}
        assert out["apiKey"] == "root" and out["dialecticCadence"] == 3

    def test_a_credential_the_command_set_wins(self, monkeypatch, tmp_path):
        disk = {"hosts": {"xiuqo": {"apiKey": "hch-at-old", "oauth": {"refreshToken": "hch-rt-old"}}}}
        honcho_cli, cfg_path = self._paths(monkeypatch, tmp_path, disk)
        cfg = honcho_cli._read_config()
        self._rotate_on_disk(cfg_path)
        cfg["hosts"]["xiuqo"]["apiKey"] = "hch-v3-pasted"
        cfg["hosts"]["xiuqo"].pop("oauth")
        honcho_cli._write_config(cfg)
        assert json.loads(cfg_path.read_text())["hosts"]["xiuqo"] == {"apiKey": "hch-v3-pasted"}

    def test_a_second_write_on_the_same_read_applies_only_the_edits_made_since_the_first(self, monkeypatch, tmp_path):
        disk = {"hosts": {"xiuqo": {"apiKey": "hch-at-old", "oauth": {"refreshToken": "hch-rt-old"}, "workspace": "A"}}}
        honcho_cli, cfg_path = self._paths(monkeypatch, tmp_path, disk)
        cfg = honcho_cli._read_config()
        cfg["hosts"]["xiuqo"]["workspace"] = "B"
        honcho_cli._write_config(cfg)
        self._rotate_on_disk(cfg_path)
        cfg["hosts"]["xiuqo"]["workspace"] = "A"
        honcho_cli._write_config(cfg)
        out = json.loads(cfg_path.read_text())["hosts"]["xiuqo"]
    ...
    def test_a_grant_the_login_installed_yields_to_a_later_rotation(self, monkeypatch, tmp_path):
        ...
        honcho_cli, cfg_path = self._paths(monkeypatch, tmp_path, {"hosts": {"xiuqo": {"peerName": "alice"}}})
        monkeypatch.setattr(honcho_cli, "_host_key", lambda: "xiuqo")
        cfg = honcho_cli._read_config()
        grant = {"access_token": "hch-at-login", "refresh_token": "hch-rt-login", "expires_in": 3600}
        cred = oauth.install_grant(cfg_path, "xiuqo", grant, client_id="c", token_endpoint="e", apply_config=False)
        honcho_cli._apply_grant_to_host(cfg, cfg["hosts"]["xiuqo"], cred)
        self._rotate_on_disk(cfg_path)
        cfg["hosts"]["xiuqo"]["recallMode"] = "tools"
        honcho_cli._write_config(cfg)
        out = json.loads(cfg_path.read_text())["hosts"]["xiuqo"]
    ...
    _SEED = {"dialecticCadence": 3, "hosts": {"xiuqo": {"peerName": "alice"}}}
    ...
    def _seeded(self, monkeypatch, tmp_path):
        ...
        return _point_cli_at(monkeypatch, local, _config_path=lambda: seed, _host_key=lambda: "xiuqo"), local
    ...
    def test_a_read_seeded_from_another_file_applies_only_its_edits_onto_the_local_file(self, monkeypatch, tmp_path):
        ...
        cfg = honcho_cli._read_config()
        grant = {"access_token": "hch-at-login", "refresh_token": "hch-rt-login", "expires_in": 3600}
        cred = oauth.install_grant(local, "xiuqo", grant, client_id="c", token_endpoint="e", apply_config=False)
        honcho_cli._apply_grant_to_host(cfg, cfg["hosts"]["xiuqo"], cred)
        ...
        oauth._persist_credential(local, "xiuqo", rotated)
        cfg["hosts"]["xiuqo"]["recallMode"] = "tools"
        honcho_cli._write_config(cfg)
        out = json.loads(local.read_text())
        assert out["hosts"]["xiuqo"]["apiKey"] == "hch-at-new"
        assert out["hosts"]["xiuqo"]["oauth"]["refreshToken"] == "hch-rt-new"
        assert out["hosts"]["xiuqo"]["recallMode"] == "tools" and out["hosts"]["xiuqo"]["peerName"] == "alice"
    ...
    def test_a_read_seeded_from_another_file_is_written_whole_while_no_local_file_exists(self, monkeypatch, tmp_path):
        ...
        cfg["hosts"]["xiuqo"]["recallMode"] = "tools"
        honcho_cli._write_config(cfg)
        assert json.loads(local.read_text()) == {"dialecticCadence": 3, "hosts": {"xiuqo": {"peerName": "alice", "recallMode": "tools"}}}
    ...
    def test_a_plain_dict_is_written_whole(self, monkeypatch, tmp_path, build):
        honcho_cli, cfg_path = self._paths(monkeypatch, tmp_path, {"hosts": {"xiuqo": {"apiKey": "hch-at-old"}}})
