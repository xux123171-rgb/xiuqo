...
from xiuqo_cli.profiles import _get_default_hermes_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "xiuqo"
        assert config.workspace_id == "xiuqo"
...
class TestFromGlobalConfig:
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"xiuqo": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"xiuqo": {"baseUrl": "http://host-block:9001"}},
        }))
    ...
    def test_base_url_full_precedence_chain(self, tmp_path):
        ...
        config_file = tmp_path / "config.json"
        layers = {
            "hosts": {"xiuqo": {"baseUrl": "http://host:1"}},
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="xiuqo"
        ):
            result = config.resolve_session_name("/home/user/xiuqo/subdir")
        assert result == "xiuqo"
...
class TestResolveConfigPath:
    def test_prefers_hermes_home_when_exists(self, tmp_path):
        ...
        xiuqo_home.mkdir()
        local_cfg = xiuqo_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: XIUQO_HOME points at ~/.xiuqo/profiles/<name>, so
        # _get_default_hermes_home() must resolve back to ~/.xiuqo — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".xiuqo"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "xiuqo_coder"
        assert profile_host_key("default") == "xiuqo"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"XIUQO_HONCHO_HOST": "xiuqo.coder"}):
            assert resolve_active_host() == "xiuqo.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("XIUQO_HONCHO_HOST", None)
            # Temporarily remove xiuqo_cli.profiles to simulate import failure
            saved = sys.modules.get("xiuqo_cli.profiles")
            sys.modules["xiuqo_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "xiuqo"
            finally:
                if saved is not None:
                    sys.modules["xiuqo_cli.profiles"] = saved
                else:
                    sys.modules.pop("xiuqo_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="xiuqo_coder")
        assert config.host == "xiuqo_coder"
        assert config.workspace_id == "xiuqo"  # shared workspace
        assert config.ai_peer == "xiuqo_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"xiuqo": {"enabled": True, "aiPeer": "xiuqo"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (xiuqo.work)
        ...
        cfg = HonchoClientConfig(
            ...
            host="xiuqo_work",
            workspace_id="xiuqo",
            raw={"hosts": {"xiuqo.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIUQO_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("xiuqo_cli.profiles.get_active_profile_name", return_value="default"), \
