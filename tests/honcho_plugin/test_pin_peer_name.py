...
Under a gateway (Telegram, Discord, Slack, ...) Xiuqo passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIUQO_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Xiuqo via Telegram AND Discord
...
class TestPinTransition:
    ...
    def test_identity_signature_reflects_both_session_prefixes(self, tmp_path, monkeypatch):
        ...
        cfg_path = tmp_path / "honcho.json"
        monkeypatch.setenv("XIUQO_HOME", str(tmp_path))
        base = {"apiKey": "k", "peerName": "Igor", "aiPeer": "xiuqo"}
    ...
    def test_identity_signature_reflects_a_repointed_host_workspace(self, tmp_path, monkeypatch):
        """``xiuqo honcho peers map`` can repoint a host block's workspace; the cached agent's manager
        ...
        cfg_path.write_text(json.dumps({**base, "hosts": {"xiuqo": {"workspace": "old"}}}))
        sig_old = provider.identity_signature()["workspace"]
        cfg_path.write_text(json.dumps({**base, "hosts": {"xiuqo": {"workspace": "new"}}}))
...
class TestProfilePeerUniqueness:
    """Each Xiuqo profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `xiuqo -p partner` pinned to a different person's peer).
