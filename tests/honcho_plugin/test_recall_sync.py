...
def test_two_queries_never_consume_previous_query_caches(tmp_path, monkeypatch):
    ...
    assert HonchoClientConfig.from_global_config(host="xiuqo", config_path=path).recall_sync
    config_provider.save_config({"hosts": {"xiuqo": {"recallSync": False}}}, str(tmp_path))
    assert not HonchoClientConfig.from_global_config(host="xiuqo", config_path=path).recall_sync
...
def test_missing_peer_notice_surfaces_once():
    ...
    assert "xiuqo honcho peer --user" in first
