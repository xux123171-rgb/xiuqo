...
class TestHonchoSession:
    def _make_session(self):
        return HonchoSession(
            ...
            assistant_peer_id="xiuqo-assistant",
...
class TestPeerLookupHelpers:
    ...
    def test_search_context_uses_peer_perspective_message_search(self):
        ...
        honcho_client.search.return_value = [
            SimpleNamespace(content="Robert runs neuralancer", peer_id="xiuqo", session_id="s-old", id="m1"),
...
class TestToolsModeInitBehavior:
    ...
    def _make_provider_with_config(self, recall_mode="tools", init_on_session_start=False,
        ...
        with patch("plugins.memory.honcho.client.HonchoClientConfig.from_global_config", return_value=cfg), \
             ...
             patch("xiuqo_constants.get_hermes_home", return_value=MagicMock()):
...
class TestPerSessionMigrateGuard:
    ...
    per-session creates a fresh Honcho session every Xiuqo run. Uploading
...
class TestDialecticLifecycleSmoke:
    ...
    def test_full_multi_turn_session(self):
        ...
        responses = iter([
            "prewarm: user is eri, works on xiuqo",      # session-start prewarm
...
class TestContextTokensForwarded:
    ...
    def _manager(self, context_tokens=4000):
        ...
        session = HonchoSession(key="cli:test", user_peer_id="robert", assistant_peer_id="xiuqo",
...
def _provider_with_raw(raw, host="xiuqo"):
...
class TestSessionStartInjection:
    @pytest.mark.parametrize("raw, headings", [
        ...
        ({"injection": {"sessionStart": ["summary"]}, "hosts": {"xiuqo": {"injection": {"sessionStart": ["peerCard"]}}}},
         ["## User Peer Card"]),
    ...
    def test_desktop_panel_writes_the_pin_the_provider_reads(self, submitted, headings):
        from xiuqo_cli.web_routers.memory_providers import _apply_field_values
    ...
    def test_initialize_reads_the_pin(self):
        raw = {"injection": {"sessionStart": ["summary"]}}
        provider = TestDialecticCadenceDefaults._make_provider(cfg_extra={"raw": raw, "host": "xiuqo"})
...
class TestInjectionAuditLog:
    ...
    @pytest.mark.parametrize("raw, env", [
        ({}, None),
        ({"logging": True, "hosts": {"xiuqo": {"logging": False}}}, None),
