...
class TestA2aRouting:
    ...
    def test_bot_colliding_with_this_agents_ai_peer_is_skipped(self):
        ...
        provider = _provider()
        provider._manager.resolve_author_peer_id.return_value = "xiuqo"
        provider._manager.assistant_peer_id.return_value = "xiuqo"
    ...
    def test_same_named_senders_on_two_connections_get_two_sessions(self):
        ...
        assert provider._a2a_session_key(east).startswith("Bot-Chat:a2a:xiuqo-assistant:bot-east-coder-")
...
class TestManagerUserPeerOverride:
    def test_get_or_create_uses_the_override_as_user_peer(self):
        mgr = HonchoSessionManager(honcho=MagicMock(), config=HonchoClientConfig(api_key="k", peer_name="eri", ai_peer="xiuqo"),
        ...
        assert session.assistant_peer_id == "xiuqo"
