"""Tests for xiuqo_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Xiuqo entry point
(xiuqo, xiuqo, xiuqo-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Xiuqo entry point imports xiuqo_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported xiuqo_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("xiuqo_bootstrap", None)
    import xiuqo_bootstrap  # noqa: WPS433
    return xiuqo_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Xiuqo CLI
...
class TestEntryPointsImportBootstrap:
    """Every Xiuqo entry point must import xiuqo_bootstrap as its
    ...
    # Entry points that invoke Xiuqo as a process.  Each one must
    # import xiuqo_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "xiuqo_cli/main.py",   # xiuqo CLI (console_script)
        "run_agent.py",          # xiuqo (console_script)
        "acp_adapter/entry.py",  # xiuqo-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import xiuqo_bootstrap' and that
        ...
        half-finished ``xiuqo update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``xiuqo_bootstrap`` as a top-level module) leaves xiuqo
        ...
        """
        # Resolve relative to the xiuqo repo root.  Tests live
        # at tests/test_xiuqo_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # xiuqo start even when xiuqo_bootstrap hasn't been
        ...
        assert first_import_name == "xiuqo_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'xiuqo_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import xiuqo_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Xiuqo's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("XIUQO_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["XIUQO_PYTHON_SRC_ROOT"] = env
            elif "XIUQO_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["XIUQO_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/xiuqo")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("XIUQO_PYTHON_SRC_ROOT", None)
            else:
                os.environ["XIUQO_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/xiuqo", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Xiuqo root.  The guard must relocate Xiuqo to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/xiuqo"])
        assert result[0] == "/opt/xiuqo"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Xiuqo root.
        assert result.index("/opt/xiuqo") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["XIUQO_PYTHON_SRC_ROOT"] = "/env/xiuqo"
            hb.harden_import_path()
            assert sys.path[0] == "/env/xiuqo"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("XIUQO_PYTHON_SRC_ROOT", None)
