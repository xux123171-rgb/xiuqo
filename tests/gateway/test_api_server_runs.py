    ...
    async def test_start_returns_202(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_create_agent") as mock_create:
                ...
                assert status["object"] == "xiuqo.run"

    @pytest.mark.asyncio
    async def test_start_binds_chat_id_for_delegation_wake_target(self, adapter):
        """/v1/runs must bind the raw session id as the api_server chat_id
        (like every other agent-entry route does via _run_agent): the async
        delegation dispatch reads XIUQO_SESSION_CHAT_ID to pick its wake
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_create_agent") as create:
                ...
                first_headers = {
                    ...
                    "X-Xiuqo-Session-Key": "memory-a",
                }
                second_headers = {
                    "Authorization": "Bearer sk-secret",
                    "Idempotency-Key": "memory-scope",
                    "X-Xiuqo-Session-Key": "memory-b",
                }
                first = await cli.post(
    ...
    async def test_replay_bypasses_concurrency_limit_and_preserves_session_header(
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_create_agent") as create:
                ...
                assert replay.headers["X-Xiuqo-Session-Key"] == "memory-a"
...
class TestHostedRoomRuns:
    ...
    async def test_invitation_uses_validated_app_managed_local_catalog(
        ...
        monkeypatch.setenv(
            "XIUQO_ROOM_LINK_URL", "https://peer.example.test/xiuqo"
        )
        app = _create_runs_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
