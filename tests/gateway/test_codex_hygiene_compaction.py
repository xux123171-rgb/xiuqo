...
* ``compression.codex_app_server_auto`` semantics hold: only ``xiuqo``
  lets Xiuqo' threshold start a compaction; ``native``/``off`` skip
...
class LiveCodexAgent:
    ...
    def __init__(self, mode="xiuqo", session=None):
...
# ---------------------------------------------------------------------------
# Core no-op regression: xiuqo mode + live thread => thread/compact runs
# ---------------------------------------------------------------------------

def test_hermes_mode_compacts_live_thread_at_rpc_boundary(tmp_path):
    agent = LiveCodexAgent(mode="xiuqo")
    key = "tg:123"
...
@pytest.mark.parametrize("mode", ["native", "xiuqo", "off"])
...
def test_force_without_live_thread_does_not_run_local_compressor(mode):
    # The #73715 branch let force=True fall through to the local Xiuqo
...
def test_timeout_records_persistent_cooldown(tmp_path):
    ...
    agent = LiveCodexAgent(mode="xiuqo", session=HangingSession())
    key = "tg:123"
    gw, db = _gateway(tmp_path, key, agent)
...
# (profile secret scope / XIUQO_HOME override). A bare run_in_executor worker
...
def test_hygiene_worker_inherits_caller_contextvars(tmp_path):
    ...
    agent = ScopeProbeAgent(mode="xiuqo")
    key = "tg:ctx"
    ...
    async def _scoped():
        ...
        try:
            return await run_codex_hygiene_compaction(
                gw, key, agent.session_id, auto_mode="xiuqo",
