...
``xiuqo -p X`` dispatcher would build, and X's notifications are rendered/filtered under X's
config.
"""
...
def served(tmp_path, monkeypatch):
    ...
    (root / ".env").write_text("XIUQO_MODEL=default-model\nTERMINAL_ENV=docker\n")
    ...
    monkeypatch.setenv("XIUQO_MODEL", "default-model")
    monkeypatch.setenv("TERMINAL_ENV", "docker")
    monkeypatch.setenv("XIUQO_KANBAN_DB", str(tmp_path / "board.db"))
...
def test_worker_for_served_profile_gets_its_own_env_and_toolset_pin(served, monkeypatch):
    ...
    monkeypatch.setattr(kbd, "_hermes_argv", lambda: ["xiuqo"], raising=False)
    kbd._default_spawn(task, str(served.alpha), board=None)

    env = spawned["env"]
    assert env["XIUQO_HOME"] == str(served.alpha)
    assert "XIUQO_MODEL" not in env and "TERMINAL_ENV" not in env
