...
(XIUQO_HOME override), not the launch home the module constants were frozen from."""
...
def two_homes(tmp_path, monkeypatch):
    """Launch home A (XIUQO_HOME) and a routed profile B with different config everywhere."""
    a = tmp_path / ".xiuqo"
    b = a / "profiles" / "b"
...
async def test_gateway_hooks_fire_per_routed_profile(two_homes):
    ...
    try:
        ...
        reset_hermes_home_override(token)
    assert ctx_a.get("seen") == ["xiuqo_hook_hook-a"]
    assert ctx_b.get("seen") == ["xiuqo_hook_hook-b"]
