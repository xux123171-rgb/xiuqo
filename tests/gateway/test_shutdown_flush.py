...
def test_recover_closes_owned_db_when_unexpected_exception_escapes(
    ...
    db = InterruptingDB()
    monkeypatch.setattr("xiuqo_state_registry.acquire", lambda: db)
    monkeypatch.setattr(
        "xiuqo_state_registry.release_or_close", lambda _: setattr(db, "released", True)
    )
...
def test_get_flush_dir_uses_get_hermes_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "xiuqo_constants.get_hermes_home", fake_get_hermes_home
    )
