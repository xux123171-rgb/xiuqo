...
class TestApiServerSessionProfileBinding:
    """XIUQO_SESSION_PROFILE must be bound per /p/<profile>/ request.
    ...
    so every API-server turn bound XIUQO_SESSION_PROFILE="" and the
    ...
    def test_profile_is_bound_into_session_vars(self):
        ...
        try:
            assert get_session_env("XIUQO_SESSION_PROFILE") == "nm-media"
        finally:
