"""E2E: ``xiuqo peer dm`` against a REAL api_server gateway whose canonical
Bot Chat session is HIDDEN (issue #91583).

Two real XIUQO homes in spirit: the "peer" side is a real
:class:`APIServerAdapter` bound to a real loopback TCP socket over a real
SQLite ``state.db`` (its own tmp XIUQO_HOME) containing a hidden
``Bot Chat`` row — exactly what Bot Mode leaves behind. The "local" side is
the stock ``xiuqo peer dm`` client code (``xiuqo_cli.subcommands.peer``),
...
def peer_gateway(tmp_path, monkeypatch):
    """A real api_server gateway (own XIUQO_HOME + state.db) on a real socket."""
