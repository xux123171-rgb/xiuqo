...
def test_session_store_default_db_uses_runtime_hermes_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime XIUQO_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    xiuqo_state before a fixture redirected XIUQO_HOME used to pin every
    default SessionDB() at the developer's real ~/.xiuqo/state.db.
    """
    config = GatewayConfig()
    fake_home = tmp_path / "alt_hermes_home"
    fake_home.mkdir()
    monkeypatch.setenv("XIUQO_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "xiuqo sessions list" in raw["_README"]
