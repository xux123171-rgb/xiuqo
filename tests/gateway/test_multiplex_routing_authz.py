...
Every test builds a bare ``GatewayRunner`` with stub adapters against a temp ``XIUQO_HOME`` and
...
def mux(tmp_path, monkeypatch):
    ...
    with patch("xiuqo_cli.profiles.profiles_to_serve", return_value=served), \
            patch("xiuqo_cli.profiles.get_profile_dir", side_effect=lambda n: home / "profiles" / n), \
            patch("xiuqo_cli.profiles.profile_exists", return_value=True):
