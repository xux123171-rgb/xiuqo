...
class TestBuildSessionContextPrompt:
    ...
    def test_local_delivery_path_uses_display_hermes_home(self):
        ...
        with patch("xiuqo_constants.display_hermes_home", return_value="~/.xiuqo/profiles/coder"):
            prompt = build_session_context_prompt(ctx)

        assert "~/.xiuqo/profiles/coder/cron/output/" in prompt
...
class TestSessionStoreRewriteTranscript:
    ...
    def store(self, tmp_path, monkeypatch):
        import xiuqo_state
        monkeypatch.setattr(xiuqo_state, "DEFAULT_DB_PATH", tmp_path / "state.db")
        config = GatewayConfig()
...
class TestWhatsAppSessionKeyConsistency:
    ...
    def test_whatsapp_group_participant_aliases_share_session_key(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIUQO_HOME", str(tmp_home))
