...
from xiuqo_cli import kanban_db as kb, kanban_db_connect as kbc, kanban_db_notify as kbn
...
def test_route_denials_leave_events_retryable_at_claim_and_send(tmp_path, monkeypatch):
    ...
    from xiuqo_constants import clear_named_profile_deleted, mark_named_profile_deleted
    yuki_home = tmp_path / ".xiuqo" / "profiles" / "yuki"
    ...
    monkeypatch.setenv("XIUQO_KANBAN_DB", str(tmp_path / "tied-routes.db"))
    runner.config.profile_routes = parse_profile_routes([
        dict(platform="discord", guild_id="guild", chat_id="parent", profile="other"),
...
def test_removed_profile_never_wakes_under_the_primary_runtime(tmp_path, monkeypatch):
    ...
    rows = collect(runner)
    assert len(rows) == 1
    shutil.rmtree(tmp_path / ".xiuqo" / "profiles" / "yuki")
    asyncio.run(deliver(runner, rows))
