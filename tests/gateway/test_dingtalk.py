...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Xiuqo"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^xiuqo"]}
        )
        assert adapter._message_matches_mention_patterns("xiuqo please help") is True
        assert adapter._message_matches_mention_patterns("please xiuqo help") is False
