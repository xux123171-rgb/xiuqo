...
process that never runs the bootstrap can never have an identity. `cmd_chat` and `xiuqo serve` run it;
this file pins that `xiuqo gateway run` does too, and does it BEFORE any adapter connects, so a fast
...
def test_gateway_bootstrap_seam_calls_the_one_creator(monkeypatch):
    ...
    import xiuqo_cli.free_tier_bootstrap as ftb
    calls: list[dict] = []
