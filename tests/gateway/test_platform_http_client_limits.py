...
def _clear_env(monkeypatch):
    monkeypatch.delenv("XIUQO_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", raising=False)
    monkeypatch.delenv("XIUQO_GATEWAY_HTTPX_MAX_KEEPALIVE", raising=False)
...
def test_env_override_rejects_garbage(monkeypatch):
    ...
    monkeypatch.setenv("XIUQO_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", "not-a-number")
    monkeypatch.setenv("XIUQO_GATEWAY_HTTPX_MAX_KEEPALIVE", "-3")
