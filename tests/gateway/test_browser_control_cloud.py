...
from xiuqo_cli.dashboard_auth.ws_tickets import _reset_for_tests, mint_ticket
...
def test_cloud_agent_context_binds_registration_principal_and_transport_family():
    ...
    try:
        ...
        assert get_session_env("XIUQO_BROWSER_CONTROL_PRINCIPAL") == _principal_digest(
            identity
        )
        assert (
            get_session_env("XIUQO_BROWSER_CONTROL_TRANSPORT_FAMILY")
            == "cloud-ticket-ws"
        )
    finally:
