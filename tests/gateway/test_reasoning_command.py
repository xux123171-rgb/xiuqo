...
class TestReasoningCommand:
    ...
    def test_resolve_session_reasoning_prefers_session_override(self, tmp_path, monkeypatch):
        xiuqo_home = tmp_path / "xiuqo"
        xiuqo_home.mkdir()
        (xiuqo_home / "config.yaml").write_text("agent:\n  reasoning_effort: low\n", encoding="utf-8")
    ...
    def test_run_agent_includes_enabled_mcp_servers_in_gateway_toolsets(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_run, "_hermes_home", xiuqo_home)
        monkeypatch.setattr(gateway_run, "_env_path", xiuqo_home / ".env")
        monkeypatch.setattr(
            gateway_run,
...
class TestLoadShowReasoningCoercion:
    ...
    def _load_with_config(self, tmp_path, monkeypatch, yaml_body: str) -> bool:
        xiuqo_home = tmp_path / "xiuqo"
        xiuqo_home.mkdir()
        (xiuqo_home / "config.yaml").write_text(yaml_body, encoding="utf-8")
        monkeypatch.setattr(gateway_run, "_hermes_home", xiuqo_home)
