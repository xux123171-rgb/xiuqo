...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.xiuqo."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /xiuqo, /btw, /stop,
        ...
        for expected in ("/xiuqo", "/btw", "/stop", "/model", "/help"):
...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``xiuqo gateway run --replace`` takes the old process through the
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "xiuqo_feedback" in created_apps[0].registered_actions
        assert "xiuqo_clarify_other" in created_apps[0].registered_actions
        ...
        assert any(
            pattern.fullmatch("xiuqo_clarify_choice_0")
            for pattern in clarify_choice_patterns
        )
        assert not any(
            pattern.fullmatch("xiuqo_clarify_choice")
            for pattern in clarify_choice_patterns
        )
    ...
    async def test_channel_delivery_honors_unfurl_config(self):
        ...
        with patch.object(_slack_mod.aiohttp, "ClientSession", return_value=session):
            result = await _slack_mod._standalone_send(
                config,
                "C123",
                "[Xiuqo](https://example.com/xiuqo)",
            )

        assert result["success"] is True
        payload = session.post.call_args.kwargs["json"]
        assert payload["text"] == "<https://example.com/xiuqo|Xiuqo>"
...
class TestSlashCommands:
    ...
    # instead of as /xiuqo subcommands. This is the Discord/Telegram parity
    ...
    async def test_legacy_hermes_prefix_still_works(self, adapter):
        """Backward compat: /xiuqo btw foo must still route to /btw foo.

        Old workspace manifests only declared /xiuqo as the single slash.
...
class TestMessageSplitting:
    ...
    async def test_send_coerces_string_unfurl_options(self, adapter):
        """`xiuqo config set` / Railway persist YAML booleans as strings.
...
class TestMissingCredentials:
    ...
    async def test_missing_app_token_sets_fatal_error(self):
        ...
        assert "xiuqo gateway setup" in fatal_errors[0]["message"].lower() or ".env" in fatal_errors[0]["message"]
...
class TestThreadImageContext:
    ...
    def _prep(self, adapter_with_session_store):
        ...
        a._download_slack_file = AsyncMock(return_value="/tmp/xiuqo-cached.png")
        return a
    ...
    async def test_cold_start_delivers_thread_root_image(
        ...
        assert msg_event.media_urls == ["/tmp/xiuqo-cached.png"]
...
class TestSlackUserAgent:
    ...
    ``user_agent_prefix=_XIUQO_SLACK_USER_AGENT_PREFIX`` on every
    ...
    def test_hermes_slack_user_agent_prefix_format(self):
        """Module constant matches the XiuqoAgent/<version> convention used
        elsewhere in the codebase for platform-partner attribution."""
        assert _slack_mod._XIUQO_SLACK_USER_AGENT_PREFIX.startswith("XiuqoAgent/")
