...
async def test_reload_mcp_only_touches_requesting_profile(
    ...
    worker_scope = xiuqo_home_key(worker_home)
    ...
    monkeypatch.setattr(
        ...
        {"default-srv": xiuqo_home_key(tmp_path), "worker-srv": worker_scope},
    )
    seen: list[tuple] = []
...
async def test_reload_mcp_reports_a_shared_server_to_a_non_owner_profile(
    ...
    worker_home.mkdir(parents=True)
    worker_scope = xiuqo_home_key(worker_home)
    launch_scope = xiuqo_home_key(tmp_path / "default")
...
def test_scope_visibility_rejects_a_foreign_or_differently_authenticated_route(
    ...
    worker_scope = xiuqo_home_key(tmp_path / "worker")
    launch_scope = xiuqo_home_key(tmp_path / "default")
...
def test_shared_server_tools_are_callable_and_removed_on_non_owner_reload(
    ...
    worker_scope = xiuqo_home_key()
    launch_scope = xiuqo_home_key(launch_home)
...
def test_deregister_scope_kwarg_targets_overlay_and_keeps_plugin_confinement() -> None:
    ...
    reg._plugin_module_scopes["xiuqo_plugins.p"] = {"/home/p1"}
    reg._caller_module = staticmethod(lambda: "xiuqo_plugins.p")
