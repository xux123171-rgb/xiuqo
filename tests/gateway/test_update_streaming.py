...
Tests the new --gateway mode for xiuqo update, including:
...
def _make_runner(xiuqo_home=None):
...
class TestGatewayPrompt:
    ...
    def test_writes_prompt_file_and_reads_response(self, tmp_path):
        ...
        def write_response():
            time.sleep(0.2)
            (xiuqo_home / ".update_response").write_text("y")
        ...
        assert not (xiuqo_home / ".update_prompt.json").exists()
        assert not (xiuqo_home / ".update_response").exists()
...
class TestUpdateCommandGatewayFlag:
    """Verify the gateway spawns xiuqo update --gateway."""
...
class TestWatchUpdateProgress:
    ...
    async def test_streams_output_to_adapter(self, tmp_path):
        ...
        # Write output
        (xiuqo_home / ".update_output.txt").write_text("→ Fetching updates...\n", encoding="utf-8")
    ...
    async def test_detects_and_forwards_prompt(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        (xiuqo_home / ".update_pending.json").write_text(json.dumps(pending))
        (xiuqo_home / ".update_output.txt").write_text("output\n")
        ...
        async def simulate_prompt_cycle():
            ...
            (xiuqo_home / ".update_prompt.json").write_text(json.dumps(prompt))
            # Simulate user responding
            await asyncio.sleep(0.2)
            (xiuqo_home / ".update_response").write_text("y")
            (xiuqo_home / ".update_prompt.json").unlink(missing_ok=True)
            await asyncio.sleep(0.2)
            (xiuqo_home / ".update_exit_code").write_text("0")
    ...
    async def test_prompt_is_recovered_after_watcher_restart(self, tmp_path):
        ...
        with patch("gateway.run._hermes_home", xiuqo_home):
            ...
            assert (xiuqo_home / ".update_prompt.json").exists()
...
class TestUpdatePromptInterception:
    ...
    async def test_recognized_slash_command_bypasses_pending_update_prompt(self, tmp_path):
        ...
        (xiuqo_home / ".update_prompt.json").write_text(json.dumps({"prompt": "test"}))
        ...
        response_path = xiuqo_home / ".update_response"
