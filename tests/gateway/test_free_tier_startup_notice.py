...
def _account_state() -> dict:
    return {"auth_method": "oauth", "access_token": _jwt(client_id="xiuqo-cli", account_tier="pro"),
...
async def test_guest_inference_adds_exactly_one_free_tier_line(nous_runner):
    ...
    assert lines[0] == "♻️ Gateway online — Xiuqo is back and ready."
...
async def test_signed_in_account_keeps_the_plain_online_notice(nous_runner):
    ...
    assert message == "♻️ Gateway online — Xiuqo is back and ready."
