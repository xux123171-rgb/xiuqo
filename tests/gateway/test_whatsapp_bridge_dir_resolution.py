...
(/opt/xiuqo/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable XIUQO_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_hermes_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable XIUQO_HOME."""
    ...
    monkeypatch.setattr(
        "xiuqo_constants.get_hermes_home", lambda: xiuqo_home
    )
    ...
    expected = xiuqo_home / "scripts" / "whatsapp-bridge"
