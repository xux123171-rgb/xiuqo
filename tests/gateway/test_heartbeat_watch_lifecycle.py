...
from xiuqo_cli.heartbeat import HeartbeatManager, HeartbeatState, save_heartbeat, migrate_heartbeat_to_session
...
async def test_watches_read_and_admit_in_each_owner_profile(tmp_path, monkeypatch):
    ...
    for name in ('alpha', 'beta'):
        home = tmp_path / '.xiuqo' / 'profiles' / name
    ...
    with _profile_runtime_scope(tmp_path / '.xiuqo' / 'profiles' / 'alpha'):
        await runner._heartbeat_poll_once(watch)
    assert {row[0] for row in rows} == {'alpha', 'beta'}
    assert all(row[1] == str(tmp_path / '.xiuqo' / 'profiles' / row[0]) for row in rows)
    assert len(watch) == 2
