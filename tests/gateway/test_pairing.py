...
class TestProfileScopedDiscovery:
    def test_list_approved_scopes_platform_discovery_to_profile_dir(self, tmp_path):
        ...
        # A profile's store anchors to the xiuqo ROOT, not the current
        # XIUQO_HOME — the current home may itself be a profile, and nesting
        ...
        with patch("gateway.pairing.PAIRING_DIR", global_dir), patch(
            ...
            # consolidated layout a standalone `xiuqo -p alice` resolves —
...
class TestProfileScopedStorage:
    ...
    under each profile's own Xiuqo home so a multiplexing gateway can keep
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``xiuqo pairing`` CLI continues to work without a profile context."""
        from xiuqo_constants import get_hermes_home
        monkeypatch.setattr("xiuqo_constants.get_hermes_home", lambda: tmp_path)
    ...
    def test_default_store_is_not_frozen_at_first_use(self, tmp_path, monkeypatch):
        ...
        whatever XIUQO_HOME resolved to the first time this module's
        default path was computed. A long-lived process (the gateway,
        started once at container/process boot) can construct a
        PairingStore before XIUQO_HOME/profile context is fully
        ...
        monkeypatch.setattr("xiuqo_constants.get_hermes_home", lambda: first_home)
        first_store = PairingStore()
        assert first_store._dir == first_home / "platforms" / "pairing"

        monkeypatch.setattr("xiuqo_constants.get_hermes_home", lambda: second_home)
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        """Explicit profile stores use that profile's normal Xiuqo layout."""
        monkeypatch.setenv("XIUQO_HOME", str(tmp_path))
    ...
    def test_profile_store_matches_profile_cli_home(self, tmp_path, monkeypatch):
        """Gateway and ``xiuqo -p`` must resolve the same pairing store."""
