...
def test_sole_addressee_text_stays_clean_and_prompt_is_session_stable(trigger):
    ...
    async def run():
        ...
        if trigger == "dm":
            ...
            msg = _group_message("/new@xiuqo_bot", entities=[SimpleNamespace(type="bot_command", offset=0, length=15)])
        elif trigger == "text_mention":
            msg = _group_message("Xiuqo hello", entities=[SimpleNamespace(type="text_mention", offset=0, length=6, user=SimpleNamespace(id=999))])
            ...
            msg = _group_message("@xiuqo_bot", reply_to_bot=True, entities=[SimpleNamespace(type="code", offset=0, length=11)])
        else:
        ...
        assert len(events) == 2
        first, second = events
        expected_text = {"command": "/new", "mention": "😀 2", "code": "@xiuqo_bot"}.get(trigger, msg.text)
        assert first.text == expected_text
        if trigger == "dm":
            assert not first.channel_prompt
        else:
            assert "@xiuqo_bot" in first.channel_prompt
