...
closure the PR changed, against a real temp ``XIUQO_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "xiuqo_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr("xiuqo_constants.get_hermes_home", lambda: xiuqo_home)
    monkeypatch.setattr("xiuqo_cli.config.get_hermes_home", lambda: xiuqo_home)
    return cfg_path
