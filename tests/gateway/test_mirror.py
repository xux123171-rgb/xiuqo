...
class TestMirrorToSession:
    ...
    def test_failed_sqlite_write_reports_false(self, tmp_path):
        ...
        with patch.object(mirror_mod, "_SESSIONS_DIR", sessions_dir), \
             patch.object(mirror_mod, "_SESSIONS_INDEX", index_file), \
             patch("xiuqo_state_registry.acquire", return_value=broken_db), \
             patch("xiuqo_state_registry.release_or_close"):
...
class TestAppendToSqlite:
    def test_connection_is_released_after_use(self, tmp_path):
        ...
        with patch("xiuqo_state_registry.acquire", return_value=mock_db), \
             patch(
                 "xiuqo_state_registry.release_or_close",
