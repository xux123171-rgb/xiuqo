...
XIUQO_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env XIUQO_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``xiuqo config get terminal.backend`` still says local.
    """
    xiuqo_home = tmp_path / ".xiuqo"
    xiuqo_home.mkdir()
    (xiuqo_home / "config.yaml").write_text(
    ...
    (xiuqo_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
