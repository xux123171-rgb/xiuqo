...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/xiuqo/.overview.md") == "viking://user/xiuqo"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/xiuqo/memories/profile.md") == "viking://user/xiuqo/memories/profile.md"
...
class TestOpenVikingSkillQuerySafety:
    ...
    def test_skill_markers_match_hermes_scaffolding(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIUQO_BUNDLES_DIR", str(bundles_dir))
...
class TestOpenVikingConfigSchema:
    ...
    def test_recall_config_reads_from_config_yaml(self, monkeypatch, tmp_path):
        ...
        # Populate config.yaml in the temp XIUQO_HOME
        xiuqo_home = tmp_path / "xiuqo_test"
        xiuqo_home.mkdir(exist_ok=True)
        config_yaml = xiuqo_home / "config.yaml"
...
class TestOpenVikingRead:
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    (("uri", "viking://user/xiuqo"),),
                ): {"result": {"content": "overview text"}},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/xiuqo/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/xiuqo/.overview.md"
        assert result["resolved_uri"] == "viking://user/xiuqo"
    ...
    def test_read_accepts_uri_batch_and_caps_batch_full_content(self):
        provider = OpenVikingMemoryProvider()
        uris = [
            "viking://user/xiuqo/memories/a.md",
            "viking://user/xiuqo/memories/b.md",
            "viking://user/xiuqo/memories/c.md",
            "viking://user/xiuqo/memories/d.md",
        ]
        provider._client = FakeVikingClient(
            {
                (
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/xiuqo/.overview.md", "level": "overview"})
            assert False, "Expected summary endpoint error to be raised"
        except RuntimeError:
            pass

        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/xiuqo"}),
        ]


class TestOpenVikingAutoRecallPrefetch:
    @pytest.mark.parametrize("peer", ["", "xiuqo"])
    def test_prefetch_e2e_sends_limit_and_reads_l2_content(self, monkeypatch, peer):
                    ...
                    if payload.get("context_type") == "memory":
                        self._send_json({
                            "result": {
                                "memories": [
                                    {
                                        "uri": "viking://user/user/peers/xiuqo/memories/e2e-full.md",
                                        "score": 0.9,
                                        "level": 2,
                                        "category": "events",
        ...
        assert records["reads"] == ["viking://user/user/peers/xiuqo/memories/e2e-full.md"]
        ...
        assert all(
            headers.get("user-agent") == f"openviking-memory-xiuqo/{_XIUQO_VERSION}"
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/xiuqo/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/xiuqo/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/xiuqo"}))

        assert result["path"] == "viking://user/xiuqo"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/xiuqo/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/xiuqo/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/xiuqo"},
...
class TestEnsureClientReloadsEnv:
    ...
    def test_ensure_client_rebuilds_when_api_key_changes(self, monkeypatch):
        ...
        class _StubClient:
            def __init__(self, endpoint, api_key, account="", user="", agent="xiuqo"):
    ...
    def test_rebuilt_client_resolves_its_own_user_space(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENVIKING_AGENT", "xiuqo")
        ...
        assert alice_uri.startswith("viking://user/alice/peers/xiuqo/")
        assert bob_uri.startswith("viking://user/bob/peers/xiuqo/")
    ...
    def test_handle_tool_call_reconnects_after_startup_health_failure(self, monkeypatch):
        ...
        assert out["session_id"].startswith("xiuqo-remember-")
    ...
    def test_remember_uses_a_distinct_one_shot_session_for_each_call(self, monkeypatch):
        ...
        assert first["session_id"].startswith("xiuqo-remember-")
        assert second["session_id"].startswith("xiuqo-remember-")
    ...
    def test_remember_reports_commit_failure_with_recovery_command(self, monkeypatch):
        ...
        assert out["error"].startswith(
            "Memory message was accepted, but commit failed for session xiuqo-remember-"
        )
        ...
        assert out["session_uri"].endswith(f"/sessions/{out['session_id']}")
        assert out["recovery_command"] == f"ov session commit {out['session_id']}"
        assert "same OpenViking profile and credentials as Xiuqo" in out["recovery_note"]
        assert len(posts) == 2
        assert posts[0][0].endswith("/messages")
...
class TestUnavailableWarningsPromiseRetry:
    ...
    reads as "it never recovers" and sends people restarting xiuqo for
    nothing (#5721).
    """

    @staticmethod
    def _assert_promises_retry(message: str) -> None:
        assert "for this Xiuqo run" not in message, message
