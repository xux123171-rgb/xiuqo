...
``xiuqo_state_ids.new_session_id`` and the id it produces is what lost-and-found salvage classifies
...
import xiuqo_state_ids
from xiuqo_state_ids import SESSION_ID_PATTERN, new_session_id

# (module, callable(mint) -> str) for each minting site; ``mint`` is the patched helper.
SITES = [
    ("xiuqo_cli.foreign_sessions", None),
    ("agent.conversation_compression", None),
    ("xiuqo_cli.cli_commands_mixin", None),
    ("xiuqo_cli.cli_session_mixin", None),
...
def test_minted_ids_are_what_salvage_classifies_as_session_ids(hex_len, expected_re):
    from xiuqo_cli.session_lost_and_found import _is_session_id
    ...
    assert SESSION_ID_PATTERN is importlib.import_module("xiuqo_cli.session_lost_and_found").SESSION_ID_PATTERN
...
def test_every_minting_site_imports_the_one_helper(module_name, call):
    ...
    minted = [name for name in ("new_session_id", "mint_session_id")
              if getattr(mod, name, None) is xiuqo_state_ids.new_session_id]
    assert minted, f"{module_name} does not mint through xiuqo_state_ids.new_session_id"
    if call is not None:
