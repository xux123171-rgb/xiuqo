...
work on a file several Xiuqo processes share (gateway service, the Desktop
app's `xiuqo serve` backend, CLI sessions, the TUI slash worker):

* `xiuqo_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `xiuqo_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
