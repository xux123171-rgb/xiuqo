...
#88532 made SessionStore follow XIUQO_HOME; this covers the TUI/agent
...
def test_open_profile_session_db_does_not_fallback_on_open_failure(homes, monkeypatch):
    ...
    monkeypatch.setattr("xiuqo_state.SessionDB", boom_db)
...
def _stub_rebuild_env(monkeypatch, server, launch) -> list[str]:
    """Neutralize the rebuild's side paths; returns the XIUQO_HOME seen by each _make_agent call."""
    homes_seen: list[str] = []
...
def test_bot_capability_rebuild_stays_on_the_profile_store(homes, monkeypatch):
    ...
    ``_make_agent`` with neither the session's ``state.db`` handle nor its XIUQO_HOME, so the replacement
    agent bound the launch ``_get_db()`` handle and built its prompt/skills from the launch profile: every
    later turn of a named-profile bot appended to ``~/.xiuqo/state.db`` under the same session id while the
