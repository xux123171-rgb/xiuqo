...
from xiuqo_state_common import FTS_CJK_STALE_KEY
...
def db(cjk_so, tmp_path, monkeypatch):
    monkeypatch.setenv("XIUQO_FTS5_CJK_SO", str(cjk_so))
...
def test_config_toggle_disables_cjk(cjk_so, tmp_path, monkeypatch):
    monkeypatch.setenv("XIUQO_FTS5_CJK_SO", str(cjk_so))
    monkeypatch.setenv("XIUQO_CJK_FTS", "0")
    d = SessionDB(db_path=tmp_path / "state.db")
    try:
...
def test_existing_v23_db_gains_cjk_via_optimize(cjk_so, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XIUQO_FTS5_CJK_SO", str(tmp_path / "absent.so"))
...
def test_optimize_demote_leaves_established_cjk_index_intact(cjk_so, tmp_path, monkeypatch):
    ...
    from xiuqo_state_common import SCHEMA_SQL
    from xiuqo_state_fts import FTS_CJK_TABLE_SQL, FTS_CJK_TRIGGER_SQL
