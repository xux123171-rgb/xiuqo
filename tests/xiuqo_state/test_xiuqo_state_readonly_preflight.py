...
- files inside the Xiuqo home tree are repaired with ``chmod u+rw``
...
import xiuqo_state
from xiuqo_state import SessionDB, preflight_db_writability
...
def xiuqo_home(tmp_path, monkeypatch):
    """Isolated XIUQO_HOME so the repair scope covers tmp DBs."""
    home = tmp_path / ".xiuqo"
    home.mkdir()
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, xiuqo_home):
        db = xiuqo_home / "state.db"
        _make_db(db)
        os.chmod(db, 0o444)
    ...
    def test_repairs_readonly_sidecars(self, xiuqo_home):
        db = xiuqo_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, xiuqo_home):
        sub = xiuqo_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, xiuqo_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, xiuqo_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, xiuqo_home):
        db = xiuqo_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, xiuqo_home):
        db_path = xiuqo_home / "state.db"
