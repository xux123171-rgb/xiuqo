...
(SQLITE_PROTOCOL — typical on NFS/SMB), Xiuqo must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``XIUQO_HOME`` silently lose
...
from xiuqo_state import SessionDB, format_session_db_unavailable, get_last_init_error
from xiuqo_state_wal import WalUnsupportedError, apply_wal_with_fallback
...
def _reset_last_init_error():
    ...
    xiuqo_state._set_last_init_error(None)
    yield
    xiuqo_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    xiuqo_state_wal._wal_delete_fallback_failed_paths.clear()
    yield
    xiuqo_state_wal._wal_fallback_warned_paths.clear()
    xiuqo_state_wal._wal_delete_fallback_failed_paths.clear()
...
def _assume_fixed_sqlite(monkeypatch):
    ...
    xiuqo_state_wal._wal_reset_bug_warned_paths.clear()
    yield
    xiuqo_state_wal._wal_reset_bug_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_error_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see xiuqo_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("xiuqo_state.sqlite3.connect", side_effect=gated_connect):
            with pytest.raises(sqlite3.OperationalError):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_adds_nfs_hint_for_locking_protocol(self):
        ...
        xiuqo_state._set_last_init_error("OperationalError: locking protocol")
