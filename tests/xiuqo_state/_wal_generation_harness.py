...
def pin_wal(monkeypatch) -> None:
    ...
    monkeypatch.setattr(xiuqo_state_wal, "resolve_journal_mode", lambda: "wal")
...
def write_second_generation(db_path: Path, n_rows: int) -> int:
    ...
    (as any non-xiuqo opener would), write ``n_rows`` messages and checkpoint them into the main file.
...
_GATEWAY_CHILD = textwrap.dedent(
    ...
    from pathlib import Path
    repo, xiuqo_home, db_path = sys.argv[1], sys.argv[2], sys.argv[3]
    sys.path.insert(0, repo)
    os.environ["XIUQO_HOME"] = xiuqo_home
    import xiuqo_state_wal
    if xiuqo_state_wal.is_sqlite_wal_reset_vulnerable():
        xiuqo_state_wal.is_sqlite_wal_reset_vulnerable = lambda version_info=None: False
    xiuqo_state_wal.resolve_journal_mode = lambda: "wal"
    from xiuqo_state import DeletedWalGenerationError, SessionDB
...
def gateway_writer(tmp_path: Path):
    ...
    repo_root = os.path.dirname(os.path.abspath(xiuqo_state.__file__))
    ...
    with stderr_path.open("w", encoding="utf-8") as stderr:
        proc = subprocess.Popen(
            [sys.executable, "-c", _GATEWAY_CHILD, repo_root, str(xiuqo_home), str(path)],
            stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=stderr,
            text=True, encoding="utf-8", bufsize=1,
            env={**os.environ, "XIUQO_STATE_DB_GUARD_BYPASS": "1"},
        )
