"""POSIX advisory locks must survive Xiuqo' own database inspection.
...
Xiuqo used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `xiuqo sessions
...
out across Xiuqo' inspection calls.
"""
...
from xiuqo_cli.sqlite_safe_read import (
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import xiuqo_cli.sqlite_safe_read as ssr
