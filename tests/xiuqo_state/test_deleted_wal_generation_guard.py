...
from xiuqo_state_dbfile import _pread_db_header, iter_deleted_sqlite_sidecar_holders
from tests.xiuqo_state._wal_generation_harness import (
...
def test_iter_holders_empty_on_non_linux(monkeypatch, tmp_path):
    monkeypatch.setattr(xiuqo_state.sys, "platform", "win32")
...
def test_iter_holders_ignores_live_unhashed_dentry(tmp_path, force_wal, monkeypatch):
    ...
    monkeypatch.setattr(xiuqo_state_dbfile.os, "readlink", fake_readlink)
    try:
...
def test_write_path_ignores_live_unhashed_dentry(tmp_path, force_wal, monkeypatch):
    ...
    monkeypatch.setattr(xiuqo_state_readpool.os, "readlink", fake_readlink)
    try:
