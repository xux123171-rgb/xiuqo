...
The defect: xiuqo_state already knows macOS `fsync()` does not guarantee
...
import xiuqo_state
from xiuqo_state import repair_state_db_schema
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in xiuqo_state_repair; xiuqo_state only
    # re-imports them.
    import xiuqo_state_repair

    source = Path(xiuqo_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(xiuqo_state_repair.__file__))
