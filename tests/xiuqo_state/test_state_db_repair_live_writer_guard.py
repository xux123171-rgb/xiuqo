...
def test_repair_refuses_while_another_connection_holds_the_db(tmp_path):
    ...
    (``xiuqo_state_holders.live_writer_holds_db``) first scans for foreign
    holders — deleted sidecar generations, uninspectable Xiuqo processes —
    ...
    SQLite builds carrying the WAL-reset bug (and on NFS/SMB) Xiuqo runs
...
def test_repair_checks_foreign_holders_before_opening_sqlite(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(xiuqo_state_repair, "_connect_repair_durable", _unexpected_probe)
...
def test_linux_holder_scan_does_not_require_psutil(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(xiuqo_state_holders, "psutil", None)

    holders = xiuqo_state_holders.foreign_state_db_holders(
...
def test_incomplete_holder_scan_keeps_unknown_sentinel(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(xiuqo_state_holders.os, "readlink", lambda _path: str(db))
    real_stat = xiuqo_state_holders.os.stat
    ...
    holders = xiuqo_state_holders.foreign_state_db_holders(db)
...
def test_uninspectable_unknown_descriptor_uses_hermes_identity_at_repair_boundary(
    ...
    if should_block:
        ...
        def _unexpected_probe(*_args, **_kwargs):
            pytest.fail("repair opened SQLite with an unproven Xiuqo descriptor")

        monkeypatch.setattr(
            xiuqo_state_repair, "_connect_repair_durable",
            _unexpected_probe,
        )
        report = repair_state_db_schema(db, backup=False)
        ...
        real_connect = xiuqo_state_repair._connect_repair_durable
        probe_reached = False
...
def test_uninspectable_watched_identity_blocks_alias_before_sqlite(
    ...
    monkeypatch.setattr(xiuqo_state_holders.os, "listdir", _listdir)
    monkeypatch.setattr(
        xiuqo_state_holders.os, "readlink", lambda _path: str(alias)
    )
    monkeypatch.setattr(xiuqo_state_holders.os, "stat", _stat)
...
def test_uninspectable_alias_descriptor_for_hermes_blocks_before_sqlite(
    ...
    """Xiuqo cannot make an aliased fd safe when its identity is unreadable."""
    db = _make_wal_db(tmp_path)
    alias = tmp_path / "namespace-alias" / "state.db"
    ...
    monkeypatch.setattr(
        xiuqo_state_holders,
        "_read_proc_argv",
        lambda _pid: ["python3", "-m", "xiuqo_cli.main", "gateway"],
    )

    def _unexpected_probe(*_args, **_kwargs):
        pytest.fail("repair opened SQLite with an unproven Xiuqo alias fd")
