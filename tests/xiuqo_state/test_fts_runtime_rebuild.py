...
import xiuqo_state
import xiuqo_state_holders
import xiuqo_state_schema
from xiuqo_state import SessionDB
from xiuqo_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from xiuqo_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m xiuqo_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        "argv",
        (
            ("journalctl", "-u", "xiuqo.service"),
            ("grep", "xiuqo", "/var/log/syslog"),
            (
                "/usr/sbin/tailscaled",
                "be-child",
                "ssh",
                "--cmd=python -m xiuqo_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/xiuqo/.venv/bin/xiuqo gateway"),
            ("python3", "/opt/xiuqo/tools/check_state.py"),
            ("xiuqo-monitor", "gateway"),
            ("hermesctl", "serve"),
            ("python3", "worker.py", "xiuqo_cli.main"),
            ("python3", "-m", "other.module", "xiuqo_cli.main"),
            ("python3", "-c", "xiuqo_cli.main"),
            ("python3", "-Icprint('xiuqo_cli.main')", "xiuqo_cli/main.py"),
        ),
    )
    def test_uninspectable_non_hermes_process_is_not_a_holder(self, argv):
        assert not xiuqo_state_holders._looks_like_hermes(argv)

    @pytest.mark.parametrize(
        "argv",
        (
            ("/usr/local/bin/xiuqo", "gateway"),
            ("/usr/local/bin/xiuqo", "serve"),
            ("/usr/local/bin/xiuqo-acp", "--stdio"),
            ("/usr/bin/python3", "-m", "xiuqo_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "xiuqo_cli.main", "gateway"),
            ("/usr/bin/python3", "-mxiuqo_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "xiuqo_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "xiuqo_cli.main"),
            (
                "/opt/xiuqo/.venv/bin/python",
                "/opt/xiuqo/xiuqo_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "xiuqo_cli/main.py", "gateway"),
            ("python3", "/opt/xiuqo/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_hermes_process_remains_a_holder(self, argv):
        assert xiuqo_state_holders._looks_like_hermes(argv)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(xiuqo_state_holders.os, "getpid", lambda: 111)
        real_listdir = os.listdir
        def _listdir(path):
            if isinstance(path, str):
                path = path.replace("/proc", str(proc_root))
            return real_listdir(path)
        monkeypatch.setattr(xiuqo_state_holders.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(xiuqo_state_holders.os, "readlink", _readlink)
        real_stat = os.stat
        ...
        monkeypatch.setattr(xiuqo_state_holders.os, "stat", _stat)

        holders = xiuqo_state_holders.foreign_state_db_holders(db_path)
    ...
    def test_foreign_holder_ignores_same_path_with_different_file_identity(
        ...
        assert xiuqo_state_holders.foreign_state_db_holders(db_path) == []
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a Xiuqo process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like Xiuqo
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(
            b"python3\x00-m\x00xiuqo_cli.main\x00chat\x00"
        )
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(xiuqo_state_holders, "_read_proc_argv", _fake_argv)

        holders = xiuqo_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "xiuqo_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from xiuqo_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(xiuqo_state_schema.time, "time", lambda: 120.0)
    ...
    def test_same_holder_set_across_futile_window_names_holder_and_gateway_safe_remedy(
        ...
        monkeypatch.setattr(
            xiuqo_state_schema, "_read_proc_argv", lambda pid: ["python", "-m", "xiuqo_cli.main", "serve"], raising=False,
        )
        clock = [1000.0]
        monkeypatch.setattr(xiuqo_state_schema.time, "time", lambda: clock[0])

        from xiuqo_cli.doctor_state import _render_state_db_stats
        from xiuqo_state_dbfile import collect_state_db_stats
        ...
        futile_attempts = getattr(xiuqo_state_schema, "_FTS_HOLDER_FUTILE_ATTEMPTS", 10)
        futile_seconds = getattr(xiuqo_state_schema, "_FTS_HOLDER_FUTILE_SECONDS", 1800.0)
        reopened = SessionDB(db_path=db_path)
        try:
            ...
            assert "pid 4242: python -m xiuqo_cli.main serve" in msg
    ...
    def test_retry_backoff_resets_when_the_blocking_holder_set_changes(
        ...
        try:
            ...
            reopened._fts_stale_retry_after = time.monotonic() + xiuqo_state_schema._FTS_STALE_RETRY_MAX_SECONDS
            reopened._fts_stale_retry_interval = xiuqo_state_schema._FTS_STALE_RETRY_MAX_SECONDS
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="xiuqo_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:
