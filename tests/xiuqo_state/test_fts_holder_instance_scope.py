...
Xiuqo instances: a main gateway (user ``ubuntu``,
XIUQO_HOME=/home/ubuntu/.xiuqo) and a demo gateway (user ``demo``,
XIUQO_HOME=/home/demo/.xiuqo).  The demo gateway runs as another user, so
...
The demo process's argv matches the Xiuqo patterns exactly, so the fallback
...
rebuild in ``xiuqo_state_schema._recover_stale_fts`` was deferred 42 times
...
xiuqo); a genuine second instance with a DIFFERENT XIUQO_HOME is still
misjudged on current main (issue #92401).

Behavior contract: an uninspectable holder identified only by argv must be
counted unless its own argv proves it is scoped to a *different* Xiuqo
...
# token lives under /home/demo/.xiuqo, the binary name matches the Xiuqo
# patterns, and nothing references the main instance's home or state.db.
DEMO_HOME_ARGV = [
    "/home/demo/.xiuqo/xiuqo/xiuqo",
    "gateway",
    "run",
]

# Same, spelled through the venv interpreter + xiuqo launcher script.
DEMO_VENV_ARGV = [
    "/home/demo/.xiuqo/xiuqo/venv/bin/python",
    "/home/demo/.xiuqo/xiuqo/xiuqo_cli/main.py",
    "gateway",
]

# A Xiuqo-shaped argv with no absolute paths: cannot disprove that this
# process touches our state.db, so it must stay fail-closed.
AMBIGUOUS_ARGV = ["xiuqo", "gateway", "run"]
...
class TestUninspectableHolderInstanceScope:
    def test_other_instance_argv_is_not_a_holder_of_our_db(self, tmp_path, monkeypatch):
        ...
        DIFFERENT Xiuqo home → not a holder of our state.db."""
    ...
    def test_argv_referencing_our_db_stays_flagged(self, tmp_path, monkeypatch):
        ...
        for argv in (
            ["xiuqo", f"--db={db_path}", "gateway"],
            ["xiuqo", "checkpoint", f"{db_path}-wal"],
            ["xiuqo", "--home", our_home, "gateway"],
        ):
            assert xiuqo_state_holders._looks_like_hermes(argv) or argv[0] == "xiuqo"
            _install_fake_proc(monkeypatch, tmp_path, unreadable_pids=(222,))
