...
def test_required_frontmatter_fields(p):
    ...
    xiuqo = (fm.get("metadata") or {}).get("xiuqo") or {}
    if not (xiuqo.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.xiuqo.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    xiuqo = (fm.get("metadata") or {}).get("xiuqo") or {}
    dangling = [
        rs for rs in (xiuqo.get("related_skills") or []) if rs not in _all_names()
    ]
