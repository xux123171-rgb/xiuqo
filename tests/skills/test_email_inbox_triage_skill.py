...
def test_frontmatter_required_fields():
    ...
    xiuqo = fm["metadata"]["xiuqo"]
    assert xiuqo["tags"]
    assert "related_skills" in xiuqo
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Xiuqo Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["xiuqo"]["related_skills"]:
        hits = (
