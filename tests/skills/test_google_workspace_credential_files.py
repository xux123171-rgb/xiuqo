...
class TestGoogleWorkspaceCredentialFiles:
    ...
    def test_entries_are_registered_when_files_exist(self, tmp_path):
        ...
        xiuqo_home.mkdir()
        (xiuqo_home / "google_token.json").write_text("{}")
        (xiuqo_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            assert "/root/.xiuqo/google_token.json" in container_paths
            assert "/root/.xiuqo/google_client_secret.json" in container_paths
        finally:
            clear_credential_files()
