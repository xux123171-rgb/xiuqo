...
Build the real image and verify the actual runtime behavior:
  ...
  3. --user <xiuqo-uid> (10000) is allowed (supported non-root start)
"""
...
def test_arbitrary_user_uid_rejected(
    ...
    assert "XIUQO_UID" in combined or "PUID" in combined, (
...
def test_user_pinned_to_hermes_uid_works(
    ...
    """docker run --user 10000:10000 (the xiuqo UID) must be allowed.
    ...
    assert r.returncode == 0, (
        f"--user 10000:10000 (xiuqo UID) was rejected: {r.stderr[-500:]}"
    )
    assert "OK" in r.stdout