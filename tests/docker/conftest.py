...
Override the image with ``XIUQO_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("XIUQO_TEST_IMAGE", "xiuqo-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``XIUQO_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("XIUQO_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged xiuqo user
# ---------------------------------------------------------------------------
#
# Background: every Xiuqo runtime path inside the container drops to UID
# 10000 (the ``xiuqo`` user) via ``s6-setuidgid xiuqo``. ``docker exec``
...
# fails (PermissionError swallowed) for xiuqo, so a test that ran as root
...
# helpers below run every probe under ``-u xiuqo`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: xiuqo).
