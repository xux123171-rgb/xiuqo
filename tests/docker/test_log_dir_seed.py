"""Runtime smoke test for Docker $XIUQO_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the xiuqo user after container boot.
...
registered later runs its log service as the dropped xiuqo user and
...
def test_logs_gateways_seeded_and_hermes_owned(
    ...
    """logs/ and logs/gateways/ must exist and be owned by xiuqo after boot."""
    ...
    assert "logs=xiuqo" in r.stdout, (
        f"logs/ not owned by xiuqo: {r.stdout}"
    )
    assert "gateways=xiuqo" in r.stdout, (
        f"logs/gateways/ not owned by xiuqo: {r.stdout}"
    )
