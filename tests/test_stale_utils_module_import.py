...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.xiuqo/xiuqo/utils.py)
...
2. ``xiuqo update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``xiuqo_cli/main.py`` (the ``xiuqo update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``xiuqo_constants`` after the
