...
Maintainer catch (#95681 arc): on Windows with a custom XIUQO_HOME under
the user profile (e.g. AppData/Local/xiuqo), ``"~/" +
str(home.relative_to(Path.home()))`` produced the mixed-separator chimera
``~/AppData\\Local\\xiuqo`` — which then leaked into every consumer that
appends sub-paths (the skill_manage schema showed the agent
``~/AppData\\Local\\xiuqo/skills/``). The ``~/`` shorthand implies POSIX
...
class TestDisplayHermesHomePosix(unittest.TestCase):
    ...
    def test_default_home_unchanged(self):
        ...
        with patch.object(hc, "get_hermes_home",
                          return_value=Path.home() / ".xiuqo"):
            out = hc.display_hermes_home()
        self.assertEqual(out, "~/.xiuqo")

    def test_outside_home_falls_back_to_absolute(self):
        import xiuqo_constants as hc

        outside = Path("/opt/xiuqo-custom") if os.name != "nt" else Path("C:/opt/xiuqo-custom")
