...
def _assert_path_preserved(env):
    """Original PATH entries survive sanitization; the xiuqo console-script
    ...
    xiuqo_bin = _resolve_hermes_bin_dir()
    if xiuqo_bin and path_val != "/usr/bin:/bin":
        assert path_val.startswith(xiuqo_bin + os.pathsep), path_val
...
def test_resolve_mcp_invocation_sanitizes_env(monkeypatch):
    ...
    monkeypatch.delenv("XIUQO_CUA_TELEMETRY", raising=False)
