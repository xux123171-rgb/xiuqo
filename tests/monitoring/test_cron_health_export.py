...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("xiuqo.gateway.up"), _M("xiuqo.gateway.active_agents"),
        _M("xiuqo.gateway.busy"), _M("xiuqo.gateway.drainable"),
        _M("xiuqo.gateway.restart_requested"),
        _M("xiuqo.platform.up"), _M("xiuqo.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("xiuqo.cron.scheduler.heartbeat_age_seconds"),
        _M("xiuqo.cron.scheduler.last_success_age_seconds"),
        _M("xiuqo.cron.scheduler.catch_up_occurrences"),
        _M("xiuqo.cron.jobs.enabled"), _M("xiuqo.cron.jobs.running"),
        _M("xiuqo.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Xiuqo Agent-owned Relay transport health" in text
