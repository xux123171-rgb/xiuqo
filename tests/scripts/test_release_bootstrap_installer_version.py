...
update_version_files / the --publish --bump git add list, so Xiuqo-Setup.dmg
...
def _patch_repo(tmp_path, monkeypatch, *, with_installer: bool = True):
    repo = tmp_path
    init_py = repo / "xiuqo_cli" / "__init__.py"
    ...
    if with_installer:
        ...
        tauri_conf.write_text(
            '{"productName":"Xiuqo","version":"0.0.1"}\n', encoding="utf-8"
        )
...
def test_update_version_files_does_not_invent_version_keys(tmp_path, monkeypatch):
    paths = _patch_repo(tmp_path, monkeypatch)
    original = '{"name":"x","productName":"Xiuqo"}\n'
