...
last of them to close it. ``xiuqo update`` deliberately runs build steps with
stdout inherited (the tee-stderr runner in ``xiuqo_cli/main.py``), so the
...
``.xiuqo-update-result.json`` is never written, ``.xiuqo-update-in-progress``
is never cleared, and the Desktop is never relaunched -- so the app sits on
"Updating Xiuqo" until the user kills the gateway by hand, and the stale marker
...
step writing to both pipes took 18.3s vs 0.29s. ``xiuqo update`` is exactly this
...
class TestIdleWatchdogCountsUpdateLogGrowth:
    ...
    Real updates are stdout-silent for 40+ minutes: ``xiuqo update`` captures
    the (very loud) Electron/vite build into ``logs/update.log`` — NOT the
    child's stdout (``xiuqo_cli/update_cmd.py``, the update-log tee) — so the
    ...
    the ``XIUQO_UPDATE_STEP_IDLE_SECONDS`` override each fails a test here.
    """
...
def test_update_step_survives_pipe_leak_flood_and_live_child_stall(
    ...
    *logstall* -- a step that is silent on its pipes but appends to the
    ...
    This is the shape of every real ``xiuqo update`` build: output streams to
    ...
    env = {
        **os.environ,
        ...
        "XIUQO_UPDATE_PIPE_DRAIN_SECONDS": "3",
        "XIUQO_UPDATE_STEP_IDLE_SECONDS": "3",
        "XIUQO_SELFTEST_HOLD_SECONDS": "45",
    }
    ...
    assert "PIPE-DRAIN SELF-TEST: PASS" in result.stdout, (
        ...
        "Xiuqo' forever) or metered a chatty step (backpressure on the running "
