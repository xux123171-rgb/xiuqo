...
`scripts/desktop-update/windows.ps1` drives `xiuqo update` for the in-app
Desktop updater. It used to invoke the update through the venv's
`venv\\Scripts\\xiuqo.exe` console-script launcher. On Windows that launcher is
a real process that keeps `xiuqo.exe` mapped as its running image and spawns
`python.exe` as a child. The update ends in `uv pip install -e .`, which rewrites
the console-script shims -- including the `xiuqo.exe` the launcher still has
...
Start Menu shortcut points at a `Xiuqo.exe` that no longer exists.

Driving the update as `python.exe -m xiuqo_cli.main update` puts the inherited
...
`$hermesExe` shim. `xiuqo.exe` may still be *named* in the file for the
...
def test_invoke_hermes_step_calls_drive_python_not_the_shim() -> None:
    ...
    assert not offenders, (
        ...
        "must drive $pythonExe, not the xiuqo.exe shim. Driving the update "
        "through the shim keeps xiuqo.exe mapped as a running image, so uv's "
...
def test_update_invocation_uses_module_entrypoint() -> None:
    ...
    assert '@("-m", "xiuqo_cli.main", "update"' in source, (
        "The update step must invoke `python.exe -m xiuqo_cli.main update ...` "
        "so the inherited image handle lands on python.exe, which uv never has "
        "to replace."
    )
    assert (
        '@("-m", "xiuqo_cli.main", "desktop", "--force-build", "--build-only")'
        in source
    ), (
        "The desktop rebuild step must also go through "
        "`python.exe -m xiuqo_cli.main desktop ...` for the same reason."
    )
...
def test_update_no_longer_invokes_the_hermes_exe_shim() -> None:
    ...
    assert "Invoke-HermesStep $hermesExe" not in source, (
        ...
        "the xiuqo.exe shim (`Invoke-HermesStep $hermesExe`). That is the "
