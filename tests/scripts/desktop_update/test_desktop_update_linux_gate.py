...
``--install-root`` keeps whatever spelling the app's Xiuqo root had (a symlinked
``~/.xiuqo/xiuqo``, or ``/home`` on Fedora/ostree where it is a link to
...
def test_symlinked_spelling_on_either_side_still_relaunches(tmp_path, root_spelling, target_spelling):
    ...
    target = tmp_path / target_spelling / "apps" / "desktop" / "release" / "linux-unpacked" / "Xiuqo"
...
def test_target_outside_the_checkout_is_still_skew(tmp_path):
    ...
    (tmp_path / "link").symlink_to(real)
    foreign = tmp_path / "opt" / "Xiuqo"
    foreign.mkdir(parents=True)
    (foreign / "xiuqo").touch()
    assert _gate(tmp_path / "link", foreign / "xiuqo") == "skew"
    # A sibling directory sharing the prefix must not be mistaken for the checkout either.
    assert _gate(tmp_path / "link", Path(str(unpacked) + "-evil") / "Xiuqo") == "skew"
