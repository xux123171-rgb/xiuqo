...
def test_shim_removes_only_its_owned_profile(tmp_path, outcome):
    ...
    install = tmp_path / "xiuqo"
    install.mkdir()
    env = {**os.environ, "HOME": str(tmp_path), "XIUQO_HOME": str(tmp_path),
           "XDG_CONFIG_HOME": str(config), "TMPDIR": str(tmp_path),
           "PATH": f"{bin_dir}:/usr/bin:/bin", "XIUQO_SELFTEST_HOLD_SECONDS": "1",
           "XIUQO_UPDATE_SHIM_GRACE_SECONDS": "1", "XIUQO_SELFTEST_FAIL": "1" if outcome == "error" else ""}
    ...
    collision = tmp_path / f"xiuqo-update-ui-{process.pid}"
    collision.mkdir()
