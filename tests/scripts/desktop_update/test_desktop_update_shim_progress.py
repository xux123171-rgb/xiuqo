...
def progress(tmp_path):
    ...
    status = tmp_path / "xiuqo-update-status"
...
# Stands in for `xiuqo update`, and reports the stage that was on screen
...
n="$(cat "$XIUQO_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$XIUQO_TEST_CALLS"
for f in "$TMPDIR"/xiuqo-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$XIUQO_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$XIUQO_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "xiuqo"
    (install_root / "venv" / "bin").mkdir(parents=True)
    xiuqo = install_root / "venv" / "bin" / "xiuqo"
    xiuqo.write_text(FAKE_HERMES)
    xiuqo.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "XIUQO_TEST_CAPTURE": str(capture),
        "XIUQO_TEST_CALLS": str(calls),
        "XIUQO_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".xiuqo-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`xiuqo update` is the longest wait in the hand-off and the one the
