...
The ``xiuqo update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m xiuqo_cli.main gateway run`` — image name
    ``pythonw``, not ``xiuqo.exe`` — so the ``taskkill /IM xiuqo.exe`` guard
    ...
    # The xiuqo.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM xiuqo.exe' in text
