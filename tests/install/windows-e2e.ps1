...
# ============================================================================
# Proves, on a real Windows machine, that a user who installs Xiuqo the way
...
# actually touches:
#
#   INSTALL   - downloads the production Xiuqo-Setup.exe from the website,
#               launches it HEADED, and AutoHotkey clicks Install, waits,
#               then clicks Launch. The real Electron Xiuqo.exe must appear.
...
#   installer's `git clone` and `xiuqo update`'s `git fetch origin`
...
param(
    ...
    # open-app-update (from a desktop-installer install) and xiuqo-update /
    ...
    # they land.
    [ValidateSet("open-app-update", "xiuqo-desktop-app-update", "xiuqo-update", "desktop-installer@latest", "installer-script", "installer-script+desktop")]
    [string]$Route = "open-app-update",

    # The OLD version: the ref served as `main` while the installer runs,
    # i.e. what the user starts on. The published Xiuqo-Setup.exe carries
    ...
    [string]$WorkRoot = $(if ($env:XIUQO_E2E_WORKROOT) { $env:XIUQO_E2E_WORKROOT } else { Join-Path $env:TEMP "xiuqo-desktop-gui-e2e" }),

    [string]$SetupExeUrl = "https://xiuqo-assets.nousresearch.com/Xiuqo-Setup.exe",
...
$HermesHome  = Join-Path $WorkRoot "xiuqo-home"
$InstallDir  = Join-Path $HermesHome "xiuqo"
...
function Invoke-Git([string[]]$GitArgs) {
    ...
    # if xiuqo ever adds a git command that calls something with ^ this will break, lol.
...
function Set-GitRedirect {
    # we redirect to our own repo so we can play around with what commit xiuqo thinks we're on.
...
function Get-DesktopExe {
    foreach ($c in @(
        (Join-Path $InstallDir "apps\desktop\release\win-unpacked\Xiuqo.exe"),
        (Join-Path $InstallDir "apps\desktop\release\win-arm64-unpacked\Xiuqo.exe")
    )) {
...
function Save-InstallSideState([string]$Label) {
    ...
    if (Test-Path -LiteralPath $logsDir) {
        Copy-Item $logsDir (Join-Path $dest "xiuqo-logs") -Recurse -Force -ErrorAction SilentlyContinue
    }
    $resultFile = Join-Path $HermesHome ".xiuqo-update-result.json"
    ...
    Get-ChildItem -LiteralPath $HermesHome -ErrorAction SilentlyContinue |
        Select-Object Name, Length, LastWriteTime |
        Format-Table -AutoSize | Out-String |
        Set-Content (Join-Path $dest "xiuqo-home-ls.txt")
}

function Test-HermesRuns([string]$Label) {
    Save-InstallSideState $Label
    $hermesExe = Join-Path $InstallDir "venv\Scripts\xiuqo.exe"
    Assert-True (Test-Path -LiteralPath $hermesExe) "$Label -- venv\Scripts\xiuqo.exe exists"
    & $hermesExe --version 2>&1 | ForEach-Object { Write-Host "    xiuqo --version| $_" }
    Assert-True ($LASTEXITCODE -eq 0) "$Label -- xiuqo --version exits 0"
}
...
function Invoke-HermesUpdate {
    ...
    Write-LogGroup "xiuqo update transcript" $log
    Assert-True ($updateExit -eq 0) "xiuqo update exited $updateExit (expected 0)"
}

function Invoke-HermesDesktopAppUpdate([string]$TargetSha) {
    # The xiuqo-desktop launch surface: `xiuqo desktop` runs its whole
    ...
    $prevCap = $env:XIUQO_E2E_CAPTURE_LAUNCH
    $env:PYTHONPATH = if ($prevPy) { "$capDir;$prevPy" } else { $capDir }
    $env:XIUQO_E2E_CAPTURE_LAUNCH = $spec
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    Push-Location $InstallDir
    try {
        ...
        $env:XIUQO_E2E_CAPTURE_LAUNCH = $prevCap
    }
    Write-LogGroup "xiuqo desktop (launch capture) transcript" $log
    Assert-True ($capExit -eq 0) "xiuqo desktop exited 0 during launch capture"
    ...
    try {
        & $node "launch-from-spec.mjs" --spec $spec `
            --result (Join-Path $HermesHome ".xiuqo-update-result.json") `
    ...
    Assert-True ($driveExit -eq 0) "app driven via captured xiuqo desktop spec; update completed"
}
...
function Stop-HermesAppProcesses([string]$Label) {
    ...
    # Only Xiuqo.exe (Electron) -- never xiuqo.exe (the venv CLI shim).
    $procs = @(Get-Process -Name "Xiuqo" -ErrorAction SilentlyContinue)
    foreach ($p in $procs) {
        try { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue } catch {}
    }
    if ($procs.Count -gt 0) {
        Write-Host "  [$Label] stopped $($procs.Count) Xiuqo.exe process(es)"
        Start-Sleep -Seconds 3
    }
}

function Get-ManagedNode {
    # `xiuqo update`/desktop builds use the Xiuqo-managed Node; use the same
...
# ----------------------------------------------------------------------------
# Phase: install-gui -- website Xiuqo-Setup.exe, headed, AHK-driven
# ----------------------------------------------------------------------------
function Invoke-PhaseInstallGui {
    ...
    Write-Step "$($Mode.ToUpper()) (GUI): Xiuqo-Setup.exe from the website, headed, AHK clicks"
    ...
    $setupExe = Join-Path $WorkRoot "Xiuqo-Setup.exe"
    if (-not (Test-Path -LiteralPath $setupExe)) {
        Write-Host "  downloading $SetupExeUrl"
        Invoke-WebRequest -Uri $SetupExeUrl -OutFile $setupExe
    }
    Assert-True ((Get-Item $setupExe).Length -gt 1MB) "Xiuqo-Setup.exe downloaded ($([math]::Round((Get-Item $setupExe).Length / 1MB, 1)) MB)"
    ...
    Remove-Item Env:XIUQO_SETUP_DEV_REPO_ROOT -ErrorAction SilentlyContinue
    ...
    try {
        ...
        Write-Host "  Xiuqo-Setup.exe launched (pid $($installer.Id))"

        # Drive it: Install click -> wait -> Launch click -> Xiuqo.exe window.
        ...
        $ahk = Start-Process -FilePath $ahkExe `
            -ArgumentList (Join-Path $AhkDir "install-and-launch.ahk"), $ahkLog, "Xiuqo-Setup.exe", (Join-Path $HermesHome "logs\bootstrap-installer.log") `
            -PassThru
        ...
        # actually be running.
        Assert-True ($null -ne (Get-Process -Name "Xiuqo" -ErrorAction SilentlyContinue)) "Xiuqo.exe process is running (installer Launch hand-off worked)"

        # Installer should have exited after Launch.
        if (-not $installer.HasExited) {
            Start-Sleep -Seconds 10
        }
        Assert-True $installer.HasExited "Xiuqo-Setup.exe exited after Launch"
    }
    finally {
    ...
    Assert-True ($null -ne (Get-DesktopExe)) "packaged Desktop Xiuqo.exe exists"
...
function Invoke-GuiUpdateDesktopRoute([string]$TargetSha) {
    ...
    Assert-True ($null -ne $desktopExe) "packaged Xiuqo.exe present before update"
    ...
    try {
        ...
        # `xiuqo update` + desktop rebuild + relaunch. Which updater depends
        # on the installed checkout, and BOTH are production paths:
        #   * checkouts shipping scripts/desktop-update.ps1 -> that script,
        #     which writes .xiuqo-update-result.json on every exit;
        #   * older checkouts -> the staged xiuqo-setup.exe --update flow,
        #     which does NOT write the result JSON.
        # So: poll for COMPLETION = (result JSON) OR (checkout reached the
        # target sha AND the marker is gone). The sha/marker/xiuqo/relaunch
        ...
        Assert-True ($null -ne (Get-DesktopExe)) "Xiuqo.exe still present after update"
        ...
        Write-Host "  waiting for the relaunched Xiuqo.exe ..."
        $rDeadline = (Get-Date).AddMinutes(5)
        $relaunched = $null
        while ((Get-Date) -lt $rDeadline) {
            $relaunched = Get-Process -Name "Xiuqo" -ErrorAction SilentlyContinue
            if ($relaunched) { break }
        ...
        # Foreground the relaunched Xiuqo window so the proof screenshot
        ...
        try {
            $mainProc = Get-Process -Name "Xiuqo" -ErrorAction SilentlyContinue |
...
function Invoke-PhaseInstall {
    ...
    # checkout at OLD, xiuqo runs, and state carries how OLD landed so any
...
function Invoke-PhaseUpdate {
    ...
    switch ($Route) {
        ...
        "desktop-installer@latest" {
            # A user re-downloading Xiuqo-Setup.exe and clicking Install over
