...
#              install landed on OLD with a working `xiuqo`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `xiuqo`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method xiuqo-update|installer-script|installer-script+desktop
...
case "$UPDATE_METHOD" in
  xiuqo-update|installer-script|installer-script+desktop|xiuqo-desktop-app-update) ;;
  *) echo "error: --update-method must be xiuqo-update, installer-script, installer-script+desktop or xiuqo-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/xiuqo-installer-script-e2e"
LOG_DIR="${XIUQO_E2E_LOG_DIR:-$WORK_ROOT/logs}"
...
# Isolated HOME: the runner's real one may carry a preinstalled xiuqo or a
# developer config, and old installer scripts hardcode $HOME/.xiuqo (the
# XIUQO_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
# above keeps working -- an explicit path wins over $HOME/.gitconfig.
export HOME="$WORK_ROOT/home"
mkdir -p "$HOME/.local/bin"
export PATH="$HOME/.local/bin:$PATH"
export XIUQO_HOME="$HOME/.xiuqo"
mkdir -p "$XIUQO_HOME"
...
assert_desktop_artifact() {
  ...
  local cand
  for cand in \
    "$release_dir/linux-unpacked/Xiuqo" \
    "$release_dir/linux-unpacked/xiuqo" \
    "$release_dir/mac-arm64/Xiuqo.app" \
    "$release_dir/mac/Xiuqo.app"; do
...
assert_checkout() {
  ...
  ok "checkout is $2 ($1)"
  local xiuqo="$INSTALL_DIR/venv/bin/xiuqo"
  [ -x "$xiuqo" ] || fail "no xiuqo console script at $xiuqo"
  "$xiuqo" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
    || fail "xiuqo --version failed after $2; log in $LOG_DIR/version-$2.log"
  ok "xiuqo --version works: $(head -c 120 "$LOG_DIR/version-$2.log" | tr -d '\n')"
}

smoke_desktop() {
  # $1: label (old|head). Prove the installed CLI can produce the desktop
  # app: `xiuqo desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `xiuqo update` itself makes. Probe the
  # INSTALLED xiuqo for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `xiuqo desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local xiuqo="$INSTALL_DIR/venv/bin/xiuqo"
  if ! "$xiuqo" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "xiuqo desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$xiuqo" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "xiuqo desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "xiuqo desktop --build-only ($1) exited $rc; transcript above"
  ok "xiuqo desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  xiuqo-update)
    ...
    # installed xiuqo; older ones read the prompt from stdin, so close it.
    XIUQO="$INSTALL_DIR/venv/bin/xiuqo"
    if "$XIUQO" update --help 2>&1 | grep -qF -- --yes; then
      update_cmd=("$XIUQO" update --yes)
    else
      update_cmd=("$XIUQO" update)
    fi
    rc=0
    (cd "$INSTALL_DIR" && "${update_cmd[@]}" < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/update.log") || rc=$?
    log_group "xiuqo update transcript" "$LOG_DIR/update.log"
    [ "$rc" -eq 0 ] || fail "xiuqo update exited $rc; transcript above, log at $LOG_DIR/update.log"
    ;;
  installer-script)
  ...
  xiuqo-desktop-app-update)
    # The real user surface: `xiuqo desktop` launches the app, the user
    ...
    # suite's own) is configured into XIUQO_HOME exactly like the dev:mock
    ...
    step "capturing the xiuqo desktop launch spec (build runs for real)"
    rc=0
    (cd "$INSTALL_DIR" && \
      PYTHONPATH="$ASSETS/launch-capture${PYTHONPATH:+:$PYTHONPATH}" \
      XIUQO_E2E_CAPTURE_LAUNCH="$SPEC" \
      "$XIUQO" desktop < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/desktop-launch-capture.log") || rc=$?
    log_group "xiuqo desktop (launch capture) transcript" "$LOG_DIR/desktop-launch-capture.log"
    [ "$rc" -eq 0 ] || fail "xiuqo desktop exited $rc during launch capture; transcript above"
    # Exit 0 without a capture means a version that never reached its
    # launch - that must fail loudly, not pass as a no-op.
    [ -f "$SPEC.captured" ] || fail "xiuqo desktop exited 0 but no launch was captured at $SPEC"
...
mkdir -p "$ildest"
cp -R "$XIUQO_HOME/logs" "$ildest/xiuqo-logs" 2>/dev/null || true
if [ -n "${XDG_DATA_HOME:-}" ]; then
  cp -R "$XDG_DATA_HOME/xiuqo/logs" "$ildest/desktop-userdata-logs" 2>/dev/null || true
fi
cp "$XIUQO_HOME/.xiuqo-update-result.json" "$ildest" 2>/dev/null || true
ls -la "$XIUQO_HOME" > "$ildest/xiuqo-home-ls.txt" 2>/dev/null || true
