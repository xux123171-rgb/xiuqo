...
# (Xiuqo-Setup.dmg from the website) can reach HEAD.
#
...
#              xiuqo-desktop-app-update  capture `xiuqo desktop`'s spawn,
#                                         launch the spec under Playwright,
#                                         click Update now
#              xiuqo-update              CLI update from the installed venv
...
#     --update-method open-app-update|xiuqo-desktop-app-update
...
UPDATE_METHOD=""
INSTALL_REF=""
DMG_URL="https://xiuqo-assets.nousresearch.com/Xiuqo-Setup.dmg"
...
case "$UPDATE_METHOD" in
  open-app-update|xiuqo-desktop-app-update|xiuqo-update|installer-script|installer-script+desktop) ;;
...
WORK_ROOT="${XIUQO_E2E_WORKROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}/xiuqo-macos-desktop-e2e}"
...
EOF
  ...
  export INSTALL_DIR="$XIUQO_HOME/xiuqo"
}
...
phase_stage() {
  ...
  arm_redirect
  mkdir -p "$XIUQO_HOME"
  touch "$XIUQO_HOME/.skip_upstream_prompt"
...
find_installed_app() {
  ...
  local cand
  for cand in \
    "$INSTALL_DIR/apps/desktop/release/mac-arm64/Xiuqo.app" \
    "$INSTALL_DIR/apps/desktop/release/mac/Xiuqo.app" \
    "/Applications/Xiuqo.app"; do
...
phase_install() {
  ...
  . "$STATE"
  arm_redirect
  step "installing OLD ($OLD_REF) via the published Xiuqo-Setup.dmg"

  local dmg="$WORK_ROOT/Xiuqo-Setup.dmg"
  ...
  # "Install Xiuqo" button with native input.
  local rc=0
  ...
  log_group "Xiuqo-Setup (dmg bootstrap) transcript" "$LOG_DIR/bootstrap-install.log"
  ...
  "$xiuqo" --version 2>&1 | ts_prefix > "$LOG_DIR/version-old.log" || fail "xiuqo --version failed after install"
  ok "xiuqo --version works: $(head -c 120 "$LOG_DIR/version-old.log" | tr -d '\n')"
  find_installed_app >/dev/null || fail "no installed Xiuqo.app after the dmg bootstrap"
...
phase_update() {
  ...
  case "$UPDATE_METHOD" in
    xiuqo-update)
      ...
      # update subcommand only in later releases; ask the installed xiuqo.
      local xiuqo="$INSTALL_DIR/venv/bin/xiuqo"
      local update_cmd=("$xiuqo" update)
      ...
      [ "$rc" -eq 0 ] || fail "xiuqo update exited $rc; transcript above"
      ;;
    installer-script)
    ...
    installer-script+desktop)
      ...
      for cand in \
        "$INSTALL_DIR/apps/desktop/release/mac-arm64/Xiuqo.app" \
        "$INSTALL_DIR/apps/desktop/release/mac/Xiuqo.app"; do
        [ -d "$cand" ] && { head_app="$cand"; break; }
      done
      [ -n "$head_app" ] || fail "no built Xiuqo.app under the checkout after the +desktop update"
    ...
    open-app-update)
      ...
      # which is exactly what the app's children (git, xiuqo update) need.
      local app app_bin
...
PYEOF
    ...
    xiuqo-desktop-app-update)
      ...
      [ "$rc" -eq 0 ] || fail "xiuqo desktop exited $rc during launch capture"
      [ -f "$spec.captured" ] || fail "xiuqo desktop exited 0 but no launch was captured"
  ...
  mkdir -p "$ildest"
  cp -R "$HOME_SANDBOX/.xiuqo/logs" "$ildest/xiuqo-logs" 2>/dev/null || true
  local ud="$HOME_SANDBOX/Library/Application Support/Xiuqo"
  ...
  "$INSTALL_DIR/venv/bin/xiuqo" --version 2>&1 | ts_prefix > "$LOG_DIR/version-head.log" \
    || fail "xiuqo --version failed after update"
  ok "xiuqo --version works post-update"
