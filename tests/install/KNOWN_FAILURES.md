...
Classification: **unfixable in the update target for the exact released `xiuqo.exe update` path**.

| Starting release | Released commit | Install → update | Verified failing job |
|---|---|---|---|
| `v2026.3.12` | `a370ab8391ca5f8de7ebbc449f05cb0df36ade7c` | `installer-script` → `xiuqo-update` | [101514756800](https://github.com/ethernet8023/xiuqo/actions/runs/34043635705/job/101514756800) |
| `v2026.4.8` | `86960cdbb0148145890e2ee90b4e157fa899f6e1` | `installer-script` → `xiuqo-update` | [101514755527](https://github.com/ethernet8023/xiuqo/actions/runs/34043635705/job/101514755527) |

The running console launcher holds `venv/Scripts/xiuqo.exe` open. The old updater pulls the new checkout, then asks uv to replace that same executable during an editable install. Windows rejects the replacement with `Access is denied. (os error 5)`. The old updater's ZIP fallback repeats the dependency install and encounters the same lock.

Evidence required: the CLI update phase failed, the traceback identifies the running `xiuqo.exe/__main__.py`, and uv reports failure to remove that install's `Scripts/xiuqo.exe` with OS error 5. A generic access-denied error on another file does not match.

The March call is in the released `xiuqo_cli/main.py:1678-1683`, with the ZIP fallback at `1571-1576`. April calls `_install_python_dependencies_with_optional_fallback`, whose released body at `3295-3321` runs the installs without launcher quarantine. Those function objects were loaded before the checkout changed. May's sampled CLI update passed; do not classify it from this record.
...
|---|---|
| `installer-script` → `xiuqo-desktop-app-update` | [101514755236](https://github.com/ethernet8023/xiuqo/actions/runs/34043635705/job/101514755236) |
| `installer-script+desktop` → `xiuqo-desktop-app-update` | [101514760893](https://github.com/ethernet8023/xiuqo/actions/runs/34043635705/job/101514760893) |
| `installer-script+desktop` → `open-app-update` | [101514756508](https://github.com/ethernet8023/xiuqo/actions/runs/34043635705/job/101514756508) |

These script installs have no staged updater. The released Electron code (`apps/desktop/electron/main.cjs:2212-2214`) logs `no staged updater; surfacing manual` and returns `{ ok: true, manual: true, command }`. It does not start an update. Each job's `logs/desktop.log` records that branch followed by `[updates] manual: xiuqo update`; no target checkout/result signal appears.
