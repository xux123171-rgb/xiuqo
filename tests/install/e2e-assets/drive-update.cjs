// drive-update.cjs — launch the INSTALLED Xiuqo.exe (real Electron desktop
...
//
//   node <this file> <path-to-Xiuqo.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <Xiuqo.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  const app = await _electron.launch({
    ...
    // Inherit the driver's env: XIUQO_HOME (isolated install) and
  ...
  // The real backend (`xiuqo serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting Xiuqo... 86%"), then the
  ...
  // The "Updating Xiuqo — this window will close" overlay should appear,
  ...
  // the staged binary), which claims XIUQO_HOME/.xiuqo-update-in-progress
  ...
  // proved the desktop had exited and `xiuqo update` was already running).
  //
  ...
  const hermesHome = process.env.XIUQO_HOME
  const markerPath = hermesHome ? path.join(hermesHome, '.xiuqo-update-in-progress') : null
  const resultPath = hermesHome ? path.join(hermesHome, '.xiuqo-update-result.json') : null
