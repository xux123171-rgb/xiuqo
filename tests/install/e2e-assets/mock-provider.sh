...
#   mock_start <workroot>      start, write config into $XIUQO_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, XIUQO_HOME, ok/fail helpers.
...
mock_start() {
  ...
  cat > "$XIUQO_HOME/config.yaml" <<EOF
model:
  default: mock-model
  provider: mock
providers:
...
EOF
  ...
  ok "provider 'mock' configured in $XIUQO_HOME (api $url/v1)"
}
