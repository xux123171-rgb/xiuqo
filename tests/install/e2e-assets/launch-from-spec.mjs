// @ts-check
/**
 * Launch the Xiuqo desktop app from a captured launch spec and click the
 * real update flow: Settings -> About -> "Update now".
 *
 * The spec is written by launch-capture/sitecustomize.py at `xiuqo
...
async function main() {
  ...
  // an "update complete, reopen Xiuqo to finish" overlay and never exits;
  ...
  const boundedClose = async (application, label) => {
    ...
    // (`xiuqo serve` python + node helpers) survives and keeps writing
  ...
  phase('relaunch');
  log('relaunching the updated app (the "reopen Xiuqo" step)');
