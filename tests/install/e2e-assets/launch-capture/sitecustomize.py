"""Driver-side spawn interception for xiuqo desktop E2E legs.

The installed ``xiuqo`` is a venv console script, so its interpreter
imports ``sitecustomize`` at startup when this directory is on
``PYTHONPATH``. Behind an explicit env-var opt-in the module wraps
``subprocess.run`` so the FINAL electron launch call of ``xiuqo
...
Opt-in: ``XIUQO_E2E_CAPTURE_LAUNCH=<path>`` -- the spec is written
there, and the marker file ``<path>.captured`` distinguishes "xiuqo
...
_SPEC_PATH = os.environ.get("XIUQO_E2E_CAPTURE_LAUNCH")
...
if _SPEC_PATH:
    ...
    def _match_shape(argv: "list[str]") -> str:
        ...
        # apps/desktop/release/ (win-unpacked/Xiuqo.exe, linux-unpacked/...,
        # mac*/Xiuqo.app/Contents/MacOS/...).
