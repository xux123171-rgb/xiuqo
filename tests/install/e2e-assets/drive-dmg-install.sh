#!/usr/bin/env bash
# Drive the Xiuqo-Setup dmg bootstrap through its first-run GUI.
#
...
# the redirect env), click "Install Xiuqo ->" with native input, then watch
...
tell application "System Events"
  set procs to (every process whose name contains "Xiuqo")
...
XIUQO_BIN="$INSTALL_DIR/venv/bin/xiuqo"
...
install_complete() {
  [ -d "$INSTALL_DIR/.git" ] && [ -x "$XIUQO_BIN" ] && installed_app
}
...
BOOTSTRAP_LOG="$HOME/.xiuqo/logs/bootstrap-installer.log"
bootstrap_error() {
...
while :; do
  if install_complete; then
    log "install landed: checkout + venv console script + Xiuqo.app present"
  ...
  if ! kill -0 "$SETUP_PID" 2>/dev/null; then
    ...
    log "Xiuqo-Setup exited (pid $SETUP_PID) before the install landed"
    exit 1
  fi
  err="$(bootstrap_error)"
  if [ -n "$err" ]; then
