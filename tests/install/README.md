...
2. Install: run the old release's own installer script. Make sure that the checkout is at the old commit and that `xiuqo --version` works.
3. Desktop smoke: run `xiuqo desktop --build-only` from the installed CLI. This proves that the installed version can build the desktop app. If the installed version does not have this flag, the phase reports a skip and continues.
4. Update: move `main` to HEAD. Apply one update method. Make sure that the checkout is at HEAD and that `xiuqo --version` works.
5. Desktop smoke again, at HEAD.

The windows GUI driver replaces phases 2 and 4 when the install method is `desktop-installer@latest`. It downloads the published `Xiuqo-Setup.exe`, clicks through the installer window with AutoHotkey, and clicks "Update now" in the running app with Playwright.
...
- `desktop-installer@latest`: the published GUI installer (`Xiuqo-Setup.exe` on windows, `Xiuqo-Setup.dmg` on macos), driven through the real user flow.
...
Running the drivers locally: don't, except in a disposable VM. The windows driver kills every process named Xiuqo during teardown and the macos driver operates on `/Applications/Xiuqo.app`; on a machine with a real Xiuqo install they will interfere with it.
