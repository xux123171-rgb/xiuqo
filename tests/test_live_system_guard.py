...
def test_argv_arguments_are_not_treated_as_executables(tmp_path):
    ...
    guard (the path contains "xiuqo" via the pytest tmp root)."""
...
def test_direct_killer_argv_is_still_blocked():
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["pkill", "-f", "xiuqo-guard-regression-nomatch"])
...
def test_wrapped_killer_command_is_still_blocked():
    ...
    shell wrapper."""
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["bash", "-c", "pkill -f xiuqo-guard-regression-nomatch"])


def test_env_wrapped_killer_command_is_still_blocked():
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["env", "GUARD_TEST=1", "pkill", "-f", "xiuqo-guard-regression-nomatch"])


def test_gateway_start_inside_a_container_exec_is_not_blocked():
    """``docker exec <ctr> xiuqo gateway start`` launches the gateway INSIDE the container,
    ...
    stub_dir = os.path.join(os.environ["XIUQO_HOME"], "stub-bin")
    ...
    result = subprocess.run(
        [stub, "exec", "-u", "xiuqo", "ctr", "sh", "-c", "xiuqo -p prof gateway start"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
...
def test_gateway_start_on_the_host_is_still_blocked():
    with pytest.raises(RuntimeError, match="REAL.*gateway runtime"):
        subprocess.run(["python", "-m", "xiuqo_cli.main", "gateway", "start"])
