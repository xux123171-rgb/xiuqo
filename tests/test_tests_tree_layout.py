...
(``tests/cli`` beside ``tests/xiuqo_cli``, ``tests/run_agent`` beside
``tests/agent``, ``tests/state`` beside ``tests/xiuqo_state``) and ~250
...
_NON_MIRROR_DIRS = {
    ...
    "security", "skills", "verify", "website", "computer_use", "xiuqo_state",
}
...
def test_every_test_directory_mirrors_a_source_directory_or_is_declared():
    ...
    assert not offenders, (
        ...
        "tests/xiuqo_cli, ...) or, for a genuinely cross-cutting suite, add the "
