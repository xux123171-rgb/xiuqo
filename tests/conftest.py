"""Shared fixtures for the xiuqo test suite.
...
2. **Isolated XIUQO_HOME.** XIUQO_HOME points to a per-test tempdir so
   code reading ``~/.xiuqo/*`` via ``get_hermes_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".xiuqo"`` instead of the canonical
   ``get_hermes_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No XIUQO_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox XIUQO_HOME before ANY test module is imported ──────────────────
# `xiuqo_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
...
# real `~/.xiuqo/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes XIUQO_HOME, but fixtures run
...
# the REAL Xiuqo root — capture it BEFORE the sandbox rewires XIUQO_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.xiuqo (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("XIUQO_KANBAN_HOME", "").strip()
_PRE_SANDBOX_XIUQO_HOME = os.environ.get("XIUQO_HOME", "")


def _hermes_home_points_at_production(value: str) -> bool:
    """True when a pre-set XIUQO_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    XIUQO_HOME=~/.xiuqo``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``xiuqo_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.xiuqo`` — the escape vector that landed pytest
    ...
    XIUQO_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".xiuqo").resolve()
    except Exception:
        return True
...
if _hermes_home_points_at_production(os.environ.get("XIUQO_HOME", "")):
    _SESSION_XIUQO_HOME = tempfile.mkdtemp(prefix="xiuqo-test-home-")
    os.environ["XIUQO_HOME"] = _SESSION_XIUQO_HOME
    atexit.register(shutil.rmtree, _SESSION_XIUQO_HOME, True)
...
# like a real CLI") — which used to disarm xiuqo_state's live-DB guard in
# the child at the same moment the child lost the XIUQO_HOME redirect.
# XIUQO_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# xiuqo_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export XIUQO_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["XIUQO_TEST_ISOLATION"] = os.environ.get("XIUQO_HOME", "") or "1"

#: XIUQO_HOME as it stood when conftest was imported - i.e. before any test
...
XIUQO_HOME_AT_CONFTEST_IMPORT = os.environ.get("XIUQO_HOME", "")
...
# XIUQO_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_XIUQO_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``xiuqo_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Xiuqo agent session inherits them and
    # xiuqo_constants home-resolution helpers prefer them over monkeypatched
    ...
    "XIUQO_MAX_ITERATIONS",
    "XIUQO_SESSION_PLATFORM",
    "XIUQO_SESSION_CHAT_ID",
    "XIUQO_SESSION_CHAT_NAME",
    "XIUQO_SESSION_CHAT_TYPE",
    "XIUQO_SESSION_THREAD_ID",
    "XIUQO_SESSION_SOURCE",
    "XIUQO_SESSION_KEY",
    "XIUQO_GATEWAY_SESSION",
    "XIUQO_CRON_SESSION",
    "_XIUQO_GATEWAY",
    "XIUQO_PLATFORM",
    "XIUQO_MODEL",
    "XIUQO_INFERENCE_MODEL",
    "XIUQO_INFERENCE_PROVIDER",
    "XIUQO_TUI_PROVIDER",
    "XIUQO_MANAGED",
    "XIUQO_MANAGED_DIR",
    "XIUQO_DEV",
    "XIUQO_CONTAINER",
    "XIUQO_EPHEMERAL_SYSTEM_PROMPT",
    "XIUQO_TIMEZONE",
    "XIUQO_REDACT_SECRETS",
    "XIUQO_BACKGROUND_NOTIFICATIONS",
    "XIUQO_EXEC_ASK",
    "XIUQO_HOME_MODE",
    "XIUQO_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.xiuqo/kanban.db instead of the per-test XIUQO_HOME.
    "XIUQO_KANBAN_DB",
    "XIUQO_KANBAN_BOARD",
    "XIUQO_KANBAN_HOME",
    "XIUQO_KANBAN_WORKSPACES_ROOT",
    "XIUQO_KANBAN_LOGS_ROOT",
    "XIUQO_KANBAN_TASK",
    "XIUQO_KANBAN_WORKSPACE",
    "XIUQO_KANBAN_RUN_ID",
    "XIUQO_KANBAN_CLAIM_LOCK",
    "XIUQO_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "xiuqo" host.
    "XIUQO_HONCHO_HOST",
    ...
    "XIUQO_DASHBOARD_OAUTH_CLIENT_ID",
    "XIUQO_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and XIUQO_HOME to per-test tempdirs so code that
    reads ``~/.xiuqo/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral XIUQO_* vars that could change test semantics.
    for name in _XIUQO_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("XIUQO_HONCHO_HOST", "xiuqo")

    # 3. Redirect XIUQO_HOME to a per-test tempdir. Code that reads
    #    ``~/.xiuqo/*`` via ``get_hermes_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.xiuqo/*`` via
    #    ``Path.home() / ".xiuqo"`` instead of ``get_hermes_home()``
    #    is a bug to fix at the callsite.
    fake_hermes_home = tmp_path / "xiuqo_test"
    fake_hermes_home.mkdir()
    (fake_hermes_home / "sessions").mkdir()
    (fake_hermes_home / "cron").mkdir()
    (fake_hermes_home / "memories").mkdir()
    (fake_hermes_home / "skills").mkdir()
    monkeypatch.setenv("XIUQO_HOME", str(fake_hermes_home))
    ...
    # xiuqo_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("XIUQO_TEST_ISOLATION", str(fake_hermes_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("XIUQO_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. xiuqo_state computes ``DEFAULT_DB_PATH = get_hermes_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from xiuqo_state import ...``) that
    ...
    xiuqo_state_mod = sys.modules.get("xiuqo_state")
    if xiuqo_state_mod is not None and hasattr(xiuqo_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            xiuqo_state_mod, "DEFAULT_DB_PATH", fake_hermes_home / "state.db"
        )
    ...
    monkeypatch.setenv("XIUQO_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.xiuqo/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import xiuqo_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path XIUQO_HOME can't leak forward. Paths
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from xiuqo_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
def _neutralize_git_safe_directory_read(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_safe_directory"):
        return
    try:
        from xiuqo_cli import _subprocess_compat
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.xiuqo. This autouse
...
# allow-list because test-level fixtures legitimately move XIUQO_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session XIUQO_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``XIUQO_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Xiuqo root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_XIUQO_HOME and not _hermes_home_points_at_production(
        _PRE_SANDBOX_XIUQO_HOME
    ):
        # XIUQO_HOME was genuinely set to a CUSTOM root before the sandbox
    ...
    # No pre-existing XIUQO_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".xiuqo").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.xiuqo`` captured at import time. Hermetic tests that legitimately
    move XIUQO_HOME to sibling tempdirs are unaffected.

    Only patches when ``xiuqo_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("xiuqo_cli.kanban_db")
    _kdbc = sys.modules.get("xiuqo_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import xiuqo_cli.kanban_db`
...
# ``xiuqo_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Xiuqo root. This fixture wires the
# test-side knobs:
...
#     installs where XIUQO_HOME is not ~/.xiuqo) into the guard's
...
# so subprocess children that import xiuqo_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("xiuqo_state")
    ...
    if _PRE_SANDBOX_XIUQO_HOME and not _hermes_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_XIUQO_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Xiuqo home override redirects every later
    ...
    # to its default.
    try:
        from xiuqo_constants import get_hermes_home_override, set_hermes_home_override
...
@pytest.fixture()
def mock_config():
    """Return a minimal xiuqo config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``xiuqo-gateway`` process
...
# correlated with a ``tests/xiuqo_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> xiuqo-gateway``
...
def _wal_is_usable() -> bool:
    """True when Xiuqo will actually put a database into WAL mode here.

    Xiuqo refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Xiuqo can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Xiuqo managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``xiuqo_state``. That module computes
    ...
    redirects ``XIUQO_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.xiuqo/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``xiuqo_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["XIUQO_VOICE_TTS"] = "1"``. The
...
#      ``xiuqo_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``xiuqo_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``xiuqo_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``xiuqo_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Xiuqo runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.xiuqo``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Xiuqo falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Xiuqo uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart xiuqo-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting xiuqo/python patterns are also blocked.
    """
    ...
    def _matches_hermes_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _XIUQO_TOKENS)
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch xiuqo-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m xiuqo_cli.main".
                if (
                    "xiuqo" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live xiuqo-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting xiuqo/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `xiuqo update` (or the
        # equivalent `python -m xiuqo_cli.main update`).  These commands
        ...
        if "update" in low and (
            # xiuqo update / xiuqo update --gateway / setsid bash -c ... xiuqo update
            ("xiuqo" in low and "update" in low.split())
            or
            # python -m xiuqo_cli.main update --gateway
            ("xiuqo_cli" in low and "update" in low.split())
            or
            # venv/bin/xiuqo update  (absolute path variant used in tests)
            (".venv/bin/xiuqo" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`xiuqo update` against the real checkout, fetching "
        ...
        # Block spawning a REAL gateway runtime (``python -m xiuqo_cli.main
        ...
        # child inherits the pytest-tmp XIUQO_HOME, resolves the DEVELOPER's
        # ``xiuqo-gateway`` systemd unit (a tmp home hashes to no profile
        ...
        # A gateway launched INSIDE a container (`docker exec … xiuqo gateway start`) cannot
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_XIUQO_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import xiuqo_cli.voice as _voice
    except Exception:
