"""Regression tests for Desktop-owned ``xiuqo serve`` lifecycle tracking."""

import pytest

from xiuqo_cli.web_server_lifecycle import (
    _is_serve_orphaned,
...
def test_parent_watchdog_treats_empty_marker_env_as_absent(monkeypatch):
    """Blank inherited XIUQO_PARENT_START_MARKER/NONCE degrade to PID-only tracking."""
    from xiuqo_cli import web_server_lifecycle

    monkeypatch.setenv("XIUQO_PARENT_PID", "4242")
    monkeypatch.setenv("XIUQO_PARENT_START_MARKER", "")
    monkeypatch.setenv("XIUQO_PARENT_NONCE", "")
    seen = {}
...
def test_parent_watchdog_warns_when_disarmed_by_unusable_marker(monkeypatch, caplog):
    ...
    monkeypatch.setenv("XIUQO_PARENT_PID", "4242")
    monkeypatch.setenv("XIUQO_PARENT_START_MARKER", "ps:Sat")
    monkeypatch.setenv("XIUQO_PARENT_NONCE", "n")
    started = []
    monkeypatch.setattr(
