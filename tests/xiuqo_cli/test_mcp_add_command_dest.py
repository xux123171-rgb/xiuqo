"""Regression test: ``xiuqo mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``xiuqo_cli/_parser.py``).  The dispatcher in ``xiuqo_cli/main.py``
...
result, ``xiuqo mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the xiuqo parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`xiuqo mcp add foo --url ...` must keep args.command == "mcp".
