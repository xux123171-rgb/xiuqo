...
xiuqo_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `xiuqo config`/`doctor`
...
def _load_cli_config(home):
    ...
    singleton), so monkeypatching XIUQO_HOME after import doesn't move it.
    ...
    this only matters for tests that swap XIUQO_HOME.)
    """
    import cli
