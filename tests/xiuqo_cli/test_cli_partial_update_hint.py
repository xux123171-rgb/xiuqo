...
``xiuqo_cli.config`` while the older ``config.py`` does not export them.
...
bare ``xiuqo`` and ``xiuqo chat`` (including the fast-chat launch path)
all go through it.
"""
...
from xiuqo_constants import emit_partial_update_hint, partial_update_hint
...
def _missing_config_name_error(name: str = "resolve_turn_limit") -> ImportError:
    exc = ImportError(
        f"cannot import name '{name}' from 'xiuqo_cli.config'"
    )
    exc.name = "xiuqo_cli.config"
    return exc
...
def test_emit_hint_for_missing_split_model_config_default():
    ...
    buf = io.StringIO()
    assert emit_partial_update_hint(exc, file=buf) is True
    assert "xiuqo update" in buf.getvalue()
