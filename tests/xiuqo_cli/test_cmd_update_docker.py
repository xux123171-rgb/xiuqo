"""Tests for ``xiuqo update`` / ``--check`` inside the Docker container.
...
fix, ``xiuqo update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Xiuqo, not an update to the running container, so the message
...
@patch("xiuqo_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``xiuqo update`` inside Docker → friendly message + exit 2, no git calls.
...
def test_format_docker_update_message_contents():
    ...
    """
    from xiuqo_cli.config import format_docker_update_message
    ...
    assert "XIUQO_HOME" in msg or "/opt/data" in msg, (
