...
four now go through ``xiuqo_cli.model_switch.persist_model_selection`` /
...
def _via_cli(home):
    from xiuqo_cli import cli_model_switch_mixin as mixin
    stub = type("Stub", (), {
...
def _via_dashboard(home):
    from xiuqo_cli.web_server_config import _apply_model_assignment_sync
...
def test_every_persist_surface_writes_the_same_model_block(seeded_home, monkeypatch, surface):
    monkeypatch.setattr("xiuqo_cli.model_switch.switch_model", lambda **_kw: _RESULT)
    monkeypatch.setattr("cli.HermesCLI._persist_model_switch_to_session", lambda *a, **k: None)
    monkeypatch.setattr("xiuqo_cli.cli_model_switch_mixin._print_switch_summary", lambda *a, **k: None)
    monkeypatch.setattr("xiuqo_cli.model_selection_guards.combined_selection_warning",
