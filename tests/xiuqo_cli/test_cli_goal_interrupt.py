...
def xiuqo_home(tmp_path, monkeypatch):
    """Isolated XIUQO_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".xiuqo"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("XIUQO_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves XIUQO_HOME each test.
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, xiuqo_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, xiuqo_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, xiuqo_home):
