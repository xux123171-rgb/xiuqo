...
the SAME provider universe as ``xiuqo model`` (the CLI/TUI picker).
...
def test_every_hermes_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `xiuqo model` universe."""
    ...
    assert not missing, (
        "providers shown in `xiuqo model` but not configurable in the desktop "
