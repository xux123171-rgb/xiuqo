...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # point XIUQO_HOME at the temp dir (the _hermes_home import-time
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "xiuqo_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in XIUQO_HOME/config.yaml
    ...
    install whose XIUQO_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read XIUQO_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh XIUQO_HOME with NO config.yaml (managed/desktop first launch).
        xiuqo_home = tmp_path / ".xiuqo"
        xiuqo_home.mkdir()
        monkeypatch.setenv("XIUQO_HOME", str(xiuqo_home))
