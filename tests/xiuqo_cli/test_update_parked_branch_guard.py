"""Regression tests for the parked-branch guard in ``xiuqo update``.
...
memory-limit``, days behind main) left there by earlier tooling. ``xiuqo
...
def _no_config(monkeypatch):
    ...
    import xiuqo_cli.config as xiuqo_config

    monkeypatch.setattr(xiuqo_config, "load_config", lambda: {})
...
def test_skip_warning_names_branch_behind_count_and_commands(repo_pair, capsys):
    ...
    assert f"git -C {repo_pair} checkout main && xiuqo update" in out
...
def test_print_update_completion_carries_branch_and_sha(
    ...
    monkeypatch.setattr(xiuqo_main, "PROJECT_ROOT", repo_pair)
...
def _patch_update_flow(monkeypatch, repo, run_real_git=True):
    ...
    """
    monkeypatch.setattr(xiuqo_main, "PROJECT_ROOT", repo)
    ...
    monkeypatch.setattr(xiuqo_main, "_record_bytecode_fingerprint", lambda *a, **k: None)
    ...
    monkeypatch.setattr(xiuqo_main, "_pause_windows_gateways_for_update", lambda: None)
    monkeypatch.setattr(
        xiuqo_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(xiuqo_main, "_capture_active_lazy_features", lambda: [])
    monkeypatch.setattr(xiuqo_main, "_capture_active_tool_dependencies", lambda: [])
