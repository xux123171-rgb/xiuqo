"""``xiuqo chat -Q`` passes the dispatcher's XIUQO_TURN_AUTHOR to ``run_conversation`` as ``turn_author``.
...
def _plain_one_shot_env(monkeypatch):
    monkeypatch.delenv("XIUQO_KANBAN_GOAL_MODE", raising=False)
    monkeypatch.delenv("XIUQO_KANBAN_TASK", raising=False)


def _run(monkeypatch, env_value, run_conversation=None):
    """One quiet turn with XIUQO_TURN_AUTHOR set to ``env_value`` (unset for None); returns the recorded call kwargs."""
