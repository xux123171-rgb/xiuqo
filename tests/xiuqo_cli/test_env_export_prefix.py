...
hand-rolled parsers — ``xiuqo_cli.config.load_env``,
``xiuqo_cli.main._has_any_provider_configured``, and
...
def test_config_load_env_strips_export_prefix(tmp_path):
    from xiuqo_cli.config import invalidate_env_cache, load_env
...
def test_has_any_provider_configured_with_export_prefix(tmp_path, monkeypatch):
    ...
    import xiuqo_cli.main as hmain

    importlib.reload(hmain)
    # get_env_path() derives from XIUQO_HOME (set above) → tmp_path/.env, so
