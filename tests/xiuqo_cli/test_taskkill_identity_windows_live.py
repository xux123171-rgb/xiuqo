...
- ``gateway.status.terminate_pid(force=True)`` requires a matching
  ...
  mismatched identity.
- ``xiuqo_cli._subprocess_compat.pid_is_hermes`` fails closed on foreign
  processes and identity mismatches.
- ``xiuqo_cli.update_cmd._refuse_gateway_ancestor_tree_kill`` refuses to
...
class TestPidIsHermesLive:
    def test_foreign_real_process_is_refused(self):
        """A live non-Xiuqo process (bare python sleeper in a temp-ish argv)
        must never be judged safe for taskkill."""
        from xiuqo_cli._subprocess_compat import pid_is_hermes

        proc = _spawn_sleeper()
        try:
            # sys.executable in CI lives under a uv/hostedtoolcache path with
            # no 'xiuqo' token; if the checkout path itself contains one this
            # assertion is environment-dependent, so guard for it.
            if "xiuqo" in sys.executable.lower():
                pytest.skip("interpreter path names xiuqo; probe would match")
...
class TestAncestorRefusalLive:
    def test_real_parent_chain_is_refused(self, capsys):
        ...
        from xiuqo_cli.update_cmd import _refuse_gateway_ancestor_tree_kill
