"""Tests for xiuqo_cli.process_identity — spawn tags, the machine spawn
...
(XIUQO_SPAWN), long-lived processes self-register (pid, create_time,
purpose, spawner) in spawn-ledger.json, and `xiuqo update` reaps holders the
...
def test_install_id_stable_and_path_scoped():
    a = pi.install_id(Path("/opt/xiuqo"))
    assert a == pi.install_id(Path("/opt/xiuqo"))
...
def test_register_self_survives_non_utf8_argv(tmp_path):
    ...
    bad_argv = ["xiuqo", "serve", os.fsdecode(b"/tmp/project-\xff")]  # surrogate-escaped path
    with patch.dict(sys.modules, {"psutil": fake}), \
         patch.object(pi, "_ledger_path", return_value=ledger), \
         patch.object(pi.os, "getpid", return_value=999), \
...
def test_register_self_falls_back_to_desktop_parent_env(tmp_path):
    # Legacy Desktop: no XIUQO_SPAWN, but XIUQO_PARENT_PID + winms marker.
    ledger = tmp_path / "spawn-ledger.json"
    fake = _fake_psutil({999: 50.0})
    env = {
        "XIUQO_PARENT_PID": "888",
        "XIUQO_PARENT_START_MARKER": "winms:1755689000123",
    }
    with patch.dict(sys.modules, {"psutil": fake}), \
         patch.dict(pi.os.environ, env, clear=False), \
...
def _holders(*pids):
    return [(p, "python.exe", f"python.exe -m xiuqo_cli.main --profile p{p} serve") for p in pids]
