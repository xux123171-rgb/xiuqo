...
1. `xiuqo secrets bitwarden setup --help` works (dispatch path)
2. `xiuqo update --check` works (update path)
3. `xiuqo secrets bitwarden disable` works (handler execution)
4. `xiuqo secrets onepassword status` works (lazy backend loads on demand)
...
def _run_hermes(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run xiuqo CLI as a subprocess from repo root.
    ...
    connectivity — but ``xiuqo update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-xiuqo-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-xiuqo-test-remote/.insteadOf",
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_setup_help(self) -> None:
        """`xiuqo secrets bitwarden setup --help` must exit 0 and print usage.
    ...
    def test_bitwarden_status(self) -> None:
        """`xiuqo secrets bitwarden status` must exit 0 (runs lazy backend)."""
    ...
    def test_bitwarden_disable(self) -> None:
        """`xiuqo secrets bitwarden disable` must exit 0."""
    ...
    def test_onepassword_status(self) -> None:
        """`xiuqo secrets onepassword status` must exit 0 (1Password lazy backend)."""
    ...
    def test_onepassword_setup_help(self) -> None:
        """`xiuqo secrets onepassword setup --help` must exit 0."""
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `xiuqo update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`xiuqo update --check` must not load cryptography._rust."""
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['xiuqo','update','--check']
...
sys.argv = ['xiuqo', 'update', '--check']

import xiuqo_cli.main as m
...
# resolves the xiuqo_cli.main module's lazily re-exported attribute —
# so the patch must land on update_cmd._cmd_update_check.
with patch('xiuqo_cli.update_cmd._cmd_update_check', capture_update_check):
    try:
        m.main()
