"""Contract tests for xiuqo_cli.local_runtime — Rollouts 1+2.

Per the design's verification plan: relationships and contracts, no
change-detector tests, real imports against temp XIUQO_HOME (the autouse
...
from xiuqo_cli.local_runtime.binaries import (
    AssetPlan,
    BinaryResolutionError,
    resolve_assets,
    select_backend,
)
from xiuqo_cli.local_runtime.detect import DetectedServer, probe_port
...
def test_sha256_mismatch_rejects(tmp_path, monkeypatch):
    ...
    from xiuqo_cli.local_runtime import binaries
...
def _make_supervisor(tmp_path, port):
    ...
    from xiuqo_cli.local_runtime.supervisor import LlamaServerSupervisor
...
def test_llamacpp_endpoint_resolution_prefers_managed(tmp_path, monkeypatch, stub_server):
    ...
    monkeypatch.setenv("XIUQO_HOME", str(tmp_path / ".xiuqo"))
    port, handler = stub_server
    from xiuqo_cli.local_runtime import endpoint as ep
...
def test_llamacpp_endpoint_stale_state_falls_through(tmp_path, monkeypatch):
    ...
    with socket.socket() as s:
        ...
        dead_port = s.getsockname()[1]
    from xiuqo_cli.local_runtime import endpoint as ep
    from xiuqo_cli.local_runtime.detect import DEFAULT_PROBE_PORTS
    from xiuqo_cli.local_runtime.supervisor import state_path
    ...
    monkeypatch.setattr("xiuqo_cli.local_runtime.detect.DEFAULT_PROBE_PORTS",
                        (dead_port,))
...
def test_llamacpp_dead_server_raises_friendly_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "xiuqo_cli.local_runtime.endpoint.resolve_llamacpp_endpoint",
        lambda *a, **k: None)
...
def test_resolution_kicks_boot_when_no_thread_is_booting(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XIUQO_HOME", str(tmp_path / ".xiuqo"))
    from xiuqo_cli.local_runtime import bootstrap as bs
    from xiuqo_cli.local_runtime import endpoint as ep
    from xiuqo_cli.local_runtime.supervisor import state_path
...
def test_staged_models_requires_every_split_part(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XIUQO_HOME", str(tmp_path / ".xiuqo"))
    import xiuqo_cli.local_runtime.bootstrap as bs
...
def test_bootstrap_skips_boot_with_no_staged_models(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("xiuqo_cli.local_runtime.binaries.ensure_runtime_installed", _boom)
...
def test_endpoint_identity_stable_across_supervisor_instances(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XIUQO_HOME", str(tmp_path / ".xiuqo"))
    from xiuqo_cli.local_runtime import supervisor as sup_mod
    from xiuqo_cli.local_runtime.supervisor import LlamaServerSupervisor
    ...
    key_file = tmp_path / ".xiuqo" / "runtimes" / "llamacpp" / ".api_key"
...
def test_runtime_provider_seam_llamacpp_alias(tmp_path, monkeypatch, stub_server):
    ...
    from xiuqo_cli.runtime_provider import _resolve_named_custom_runtime
...
def test_bootstrap_disabled_is_noop(tmp_path, monkeypatch):
    monkeypatch.setenv("XIUQO_HOME", str(tmp_path / ".xiuqo"))
    from xiuqo_cli.local_runtime import bootstrap
...
def test_bootstrap_reuses_running_server(tmp_path, monkeypatch, stub_server):
    ...
    called = []
    monkeypatch.setattr(
        "xiuqo_cli.local_runtime.binaries.ensure_runtime_installed",
...
def test_bootstrap_failure_never_raises(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "xiuqo_cli.local_runtime.binaries.ensure_runtime_installed", boom)
