...
1. The venv-repair import probes (early recovery + `xiuqo update`) must
...
2. ``xiuqo doctor`` must fail the certificate check in that state, and
   ``xiuqo doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `xiuqo update`'s venv repair must apply
...
# =========================================================================
# 2. xiuqo doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "xiuqo doctor --fix" in message
