...
def test_installed_tags_newest_first(xiuqo_home):
    from xiuqo_cli.local_runtime.binaries import installed_tags

    assert installed_tags() == []
    _install_fake_tag(xiuqo_home, "b10290")
    _install_fake_tag(xiuqo_home, "b10412")
    assert installed_tags() == ["b10412", "b10290"]


def test_default_tag_flows_from_default_config(xiuqo_home):
    """Unpinned users inherit the Xiuqo-release default (deep-merge);
...
def test_update_available_requires_enabled_and_installed(xiuqo_home, monkeypatch):
...
def test_boot_never_downloads_missing_tag(xiuqo_home, monkeypatch):
...
def test_prune_keeps_n_minus_one(xiuqo_home):
    from xiuqo_cli.local_runtime.binaries import installed_tags, prune_old_tags

    for tag in ("b10100", "b10200", "b10290"):
        _install_fake_tag(xiuqo_home, tag)
    prune_old_tags(["b10290", "b10200"])
    assert installed_tags() == ["b10290", "b10200"]
    # downloads/ cache dir must survive pruning when present.
    downloads = xiuqo_home / "runtimes" / "llamacpp" / "downloads"
