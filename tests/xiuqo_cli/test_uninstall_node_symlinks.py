"""Tests for xiuqo_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $XIUQO_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Xiuqo-managed node dir.
"""
...
def _make_hermes_node(xiuqo_home: Path) -> Path:
    """Create a fake $XIUQO_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = xiuqo_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    xiuqo_home = fake_home / ".xiuqo"
    _make_hermes_node(xiuqo_home)
    ...
    removed = uninstall.remove_node_symlinks(xiuqo_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    xiuqo_home = fake_home / ".xiuqo"
    node_bin = _make_hermes_node(xiuqo_home)
