...
import xiuqo_cli.web_models as _web_models
import xiuqo_cli.web_routers.cron as _rt_cron
import xiuqo_cli.web_server_cron as _web_server_cron
...
async def test_dashboard_cron_noop_inference_fields_keep_existing_snapshots(
    ...
    from xiuqo_cli import runtime_provider, web_server
...
async def test_create_cron_job_without_profile_uses_backend_own_profile(
    ...
    ``~/.xiuqo`` when the request carries no explicit ``profile`` (the
    Desktop app's pre-profileScoped clients sent none)."""
    from xiuqo_cli import web_server

    monkeypatch.setenv(
        "XIUQO_HOME", str(isolated_profiles["worker_alpha"])
    )
...
async def test_create_cron_job_without_profile_defaults_when_unscoped(
    ...
    """XIUQO_HOME at the default home (or unrecognized) keeps the legacy
    ``default`` fallback."""
    from xiuqo_cli import web_server

    monkeypatch.setenv("XIUQO_HOME", str(isolated_profiles["default"]))
