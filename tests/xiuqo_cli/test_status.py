...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    from xiuqo_cli import status as status_mod
    ...
    assert "Start with:   xiuqo gateway" in output
...
class TestShowStatusXaiOAuth:
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status",
                            lambda: {"logged_in": True, "auth_store": "/home/u/.xiuqo/auth.json"},
                            raising=False)

        status_mod.show_status(SimpleNamespace(all=False, deep=False))
        out = capsys.readouterr().out

        assert "Auth file:  /home/u/.xiuqo/auth.json" in out
    ...
    def test_status_function_returns_none_does_not_crash(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: xiuqo auth add xai-oauth)" in out


def test_show_status_reports_gateway_session_last_activity(monkeypatch, capsys, tmp_path):
    """xiuqo status should surface freshest gateway last_active (#72016)."""
    from xiuqo_cli import status as status_mod
    import xiuqo_cli.auth as auth_mod
    import xiuqo_cli.gateway as gateway_mod
    import xiuqo_state
    ...
    monkeypatch.setattr(xiuqo_state, "SessionDB", _FakeDB)
