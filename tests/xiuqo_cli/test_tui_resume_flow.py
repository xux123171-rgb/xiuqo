...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from xiuqo_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """xiuqo -z bypasses HermesCLI, but recall still needs SessionDB."""
    from xiuqo_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "xiuqo_state", mod("xiuqo_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "xiuqo_cli.config",
        mod("xiuqo_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "xiuqo_cli.models",
        mod("xiuqo_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "xiuqo_cli.runtime_provider",
        mod(
            "xiuqo_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "xiuqo_cli.tools_config",
        mod("xiuqo_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["XIUQO_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/xiuqo-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["XIUQO_MODEL"] == "nous/xiuqo-test"
    assert env["XIUQO_INFERENCE_MODEL"] == "nous/xiuqo-test"
    assert env["XIUQO_TUI_PROVIDER"] == "nous"
    assert env["XIUQO_INFERENCE_PROVIDER"] == "nous"
    assert env["XIUQO_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["XIUQO_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("xiuqo-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_hermes_ink(monkeypatch, main_mod, tmp_path):
    ...
    ink_dir = tui_dir / "packages" / "xiuqo-ink"
    ...
    monkeypatch.delenv("XIUQO_TUI_DIR", raising=False)
