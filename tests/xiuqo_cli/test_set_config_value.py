...
def _isolated_hermes_home(tmp_path):
    """Point XIUQO_HOME at a temp dir so tests never touch real config."""
...
class TestListNavigation:
    """xiuqo config set must preserve YAML list fields when using numeric
...
class TestCronModelChangeNotice:
    ...
    def test_notice_says_jobs_keep_running_and_names_the_user_owned_pin_path(
        ...
        assert "xiuqo cron edit <job_id> --provider <provider> --model <model>" in notice
...
class TestSecretRedactionInDisplay:
    ...
    def test_redact_config_value_masks_nested_api_key(self):
        from xiuqo_cli.config import redact_config_value
...
class TestSchemaValidation:
    """#34067: ``xiuqo config set`` must not report bare success for
    ...
    a post-write notice warns that Xiuqo may never read it and suggests the
    ...
    def test_desktop_macos_signing_identity_is_accepted(self, _isolated_hermes_home, capsys):
        ...
        set_config_value("desktop.macos_signing_identity", "Xiuqo Local Signing")
        import yaml
        saved = yaml.safe_load(_read_config(_isolated_hermes_home))
        assert saved["desktop"]["macos_signing_identity"] == "Xiuqo Local Signing"
        assert "not a recognized config key" not in capsys.readouterr().out
...
class TestDisplaySkinTouch:
    ...
    (`xiuqo config set display.skin X` while it is already X — the recovery
...
class TestMappingGuard:
    """``xiuqo config set <section> <scalar>`` must not silently destroy an
    ...
    def test_bare_model_shorthand_preserves_siblings(self, _isolated_hermes_home):
        """xiuqo config set model <id> → model.default, siblings survive."""
        self._write_config(_isolated_hermes_home, {
            "model": {
                "default": "gpt-4o",
                "provider": "openai-api",
    ...
    def test_non_model_mapping_is_refused(self, _isolated_hermes_home):
        """xiuqo config set terminal bash → refuse, terminal has sub-keys."""
        self._write_config(_isolated_hermes_home, {
            "terminal": {
                "backend": "docker",
    ...
    def test_non_model_mapping_force_overwrites(self, _isolated_hermes_home):
        """xiuqo config set --force terminal bash → proceed, section wiped."""
        self._write_config(_isolated_hermes_home, {
            "terminal": {
                "backend": "docker",
                "shell": "bash",
    ...
    def test_model_force_overwrites_entire_section(self, _isolated_hermes_home):
        """xiuqo config set --force model <id> → overwrite entire section."""
        self._write_config(_isolated_hermes_home, {
            "model": {
                "default": "gpt-4o",
                "provider": "openai-api",
...
class TestLiteralDotKeyEscaping:
    """``xiuqo config set/unset/get`` must not split a key segment on a
    ...
    def test_split_key_path_escaped_dot(self):
        from xiuqo_cli.config import _split_key_path
