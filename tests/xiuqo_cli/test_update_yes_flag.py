"""Tests for `xiuqo update --yes / -y` — assume yes for interactive prompts.
...
def _isolate_update(isolated_update_runtime, monkeypatch):
    import shutil
    from xiuqo_cli import managed_uv, update_cmd
...
class TestUpdateYesConfigMigration:
    ...
    @patch("xiuqo_cli.update_cmd._reload_config_modules")
    @patch("xiuqo_cli.update_cmd._run_migrate_config_fresh")
    @patch("xiuqo_cli.update_cmd._run_config_check_fresh", return_value=(1, 2))
    @patch("xiuqo_cli.config.get_missing_config_fields", return_value=[])
    @patch("xiuqo_cli.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``xiuqo_cli.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``xiuqo_cli.main`` first could leave
...
class TestUnicodeDecodeErrorInUpdatePrompts:
    ...
    the exception escape and crash `xiuqo update` mid-flight.
    """
    ...
    def test_upstream_remote_prompt_unicode_decode_error_falls_through_to_skip(
        ...
        from xiuqo_cli.update_cmd import _sync_with_upstream_if_needed

        with patch(
            "xiuqo_cli.update_cmd._has_upstream_remote", return_value=False
        ), patch(
            "xiuqo_cli.update_cmd._should_skip_upstream_prompt", return_value=False
        ), patch(
            "xiuqo_cli.update_cmd._add_upstream_remote"
        ) as mock_add, patch(
            "builtins.input",
            side_effect=UnicodeDecodeError("utf-8", b"\xff", 0, 1, "invalid byte"),
        ):
