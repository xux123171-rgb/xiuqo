...
against a temp ``XIUQO_HOME`` with the network catalog fetch stubbed.
"""
...
from xiuqo_cli.auth import _save_auth_store
...
def guest_home(monkeypatch, tmp_path):
    ...
    from xiuqo_cli import model_switch_providers as msp
    ...
    return Path(os.environ["XIUQO_HOME"])
...
def _cli_nous_rows(config):
    from xiuqo_cli.main_provider_setup import _build_provider_picker_rows
...
def test_guest_identity_shows_free_tier_row_with_only_welcome_model(guest_home, monkeypatch):
    ...
    # The `xiuqo model` provider picker applies the same rule from the same helper.
