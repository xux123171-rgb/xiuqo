...
from xiuqo_cli.dashboard_procs import _is_desktop_local_serve_cmdline
from xiuqo_cli.update_cmd_windows import _hermes_holder_subcommand, _is_backend_argv
...
# value. The updater's kill set additionally requires the Desktop's `-m xiuqo_cli.main` spawn shape —
# a user-launched `xiuqo serve` / `xiuqo dashboard` is refused on, never tree-killed.
CMDLINES = [
    ("python -m xiuqo_cli.main serve " + LOOPBACK, "serve", True, True),
    ("python -m xiuqo_cli.main dashboard", "dashboard", False, True),
    ("/venv/bin/xiuqo serve --isolated --host=127.0.0.1 --port=0 --ssh-owner-nonce abc", "serve", True, False),
    (r"C:\xiuqo\.venv\Scripts\xiuqo.exe serve --host 100.106.105.2 --port 9119", "serve", False, False),
    ("xiuqo.exe dashboard", "dashboard", False, False),
    ("xiuqo --profile ops serve " + LOOPBACK, "serve", True, False),
    ("xiuqo -m serve kanban --preserve-cache " + LOOPBACK, "kanban", False, False),
    ("python -m xiuqo_cli.main kanban --preserve-cache " + LOOPBACK, "kanban", False, False),
    ("xiuqo --reasoning high dashboard " + LOOPBACK, "dashboard", False, False),
    ("xiuqo gateway run --replace", "gateway", False, False),
    ("xiuqo chat --model serve", "chat", False, False),
...
def test_desktop_local_serve_spares_fixed_port_and_remote_hosts():
    assert not _is_desktop_local_serve_cmdline("xiuqo serve --host 100.106.105.2 --port 9119 --skip-build")
    assert not _is_desktop_local_serve_cmdline("xiuqo serve --host 127.0.0.1 --port 9119")
    assert _is_desktop_local_serve_cmdline("xiuqo serve --host localhost --port 0")
