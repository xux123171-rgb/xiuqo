"""xiuqo_cli.plugin_compat: detect plugins on old import paths, tell the user, disable after the date.

Kept with the compat layer (tests/xiuqo_cli/test_compat_manifest_targets.py); deleted with it.
"""
...
from xiuqo_cli import plugin_compat as pc
...
MANIFEST = {"tools.web_tools": {"prefers_gateway": "tools.tool_backend_helpers.prefers_gateway"},
            "xiuqo_cli.kanban_db": {"connect": "xiuqo_cli.kanban_db_connect.connect"}}
...
@pytest.mark.parametrize("src, expect", [
    ...
    ("from unittest.mock import patch\npatch('xiuqo_cli.kanban_db.connect')\n", ["xiuqo_cli.kanban_db.connect"]),
...
def test_scan_plugin_walks_dir_and_skips_tests(tmp_path):
    ...
    (tmp_path / "sub").mkdir(); (tmp_path / "sub" / "m.py").write_text("import xiuqo_cli.kanban_db as k\nk.connect()\n")
...
def test_disable_only_after_the_date_and_not_when_allowed(tmp_path, monkeypatch):
    ...
    assert reason and pc.COMPAT_REMOVAL in reason and "xiuqo plugins compat" in reason
...
def test_summary_lines_name_plugins_and_the_date():
    ...
    assert "xiuqo plugins compat" in tail
...
def test_loader_skips_hitting_plugin_after_date(tmp_path, monkeypatch):
    ...
    from xiuqo_cli.plugins_manifest import PluginManifest
