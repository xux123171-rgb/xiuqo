"""Tests for the stale-dashboard handling run at the end of ``xiuqo update``.

``xiuqo update`` detects ``xiuqo dashboard`` processes left over from the
...
from xiuqo_cli.main_dashboard import _find_stale_dashboard_pids
from xiuqo_cli.dashboard_procs import _kill_stale_dashboard_processes
from xiuqo_cli import dashboard_procs
from xiuqo_cli import main_dashboard
from xiuqo_cli import update_cmd
from xiuqo_cli.update_cmd import _finish_dashboard_update_cleanup
from xiuqo_cli.main_dashboard import _restart_managed_dashboard_service
from xiuqo_cli.dashboard_procs import _kill_stale_dashboard_processes as _warn_stale_dashboard_processes
...
def _write_valid_ssh_backend_lock(tmp_path, monkeypatch) -> int:
    ...
    (lock_dir / "backend.lock.json").write_text(json.dumps({
        ...
        "port": 46369,
        "profile": "default",
        "hermesPath": "/opt/xiuqo/bin/xiuqo",
    ...
    monkeypatch.delenv("XIUQO_DESKTOP_CHILD_PID", raising=False)
    return pid
...
def test_update_cleanup_spares_backend_owned_by_valid_ssh_lock(tmp_path, monkeypatch):
    ...
    with patch(
        "xiuqo_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m xiuqo_cli.main dashboard"),
                    _ps_line(12345, "xiuqo dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("xiuqo_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "xiuqo-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="xiuqo-dashboard.service enabled enabled\n", stderr="")
            if args == ["systemctl", "--user", "is-active", "xiuqo-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args == ["systemctl", "--user", "is-enabled", "xiuqo-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "--user", "restart", "xiuqo-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("xiuqo_cli.main_dashboard._find_stale_dashboard_pids", return_value=[]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert calls == [
            ["systemctl", "--user", "list-unit-files", "xiuqo-dashboard.service", "--no-legend", "--no-pager"],
            ["systemctl", "--user", "is-active", "xiuqo-dashboard.service"],
            ["systemctl", "--user", "is-enabled", "xiuqo-dashboard.service"],
            ["systemctl", "--user", "restart", "xiuqo-dashboard.service"],
        ]
        ...
        assert "✓ restarted xiuqo-dashboard.service" in capsys.readouterr().out
...
class TestKillStaleDashboardWindows:
    ...
    def test_taskkill_invoked_for_each_pid(self, capsys):
        ...
        with patch("xiuqo_cli.main_dashboard._find_stale_dashboard_pids",
             ...
             patch("xiuqo_cli._subprocess_compat.pid_is_hermes", return_value=True), \
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys):
        with patch(
            "xiuqo_cli.main._kill_stale_dashboard_processes",
...
class TestWindowsWmicEncoding:
    ...
    `xiuqo update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("xiuqo_cli._subprocess_compat.bounded_probe_run") as mock_probe:
            mock_probe.return_value = subprocess.CompletedProcess(
                args=["wmic"],
                returncode=0,
                stdout=(
                    "CommandLine=python -m xiuqo_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_probe_failure_fails_open_to_empty_list(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("xiuqo_cli._subprocess_compat.bounded_probe_run",
                   return_value=None):
            assert _find_stale_dashboard_pids() == []
...
class TestSupervisedBackendRestart:
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch.object(main_dashboard, "_get_pid_cgroup_path",
                          return_value="/system.slice/xiuqo-serve.service"), \
             patch.object(main_dashboard, "_get_systemd_service_for_pid",
                          return_value="xiuqo-serve.service"), \
        ...
        restart.assert_called_once_with(
            "xiuqo-serve.service", "/system.slice/xiuqo-serve.service"
        )
        out = capsys.readouterr().out
        assert "✓ restarted systemd service xiuqo-serve.service" in out
    ...
    def test_already_restarted_unit_is_left_untouched(self):
        """Review on #83595: xiuqo update's systemd fleet-restart loop may
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("time.sleep"):
            result = _kill_stale_dashboard_processes(
                restart_managed=True, already_restarted_units={"xiuqo-serve"}
            )
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        live = self._live()
        argv = ["xiuqo", "dashboard", "--port", "8300"]
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("xiuqo_cli.dashboard_procs._hermes_home_for_pid", return_value=None), \
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        with patch.object(live.subprocess, "Popen", _FakePopen):
            failed = live._respawn_dashboard_processes([
                ["xiuqo", "dashboard", "--port", "8300"],
                ["xiuqo", "serve", "--host", "0.0.0.0"],
            ])

        assert failed == []
        assert spawned[0] == ["xiuqo", "dashboard", "--port", "8300", "--no-open"]
        assert spawned[1] == ["xiuqo", "serve", "--host", "0.0.0.0"]
    ...
    def test_respawn_failure_returned(self, tmp_path, monkeypatch, capsys):
        ...
        with patch.object(live.subprocess, "Popen", side_effect=OSError("no such file")):
            failed = live._respawn_dashboard_processes([["xiuqo", "serve"]])

        assert failed == [["xiuqo", "serve"]]
        out = capsys.readouterr().out
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        from xiuqo_cli.dashboard_procs import _filter_dashboard_respawn_candidates
        ...
        assert _filter_dashboard_respawn_candidates([
            (42, argv, "/home/u/.xiuqo/profiles/mini-cat"),
        ]) == []
    ...
    def test_keeps_ppid1_fixed_port_for_repeat_update(self):
        ...
        argv = ["xiuqo", "dashboard", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([(10, argv, None)]) == [argv]

    def test_dedupes_identical_normalized_cmdlines(self):
        from xiuqo_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        a = ["/usr/bin/python3", "-m", "xiuqo_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "xiuqo_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_caps_one_per_profile(self):
        ...
        a = ["xiuqo", "--profile", "coder", "dashboard", "--port", "8300"]
        b = ["xiuqo", "--profile", "coder", "dashboard", "--port", "8301"]
        c = ["xiuqo", "--profile", "writer", "dashboard", "--port", "8302"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, None),
            (3, c, None),
    ...
    def test_caps_one_per_hermes_home(self):
        ...
        home = "/tmp/xiuqo-home-a"
        a = ["xiuqo", "dashboard", "--port", "8300"]
        b = ["xiuqo", "dashboard", "--port", "8301"]
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, home),
                (2, b, home),
    ...
    def test_profile_flag_and_profiles_home_share_cap(self):
        ...
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, "/home/u/.xiuqo/profiles/coder"),
        ])
        assert out == [a]
    ...
    def test_default_profile_same_root_home_caps(self):
        ...
        a = ["xiuqo", "--profile", "default", "dashboard", "--port", "8300"]
        b = ["xiuqo", "dashboard", "--port", "8301"]
        home = "/home/u/.xiuqo"
        out = _filter_dashboard_respawn_candidates(
            [
    ...
    def test_foreign_home_backend_is_not_replayed(self):
        """A backend from another XIUQO_HOME is never respawned (#94030).
        ...
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, "/home/u/.xiuqo"),
                (2, b, "/work/project/.xiuqo"),
            ],
            own_home="/home/u/.xiuqo",
        )
        assert out == [a]
    ...
    def test_skips_sidecar_fixed_port_serve_on_foreign_home(self):
        ...
        argv = [
            "/Users/u/.xiuqo-sidecar/xiuqo/venv/bin/python",
            "-m", "xiuqo_cli.main",
            "serve", "--host", "127.0.0.1", "--port", "9118", "--skip-build",
        ]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.xiuqo-lifeos")],
            own_home="/Users/u/.xiuqo",
        )
        assert out == []
    ...
    def test_matching_hermes_home_is_kept(self):
        ...
        argv = ["xiuqo", "serve", "--host", "127.0.0.1", "--port", "9118"]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.xiuqo")],
            own_home="/Users/u/.xiuqo",
        )
        assert out == [argv]
    ...
    def test_unknown_home_stays_eligible(self):
        """Unreadable XIUQO_HOME (env probe failed) keeps pre-#94030 behaviour."""
        from xiuqo_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_own_home_defaults_to_get_hermes_home(self, monkeypatch):
        ...
        monkeypatch.setattr(
            xiuqo_constants, "get_hermes_home", lambda: Path("/home/u/.xiuqo")
        )
        argv = ["xiuqo", "serve", "--port", "9118"]
        foreign = _filter_dashboard_respawn_candidates([
            (1, argv, "/Users/u/.xiuqo-lifeos"),
        ])
        assert foreign == []
    ...
    def test_keeps_fixed_port_serve(self):
        ...
        argv = ["xiuqo", "serve", "--host", "0.0.0.0", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([
            (9, argv, None),
        ]) == [argv]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00xiuqo_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "xiuqo_cli.main", "serve"]
...
class TestPostUpdateStaleModuleReload:
    ...
    ``xiuqo update`` runs in the PRE-pull Python process. When the update
    adds a new symbol to ``xiuqo_cli._subprocess_compat`` (as #87134 added
    ``bounded_probe_run``), the post-update dashboard cleanup's lazy
    ``from xiuqo_cli._subprocess_compat import bounded_probe_run`` hits the
    ...
    def test_node_failures_skip_reload_and_kill(self):
        ...
        with patch.object(update_cmd, "_reload_process_scan_modules") as mock_reload, \
             patch("xiuqo_cli.main._kill_stale_dashboard_processes") as mock_kill:
    ...
    def test_reload_restores_missing_symbol(self):
        ...
        import xiuqo_cli._subprocess_compat as compat
        from xiuqo_cli import update_cmd
        ...
        try:
            ...
            stale = sys.modules["xiuqo_cli._subprocess_compat"]
            assert hasattr(stale, "bounded_probe_run")
        finally:
            importlib.reload(sys.modules["xiuqo_cli._subprocess_compat"])
            importlib.reload(sys.modules["xiuqo_cli.dashboard_procs"])
    ...
    def test_config_reload_list_includes_process_scan_modules(self):
        ...
        assert "xiuqo_cli._subprocess_compat" in reloaded
        assert "xiuqo_cli.dashboard_procs" in reloaded
