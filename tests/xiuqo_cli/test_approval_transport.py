...
from xiuqo_cli.plugins import PluginContext, PluginManager, PluginManifest
...
def _request():
    from xiuqo_cli.approval_transport import ApprovalRequest
...
def test_transport_registry_is_manager_and_profile_isolated(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("XIUQO_HOME", str(tmp_path / "profiles" / "work"))
    ...
    monkeypatch.setenv("XIUQO_HOME", str(tmp_path / "profiles" / "personal"))
    assert first.get_approval_transport("phone") is None


def test_host_accepts_bound_sync_and_async_decisions():
    from xiuqo_cli.approval_transport import invoke_approval_transport
...
def test_host_rejects_scope_not_offered_by_request():
    from xiuqo_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def test_host_rejects_decision_completed_after_deadline(monkeypatch):
    import xiuqo_cli.approval_transport as transport_module
...
def test_host_caps_hung_transport_workers():
    from xiuqo_cli.approval_transport import (
...
def test_live_temp_home_fixture_plugin_routes_and_hardline_stays_core_owned(
    ...
    """Real discovery + config + guard path under an isolated XIUQO_HOME."""
    import xiuqo_cli.plugins as plugins_module
    from tools import approval
...
def present(request):
    output = Path(os.environ["XIUQO_HOME"]) / "transport-invocations.jsonl"
...
"""
    ...
    try:
        routed = approval.check_all_command_guards(
            "rm -rf /tmp/xiuqo-approval-transport-fixture", "local"
        )
        manager.discover_and_load(force=True)
        reloaded = approval.check_all_command_guards(
            "rm -rf /tmp/xiuqo-approval-transport-fixture-reloaded", "local"
        )
        ...
        try:
            gateway_routed = approval.check_all_command_guards(
                "rm -rf /tmp/xiuqo-approval-transport-fixture-gateway", "local"
            )
        finally:
