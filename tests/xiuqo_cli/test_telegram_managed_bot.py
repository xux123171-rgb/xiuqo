"""Tests for xiuqo_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "xiuqo_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/HermesSetupBot/xiuqo_abcdefghijklmnop_bot?name=Xiuqo+Agent",
            "qr_payload": "https://t.me/newbot/HermesSetupBot/xiuqo_abcdefghijklmnop_bot?name=Xiuqo+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "xiuqo_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="Xiuqo Agent")
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="xiuqo_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/HermesSetupBot/xiuqo_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/HermesSetupBot/xiuqo_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "xiuqo_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "xiuqo_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("xiuqo_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_eventual_success(self):
        ...
        with patch("xiuqo_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("xiuqo_cli.telegram_managed_bot.time.sleep"):
                result = poll_for_setup_result(
...
class TestSetupTelegramAuto:
    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        from xiuqo_cli import setup

        seen = {}
        profile_home = tmp_path / ".xiuqo" / "profiles" / "oracle"
        ...
        monkeypatch.setattr(
            "xiuqo_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from xiuqo_cli.setup_platforms import _profile_name_from_hermes_home

        assert (
            _profile_name_from_hermes_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\xiuqo\profiles\oracle")
            )
            == "oracle"
        )
