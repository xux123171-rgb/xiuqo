...
from xiuqo_cli import gateway, main, update_cmd_fleet as fleet
...
def test_pending_marker_requires_complete_systemd_recovery(monkeypatch, tmp_path, failure):
    ...
    def systemctl(cmd, **kw):
        if "list-units" in cmd:
            ...
            return SimpleNamespace(returncode=int(failure == "listing"), stdout=(
                "xiuqo-gateway-one.service loaded active running\n"
                "xiuqo-gateway-two.service loaded failed failed\n"), stderr="")
        bad = cmd[-1] == "xiuqo-gateway-two"
        if "restart" in cmd:
    ...
    if failure not in (None, "running"):
        ...
        assert set(recovered) == {"xiuqo-gateway-one", "xiuqo-gateway-two"}
...
def test_pending_launchd_requires_complete_supervision(monkeypatch, tmp_path, failure):
    ...
    current, sibling = "ai.xiuqo.gateway", "ai.xiuqo.gateway-two"
