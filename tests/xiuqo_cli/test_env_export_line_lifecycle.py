...
``~/.xiuqo/.env``. ``load_env()`` parses the export prefix (#6659), so every
...
def test_classic_pat_save_via_endpoint_succeeds(xiuqo_home):
...
def test_remove_export_prefixed_token(xiuqo_home):
    """DELETE must clear an ``export KEY=...`` line, not 404 on it."""
    _write_env_raw(xiuqo_home, f"export GITHUB_TOKEN={OLD_PAT}\n")
    ...
    env_text = xiuqo_home.joinpath(".env").read_text(encoding="utf-8")
...
def test_update_export_prefixed_token_does_not_duplicate(xiuqo_home):
...
def test_plain_line_save_and_remove_still_work(xiuqo_home):
    """Sanity: the ordinary KEY= path is unchanged."""
    from xiuqo_cli.config import load_env, remove_env_value, save_env_value
...
def test_export_line_with_comment_untouched(xiuqo_home):
...
def test_remove_token_written_with_spaces_around_equals(xiuqo_home):
    ...
    """
    _write_env_raw(xiuqo_home, f"GITHUB_TOKEN = {OLD_PAT}\n")
...
def test_spaced_token_rotate_then_delete_does_not_resurrect(xiuqo_home):
...
def test_writer_matches_exactly_what_load_env_accepts(xiuqo_home):
    """Parity: the writers must recognise a line iff ``load_env()`` does."""
    from xiuqo_cli.config import _env_line_defines_key, load_env
    ...
    for line, expected in [
        ...
        _write_env_raw(xiuqo_home, line + "\n")
