...
which broke ``xiuqo --tui`` because ``ui-tui/`` had vanished.
...
from xiuqo_cli.update_cmd import _atomic_replace_dir
...
def test_atomic_replace_swaps_content_on_success(tmp_path: Path) -> None:
    ...
    assert not (dst.parent / "ui-tui.xiuqo-update-staging").exists()
    assert not (dst.parent / "ui-tui.xiuqo-update-old").exists()
