...
stale grafts break ``merge-base`` and push ``xiuqo update`` into the
orphan-divergence reset path. ``prune_stale_shallow_grafts()`` drops grafts no
live ref points at; ``xiuqo update --check`` calls it after its successful
...
from xiuqo_cli.gitlock import prune_stale_shallow_grafts
...
def test_update_check_prunes_and_reports_count(tmp_path, monkeypatch, capsys):
    """`xiuqo update --check` prunes grafts after its depth-1 fetch and reports the prune."""
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(
        "xiuqo_cli.update_contract.evaluate_update_admission", lambda root: None
    )
    ...
    monkeypatch.setattr("xiuqo_cli.banner._github_compare_behind", lambda *a, **k: 0)
    prune_calls = []
    monkeypatch.setattr(
        "xiuqo_cli.gitlock.prune_stale_shallow_grafts",
