...
External secret backends are useful during normal Xiuqo startup, but the
...
""",
    ...
    dispatch = (
        "xiuqo_main.cmd_update = lambda _args: 0\n"
        "xiuqo_main.main()\n"
        if run_main
        else ""
    )
    probe = (
        "import json, sys\n"
        f"sys.argv = {argv!r}\n"
        "import xiuqo_cli.main as xiuqo_main\n"
        f"{dispatch}"
    ...
    result = subprocess.run(
        ...
        timeout=120,
        cwd=REPO_ROOT,
        env={**os.environ, "XIUQO_HOME": str(home), "BWS_ACCESS_TOKEN": ""},
    )
...
def test_update_startup_does_not_import_bitwarden_or_cryptography(tmp_path):
    loaded = _probe_startup_modules(tmp_path, ["xiuqo", "update", "--check"])
...
def test_normal_startup_still_loads_enabled_external_secret_source(tmp_path):
    loaded = _probe_startup_modules(tmp_path, ["xiuqo", "chat"])
