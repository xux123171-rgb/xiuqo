"""Tests for the unified provider catalog (xiuqo_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``xiuqo model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from xiuqo_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
