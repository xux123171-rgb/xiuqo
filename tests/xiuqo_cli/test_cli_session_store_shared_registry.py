...
managers acquire the same path through ``xiuqo_state_registry`` from the REPL thread a
...
import xiuqo_cli.goals as goals
...
def test_cli_session_store_is_the_registry_handle_goals_reuse(monkeypatch):
    ...
    constructed = []
    real_open = xiuqo_state_registry._open_session_db

    def recording_open(path):
        constructed.append(path)
        return real_open(path)

    monkeypatch.setattr(xiuqo_state_registry, "_open_session_db", recording_open)
    ...
    try:
        ...
        xiuqo_state_registry.close_all()
