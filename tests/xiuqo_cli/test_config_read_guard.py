...
Canonical owners:
  ...
  * ``xiuqo_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/xiuqo/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "xiuqo_cli/managed_scope.py",
