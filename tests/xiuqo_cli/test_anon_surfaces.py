...
Contract (R-USR-1): wherever a free-tier identity renders (``xiuqo auth status nous``,
``xiuqo auth list``, ``xiuqo status``, ``xiuqo portal info``) the user sees the free-tier label
...
from xiuqo_cli.auth import _load_auth_store  # noqa: F401  (store import name kept for parity with core tests)
...
def _account_state() -> dict:
    return {
        "auth_method": "oauth_device_code", "client_id": "xiuqo-cli",
        "access_token": _jwt(sub="nas_user:real", client_id="xiuqo-cli", account_tier="standard", paid_access=True),
...
def _render_all(capsys) -> dict[str, str]:
    ...
    out["xiuqo status"] = capsys.readouterr().out
...
def test_every_in_chat_free_tier_string_names_the_slash_command(monkeypatch):
    ...
    monkeypatch.setattr("xiuqo_cli.auth.resolve_provider", lambda _requested: "nous")
    ...
    for text in (*command_copy, *refusal_copy):
        # The ruled refusal uses Xiuqo as the grammatical subject; only that exact product-name
        # phrase is exempt from the broad top-level-command gate.
        assert "xiuqo " not in text.replace("this Xiuqo can", "this product can").lower()
...
def test_no_chat_copy_of_any_sign_in_state_leaks_a_terminal_verb_or_a_forbidden_word():
    ...
    terminal_or_url = re.compile(r"xiuqo |https?://", re.IGNORECASE)
...
def test_terminal_only_strings_keep_the_terminal_verb(monkeypatch, capsys):
    assert "xiuqo " in nous_account.FREE_TIER_NEEDS_ACCOUNT
    assert "xiuqo " in anon_auth.UPGRADE_UNAVAILABLE
    assert "xiuqo " in anon_auth.FREE_TIER_NOT_SIGNED_IN

    monkeypatch.setattr("xiuqo_cli.auth.get_provider_auth_state", lambda _provider: {"access_token": "token"})
    monkeypatch.setattr("xiuqo_cli.model_switch_providers._free_tier_nous_row", lambda _provider: None)
    model_setup_flows._model_flow_nous({})
    assert "xiuqo " in capsys.readouterr().out
...
async def test_the_wait_line_is_only_composed_on_the_state(monkeypatch):
    ...
    monkeypatch.setattr(
        "xiuqo_cli.auth_device_flow._print_device_code_instructions", lambda *_args, **_kwargs: None
    )
    cli_copy = []
    ...
    for rel in ("gateway/slash_commands_login.py", "xiuqo_cli/cli_commands_mixin.py"):
