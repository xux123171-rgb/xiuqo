...
profile's XIUQO_HOME, and the dashboard's own profile stays untouched.
"""
import pytest
import yaml
...
class TestProfileScopedMessagingReads:
    ...
    def test_scoped_read_returns_profile_path_command_and_startup_failure(
        ...
        monkeypatch.setattr(
            ...
            # relying on process-level XIUQO_HOME resolution (#71211).
            lambda *a, **k: {
        ...
        assert payload["gateway_start_command"] == (
            "xiuqo -p worker_alpha gateway start"
        )
...
def test_named_current_home_matches_unscoped(client, isolated_profiles, monkeypatch):
    from xiuqo_cli.web_server_profiles import _config_profile_scope, _hermes_home_scope
    from xiuqo_constants import get_hermes_home
...
def test_scoped_enablement_uses_only_own_credentials(client, isolated_profiles, monkeypatch):
    ...
    from xiuqo_cli.web_server_messaging import _messaging_platform_catalog
...
def test_credential_write_hot_serves_a_multiplexed_profile(client, isolated_profiles, monkeypatch, topology):
    ...
    ``?profile=`` and a pooled ``xiuqo --profile X serve`` that receives the PUT unscoped (#109088)."""
    import xiuqo_cli.gateway as gateway_cli
    import xiuqo_cli.gateway_multiplex_served as served_mod
    notified = []
