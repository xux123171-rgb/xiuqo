"""Tests for xiuqo_cli.agent_import — ``xiuqo import-agent``.

Covers: source detection, Claude Code and Codex parsing, mapping into the
real Xiuqo stores (memories/MEMORY.md, config.yaml command_allowlist /
...
Uses the profile_env fixture pattern from tests/xiuqo_cli/test_profiles.py:
Path.home() and XIUQO_HOME are redirected to tmp_path so nothing touches
the real ~/.xiuqo.
"""
...
from xiuqo_cli.agent_import import (
    ENTRY_DELIMITER,
    AgentImporter,
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and XIUQO_HOME (profile_env pattern)
# ---------------------------------------------------------------------------

@pytest.fixture()
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() -> tmp_path, XIUQO_HOME -> tmp/.xiuqo."""
...
@pytest.fixture()
def xiuqo_home(profile_env):
    return profile_env / ".xiuqo"
...
def run_import(agent, source, xiuqo_home, execute, overwrite=False):
    return AgentImporter(
        agent=agent,
        source_root=source,
        target_root=xiuqo_home,
        execute=execute,
        overwrite=overwrite,
    ).run()
...
class TestDetection:
    ...
    def test_unsupported_agent_raises(self, xiuqo_home, tmp_path):
        with pytest.raises(ValueError):
            AgentImporter("cursor", tmp_path, xiuqo_home)
...
class TestDryRun:
    def test_claude_dry_run_writes_nothing(self, claude_tree, xiuqo_home):
        before = snapshot_tree(xiuqo_home)
        report = run_import("claude-code", claude_tree, xiuqo_home, execute=False)
        assert snapshot_tree(xiuqo_home) == before
        assert report["dry_run"] is True
        assert report["summary"]["imported"] > 0


    def test_dry_run_and_real_run_plan_same_items(self, claude_tree, xiuqo_home):
        preview = run_import("claude-code", claude_tree, xiuqo_home, execute=False)
        applied = run_import("claude-code", claude_tree, xiuqo_home, execute=True)
...
class TestClaudeCodeImport:
    @pytest.fixture()
    def report(self, claude_tree, xiuqo_home):
        return run_import("claude-code", claude_tree, xiuqo_home, execute=True)


    def test_allowlist_lands_in_config_yaml(self, report, xiuqo_home):
        config = yaml.safe_load((xiuqo_home / "config.yaml").read_text())
...
class TestCodexImport:
    @pytest.fixture()
    def report(self, codex_tree, xiuqo_home):
        return run_import("codex", codex_tree, xiuqo_home, execute=True)


    def test_mcp_servers_from_config_toml(self, report, xiuqo_home):
        config = yaml.safe_load((xiuqo_home / "config.yaml").read_text())
    ...
    def test_skill_copied(self, report, xiuqo_home):
        assert (xiuqo_home / "skills" / "codex-imports" / "db-migrate" / "SKILL.md").exists()
...
class TestSecretsNeverImported:
    def test_no_secret_values_anywhere_claude(self, claude_tree, xiuqo_home):
        run_import("claude-code", claude_tree, xiuqo_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in xiuqo_home.rglob("*") if p.is_file()
        )
        assert "ghp_SECRET123" not in blob
        assert "Bearer abc123" not in blob
        assert "sk-ant-SUPERSECRET" not in blob

    def test_no_secret_values_anywhere_codex(self, codex_tree, xiuqo_home):
        run_import("codex", codex_tree, xiuqo_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in xiuqo_home.rglob("*") if p.is_file()
        )
    ...
    def test_stripped_secrets_reported(self, claude_tree, xiuqo_home):
        report = run_import("claude-code", claude_tree, xiuqo_home, execute=True)
    ...
    def test_non_secret_header_kept(self, claude_tree, xiuqo_home):
        run_import("claude-code", claude_tree, xiuqo_home, execute=True)
        config = yaml.safe_load((xiuqo_home / "config.yaml").read_text())
...
class TestMalformedInputs:
    def test_bad_settings_json_reports_error(self, profile_env, xiuqo_home):
        root = profile_env / ".claude"
        root.mkdir()
        (root / "settings.json").write_text("{not json!!", encoding="utf-8")
        (root / "CLAUDE.md").write_text("- still importable\n", encoding="utf-8")
        report = run_import("claude-code", root, xiuqo_home, execute=True)
        ...
        assert "still importable" in (
            xiuqo_home / "memories" / "MEMORY.md").read_text()


    def test_empty_tree_all_skipped(self, profile_env, xiuqo_home):
        root = profile_env / ".codex"
        root.mkdir()
        report = run_import("codex", root, xiuqo_home, execute=True)
        assert report["summary"]["imported"] == 0
...
class TestMergeSemantics:
    ...
    def test_existing_skill_conflicts_without_overwrite(
            self, claude_tree, xiuqo_home):
        dest = xiuqo_home / "skills" / "claude-code-imports" / "deploy-helper"
    ...
    def test_reimport_is_idempotent_for_memory(self, claude_tree, xiuqo_home):
        run_import("claude-code", claude_tree, xiuqo_home, execute=True)
        first = (xiuqo_home / "memories" / "MEMORY.md").read_text()
        report = run_import("claude-code", claude_tree, xiuqo_home, execute=True)
        assert (xiuqo_home / "memories" / "MEMORY.md").read_text() == first
...
class TestExistingMemoryStorePreserved:
    ...
    @pytest.fixture()
    def seeded_home(self, xiuqo_home):
        memory = xiuqo_home / "memories" / "MEMORY.md"
...
class TestExistingConfigPreserved:
    ...
    @pytest.fixture()
    def config_path(self, xiuqo_home):
        return xiuqo_home / "config.yaml"
    ...
    def test_malformed_config_is_left_byte_identical(
            self, claude_tree, xiuqo_home, config_path):
    ...
    def test_malformed_config_records_an_error_not_an_import(
        ...
        for kind in CONFIG_WRITING_KINDS:
            ...
            assert "xiuqo config edit" in reason
    ...
    def test_unreadable_config_is_left_byte_identical(
        ...
        try:
            report = run_import("claude-code", claude_tree, xiuqo_home,
                                execute=True)
            statuses = {
                i["status"]
                for kind in CONFIG_WRITING_KINDS
    ...
    def test_config_write_is_atomic_and_leaves_no_temp_files(
        ...
        leftovers = [p.name for p in xiuqo_home.glob(".tmp*")]
        assert leftovers == []

    def test_symlinked_config_stays_a_symlink(
            self, claude_tree, xiuqo_home, tmp_path, config_path):
    ...
    def test_failed_write_does_not_truncate_the_existing_config(
            self, xiuqo_home, config_path, monkeypatch):
        """An interrupted dump leaves the previous file complete."""
        from xiuqo_cli import agent_import
...
class TestCliWiring:
    ...
    def test_command_dry_run_via_cli_writes_nothing(
            self, claude_tree, xiuqo_home, capsys):
        """End-to-end through import_agent_command with --dry-run."""
        import types
        from xiuqo_cli.agent_import import import_agent_command
        ...
        assert not (xiuqo_home / "memories" / "MEMORY.md").exists()
        assert not (xiuqo_home / "skills" / "claude-code-imports").exists()
        config_text = (xiuqo_home / "config.yaml").read_text(encoding="utf-8") \
            if (xiuqo_home / "config.yaml").exists() else ""
