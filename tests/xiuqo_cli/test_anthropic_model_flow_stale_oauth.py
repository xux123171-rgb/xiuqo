"""Tests for Bug #12905 fix — stale OAuth token detection in xiuqo model flow.

Bug 3: `xiuqo model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.xiuqo/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("xiuqo_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from xiuqo_cli.model_setup_flows import _model_flow_anthropic
        cfg = {}
