...
from xiuqo_cli.web_routers import memory_providers as mp
...
def _point_at(monkeypatch, path):
    ...
    monkeypatch.setattr(mp, "_honcho_resolvers", lambda: (lambda: "xiuqo", lambda: path, _host_block))
...
def test_web_save_never_replaces_an_unparseable_honcho_json(tmp_path, monkeypatch, corrupt):
    ...
    assert data["hosts"]["xiuqo"]["recallMode"] == "tools"
