...
This rewrites files Xiuqo does not own (``~/.bashrc``, ``~/.zshrc``, ...) and
...
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point both ``Path.home()`` and ``XIUQO_HOME`` at a throwaway dir."""
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))
    monkeypatch.setenv("XIUQO_HOME", str(home / ".xiuqo"))
    return home
...
class TestHappyPath:
    def test_hermes_path_block_is_removed(self, fake_home: Path):
        ...
        assert "# Xiuqo Agent" not in text
...
class TestCrashDurability:
    def test_shell_config_survives_an_interrupted_rewrite(self, fake_home: Path):
        ...
        # Path.home()/XIUQO_HOME patches the fake_home fixture installed.
    ...
    def test_symlinked_shell_config_stays_a_symlink(self, fake_home: Path):
        ...
        assert "# Xiuqo Agent" not in real.read_text(encoding="utf-8")
