...
A ``xiuqo update`` killed after git pull advanced HEAD but before the
...
- next ``xiuqo update`` with git already up to date still runs the
...
import xiuqo_cli.update_cmd_fleet as update_cmd_fleet
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(xiuqo_main, "_resolve_update_branch", lambda args: "main")
    ...
    monkeypatch.setattr(xiuqo_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(xiuqo_main, "_build_web_ui", lambda *a, **k: None)
    ...
    monkeypatch.setattr(xiuqo_main, "_purge_stale_hermes_modules", lambda: None)

    import xiuqo_cli.gateway as xiuqo_gateway

    monkeypatch.setattr(
        xiuqo_gateway, "find_gateway_pids", lambda **_kwargs: []
    )
    monkeypatch.setattr(xiuqo_gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "xiuqo_cli.update_inventory.collect_runtime_inventory",
...
def test_run_pending_restart_true_when_no_gateways(monkeypatch, capsys):
    monkeypatch.setattr(
        "xiuqo_cli.gateway.find_gateway_pids", lambda **k: []
    )
    monkeypatch.setattr(xiuqo_main, "_purge_stale_hermes_modules", lambda: None)
...
def test_clean_update_escalates_surviving_serve_as_unaccounted(
    ...
    ``xiuqo-gateway.service``) and an unmanaged ``serve`` on the same
...
def test_interrupt_between_pull_and_restart_leaves_marker(
    ...
    monkeypatch.setattr(xiuqo_main, "_clear_bytecode_cache", _interrupt)
...
def test_startup_warn_prints_when_marker_present(capsys):
    ...
    assert "xiuqo gateway restart" in err
