"""Tests for issue #26670 — concurrent xiuqo.exe detection and improved
quarantine retry / reboot-deferred fallback during `xiuqo update` on Windows.
...
# helper (and need the autouse stub in tests/xiuqo_cli/conftest.py disabled),
...
def _make_proc(pid: int, exe: str, name: str = "xiuqo.exe"):
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "xiuqo.exe"),
    ]
    ...
    assert result == [(launcher_pid, "xiuqo.exe")]
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    assert orig == shim
    assert quarantine.name.startswith("xiuqo.exe.old.")
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "xiuqo_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "xiuqo_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_pause_and_resume_windows_gateway_service(
    ...
    import xiuqo_cli.update_cmd_windows as update_cmd_windows
...
def test_unreadable_argv_falls_back_to_the_captured_prefix(monkeypatch):
    ...
    gateway_prefix = r"venv\Scripts\python.exe -m xiuqo_cli.main gateway run"
...
# when every concurrent xiuqo.exe is a gateway runtime — the pause
...
def test_classify_concurrent_instance_recognises_gateway_runtimes(monkeypatch):
    ...
    launcher shape (python -m, xiuqo.exe shim, xiuqo-gateway.exe,
    gateway/run.py, bare `xiuqo gateway` which defaults to run)."""
    cases = [
        [r"C:\venv\Scripts\python.exe", "-m", "xiuqo_cli.main", "gateway", "run"],
        [r"C:\venv\Scripts\xiuqo.exe", "gateway", "run"],
        [r"C:\venv\Scripts\xiuqo-gateway.exe"],
        [r"C:\venv\Scripts\python.exe", "gateway/run.py"],
        ["xiuqo.exe", "GATEWAY", "RUN"],  # matcher is case-insensitive
        ["xiuqo.exe", "gateway"],  # bare `xiuqo gateway` defaults to run
        # profile selector before the subcommand — canonical matcher strips it
        ["xiuqo.exe", "--profile", "work", "gateway", "run"],
    ]
    for argv in cases:
...
def test_classify_concurrent_instance_recognises_non_gateways(monkeypatch):
    ...
    cases = [
        [r"C:\venv\Scripts\xiuqo.exe"],  # interactive REPL
        [r"C:\venv\Scripts\xiuqo.exe", "dashboard"],
        ["xiuqo.exe", "gateway", "status"],  # management, not runtime
        ["xiuqo.exe", "gateway", "stop"],
...
def test_filter_non_gateway_concurrent_instances_splits(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                100: ["xiuqo.exe", "gateway", "run"],
                200: ["xiuqo.exe"],  # REPL — keep
                300: ["xiuqo.exe", "dashboard"],  # keep
    ...
    assert kept == [(200, "xiuqo.exe"), (300, "xiuqo.exe"), (400, "xiuqo.exe")]
...
def test_filter_non_gateway_concurrent_instances_gateway_only(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                111: ["xiuqo.exe", "gateway", "run"],
                222: [r"C:\venv\Scripts\xiuqo-gateway.exe"],
            }
        ),
    )
    matches = [(111, "xiuqo.exe"), (222, "xiuqo-gateway.exe")]
...
def test_update_gate_skips_abort_when_only_concurrent_is_gateway(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_hermes_instances",
        return_value=[(1000, "xiuqo.exe"), (2000, "xiuqo-gateway.exe")],
    ), patch.object(
    ...
    captured = capsys.readouterr().out
    assert "Another xiuqo.exe is running" not in captured
...
def test_update_gate_still_aborts_on_non_gateway_concurrent(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_hermes_instances",
        return_value=[(1000, "xiuqo.exe"), (3000, "xiuqo.exe")],
    ), patch.object(
        cli_main,
        "_filter_non_gateway_concurrent_instances",
        return_value=[(3000, "xiuqo.exe")],
    ), patch.object(
        cli_main, "_run_pre_update_backup"
    ) as mock_backup:
...
def test_update_impl_refuses_before_terminating_gateway_ancestor(
    ...
    holder = (
        300,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m xiuqo_cli.main gateway run",
    )
    monkeypatch.setattr(
        gateway_cli,
        "_is_pid_ancestor_of_current_process",
