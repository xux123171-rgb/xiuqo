...
2. ``TestWatcherRespawnLive`` — drives the REAL
   ``xiuqo_cli.gateway._spawn_gateway_restart_watcher`` end to end with a
   real stub gateway process, against a temp XIUQO_HOME:
   ...
   - the respawn env must carry ``_XIUQO_GATEWAY_BREAKAWAY=1`` (the stamp
...
class TestWatcherRespawnLive:
    ...
    def _run_watcher_cycle(self, tmp_path: Path, monkeypatch) -> tuple[Path, Path]:
        ...
        stub = (
            ...
            "    os.environ.get('_XIUQO_GATEWAY_BREAKAWAY', 'MISSING'),\n"
            "    encoding='utf-8')\n"
