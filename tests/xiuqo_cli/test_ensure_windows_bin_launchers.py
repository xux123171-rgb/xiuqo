"""``xiuqo`` must survive git operations on the checkout (launcher layout).

The Windows ``xiuqo`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``XIUQO_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``xiuqo\\bin``) was swept by ``xiuqo update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``xiuqo`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``xiuqo update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``xiuqo_constants.venv_bin_dir``), so these
...
from xiuqo_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: XIUQO_HOME/xiuqo/venv/Scripts + launchers."""
    home = tmp_path / "xiuqo"
    root = home / "xiuqo"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under XIUQO_HOME gains nothing anywhere."""
    home = tmp_path / "xiuqo-home"
    home.mkdir()
    monkeypatch.setenv("XIUQO_HOME", str(home))
    root = tmp_path / "src" / "xiuqo"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``xiuqo -p <name>`` XIUQO_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("XIUQO_HOME", str(home / "profiles" / "work"))
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `xiuqo update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "xiuqo.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "xiuqo.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\xiuqo\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/XIUQO/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``xiuqo update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/xiuqo.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/xiuqo.exe is not gitignored — xiuqo update's autostash "
