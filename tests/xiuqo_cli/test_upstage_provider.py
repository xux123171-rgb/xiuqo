...
Regression guard for the bug where `xiuqo model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`xiuqo_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in XIUQO_OVERLAYS
        overlay = XIUQO_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `xiuqo_cli/main.py` (cmd_model): read
