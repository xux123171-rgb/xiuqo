...
from xiuqo_cli.auth import _load_auth_store, resolve_provider
...
def portal(monkeypatch, tmp_path):
    fake = FakePortal()
    monkeypatch.setenv("XIUQO_PORTAL_BASE_URL", PORTAL)
    monkeypatch.setenv("XIUQO_ANON_API_SECRET", "test-secret")
    monkeypatch.setenv("XIUQO_SHARED_AUTH_DIR", str(tmp_path / "shared-store"))
    monkeypatch.setenv("XIUQO_GUEST_ONBOARDING", "1")
    for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY", "NOUS_API_KEY"):
        monkeypatch.delenv(var, raising=False)
    from xiuqo_cli import auth_nous
    ...
    from xiuqo_cli import free_tier_bootstrap as _fb
    _fb.reset_for_tests()
...
class TestIdentityLifecycle:
    ...
    def test_second_profile_under_same_root_adopts_from_shared_store(self, portal, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIUQO_HOME", str(other_home))
        before = len(portal.calls)
    ...
    def test_launch_gate_off_means_no_free_tier_at_all(self, portal, monkeypatch):
        """Without ``XIUQO_GUEST_ONBOARDING=1`` the free tier does not exist: no mint, no portal
        ...
        for raw in ("", "0", "true", "yes", "new"):
            monkeypatch.setenv("XIUQO_GUEST_ONBOARDING", raw)
...
class TestExplicitProvision:
    ...
    def test_provision_mints_once_then_everything_adopts(self, portal, monkeypatch, tmp_path):
        ...
        sibling.mkdir()
        monkeypatch.setenv("XIUQO_HOME", str(sibling))
    ...
    def test_retired_identity_is_replaced(self, portal, monkeypatch):
        ...
        from xiuqo_cli.auth_nous import resolve_nous_runtime_credentials
...
class TestRouteFallback:
    ...
    def test_guest_state_without_url_never_resolves_to_the_paid_host(self, portal):
        from xiuqo_cli.auth_nous import _nous_effective_routing
    ...
    def test_shared_store_shape_keeps_a_guest_on_the_welcome_host(self, portal):
        from xiuqo_cli.auth_nous import _nous_shared_shape
...
class TestTokenAcquisitionSeam:
    def test_expired_guest_jwt_reexchanges_and_never_hits_oauth_token(self, portal):
        anon_auth.ensure_portal_identity(explicit=True)
        from xiuqo_cli.auth import _auth_store_lock, _save_auth_store
    ...
    def test_tool_gateway_token_path_reexchanges(self, portal):
        anon_auth.ensure_portal_identity(explicit=True)
        from xiuqo_cli.auth import _auth_store_lock, _save_auth_store, resolve_nous_access_token
        with _auth_store_lock():
            store = _load_auth_store()
            store["providers"]["nous"]["expires_at"] = "2000-01-01T00:00:00+00:00"
            _save_auth_store(store)
...
class TestLogout:
    def test_logout_with_only_free_tier_is_a_true_noop(self, portal, capsys):
        ...
        from xiuqo_cli.auth import _auth_file_path, logout_command
        anon_auth.ensure_portal_identity(explicit=True)
    ...
    def test_logout_of_real_account_clears_shared_store(self, portal, tmp_path):
        ...
        from xiuqo_cli.auth_nous import persist_nous_credentials
        persist_nous_credentials({"access_token": _jwt(client_id="xiuqo-cli", account_tier="free"),
...
class TestModelSwitchCopy:
    def test_switching_away_from_welcome_names_the_account_path_not_another_provider(self, portal, monkeypatch):
        anon_auth.ensure_portal_identity(explicit=True)
        from xiuqo_cli import model_switch
...
class TestIdentityOfRecordIsTheSharedStore:
    def test_stale_profile_guest_adopts_a_newer_shared_account(self, portal, tmp_path):
        ...
        _write_shared_nous_state({"access_token": _jwt(client_id="xiuqo-cli", account_tier="free"),
    ...
    def test_clearing_a_dead_guest_leaves_a_sibling_identity_alone(self, portal, tmp_path):
        anon_auth.ensure_portal_identity(explicit=True)
        from xiuqo_cli.auth_nous import _write_shared_nous_state
        _write_shared_nous_state({"access_token": _jwt(client_id="xiuqo-cli"), "refresh_token": "rt-sibling",
    ...
    def test_lock_order_is_profile_then_shared(self, portal, monkeypatch):
        order = []
        from xiuqo_cli import auth as auth_mod, auth_nous
