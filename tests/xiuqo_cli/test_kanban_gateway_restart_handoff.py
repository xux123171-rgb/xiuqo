...
def worker_setup(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> tuple[Path, kb.Task]:
    ...
    monkeypatch.setattr(kbd, "_resolve_hermes_argv", lambda: ["xiuqo"])
...
def test_managed_gateway_worker_is_spawned_in_restart_safe_scope(
    ...
    assert captured_cmd[unit_index + 1] == "xiuqo-worker-kanban-t_candidate_restart-run-23"
    assert "MemoryMax=536870912" in captured_cmd
    separator = captured_cmd.index("--")
    assert captured_cmd[separator + 1 : separator + 4] == ["xiuqo", "-p", "coder"]
    assert captured_cwd == str(workspace)
    assert captured_env["XIUQO_KANBAN_TASK"] == task.id
    assert captured_env["XIUQO_KANBAN_RUN_ID"] == "23"
...
def test_standalone_dispatcher_keeps_direct_worker_spawn(
    ...
    assert captured_cmd[:3] == ["xiuqo", "-p", "coder"]
...
def test_real_user_systemd_scope_preserves_worker_context(
    ...
    script = (
        ...
        "'task': os.environ.get('XIUQO_KANBAN_TASK'), "
        "'run': os.environ.get('XIUQO_KANBAN_RUN_ID'), "
    ...
    assert "xiuqo-gateway.service" not in payload["cgroup"]
