...
def test_fast_serve_launch_dispatches_only_unambiguous_serve(monkeypatch) -> None:
    ...
    monkeypatch.setattr(sys, "argv", ["xiuqo", "serve", "--host", "127.0.0.1", "--port", "0"])
    ...
    for argv in (["serve", "--future-flag"], ["serve", "--help"], ["chat"]):
        monkeypatch.setattr(sys, "argv", ["xiuqo", *argv])
        assert main_mod._try_fast_serve_launch() is False
    monkeypatch.setenv("XIUQO_DISABLE_FAST_SERVE_LAUNCH", "1")
    monkeypatch.setattr(sys, "argv", ["xiuqo", "serve"])
    assert main_mod._try_fast_serve_launch() is False
    monkeypatch.delenv("XIUQO_DISABLE_FAST_SERVE_LAUNCH")
