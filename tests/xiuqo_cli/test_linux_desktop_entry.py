"""Tests for the Linux XDG desktop entry installed by ``xiuqo desktop``."""
...
def xdg_home(tmp_path, monkeypatch) -> Path:
    ...
    # ~/.local/bin/xiuqo on the dev machine.
...
def _stub_install(tmp_path, monkeypatch) -> None:
    xiuqo_bin = tmp_path / "bin" / "xiuqo"
    xiuqo_bin.parent.mkdir(exist_ok=True)
    xiuqo_bin.write_text("", encoding="utf-8")
    monkeypatch.setattr(
        "xiuqo_cli.relaunch.resolve_hermes_bin", lambda: str(xiuqo_bin)
    )
...
def test_install_writes_entry_with_absolute_exec_and_icon(
    ...
    assert entry == xdg_home / "applications" / "xiuqo.desktop"
    values = _parse(entry.read_text(encoding="utf-8"))

    # Exec must be the absolute path of the resolved binary. The launcher
    # runs with a minimal PATH, so a bare `xiuqo` would not resolve.
    assert values["Exec"] == f"{xiuqo_bin} desktop"
...
def test_install_prefers_themed_icon_from_hicolor(tmp_path, xdg_home, monkeypatch):
    ...
    dest = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "xiuqo.png"
    assert dest.is_file()
...
def test_installed_entry_is_executable(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr(
        "xiuqo_cli.relaunch.resolve_hermes_bin", lambda: "/usr/bin/xiuqo"
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_falls_back_to_interpreter_module(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("xiuqo_cli.relaunch.resolve_hermes_bin", lambda: None)
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)
    exec_line = _parse(entry.read_text(encoding="utf-8"))["Exec"]

    assert exec_line.endswith("-m xiuqo_cli.main desktop")
    assert Path(exec_line.split(" ")[0]).is_absolute()


# #90292: the shell installer's bash wrapper makes argv[0] the repo `xiuqo`
...
def test_exec_prefixes_interpreter_for_env_shebang_python_script(
    ...
    xiuqo_bin = tmp_path / "bin" / "xiuqo"
    xiuqo_bin.parent.mkdir()
    xiuqo_bin.write_text(
        "#!/usr/bin/env python3\nimport xiuqo_cli\n", encoding="utf-8"
    )
    xiuqo_bin.chmod(0o755)
    monkeypatch.setattr(
        "xiuqo_cli.relaunch.resolve_hermes_bin", lambda: str(xiuqo_bin)
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
    ...
    assert str(xiuqo_bin) in exec_line
...
def test_exec_leaves_shell_wrapper_launchers_alone(tmp_path, xdg_home, monkeypatch):
    ...
    xiuqo_bin.parent.mkdir()
    xiuqo_bin.write_text(
        '#!/bin/bash\nexec /opt/xiuqo/venv/bin/python "$@"\n', encoding="utf-8"
    )
    xiuqo_bin.chmod(0o755)
    monkeypatch.setattr(
        "xiuqo_cli.relaunch.resolve_hermes_bin", lambda: str(xiuqo_bin)
    ...
    assert exec_line == f"{xiuqo_bin} desktop"
...
def test_exec_leaves_venv_shebang_scripts_alone(tmp_path, xdg_home, monkeypatch):
    ...
    xiuqo_bin.parent.mkdir()
    interpreter = os.path.abspath(sys.executable)
    xiuqo_bin.write_text(f"#!{interpreter}\nimport xiuqo_cli\n", encoding="utf-8")
    xiuqo_bin.chmod(0o755)
    monkeypatch.setattr(
        "xiuqo_cli.relaunch.resolve_hermes_bin", lambda: str(xiuqo_bin)
    )
...
def test_exec_converges_from_repo_script_argv0_to_installed_wrapper(
    ...
    Launching with argv[0] = <checkout>/xiuqo (what the broken entry
    ...
    root = _make_project(tmp_path)
    repo_script = root / "xiuqo"  # checkout-internal launcher candidate
    repo_script.write_text(
        "#!/usr/bin/env python3\nimport xiuqo_cli\n", encoding="utf-8"
    )
    repo_script.chmod(0o755)
    wrapper = tmp_path / "installed" / "bin" / "xiuqo"
    ...
    monkeypatch.setattr(
        "shutil.which", lambda name: str(wrapper) if name == "xiuqo" else None
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_never_persists_a_bare_interpreter_command(
    ...
    """The `python -m xiuqo_cli.main` relaunch context must not write
...
def test_exec_keeps_resolver_fallback_when_no_wrapper_on_path(
    ...
    `sys.executable -m xiuqo_cli.main desktop` fallback. Persisting the
    ...
    monkeypatch.setattr("xiuqo_cli.relaunch.resolve_hermes_bin", fake_resolve)
...
def test_exec_uses_known_wrapper_when_path_lookup_misses(
    ...
    # checkout's xiuqo script (the aidiyet check requires it to target
    # the writing checkout).
    known_wrapper = tmp_path / "known-home" / ".local" / "bin" / "xiuqo"
    known_wrapper.parent.mkdir(parents=True)
    known_wrapper.write_text(
        f'#!/bin/bash\nexec {root / "venv" / "bin" / "python"} {root / "xiuqo"} "$@"\n',
        encoding="utf-8",
    )
...
def test_exec_rejects_known_wrapper_from_another_checkout(
    ...
    On machines with multiple installs over time, ~/.local/bin/xiuqo may
    ...
    other_root.mkdir()
    foreign_wrapper = tmp_path / "known-home" / ".local" / "bin" / "xiuqo"
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "user",
            {},
            "HOME-SET-BY-TEST/.local/bin/xiuqo",
            id="user-layout",
        ),
        pytest.param(
            "termux",
            {"PREFIX": "PREFIX-SET-BY-TEST"},
            "PREFIX-SET-BY-TEST/bin/xiuqo",
...
def test_known_wrapper_candidates_cover_installer_layouts(
    ...
    if layout == "root-fhs":
        ...
        assert candidates.index("/usr/local/bin/xiuqo") < candidates.index(
            f"{sentinel_home}/.local/bin/xiuqo"
        )
    if layout == "non-root-no-fhs":
        # Non-root euid: /usr/local/bin must be excluded outright.
        assert "/usr/local/bin/xiuqo" not in candidates
...
def test_install_without_source_icon_uses_themed_name(tmp_path, xdg_home, monkeypatch):
    ...
    # when Xiuqo is installed some other way.
    assert _parse(entry.read_text(encoding="utf-8"))["Icon"] == "xiuqo"
...
def test_exec_arg_quoting_handles_spaces(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    spaced = tmp_path / "my apps" / "xiuqo"
    spaced.parent.mkdir()
    spaced.write_text("", encoding="utf-8")
    monkeypatch.setattr("xiuqo_cli.relaunch.resolve_hermes_bin", lambda: str(spaced))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_can_import_probe_runs_and_caches(tmp_path):
    ...
    deliberately NOT asserted - a CI host with xiuqo pip-installed
    ...
    try:
        first = lde._can_import_xiuqo_cli(real)
        assert isinstance(first, bool)
        assert str(real) in lde._probe_cache
        t0 = time.monotonic()
        second = lde._can_import_xiuqo_cli(real)
...
def test_exec_falls_back_to_running_interpreter_when_probe_fails(
    ...
    monkeypatch.setattr(lde, "_can_import_xiuqo_cli", lambda p: False)
...
def test_wrapper_ownership_accepts_shim_via_symlinked_home(tmp_path, monkeypatch):
    ...
    lexical_checkout = home_link / "xiuqo"
    (home_real / "xiuqo").mkdir()

    shim = home_link / ".local" / "bin" / "xiuqo"
    shim.parent.mkdir(parents=True)
    shim.write_text(
        f"#!/bin/bash\n"
        f"exec {lexical_checkout}/venv/bin/python "
        f'{lexical_checkout}/xiuqo "$@"\n',
        encoding="utf-8",
    )
    ...
    monkeypatch.setattr("shutil.which", lambda name: None)
    repo_script = lexical_checkout / "xiuqo"
...
def test_probe_skips_wrapper_with_escaping_python_shebang(
    ...
    broken_wrapper = xdg_home / ".local" / "bin" / "xiuqo"
...
def test_probe_accepts_shell_launcher_wrapper(tmp_path, xdg_home, monkeypatch):
    ...
    good_wrapper = xdg_home / ".local" / "bin" / "xiuqo"
...
def test_install_places_1024_png_in_256x256_not_scalable(
    ...
    dest = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "xiuqo.png"
    stale = xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "xiuqo.png"
    assert values["Icon"] == "xiuqo"
    assert dest.is_file()
    assert dest.read_bytes() == lde.icon_path(root).read_bytes()
...
def test_install_removes_stale_scalable_png(tmp_path, xdg_home, monkeypatch):
    """v2026.8.31 wrote the PNG into scalable/. A later xiuqo desktop
...
def test_install_exact_48_png_uses_48x48_dir(tmp_path, xdg_home, monkeypatch):
    ...
    dest = xdg_home / "icons" / "hicolor" / "48x48" / "apps" / "xiuqo.png"
    assert dest.is_file()
    assert not (
        xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "xiuqo.png"
    ).exists()
...
def test_install_resizes_decodable_png_to_panel_sizes(
    ...
    dest_24 = xdg_home / "icons" / "hicolor" / "24x24" / "apps" / "xiuqo.png"
    dest_256 = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "xiuqo.png"
    stale = xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "xiuqo.png"
