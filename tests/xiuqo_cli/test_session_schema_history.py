"""``xiuqo_cli.session_schema_history`` must track SCHEMA_SQL.
...
from xiuqo_cli import session_schema_history as history
...
def test_replayed_history_ends_at_current_schema(table: str) -> None:
    ...
    hint = f"SCHEMA_HISTORY[{table!r}].events in xiuqo_cli/session_schema_history.py"
    assert not missing, (
