...
- ``xiuqo_cli/_subprocess_compat.pid_is_hermes`` / ``kill_process_tree``
- ``xiuqo_cli/dashboard_procs._kill_stale_dashboard_processes`` (win32)
- ``xiuqo_cli/update_cmd._stop_process_trees``
...
class TestPidIsHermes:
    ...
    def test_hermes_match_requires_token_boundary(self):
        # "xiuqo" buried inside an unrelated path segment must not match.
        assert _subprocess_compat._text_names_hermes(
            r"c:\users\shermesa\app.exe"
        ) is False
        assert _subprocess_compat._text_names_hermes(
            r"C:\Users\x\.xiuqo-runtime\python.exe -m xiuqo_cli.main"
        ) is True
        assert _subprocess_compat._text_names_hermes(
    ...
    def test_missing_pid_real_probe_fails_closed(self):
        # A PID that cannot exist must never be judged Xiuqo-owned.
...
class TestStopProcessTrees:
    ...
    def test_foreign_pids_only_probed(self):
        with mock.patch(
            ...
            "xiuqo_cli._subprocess_compat.pid_is_hermes", return_value=False
...
class TestKillStaleDashboardProcesses:
    ...
    def test_foreign_pid_reported_not_killed(self):
        ...
        assert result["failed"] == [
            (12345, "not xiuqo-owned or process identity changed")
        ]
        run.assert_not_called()
