...
def _write_plugin(xiuqo_home: Path) -> None:
    plugin_dir = xiuqo_home / "plugins" / "ledger_probe"
...
def _write_profile_probe(xiuqo_home: Path, marker: str) -> None:
    plugin_dir = xiuqo_home / "plugins" / "profile_probe"
    plugin_dir.mkdir(parents=True)
    (plugin_dir / "plugin.yaml").write_text(
        yaml.safe_dump(
            {
...
def test_scoped_plugin_cannot_deregister_a_process_global_tool():
    ...
    module_name = "xiuqo_plugins.scoped_cleanup"
...
def test_direct_plugin_platform_registration_infers_immutable_scope(tmp_path):
    ...
    module_name = "company.xiuqo.direct_platform_probe"
