"""`xiuqo sessions repair --check-only` must fail (non-zero) when the store is unhealthy.
...
import xiuqo_state
from xiuqo_cli import sessions_cmd
...
def test_check_only_exit_status_tracks_probe_verdict(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(xiuqo_state, "DEFAULT_DB_PATH", db_path)
