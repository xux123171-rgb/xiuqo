"""Regression coverage for stale ``xiuqo serve`` generations after an abort.

Issue #92145: ``xiuqo update`` moves the checkout from generation N to N+1
...
``xiuqo serve`` is not a gateway profile.  It hosts ``tui_gateway.server``,
systemd lists ``xiuqo-gateway.service`` before ``xiuqo-serve.service`` (so
...
from xiuqo_cli import update_abort_recovery as abort_recovery
from xiuqo_cli import update_cmd
from xiuqo_cli import update_restart_recovery as recovery
...
def _identity_module():
    return __import__("xiuqo_cli.process_identity", fromlist=["ledger_entries"])
...
def test_failed_serve_unit_blocks_completion():
    ...
    result["serve_units"] = {"verified": [], "failed": ["xiuqo-serve"]}
    assert (
        update_cmd._abort_recovery_is_complete(
            planned_gateway_profiles={"default"},
            covered_gateway_profiles={"default"},
...
class _Systemctl:
    ...
    def __call__(self, argv, **kwargs):
        if "list-units" in argv:
            ...
            body = "\n".join(
                f"{unit} loaded active running Xiuqo" for unit in self.listed
            )
...
def test_serve_unit_verified_when_main_pid_changes(linux_systemctl):
    fake = _Systemctl(
        listed=["xiuqo-serve.service"],
        active={"xiuqo-serve.service": True},
        main_pids={"xiuqo-serve.service": 4242},
    )
    ...
    assert out == {"verified": ["user/xiuqo-serve"], "failed": []}
    assert fake.restarted == ["xiuqo-serve.service"]
...
def test_unchanged_main_pid_is_not_verified(linux_systemctl):
    ...
    assert out == {"verified": [], "failed": ["user/xiuqo-serve"]}
...
def test_unit_that_does_not_come_back_active_is_failed(linux_systemctl):
    ...
    def dies(argv, **kwargs):
        if "restart" in argv:
            fake.restarted.append(argv[-1])
            fake.active["xiuqo-serve.service"] = False
            return _Completed(0)
        return original_call(argv, **kwargs)
...
def test_inactive_serve_unit_is_left_alone(linux_systemctl):
    ...
    fake = _Systemctl(
        listed=["xiuqo-serve.service"],
        active={"xiuqo-serve.service": False},
        main_pids={"xiuqo-serve.service": 0},
    )
    out = recovery.restart_serve_units(run=fake, sleep=lambda _: None)
...
def test_units_already_restarted_by_the_phase_are_skipped(linux_systemctl):
    fake = _Systemctl(
        listed=["xiuqo-serve.service", "xiuqo-serve-work.service"],
        active={"xiuqo-serve.service": True, "xiuqo-serve-work.service": True},
        main_pids={"xiuqo-serve.service": 1, "xiuqo-serve-work.service": 2},
    )
    out = recovery.restart_serve_units(
        skip_units=[{"scope": "user", "unit": "xiuqo-serve"}],
        run=fake,
        sleep=lambda _: None,
    )
    assert fake.restarted == ["xiuqo-serve-work.service"]
    assert out == {"verified": ["user/xiuqo-serve-work"], "failed": []}


def test_unrelated_hermes_server_service_is_never_touched(linux_systemctl):
    """``xiuqo-serve*`` is a systemd glob, not a name gate (review on #83595)."""
    fake = _Systemctl(
        listed=["xiuqo-server.service", "xiuqo-serve.service"],
        active={"xiuqo-server.service": True, "xiuqo-serve.service": True},
        main_pids={"xiuqo-server.service": 7, "xiuqo-serve.service": 8},
    )
    out = recovery.restart_serve_units(run=fake, sleep=lambda _: None)
    assert fake.restarted == ["xiuqo-serve.service"]
    assert out["verified"] == ["user/xiuqo-serve"]
...
def test_gateway_units_are_not_restarted_by_the_serve_pass(linux_systemctl):
    ...
    fake = _Systemctl(
        listed=["xiuqo-gateway.service"],
        active={"xiuqo-gateway.service": True},
        main_pids={"xiuqo-gateway.service": 5},
    )
    out = recovery.restart_serve_units(run=fake, sleep=lambda _: None)
    assert fake.restarted == []
    assert out == {"verified": [], "failed": []}
...
def test_same_unit_name_in_both_scopes_is_proven_in_both(linux_systemctl):
    ...
    def dual_scope(argv, **kwargs):
        ...
        if "list-units" in argv:
            return _Completed(0, stdout="xiuqo-serve.service loaded active running x")
        if "is-active" in argv:
    ...
    assert out == {
        "verified": ["user/xiuqo-serve"],
        "failed": ["system/xiuqo-serve"],
    }
            ...
            json.dumps(
                {
                    ...
                    "serve_units": {
                        ...
                        "skip": [
                            "xiuqo-serve",
                            "xiuqo-serve-work.service",
                            {"scope": "system", "unit": "xiuqo-serve"},
                            {"scope": "user", "unit": "xiuqo-serve-work.service"},
                            "xiuqo-server",
                            "../../etc/systemd/evil",
                            "xiuqo-serve; rm -rf /",
                            "xiuqo-gateway",
...
def test_serve_only_fleet_still_spawns_the_recovery_child(monkeypatch):
            ...
            stdout=json.dumps(
                {
                    ...
                    "relaunch_attempted": [],
                    "failed": [],
                    "serve_units": {"verified": ["xiuqo-serve"], "failed": []},
                }
            ),
        )
    ...
    assert result["serve_units"] == {"verified": ["xiuqo-serve"], "failed": []}
...
def test_verified_serve_units_do_not_enter_gateway_restart_vocabulary(monkeypatch):
    ...
    assert "xiuqo-serve" not in result["verified"]
...
def test_already_restarted_units_are_forwarded_as_skips(monkeypatch):
    ...
    update_cmd._recover_gateway_restart_after_abort(
        _plan(_runtime("gateway", "default", "systemd", 111)),
        gateway_mode=False,
        skip_units={"user/xiuqo-serve"},
    )
    # The scope travels with the unit: settling `user/xiuqo-serve` must not
    # suppress recovery of a stale system-scope unit of the same name.
    assert captured["payload"]["serve_units"]["skip"] == [
        {"scope": "user", "unit": "xiuqo-serve"}
    ]
...
def test_serve_coverage_reaches_the_persisted_receipt(tmp_path, monkeypatch):
    from xiuqo_cli import update_receipt
    ...
    update_receipt.record_gateway_restart(
        restarted_services=[],
        incomplete=True,
        phase_error="cannot import name 'line_input' from 'xiuqo_cli.cli_output'",
        fresh_recovery={
            "requested": ["default"],
            "verified": ["default"],
            "relaunch_attempted": [],
            ...
            "serve_units": {
                "verified": ["user/xiuqo-serve"],
                "failed": ["system/xiuqo-serve-work"],
            },
            "stale_runtimes": [
                {
                    "pid": 4242,
...
def test_stale_serve_warning_names_the_process_and_the_fix(capsys):
    ...
    assert "4242" in out
    assert "serve" in out
    assert "systemctl --user restart xiuqo-serve.service" in out
...
def test_recovery_module_reports_serve_units_in_a_real_process():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "xiuqo_cli.update_restart_recovery", "--stdin"],
        input=json.dumps(
...
def _stub_dashboard_helpers(monkeypatch, **helpers):
    """Stub the ``xiuqo_cli.main_dashboard`` helpers the dashboard-cleanup path reads at call time."""
...
def _dashboard_main_stub(scan_calls, *, restart_result=True):
    return dict(
        _DASHBOARD_SYSTEMD_UNIT="xiuqo-dashboard.service",
...
def test_managed_dashboard_restart_still_scans_for_serve_backends(monkeypatch):
    ...
    The reporter's host runs ``xiuqo-dashboard.service`` AND
    ``xiuqo-serve.service``.  Returning as soon as the dashboard unit was
...
def test_restarted_dashboard_unit_is_not_killed_by_the_continued_scan(monkeypatch):
    ...
    _stub_dashboard_helpers(
        ...
        _get_systemd_service_for_pid=lambda pid: "xiuqo-dashboard.service",
...
def test_serve_backend_survives_selection_when_the_dashboard_unit_restarts(monkeypatch):
    ...
    _stub_dashboard_helpers(
        ...
        _get_pid_cgroup_path=lambda pid: "/user.slice/xiuqo-serve.service",
        _get_systemd_service_for_pid=lambda pid: "xiuqo-serve.service",
        _dashboard_cmdline_for_pid=lambda pid: None,
    ...
    assert restarted == ["xiuqo-serve.service"]
...
def _dual_scope_systemctl(*, user_pid=10, system_pid=20, settle=("user", "system")):
    """Both managers own a ``xiuqo-serve.service``; each is its own process."""
...
def test_settled_user_scope_does_not_suppress_the_stale_system_scope(
    ...
    ``user/xiuqo-serve`` was restarted before the phase aborted;
    ``system/xiuqo-serve`` is a different process still on the pre-update
    generation. A bare ``xiuqo-serve`` skip token suppressed BOTH, leaving
    ...
    assert out == {"verified": ["system/xiuqo-serve"], "failed": []}
...
def test_one_authorized_scope_never_mutates_the_same_name_in_the_other(
    ...
    recovery.restart_serve_units(
        skip_units=[{"scope": "user", "unit": "xiuqo-serve.service"}],
        run=run,
        sleep=lambda _: None,
    )
    user_verbs = [
...
def test_legacy_unqualified_skip_is_honoured_in_both_scopes(linux_systemctl):
    ...
    run, calls = _dual_scope_systemctl()
    out = recovery.restart_serve_units(
        skip_units=["xiuqo-serve"], run=run, sleep=lambda _: None
    )
...
def test_qualified_skip_payload_shape():
    ...
    assert update_cmd._qualified_serve_skips(
        {"user/xiuqo-serve", "system/xiuqo-serve-work"}
    ) == [
        {"scope": "system", "unit": "xiuqo-serve-work"},
        {"scope": "user", "unit": "xiuqo-serve"},
    ]
...
def test_unscoped_bookkeeping_entries_are_forwarded_without_a_scope():
    ...
    assert update_cmd._qualified_serve_skips({"xiuqo-serve"}) == [
        {"unit": "xiuqo-serve"}
    ]
