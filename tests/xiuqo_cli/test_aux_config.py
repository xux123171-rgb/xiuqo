"""Tests for the auxiliary-model configuration UI in ``xiuqo model``.
...
from xiuqo_cli.config import DEFAULT_CONFIG, load_config
from xiuqo_cli.main_provider_setup import _AUX_TASKS, _DELEGATION_TASK_KEY, _delegation_cfg_as_task, _format_aux_current, _reset_aux_to_auto, _save_aux_choice
...
def test_save_aux_choice_persists_to_config_yaml(tmp_path, monkeypatch):
    ...
    (tmp_path / ".xiuqo").mkdir(exist_ok=True)
...
def test_reset_aux_clears_delegation_routing_preserves_settings(tmp_path, monkeypatch):
    ...
    from xiuqo_cli.config import load_config as _lc, save_config
...
def test_leave_unchanged_replaces_cancel_label(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(xiuqo_cli_main_provider_setup, "_prompt_provider_choice", fake_prompt)
