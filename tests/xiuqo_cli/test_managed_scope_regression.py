...
def xiuqo_home(tmp_path, monkeypatch):
    ...
    # /etc/xiuqo on the dev/CI box can't influence the test.
    monkeypatch.setenv("XIUQO_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(xiuqo_home, monkeypatch):
...
def test_env_expansion_in_user_config(xiuqo_home, monkeypatch):
    from xiuqo_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_hermes_dotenv(xiuqo_home=str(home))
