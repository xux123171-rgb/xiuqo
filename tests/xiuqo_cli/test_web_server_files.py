...
import xiuqo_cli.web_routers.files as _rt_files
...
def forced_files_client(monkeypatch, tmp_path):
    root = tmp_path / "data"
    monkeypatch.setenv("XIUQO_DASHBOARD_FILES_ROOT", str(root))
...
def local_files_client(monkeypatch, tmp_path):
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.delenv("XIUQO_DASHBOARD_FILES_ROOT", raising=False)
...
def test_download_resolves_paths_in_the_originating_profile_session(local_files_client, monkeypatch):
    ...
    xiuqo_home = home / "isolated-xiuqo"
    ...
    for profile, sid, cwd in [("default", "origin-session", str(session_cwd)),
        ...
        db_home = xiuqo_home if profile == "default" else xiuqo_home / "profiles" / profile
...
def test_other_credential_store_basenames_blocked(forced_files_client):
    ...
    point the managed root at XIUQO_HOME itself (#57505), which contains
