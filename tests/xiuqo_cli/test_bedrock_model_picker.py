...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (XIUQO_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via XIUQO_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in XIUQO_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from xiuqo_cli.providers import XIUQO_OVERLAYS
        assert "bedrock" in XIUQO_OVERLAYS
