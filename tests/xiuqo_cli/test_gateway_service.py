...
import xiuqo_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`xiuqo gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `xiuqo gateway restart` in the background; when a
        ...
        assert "xiuqo gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``XIUQO_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../xiuqo_test`` XIUQO_HOME — silently breaking
        ...
        carry those markers.
        """
        unit_path = tmp_path / "xiuqo-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")

        monkeypatch.setattr(
            gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path
        )
        # Realistic generated unit referencing a pytest tmpdir XIUQO_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="XIUQO_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/xiuqo_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="XIUQO_HOME=/tmp/xiuqo-e2e-41264"\n'
        assert (
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="XIUQO_HOME={tmp_path}/xiuqo-home"\n'
...
class TestServiceIdentityForForeignHome:
    """A XIUQO_HOME that is neither ``~/.xiuqo`` nor ``~/.xiuqo/profiles/<name>`` must never resolve to
    the default profile's ``xiuqo-gateway`` unit (a temp-home harness uninstalled the production gateway)."""
    ...
    def test_foreign_home_gets_its_own_unit(self, machine_home, tmp_path, monkeypatch):
        ...
        foreign.mkdir()
        monkeypatch.setenv("XIUQO_HOME", str(foreign))

        default_unit = machine_home / ".config" / "systemd" / "user" / "xiuqo-gateway.service"
        assert gateway_cli.get_service_name() != "xiuqo-gateway"
    ...
    def test_default_and_named_profile_homes_keep_their_names(self, machine_home, monkeypatch):
        default_home = machine_home / ".xiuqo"
        (default_home / "profiles" / "alpha").mkdir(parents=True)

        monkeypatch.setenv("XIUQO_HOME", str(default_home))
        assert gateway_cli.get_service_name() == "xiuqo-gateway"

        monkeypatch.setenv("XIUQO_HOME", str(default_home / "profiles" / "alpha"))
        assert gateway_cli.get_service_name() == "xiuqo-gateway-alpha"

    def test_sudo_user_default_home_keeps_bare_service_name(self, machine_home, tmp_path, monkeypatch):
        sudo_home = tmp_path / "alice"
        sudo_default = sudo_home / ".xiuqo"
        ...
        # After unit sync, XIUQO_HOME points at the invoking user's native home.
        monkeypatch.setenv("XIUQO_HOME", str(sudo_default))
        assert gateway_cli.get_service_name() == "xiuqo-gateway"


class TestUninstallRefusesForeignUnit:
    """systemd_uninstall must not stop/disable/unlink a unit pinned to another XIUQO_HOME."""

    def test_unit_for_other_home_is_left_alone(self, tmp_path, monkeypatch, capsys):
        unit_path = tmp_path / "xiuqo-gateway.service"
        unit_path.write_text('[Service]\nEnvironment="XIUQO_HOME=/somewhere/else"\n', encoding="utf-8")
        monkeypatch.setenv("XIUQO_HOME", str(tmp_path / "mine"))
        monkeypatch.setattr(gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path)
...
class TestGetCronDrainTimeout:
    def test_missing_config_falls_back_to_default(self, monkeypatch):
        monkeypatch.delenv("XIUQO_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_env_overrides_config(self, monkeypatch):
        monkeypatch.setenv("XIUQO_CRON_DRAIN_TIMEOUT", "45")
        monkeypatch.setattr(
            gateway_cli,
            "read_raw_config",
...
class TestGeneratedSystemdUnits:
    ...
    def test_timeout_stop_sec_covers_default_cron_drain_floor(self, monkeypatch):
        ...
        monkeypatch.delenv("XIUQO_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("XIUQO_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_restart_exit_code_is_also_declared_a_success_status(self):
        ...
        and any OnFailure= alert unit fires on every routine restart (xiuqo update,
        xiuqo gateway restart, the in-app restart). SuccessExitStatus=75 keeps the same
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".xiuqo" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `xiuqo gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import xiuqo_cli.resource_limits as resource_limits
...
class TestLaunchdServiceRecovery:
    ...
    def test_refresh_defers_reload_when_running_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        plist_path = tmp_path / "ai.xiuqo.gateway.plist"
        ...
        monkeypatch.setattr(
            ...
            lambda: (
                "<plist>--replace\n<key>XIUQO_HOME</key>"
                "<string>/Users/alice/.xiuqo</string></plist>"
            ),
        )
...
class TestGatewaySystemServiceRouting:
    ...
    def test_launchd_restart_uses_sigusr1_and_exit_wait_budget(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.xiuqo.gateway")
        ...
        assert ("observe", "ai.xiuqo.gateway", 654, "gui/501") in calls
        out = capsys.readouterr().out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.xiuqo.gateway"])
            ),
        )
...
class TestSystemUnitHermesHome:
    """XIUQO_HOME in system units must reference the target user, not root."""

    def test_empty_managed_node_dir_uses_only_ambient_fallback(
        self, monkeypatch, tmp_path
    ):
        managed_bin = tmp_path / ".xiuqo" / "node" / "bin"
        ...
        gateway_cli._append_node_dir_for_service(entries, tmp_path / ".xiuqo")
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        target_hermes = target_home / ".xiuqo"
        root_home = tmp_path / "root"
        root_hermes = root_home / ".xiuqo"
        ...
        monkeypatch.setenv("XIUQO_HOME", str(root_hermes))
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "xiuqo_constants.iter_hermes_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "xiuqo_constants.xiuqo_managed_node_tree_present",
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'XIUQO_HOME=/home/alice/.xiuqo' in unit
        assert '/root/.xiuqo' not in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's XIUQO_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        xiuqo_home = str(gateway_cli.get_hermes_home().resolve())
        assert f'XIUQO_HOME={xiuqo_home}' in unit


class TestSystemUnitRefreshSyncsHermesHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.xiuqo."""

    def test_refresh_adopts_unit_hermes_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_hermes = root_home / ".xiuqo"
        alice_hermes = alice_home / ".xiuqo"
        root_hermes.mkdir(parents=True)
        ...
        # Correct installed unit (operator's XIUQO_HOME + drain timeout).
        monkeypatch.setenv("XIUQO_HOME", str(alice_hermes))
        ...
        # Simulate sudo without inherited XIUQO_HOME (falls back to root).
        monkeypatch.setenv("XIUQO_HOME", str(root_hermes))
        ...
        assert os.environ["XIUQO_HOME"] == str(alice_hermes)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned XIUQO_HOME *before* it reads/compares the unit.
...
class TestHermesHomeForTargetUser:
    ...
    def test_remaps_default_home(self, monkeypatch):
        ...
        assert result == "/home/alice/.xiuqo"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "xiuqo gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_home = tmp_path / "root"
        target_home = tmp_path / "home" / "alice"
        root_profile = root_home / ".xiuqo" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("XIUQO_HOME", str(root_profile))
        ...
        assert f'XIUQO_HOME={target_home / ".xiuqo" / "profiles" / "mybot"}' in unit

    def test_launchd_plist_wraps_gateway_stderr_with_timestamps(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".xiuqo" / "profiles" / "mybot"
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".xiuqo" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.xiuqo.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".xiuqo" / "xiuqo"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".xiuqo" / "xiuqo")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("XIUQO_HOME", str(root_home / ".xiuqo"))
        monkeypatch.setattr(gateway_cli, "get_hermes_home", lambda: root_home / ".xiuqo")
        ...
        # WorkingDirectory is anchored at the target user's XIUQO_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.xiuqo" in unit
        assert "WorkingDirectory=/home/alice/.xiuqo/xiuqo" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "xiuqo-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "xiuqo-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'xiuqo gateway install' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyHermesUnitDetection:
    ...
    older install left a ``xiuqo.service`` unit behind when the service was
    renamed to ``xiuqo-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``xiuqo.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``xiuqo-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Xiuqo Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m xiuqo_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        (user_dir / "xiuqo.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "xiuqo.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m xiuqo_cli.main gateway run
          - python path/to/xiuqo_cli/main.py gateway run
          - xiuqo gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m xiuqo_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/xiuqo/xiuqo_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/xiuqo gateway run --replace",
            "ExecStart=/venv/bin/python /opt/xiuqo/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = "xiuqo.service" if i == 0 else "xiuqo.service"  # same name
            # Test each variant fresh
            (user_dir / "xiuqo.service").write_text(
                f"[Unit]\nDescription=Old Xiuqo\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_hermes_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "xiuqo.service" in out
        assert "xiuqo gateway migrate-legacy" in out
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop xiuqo.service" in c for c in cmds_joined)
        assert any("--user disable xiuqo.service" in c for c in cmds_joined)
        assert any("--user daemon-reload" in c for c in cmds_joined)

    def test_does_not_touch_profile_units_during_migration(
        self, tmp_path, monkeypatch, capsys
    ):
        """Teknium's constraint: profile units (xiuqo-gateway-coder.service)
        ...
        profile_unit = user_dir / "xiuqo-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "xiuqo-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `xiuqo gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import xiuqo_cli.main as cli_main
        ...
        # public, just confirm that `xiuqo gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import xiuqo_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "xiuqo_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "xiuqo_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "xiuqo-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "xiuqo-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "xiuqo-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``xiuqo gateway start --system``).
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "xiuqo-gateway")
        ...
        assert "sudo systemctl start xiuqo-gateway" in out
        assert "sudo xiuqo gateway uninstall --system" in out
        assert "xiuqo gateway install" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``xiuqo gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (XIUQO_HOME), never the source checkout / worktree, so a relocated or
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.xiuqo.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.xiuqo.gateway"
...
class TestLaunchdUnloadedJobStderrStaysOffTerminal:
    ...
    def fake_launchctl(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.xiuqo.gateway")
        monkeypatch.setattr(gateway_cli, "_launchd_domain", lambda: "gui/501")
        monkeypatch.setattr(gateway_cli, "get_launchd_plist_path", lambda: tmp_path / "ai.xiuqo.gateway.plist")
        monkeypatch.setattr(gateway_cli, "_clear_launchd_unsupported_marker", lambda: None)
...
class TestTimeoutStopSecCoversCronFloor:
    ...
    def _unit_with_config(self, tmp_path, monkeypatch, config_yaml, env=None):
        xiuqo = tmp_path / "home" / ".xiuqo"
        xiuqo.mkdir(parents=True)
        (xiuqo / "config.yaml").write_text(config_yaml, encoding="utf-8")
        monkeypatch.setenv("XIUQO_HOME", str(xiuqo))
        monkeypatch.delenv("XIUQO_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("XIUQO_CRON_DRAIN_TIMEOUT", raising=False)
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
        monkeypatch.setattr(
    ...
    def test_env_override_extends_the_leash(self, tmp_path, monkeypatch):
        unit = self._unit_with_config(
            ...
            monkeypatch,
            "agent:\n  restart_drain_timeout: 0\n",
            env={"XIUQO_CRON_DRAIN_TIMEOUT": "200"},
        )
        assert "TimeoutStopSec=240" in unit


class TestUnitAnchoredServiceIdentity:
    """The installed ``xiuqo-gateway.service`` owns the bare name: under ``sudo`` the naming basis moves
    mid-command when ``_sync_hermes_home_from_systemd_unit()`` adopts the unit's XIUQO_HOME (#108674).
    ...
    def test_home_not_pinned_by_unit_keeps_its_suffix(self, tmp_path, monkeypatch):
        alice_home = tmp_path / "alice" / ".xiuqo"
        alice_home.mkdir(parents=True)
        bob_home = tmp_path / "bob" / ".xiuqo"
        bob_home.mkdir(parents=True)
        root_home = tmp_path / "root" / ".xiuqo"
        root_home.mkdir(parents=True)
        unit_dir = tmp_path / "systemd"
        unit_dir.mkdir()
        (unit_dir / f"{gateway_cli._SERVICE_BASE}.service").write_text(
            f'[Service]\nEnvironment="XIUQO_HOME={alice_home}"\n', encoding="utf-8"
        )
        monkeypatch.setattr(gateway_cli, "_SYSTEM_UNIT_DIR", unit_dir)
        monkeypatch.setattr(xiuqo_constants, "_get_platform_default_hermes_home", lambda: root_home)
        monkeypatch.setenv("XIUQO_HOME", str(bob_home))
    ...
    def test_unprivileged_profile_command_ignores_the_system_unit(self, tmp_path, monkeypatch):
        ...
        profile_home = tmp_path / "alice" / ".xiuqo" / "profiles" / "kimi"
        profile_home.mkdir(parents=True)
        unit_dir = tmp_path / "systemd"
        unit_dir.mkdir()
        (unit_dir / f"{gateway_cli._SERVICE_BASE}.service").write_text(
            f'[Service]\nEnvironment="XIUQO_HOME={profile_home}"\n', encoding="utf-8"
        )
        monkeypatch.setattr(gateway_cli, "_SYSTEM_UNIT_DIR", unit_dir)
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        monkeypatch.setattr(os, "geteuid", lambda: 1000)
        monkeypatch.setenv("XIUQO_HOME", str(profile_home))
        assert gateway_cli.get_service_name() == "xiuqo-gateway-kimi"
    ...
    def test_real_unit_sync_keeps_the_name_it_validated(self, tmp_path, monkeypatch):
        ...
        assert os.environ["XIUQO_HOME"] == str(alice_home)  # the sync really ran
