...
``os.environ["XIUQO_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``xiuqo --yolo`` (process-start env), ``XIUQO_YOLO_MODE=1``,
and ``xiuqo config set approvals.mode off`` actually bypassed.
...
def _clear_approval_state(monkeypatch):
    ...
    # Xiuqo-driven test runs may inherit XIUQO_YOLO_MODE=1 from the parent
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``xiuqo --yolo`` sets ``XIUQO_YOLO_MODE`` before tool imports, so
