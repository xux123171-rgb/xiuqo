"""Fixtures shared across xiuqo_cli tests."""
...
def all_assignees_spawnable(monkeypatch):
    """Pretend every assignee maps to a real Xiuqo profile.
...
def _suppress_concurrent_hermes_gate(request, monkeypatch):
    ...
    ``xiuqo.exe`` is detected (issue #26670). On a developer's Windows
    machine running the test suite via ``xiuqo`` itself, this would
    ...
    if request.node.get_closest_marker("real_concurrent_gate"):
        return
    try:
        from xiuqo_cli import main as _cli_main
    except Exception:
        return
    # raising=False: under pytest's per-test spawn isolation, a concurrent
    # xdist worker importing a module that transitively touches xiuqo_cli.main
...
def isolated_update_runtime(monkeypatch, tmp_path, request):
    ...
    from xiuqo_cli import gateway, main, update_cmd, update_cmd_fleet
    from xiuqo_cli import update_inventory, update_receipt
