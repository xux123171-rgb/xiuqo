"""A pooled Desktop backend (``xiuqo --profile X serve``, XIUQO_HOME=<root>/profiles/X) answers its
REST without ``?profile=``. Those UNSCOPED reads/verbs are about X — a profile the live default
multiplexer serves — and must agree with the scoped ``?profile=X`` answer and with ``xiuqo -p X status``:
...
restart`` under X's XIUQO_HOME that exited 78 while the UI reported success.
"""
...
def pooled_served_process(tmp_path, monkeypatch):
    """Process whose XIUQO_HOME is a served named profile; the default home records a live multiplexer."""
    root = tmp_path / "xiuqo"
    ...
    (root / "gateway_state.json").write_text(json.dumps({
        "pid": os.getpid(), "xiuqo_home": str(root), "gateway_state": "running",
        "served_profiles": ["default", "alpha"],
        "platforms": {"api_server": {"state": "connected"}, "alpha:telegram": {"state": "connected"}}}))
    monkeypatch.setenv("XIUQO_HOME", str(root / "profiles" / "alpha"))
...
def test_unscoped_lifecycle_verbs_in_a_served_profile_process_address_the_multiplexer(pooled_served_process):
    ...
    # The child must run under the DEFAULT home (the multiplexer's), not inherit alpha's XIUQO_HOME.
    assert _profile_action_environment(restart)["XIUQO_HOME"] == str(pooled_served_process)
