"""Invariants for the canonical SQLite connect/transaction layer (``xiuqo_cli/sqlite_util.py``).
...
from xiuqo_cli import sqlite_util
...
def test_every_store_opens_through_the_canonical_open_db(monkeypatch, tmp_path, module_name, attr):
    ...
    if module_name == "xiuqo_cli.projects_db":
...
def test_plugin_db_wal_goes_through_the_shared_fallback(monkeypatch, tmp_path):
    ...
    seen = []
    real = xiuqo_state_wal.apply_wal_with_fallback

    def spy(conn, **kwargs):
        seen.append(kwargs["db_label"])
        return real(conn, **kwargs)

    monkeypatch.setattr(xiuqo_state_wal, "apply_wal_with_fallback", spy)
