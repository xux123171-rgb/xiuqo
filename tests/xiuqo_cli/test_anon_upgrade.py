"""``xiuqo auth upgrade``: the free tier signs into a Nous account, keeping its connectors.
...
from xiuqo_cli.auth import _auth_file_path, _load_auth_store
...
class FakePortal:
    ...
    def handler(self, request: httpx.Request) -> httpx.Response:
        ...
        if path == "/api/oauth/token":
            ...
            self.token_grants += 1
            return httpx.Response(200, json={
                "access_token": _jwt(sub="nas_user:9", client_id="xiuqo-cli", account_tier="free"),
...
def free_account(monkeypatch):
    ...
    from xiuqo_cli import models as m
...
class TestSignInCompletionSettlesTheModel:
    ...
    def test_no_eligible_recommendation_leaves_no_default_rather_than_a_model_the_account_may_not_use(
        ...
        assert "run `xiuqo model` to pick one" in out
