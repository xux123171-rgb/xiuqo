"""Tests for npm ``EBADENGINE`` recovery (``xiuqo_cli/npm_engine.py``).

The behaviour under test is a contract about *reacting* to npm's own engine
check: npm states the range it wants in the failure, Xiuqo upgrades only an
...
import xiuqo_cli.npm_engine as npm_engine
from xiuqo_cli.npm_engine import (
    actual_npm_version,
    is_ebadengine,
    managed_npm_prefix,
...
npm error engine Not compatible with your version of node/npm: xiuqo@1.0.0
npm error notsup Not compatible with your version of node/npm: xiuqo@1.0.0
...
class TestManagedDetection:
    ...
    no other npm — this is the boundary between "Xiuqo fixes it" and "the
...
class TestRepairDecision:
    ...
    def test_foreign_npm_provisions_managed_runtime_instead(
        ...
        """A system/nvm/brew/Nix npm is never modified — Xiuqo provisions its
