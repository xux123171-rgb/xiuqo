"""Tests for xiuqo_cli/goals.py — persistent cross-turn goals."""
...
def xiuqo_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves XIUQO_HOME.
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from xiuqo_cli.goals import _parse_judge_response
...
class TestGoalManager:
    ...
    def test_set_then_status(self, xiuqo_home):
    ...
    def test_continuation_prompt_shape(self, xiuqo_home):
        ...
        mgr.set("port goal command to xiuqo")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to xiuqo" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from xiuqo_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, xiuqo_home):
        """N=3 consecutive parse failures → auto-pause with config pointer."""
        from xiuqo_cli import goals
        from xiuqo_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        from xiuqo_cli.goals import GoalState
...
class TestMigrateGoalToSession:
    ...
    def test_migrates_active_goal_to_child(self, xiuqo_home):
        from xiuqo_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
    ...
    def test_does_not_clobber_existing_child_goal(self, xiuqo_home):
        from xiuqo_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
...
class TestGoalManagerSubgoals:
    def test_add_subgoal(self, xiuqo_home):
        from xiuqo_cli.goals import GoalManager
    ...
    def test_remove_subgoal_out_of_range(self, xiuqo_home):
        import pytest
        from xiuqo_cli.goals import GoalManager
...
class TestJudgeGoalWithSubgoals:
    def test_judge_uses_subgoals_template_when_provided(self, xiuqo_home):
    ...
    def test_judge_uses_original_template_when_no_subgoals(self, xiuqo_home):
        from unittest.mock import patch
        from xiuqo_cli import goals
...
class TestStatusLineSubgoalCount:
    ...
    def test_status_line_with_subgoals(self, xiuqo_home):
        from xiuqo_cli.goals import GoalManager
...
class TestWaitBarrier:
    ...
    def test_parked_on_live_pid_does_not_continue_or_judge(self, xiuqo_home):
        from xiuqo_cli import goals
        from xiuqo_cli.goals import GoalManager
    ...
    def test_stop_waiting_clears_barrier(self, xiuqo_home):
        from xiuqo_cli.goals import GoalManager
    ...
    def test_barrier_on_a_process_that_never_exits_expires(self, xiuqo_home):
...
class TestJudgeDrivenWait:
    ...
    def test_judge_wait_pid_parks_loop(self, xiuqo_home):
        from xiuqo_cli import goals
        from xiuqo_cli.goals import GoalManager
    ...
    def test_time_barrier_clears_after_deadline(self, xiuqo_home):
        from xiuqo_cli.goals import GoalManager
    ...
    def test_continue_verdict_still_continues_with_background(self, xiuqo_home):
...
class TestSessionTriggerBarrier:
    ...
    def test_registry_releases_on_watch_match_while_alive(self, xiuqo_home):
    ...
    def test_wait_on_session_validation(self, xiuqo_home):
        from xiuqo_cli.goals import GoalManager
...
class TestParseContract:
    ...
    def test_inline_fields_parsed(self):
        from xiuqo_cli.goals import parse_contract
...
class TestGoalContractSerialization:
    def test_roundtrip_with_contract(self):
        from xiuqo_cli.goals import GoalState, GoalContract
    ...
    def test_render_block_omits_empty_fields(self):
        from xiuqo_cli.goals import GoalContract
...
class TestGoalManagerContract:
    ...
    def test_set_contract_after_the_fact(self, xiuqo_home):
        from xiuqo_cli.goals import GoalManager, GoalContract
        ...
        # Survives reload.
        from xiuqo_cli.goals import GoalManager as GM2
        assert GM2(session_id="c-after").has_contract()

    def test_persistence_roundtrip(self, xiuqo_home):
        from xiuqo_cli.goals import GoalManager, GoalContract
...
class TestJudgeWithContract:
    ...
    def test_judge_uses_contract_template(self, xiuqo_home):
        from unittest.mock import patch
        from xiuqo_cli import goals
        from xiuqo_cli.goals import GoalContract
...
class TestDraftContract:
    def test_draft_parses_json(self, xiuqo_home):
        from unittest.mock import patch
        from xiuqo_cli import goals
    ...
    def test_draft_returns_none_when_no_client(self, xiuqo_home):
        from unittest.mock import patch
        from xiuqo_cli import goals
...
class TestContractAndBackgroundCompose:
    ...
    def test_judge_prompt_carries_contract_and_background(self, xiuqo_home):
        from unittest.mock import patch
        from xiuqo_cli import goals
        from xiuqo_cli.goals import GoalContract
...
class TestBlockedVerdict:
    ...
    def test_blocked_verdict_pauses_goal_instead_of_done(self, xiuqo_home):
        from unittest.mock import patch
        from xiuqo_cli.goals import GoalManager
...
def test_goal_session_db_is_the_registry_shared_handle(xiuqo_home):
    ...
    from xiuqo_cli import goals
    import xiuqo_state_registry as registry
