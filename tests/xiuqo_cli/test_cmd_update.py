...
from xiuqo_cli.main import cmd_update, PROJECT_ROOT
...
def _patch_managed_uv(request):
    ...
    with patch("xiuqo_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         patch("xiuqo_cli.managed_uv.ensure_uv", side_effect=_fake_ensure_uv), \
         patch("xiuqo_cli.managed_uv.update_managed_uv", side_effect=_fake_update_managed_uv), \
         patch(
             "xiuqo_cli.update_cmd._post_update_sqlite_runtime_status",
...
class TestCmdUpdateNpmLockfileCache:
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `xiuqo update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        shared_root = tmp_path / ".xiuqo"
        named_profile = shared_root / "profiles" / "work"
        named_profile.mkdir(parents=True)

        monkeypatch.setattr(hm, "PROJECT_ROOT", checkout)
        monkeypatch.setattr(xiuqo_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            xiuqo_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("XIUQO_HOME", str(shared_root))
            update_cmd._update_node_dependencies()

            monkeypatch.setenv("XIUQO_HOME", str(named_profile))
            update_cmd._update_node_dependencies()
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm, "_is_termux_env", lambda env=None: True)
        # Production resolve_uv only checks $XIUQO_HOME/bin/uv; model an empty
        # managed dir so the PATH probe is what surfaces the packaged uv.
        monkeypatch.setattr("xiuqo_cli.managed_uv.resolve_uv", lambda: None)
...
class TestUpdateManagedPythonEnvIsolation:
    ...
    forces UV_PYTHON_INSTALL_DIR to .xiuqo-runtime/python), then re-points
    ...
    def test_update_uv_env_points_venv_and_runtime_store(self):
        ...
        assert ".xiuqo-runtime" in uv_env.get("UV_PYTHON_INSTALL_DIR", "")
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind xux123171-rgb/xiuqo silently misses updates.
        """
        from xiuqo_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/xiuqo.git",
    ...
    def test_current_checkout_python_repair_failure_is_durable(
        ...
        with patch.object(
            ...
            "xiuqo_cli.update_receipt.finalize_update_receipt"
        ) as finalize_receipt, patch(
            "xiuqo_cli.update_receipt.finalize_pending_update_receipt"
        ):
    ...
    def test_fork_upstream_sync_that_moves_head_runs_post_update_steps(
        ...
        with patch.object(
            ...
            "xiuqo_cli.update_receipt.collect_fleet_versions",
            return_value=[],
        ), patch(
            ...
            "xiuqo_cli.gateway.find_gateway_pids",
            return_value=[],
        ), patch(
            "xiuqo_cli.gateway.find_profile_gateway_processes",
            return_value=[],
        ), patch(
            "xiuqo_cli.gateway._get_service_pids",
            return_value=set(),
        ), patch.object(
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "xiuqo_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "xiuqo_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch(
            "xiuqo_cli.update_cmd._reload_config_modules"
        ), patch(
            "xiuqo_cli.update_cmd._run_config_check_fresh", return_value=(1, 2)
        ), patch(
            "xiuqo_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ) as migrate_config, patch("xiuqo_cli.main.sys") as mock_sys:
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "xiuqo_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "xiuqo_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "xiuqo_cli.update_cmd._reload_config_modules"
        ), patch(
            "xiuqo_cli.update_cmd._run_config_check_fresh", return_value=(5, 24)
        ), patch(
            "xiuqo_cli.update_cmd._run_migrate_config_fresh",
    ...
    def test_version_bump_only_surfaces_migration_resets(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "xiuqo_cli.update_cmd._run_config_check_fresh", return_value=(33, 34)
        ), patch(
            "xiuqo_cli.update_cmd._run_migrate_config_fresh",
            return_value={
                "env_added": [],
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "xiuqo_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "xiuqo_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "xiuqo_cli.update_cmd._reload_config_modules"
        ), patch(
            "xiuqo_cli.update_cmd._run_config_check_fresh", return_value=(1, 24)
        ), patch(
            "xiuqo_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("xiuqo_cli.main.sys") as mock_sys:
...
class TestConfigVersionCheckUsesFreshModules:
    ...
    Before the fix, ``xiuqo update`` ran in the PRE-pull Python process.
    After ``git pull`` updated the source on disk, function-level imports
    returned the OLD cached ``xiuqo_cli.config`` module — so
    ...
    def test_run_config_check_fresh_reloads_modules(self):
        ...
        sys.modules held the OLD xiuqo_cli.config with the OLD
        DEFAULT_CONFIG["_config_version"] after git pull.
        """
        from unittest.mock import patch

        import xiuqo_cli.update_cmd as update_cmd
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.xiuqo"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.xiuqo/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.xiuqo/profiles/work"))
        ...
        with (
            patch("xiuqo_cli.profiles.list_profiles", return_value=all_profiles),
            patch("xiuqo_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("xiuqo_cli.profiles.list_profiles", return_value=[default_p]),
            patch("xiuqo_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``xiuqo update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``xiuqo update --check --branch <name>`` honors the branch override.
...
class TestCmdUpdateZipBranchRefusal:
    """``xiuqo update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from xiuqo_cli.update_cmd import _update_via_zip
...
class TestNodeRuntimeNpmResolution:
    ...
    def test_wsl_update_skips_windows_npm_build_paths(self, mock_args, monkeypatch):
        ...
        monkeypatch.setattr(xiuqo_constants, "is_wsl", lambda: True)
        monkeypatch.setattr(
            xiuqo_constants,
    ...
    def test_update_rebuilds_desktop_that_disappears_mid_update(self):
        ...
        packaged_exe = desktop_dir / "release" / "win-unpacked" / "Xiuqo.exe"
        ...
        desktop_build.assert_called_once_with(
            [hm.sys.executable, "-m", "xiuqo_cli.main", "desktop", "--build-only"],
            cwd=PROJECT_ROOT,
            env=ANY,
        )
    ...
    def test_git_failure_zip_fallback_rebuilds_missing_desktop(self, tmp_path, monkeypatch):
        ...
        desktop app (release/win-unpacked/Xiuqo.exe) is preserved THROUGH
        ...
        def write_source_zip(_url, destination):
            with zipfile.ZipFile(destination, "w") as archive:
                archive.writestr("xiuqo-main/apps/desktop/package.json", "{}")
        ...
        monkeypatch.setattr(update_cmd, "get_hermes_home", lambda: tmp_path / "xiuqo-home")
        ...
        with (
            ...
            patch("xiuqo_cli.managed_uv.ensure_uv", return_value="uv"),
            patch("xiuqo_cli.managed_uv.update_managed_uv"),
            patch(
            ...
            patch("xiuqo_cli.model_catalog.seed_cache_from_checkout", return_value=False),
        ):
...
class TestUpdateNodeDependencies:
    ...
    def test_records_lockfile_hash_only_on_success(self, _which, mock_popen, tmp_path, monkeypatch):
        ...
        from xiuqo_cli import update_cmd_deps
...
class TestGitTrampolineSelfHeal:
    ...
    def test_healthy_git_command_unchanged(self):
        ...
        with (
            ...
            patch(
                "xiuqo_cli.update_cmd.subprocess.run",
                side_effect=self._fake_run_healthy,
            ),
            patch("xiuqo_cli.update_cmd._locate_real_git") as locate,
        ):
    ...
    def test_trampoline_swaps_to_real_git(self, capsys):
        ...
        with (
            ...
            patch(
                "xiuqo_cli.update_cmd._locate_real_git", return_value=real
            ),
        ):
            result = update_cmd._ensure_non_trampoline_git(git_cmd)
    ...
    def test_trampoline_no_real_git_keeps_command(self, capsys):
        ...
        with (
            ...
            patch("xiuqo_cli.update_cmd._locate_real_git", return_value=None),
        ):
            result = update_cmd._ensure_non_trampoline_git(git_cmd)
        assert result == git_cmd
        out = capsys.readouterr().out
    ...
    def test_off_windows_noop(self):
        ...
        with (
            ...
            patch("xiuqo_cli.update_cmd.subprocess.run") as run,
        ):
            result = update_cmd._ensure_non_trampoline_git(git_cmd)
        assert result == git_cmd
    ...
    def test_portable_git_candidates_check_shared_root_first(self, tmp_path, monkeypatch):
        # Profile-scoped layout: XIUQO_HOME = <root>/profiles/foo, but the
