...
Regression: the old implementation wrote ``xiuqo_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``xiuqo sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.xiuqo/sessions/saved/`` and prints
...
def xiuqo_home(tmp_path, monkeypatch):
    ...
    # Clear any cached xiuqo_home computation
    import xiuqo_constants
    if hasattr(xiuqo_constants, "_hermes_home_cache"):
        xiuqo_constants._hermes_home_cache = None
    return home
...
def test_save_conversation_writes_under_hermes_home(xiuqo_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.xiuqo/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the XIUQO_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "xiuqo_constants"]:
    ...
    cwd_leak = list(work.glob("xiuqo_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.xiuqo/sessions/saved/
    saved_dir = xiuqo_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("xiuqo_conversation_*.json"))
    ...
    assert "xiuqo --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(xiuqo_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "xiuqo_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bare_shows_usage(xiuqo_home, capsys):
...
def test_save_conversation_bad_format_shows_usage(xiuqo_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "xiuqo_constants"]:
        sys.modules.pop(mod, None)
    import cli
