"""Tests for xiuqo_cli.gateway_windows."""
...
import xiuqo_cli.setup as setup
...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda xiuqo_home: "")
    monkeypatch.setattr("xiuqo_cli.config.get_hermes_home", lambda: str(xiuqo_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(venv_python), "-m", "xiuqo_cli.main"]
    assert cwd == str(xiuqo_home.resolve())
...
def test_spawn_detached_marks_primary_breakaway_success(monkeypatch, tmp_path, caplog):
    ...
    argv = ["python.exe", "-m", "xiuqo_cli.main", "gateway", "run"]
    cwd = str(tmp_path)
    calls = []
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_build_gateway_argv",
        lambda: (argv, cwd, {"XIUQO_GATEWAY_DETACHED": "1"}),
    )
    monkeypatch.setattr("xiuqo_cli.config.get_hermes_home", lambda: tmp_path)
...
def test_spawn_detached_warns_and_marks_no_breakaway_fallback(
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            argv,
            cwd,
            {"XIUQO_GATEWAY_DETACHED": "1", "SECRET_SENTINEL": "do-not-log"},
        ),
    )
    monkeypatch.setattr("xiuqo_cli.config.get_hermes_home", lambda: tmp_path)
    monkeypatch.setattr(gateway_windows.subprocess, "Popen", fake_popen)
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_hermes_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".xiuqo"
        home.mkdir()
        monkeypatch.setattr("xiuqo_cli.config.get_hermes_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".xiuqo"
        project = tmp_path / "checkout"
        monkeypatch.setattr("xiuqo_cli.config.get_hermes_home", lambda: missing)
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Xiuqo\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Xiuqo\venv\Scripts\python.exe"
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "xiuqo_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("XIUQO_HOME", "PYTHONIOENCODING", "XIUQO_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
# resume_pending=True) never fires when `xiuqo gateway stop` kills the
