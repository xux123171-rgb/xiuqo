...
from xiuqo_cli.subcommands.dashboard import build_dashboard_parser
...
from xiuqo_cli.subcommands.whatsapp import build_whatsapp_parser
...
def test_config_get_unset_subcommands_parse():
    """`xiuqo config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="xiuqo")
...
# ── deprecated `xiuqo login` fails gracefully, not with argparse error ────
#
# `xiuqo login` is a removed command; its handler (`login_command` in
# `xiuqo_cli/auth.py`) prints a deprecation notice pointing at `xiuqo auth` /
# `xiuqo model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `xiuqo --help`.
