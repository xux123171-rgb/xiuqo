"""Tests for xiuqo_cli.update_receipt — Phase 1 of the fleet-update plan (#91277).
...
def receipt_home(tmp_path, monkeypatch):
    """Isolated XIUQO_HOME for receipt writes."""
    home = tmp_path / ".xiuqo"
    home.mkdir()
    monkeypatch.setattr(
        "xiuqo_cli.config.get_hermes_home", lambda: home, raising=False
    )
...
class TestReceiptLifecycle:
    def test_begin_record_finalize_roundtrip(self, receipt_home):
        ...
        ur.record_gateway_restart(
            restarted_services=["xiuqo-gateway"],
        ...
        assert gr["restarted_services"] == ["xiuqo-gateway"]
...
class TestCommandBoundaryFinalization:
    ...
    def test_pending_receipt_persisted_on_exit_2_refusal(self, receipt_home):
        ur.begin_update_receipt()
        ur.record_step("windows_preflight", False, "another xiuqo.exe running")
    ...
    def test_cmd_update_boundary_finalizes_on_early_exit(
        ...
        def _fake_impl(args, gateway_mode):
            ur.begin_update_receipt()
            ur.record_step("windows_preflight", False, "xiuqo.exe holds venv")
            sys.exit(2)

        monkeypatch.setattr(update_cmd, "_cmd_update_impl", _fake_impl)
        monkeypatch.setattr(
            xiuqo_main, "detect_install_method", lambda *a, **k: "git", raising=False
        )
        monkeypatch.setattr(
            xiuqo_main,
        ...
        monkeypatch.setattr(
            xiuqo_main, "_finalize_update_output", lambda state: None, raising=False
        )
        ...
        import xiuqo_cli.update_lock as update_lock_mod
...
class TestFleetClassification:
    def _fleet_with(self, monkeypatch, tmp_path, record, expected_sha="a" * 40):
        ...
        monkeypatch.setattr(
            "xiuqo_cli.profiles._get_default_hermes_home", lambda: home
        )
        monkeypatch.setattr(
            "xiuqo_cli.profiles._get_profiles_root",
...
class TestGatewayStatusStamping:
    ...
    def test_code_identity_failure_degrades_to_absent(self, monkeypatch):
        ...
        monkeypatch.setattr("xiuqo_cli.build_info.get_code_identity", _boom)
        record = gs._build_runtime_status_record()
