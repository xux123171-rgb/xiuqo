...
``xiuqo config get`` against broken YAML used to run on ``DEFAULT_CONFIG`` — dropping every
...
from xiuqo_cli.config_backups import list_config_backups
...
def _fresh_load(home: Path) -> tuple[dict, str]:
    env = {**os.environ, "XIUQO_HOME": str(home), "PYTHONPATH": str(REPO), "LKG_TOKEN": "expanded-secret"}
    proc = subprocess.run(
        [sys.executable, "-c", "import json; from xiuqo_cli.config import load_config; print(json.dumps(load_config()))"],
