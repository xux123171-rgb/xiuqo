"""config.yaml backups: one dir, deduped, bounded — never a pile of siblings in XIUQO_HOME."""
from pathlib import Path

from xiuqo_cli.config_backups import backup_config, list_config_backups
...
def test_repeat_backups_dedupe_and_rotate(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("xiuqo_cli.config_backups.time.strftime", lambda _fmt: next(stamps))

    first = backup_config(cfg, "pre-setup", keep=2)
    assert first is not None and first.parent == tmp_path / "backups" / "config"
    # Same bytes again → no new file (the xiuqo-setup-three-times case).
