...
board into `XIUQO_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
`xiuqo kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `xiuqo kanban boards switch` from another session
...
def _isolate_kanban_board_env():
    """Snapshot `XIUQO_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("XIUQO_KANBAN_BOARD")
    os.environ.pop("XIUQO_KANBAN_BOARD", None)
    try:
        yield
    finally:
        if prev is None:
...
def test_pin_does_not_overwrite_existing_env(monkeypatch):
    ...
    assert main_mod.os.environ.get("XIUQO_KANBAN_BOARD") == "preset"
