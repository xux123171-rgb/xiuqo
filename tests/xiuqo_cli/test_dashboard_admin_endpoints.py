...
def _client():
    ...
    # Keep the state DB under the isolated XIUQO_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def test_http_bearer_auth_separates_secret_from_config(
        ...
        env_text = (xiuqo_home / ".env").read_text()
    ...
    def test_http_oauth_mode_is_persisted_for_existing_auth_flow(self):
        ...
        from xiuqo_cli.mcp_config import _get_mcp_servers
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.xiuqo/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `xiuqo auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `xiuqo auth add`, which clears every suppression for the
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "xiuqo_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("xiuqo-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # xiuqo-index source advertises availability + featured search.
            idx = _Src("xiuqo-index")
        ...
        assert {"official", "github", "xiuqo-index"} <= ids
...
class TestOfficialSkillsCatalogEndpoint:
    ...
    def test_lists_full_catalog_with_installed_flags(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "xiuqo_cli.web_routers.skills._installed_hub_identifiers",
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "xiuqo_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/xiuqo/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/xiuqo/update`` runs ``xiuqo update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import xiuqo_cli.banner as banner

        monkeypatch.setattr(banner, "check_for_updates", lambda: 5)

        r = self.client.get("/api/xiuqo/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_managed_runtime_dashboard_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/xiuqo/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        ...
        monkeypatch.setattr("xiuqo_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `xiuqo tools` configurator."""
...
def test_spawn_hermes_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    ...
    _XIUQO_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    assert "_XIUQO_GATEWAY" not in captured["env"]
    assert captured["env"]["XIUQO_NONINTERACTIVE"] == "1"
...
def test_named_profile_action_isolates_parent_env_and_loads_target_env(monkeypatch, tmp_path):
    ...
    import xiuqo_cli.env_loader as env_loader
    import xiuqo_cli.web_server as ws

    user_home = tmp_path / "user"
    default_home = user_home / ".xiuqo"
    ...
    for key, value in {
        ...
        "XIUQO_ACP_AUTH_METHOD": "default-acp",
    ...
    assert child_env["XIUQO_HOME"] == str(target_home)
    assert child_env["XIUQO_NONINTERACTIVE"] == "1"
    ...
    for leaked_key in (
        ...
        "A2A_AUTH_MINI", "EXTERNAL_PROFILE_AUTH", "XIUQO_ACP_AUTH_METHOD",
    ):
    ...
    probe = subprocess.run(
        [
            ...
            "load_hermes_dotenv(xiuqo_home=os.environ['XIUQO_HOME']); "
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    ...
    startup under XIUQO_DESKTOP=1 so the stale orphan is cleared first.
    """
    import xiuqo_cli.web_server as ws
    ...
    # web_server imports the reaper lazily from xiuqo_cli.gateway, so patch it
    # on that module.
    import xiuqo_cli.gateway as g
