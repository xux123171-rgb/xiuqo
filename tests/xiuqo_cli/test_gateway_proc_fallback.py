...
_GATEWAY_CMD = "python -m xiuqo_cli.main gateway run"
...
class TestProcFallback:
    ...
    def test_detects_gateway_pid_via_proc(self):
        my_pid = os.getpid()
        entries = {
            my_pid: "python -m xiuqo_cli.main",   # own process — excluded
            12345: _GATEWAY_CMD,
            99999: _OTHER_CMD,
        }
        ...
        with (
            ...
            patch("xiuqo_cli.gateway._get_ancestor_pids", return_value=set()),
...
class TestPsFallbackBsdCompat:
    ...
    def test_ps_flags_are_bsd_compatible(self):
        ...
        with (
            patch("xiuqo_cli.gateway.is_windows", return_value=False),
...
class TestGetServicePidsAllProfiles:
    ...
    def test_default_scope_uses_current_profile_label(self):
        ...
        with (
            patch("xiuqo_cli.gateway.is_macos", return_value=True),
            patch("xiuqo_cli.gateway.supports_systemd_services", return_value=False),
            patch(
                "xiuqo_cli.gateway.get_launchd_label",
                return_value="ai.xiuqo.gateway.myprofile",
            ),
            patch(
                "xiuqo_cli.gateway._locate_launchd_gateway_service",
        ...
        assert located == ["ai.xiuqo.gateway.myprofile"]
        launchctl_calls = [
            c[0][0]
    ...
    def test_all_profiles_enumerates_all_gateway_labels(self):
        ...
        widens the EXCLUDE set with unmapped ai.xiuqo.gateway* agents
        (#74075 belt-and-suspenders)."""
        located = []
        label_pids = {
            "ai.xiuqo.gateway": 123,
            "ai.xiuqo.gateway-profile-b": 456,
        }
        ...
        with (
            ...
            patch(
                "xiuqo_cli.gateway.get_launchd_label",
                return_value="ai.xiuqo.gateway",
            ),
            patch(
                "xiuqo_cli.gateway.launchd_gateway_labels_for_install",
                return_value=["ai.xiuqo.gateway", "ai.xiuqo.gateway-profile-b"],
            ),
            patch(
                "xiuqo_cli.gateway._locate_launchd_gateway_service",
                side_effect=_fake_locate,
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "999\t0\tai.xiuqo.gateway-unmapped\n"
    ...
    def test_all_profiles_empty_when_no_gateway_labels(self):
        """When no ai.xiuqo.gateway* labels exist, all_profiles returns empty."""
        with (
            patch("xiuqo_cli.gateway.is_macos", return_value=True),
            patch("xiuqo_cli.gateway.supports_systemd_services", return_value=False),
            patch("subprocess.run") as mock_run,
    ...
    def test_all_profiles_handles_broken_pid_column(self):
        ...
        with (
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "-\t0\tai.xiuqo.gateway.broken\n"
                    "  123  \t0\tai.xiuqo.gateway.ok\n"
                ),
                stderr="",
            )
            pids = gateway_mod._get_service_pids(all_profiles=True)
    ...
    def test_all_profiles_preserves_systemd_behavior(self):
        ...
        lists every xiuqo-gateway* unit unconditionally."""
        with (
