...
from xiuqo_cli.web_routers import _common, analytics
...
def test_corrupt_store_polls_return_status_and_warn_once_per_interval(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(xiuqo_state, "_default_db_path", lambda: db_path)
    monkeypatch.setattr(_common, "_corrupt_store_warned_at", {})
    app = FastAPI()
    app.include_router(analytics.router)
    client = TestClient(app)

    with caplog.at_level(logging.DEBUG, logger="xiuqo_cli.web_server"):
    ...
    for resp in (first, second, third):
        ...
        assert "xiuqo doctor" in resp.json()["detail"]["message"]
    # Only the dashboard's own warning counts: xiuqo_state logs an unrelated
    # once-per-process SQLite-version advisory on some interpreters (CI's 3.50.4).
    warnings = [
        r for r in caplog.records
        if r.levelno >= logging.WARNING and r.name.startswith("xiuqo_cli.web_server")
    ]
    ...
    caplog.clear()
    with caplog.at_level(logging.WARNING, logger="xiuqo_cli.web_server"):
        assert client.get("/api/analytics/usage?days=7").status_code == 503
    assert sum(
        r.levelno >= logging.WARNING and r.name.startswith("xiuqo_cli.web_server")
