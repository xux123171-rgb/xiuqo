"""Regression: ``xiuqo doctor --fix`` must not checkpoint the live WAL under a running gateway.
...
from xiuqo_cli.doctor_state import _state_db_wal


# NOTE: no ``requires_wal`` marker here on purpose. That gate exists for tests
# that depend on Xiuqo *choosing* WAL mode (declined on vulnerable SQLite
