...
``xiuqo auth add copilot`` or by ``_seed_from_env`` when the env var
is set in ``~/.xiuqo/.env`` — the picker was silently dropping back to
...
from xiuqo_cli.models import _resolve_copilot_catalog_api_key
