...
def test_memo_does_not_leak_across_multiplex_profile_contexts(tmp_path):
    ...
    xiuqo_constants.set_hermes_home_override (gateway/run.py,
    tui_gateway/server.py), not the XIUQO_HOME env var — the memo must key
    ...
    reset_token = xiuqo_constants.set_hermes_home_override(str(profile_a))
    try:
        token_a = auth.resolve_nous_access_token()
    finally:
        xiuqo_constants.reset_hermes_home_override(reset_token)
    ...
    reset_token = xiuqo_constants.set_hermes_home_override(str(profile_b))
    try:
