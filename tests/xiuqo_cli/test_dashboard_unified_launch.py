...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "xiuqo_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # XIUQO_HOME.  For a standard install (XIUQO_HOME unset) that root is
        # the platform-native default (~/.xiuqo), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from xiuqo_constants import get_default_hermes_root
        assert env.get("XIUQO_HOME") == str(get_default_hermes_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (XIUQO_DESKTOP=1) must NOT
