"""Tests for the `xiuqo memory reset` CLI command.
...
- Profile-scoped reset (uses XIUQO_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake XIUQO_HOME with memory files."""
    xiuqo_home = tmp_path / ".xiuqo"
    memories = xiuqo_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("XIUQO_HOME", str(xiuqo_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nHermes repo is at ~/.xiuqo/xiuqo\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `xiuqo memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        xiuqo_home = tmp_path / ".xiuqo"
        (xiuqo_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("XIUQO_HOME", str(xiuqo_home))
