"""Tests for xiuqo_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate XIUQO_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".xiuqo"
    home.mkdir()
    ...
    import importlib
    from xiuqo_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from xiuqo_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    FALLBACK = (
        "https://raw.githubusercontent.com/xux123171-rgb/xiuqo"
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import xiuqo_cli.model_catalog as model_catalog
        from xiuqo_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestRefreshCadence:
    def test_default_ttl_is_twenty_minutes_and_legacy_hours_honoured(self):
        ...
        with patch("xiuqo_cli.config.load_config", return_value={"model_catalog": {"ttl_minutes": 20}}):
        ...
        with patch("xiuqo_cli.config.load_config", return_value={"model_catalog": {"ttl_minutes": 20, "ttl_hours": 3}}):
    ...
    def test_refresh_catalogs_forces_every_source(self):
        ...
        with patch.object(model_catalog, "_load_catalog_config", return_value={
            "enabled": True, "url": "http://master", "ttl_hours": 1.0, "providers": {},
             ...
             patch("xiuqo_cli.models.fetch_openrouter_models") as orm, \
             patch("xiuqo_cli.models.fetch_nous_recommended_models") as nous:
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in xiuqo_cli.models."""
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``xiuqo model`` CLI — not the live
        ...
        # ``XIUQO_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` XIUQO_HOME directly instead.
        import importlib
        from xiuqo_cli import model_catalog
        from xiuqo_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from xiuqo_cli.model_switch_providers import list_picker_providers

            active_home = Path(os.environ["XIUQO_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("xiuqo_cli.models.check_nous_free_tier", return_value=False), patch(
                "xiuqo_cli.models.union_with_portal_free_recommendations",
