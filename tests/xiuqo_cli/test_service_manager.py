"""Tests for xiuqo_cli.service_manager — the abstract ServiceManager
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `xiuqo_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import xiuqo_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("xiuqo_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
...
# unprivileged xiuqo user from every lifecycle op. These tests run
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from xiuqo_cli.service_manager import _seed_supervise_skeleton
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``xiuqo gateway run``, an agent action, the Open WebUI helper) holds
    the per-XIUQO_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root XIUQO_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `xiuqo gateway run --replace` (no -p flag).
    assert "xiuqo gateway run --replace" in default_text
    assert "xiuqo -p default" not in default_text
...
#
# `xiuqo gateway stop` inside a container dispatches through
...
def test_s6_log_run_creates_leaf_as_hermes_without_chown(
    ...
    """log/run must not root-chown/unlink volume paths; create leaf as xiuqo.

    #45258 parent ownership is stage2's job (``logs/gateways`` seeded as
    xiuqo). Restartable log/run must not pathname-chown or pathname-rm a
    xiuqo-writable tree from root — that is a symlink TOCTOU hole.
    """
    ...
    assert not any(line.lstrip().startswith("chown ") for line in log_text.splitlines()), (
        "restartable log/run must not invoke chown on xiuqo-writable paths; "
        f"saw: {log_text!r}"
    )
    assert 's6-setuidgid xiuqo mkdir -p "$log_dir"' in log_text
    assert 's6-setuidgid xiuqo rm -f "$log_dir/lock"' in log_text
    assert 'else\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n' in log_text
    ...
    mkdir_as_hermes_idx = log_text.index('s6-setuidgid xiuqo mkdir -p "$log_dir"')
    rm_as_hermes_idx = log_text.index('s6-setuidgid xiuqo rm -f "$log_dir/lock"')
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    gateways = xiuqo_home / "logs" / "gateways"
    ...
    # Mark the drop so fake rm can refuse unlink outside XIUQO_HOME the way
    # a real xiuqo uid cannot delete a foreign root-owned lock.
    ...
    (bin_dir / "s6-setuidgid").write_text(
        "#!/bin/sh\n"
        "shift\n"
        'XIUQO_TEST_DROPPED=1 exec "$@"\n',
        encoding="utf-8",
    )
    real_rm = "/bin/rm"
    (bin_dir / "rm").write_text(
        "#!/bin/sh\n"
        # Privilege-dropped: no-op. Models that xiuqo cannot unlink a foreign
