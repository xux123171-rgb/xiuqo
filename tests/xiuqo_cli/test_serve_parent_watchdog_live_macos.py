...
calls ``os._exit(0)`` ~5ms after ``XIUQO_BACKEND_READY``, which no in-process
...
def test_live_backend_survives_timezone_drifted_parent_marker(tmp_path):
    ...
    try:
        ...
        env.update(
            ...
            PYTHONUNBUFFERED="1",
            XIUQO_PARENT_PID=str(parent.pid),
            XIUQO_PARENT_START_MARKER=drifted_marker,
            XIUQO_PARENT_NONCE="nonce-95693",
            XIUQO_SERVE_WATCHDOG_POLL_S="0.5",
        )
        env.pop("XIUQO_DESKTOP", None)
        code = (
        ...
        assert _read_until(serve, "XIUQO_BACKEND_READY"), "backend never announced READY"
