...
import pytest
from xiuqo_cli import profile_cmd
...
def profiles():
    ...
    Sibling test files reload ``xiuqo_cli.profiles`` / ``xiuqo_cli.main``;
    a top-level ``from xiuqo_cli.profiles import ...`` here would bind the
    ...
    """
    return importlib.import_module("xiuqo_cli.profiles")
...
def test_checkout_detection_does_not_depend_on_cwd(tmp_path, monkeypatch, profiles):
    """XIUQO_HOME inside a checkout must be detected even when cwd is elsewhere.
    ...
    checkout.mkdir()
    (checkout / ".git").write_text("gitdir: ../git\n", encoding="utf-8")
    xiuqo_home = checkout / "data_home"
    xiuqo_home.mkdir()
    elsewhere = tmp_path / "elsewhere"
    elsewhere.mkdir()
    monkeypatch.chdir(elsewhere)  # cwd has no .git ancestor inside tmp_path
    monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
    monkeypatch.setattr(profiles, "_get_default_hermes_home", lambda: xiuqo_home)
...
def test_cli_export_rejects_bad_profile_name_without_traceback(
    ...
    main_mod = importlib.import_module("xiuqo_cli.main")
    default_home = tmp_path / ".xiuqo"
    default_home.mkdir()
    monkeypatch.setattr(profiles, "_get_default_hermes_home", lambda: default_home)
...
def test_cli_export_default_does_not_write_into_the_current_checkout(
    ...
    monkeypatch.setattr(profiles, "_get_default_hermes_home", lambda: default_home)
    monkeypatch.setattr(
        "xiuqo_constants.get_default_hermes_root", lambda: default_home
    )
...
def test_slash_export_uses_the_same_managed_destination(
    ...
    mixin_mod = importlib.import_module("xiuqo_cli.cli_commands_mixin")
    default_home = tmp_path / ".xiuqo"
    default_home.mkdir()
    monkeypatch.setattr(profiles, "_get_default_hermes_home", lambda: default_home)
...
async def test_profile_export_api_uses_the_shared_managed_destination(
    ...
    from xiuqo_cli.web_models import ProfileExport

    router_mod = importlib.import_module("xiuqo_cli.web_routers.profiles")
...
def test_cwd_in_unrelated_checkout_does_not_prove_safety(
    ...
    of a XIUQO_HOME inside checkout B."""
