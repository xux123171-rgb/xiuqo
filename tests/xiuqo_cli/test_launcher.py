"""Tests for the top-level `./xiuqo` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./xiuqo` should use `xiuqo_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "xiuqo"
    called = []

    fake_main_module = types.ModuleType("xiuqo_cli.main")

    def fake_main():
        called.append("xiuqo_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "xiuqo_cli.main", fake_main_module)
