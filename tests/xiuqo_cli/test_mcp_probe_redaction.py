...
def _isolate_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("xiuqo_cli.config.get_config_path", lambda: config_path)
    monkeypatch.setattr("xiuqo_cli.config.get_env_path", lambda: env_path)
    return tmp_path
...
class TestRedactMcpProbeText:
    ...
    def test_header_display_masks_opaque_api_key(self):
        from xiuqo_cli.mcp_config import redact_mcp_header_display
...
class TestProbeHelperRedactsBeforeRaise:
    def test_probe_exception_leaving_helper_is_already_safe(self, monkeypatch):
        ...
        from xiuqo_cli.mcp_config import _probe_single_server
...
class TestCmdMcpTestRedaction:
    ...
    def test_probe_exception_is_redacted(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("xiuqo_cli.mcp_config._probe_single_server", boom)
        from xiuqo_cli.mcp_config import cmd_mcp_test
...
class TestSiblingProbeConsumersRedact:
    def test_mcp_add_redacts_probe_exception(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("xiuqo_cli.mcp_config._probe_single_server", boom)
        monkeypatch.setattr("xiuqo_cli.mcp_config._confirm", lambda *a, **k: False)
