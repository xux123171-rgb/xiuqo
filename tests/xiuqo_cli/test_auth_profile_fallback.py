...
When ``XIUQO_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.xiuqo/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.xiuqo            (has its own auth.json fixture)
    * Profile     -> tmp_path/.xiuqo/profiles/coder   (active, XIUQO_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Xiuqo is not
...
def test_codex_cooldown_clear_writes_to_the_store_that_owns_the_borrowed_pool(profile_env):
    ...
    from xiuqo_cli.auth_codex import clear_codex_pool_quota_cooldowns
...
def test_root_write_through_is_visible_to_the_next_fallback_read(profile_env):
    ...
    import os
    from xiuqo_cli.auth import _save_auth_store, read_credential_pool
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (XIUQO_HOME != ~/.xiuqo, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    xiuqo_home = tmp_path / "classic"
    xiuqo_home.mkdir()
