...
same way the `xiuqo model` CLI picker does.
"""

from threading import Event
from time import monotonic

import xiuqo_cli.inventory as inv
...
def test_prewarm_preserves_context_and_runs_once_per_profile(tmp_path, monkeypatch):
    ...
    def capture_context(_rows):
        ...
        observed[label] = (xiuqo_home_key(), dict(scope))
        started[label].set()
    ...
    try:
        ...
        assert observed["a"] == (
            xiuqo_home_key(tmp_path / "a"),
            {"PROFILE_MARKER": "a"},
        )
        assert observed["b"] == (
            xiuqo_home_key(tmp_path / "b"),
            {"PROFILE_MARKER": "b"},
        )
    finally:
        release.set()
