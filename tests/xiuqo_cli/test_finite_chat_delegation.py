...
def test_finite_chat_joins_parallel_children_before_final_response(tmp_path, mode):
    ...
    env.update(HOME=str(tmp_path), USERPROFILE=str(tmp_path), XIUQO_HOME=str(home),
               XIUQO_MANAGED_DIR=str(tmp_path / "managed"), TERMINAL_CWD=str(tmp_path),
    ...
    command = [
        sys.executable, "-c", "from xiuqo_cli.main import main; main()", "chat",
...
def test_tty_seeded_chat_keeps_background_delegation(monkeypatch, query, image):
    ...
    try:
        ...
        assert "XIUQO_SINGLE_QUERY_SESSION" not in os.environ
    finally:
        reset_session_vars()
