...
The notice prints the most recent curator run summary on `xiuqo update`,
...
session start) so users normally never see the rename map. `xiuqo update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from xiuqo_cli import update_cmd_maint
