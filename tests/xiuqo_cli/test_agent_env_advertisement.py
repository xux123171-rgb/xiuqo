"""Tests for the AI_AGENT / XIUQO_AGENT harness-attribution env vars.
...
Xiuqo-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal Xiuqo' id in the public agent-harness
registry (``xiuqo`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the Xiuqo process env, and survives the
cross-session leak guard that strips ``XIUQO_SESSION_*`` from subprocess
...
from xiuqo_cli.main import _advertise_agent_env
...
class TestAdvertiseAgentEnv:
    def test_sets_both_vars_when_unset(self, monkeypatch):
        ...
        monkeypatch.delenv("XIUQO_AGENT", raising=False)
        _advertise_agent_env()
        assert os.environ["AI_AGENT"] == HARNESS_ID
        assert os.environ["XIUQO_AGENT"] == "true"
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__XIUQO_CWD_testsession0__"
        env._snapshot_path = "/tmp/xiuqo-snap-testsession0.sh"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'XIUQO_AGENT="${XIUQO_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT XIUQO=$XIUQO_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "XIUQO_AGENT")}
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=clean_env, timeout=30,
        )
        assert f"AI={HARNESS_ID} XIUQO=true" in out.stdout

        outer_env = dict(clean_env, AI_AGENT="pi", XIUQO_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=outer_env, timeout=30,
        )
        assert "AI=pi XIUQO=false" in out.stdout
