"""Tests for xiuqo_cli/completion.py — shell completion script generation."""
...
from xiuqo_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real xiuqo structure."""
    p = argparse.ArgumentParser(prog="xiuqo")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "complete -F _hermes_completion xiuqo" in out
...
class TestProfileCompletion:
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'xiuqo profile use', complete with profile names."""
        out = generate_bash(_make_parser())
