...
from xiuqo_cli.providers import normalize_provider as normalize_overlay_provider
...
def test_actual_runtime_repairs_stale_responses_mode(monkeypatch, caplog):
    _clear_actual_env(monkeypatch)
    caplog.set_level(logging.INFO, logger="xiuqo_cli.auth")
    monkeypatch.setenv("ACTUAL_API_KEY", "actual-test-key")
    monkeypatch.setattr(
        rp,
        "_get_model_config",
...
def test_actual_profile_fetch_models_normalizes_env_base_url(monkeypatch):
    ...
    monkeypatch.setattr("xiuqo_cli.urllib_security.open_credentialed_url", _open)
...
def test_actual_oneshot_reasoning_override_reaches_agent(monkeypatch):
    ...
    monkeypatch.setattr(
        "xiuqo_cli.mcp_startup.ensure_mcp_discovery_before_agent_build",
        lambda **_kwargs: None,
    )
...
def test_oneshot_dispatch_forwards_reasoning_override(monkeypatch):
    from xiuqo_cli import main as main_mod
    from xiuqo_cli import oneshot
