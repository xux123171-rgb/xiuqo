"""Tests for xiuqo_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `xiuqo setup` and `xiuqo import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("xiuqo_constants.is_container", lambda: container)
