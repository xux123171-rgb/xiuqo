"""Tests for the update check mechanism in xiuqo_cli.banner.
...
def git_repo(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "xiuqo"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    monkeypatch.setenv("XIUQO_HOME", str(tmp_path))
    monkeypatch.delenv("XIUQO_REVISION", raising=False)
    ...
    monkeypatch.setattr("xiuqo_cli.config.get_project_root", lambda: repo_dir)
    return repo_dir


def _stub_git(monkeypatch, *, head=SHA_A, origin="https://github.com/xux123171-rgb/xiuqo.git"):
    calls = []
...
def test_cache_is_daily_but_invalidated_when_head_moves(git_repo, monkeypatch):
    """A fresh cache answers without any network; ``xiuqo update`` moving HEAD busts it at once;
