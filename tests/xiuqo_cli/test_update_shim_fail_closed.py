...
The bug class: on Windows, when `xiuqo.exe`/sibling shims could not be
...
def _make_shims(scripts_dir: Path, names=("xiuqo", "xiuqo-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # xiuqo.exe cannot be renamed; xiuqo-gateway.exe can
    real_rename = Path.rename

    def deny_hermes(self, target):
        if self.name == "xiuqo.exe":
    ...
    assert install_ran == []
    assert "xiuqo.exe" in exc_info.value.failed_shims
    # The successful rename (xiuqo-gateway.exe) was rolled back.
    assert (scripts / "xiuqo-gateway.exe").exists()
    assert not list(scripts.glob("xiuqo-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("xiuqo",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("xiuqo",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = main_install_repair.ShimQuarantineError(["xiuqo.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "xiuqo.exe" in out
