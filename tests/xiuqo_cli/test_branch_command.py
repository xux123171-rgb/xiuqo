...
def session_db(tmp_path):
    ...
    os.environ["XIUQO_HOME"] = str(tmp_path / ".xiuqo")
    os.makedirs(tmp_path / ".xiuqo", exist_ok=True)
    from xiuqo_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".xiuqo" / "test_sessions.db")
    yield db
    db.close()
...
class TestBranchCommandDef:
    ...
    def test_branch_in_registry(self):
        ...
        from xiuqo_cli.commands import COMMAND_REGISTRY
