...
underneath are exercised against temp XIUQO_HOME (autouse fixture). Network
...
def test_status_lists_staged_models_with_labels(client, tmp_path):
    from xiuqo_cli.local_runtime.bootstrap import models_dir
...
def test_status_tracks_preset_spill_and_restored_window(client, tmp_path, monkeypatch):
    ...
    from xiuqo_cli.local_runtime import bootstrap, presets
    ...
    from xiuqo_cli.web_routers import local_models
...
def test_catalog_never_hides_unaffordable_models(client, monkeypatch):
    ...
    monkeypatch.setattr("xiuqo_cli.local_runtime.hardware.probe_budget",
                        lambda **kw: tiny)
    data = client.get("/api/local-models/catalog").json()
...
def test_download_already_downloaded_short_circuits(client, monkeypatch):
    from xiuqo_cli.local_runtime.bootstrap import models_dir
    from xiuqo_cli.local_runtime.catalog import CATALOG, select_variant
    from xiuqo_cli.local_runtime.estimator import HardwareBudget
...
def test_runtime_install_rejects_impossible_combo(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "xiuqo_cli.local_runtime.binaries._host_os_arch", lambda: ("win", "arm64"))
...
def test_eject_without_supervisor_is_not_a_500(client, monkeypatch):
    ...
    server session."""
    monkeypatch.setattr(
        "xiuqo_cli.local_runtime.bootstrap.get_supervisor", lambda: None)
    # No running server either: the route must answer 409 (no server),
    # never a NameError 500.
    monkeypatch.setattr(
        "xiuqo_cli.web_routers.local_models._state_endpoint", lambda: None)
...
def test_download_tolerates_stale_catalog_size(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "xiuqo_cli.local_runtime.bootstrap.refresh_local_runtime",
        lambda: False)
