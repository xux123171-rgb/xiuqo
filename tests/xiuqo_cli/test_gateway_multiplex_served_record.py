...
- Probe: ``named_profile_served_by_running_multiplexer`` reads the pid-verified default
  ``gateway_state.json`` first (env-only opt-in on the default profile is invisible to ``xiuqo -p X``);
...
- ``xiuqo status`` / ``cron status`` on a satellite say running-via-multiplexer; the default lists served.
"""
...
def served_root(tmp_path, monkeypatch):
    ...
    (root / "gateway.pid").write_text(json.dumps({"pid": os.getpid(), "xiuqo_home": str(root)}))
    (root / "gateway_state.json").write_text(json.dumps(
        {"pid": os.getpid(), "xiuqo_home": str(root), "served_profiles": ["default", "coder"]}))
    monkeypatch.setenv("XIUQO_HOME", str(root / "profiles" / "coder"))
    monkeypatch.delenv("GATEWAY_MULTIPLEX_PROFILES", raising=False)
    import xiuqo_constants
    monkeypatch.setattr(xiuqo_constants, "_default_hermes_root_memo", None)
    return root
...
def test_status_surfaces_agree_for_a_satellite_profile(served_root, monkeypatch):
    import xiuqo_cli.gateway as gw
    import xiuqo_cli.status as st
    import xiuqo_cli.cron as cr
...
def test_dashboard_liveness_ladder_reports_served_profile_running(served_root):
    ...
    "stopped" while `xiuqo -p X status` said running. Alpha's `<X>:<platform>` entries project as its own."""
    ...
    (served_root / "gateway_state.json").write_text(json.dumps({
        "pid": os.getpid(), "xiuqo_home": str(served_root), "gateway_state": "running",
...
def test_dashboard_lifecycle_verbs_target_the_multiplexer(served_root, monkeypatch):
    ...
    from xiuqo_cli.web_server_gateway import _gateway_subcommand, _profile_action_environment, multiplexed_profile_refusal
    monkeypatch.setattr(profiles_mod, "_check_gateway_running", lambda home: False)
    # This process's own XIUQO_HOME is coder's; the restart child must still run under the DEFAULT
    ...
    assert _profile_action_environment(restart)["XIUQO_HOME"] == str(served_root)
