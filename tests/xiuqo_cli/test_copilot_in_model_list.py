...
from xiuqo_cli import model_switch_providers
from xiuqo_cli import models
...
def test_copilot_picker_uses_live_catalog_when_available():
    ...
    with patch("agent.models_dev.fetch_models_dev", return_value={}), \
         patch("xiuqo_cli.models._resolve_copilot_catalog_api_key", return_value="gh-token"), \
         patch("xiuqo_cli.models._fetch_github_models", return_value=live_models):
...
def _no_other_copilot_creds(monkeypatch):
    ...
    # the provider configured with no executable at all (xiuqo_cli/auth.py), so
    # a host that sets it would decide the outcome instead of the test.
    for var in ("GH_TOKEN", "GITHUB_TOKEN", "XIUQO_COPILOT_ACP_COMMAND",
...
def test_copilot_acp_listed_when_executable_resolves(tmp_path, monkeypatch, _no_other_copilot_creds):
    ...
    fake.chmod(0o755)
    monkeypatch.setenv("XIUQO_COPILOT_ACP_COMMAND", str(fake))

    with patch("agent.models_dev.fetch_models_dev", return_value={}), \
         patch("xiuqo_cli.models._resolve_copilot_catalog_api_key", return_value=None), \
         patch("xiuqo_cli.models._fetch_github_models", return_value=[]):
        providers = list_authenticated_providers(current_provider="openrouter", max_models=50)
...
def test_copilot_acp_catalog_prefers_authenticated_session(
    ...
    with patch("xiuqo_cli.auth.resolve_external_process_provider_credentials", return_value=_ACP_CREDS), \
         patch("agent.copilot_acp_client.CopilotACPClient.list_models", **session_probe) as list_models, \
         patch("xiuqo_cli.models._resolve_copilot_catalog_api_key", return_value=github_token), \
         patch("xiuqo_cli.models._fetch_github_models", return_value=github_models) as github:
...
def test_copilot_acp_session_probe_is_memoized_across_model_switch_validation(_fresh_acp_memo):
    ...
    with patch("xiuqo_cli.auth.resolve_external_process_provider_credentials", return_value=_ACP_CREDS), \
         ...
         patch("xiuqo_cli.models._resolve_copilot_catalog_api_key", return_value=""), \
         patch("xiuqo_cli.models._fetch_github_models", return_value=[]):
        for _ in range(3):
