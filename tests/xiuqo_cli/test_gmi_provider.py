...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from xiuqo_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # xiuqo model, not via upgrade prompts.
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in XIUQO_OVERLAYS
        overlay = XIUQO_OVERLAYS["gmi"]
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a XiuqoAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("XiuqoAgent/")

    def test_gmi_profile_declares_hermes_user_agent(self):
        """The GMI plugin sets a XiuqoAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("XiuqoAgent/"), (
            f"expected GMI profile User-Agent to start with 'XiuqoAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("xiuqo_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "xiuqo_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["xiuqo", "chat", "--provider", "gmi"])

        from xiuqo_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("xiuqo_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("xiuqo_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("xiuqo_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            from xiuqo_cli.model_setup_flows import _model_flow_api_key_provider
