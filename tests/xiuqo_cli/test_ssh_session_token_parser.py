...
from xiuqo_cli.main_dashboard import _read_ssh_session_token_file
...
def test_token_anchor_is_os_home_not_active_profile(tmp_path, monkeypatch):
    ...
    ``$HOME/.xiuqo/desktop-ssh`` (a literal ``~/.xiuqo/desktop-ssh`` in
    ...
    ``<root>/profiles/<name>``, and a Docker-style ``XIUQO_HOME`` can point
    elsewhere entirely — neither must move the validator off
    ``$HOME/.xiuqo/desktop-ssh``, or the token is wrongly rejected."""
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    token_dir = home / ".xiuqo" / "desktop-ssh" / ("a" * 32)
    token_dir.mkdir(parents=True, mode=0o700)
    token_path = token_dir / "0123456789abcdef.token"

    # A sticky profile and a custom (Docker) root both point get_hermes_home()
    # away from $HOME/.xiuqo; the anchor must ignore both.
    for elsewhere in (home / ".xiuqo" / "profiles" / "coder", tmp_path / "opt" / "data"):
        token_path.write_text("b" * 64)
        token_path.chmod(0o600)
...
def test_token_under_profile_desktop_ssh_is_rejected(tmp_path, monkeypatch):
    ...
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    profile_home = home / ".xiuqo" / "profiles" / "coder"
...
def test_token_file_rejects_symlink(tmp_path, monkeypatch):
    ...
    override = set_hermes_home_override(home / ".xiuqo")
    try:
...
def test_token_file_rejects_parent_escape(tmp_path, monkeypatch):
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    token_root = home / ".xiuqo" / "desktop-ssh"
...
def test_windows_runtime_root_stays_at_machine_root_for_named_profile(tmp_path, monkeypatch):
    from xiuqo_cli import windows_ssh_runtime

    machine_root = tmp_path / "custom-xiuqo-root"
    monkeypatch.setenv("XIUQO_HOME", str(machine_root / "profiles" / "writer_2"))
