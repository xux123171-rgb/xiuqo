"""Tests for `xiuqo fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate XIUQO_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".xiuqo" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from xiuqo_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from xiuqo_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("xiuqo_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("xiuqo_cli.main._require_tty"):
            from xiuqo_cli.fallback_cmd import cmd_fallback_add
    ...
    def test_restore_preserves_absent_active_provider(self):
        ...
        from xiuqo_cli import auth, fallback_cmd
    ...
    def test_picker_failure_restores_persisted_primary_without_masking_error(
        ...
        from xiuqo_cli import fallback_cmd
        ...
        with patch(
            "xiuqo_cli.main.select_provider_and_model",
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        with patch("xiuqo_cli.setup._curses_prompt_choice", return_value=1):
            from xiuqo_cli.fallback_cmd import cmd_fallback_remove
...
class TestClearCommand:
    ...
    def test_clear_with_confirmation(self, isolated_home, capsys, monkeypatch):
        ...
        from xiuqo_cli.fallback_cmd import cmd_fallback_clear
...
class TestDispatcher:
    ...
    def test_unknown_subcommand_exits(self, isolated_home):
        _write_config(isolated_home, {})
        from xiuqo_cli.fallback_cmd import cmd_fallback
...
class TestArgparseWiring:
    """Verify `xiuqo fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "xiuqo_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
