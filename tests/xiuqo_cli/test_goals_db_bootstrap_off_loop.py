...
def _patch_sessiondb(monkeypatch, cls=_RecordingDB):
    # goals.py acquires through xiuqo_state_registry (shared writer per home); patch its
    # construction seam, and keep every acquired fake out of the registry so tests don't share.
    import xiuqo_state_registry

    monkeypatch.setattr(xiuqo_state_registry, "_open_session_db", lambda path: cls(db_path=path))
