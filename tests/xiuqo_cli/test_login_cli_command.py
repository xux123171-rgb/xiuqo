...
from xiuqo_cli import cli_commands_mixin as commands
...
def test_the_in_chat_and_terminal_completion_use_their_own_copy():
    ...
    assert "run `xiuqo model` to pick one" in anon_auth.drain_sign_in_copy(iter([state]), chat=False)
...
def test_upgrade_guest_keeps_the_terminal_timeout(monkeypatch):
    ...
    monkeypatch.setattr("xiuqo_cli.auth_device_flow._is_remote_session", lambda: True)
