...
from xiuqo_cli.console_engine import HermesConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/xiuqo-console-test.zip",
    "import /tmp/xiuqo-console-test.zip",
...
def test_sessions_export_rejects_oversized_single_before_touching_output(
    ...
    monkeypatch.setattr(xiuqo_state, "resolved_max_export_messages", lambda: 2)
    materialized = []
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_hermes_home):
    ...
    assert code == 0
    assert "Xiuqo Console" in stdout.getvalue()
    assert "xiuqo>" not in stdout.getvalue()
    assert stderr.getvalue() == ""


def test_capture_output_surfaces_string_exit_code_as_command_error():
    from xiuqo_cli.console_engine import ConsoleCommandError, _capture_output
