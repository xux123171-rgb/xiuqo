...
``.xiuqo-catalog.json`` sidecar) and the install endpoint has NO kill-list bypass."""
...
from xiuqo_cli import plugin_catalog as pc_cat
...
def _install(name: str, sidecar: dict | None):
    ...
    if sidecar:
        (d / ".xiuqo-catalog.json").write_text(json.dumps(sidecar))
...
def test_install_endpoint_refuses_removed_plugins_with_no_bypass(client, monkeypatch):
    from xiuqo_cli import plugins_cmd
    ...
    for body in ({"identifier": "https://github.com/evil/bad-plugin.git"},
        ...
        assert "removed" in resp.json()["detail"] or "not in the Xiuqo plugin catalog" in resp.json()["detail"]
