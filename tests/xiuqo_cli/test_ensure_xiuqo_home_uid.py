...
When Xiuqo runs in Docker with ``XIUQO_UID=1000`` / ``XIUQO_GID=911``,
the entrypoint chowns the top-level ``XIUQO_HOME`` once at startup. But
...
class TestResolveHermesUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("XIUQO_UID", "1000")
        monkeypatch.setenv("XIUQO_GID", "911")
        from xiuqo_cli.config import _resolve_hermes_uid_gid
...
class TestChownToHermesUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("XIUQO_UID", "1000")
        monkeypatch.setenv("XIUQO_GID", "911")
        from xiuqo_cli import config as cfg
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("XIUQO_UID", raising=False)
        monkeypatch.delenv("XIUQO_GID", raising=False)
        from xiuqo_cli import config as cfg
