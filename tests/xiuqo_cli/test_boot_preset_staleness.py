...
def test_presets_stale_when_a_staged_model_has_no_section(xiuqo_home):
    from xiuqo_cli.local_runtime.bootstrap import _presets_stale

    _stage(xiuqo_home, "model-a")
    _stage(xiuqo_home, "model-b")
    _write_presets(xiuqo_home, "model-a")
    assert _presets_stale() is True


def test_presets_current_when_every_staged_model_is_covered(xiuqo_home):
    from xiuqo_cli.local_runtime.bootstrap import _presets_stale
...
def test_legacy_presets_without_model_paths_are_regenerated(xiuqo_home):
    from xiuqo_cli.local_runtime.bootstrap import _presets_stale

    _stage(xiuqo_home, "model-a")
    _write_presets(xiuqo_home, "model-a")
    ini = xiuqo_home / "runtimes/llamacpp/presets.ini"
    ini.write_text("[model-a]\nctx-size = 65536\n")
    assert _presets_stale()


def test_no_models_is_never_stale(xiuqo_home):
    from xiuqo_cli.local_runtime.bootstrap import _presets_stale
...
def test_boot_replaces_incumbent_with_stale_presets(xiuqo_home, monkeypatch):
    ...
    import xiuqo_cli.local_runtime.bootstrap as boot
    ...
    stopped = {}
    monkeypatch.setattr(
        "xiuqo_cli.local_runtime.endpoint._state_endpoint",
    ...
    monkeypatch.setattr(
        "xiuqo_cli.local_runtime.binaries.ensure_runtime_installed", fake_boot)
...
def test_refresh_bounces_an_adopted_server(xiuqo_home, monkeypatch):
...
def test_refresh_no_server_anywhere_is_a_noop(xiuqo_home, monkeypatch):
    import xiuqo_cli.local_runtime.bootstrap as boot

    monkeypatch.setattr(boot, "_SUPERVISOR", None)
    monkeypatch.setattr(
        "xiuqo_cli.local_runtime.endpoint._state_endpoint", lambda: None)
