...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            xiuqo_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(xiuqo_constants, "get_hermes_home", lambda: tmp_path)
    monkeypatch.setattr(xiuqo_state, "SessionDB", FakeSessionDB)
    ...
    assert "xiuqo sessions optimize-storage" in capsys.readouterr().out
    conn.close()
