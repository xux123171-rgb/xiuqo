...
def _isolate(tmp_path, monkeypatch):
    """Isolate XIUQO_HOME so tests don't touch real config."""
    home = tmp_path / ".xiuqo"
    home.mkdir()
...
def cli_obj(_isolate):
    ...
    with patch("cli.load_cli_config", return_value={
        ...
        "terminal": {},
         ...
         patch("xiuqo_cli.banner.build_welcome_banner"):
        from cli import HermesCLI
        obj = HermesCLI.__new__(HermesCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["xiuqo-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Xiuqo' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Xiuqo' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Xiuqo' minimum context length."""
