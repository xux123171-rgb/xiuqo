...
import xiuqo_cli.gateway as gateway


_BREAKAWAY_MARKER = "_XIUQO_GATEWAY_BREAKAWAY"
...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /xiuqo-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``XIUQO_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``XIUQO_HOME`` value, which
...
def _run_native_windows_gateway_start_diag(
    ...
    script = textwrap.dedent(
        ...
        async def start_gateway(*, replace, verbosity):
            assert "_XIUQO_GATEWAY_BREAKAWAY" not in os.environ
            return True
        ...
        diag_path = pathlib.Path(os.environ["XIUQO_HOME"]) / "logs" / "gateway-exit-diag.log"
    ...
    env.update(
        {
            ...
            "XIUQO_GATEWAY_DETACHED": "1",
            "XIUQO_GATEWAY_EXIT_DIAG": "1",
            "XIUQO_GATEWAY_MAX_STARTS": "0",
    ...
    from xiuqo_cli._subprocess_compat import windows_detach_flags_without_breakaway
...
def test_gateway_run_subprocess_preserves_daemon_exit_codes(
    ...
    script = textwrap.dedent(
        ...
        outcome = os.environ["XIUQO_TEST_GATEWAY_OUTCOME"]
    ...
    env = {
        **os.environ,
        "XIUQO_HOME": str(tmp_path),
        "XIUQO_GATEWAY_EXIT_DIAG": "0",
        "XIUQO_TEST_GATEWAY_OUTCOME": outcome,
...
def _clear_supervisor_markers(monkeypatch):
    ...
    monkeypatch.delenv("XIUQO_S6_SUPERVISED_CHILD", raising=False)
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("xiuqo_constants.is_container", lambda: True)
    monkeypatch.setattr("xiuqo_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("xiuqo_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "xiuqo-gateway.service"

    monkeypatch.setattr(gateway, "get_systemd_unit_path", lambda system=False: unit_path)
    # Synthetic unit with a non-temp home: the real generator bakes the
    # hermetic test XIUQO_HOME (a tmp dir), which the temp-home write
    # guard correctly refuses.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
            '[Service]\nEnvironment="XIUQO_HOME=/home/alice/.xiuqo"\n'
        ),
    )
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    fake_path = tmp_path / "xiuqo-gateway.service"
...
class TestWaitForGatewayExit:
    ...
    def test_kill_gateway_processes_force_uses_helper(self, monkeypatch):
        ...
        monkeypatch.setattr(
            gateway, "_capture_gateway_argv", lambda pid: ["python", "-m", "xiuqo_cli.main", "gateway", "run"]
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
    ...
    def test_kill_gateway_processes_force_refuses_recycled_pid(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda pid: None if pid == 11 else ["python", "-m", "xiuqo_cli.main", "gateway", "run"],
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
...
class TestReapUnsupervisedGatewayOrphansMacOS:
    ...
    gateway every time Xiuqo Desktop opens (``xiuqo serve`` calls
...
class TestReapUnsupervisedGatewayOrphansWindows:
    ...
    Scheduled-Task-supervised gateway every time Xiuqo Desktop opens
    (``xiuqo serve`` calls ``_reap_unsupervised_gateway_orphans`` during
...
class TestReaperCandidateIsSupervisorOwned:
    ...
    def test_windows_scheduled_task_gateway_spared_without_pidfile(self, monkeypatch):
        ...
        gateway_pid = 52615
        bootstrap_pid = 52616   # Task-launched `xiuqo gateway run` bootstrap
        ...
        bootstrap = SimpleNamespace(
            pid=bootstrap_pid, parent=lambda: services, name=lambda: "xiuqo-gateway.exe"
        )
        gw = SimpleNamespace(
            pid=gateway_pid, parent=lambda: bootstrap, name=lambda: "xiuqo-gateway.exe"
        )
        ...
        orphan = SimpleNamespace(pid=orphan_pid, parent=lambda: None, name=lambda: "xiuqo-gateway.exe")
    ...
    def test_windows_backstop_fails_open_when_bootstrap_exited(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        stranded = SimpleNamespace(pid=4242, parent=lambda: None, name=lambda: "xiuqo-gateway.exe")
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "xiuqo_cli.gateway"
...
class TestWindowsScheduledTaskSupervisorGuard:
    ...
    def test_running_task_skips_reap(self, monkeypatch):
        ...
        import xiuqo_cli.gateway_windows as gateway_windows
