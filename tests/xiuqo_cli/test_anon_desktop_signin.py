...
FastAPI routes against a fake account service (the same fake ``xiuqo auth upgrade`` is tested with).
"""
...
from tests.xiuqo_cli.test_anon_upgrade import (
...
def test_a_sign_in_cancelled_while_waiting_never_persists_the_account(portal, free_account, monkeypatch):
    ...
    import threading
    from xiuqo_cli import web_server_oauth
    guest = anon_auth.ensure_portal_identity(explicit=True)
