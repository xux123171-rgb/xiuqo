...
fires SIGTERM + app.quit() and spawns xiuqo-setup, but its Python backend
(``python.exe -m xiuqo_cli.main serve``) survives the teardown race. The
Desktop is gone — nothing will respawn that backend — yet the venv-holder
guard refused on it and the update dead-ended with "Xiuqo is still running"
...
reap (with its full child tree — the managed .xiuqo-runtime interpreter
...
_SERVE_ARGV = [
    "C:\\xiuqo\\venv\\Scripts\\python.exe",
    "-m",
    "xiuqo_cli.main",
    "serve",
    "--host",
...
def _holders(pid=200, cmdline="python.exe -m xiuqo_cli.main serve"):
...
def test_backend_with_live_parent_keeps_refusal():
    parent = _proc(50, ["Xiuqo.exe"], create_time=10.0)
...
def test_non_backend_holder_keeps_refusal():
    repl = _proc(300, ["python.exe", "-m", "xiuqo_cli.main", "chat"], ppid=999)
    fake = _fake_psutil({300: repl})
    with patch.dict(sys.modules, {"psutil": fake}):
        holders = _holders(pid=300, cmdline="python.exe -m xiuqo_cli.main chat")
...
def test_orphan_root_plus_managed_runtime_descendant_qualifies():
    ...
    # serve root and its .xiuqo-runtime interpreter child. The child's live
    # parent IS the orphan root, so the set is safe — only the root is
    # returned (taskkill /T reaps the descendant with it).
    backend = _proc(200, _SERVE_ARGV, ppid=999)
    child_argv = [
        "C:\\xiuqo\\.xiuqo-runtime\\python\\generation-1\\python.exe",
        "worker.py",
    ]
...
def test_stop_process_trees_kills_full_tree():
    ...
    with patch("gateway.status.get_process_start_time", return_value=123), patch(
        "xiuqo_cli._subprocess_compat.pid_is_hermes", return_value=True
