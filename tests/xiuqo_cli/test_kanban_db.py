"""Tests for the Kanban DB layer (xiuqo_cli.kanban_db)."""
...
from xiuqo_cli import kanban_db_workspace as kbw


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated XIUQO_HOME with an empty kanban DB."""
    home = tmp_path / ".xiuqo"
    home.mkdir()
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import xiuqo_cli.kanban_db as _kb
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    import xiuqo_cli.kanban_db as _kb
    from xiuqo_cli import kanban_db_dispatch as _kbd

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("XIUQO_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("XIUQO_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Xiuqo' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Xiuqo' DB/metadata/logs on task completion.
    """
...
# spawned with `xiuqo -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's XIUQO_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's XIUQO_HOME."""

    def _set_home(self, monkeypatch, tmp_path, xiuqo_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XIUQO_HOME", str(xiuqo_home))
        monkeypatch.delenv("XIUQO_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.xiuqo/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.xiuqo/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.xiuqo/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # XIUQO_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile XIUQO_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # XIUQO_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is XIUQO_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["XIUQO_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["XIUQO_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["XIUQO_KANBAN_TASK"] == "t_dispatch_env"
        assert env["XIUQO_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "XIUQO_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see xiuqo_state_wal.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import xiuqo_state_wal as _xiuqo_state_wal
    monkeypatch.setattr(
        _xiuqo_state_wal, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _xiuqo_state_wal._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    xiuqo_state_wal._wal_fallback_warned_paths.clear()
    ...
    with _patch("xiuqo_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="xiuqo_state"):
            conn = kbc.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    with _patch(
        "xiuqo_cli.kanban_db.sqlite3.connect",
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'xiuqo kanban recompute' would promote it.
    """
    with kbc.connect() as conn:
        # A is done.
...
def test_add_column_if_missing_is_idempotent_on_race(kanban_home):
    ...
    from xiuqo_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
#
# Workers spawned by the dispatcher must use a `xiuqo` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["xiuqo", ...]`
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m xiuqo_cli.main`.

    Pins the correct module name (NOT `xiuqo` — there is no top-level
    `xiuqo` package). Regression for #23198: the original PR shipped
    `python -m xiuqo` which fails with `No module named xiuqo` on every
    invocation.
    """
    import shutil
    import sys
    import xiuqo_cli.kanban_db as kb
    from xiuqo_cli import kanban_db_dispatch as kbd

    monkeypatch.delenv("XIUQO_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kbd._resolve_hermes_argv()
    assert argv == [sys.executable, "-m", "xiuqo_cli.main"]
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    sufficient — if `xiuqo_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m xiuqo_cli.main --version`
    ...
    assert "Xiuqo Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="xiuqo_cli.kanban_db"):
        with kbc.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from xiuqo_cli.kanban_db_connect import connect
    from xiuqo_cli.sqlite_safe_read import file_length_matches_header
