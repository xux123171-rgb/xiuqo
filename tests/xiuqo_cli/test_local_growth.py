...
def test_overrides_roundtrip_and_clear(xiuqo_home):
    from xiuqo_cli.local_runtime.growth import (
...
def test_corrupt_overrides_read_as_empty(xiuqo_home):
    from xiuqo_cli.local_runtime.growth import (
        load_window_overrides,
...
def test_growth_declines_foreign_endpoints(xiuqo_home):
    ...
    from xiuqo_cli.local_runtime.growth import maybe_grow_window
...
def test_occupancy_confirmed_skips_gate_one():
    ...
    from xiuqo_cli.local_runtime.context_policy import growth_decision
    from xiuqo_cli.local_runtime.estimator import (
        HardwareBudget,
        LayerKind,
        ModelProfile,
    )
    ...
    # Xiuqo' threshold (e.g. 80% of window) can sit BELOW the ladder's 85%
...
def _tiny_profile(model_id: str):
    from xiuqo_cli.local_runtime.estimator import LayerKind, ModelProfile
...
def test_preset_generation_for_catalog_model_with_mmproj(xiuqo_home, tmp_path, monkeypatch):
    ...
    import xiuqo_cli.local_runtime.presets as presets_mod

    from xiuqo_cli.local_runtime.catalog import CATALOG
    from xiuqo_cli.local_runtime.estimator import HardwareBudget
...
def test_preset_restores_grown_window_capped_at_native(xiuqo_home, tmp_path, monkeypatch):
    ...
    from xiuqo_cli.local_runtime.estimator import HardwareBudget
    from xiuqo_cli.local_runtime.growth import save_window_override
...
def test_preset_ignores_override_below_launch_window(xiuqo_home, tmp_path, monkeypatch):
...
def test_preset_restores_grown_window_midladder(xiuqo_home, tmp_path, monkeypatch):
    """The real growth shape: launch at a lower rung, override to a middle
    rung -> the preset window follows the override."""
    import xiuqo_cli.local_runtime.presets as presets_mod

    from xiuqo_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile
    from xiuqo_cli.local_runtime.growth import save_window_override
...
def test_mtp_plan_matches_cost_at_initial_and_restored_windows(xiuqo_home, tmp_path, monkeypatch):
    from dataclasses import replace
    from types import SimpleNamespace

    from xiuqo_cli.local_runtime import presets
    from xiuqo_cli.local_runtime.context_policy import FLOOR, RUNTIME_OVERHEAD_BYTES, ub_logits_bytes
    from xiuqo_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile, ctx_bytes
    from xiuqo_cli.local_runtime.growth import save_window_override
...
def test_growth_requires_an_admissible_materialized_preset(xiuqo_home, tmp_path, monkeypatch):
    from dataclasses import replace
    from types import SimpleNamespace

    from xiuqo_cli.local_runtime import bootstrap, catalog, growth, hardware, presets
    from xiuqo_cli.local_runtime.context_policy import FLOOR, RUNTIME_OVERHEAD_BYTES, ub_logits_bytes
    from xiuqo_cli.local_runtime.estimator import HardwareBudget, ctx_bytes
    ...
    from xiuqo_cli.local_runtime import gguf, estimator
    ...
    from xiuqo_cli.local_runtime.binaries import runtimes_root
...
def test_sampling_ladder_file_beats_catalog_beats_nothing(xiuqo_home, tmp_path, monkeypatch):
