...
from xiuqo_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    XIUQO_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def xiuqo_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("xiuqo_cli.config") or mod == "xiuqo_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, xiuqo_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, xiuqo_home, capsys, monkeypatch):
