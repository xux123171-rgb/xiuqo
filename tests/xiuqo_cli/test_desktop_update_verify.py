...
from xiuqo_cli import desktop_update_verify as verify
from xiuqo_cli.main_desktop import _write_desktop_build_stamp
...
def bundle(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(verify, '_desktop_packaged_executable', lambda _: resources.parent / 'Xiuqo.exe')
    ...
    monkeypatch.setattr(main_desktop, '_desktop_packaged_executable', lambda _: resources.parent / 'Xiuqo.exe')
...
def test_default_root_is_the_imported_checkout_not_cwd(tmp_path, monkeypatch):
    # The Windows hand-off is spawned from XIUQO_HOME; the receipt must describe the checkout anyway.
    ...
    assert (verify.checkout_root() / 'xiuqo_cli' / 'desktop_update_verify.py').is_file()
