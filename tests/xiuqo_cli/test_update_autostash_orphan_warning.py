...
``xiuqo update`` can legitimately leave an autostash behind (--keep-stash
parks it; a conflicted restore preserves it), but nothing ever mentioned those
entries again — they persisted invisibly for weeks. ``xiuqo update`` now
warns about ``xiuqo-update-autostash-*`` entries older than the threshold.
...
def _make_repo_with_autostash(tmp_path, age_days: float):
    """Real repo with one xiuqo-update-autostash entry aged ``age_days``."""
    import shutil
    ...
    name = f"xiuqo-update-autostash-{stamp}"
...
def test_unparseable_autostash_timestamp_left_alone(tmp_path, capsys):
    ...
    _git(tmp_path, "stash", "push", "-m", "xiuqo-update-autostash-notadate")
    count = update_cmd._warn_orphaned_update_autostashes(["git"], tmp_path)
    assert count == 0
