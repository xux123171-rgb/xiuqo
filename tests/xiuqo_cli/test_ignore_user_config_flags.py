"""Tests for --ignore-user-config and --ignore-rules flags on `xiuqo chat`.
...
files. In Xiuqo the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.xiuqo/config.yaml`` in ``load_cli_config()``
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("XIUQO_IGNORE_USER_CONFIG", "XIUQO_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``XIUQO_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<xiuqo_home>/config.yaml`` is
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With XIUQO_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("XIUQO_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    ...
    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting XIUQO_IGNORE_RULES=1 flips HermesCLI.ignore_rules True."""
        monkeypatch.setenv("XIUQO_IGNORE_RULES", "1")
        ...
        obj.ignore_rules = ignore_rules or os.environ.get("XIUQO_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True


class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``xiuqo_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("XIUQO_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("XIUQO_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("XIUQO_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("XIUQO_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "XIUQO_IGNORE_USER_CONFIG" not in os.environ
        assert "XIUQO_IGNORE_RULES" not in os.environ
