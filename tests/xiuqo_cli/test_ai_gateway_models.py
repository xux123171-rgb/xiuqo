...
where xiuqo's shared picker expects ``prompt`` / ``completion``; these tests
pin the translation and the curated-list filtering.
"""
import json
from unittest.mock import patch, MagicMock

from xiuqo_cli import models as models_module
from xiuqo_cli import models_pricing
from xiuqo_cli.models import VERCEL_AI_GATEWAY_MODELS, _ai_gateway_model_is_free, fetch_ai_gateway_models
from xiuqo_cli.models_pricing import fetch_ai_gateway_pricing
...
def test_fetch_ai_gateway_models_filters_against_live_catalog():
    ...
    with patch("xiuqo_cli.models._urlopen_model_catalog_request", return_value=_mock_urlopen(payload)):
...
def test_fetch_ai_gateway_models_falls_back_on_error():
    _reset_caches()
    with patch("xiuqo_cli.models._urlopen_model_catalog_request", side_effect=OSError("network")):
        result = fetch_ai_gateway_models(force_refresh=True)
...
def test_fetch_ai_gateway_models_routes_bearer_through_catalog_opener(monkeypatch):
    ...
    with patch("xiuqo_cli.models._urlopen_model_catalog_request", return_value=_mock_urlopen({"data": []})) as opener:
