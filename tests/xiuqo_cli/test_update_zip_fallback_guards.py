...
install``. A locked ``xiuqo.exe`` makes the install exit 2, the handler
...
def test_windows_dep_failure_does_not_zip_fallback(monkeypatch):
    monkeypatch.setattr(xiuqo_main, "_is_windows", lambda: True)
...
def test_zip_overlay_blocked_on_modified_file(tmp_path, monkeypatch):
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(
        update_cmd.subprocess, "run", _porcelain_run(" M xiuqo_cli/update_cmd.py\n")
    )
...
def test_update_via_zip_aborts_before_download_when_dirty(
    ...
    monkeypatch.setattr(xiuqo_main, "PROJECT_ROOT", fake_root)
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
...
def test_staging_artifact_lines_are_recognized():
    ...
    assert is_artifact("?? agent.xiuqo-update-staging/")
    assert is_artifact("?? cli.py.xiuqo-update-staging")
    assert is_artifact("?? tools.xiuqo-update-old/")
    ...
    assert not is_artifact(" M xiuqo_cli/update_cmd.py")
    assert not is_artifact("?? notes.xiuqo-update-staging.txt")
...
def test_recheck_ignores_own_staging_artifacts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
        _porcelain_run("?? agent.xiuqo-update-staging/\n?? cli.py.xiuqo-update-old\n"),
    )
    assert (
