...
interactive ``xiuqo model`` CLI picker.

The CLI picker (``xiuqo_cli.main.select_provider_and_model``) builds its
...
def test_cli_picker_hides_excluded_provider(config_home):
    ...
    from the ``xiuqo model`` provider menu."""
...
def test_cli_picker_hides_excluded_provider_by_alias(config_home):
    ...
    xiuqo_id / alias names."""
    # 'openai' is an alias-style xiuqo id; ensure excluding it hides the
    # canonical openai provider row if present. Use the canonical slug's
    # alias from _PROVIDER_ALIASES to stay robust to renames.
    from xiuqo_cli.models import _PROVIDER_ALIASES, CANONICAL_PROVIDERS
