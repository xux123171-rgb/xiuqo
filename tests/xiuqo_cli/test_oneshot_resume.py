"""Tests for `xiuqo -z --resume <session>` (#105892).
...
class TestLoadResumeTarget:
    ...
    def test_empty_session_keeps_resolved_id(self, tmp_path):
        ...
        # re-minted a session for `xiuqo -z "hello" -c <title> --create-if-missing`,
...
class TestRunAgentResumeRuntime:
    ...
    def test_run_agent_uses_stored_runtime_and_reopens_row(self, tmp_path, monkeypatch):
        import xiuqo_cli.oneshot as oneshot_mod
        ...
        monkeypatch.setattr("xiuqo_cli.config.load_config", lambda: {"model": {"default": "ambient-model", "provider": "openrouter"}})
        monkeypatch.setattr(
        ...
        monkeypatch.setattr("xiuqo_cli.tools_config._get_platform_tools", lambda _cfg, _p: [])
        monkeypatch.setattr("xiuqo_cli.mcp_startup.ensure_mcp_discovery_before_agent_build", lambda **_kw: None)
...
class TestRunOneshotForwardsResume:
    def test_resume_kwarg_reaches_run_agent(self, monkeypatch):
        ...
        monkeypatch.setattr("xiuqo_cli.oneshot._run_agent", _fake_run_agent)
