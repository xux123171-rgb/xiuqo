...
`xiuqo` launch re-read the stale base_url and routed the new model at the
old host.
...
canonical ``xiuqo_cli.model_switch.persist_model_selection`` shape shared by every
surface.
"""
...
def _run_switch(monkeypatch, result, cmd="/model MiniMax-M3 --global"):
    ...
    monkeypatch.setattr("xiuqo_cli.model_switch.switch_model", lambda **kw: result)
    monkeypatch.setattr(
...
def test_session_only_switch_does_not_touch_config(monkeypatch):
    ...
    monkeypatch.setattr("xiuqo_cli.model_switch.switch_model", lambda **kw: _make_result())
    monkeypatch.setattr(
        "xiuqo_cli.inventory.load_picker_context",
        lambda: (_ for _ in ()).throw(RuntimeError("no picker context in test")),
    )
