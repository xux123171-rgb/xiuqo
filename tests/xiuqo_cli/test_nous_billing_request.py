"""Tests for the xiuqo_cli.nous_billing HTTP client's response handling.
...
from xiuqo_cli import nous_billing as nb
...
def test_billing_token_cache_is_scoped_per_profile_home(monkeypatch, tmp_path):
    ...
    multiplex gateway, where the per-turn XIUQO_HOME override selects the auth.json."""
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: f"tok-{xiuqo_constants.get_hermes_home().name}")
    a, b = tmp_path / "a", tmp_path / "b"
    a.mkdir(); b.mkdir()
    seen = []
    for home in (a, b):
        tok = xiuqo_constants.set_hermes_home_override(str(home))
        try:
