...
`xiuqo --tui --resume latest --in ./dir` (and the classic-CLI equivalents)
...
def test_top_level_in_value_not_mistaken_for_subcommand(monkeypatch):
    # `xiuqo --in chat` — "chat" is the flag's value, not the subcommand.
    import sys

    import xiuqo_cli.main as mod

    monkeypatch.setattr(sys, "argv", ["xiuqo", "--in", "chat", "--resume", "latest"])
...
def test_in_dir_replaces_inherited_terminal_cwd(main_mod, monkeypatch, tmp_path):
    """A parent Xiuqo surface, the shell or .env can export TERMINAL_CWD before
