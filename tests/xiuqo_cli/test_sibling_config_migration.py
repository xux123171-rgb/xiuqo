...
`xiuqo update` migrated only the active profile's config.yaml; sibling
...
context-local XIUQO_HOME override.
...
import xiuqo_cli.update_cmd_config as update_cmd_config
...
def _setup(monkeypatch, tmp_path, active_home: Path):
    ...
    monkeypatch.setattr(
        xiuqo_constants, "get_process_hermes_home", lambda: active_home
    )
    monkeypatch.setattr(
