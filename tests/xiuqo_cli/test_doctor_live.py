"""Tests for ``xiuqo doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from xiuqo_cli import doctor_live
from xiuqo_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `xiuqo doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("xiuqo_cli.doctor.XIUQO_HOME", tmp_path / "home")
        monkeypatch.setattr("xiuqo_cli.doctor.PROJECT_ROOT", tmp_path / "root")
