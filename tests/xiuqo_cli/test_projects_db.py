"""Tests for the per-profile Projects store (xiuqo_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Xiuqo Agent", folders=["/tmp/xiuqo"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "xiuqo"
    assert proj.name == "Xiuqo Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/xiuqo"
    assert [f.path for f in proj.folders] == ["/tmp/xiuqo"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "xiuqo").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' XIUQO_HOME.
