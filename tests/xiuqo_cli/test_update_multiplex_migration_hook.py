"""``xiuqo update`` runs the multiplex auto-migration hook only after a verified-healthy fleet restart.
...
from xiuqo_cli import gateway_migrate

from tests.xiuqo_cli.test_update_fleet_restart_pending import (
...
def test_successful_update_runs_multiplex_hook_once(monkeypatch, tmp_path, hook_calls):
    ...
    xiuqo_main.cmd_update(_update_args())
...
def test_incomplete_fleet_verification_skips_multiplex_hook(monkeypatch, tmp_path, hook_calls):
    ...
    _patch_update_deps(monkeypatch, tmp_path, _make_head_moved_side_effect())
    monkeypatch.setattr("xiuqo_cli.update_receipt.print_fleet_version_matrix", lambda rows: True)
