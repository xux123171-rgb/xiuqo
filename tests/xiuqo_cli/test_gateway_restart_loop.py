...
Covers:
- Defense 1: gateway stop/restart refuse when _XIUQO_GATEWAY=1
...
from xiuqo_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "xiuqo gateway restart",
        "xiuqo gateway stop",
        "xiuqo gateway uninstall",
        "xiuqo  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "XIUQO GATEWAY RESTART",           # uppercase
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.xiuqo.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.xiuqo/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l xiuqo-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.xiuqo.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.xiuqo.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.xiuqo.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.xiuqo/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.xiuqo.gateway-apollo:/a.plist' "
            "'ai.xiuqo.gateway-cronus:/c.plist' 'ai.xiuqo.gateway-plutus:/p.plist' "
            "'ai.xiuqo.gateway:/Users/botuser/Library/LaunchAgents/ai.xiuqo.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated xiuqo note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.xiuqo.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.xiuqo.gateway',
        "launchctl kick'start' -k gui/501/ai.xiuqo.gateway",
        "launchctl kick\\start -k gui/501/ai.xiuqo.gateway",
        'launchctl "kickstart" -k gui/501/ai.xiuqo.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.xiuqo.gateway',
        "launchctl dis\\able gui/501/ai.xiuqo.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.xiuqo."gateway"',
        # Same class on the systemctl and xiuqo-CLI branches.
        'systemctl re"start" xiuqo-gateway',
        'xiuqo gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.xiuqo.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.xiuqo.update-checker',
        'systemctl re"start" xiuqo-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "xiuqo cron list",
        "xiuqo update",
        "xiuqo config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `xiuqo gateway start` is benign — starting a gateway from inside a
        ...
        "xiuqo gateway start",
        "xiuqo gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway xiuqo
        # services must not be falsely blocked (the old `.*xiuqo` matched any
        # xiuqo token).
        "launchctl unload ai.xiuqo.update-checker.plist",
        "launchctl restart ai.xiuqo.daemon",
        ...
        "systemctl restart xiuqo-meta.service",
        "systemctl restart xiuqo-cron-helper",
        ...
        "echo after the xiuqo gateway restarted cleanly",
        "the xiuqo gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "xiuqo skill view gateway-notes && echo xiuqo gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `xiuqo` is a path component
        # there, not a command.
        "cat '/docs/xiuqo gateway restart-notes.md'",
        "less /home/user/notes/xiuqo gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "xiuqo gateway restart",
        "xiuqo gateway restart; echo done",
        "xiuqo gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;xiuqo gateway restart",
        "true && xiuqo gateway stop",
        "echo $(xiuqo gateway restart)",
        "echo `xiuqo gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.xiuqo.gateway"])',
        'subprocess.run(["xiuqo", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.xiuqo.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["xiuqo", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: xiuqo gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `xiuqo -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry XIUQO_HOME/XIUQO_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("XIUQO_PROFILE", "zeus")
        monkeypatch.delenv("XIUQO_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "xiuqo -p zeus gateway stop",
        "xiuqo -p zeus gateway restart",
        "xiuqo --profile zeus gateway restart",
        "xiuqo --profile zeus gateway stop",
        "xiuqo --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "xiuqo -v -p zeus gateway restart",
        "xiuqo -p zeus -v gateway restart",
        "xiuqo --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "xiuqo -p venus gateway stop",
        "xiuqo -p venus gateway restart",
        "xiuqo --profile venus gateway restart",
        "xiuqo --profile=venus gateway stop",
        "xiuqo -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "xiuqo -p zeus gateway start",
        "xiuqo -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("xiuqo gateway restart")
        assert _contains_gateway_lifecycle_command("xiuqo gateway stop")

    def test_hermes_home_derived_profile(self, monkeypatch):
        # Without XIUQO_PROFILE the guard falls back to the XIUQO_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("XIUQO_PROFILE", raising=False)
        monkeypatch.delenv("XIUQO_PROFILE_NAME", raising=False)
        import xiuqo_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("xiuqo -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("xiuqo -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("xiuqo -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("xiuqo gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_hermes_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade xiuqo then run xiuqo gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # XIUQO_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("XIUQO_HOME", str(tmp_path / ".xiuqo"))
        scripts_dir = tmp_path / ".xiuqo" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_XIUQO_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _XIUQO_GATEWAY=1.

    Issue #37453: systemctl --user restart xiuqo-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart xiuqo-gateway",
        "systemctl --user restart xiuqo-gateway",
        "systemctl stop xiuqo-gateway.service",
        "xiuqo gateway restart",
        "xiuqo gateway uninstall",
        "launchctl kickstart gui/501/ai.xiuqo.gateway",
        "launchctl bootout gui/501/ai.xiuqo.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.xiuqo.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.xiuqo/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.xiuqo.gateway.restart-once.plist",
        "pkill -f xiuqo.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart xiuqo-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.xiuqo.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-xiuqo label: label-independent detection is the point
        # (#62891 second reproduction used `ai.xiuqo.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.xiuqo.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _XIUQO_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _XIUQO_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_XIUQO_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="xiuqo gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["xiuqo gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.xiuqo.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.xiuqo.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-xiuqo systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run xiuqo gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("xiuqo gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under XIUQO_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("xiuqo gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/xiuqo\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 xiuqo gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        ...
        RuntimeError when neither XIUQO_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart xiuqo-gateway' .",
        "grep -rn 'xiuqo gateway restart' ./logs",
        "rg 'xiuqo gateway restart' ../archive",
        "grep -c 'systemctl stop xiuqo-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM xiuqo_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell xiuqo gateway restart"',
        'sqlite3 ./db ".system systemctl restart xiuqo-gateway"',
        'psql ./x -c "\\! systemctl restart xiuqo-gateway"',
        "grep -r 'xiuqo gateway restart' . | sh",
        "grep -r 'xiuqo gateway restart' ./logs | bash",
        "grep -r 'xiuqo gateway restart' . | sudo sh",
        "grep -r 'x' . ; xiuqo gateway restart",
        "grep -r 'x' . && systemctl restart xiuqo-gateway",
        'grep -r "$(xiuqo gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart xiuqo-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell xiuqo gateway restart"',
        'sqlite3 db ".system systemctl restart xiuqo-gateway"',
        'psql -c "\\! systemctl restart xiuqo-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    xiuqo_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run xiuqo gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XIUQO_HOME", str(tmp_path / ".xiuqo"))
        (tmp_path / ".xiuqo").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run xiuqo gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop xiuqo-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart xiuqo-gateway' /var/log/syslog",
        "rg 'xiuqo gateway restart' /home/user/.xiuqo/logs/",
        "journalctl -u xiuqo-gateway --grep 'systemctl restart xiuqo-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'xiuqo_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell xiuqo gateway restart"',
        'psql -c "\\! systemctl restart xiuqo-gateway"',
        "grep 'systemctl restart xiuqo-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart xiuqo-gateway",
        'grep "$(systemctl restart xiuqo-gateway)" f',
        "grep 'restart' log; systemctl restart xiuqo-gateway",
        'sqlite3 db "SELECT 1"; xiuqo gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "xiuqo gateway restart",
        "sudo systemctl stop xiuqo-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart xiuqo-gateway%'\""
        )
