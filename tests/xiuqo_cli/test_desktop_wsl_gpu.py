"""``xiuqo gui`` under WSLg selects the installed Mesa D3D12 driver before Electron spawns
...
def _launch_env(monkeypatch, tmp_path, *, wsl: bool, dxg: bool, driver: bool) -> dict:
    ...
    monkeypatch.setattr(xiuqo_constants, "_wsl_detected", wsl)
...
def test_wslg_with_dxg_and_installed_d3d12_driver_selects_gpu_mesa_backend(monkeypatch, tmp_path):
    ...
    assert Path(env["XIUQO_DESKTOP_CWD"]) == tmp_path.resolve()  # the rest of the env still builds
