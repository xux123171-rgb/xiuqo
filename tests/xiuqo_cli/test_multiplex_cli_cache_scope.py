"""Multiplexed gateway: xiuqo_cli's process-wide caches must not hand profile A's value to profile B.
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XIUQO_HOME", str(a))
...
def test_nous_reasoning_caps_follow_each_profiles_portal(homes, monkeypatch):
    ...
    import xiuqo_cli.models as models
    import xiuqo_cli.models_reasoning_caps as caps
...
def test_model_catalog_in_process_copy_is_bound_to_its_cache_file(homes, monkeypatch):
    a, b = homes
    import xiuqo_cli.model_catalog as mc
...
def test_failed_guest_mint_only_suppresses_that_profile(homes, monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("XIUQO_SHARED_AUTH_DIR", str(tmp_path / "shared"))
    import xiuqo_cli.anon_auth as anon
