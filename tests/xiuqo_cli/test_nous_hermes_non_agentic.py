...
``"xiuqo"`` anywhere (case-insensitive). That false-positived on unrelated
local Modelfiles such as ``xiuqo-brain:qwen3-14b-ctx16k`` — a tool-capable
Qwen3 wrapper that happens to live under the "xiuqo" tag namespace.
...
def test_matches_real_nous_hermes_chat_models(model_name: str) -> None:
    ...
    assert _check_hermes_model_warning(model_name) == _XIUQO_MODEL_WARNING
