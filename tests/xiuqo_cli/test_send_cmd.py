"""Tests for the ``xiuqo send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``xiuqo_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from xiuqo_cli import send_cmd
...
def test_load_hermes_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``xiuqo send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``xiuqo config set``) lives
    ...
    xiuqo_home.mkdir()
    (xiuqo_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    import xiuqo_cli.config as _hc_config
    reload(_hc_config)
...
def test_load_hermes_env_utf8_bom_preserves_first_key(tmp_path, monkeypatch):
    ...
    `xiuqo send` failed to authenticate.
    """
    import os
...
def test_load_hermes_env_bomless_utf8_still_loads(tmp_path, monkeypatch):
    ...
    xiuqo_home = tmp_path / ".xiuqo"
    xiuqo_home.mkdir()
    (xiuqo_home / ".env").write_bytes(b"SEND_PLAIN_TOKEN=plain-val\n")
...
def test_load_hermes_env_latin1_fallback_overrides_shell(tmp_path, monkeypatch):
    ...
    (xiuqo_home / ".env").write_bytes(b"SEND_OVR_LABEL=caf\xe9-file\n")
...
def test_load_hermes_env_fallback_read_error_is_swallowed(tmp_path, monkeypatch):
    ...
    (xiuqo_home / ".env").write_bytes(b"SEND_ERR_TOKEN=caf\xe9\n")
...
def test_load_hermes_env_bom_only_env_is_noop(tmp_path, monkeypatch):
    ...
    xiuqo_home = tmp_path / ".xiuqo"
    xiuqo_home.mkdir()
    (xiuqo_home / ".env").write_bytes(b"\xef\xbb\xbf")
