...
Driven directly against the same fake account service ``xiuqo auth upgrade`` is tested with, so the
...
from xiuqo_cli.auth_constants import AuthError
from tests.xiuqo_cli.test_anon_upgrade import (  # noqa: F401  (fixtures used by name)
...
def test_a_timeout_yields_timed_out_and_keeps_the_enriched_detail(portal, monkeypatch):
    ...
    from xiuqo_cli import auth_device_flow
...
def test_already_signed_in_short_circuits_before_any_network(portal):
    from xiuqo_cli.auth import _load_auth_store, _save_auth_store, _save_provider_state
