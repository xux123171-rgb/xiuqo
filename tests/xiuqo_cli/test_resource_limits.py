...
from xiuqo_cli import resource_limits
...
def test_fresh_process_import_without_posix_resource_is_a_safe_noop():
    code = textwrap.dedent(
        ...
        spec = importlib.util.spec_from_file_location(
            "xiuqo_cli._resource_limits_without_posix_resource",
            module_path,
        )
...
def test_serve_startup_applies_limit_before_web_server(monkeypatch):
    ...
    import xiuqo_cli.plugins
    import xiuqo_cli.web_server

    # cmd_dashboard(headless_backend=True) exports XIUQO_SERVE_HEADLESS=1 into
    ...
    # 404 path.
    monkeypatch.setenv("XIUQO_SERVE_HEADLESS", "0")
    ...
    monkeypatch.setattr(xiuqo_cli.plugins, "discover_plugins", lambda: None)
    monkeypatch.setattr(
        xiuqo_cli.web_server,
        "start_server",
...
def test_named_profile_reroute_defers_limit_to_final_process(monkeypatch, tmp_path):
    ...
    assert calls == []
    assert exec_call["argv"][1:5] == ["-m", "xiuqo_cli.main", "-p", "default"]
    assert exec_call["env"]["XIUQO_HOME"] == str(tmp_path)
...
def test_dashboard_lifecycle_flags_skip_limit_adjustment(monkeypatch, lifecycle_flag):
    ...
    from xiuqo_cli import main as cli_main
    import xiuqo_cli.main_dashboard as xiuqo_cli_main_dashboard
    ...
    monkeypatch.setattr(xiuqo_cli_main_dashboard, "_find_stale_dashboard_pids", lambda: [])
