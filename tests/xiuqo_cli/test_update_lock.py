"""Cross-process update mutual exclusion (``xiuqo_cli.update_lock``).

Three surfaces can start an update of one install tree: a terminal ``xiuqo
...
dashboard-spawned ``xiuqo update`` was mid-``npm install``, which then failed
...
@pytest.fixture
def marker(tmp_path):
    return tmp_path / ".xiuqo-update-in-progress"
...
def test_marker_path_follows_process_hermes_home(tmp_path, monkeypatch):
    ...
    All three resolve the *process* XIUQO_HOME; a profile-scoped path would
    put the lock somewhere the other two owners never read.
    """
    monkeypatch.setenv("XIUQO_HOME", str(tmp_path))
    assert update_marker_path() == tmp_path / ".xiuqo-update-in-progress"
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``xiuqo update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "Xiuqo is still running" and retrying
...
class TestAncestryHandoff:
    ...
    ``xiuqo-setup`` under ``~/.xiuqo`` is only refreshed by a full installer
    ...
    def _liveness_pinned_true(self, monkeypatch):
        monkeypatch.setattr("xiuqo_cli.update_lock._pid_alive", lambda pid: True)
