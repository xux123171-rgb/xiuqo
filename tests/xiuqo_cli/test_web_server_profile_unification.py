...
profile switcher can target any profile's XIUQO_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via XIUQO_HOME.
"""
import json
...
import xiuqo_cli.web_server_profiles as _web_server_profiles
...
class TestProfileScopedMcp:
    ...
    def test_mcp_test_oauth_server_without_token_is_not_ok(
        ...
        import xiuqo_cli.mcp_config as mcp_config
...
class TestProfileScopedModel:
    ...
    def _accept_any_model(self, monkeypatch):
        ...
        monkeypatch.setattr("xiuqo_cli.model_switch.switch_model", _switch)
    ...
    def test_model_options_uses_config_only_scope_for_selected_profile(
        ...
        monkeypatch.setattr(
            "xiuqo_cli.inventory.load_picker_context", lambda: object()
        )
        monkeypatch.setattr(
            "xiuqo_cli.inventory.build_model_options_payload",
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same XIUQO_HOME the rest
        ...
        monkeypatch.setattr(
            "xiuqo_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        assert resp.json()["xiuqo_home"] == str(isolated_profiles["worker_beta"])
