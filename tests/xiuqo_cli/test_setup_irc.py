"""Tests for IRC gateway configuration via `xiuqo setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Xiuqo install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Xiuqo install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#xiuqo")
            monkeypatch.setenv("IRC_NICKNAME", "xiuqo-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `xiuqo setup gateway` experience with IRC present."""
