...
def test_main_applies_preloaded_skills_to_system_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda skills, task_id=None: ("skill prompt", ["xiuqo-dev", "github-auth"], []),
    )

    with pytest.raises(SystemExit):
        cli_mod.main(skills="xiuqo-dev,github-auth", list_tools=True)
    ...
    assert cli_obj.preloaded_skills == ["xiuqo-dev", "github-auth"]
...
def test_show_banner_does_not_print_skills():
    ...
    cli_obj.preloaded_skills = ["xiuqo-dev", "github-auth"]
    cli_obj.console = MagicMock()

    with patch("xiuqo_cli.banner.build_welcome_banner") as mock_banner, patch(
