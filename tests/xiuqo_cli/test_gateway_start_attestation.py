...
def _install_pid_sequence(monkeypatch, snapshots):
    ...
    monkeypatch.setattr("xiuqo_cli.gateway.find_gateway_pids", _fake)
    return calls
...
def test_report_gateway_start_failure_is_loud_not_checkmark(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        "xiuqo_cli.config.get_hermes_home", lambda: str(tmp_path)
    )
...
def attest_home(monkeypatch, tmp_path):
    monkeypatch.setattr("xiuqo_cli.config.get_hermes_home", lambda: str(tmp_path))
