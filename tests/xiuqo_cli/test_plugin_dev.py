...
def test_doctor_restores_global_tool_policy_and_module_state(tmp_path: Path) -> None:
    ...
    before_modules = {
        ...
        if name == "xiuqo_plugins" or name.startswith("xiuqo_plugins.")
    }
...
def test_resolve_rejects_directory_without_manifest(tmp_path: Path) -> None:
    ...
    from xiuqo_cli.plugin_dev import resolve_plugin_path
...
def test_doctor_default_target_does_not_copy_cwd(
    ...
    """``xiuqo plugins doctor`` with no argument defaults to ``.``.

    Before the manifest guard, that copied the whole working directory into
    a temporary XIUQO_HOME — running it from ``$HOME`` copied the home
    directory, cloud-storage placeholders included.
    """
    import os
    import shutil

    from xiuqo_cli import plugin_dev
...
def test_resolve_prefers_installed_id_over_unrelated_local_dir(
    ...
    installed = xiuqo_home / "plugins" / "sample"
    ...
    monkeypatch.chdir(workdir)
    monkeypatch.setattr(plugin_dev, "get_hermes_home", lambda: xiuqo_home)
...
def test_doctor_removes_temp_home_when_staging_copy_fails(
    ...
    """A copy failure (e.g. ENOSPC) must not strand a xiuqo-plugin-doctor-* dir."""
    import errno
    import shutil
    import tempfile
    ...
    # exact window where a stranded xiuqo-plugin-doctor-* dir was observed.
    leftovers = list(scratch.glob("xiuqo-plugin-doctor-*"))
