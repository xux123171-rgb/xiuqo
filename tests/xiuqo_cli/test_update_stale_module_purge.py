...
sys.modules breaking the gateway auto-restart after `xiuqo update`.

Field failure (2026-08-20, Teknium's Linux box): `xiuqo update` pulled a
checkout where xiuqo_cli/gateway.py newly imports `line_input` from
xiuqo_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from xiuqo_cli.gateway
...
the Xiuqo package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Xiuqo modules from the cache; later
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_hermes_modules()
    assert sys.modules.get("xiuqo_cli.update_cmd") is update_cmd
    assert sys.modules.get("xiuqo_cli.main") is cli_main
    assert "xiuqo_cli" in sys.modules
...
def test_purge_preserves_active_update_receipt(tmp_path, monkeypatch):
    ...
    import xiuqo_cli.update_receipt as receipt
    ...
    try:
        ...
        cli_main._purge_stale_hermes_modules()
        post_purge_receipt = importlib.import_module("xiuqo_cli.update_receipt")
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["xiuqo_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_hermes_modules()
    finally:
        sys.modules.pop("xiuqo_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from xiuqo_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
...
def test_purge_keeps_plan_record_class_identity():
    ...
    from xiuqo_cli.update_inventory import RuntimeRecord as before

    cli_main._purge_stale_hermes_modules()
    from xiuqo_cli.update_inventory import RuntimeRecord as after
