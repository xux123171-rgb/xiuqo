...
xiuqo_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # xiuqo_time checks an env override first; ensure it's unset so config wins.
    ...
    assert xiuqo_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (XIUQO_TIMEZONE, XIUQO_REDACT_SECRETS, XIUQO_MAX_ITERATIONS,
    ...
    # writing XIUQO_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
