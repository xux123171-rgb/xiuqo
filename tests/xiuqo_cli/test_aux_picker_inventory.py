...
Every aux picker (``xiuqo model`` → Configure auxiliary models, the
``xiuqo tools`` vision picker, and any future one) must route through
``xiuqo_cli.inventory.build_aux_picker_rows()`` so it shows the same
...
import pytest
import yaml
from xiuqo_cli import main_provider_setup
...
def configured_home(tmp_path, monkeypatch):
    """A XIUQO_HOME with one ``providers:`` entry and one legacy
...
def test_aux_picker_surfaces_user_defined_providers(configured_home):
    ...
    """
    from xiuqo_cli.inventory import build_aux_picker_rows
...
def test_aux_picker_requests_exhausted_pool_visibility(configured_home):
    ...
    with patch("xiuqo_cli.model_switch.list_authenticated_providers", _capture):
...
def test_aux_pickers_route_through_the_shared_substrate(configured_home):
    ...
    with (
        patch("xiuqo_cli.model_switch.list_authenticated_providers", _direct),
        patch("xiuqo_cli.inventory.build_aux_picker_rows", _substrate),
        patch("xiuqo_cli.main_provider_setup._prompt_provider_choice", return_value=None),
    ):
    ...
    assert direct_calls == [], (
        ...
        "route through xiuqo_cli.inventory.build_aux_picker_rows so custom "
