...
from xiuqo_cli import update_cmd_fleet as fleet
...
def test_unit_transaction_budget_preserves_scope_and_health(monkeypatch, graceful, retry):
    ...
    def systemctl(cmd, *, timeout):
        ...
        if "list-units" in cmd:
            return subprocess.CompletedProcess(cmd, 0, "xiuqo-serve-test.service loaded active running", "")
        if "show" in cmd:
    ...
    name = "xiuqo-gateway-test" if graceful else "xiuqo-serve-test"
    restarted, failed = [], []
...
def test_budget_fallback_keeps_real_errors(monkeypatch, limits, outcome):
    ...
    if outcome == "timeout":
        with pytest.raises(subprocess.TimeoutExpired):
            fleet._systemctl_reset_and_restart(["systemctl"], "xiuqo-serve-test")
    else:
        result = fleet._systemctl_reset_and_restart(["systemctl"], "xiuqo-serve-test")
