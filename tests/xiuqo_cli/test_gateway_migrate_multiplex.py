"""``xiuqo gateway migrate``: preflight verdicts, apply/rollback bookkeeping, and the update hook.

Service layer is faked through the module's ``_installed_service`` / ``_service_op`` seams (the same
shape ``xiuqo gateway install`` tests use); the default gateway boot is faked by writing the
...
from xiuqo_cli import gateway_migrate as gm
...
def fleet(tmp_path, monkeypatch):
    ...
    def _service_op(kind, system, verb, home):
        ...
        if verb == "start" and name != "default":
            # What the real `xiuqo -p <name> gateway run` checks first: is a live multiplexer
        ...
        if verb == "uninstall":
            ...
            runtime.update({"pid": os.getpid(), "xiuqo_home": str(root), "gateway_state": "running"})
...
def _name(home: Path) -> str:
    return xiuqo_constants.profile_name_for_home(home) or "default"
...
def test_explicit_migrate_with_no_standalone_secondaries_still_flips_flag_and_restarts_default(fleet, capsys, monkeypatch):
    ...
    def _detached(home):  # no service manager anywhere -> detached start writes the served record
        ...
        (fleet.root / "gateway.pid").write_text(json.dumps({"pid": os.getpid(), "xiuqo_home": str(fleet.root)}))
        (fleet.root / "gateway_state.json").write_text(json.dumps({
            "pid": os.getpid(), "xiuqo_home": str(fleet.root), "gateway_state": "running",
