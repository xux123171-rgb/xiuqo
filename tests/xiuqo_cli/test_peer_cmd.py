"""Tests for ``xiuqo peer`` — cross-machine bot-to-bot DMs."""
...
from xiuqo_cli.subcommands import peer as peer_cmd
...
class _FakePeer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/api/sessions":
            ...
            return self._json({"object": "xiuqo.session", "session": {"id": "bc_1", "title": body.get("title")}}, 201)
...
# ── per-turn author (XIUQO_TURN_AUTHOR set by the message_agent runner) ─────
...
class _AttackerOrigin(BaseHTTPRequestHandler):
    ...
    a compromised/MITM'd peer could redirect a ``xiuqo peer dm`` request to."""
...
class _RedirectingPeer(BaseHTTPRequestHandler):
    ...
    the shape of a compromised peer or a LAN MITM answering ``xiuqo peer
