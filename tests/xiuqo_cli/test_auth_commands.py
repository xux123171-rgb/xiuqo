...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (xiuqo_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from xiuqo_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "xiuqo" / "auth.json").read_text())
...
def test_auth_add_configured_provider_uses_canonical_pool_key(tmp_path, monkeypatch):
    ...
    payload = json.loads((xiuqo_home / "auth.json").read_text(encoding="utf-8"))
...
def test_auth_add_migrates_display_name_derived_legacy_pool_key(
    ...
    with patch("xiuqo_cli.models.clear_provider_models_cache") as clear_cache:
        auth_add_command(_Args())
...
def test_auth_list_includes_non_registry_configured_provider(
    ...
    from xiuqo_cli.auth_commands import auth_list_command
...
def test_interactive_auth_add_accepts_non_registry_configured_provider(
    ...
    from xiuqo_cli import auth_commands
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "xiuqo_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `xiuqo auth add nous` must also populate providers.nous so the
    ...
    # raises "Xiuqo is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`xiuqo auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``xiuqo auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``xiuqo auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("xiuqo_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from xiuqo_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from xiuqo_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """xiuqo auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``xiuqo auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "xiuqo" / "auth.json").read_text(encoding="utf-8"))
...
def test_clear_provider_auth_removes_provider_pool_entries(tmp_path, monkeypatch):
    ...
    from xiuqo_cli.auth import clear_provider_auth
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`xiuqo logout --provider openai-codex` must still clear model.provider.
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from xiuqo_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source Xiuqo reads from has a
...
def test_seed_from_singletons_respects_hermes_pkce_suppression(tmp_path, monkeypatch):
    """anthropic xiuqo_pkce must not re-seed from ~/.xiuqo/.anthropic_oauth.json when suppressed."""
    xiuqo_home = tmp_path / "xiuqo"
    xiuqo_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("XIUQO_HOME", str(xiuqo_home))

    import yaml
    (xiuqo_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (xiuqo_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["xiuqo_pkce"]},
    }))

    # Stub the readers so only xiuqo_pkce is "available"; claude_code returns None
    ...
    # xiuqo_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "xiuqo_pkce" not in active
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        "xiuqo_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "xiuqo_cli.copilot_auth.get_copilot_api_token",
...
def test_qwen_oauth_login_marks_active_through_moved_owner(monkeypatch):
    ...
    import xiuqo_cli.auth_commands as auth_commands
    import xiuqo_cli.auth_qwen as auth_qwen
...
def test_auth_add_openrouter_oauth_persists_pkce_key_without_touching_api_key_default(tmp_path, monkeypatch):
    """`xiuqo auth add openrouter --type oauth` stores the PKCE-minted key as an ``api_key`` pool row
    ...
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    monkeypatch.setattr("xiuqo_cli.auth._openrouter_pkce_login", lambda **kw: {"api_key": "sk-or-v1-from-pkce"})

    from xiuqo_cli.auth import resolve_provider
    from xiuqo_cli.auth_commands import auth_add_command
...
def test_openrouter_loopback_callback_binds_nonce_path_and_rejects_forged_redirect(monkeypatch):
    ...
    import xiuqo_cli.auth_openrouter as orm
