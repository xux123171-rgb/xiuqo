...
def _stale_plist(tmp_path, monkeypatch, *, registered: bool):
    plist_path = tmp_path / "com.xiuqo.plist"
    ...
    monkeypatch.setattr(gw, "get_launchd_label", lambda: "com.xiuqo.agent")
...
def test_install_repair_warns_instead_of_claiming_success(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("xiuqo_constants.display_hermes_home", lambda: "~/.xiuqo-work")
    ...
    assert "~/.xiuqo-work/logs/launchd-reload.log" in out
