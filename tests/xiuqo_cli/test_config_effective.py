"""Invariants for ``xiuqo_cli.config_effective.load_user_config_effective`` — the one loader every
defaults-free config reader (gateway runtime, TUI gateway, cron, ``xiuqo send`` bridge, doctor,
...
def _reset_caches():
    ...
    from xiuqo_cli import config_effective, managed_scope
...
def _reset_caches_keep_last_good():
    import xiuqo_cli.config as cfg
    from xiuqo_cli import config_effective
