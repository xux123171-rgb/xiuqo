...
- Bare `xiuqo setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `xiuqo setup --quick` runs the narrower "fill in missing items" flow.
- `xiuqo setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("xiuqo_cli.setup.ensure_hermes_home", {}),
        ("xiuqo_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("xiuqo_cli.config.is_managed", {"return_value": False}),
        ("xiuqo_cli.setup.load_config", {"return_value": {}}),
        ("xiuqo_cli.setup.save_config", {}),
        ("xiuqo_cli.setup.get_env_value", {"return_value": None}),
        ("xiuqo_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("xiuqo_cli.setup._print_setup_summary", {}),
        ("xiuqo_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("xiuqo_cli.setup.load_config", {"return_value": {}}),
        ("xiuqo_cli.setup.save_config", {}),
        ("xiuqo_cli.auth.get_active_provider", {"return_value": None}),
        ("xiuqo_cli.setup.get_env_value", {"return_value": None}),
        ("xiuqo_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `xiuqo setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="xiuqo_cli.setup.prompt_choice",
                quick="xiuqo_cli.setup._run_quick_setup",
                model="xiuqo_cli.setup.setup_model_provider",
                terminal="xiuqo_cli.setup.setup_terminal_backend",
                agent="xiuqo_cli.setup.setup_agent_settings",
                gateway="xiuqo_cli.setup.setup_gateway",
                tools="xiuqo_cli.setup.setup_tools",
            )
            from xiuqo_cli.setup import run_setup_wizard
...
class TestQuickFlag:
    ...
    def test_quick_flag_runs_quick_setup_only(self, existing_install):
        ...
        with ExitStack() as stack:
            ...
            from xiuqo_cli.setup import run_setup_wizard
            from xiuqo_cli import setup as setup_mod
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("xiuqo_cli.setup.prompt_choice", {"return_value": 0}),
                first="xiuqo_cli.setup_quick._run_first_time_quick_setup",
            )
            from xiuqo_cli.setup import run_setup_wizard
            from xiuqo_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("xiuqo_cli.setup.prompt_choice", {"return_value": 2}),
                blank="xiuqo_cli.setup_quick._run_blank_slate_setup",
            )
            from xiuqo_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "xiuqo_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["xiuqo", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
