"""Tests for kanban board export / import (``xiuqo_cli.kanban_transfer``).
...
from xiuqo_cli import kanban_transfer as kt
from xiuqo_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("XIUQO_KANBAN_HOME", str(root))
        for var in ("XIUQO_KANBAN_DB", "XIUQO_KANBAN_WORKSPACES_ROOT",
                    "XIUQO_KANBAN_ATTACHMENTS_ROOT", "XIUQO_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Xiuqo"):
