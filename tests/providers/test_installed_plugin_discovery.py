"""A provider installed by ``xiuqo plugins install`` must actually be found.

The installer clones into ``$XIUQO_HOME/plugins/<name>/`` (flat), provider
...
def test_flat_installed_model_provider_plugins_are_discovered_alongside_nested_ones(xiuqo_home):
    _write_plugin(xiuqo_home / "plugins" / "installed-acp", name="installed-acp",
                  manifest='name: installed-acp\nkind: "model-provider"\n')
    _write_plugin(xiuqo_home / "plugins" / "model-providers" / "nested-acp", name="nested-acp",
...
def test_other_plugins_in_the_flat_directory_are_left_to_the_plugin_manager(xiuqo_home):
    _write_plugin(xiuqo_home / "plugins" / "other-standalone", name="other-standalone",
                  manifest="name: other-standalone\nkind: standalone\n")
    _write_plugin(xiuqo_home / "plugins" / "manifestless", name="manifestless", manifest=None)
    _write_plugin(xiuqo_home / "plugins" / "broken-manifest", name="broken-manifest",
