...
const SIDEBAR_COLLAPSED_KEY = "xiuqo-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="xiuqo-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="xiuqo-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'xiuqo update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "xiuqo update";
    return (
      t.status.updateHermesConfirmMessage ??
      `This will run 'xiuqo update' (${cmd}) and restart the gateway when it finishes.`
    );
      ...
      description={
        sharedGateway
          ...
          : (t.status.restartGatewayConfirmMessage ??
            "This restarts the Xiuqo gateway process. Connected channels and active sessions will reconnect afterward.")
      }
