...
import { api, XIUQO_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "xiuqo:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${XIUQO_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${XIUQO_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?xiuqo_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-xiuqo-plugin", manifest.name);
