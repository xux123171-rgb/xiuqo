...
export function HermesConsoleModal({ open, onClose }: HermesConsoleModalProps) {
  ...
  const promptRef = useRef("xiuqo> ");
  const inputPromptRef = useRef("xiuqo> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "xiuqo> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Xiuqo Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="xiuqo-console-title"
    >
      <div
        className={cn(
          themedBody,
