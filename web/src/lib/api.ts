...
// (e.g. https://mission-control.tilos.com/xiuqo/). The Python backend
// injects ``window.__XIUQO_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__XIUQO_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const XIUQO_BASE_PATH = readBasePath();
const BASE = XIUQO_BASE_PATH;
...
declare global {
  interface Window {
    __XIUQO_SESSION_TOKEN__?: string;
    __XIUQO_BASE_PATH__?: string;
    ...
    __XIUQO_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Xiuqo-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__XIUQO_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``xiuqo update``, ``xiuqo gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__XIUQO_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__XIUQO_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__XIUQO_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``xiuqo_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__XIUQO_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Xiuqo-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``xiuqo_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__XIUQO_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateHermes: () =>
    fetchJSON<ActionResponse>("/api/xiuqo/update", { method: "POST" }),
  checkHermesUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/xiuqo/update/check${force ? "?force=true" : ""}`,
    ),
...
/** Preflight for `xiuqo gateway migrate --multiplex` (mirrors the CLI plan JSON). */
...
export interface SkillHubSource {
  ...
  /** xiuqo-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``xiuqo update``. */
  ...
  /** NS-656: disk-usage rollup for the XIUQO_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the XIUQO_HOME volume — no staleness dimension, so no
 * sampled_at. */
