...
const BACKEND = process.env.XIUQO_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `xiuqo dashboard` server injects a one-shot
 * session token into `index.html` (see `xiuqo_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__XIUQO_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function hermesDevToken(): Plugin {
  const TOKEN_RE = /window\.__XIUQO_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__XIUQO_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "xiuqo:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[xiuqo] Could not find session token in ${BACKEND} — ` +
              `is \`xiuqo dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__XIUQO_SESSION_TOKEN__="${match[1]}";` +
              `window.__XIUQO_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[xiuqo] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`xiuqo dashboard\` or set XIUQO_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@xiuqo/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `xiuqo dashboard` must serve these; Vite has no
