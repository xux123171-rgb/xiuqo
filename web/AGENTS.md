# web/ + xiuqo_cli/web_routers/ — the dashboard (`xiuqo dashboard` → `/chat`)

Applies on top of the root `AGENTS.md`. Backend routers: `xiuqo_cli/web_routers/*.py`, one file per
dashboard surface, mounted by `xiuqo_cli/web_server.py` (+ `web_server_*.py` siblings). Frontend:
`web/src/`. Shared JSON-RPC/WS client: `apps/shared` (`@xiuqo/shared`), also used by the desktop.

## The dashboard embeds the REAL `xiuqo --tui` — not a rewrite

`xiuqo_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `web_server.py`:
...
- The server spawns exactly what `xiuqo --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL
...
`cmd_dashboard` skips `_build_web_ui` and exports `XIUQO_SERVE_HEADLESS=1` so `mount_spa()`
...
- Routers are one-file-per-surface; a new surface is a new `web_routers/<surface>.py`, not a growing
  `web_server.py`.
- Tests: Python in `tests/xiuqo_cli/` (routers, pty bridge); JS in the `web/` vitest suite. Python
