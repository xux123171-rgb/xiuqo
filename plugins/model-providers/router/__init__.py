...
``api_mode="codex_responses"`` + the ``api.router.com`` host mandate in ``xiuqo_cli/providers.py``
...
class _CacheState:
    """Efforts cache + once-only flags for one Xiuqo home (same names as the module slots)."""
...
def _state() -> Any:
    ...
    with _efforts_lock:
        return _state_by_home.setdefault(xiuqo_home_key(), _CacheState())


def _base_url() -> str:
    """Router base URL: profile ``.env`` first (scope-aware), plain os.environ as the fallback."""
    try:
        from xiuqo_cli.config import get_env_value_prefer_dotenv as prefer_dotenv
    except Exception:
        prefer_dotenv = None
...
router = RouterProfile(
    ...
    default_headers={"User-Agent": f"Xiuqo-Agent/{_XIUQO_VERSION}"},
