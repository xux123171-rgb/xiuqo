...
An out-of-tree ACP provider (``~/.xiuqo/plugins/model-providers/`` or a pip entry point)
...
class CopilotACPProfile(ProviderProfile):
    ...
    def fetch_models(
        ...
        The CLI may keep its login in an OS credential store with no token Xiuqo can reuse, so
        ...
        """
        from xiuqo_cli.auth import resolve_external_process_provider_credentials
...
copilot_acp = CopilotACPProfile(
    ...
    # xiuqo_cli/auth.py), so existing setups keep working.
    process_command="copilot",
    process_args=("--acp", "--stdio"),
    process_command_env_vars=("XIUQO_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH"),
    process_args_env_var="XIUQO_COPILOT_ACP_ARGS",
)
