...
This plugin ships bundled with Xiuqo but is **opt-in** — it only loads when
...
xiuqo tools  # → Langfuse Observability

# Manual
pip install langfuse
xiuqo plugins enable observability/langfuse
```

## Required credentials

Set these in `~/.xiuqo/.env` (or via `xiuqo tools`):

```bash
XIUQO_LANGFUSE_PUBLIC_KEY=pk-lf-...
XIUQO_LANGFUSE_SECRET_KEY=sk-lf-...
XIUQO_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
```bash
xiuqo plugins list                 # observability/langfuse should show "enabled"
xiuqo chat -q "hello"              # then check Langfuse for a "Xiuqo turn" trace
```

Generation observations include the Xiuqo system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `XIUQO_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
XIUQO_LANGFUSE_ENV=production       # environment tag
XIUQO_LANGFUSE_RELEASE=v1.0.0       # release tag
XIUQO_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
XIUQO_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
XIUQO_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
XIUQO_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`XIUQO_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
xiuqo plugins disable observability/langfuse
```
