"""langfuse — Xiuqo plugin tracing conversations, LLM calls and tool usage to Langfuse.

Activated via ``plugins.enabled``; hooks are inert without the ``langfuse`` SDK
and credentials. Env: XIUQO_LANGFUSE_PUBLIC_KEY / SECRET_KEY (required),
...
# Under a multiplexed profile override, one settled client (or _INIT_FAILED) per Xiuqo home: the
...
_LANGFUSE_KEY_PREFIXES: Dict[str, str] = {
    "XIUQO_LANGFUSE_PUBLIC_KEY": "pk-lf-",
    "XIUQO_LANGFUSE_SECRET_KEY": "sk-lf-",
}
...
def _debug(message: str) -> None:
    if _env("XIUQO_LANGFUSE_DEBUG").lower() in {"1", "true", "yes", "on"}:
...
def _capture_mode() -> str:
    ...
    value = _env("XIUQO_LANGFUSE_CAPTURE").lower()
    ...
    if not _warned_invalid_capture:
        ...
        logger.warning(
            "Langfuse plugin: invalid XIUQO_LANGFUSE_CAPTURE=%r, falling back "
            "to %r (valid: %s)",
...
def _settled_client() -> Any:
    ...
    return _LANGFUSE_CLIENT_BY_HOME.get(xiuqo_home_key())
...
def _settle_client() -> Any:
    ...
    if get_hermes_home_override() is None:
        ...
        _LANGFUSE_CLIENT_BY_HOME[xiuqo_home_key()] = settled
...
def _build_client() -> Optional[Langfuse]:
    ...
    if Langfuse is None:
        logger.warning(
            ...
            "tracing is disabled. Run `xiuqo tools` and configure Langfuse "
            "Observability to reinstall it."
        )
        return None

    public_key, secret_key = (_secret(f"XIUQO_LANGFUSE_{n}") or _secret(f"LANGFUSE_{n}") for n in ("PUBLIC_KEY", "SECRET_KEY"))
    ...
    placeholder_issues = [issue for issue in (
        _validate_langfuse_key("XIUQO_LANGFUSE_PUBLIC_KEY", public_key),
        _validate_langfuse_key("XIUQO_LANGFUSE_SECRET_KEY", secret_key),
    ) if issue]
    if placeholder_issues:
        logger.warning(
            "Langfuse plugin: credentials look like placeholders, traces will "
            "NOT be emitted (%s). Set real Langfuse keys (pk-lf-... / sk-lf-...) "
            "or unset XIUQO_LANGFUSE_PUBLIC_KEY / XIUQO_LANGFUSE_SECRET_KEY to "
    ...
    for key, name, default in (("base_url", "BASE_URL", "https://cloud.langfuse.com"), ("environment", "ENV", ""),
                               ("release", "RELEASE", "")):
        value = _secret(f"XIUQO_LANGFUSE_{name}") or _secret(f"LANGFUSE_{name}") or default
        if value:
            kwargs[key] = value
    sample_rate = _secret("XIUQO_LANGFUSE_SAMPLE_RATE")
    if sample_rate:
        try:
            kwargs["sample_rate"] = float(sample_rate)
        except ValueError:
            logger.warning("Invalid XIUQO_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("XIUQO_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def _canonical_usage_and_cost(canonical: Any, *, provider: str, model: str,
    ...
    """Translate canonical Xiuqo usage into Langfuse usage and cost maps."""
    ...
    # need an explicit total (Xiuqo estimate also includes request pricing).
...
def _start_root_trace(task_key: str, *, task_id: str, session_id: str, platform: str, provider: str, model: str,
    ...
    metadata = {
        "source": "xiuqo", "task_id": task_id, "turn_id": turn_id, "api_request_id": api_request_id,
    ...
    def open_root():
        ctx = client.start_as_current_observation(trace_context=trace_ctx, name="Xiuqo turn", as_type="chain",
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(session_id=session_id or task_key, trace_name="Xiuqo turn",
                                      tags=["xiuqo", "langfuse"]):
...
def register(ctx) -> None:
    # Both hook-name variants so the plugin works across Xiuqo versions:
