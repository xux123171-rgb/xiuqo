...
after 14 days; empty dirs under XIUQO_HOME always. Prompt-only: research
(keep 10 newest, > 30 days), chrome-profile > 14 days, any file > 500 MB.
Scope: strictly XIUQO_HOME and /tmp/xiuqo-*; never ~/.xiuqo/logs/ or system dirs.
"""
...
def _state_file(name: str) -> Path:
    """``$XIUQO_HOME/disk-cleanup/<name>`` — deliberately outside ``$XIUQO_HOME/logs/``."""
    return get_hermes_home() / "disk-cleanup" / name


def is_safe_path(path: Path) -> bool:
    """Accept only paths under XIUQO_HOME or ``/tmp/xiuqo-*`` (rejects /mnt/c etc.)."""
    with contextlib.suppress(ValueError, OSError):
        path.resolve().relative_to(get_hermes_home())
        return True
    parts = path.parts
    return len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("xiuqo-")
...
# Top-level XIUQO_HOME dirs whose empty subdirs are never swept (last row: user project trees).
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    "logs", "memories", "sessions", "cron", "cronjobs",
    "cache", "skills", "plugins", "disk-cleanup", "optional-skills",
    "xiuqo", "backups", "profiles", ".worktrees",
    "patches", "projects", "skins", "themes", "contributors"})

_EMPTY_DIR_SWEEP_PRUNE_DIRS = frozenset({
    ".git", "node_modules", "venv", ".venv", "site-packages", "__pycache__"})

# Top-level XIUQO_HOME entries guess_category() never auto-tracks: state, logs, memory,
...
# Paths under $XIUQO_HOME that must NEVER be deleted by quick(), regardless of what the stored category
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside XIUQO_HOME)")
        return False
...
def _sweep_empty_dirs(xiuqo_home: Path) -> int:
    """Remove empty dirs under XIUQO_HOME without recursing into durable/heavy trees (a full
    rglob over a checkout+venv under XIUQO_HOME can stall the gateway loop for minutes).
    Iterative post-order so parents emptied by child removal are caught."""
    removed = 0
    stack: List[Tuple[Path, bool]] = [
        (top, False) for top in _subdirs(xiuqo_home, _EMPTY_DIR_PROTECTED_TOP_LEVEL | _EMPTY_DIR_SWEEP_PRUNE_DIRS)]
    while stack:
...
def guess_category(path: Path) -> Optional[str]:
    ...
    if not is_safe_path(path):
        return None
    with contextlib.suppress(ValueError):  # not under XIUQO_HOME (/tmp/xiuqo-*) — name rules only
