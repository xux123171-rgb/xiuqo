...
Auto-tracks and cleans up ephemeral files created during Xiuqo Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$XIUQO_HOME` and `/tmp/xiuqo-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `XIUQO_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under XIUQO_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `XIUQO_HOME` or `/tmp/xiuqo-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$XIUQO_HOME/disk-cleanup/` is itself excluded
- `$XIUQO_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
