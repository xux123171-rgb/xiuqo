...
_DEFAULT_AGENT = ""
_OPENVIKING_USER_AGENT = f"openviking-memory-xiuqo/{_XIUQO_VERSION}"
...
_SYNC_TRACE_ENV = "XIUQO_OPENVIKING_SYNC_TRACE"
...
_RETRY_LATER = (
    "OpenViking memory is temporarily unavailable; Xiuqo will retry on a later access or when the config changes."
)
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Xiuqo slash-skill scaffolding before sending content to OpenViking
...
# One entry per Xiuqo home: a multiplexed gateway initializes a provider per profile and every
...
def _normalize_openviking_url(url: str) -> str:
    ...
    try:
        ...
        raise _OpenVikingEndpointError("OpenViking endpoint safety validation failed; Xiuqo refused the connection.") from exc
    if blocked:
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        return _LOCAL_SERVER_OCCUPIED, (
            f"Port {host}:{port} is occupied by {_describe_local_port_listener(host, port)}. Xiuqo did not start "
    ...
    try:
        ...
        # Strip PYTHONPATH: the Desktop backend puts the Xiuqo venv on it, which
        # would shadow openviking-server's own site-packages (and on Windows lock
        # the Xiuqo venv's .pyd files, breaking `xiuqo update`).
        # Do not let the server child inherit this process's PYTHONPATH. If inherited, openviking-server
        # would import aiohttp and friends from the Xiuqo venv instead of its own (its venv's site-packages
        # are shadowed because PYTHONPATH precedes them) — and on Windows the loaded DLLs then lock the
        # Xiuqo venv, aborting `xiuqo update` with access-denied on .pyd files. (#78153)
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # increment. See xiuqo#28296 review.
    ...
    def save_config(self, values: Dict[str, Any], xiuqo_home: str) -> None:
    ...
    def post_setup(self, xiuqo_home: str, config: dict) -> None:
        """Interactive setup that can reuse OpenViking's shared CLI config (see ``_setup``)."""
        _setup.run_setup(xiuqo_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        self._hermes_home = str(kwargs.get("xiuqo_home") or "").strip() or str(get_hermes_home())
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        so OPENVIKING_* values added to ``~/.xiuqo/.env`` after startup never reach the live client and
        tools keep running against stale auth until the user restarts xiuqo (#21130).
        """
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Xiuqo will retry on a later access (after cooldown) or when the config changes.",
                health_message,
            )
        else:
    ...
    def _extract_current_turn_messages(messages: Optional[List[Dict[str, Any]]], user_content: str, assistant_content: str) -> List[Dict[str, Any]]:
        """Slice the completed turn out of Xiuqo' full canonical transcript: the last
    ...
    def _messages_to_openviking_batch(messages: List[Dict[str, Any]], *, assistant_peer_id: str = "") -> List[Dict[str, Any]]:
        """Convert Xiuqo canonical messages into OpenViking batch payloads.
    ...
    def _state_path(self, kind: str, name: str) -> Optional[Path]:
        """Marker/lock file under XIUQO_HOME: ``pending`` -> pending_sessions/<sid>.json,
    ...
    def _tool_remember(self, args: dict) -> str:
        """Submit content through a dedicated session so it never touches the live Xiuqo session."""
        ...
        session_id = f"xiuqo-remember-{uuid.uuid4().hex[:12]}"
        ...
        def failure(message: str, *, stage: str, message_status: str) -> str:
            return tool_error(
                ...
                recovery_note=(
                    ...
                    "Xiuqo. Otherwise, do not resubmit automatically; report the uncertain state to the user."
                ),
            )
        try:
