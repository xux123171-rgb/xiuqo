"""Interactive ``xiuqo memory setup`` wizard for the OpenViking provider.

Pure UI flow: prompts, menus, and persistence of the chosen connection (Xiuqo
``.env`` only, or mirrored to an ``ovcli.conf.<name>`` profile that Xiuqo then
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Xiuqo session to activate.\n")
...
def _run_create_profile_setup(*, prompt, select, cancelled, config: dict, provider_config: dict, env_path: Path) -> bool | object:
    ...
    save_choice = select("  Save OpenViking config",
                         [("Keep in Xiuqo only", "write values only to Xiuqo .env"),
    ...
    _print_openviking_ready("Connection saved to Xiuqo .env.")
    return True


def run_setup(xiuqo_home: str, config: dict) -> None:
    """Entry point for ``OpenVikingMemoryProvider.post_setup``."""
    from xiuqo_cli.config import save_config
    from xiuqo_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

    env_path = Path(xiuqo_home) / ".env"
