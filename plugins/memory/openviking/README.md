...
- OpenViking server running and reachable from Xiuqo

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Xiuqo can identify older servers that expose the legacy status-only health
...
Then configure Xiuqo:

```bash
xiuqo memory setup    # select "openviking"
```

The setup can link to an existing `~/.openviking/ovcli.conf`, copy its current
connection values into Xiuqo, or create a minimal `ovcli.conf` when one does
not exist.

Or manually:

```bash
xiuqo config set memory.provider openviking
```

Add the connection settings to the active profile's `.env` file. For the
default profile that is `~/.xiuqo/.env`; for a named profile use
`~/.xiuqo/profiles/<profile>/.env`.
...
OpenViking's server config is separate from Xiuqo:
...
Xiuqo-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Xiuqo lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Xiuqo sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Xiuqo also sends `User-Agent: openviking-memory-xiuqo/<version>` on
OpenViking requests. This standard harness identifier contains the Xiuqo
...
does not ask for a peer ID. Without a configured peer, Xiuqo sends neither
...
order remains environment, linked OpenViking config, then Xiuqo YAML. To use
...
on the old implicit `xiuqo` peer now use user memory for new writes. Without
...
Set `agent: xiuqo` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `xiuqo-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Xiuqo
...
Use the same OpenViking profile and credentials as Xiuqo for manual recovery.
...
Xiuqo built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Xiuqo action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Xiuqo
...
file URIs, such as
`viking://user/default/peers/xiuqo/memories/preferences/mem_abc123.md` (any
