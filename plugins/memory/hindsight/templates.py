...
catalog, filter to the ``xiuqo`` integration, apply a manifest via
...
def fetch_hermes_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``xiuqo`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "xiuqo" in (e.get("integrations") or [])]
