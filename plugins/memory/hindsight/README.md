...
```bash
xiuqo memory setup    # select "hindsight"
```
...
```bash
xiuqo config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.xiuqo/.env
```
...
Xiuqo spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.xiuqo/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p xiuqo ui start
```
...
Config file: `~/.xiuqo/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `xiuqo` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `xiuqo-{profile}` isolates memory per active Xiuqo profile. Empty placeholders collapse cleanly (e.g. `xiuqo-{user}` with no user becomes `xiuqo`). |
...
| `retain_context` | `conversation between Xiuqo Agent and the User` | Context label for retained memories |
| `retain_tags` | — | Default tags applied to retained memories; merged with per-call tool tags |
| `retain_source` | — | Opt-in `metadata.source` attached to retained memories (identifies the storing client, e.g. `xiuqo`). Empty by default — no attribution tag ships unless you set it. |
...
The LLM API key is stored in `~/.xiuqo/.env` as `HINDSIGHT_LLM_API_KEY`.
