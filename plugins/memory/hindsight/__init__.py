...
Config: $XIUQO_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/
...
_RETAIN_CONTEXT_DEFAULT = "conversation between Xiuqo Agent and the User"
...
def _load_config() -> dict:
    """$XIUQO_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/config.json
    ...
    return {
        ...
        "banks": {"xiuqo": {"bankId": get_secret("HINDSIGHT_BANK_ID", "") or "xiuqo",
                             "budget": os.environ.get("HINDSIGHT_BUDGET", "mid"), "enabled": True}},
    }


def _event_timestamp() -> str:
    """Configured Xiuqo event time with an explicit UTC offset."""
    event_time = _hermes_now()
    # xiuqo_time.now() is aware; guard a replacement clock emitting offset-less dates.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._bank_id, self._budget, self._bank_id_template = "xiuqo", "mid", ""
    ...
    def save_config(self, values, xiuqo_home):
        """Merge *values* into $XIUQO_HOME/hindsight/config.json."""
        from utils import atomic_json_write
        config_path = Path(xiuqo_home) / "hindsight" / "config.json"
    ...
    def post_setup(self, xiuqo_home: str, config: dict) -> None:
        ...
        run_setup(self, xiuqo_home, config)
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "xiuqo"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: xiuqo-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Xiuqo Agent and the User"},
    ...
    def _new_embedded_client(self):
        ...
        logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                     cfg.get("profile", "xiuqo"), llm_provider)
        ...
        kwargs = dict(profile=cfg.get("profile", "xiuqo"), llm_provider=llm_provider,
    ...
    def _apply_connection_settings(self, cfg: dict) -> None:
        ...
        banks = cfg_get(cfg, "banks", "xiuqo", default={})
        self._bank_id_template = cfg.get("bank_id_template", "") or ""
        self._bank_id = _resolve_bank_id_template(
            self._bank_id_template,
            fallback=cfg.get("bank_id") or banks.get("bankId", "xiuqo"),
    ...
    def _start_embedded_daemon(self) -> None:
        ...
        if hasattr(os, "geteuid") and os.geteuid() == 0:
            msg = ("Hindsight local_embedded mode cannot run as root "
                   ...
                   "memory daemon. Run Xiuqo as a non-root user, or switch "
                   "to cloud / local_external mode via 'xiuqo memory setup'.")
            logger.warning(msg)
            # Also print: otherwise the user would only see Xiuqo get sluggish.
            with contextlib.suppress(Exception):
                # Surface to the terminal too — a daemon that never starts would otherwise fail silently and
                # the user would only see Xiuqo get sluggish. (issue #13125)
    ...
    def _daemon_start_worker(self) -> None:
        ...
        try:
            ...
            profile = self._config.get("profile", "xiuqo")
