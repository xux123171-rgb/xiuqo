...
```bash
xiuqo memory setup    # select "mem0"
```

Or manually:
```bash
xiuqo config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.xiuqo/.env
```

## Config

Behavioral settings live in `$XIUQO_HOME/mem0.json` (set them via `xiuqo memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.xiuqo/.env`.
...
| `user_id` | `xiuqo-user` | User identifier on Mem0 |
| `agent_id` | `xiuqo` | Agent identifier |
...
2. Point the plugin at it — via the setup wizard:
   ```bash
   xiuqo memory setup    # select "mem0" → "Self-hosted server"
   # Or non-interactive:
   xiuqo memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
   ```
   or via env vars:
   ```bash
   echo "MEM0_HOST=http://localhost:8888" >> ~/.xiuqo/.env
   echo "MEM0_API_KEY=your-admin-api-key" >> ~/.xiuqo/.env
   ```
   or in `$XIUQO_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh Xiuqo session and call `mem0_search` — it connects to your server.
...
```bash
xiuqo memory setup mem0 --mode oss \
...
```bash
xiuqo memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$XIUQO_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.xiuqo/mem0_qdrant"}}
  }
}
```
...
```bash
xiuqo memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
xiuqo memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$XIUQO_HOME/mem0.json`).
