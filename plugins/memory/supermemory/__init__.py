...
_DEFAULT_CONTAINER_TAG = "xiuqo"
_VALID_SEARCH_MODES = ("hybrid", "memories", "documents")
_DEFAULT_BASE_URL = "https://api.supermemory.ai"
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=xiuqo"
...
def _load_supermemory_config(xiuqo_home: Optional[str] = None) -> dict:
    """Defaults overlaid with $xiuqo_home/supermemory.json (None = defaults only), every key normalized."""
    config = {k: (list(d) if isinstance(d, list) else d) for k, (d, _) in _CONFIG_SPEC.items()}
    if xiuqo_home is not None:
        config.update({k: v for k, v in _read_json_dict(Path(xiuqo_home) / "supermemory.json").items() if v is not None})
    for key, (_, normalize) in _CONFIG_SPEC.items():
        config[key] = normalize(config[key])
    return config


def _save_supermemory_config(values: dict, xiuqo_home: str) -> None:
    from utils import atomic_json_write
    config_path = Path(xiuqo_home) / "supermemory.json"
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(api_key=api_key, base_url=self._base_url, timeout=timeout, max_retries=0,
                                   default_headers={"x-sm-source": "xiuqo"})

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Xiuqo writes into the "Xiuqo" Space in the Supermemory app so the user
        # can filter / bulk-manage them per source agent (a routing key for the user, not telemetry).
        merged = {"sm_source": "xiuqo", **(metadata or {})}
...
def _probe_supermemory_connection(api_key: str, xiuqo_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(xiuqo_home)
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only the API key is prompted during `xiuqo memory setup`; other options live in supermemory.json / env.
    ...
    def save_config(self, values, xiuqo_home):
        ...
        _save_supermemory_config(sanitized, xiuqo_home)
    ...
    def post_setup(self, xiuqo_home: str, config: dict) -> None:
        ...
        from xiuqo_cli.memory_setup import _prompt, _write_env_vars
        ...
        save_config(config)
        if val:
            _write_env_vars({"SUPERMEMORY_API_KEY": val}, xiuqo_home=xiuqo_home)
        ...
        status = _probe_supermemory_connection(api_key, xiuqo_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from xiuqo_constants import get_hermes_home
        self._hermes_home = kwargs.get("xiuqo_home") or str(get_hermes_home())
