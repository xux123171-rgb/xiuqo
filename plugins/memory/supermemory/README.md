...
- Hosted: API key from [app.supermemory.ai/integrations?connect=xiuqo](http://app.supermemory.ai/integrations?connect=xiuqo)
...
```bash
xiuqo memory setup    # select "supermemory"
```

Or manually:

```bash
xiuqo config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.xiuqo/.env
```
...
Before running `xiuqo memory setup`, add the local endpoint to
`$XIUQO_HOME/supermemory.json`:
...
Then run `xiuqo memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$XIUQO_HOME/supermemory.json`
...
| `container_tag` | `xiuqo` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `xiuqo-{identity}` → `xiuqo-coder`). |
...
`https://api.supermemory.ai`. Xiuqo resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: xiuqo`, and document writes stamp
`metadata.sm_source: xiuqo`. This is a **functional routing key, not telemetry**:
it groups Xiuqo-written memories into a dedicated "Xiuqo" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Xiuqo profile:

```json
{
  "container_tag": "xiuqo-{identity}"
}
```

For a profile named `coder`, this resolves to `xiuqo-coder`. The default profile resolves to `xiuqo-default`. Without `{identity}`, all profiles share the same container.
