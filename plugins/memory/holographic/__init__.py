"""xiuqo-memory-store — holographic memory plugin (MemoryProvider): structured fact storage with entity
resolution, trust scoring, and HRR-based compositional retrieval. Original plugin by dusterbloom (PR #2351).
Config in $XIUQO_HOME/config.yaml under plugins.xiuqo-memory-store: db_path ($XIUQO_HOME/memory_store.db),
...
def _load_plugin_config() -> dict:
    try:
        from xiuqo_cli.config import load_config_readonly  # canonical: managed-scope overlay + ${VAR} expansion
        return cfg_get(load_config_readonly(), "plugins", "xiuqo-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, xiuqo_home):
        """Write config to config.yaml under plugins.xiuqo-memory-store."""
        # The canonical writer: config lock, managed-mode refusal, default stripping, atomic replace.
        # ``merge_existing`` keeps every other section; *xiuqo_home* is the active profile already.
        from xiuqo_cli.config import save_config
        save_config({"plugins": {"xiuqo-memory-store": dict(values)}}, merge_existing=True)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        if isinstance(db_path, str):  # expand $XIUQO_HOME so paths resolve to the active profile
            db_path = db_path.replace("$XIUQO_HOME", _hermes_home).replace("${XIUQO_HOME}", _hermes_home)
