...
```bash
xiuqo memory setup    # select "holographic"
```

Or manually:
```bash
xiuqo config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.xiuqo-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$XIUQO_HOME/memory_store.db` | SQLite database path |
