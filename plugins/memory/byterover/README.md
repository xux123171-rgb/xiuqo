...
```bash
xiuqo memory setup    # select "byterover"
```

Or manually:
```bash
xiuqo config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.xiuqo/.env
```
...
Working directory: `$XIUQO_HOME/byterover/` (profile-scoped).
