...
Config chain: $XIUQO_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
"""
...
class HonchoMemoryProvider(DialecticMixin, MemoryProvider):
    ...
    def save_config(self, values, xiuqo_home):
        """Merge ``values`` into $XIUQO_HOME/honcho.json (Honcho SDK native format); a file that does not parse raises.
        ...
        config_path = Path(xiuqo_home) / "honcho.json"
    ...
    def _resolve_session_key(self, cfg, session_id: str, **kwargs) -> str:
        ...
        ) or session_id or "xiuqo-default"
    ...
    def _run_session_init(self, label: str) -> bool:
        ...
        try:
            self._do_session_init(self._config, self._lazy_init_session_id or "xiuqo-default", **dict(init_kwargs))
        except Exception as e:
            self._manager = None
            self._session_initialized = False
            detail: object = e
    ...
    def _pop_auth_notice(self) -> str:
        ...
        return ("[Honcho memory status] Authentication with the Honcho memory backend has expired and automatic "
                ...
                "Tell the user (once) that Honcho memory is paused and that running 'xiuqo honcho setup' "
    ...
    def _peer_failure_text(self) -> str:
        ...
        if self._init_peer_platform in _LOCAL_PLATFORMS:
            return f"{text} Set one with 'xiuqo honcho peer --user <name>'."
