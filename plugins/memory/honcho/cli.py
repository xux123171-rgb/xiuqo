"""``xiuqo honcho`` subcommands: setup wizard, status, peers, sessions, identity, migrate."""
...
from xiuqo_cli.config import cfg_get
from utils import read_json_or_empty

RULE = "─" * 40
REASONING_LEVELS = ("minimal", "low", "medium", "high", "max")
_RETRY_HINT = "  Re-run 'xiuqo honcho setup' to retry, or choose an API key instead.\n"
...
def _label(host: str) -> str:
    return f"[{host}] " if host != "xiuqo" else ""
...
def _sync_profiles(verbose: bool) -> int:
    ...
    cfg = _read_config()
    if not cfg:
        return say("  No Honcho config found. Run 'xiuqo honcho setup' first.\n") or 0
    default_block, has_key = _default_block_and_key(cfg)
    if not default_block and not has_key:
        return say("  Honcho not configured on default profile. Run 'xiuqo honcho setup' first.\n") or 0
...
def sync_honcho_profiles_quiet() -> int:
    """Sync host blocks for all profiles from `xiuqo update`; no output, no exceptions."""
...
def cmd_enable(args) -> None:
    ...
    if not _resolve_api_key(cfg, block, env=False):
        ...
        setup = "xiuqo honcho setup" + (f" --target-profile {profile}" if profile != "default" else "")
...
def _resolve_effective_identity_mapping(cfg: dict, xiuqo_host: dict) -> tuple[bool, dict, str, bool, bool]:
    ...
    pin_sources = (xiuqo_host.get("pinUserPeer"), xiuqo_host.get("pinPeerName"),
                   cfg.get("pinUserPeer"), cfg.get("pinPeerName"))
    pin = bool(next((v for v in pin_sources if v is not None), False))

    def _inherit(key):
        if key in xiuqo_host:
            return xiuqo_host.get(key), False
        val = cfg.get(key)
...
def _apply_runtime_prefix(xiuqo_host: dict, current_prefix: str, prefix_from_root: bool, label: str) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        xiuqo_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(xiuqo_host: dict) -> None:
    print(f"  resolved →\n    pinUserPeer       = {bool(xiuqo_host.get('pinUserPeer'))}\n"
          f"    userPeerAliases   = {xiuqo_host.get('userPeerAliases') or '{}'}\n"
          f"    runtimePeerPrefix = {xiuqo_host.get('runtimePeerPrefix') or '(none)'}")


def _configure_raw_identity_mapping(xiuqo_host, current_pin, current_aliases, current_prefix,
    ...
    _scrub_identity_mapping(xiuqo_host)
    xiuqo_host["pinUserPeer"] = pin
    if pin:
        return
    ...
    while entry := _prompt("    alias", default="").strip():
        ...
        if rid and peer:
            aliases[rid] = peer
    if aliases:
        xiuqo_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(xiuqo_host, current_prefix, prefix_from_root,
                          "runtimePeerPrefix — namespace for unknown IDs (blank for none)")


def _setup_identity_mapping(cfg: dict, xiuqo_host: dict, current_peer: str, new_host: bool) -> None:
    ...
    current_pin, current_aliases, current_prefix, aliases_from_root, prefix_from_root = (
        _resolve_effective_identity_mapping(cfg, xiuqo_host))
    ...
    if gw_platforms:
        ...
        notice, question = (
            ("\n  Each gateway account (a Telegram user, a Discord user, ...)\n"
             ...
             "Running the Xiuqo gateway (Telegram/Discord/etc.)? (y/N)") if gw_platforms is None else
            ("\n  No gateway platforms connected — nothing to map.", "Configure anyway? (y/N)"))
        print(notice)
        if not _yes(_prompt(question, default="n")):
            return

    peer_target = xiuqo_host.get("peerName") or current_peer or "user"
    ai_peer_label = xiuqo_host.get("aiPeer") or cfg.get("aiPeer") or "xiuqo"
    ...
    print("\n  This step covers the HUMAN mapping only. Each account using the\n"
          ...
          "  Xiuqo profile brings its own AI peer to the gateway.\n"
    ...
    if shape == "raw":
        _configure_raw_identity_mapping(xiuqo_host, current_pin, current_aliases, current_prefix,
        ...
        if shape == "multi" and aliases_from_root:
            prior_aliases = {}
        _scrub_identity_mapping(xiuqo_host)  # each shape starts from a clean slate
        xiuqo_host["pinUserPeer"] = shape == "single"
        if shape == "single":
    ...
    _echo_identity_mapping(xiuqo_host)
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if xiuqo_cli internals move."""
    try:
        from xiuqo_cli.auth import _can_open_graphical_browser, _is_remote_session
        return _is_remote_session(), _can_open_graphical_browser()
    except Exception:
        return False, True


def _apply_grant_to_host(cfg: dict, xiuqo_host: dict, cred) -> None:
    ...
    xiuqo_host["apiKey"] = cred.access_token
    xiuqo_host["oauth"] = cred.oauth_block()
    ...
    if cred.consent_peer_name:  # default the peer prompt to the consent name
        xiuqo_host["peerName"] = cred.consent_peer_name
    print("  Authorized — token saved. Let's finish configuring.\n")


def _setup_local_auth(cfg: dict, xiuqo_host: dict) -> None:
    ...
    if new_url := _prompt("Base URL", default=cfg.get("baseUrl") or "http://localhost:8000"):
        cfg["baseUrl"] = new_url
    current_host_key = xiuqo_host.get("apiKey", "")
    ...
    if new_local_key:
        xiuqo_host["apiKey"] = new_local_key
...
def _setup_device_login(cfg: dict, xiuqo_host: dict, write_path: Path, *, open_browser: bool) -> bool:
    ...
    try:
        ...
        print(" approved")
        _apply_grant_to_host(cfg, xiuqo_host, cred)
        return True
    return False


def _setup_browser_login(cfg: dict, xiuqo_host: dict, write_path: Path) -> bool:
    ...
    try:
        cred = authorize_via_loopback(config_path=write_path, source="xiuqo-cli", apply_config=False, open_url=_open)
    except Exception as e:
...
def _setup_cloud_auth(cfg: dict, xiuqo_host: dict, write_path: Path) -> bool:
    ...
    existing_oauth = OAuthCredential.from_host_block(xiuqo_host)
    ...
    if device_available and method in {"device", "d"}:
        return _setup_device_login(cfg, xiuqo_host, write_path, open_browser=can_browse and not is_remote)
    if method in {"oauth", "o"}:
        return _setup_browser_login(cfg, xiuqo_host, write_path)
    # A leftover grant on the host block would shadow the pasted key.
    stale_grant = existing_oauth is not None or is_oauth_access_token(xiuqo_host.get("apiKey"))
    current = ("" if stale_grant else xiuqo_host.get("apiKey", "")) or cfg.get("apiKey", "")
    ...
    if not key:
        print("\n  No API key configured. Get yours at https://app.honcho.dev\n"
              "  Run 'xiuqo honcho setup' again once you have a key.\n")
        return False
    xiuqo_host.pop("oauth", None)
    xiuqo_host["apiKey"] = key
    return True
...
def _choice_step(xiuqo_host, key, current, label, valid, fallback=None) -> None:
    """Prompt for one of ``valid``; an invalid answer writes ``fallback`` (None = keep current)."""
    new = _prompt(label, default=current)
    if new in valid:
        xiuqo_host[key] = new
    elif fallback is not None:
        xiuqo_host[key] = fallback


def _setup_tuning(cfg: dict, xiuqo_host: dict) -> None:
    ...
    _choice_step(xiuqo_host, "observationMode", _pref(xiuqo_host, cfg, "observationMode", "directional"),
    ...
    new_wf = _prompt("Write frequency", default=str(_pref(xiuqo_host, cfg, "writeFrequency", "async")))
    xiuqo_host["writeFrequency"] = _first_parsed([new_wf], int, new_wf if new_wf in {"async", "turn", "session"} else "async")

    _menu("Recall mode", *(f"{m:<7} -- {desc}" for m, desc in _MODES.items()))
    raw_recall = _pref(xiuqo_host, cfg, "recallMode", "hybrid")
    _choice_step(xiuqo_host, "recallMode", raw_recall if raw_recall in _MODES else "hybrid", "Recall mode", _MODES)

    xiuqo_host["recallSync"] = _yes(_prompt(
        "Wait for current-query recall (bounded by request timeout, default 5s)? (y/N)",
        default="y" if xiuqo_host.get("recallSync", cfg.get("recallSync", False)) else "n"))

    current_ctx_tokens = _pref(xiuqo_host, cfg, "contextTokens")
    ...
    _menu("Dialectic cadence",
          ...
          "Recommended: 1-5.")
    new_dialectic = _prompt("Dialectic cadence", default=str(_pref(xiuqo_host, cfg, "dialecticCadence") or "2"))
    if (val := _first_parsed([new_dialectic], int, None)) is None:
        xiuqo_host["dialecticCadence"] = 2
    elif val >= 1:
        xiuqo_host["dialecticCadence"] = val
    ...
    _choice_step(xiuqo_host, "dialecticReasoningLevel", _pref(xiuqo_host, cfg, "dialecticReasoningLevel") or "low",
                 "Reasoning level", REASONING_LEVELS, "low")

    _menu("Session strategy", *(f"{s:<13} -- {desc}" for s, desc in _STRATEGIES.items()))
    _choice_step(xiuqo_host, "sessionStrategy", _pref(xiuqo_host, cfg, "sessionStrategy", "per-session"),
...
def _setup_wizard(args) -> None:
    ...
    print(f"\nHoncho memory setup\n{RULE}\n  Honcho gives Xiuqo persistent cross-session memory.\n  Config: {write_path}")
    ...
    xiuqo_host = cfg.setdefault("hosts", {}).setdefault(_host_key(), {})
    _migrate_pin_key(cfg)  # canonicalize legacy pinPeerName before detection/writes
    _migrate_pin_key(xiuqo_host)
    # Taken before the prompts populate the block: an existing install must not default to pinning every account.
    new_host = not any(k in xiuqo_host or k in cfg for k in (*_IDENTITY_MAPPING_KEYS, "peerName", "workspace", "enabled"))
    ...
    if is_local:
        _setup_local_auth(cfg, xiuqo_host)
    elif not _setup_cloud_auth(cfg, xiuqo_host, write_path):
        return

    # --- 3. Identity ---
    current_peer = xiuqo_host.get("peerName") or cfg.get("peerName", "")
    for key, label, default in (
        ("peerName", "Your name (user peer)", current_peer or os.getenv("USER", "user")),
        ("aiPeer", "AI peer name", _pref(xiuqo_host, cfg, "aiPeer", "xiuqo")),
        ("workspace", "Workspace ID", _pref(xiuqo_host, cfg, "workspace", "xiuqo")),
    ):
        if new := _prompt(label, default=default):
            xiuqo_host[key] = new

    _setup_identity_mapping(cfg, xiuqo_host, current_peer, new_host)
    print("\n  For a gateway with many users and agents, run\n"
          "  'xiuqo honcho peers map' to map accounts interactively.")

    _setup_tuning(cfg, xiuqo_host)
    xiuqo_host["enabled"] = True
    xiuqo_host.setdefault("saveMessages", True)
    _write_config(cfg)
    print(f"\n  Config written to {write_path}")

    try:  # auto-enable Honcho as memory provider in config.yaml
        from xiuqo_cli.config import load_config, save_config
        xiuqo_config = load_config()
        xiuqo_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(xiuqo_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}\n  Run: xiuqo config set memory.provider honcho")
  ...
  Other commands:
    xiuqo honcho status     -- show full config
    xiuqo honcho mode       -- change recall/observation mode
    xiuqo honcho tokens     -- tune context and dialectic budgets
    xiuqo honcho peer       -- update peer names
    xiuqo honcho map <name> -- map this directory to a session name
""")
...
def _active_profile_name() -> str:
    """Active Xiuqo profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    cfg = _read_config()
    # _host_block (not hosts.get) keeps legacy dot-form keys ("xiuqo.work") readable.
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: xiuqo honcho setup\n")
        return
...
def _state_db_path() -> Path:
    ...
    if _profile_override and _profile_override not in {"default", "custom"}:
        try:
            from xiuqo_cli.profiles import get_profile_dir
...
def _classify_workspace_peers(
    ...
    xiuqo_hosts = {hostk for _, hostk, _ in profile_rows}
    ...
    # Host blocks that are not Xiuqo profiles: other apps sharing the config.
    for hostk, block in (cfg.get("hosts") or {}).items():
        if hostk in xiuqo_hosts or not isinstance(block, dict):
            continue
...
def cmd_peers_map(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    xiuqo_host = _host_block(cfg, host)
    pin, aliases, prefix, aliases_from_root, _ = _resolve_effective_identity_mapping(cfg, xiuqo_host)
    peer_name = xiuqo_host.get("peerName") or cfg.get("peerName") or ""

    if pin:
        print("\n  pinUserPeer is on: every gateway account resolves to peer")
        print(f"  '{peer_name or '(peerName not set)'}' and aliases have no effect.")
        print("  Turn the pin off with 'xiuqo honcho setup' to use per-account peers.")
    ...
    workspace = (
        ...
        or xiuqo_host.get("workspace") or cfg.get("workspace") or host
    )
...
Honcho peers
{RULE}
  User peer:   {_pref(xiuqo, cfg, 'peerName') or '(not set)'}
    Your identity in Honcho. Messages you send build this peer's card.
  AI peer:     {_pref(xiuqo, cfg, 'aiPeer') or _host_key()}
    Xiuqo' identity in Honcho. Seed with 'xiuqo honcho identity <file>'.
    Dialectic calls ask this peer questions to warm session context.

  Dialectic reasoning:  {_pref(xiuqo, cfg, 'dialecticReasoningLevel') or 'low'}  ({', '.join(REASONING_LEVELS)})
  Dialectic cap:        {_pref(xiuqo, cfg, 'dialecticMaxChars') or 600} chars
""")
...
def _show_or_set_choice(args, *, attr: str, key: str, noun: str, title: str, choices: dict,
    ...
    if value is None:
        ...
        return print(f"\n  Set with: xiuqo honcho {attr} [{'|'.join(choices)}]\n")
...
{RULE}
  ...
  Context     {_pref(xiuqo, cfg, 'contextTokens') or '(Honcho default)'} tokens
  ...
  Dialectic   {_pref(xiuqo, cfg, 'dialecticMaxChars') or 600} chars, reasoning: {_pref(xiuqo, cfg, 'dialecticReasoningLevel') or 'low'}
    AI-to-AI inference. Xiuqo asks Honcho's AI peer a question
  ...
  Set with: xiuqo honcho tokens [--context N] [--dialectic N]
""")
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        return print("  No API key configured. Run 'xiuqo honcho setup' first.\n")
    ...
    if getattr(args, "show", False):
        ...
        print(ai_rep.get("representation") or ai_rep.get("card")
              or "  No representation built yet.\n  Run 'xiuqo honcho identity <file>' to seed one.")
        print()
        return
...
{RULE}
  ...
  AI peer:   {hcfg.ai_peer}
    ...
    xiuqo honcho identity --show        — show both peer representations
    xiuqo honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt
""")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Xiuqo + Honcho."""
    ...
    print("\nHoncho migration: OpenClaw native memory → Xiuqo\n" + "─" * 50)
    print("""
...
""")
    if has_key:
        ...
        print("""  Honcho is a cloud memory service that gives Xiuqo persistent memory
  across sessions. You need an API key to use it.

  1. Get your API key at https://app.honcho.dev
  2. Run:  xiuqo honcho setup
     Paste the key when prompted.
""")
        if _yes(_prompt("  Run 'xiuqo honcho setup' now?", default="y")):
            cmd_setup(args)
            cfg = _read_config()
            has_key = bool(cfg.get("apiKey", ""))
        else:
            print("\n  Run 'xiuqo honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  No OpenClaw native memory files found in cwd or ~/.openclaw/.\n"
              ...
              "  or seed them manually:  xiuqo honcho identity <path/to/file>")
...
""")
    if user_files:
        ...
        print("""
  These are picked up automatically the first time you run 'xiuqo'
  with Honcho configured and no prior session history.
  (Xiuqo calls migrate_memory_files() on first session init.)

  If you want to migrate them now without starting a session:""")
        print("    xiuqo honcho migrate  — this step handles it interactively\n" * len(user_files), end="")
        if has_key:
            _offer("  Upload user memory files to Honcho now?", _migrate_upload, user_files)
        else:
            print("  Run 'xiuqo honcho setup' first, then re-run this step.")
    else:
...
Step 4  Seed AI identity files → Honcho AI peer
  ...
  In Xiuqo, they are seeded once into Honcho's AI peer through the
...
""")
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'xiuqo honcho setup' first, then seed manually:")
            print("\n".join(f"    xiuqo honcho identity {f}" for f in agent_files))
    else:
        print("  No agent identity files detected.\n  To seed manually:  xiuqo honcho identity <path/to/SOUL.md>")
...
Step 5  What changes vs. OpenClaw native memory
  ...
  Session naming
    ...
    Xiuqo:   per-session by default — each run gets its own session
              Map a custom name:  xiuqo honcho map <session-name>

Step 6  Next steps
""")
    if not has_key:
        print("  1. xiuqo honcho setup              — configure API key (required)\n"
              "  2. xiuqo honcho migrate            — re-run this walkthrough")
    else:
        print("""  1. xiuqo honcho status             — verify Honcho connection
  2. xiuqo                           — start a session
     (user memory files auto-uploaded on first turn if not done above)
  3. xiuqo honcho identity --show    — verify AI peer representation
  4. xiuqo honcho tokens             — tune context and dialectic budgets
  5. xiuqo honcho mode               — view or change memory mode""")
    print()
...
_SUBCOMMANDS = (
    ("setup", "Initial Honcho setup (redirects to xiuqo memory setup)", None, ()),
    ...
    ("migrate", "Step-by-step migration guide from openclaw-honcho to Xiuqo Honcho", cmd_migrate, ()),
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":  # honcho setup goes through the unified memory-provider path
        print("\n  Honcho is configured via the memory provider system.\n  Running 'xiuqo memory setup'...\n")
        from xiuqo_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``xiuqo honcho`` argparse subcommand tree on the ``xiuqo honcho`` parser."""
