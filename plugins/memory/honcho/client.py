...
Config file resolution: $XIUQO_HOME/honcho.json -> ~/.honcho/config.json -> env vars
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Xiuqo profile."""
...
def resolve_active_host() -> str:
    """Honcho host key: XIUQO_HONCHO_HOST (profile-scoped .env), else the active profile. The config's
    ...
    explicit = (get_secret("XIUQO_HONCHO_HOST", "") or "").strip()
    if explicit:
        return explicit
    try:
...
def resolve_config_path() -> Path:
    """Active Honcho config path: $XIUQO_HOME/honcho.json -> default profile's honcho.json
...
def _connection_fields(look: _HostLookup, host: str, path: Path) -> dict[str, Any]:
    ...
    # read it before the flat Xiuqo spellings.
...
class HonchoClientConfig:
    ...
    @classmethod
    def from_env(cls, workspace_id: str = "xiuqo", host: str | None = None) -> HonchoClientConfig:
        ...
        return cls(
            ...
            config_path=resolve_config_path(), xiuqo_home=get_hermes_home(),
        )
    ...
    def from_global_config(cls, host: str | None = None, config_path: Path | None = None) -> HonchoClientConfig:
        ...
        derives it from the active Xiuqo profile."""
        resolved_host = host or resolve_active_host()
        ...
        return cls(
            ...
            config_path=path, xiuqo_home=get_hermes_home(),
        )
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError("Honcho API key not found. Get your API key at https://app.honcho.dev, "
                         "then run 'xiuqo honcho setup' or set HONCHO_API_KEY. "
...
def _build_client(config: HonchoClientConfig) -> "Honcho":
    ...
    try:
        ...
        raise ImportError("honcho-ai is required for Honcho integration. Install it with: pip install honcho-ai  "
                          "(or run `xiuqo honcho setup` to configure).")
