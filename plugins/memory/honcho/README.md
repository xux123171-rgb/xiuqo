...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/xiuqo>
...
```bash
xiuqo memory setup honcho   # configure Honcho directly (works on a fresh install)
xiuqo memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
xiuqo config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.xiuqo/.env
```

> `xiuqo honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `xiuqo memory setup honcho`.
...
Set `"recallSync": true` in `$XIUQO_HOME/honcho.json` (at the root or under
`hosts.xiuqo`), or enable **Current-query recall** in the memory settings or
`xiuqo honcho setup`. An explicit host-level `false` overrides root-level `true`.
...
cycle (not per pass). Select a fast, inexpensive model under `xiuqo model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$XIUQO_HOME/honcho.json` | Profile-local (isolated Xiuqo instances) |
| 2 | `~/.xiuqo/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Xiuqo profile: `xiuqo` (default) or `hermes_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `xiuqo honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Xiuqo session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`xiuqo`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`xiuqo`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the user peer name is prepended: `alice-xiuqo`.
...
- **`per-session`** — Xiuqo session ID (timestamp + hex). Every `xiuqo` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Xiuqo profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`xiuqo`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Xiuqo profile: `xiuqo` (default) or `hermes_<profile>` (e.g. `xiuqo -p coder` -> host key `xiuqo_coder`). Older `xiuqo.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `XIUQO_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `xiuqo`) |
...
|---------|-------------|
| `xiuqo memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `xiuqo honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `xiuqo memory setup`) |
| `xiuqo honcho status` | Show resolved config for active profile |
| `xiuqo honcho enable` / `disable` | Toggle Honcho for active profile |
| `xiuqo honcho mode <mode>` | Change recall or observation mode |
| `xiuqo honcho peer --user <name>` | Update user peer name |
| `xiuqo honcho peer --ai <name>` | Update AI peer name |
| `xiuqo honcho tokens --context <N>` | Set context token budget |
| `xiuqo honcho tokens --dialectic <N>` | Set dialectic max chars |
| `xiuqo honcho map <name>` | Map current directory to a session name |
| `xiuqo honcho sync` | Create host blocks for all Xiuqo profiles |
