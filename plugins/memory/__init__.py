...
``$XIUQO_HOME/plugins/<name>/``, project ``./.xiuqo/plugins/<name>/`` (opt-in via
XIUQO_ENABLE_PROJECT_PLUGINS), then ``xiuqo_agent.memory_providers`` entry points.
...
ENTRY_POINTS_GROUP = "xiuqo_agent.memory_providers"
# Per Xiuqo home (plugin managers are per home too): pruning under one multiplexed profile must
...
def _registered_skills_for_active_home() -> dict[str, Path]:
    ...
    return _REGISTERED_MEMORY_PROVIDER_SKILLS.setdefault(xiuqo_home_key(), {})
...
def _get_project_plugins_dir() -> Optional[Path]:
    """``./.xiuqo/plugins/`` or None. Gated on XIUQO_ENABLE_PROJECT_PLUGINS like the
    PluginManager scan: a repo you merely ``cd`` into must not offer a memory backend."""
    try:
        from xiuqo_cli.plugins import _env_enabled

        if not _env_enabled("XIUQO_ENABLE_PROJECT_PLUGINS"):
            return None
        d = Path.cwd() / ".xiuqo" / "plugins"
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    provider silently loses its dashboard panel and ``xiuqo <provider>`` commands."""
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from xiuqo_cli.plugins import resolve_module_origin
...
class _ProviderCollector:
    ...
    def _plugin_context(self):
        ...
        if self._context is None:
            from xiuqo_cli.plugins import PluginContext, PluginManifest, get_plugin_manager
