...
keyless ring — keyless access is opt-in by selecting Tavily in ``xiuqo tools``.
"""
...
def _missing_key_error(action: str) -> str:
    return f"TAVILY_API_KEY is not set. Get a key at https://app.tavily.com/home or select Tavily in `xiuqo tools` for opt-in keyless {action}."
