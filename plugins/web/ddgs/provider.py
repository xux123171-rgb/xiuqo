...
reflects package importability; the plugin registers either way so ``xiuqo tools`` can offer to
...
def _run_ddgs_search_bounded(query: str, safe_limit: int) -> list[dict[str, Any]]:
    ...
    if _test_hook:
        ...
        env["XIUQO_DDGS_ALLOW_TEST_HOOKS"] = "1"
...
class DDGSWebSearchProvider(BaseWebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        tool-registration time and on every ``xiuqo tools`` paint."""
        try:
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        hung native ``primp`` call cannot freeze the Xiuqo process.
