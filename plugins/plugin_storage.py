"""Per-plugin persistent storage: ``<xiuqo home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<xiuqo home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `xiuqo plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<xiuqo home>/plugin-data/<name>/``; resolves ``get_hermes_home()`` on
