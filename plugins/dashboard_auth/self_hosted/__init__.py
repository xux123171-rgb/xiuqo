...
``dashboard.oauth.self_hosted.{issuer,client_id,scopes,client_secret}`` or ``XIUQO_DASHBOARD_OIDC_*``.
"""
...
from xiuqo_cli.dashboard_auth import LoginStart, ProviderError, Session
...
def _settings() -> dict:
    ...
    issuer = setting("XIUQO_DASHBOARD_OIDC_ISSUER", "issuer")
    client_id = setting("XIUQO_DASHBOARD_OIDC_CLIENT_ID", "client_id")
    if not issuer or not client_id:
        raise SkipRegistration(
            "Self-hosted OIDC dashboard auth is not configured. Set both an issuer and "
            "a client_id — either as env vars (XIUQO_DASHBOARD_OIDC_ISSUER + "
            "XIUQO_DASHBOARD_OIDC_CLIENT_ID) or under "
    ...
    return {
        ...
        "scopes": setting("XIUQO_DASHBOARD_OIDC_SCOPES", "scopes") or _DEFAULT_SCOPES,
        # Credential: canonical home is the env var / ~/.xiuqo/.env. Empty ⇒ public client.
        "client_secret": setting("XIUQO_DASHBOARD_OIDC_CLIENT_SECRET", "client_secret")}
...
_PLUGIN_COMPAT_LAZY = {
    'DashboardAuthProvider': ('xiuqo_cli.dashboard_auth', 'DashboardAuthProvider'),
    'InvalidCodeError': ('xiuqo_cli.dashboard_auth', 'InvalidCodeError'),
    'RefreshExpiredError': ('xiuqo_cli.dashboard_auth', 'RefreshExpiredError'),
    'classify_jwks_lookup_error': ('xiuqo_cli.dashboard_auth', 'classify_jwks_lookup_error'),
}
