...
secret (``XIUQO_DASHBOARD_DRAIN_SECRET``, env-only — it is a credential); an inbound bearer
...
from xiuqo_cli.dashboard_auth import DashboardAuthProvider, Session, TokenPrincipal
...
def _settings() -> dict:
    ...
    secret = os.environ.get("XIUQO_DASHBOARD_DRAIN_SECRET", "").strip()
    if not secret:
        raise SkipRegistration(
            "XIUQO_DASHBOARD_DRAIN_SECRET is not set. Set a per-agent >=256-bit secret "
    ...
    if reason is not None:
        raise SkipRegistration(
            f"XIUQO_DASHBOARD_DRAIN_SECRET rejected — {reason}. The drain endpoint stays disabled (fail-closed).",
            level="warning")
...
def register(ctx) -> None:
    ...
    reason) when ``XIUQO_DASHBOARD_DRAIN_SECRET`` is unset or fails the entropy gate. On
    ...
    try:
        from xiuqo_cli.dashboard_auth.token_auth import register_token_route
