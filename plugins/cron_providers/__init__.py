...
``$XIUQO_HOME/plugins/<name>/`` (bundled wins on collision). The built-in InProcessCronScheduler
...
def _user_provider_dirs() -> List[Path]:
    """User-installed ``$XIUQO_HOME/plugins/<name>/`` dirs that look like cron providers."""
