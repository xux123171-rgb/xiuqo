...
models from :func:`xiuqo_cli.models._fetch_deepinfra_models_by_tag`; no ids hardcoded).
...
class DeepInfraImageGenProvider(StaticImageGenProvider):
    ...
    def generate(self, prompt: str, aspect_ratio: str = DEFAULT_ASPECT_RATIO, **kwargs: Any) -> Dict[str, Any]:
        ...
        if not api_key:
            return fail(
                "DEEPINFRA_API_KEY not set. Run `xiuqo tools` → Image "
                "Generation → DeepInfra to configure, or `xiuqo setup` "
                "to add the key.",
                "auth_required")
        ...
        from xiuqo_cli.models import deepinfra_base_url
