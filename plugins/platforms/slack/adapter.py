...
# User-Agent prefix (``XiuqoAgent/<version>``) for platform-partner attribution of API calls.
try:
    from xiuqo_cli import __version__ as _XIUQO_VERSION
except Exception:
    _XIUQO_VERSION = "unknown"
_XIUQO_SLACK_USER_AGENT_PREFIX = f"XiuqoAgent/{_XIUQO_VERSION}"
...
_MODEL_PICKER_PROVIDER_ACTION = "xiuqo_model_provider"
_MODEL_PICKER_MODEL_ACTION = "xiuqo_model_model"
_MODEL_PICKER_BACK_ACTION = "xiuqo_model_back"
_MODEL_PICKER_CANCEL_ACTION = "xiuqo_model_cancel"
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    if not text.startswith("!"):
        return text
    try:
        from xiuqo_cli.commands import is_gateway_known_command
...
def _normalize_slack_text_for_dedupe(text: str, bot_uid: str = "") -> str:
    ...
    # URL); labels after dates (dates carry a label); bot mention after labels (``<@U…|xiuqo>``).
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if team_key not in warned and "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "from `xiuqo slack` produces a manifest with these already included.",
    ...
    def _register_bolt_handlers(self) -> None:
        ...
        # Catch-all no-op ack for any other subscribed event type that Xiuqo has no listener for (e.g.
        ...
        async def handle_unhandled_event(event, body, logger):
            logger.debug(
                ...
                "events not handled by Xiuqo can be removed from the Slack app manifest via "
                "`xiuqo slack manifest`)",
        ...
        # ALSO be declared in the app manifest (`xiuqo slack manifest`): Socket Mode won't
        # deliver undeclared commands at all.
        from xiuqo_cli.commands_platforms import slack_native_slashes
        _slash_names = [name for name, _d, _h in slack_native_slashes()]
        if _slash_names:
            _slash_pattern = re.compile(
                r"^/(?:" + "|".join(re.escape(n) for n in _slash_names) + r")$")
        else:  # pragma: no cover - registry always non-empty
            _slash_pattern = re.compile(r"^/xiuqo$")
        ...
        self._app.action("xiuqo_feedback")(self._handle_feedback_action)
        # Clarify buttons (tools/clarify_gateway.py); indexed action IDs because
        # Block Kit requires unique IDs within an actions block.
        self._app.action(re.compile(r"^xiuqo_clarify_choice_\d+$"))(self._handle_clarify_action)
        self._app.action("xiuqo_clarify_other")(self._handle_clarify_action)
    ...
    def _new_web_client(token: str, proxy_url: Optional[str]) -> Any:
        client = AsyncWebClient(token=token, user_agent_prefix=_XIUQO_SLACK_USER_AGENT_PREFIX)
        _apply_slack_proxy(client, proxy_url)
        return client
    ...
    def _fatal_missing_env(self, env_name: str) -> None:
        ...
        logger.error(
            "[Slack] %s not set — this is a permanent config error; set %s via `xiuqo "
            "gateway setup` or in the active profile's ~/.xiuqo/.env file, then restart the "
            "gateway.", env_name, env_name)
        self._set_fatal_error(
            f"missing_{env_name.lower()}",
            f"{env_name} not configured. Use `xiuqo gateway setup` "
            "or add it to your active profile's ~/.xiuqo/.env file, then restart the gateway.",
            retryable=False)
    ...
    def _hint_allow_bots(self) -> None:
        ...
        if _allow_bots_cfg != "none":
            logger.info(
                ...
                "appropriate (run 'xiuqo slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(self, parent_chat_id: str, name: str) -> Optional[str]:
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: Xiuqo handoff — *{(name or 'session').strip()[:80]}*"
    ...
    async def send_native_task_card_progress(
        self, chat_id: str, tasks: List[Dict[str, str]], *, title: str = "Xiuqo is working",
    ...
    _EA_ACTION_IDS = {"once": "xiuqo_approve_once", "session": "xiuqo_approve_session",
                      "always": "xiuqo_approve_always", "deny": "xiuqo_deny"}
        ...
        def _build() -> Tuple[str, list]:
            ...
            blocks = [
                {"type": "section", "text": {"type": "mrkdwn", "text": f"*{_title}*\n\n{body}"}},
                {
                    "type": "actions",
                    "elements": [
                        self._button("Approve Once", "xiuqo_confirm_once", value, style="primary"),
                        self._button("Always Approve", "xiuqo_confirm_always", value),
                        self._button("Cancel", "xiuqo_confirm_cancel", value, style="danger")]}]
    ...
    async def send_model_picker(
        ...
        try:
            ...
            try:
                from xiuqo_cli.providers import get_label
    ...
    async def send_clarify(
        ...
        """Clarify prompt as Block Kit buttons: one ``xiuqo_clarify_choice_<idx>`` per option
        (value ``clarify_id|idx``) plus "✏️ Other…" (``xiuqo_clarify_other``), which flips the
        ...
        def _build() -> Tuple[str, list]:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append(
                    self._button(
                        label[:75], f"xiuqo_clarify_choice_{idx}",
                        f"{clarify_id}|{idx}", emoji=True))
            elements.append(
                self._button("✏️ Other…", "xiuqo_clarify_other", f"{clarify_id}|other", emoji=True)
            )
    ...
    _APPROVAL_CHOICES: ClassVar[Dict[str, str]] = {
        "xiuqo_approve_once": "once", "xiuqo_approve_session": "session",
        "xiuqo_approve_always": "always", "xiuqo_deny": "deny"}
    ...
    _CONFIRM_CHOICES: ClassVar[Dict[str, str]] = {
        "xiuqo_confirm_once": "once", "xiuqo_confirm_always": "always",
        "xiuqo_confirm_cancel": "cancel"}
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "xiuqo_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        ``/xiuqo <subcommand> [args]``; other text after ``/xiuqo`` is a regular message."""
        ...
        # events only: free-form "/xiuqo <question>" replies must stay public.
    ...
    def _slash_command_text(command: dict) -> str:
        ...
        payload verbatim (internal/trailing spacing). ``/xiuqo`` (or a missing ``command``) maps
...
# in ``xiuqo_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in ``tools/send_message_tool.py``).
# ──────────────────────────────────────────────────────────────────────────
...
_SETUP_HOME_CHANNEL_HELP = (
    "📬 Home Channel: where Xiuqo delivers cron job results,",
...
def _write_slack_manifest_and_instruct() -> None:
    """Write the manifest under XIUQO_HOME and print paste instructions; non-fatal."""
    from xiuqo_cli.cli_output import print_info, print_success, print_warning
    try:
        from xiuqo_cli.slack_cli import _build_full_manifest
        from xiuqo_constants import get_hermes_home
        manifest = _build_full_manifest(
            bot_name="Xiuqo", bot_description="Your Xiuqo agent on Slack")
        ...
        print_info(
            "   Re-run `xiuqo slack manifest --write` anytime to refresh after "
            "Xiuqo adds new commands.")
    except Exception as e:
...
def interactive_setup() -> None:
    ...
    from xiuqo_cli.config import remove_env_value, save_env_value
    from xiuqo_cli.cli_output import (
        prompt, prompt_yes_no, print_header, print_info, print_success, print_warning)
    from xiuqo_cli.setup_platforms import declines_reconfigure
    ...
    if declines_reconfigure("Slack", "Reconfigure Slack?", "SLACK_BOT_TOKEN"):
        ...
        if prompt_yes_no(
            ...
            "list? (recommended after `xiuqo update`)", True):
...
def register(ctx) -> None:
    """Plugin entry point — called by the Xiuqo plugin system."""
    ctx.register_platform(
        ...
        install_hint="Run `xiuqo setup` to install Slack support.",
