...
_NATIVE_SLASH_COMMANDS: tuple = (
    ...
    ("reset", "Reset your Xiuqo session", (), "/reset", "Session reset~"),
    ...
    ("status", "Show Xiuqo session status", (), "/status", "Status sent~"),
    ("sethome", "Set this chat as the home channel", (), "/sethome", None),
    ("stop", "Stop the running Xiuqo agent", (), "/stop", "Stop requested~"),
    ...
    ("reload-skills", "Re-scan ~/.xiuqo/skills/ for new or removed skills", (), "/reload-skills", None),
    ("voice", "Toggle voice reply mode",
     ...
     "/voice {mode}", None),
    ("update", "Update Xiuqo Agent to the latest version", (), "/update", "Update initiated~"),
    ("restart", "Gracefully restart the Xiuqo gateway", (), "/restart", "Restart requested~"),
    ...
    ("thread", "Create a new thread and start a Xiuqo session in it", (), None, None),
...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("XIUQO_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning("Ignoring invalid XIUQO_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r", raw)
    return 30.0
...
class DiscordAdapter(DiscordMediaMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("XIUQO_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("XIUQO_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            env_key="XIUQO_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold", 2,
            env_key="XIUQO_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("XIUQO_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs Xiuqo work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit Xiuqo/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:xiuqo|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Xiuqo manages."""
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "xiuqo_voice", f"ack_{_uuid.uuid4().hex[:12]}.mp3",
        )
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Xiuqo pairing grant."""
    ...
    def _register_thread_slash(self, tree, name: str, description: str) -> None:
        ...
        @discord.app_commands.describe(
            name="Thread name", message="Optional first message to send to Xiuqo in the thread",
    ...
    def _register_slash_commands(self) -> None:
        ...
        def _auto_register(name: str, description: str, args_hint: str) -> None:
            ...
            try:
                ...
                pass
        try:
            from xiuqo_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
            try:
        ...
        try:
            from xiuqo_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill", description="Run a Xiuqo skill", callback=_skill_handler,
            )
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        from xiuqo_cli.commands_platforms import discord_skill_commands_by_category
    ...
    async def _create_thread(
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Xiuqo: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Real semantic naming is done after the first agent turn, when Xiuqo has an LLM-generated session
        ...
        thread_name = content[:80] if content else "Xiuqo"
        if len(content) > 80:
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Xiuqo: **{thread_name}**"
                    )
    ...
    async def rename_thread(
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Xiuqo semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(self, parent_chat_id: str, name: str) -> Optional[str]:
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Xiuqo handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
    ...
    _EA_HEADER = ("⚠️ **Command Approval Required**\n\n"
                  "Do you want Xiuqo to run this command?\n\n"
                  "**Requested command:**\n")
    _EA_CODE_OPEN = "```bash\n"
    _EA_CODE_CLOSE = "\n```\n"
    ...
    async def send_clarify(
        ...
        def _build(_channel):
            ...
            content = self._self_contained_prompt_content(
                "❓ **Xiuqo needs your input**", str(question or "").strip(), tail=f"\n\n{hint}",
            )
    ...
    async def send_update_prompt(
        ...
        """Yes/No prompt for the gateway ``/update`` watcher when ``xiuqo update --gateway`` needs input."""
        def _build(_channel):
                ...
                if thread:
                    ...
                    try:
                        ...
                        await message.channel.send(
                            "⚠️ Xiuqo could not create a Discord thread for "
...
def _define_discord_view_classes() -> None:
    ...
    class _HermesView(discord.ui.View):
        """Shared plumbing for Xiuqo component views: allowlist auth, single-use
    ...
    class UpdatePromptView(_HermesView):
        """Yes/No buttons for ``xiuqo update`` prompts; the answer is written to
...
# process (e.g. standalone ``xiuqo cron``); same forum/thread/multipart logic via Discord REST.
...
def _prompt_discord_bot_token(prompt) -> str:
    ...
    from xiuqo_cli.cli_output import print_error
    token = ""
...
def interactive_setup() -> None:
    ...
    from xiuqo_cli.config import get_env_value, remove_env_value, save_env_value
...
_YAML_WEBSOCKET_LIVENESS_KEYS = (
    ("websocket_liveness_interval_seconds", "liveness_interval_seconds", "XIUQO_DISCORD_LIVENESS_INTERVAL_SECONDS"),
    ("websocket_liveness_failure_threshold", "liveness_failure_threshold", "XIUQO_DISCORD_LIVENESS_FAILURE_THRESHOLD"),
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `xiuqo setup` to install Discord support.",
