...
from xiuqo_constants import (find_node_executable, get_hermes_dir, with_hermes_node_path)
...
def _is_allowed_bridge_path(url: str) -> bool:
    """Absolute bridge path resolves (symlinks included) inside a Xiuqo cache dir — a rogue bridge could hand back /etc/passwd."""
    try:
...
def check_whatsapp_requirements() -> bool:
    """Node.js (Xiuqo-managed first, so a bad system Node on PATH can't break Windows) is available."""
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def _ensure_bridge_deps(self, bridge_dir: Path) -> bool:
        ...
        _dep_stamp = bridge_dir / "node_modules" / ".xiuqo-pkg-hash"  # holds the package.json hash of the last install
        ...
        # Xiuqo-managed portable Node's npm.cmd first (Windows), then PATH.
        ...
        self._set_fatal_error("whatsapp_npm_install_failed", f"WhatsApp bridge npm install failed{detail}. Run `cd {bridge_dir} && {_npm_bin} install` "
                              "manually, then restart `xiuqo gateway`.", retryable=False)
        return False
    ...
    def _bridge_env(self) -> dict:
        ...
        for _key, _v in [("WHATSAPP_MODE", _wenv("WHATSAPP_MODE", "self-chat"))] + [(k, _wenv(k)) for k in _BRIDGE_PASSTHROUGH_ENV]:
            if _v:
                bridge_env[_key] = _v
        # Without these the bridge hardcodes ~/.xiuqo/{image,audio,document}_cache (wrong under XIUQO_HOME/profiles/cache layout).
        img_dir, audio_dir, _video_dir, doc_dir = _cache_dirs()
        bridge_env.update(XIUQO_IMAGE_CACHE_DIR=str(img_dir), XIUQO_AUDIO_CACHE_DIR=str(audio_dir), XIUQO_DOCUMENT_CACHE_DIR=str(doc_dir))
        return bridge_env
    ...
    async def _wait_for_bridge(self) -> bool:
        ...
        if data.get("status") != "connected":
            ...
            if connected is None:
                ...
                print(f"[{self.name}]   If session expired, re-pair: xiuqo whatsapp")
        return True
    ...
    def _preflight(self) -> bool:
        ...
        checks = (
            (check_whatsapp_requirements, ("[%s] Node.js not found. WhatsApp requires Node.js.", self.name),
             "whatsapp_node_missing", "Node.js is not installed — install Node.js and re-run `xiuqo gateway`."),
            ...
            (creds_path.exists, ("[%s] WhatsApp is enabled but not paired (no creds.json at %s). Pair from the dashboard or run "
                                 "`xiuqo whatsapp`; remove WHATSAPP_ENABLED from your .env to disable.", self.name, creds_path),
             "whatsapp_not_paired", "WhatsApp enabled but not paired — pair from the dashboard or run `xiuqo whatsapp`."),
        )
...
# ── Plugin glue: register(ctx) plus the hooks for gateway/run.py, gateway/config.py, xiuqo_cli/gateway.py, send_message_tool.py.
...
def interactive_setup() -> None:
    ...
    from xiuqo_cli.cli_output import prompt, prompt_yes_no, print_header, print_info, print_success
...
def _is_connected(config) -> bool:
    ...
    if config is not None and getattr(config, "enabled", False) and (getattr(config, "extra", {}) or {}):
        return True
    # Via xiuqo_cli.gateway.get_env_value (not os.getenv) so setup-status callers that patch it observe the same value.
