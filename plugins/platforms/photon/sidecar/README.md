...
Small Node helper that bridges Xiuqo Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Xiuqo is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Xiuqo-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Xiuqo can receive inbound messages
...
The Xiuqo plugin's `xiuqo photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Xiuqo is Python, so replies go through this sidecar
