...
// `message.content.text` before Xiuqo can see it. We rewrite the two inbound
...
const MARKER = "Xiuqo patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Xiuqo to patch; keep the legacy v8 path below for older pinned installs.
    if (
