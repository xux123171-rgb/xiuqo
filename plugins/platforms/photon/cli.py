"""``xiuqo photon ...`` CLI subcommands (registered via ``ctx.register_cli_command()``):
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `xiuqo photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None, help="Project name (default: 'Xiuqo Agent')")
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    try:
        from xiuqo_cli.config import write_platform_config_field
    ...
    print("\n✓ Photon setup complete.\n  Start the gateway:  xiuqo gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_deps_installed() else '✗ run `xiuqo photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`xiuqo photon telemetry on|off`)")
    return 0


def _telemetry_enabled() -> bool:
    """PHOTON_TELEMETRY from env / ~/.xiuqo/.env; truthy set mirrors the sidecar's."""
    try:
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `xiuqo photon telemetry on` / `xiuqo photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.xiuqo/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  xiuqo gateway restart")
    return 0
...
def gateway_setup() -> None:
    """Run Photon first-time setup from the unified `xiuqo gateway setup` wizard (same flow
    as ``xiuqo photon setup``; phone is prompted when stdin is a TTY)."""
