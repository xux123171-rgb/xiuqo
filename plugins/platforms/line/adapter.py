...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, XIUQO_HOME).
        ...
        try:
            ...
            xiuqo_home = Path(get_hermes_home()).resolve()
        except Exception:
            xiuqo_home = Path.home().joinpath(".xiuqo").resolve()
        resolved = path.resolve()
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), xiuqo_home)):
...
def is_connected(config) -> bool:
    """Surface in ``xiuqo status`` even before the adapter is instantiated."""
    return validate_config(config)


def _env_enablement() -> Optional[Dict[str, Any]]:
    """``env_enablement_fn``: seed ``PlatformConfig.extra`` from env-only setups so ``xiuqo status`` sees them."""
    if not _env_credentials_present():
        return None
...
def interactive_setup() -> None:
    """``xiuqo setup line`` wizard (writes ``~/.xiuqo/.env``); CLI helpers are lazy-imported."""
    from xiuqo_cli.config import get_env_value, save_env_value
    from xiuqo_cli.cli_output import print_header, print_info, prompt
