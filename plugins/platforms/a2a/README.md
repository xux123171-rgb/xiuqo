# A2A — Agent-to-Agent protocol for Xiuqo
...
peer (another Xiuqo, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
xiuqo gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Xiuqo serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.xiuqo/a2a_audit.jsonl`.
- Conversations persist to `~/.xiuqo/a2a_conversations/` — they survive context
