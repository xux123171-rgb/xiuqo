...
legacy single-user mode). Files under ``${XIUQO_HOME}``: ``google_chat_user_tokens/
...
from xiuqo_constants import display_hermes_home, get_hermes_home
...
# human-readable under ``ls ~/.xiuqo/google_chat_user_tokens/``.
...
def _token_rel(email: Optional[str]) -> str:
    """XIUQO_HOME-relative token file: per-user under the tokens dir, else the legacy path."""
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `xiuqo setup` to install Google Chat support."
        )
        return None
    try:
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("Run `xiuqo setup` to repair the managed installation, then retry.")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into XIUQO_HOME."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Xiuqo (native attachment delivery)"
    )
