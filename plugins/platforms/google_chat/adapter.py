...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        from xiuqo_constants import get_hermes_home as _get_hermes_home
    ...
    async def _build_message_event(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> Optional[MessageEvent]:
        """Parse a Chat API message into a xiuqo MessageEvent."""
    ...
    async def send_clarify(
        ...
        def _button(text: str, choice: str) -> Dict[str, Any]:
            return {"text": text, "action": "xiuqo_clarify", "parameters": {"clarify_id": clarify_id, "choice": choice}}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Xiuqo is thinking…' marker (Chat has no typing API); ``send()``
        ...
        body = _thread_body(getattr(self.config, "typing_status_text", None) or "Xiuqo is thinking…", thread_id)
...
Walkthrough:
  ...
  3. Create a Pub/Sub topic (e.g. xiuqo-chat-events) and a Pull subscription.
...
def interactive_setup() -> None:
    """``xiuqo setup`` wizard: print GCP instructions, prompt for env vars, persist to ``~/.xiuqo/.env``."""
    from xiuqo_cli.cli_output import print_info, print_success, print_warning, prompt, prompt_yes_no
    ...
    print()
    print_success("Google Chat configuration saved to ~/.xiuqo/.env")
    print_info("Restart the gateway: xiuqo gateway restart")
...
async def _standalone_send(
    ...
    gateway runner is not in-process, e.g. ``xiuqo cron``). Needs SA credentials and a
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `xiuqo setup` to install Google Chat support.",
        ...
        platform_hint=(
            ...
            "confirmations instead. While you are generating a response, a 'Xiuqo is thinking…' marker message "
