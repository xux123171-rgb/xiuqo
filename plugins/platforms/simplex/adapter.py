...
XIUQO_SIMPLEX_TEXT_BATCH_DELAY (quiet seconds, default 0.8, merging rapid-fire inbound text).
...
_CORR_PREFIX = "xiuqo-"  # marks requests we sent so our own echoes can be ignored
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay_seconds = float(os.getenv("XIUQO_SIMPLEX_TEXT_BATCH_DELAY", "0.8"))
...
async def _standalone_send(
    ...
    this process (``xiuqo cron``). ``thread_id``/``force_document`` are signature parity only;
...
def interactive_setup() -> None:
    """``xiuqo setup gateway`` → SimpleX wizard (writes ``~/.xiuqo/.env``); CLI helpers are lazy-imported."""
