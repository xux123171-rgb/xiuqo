...
``<xiuqo-venv>/bin/pip install microsoft-teams-apps aiohttp``) and credentials via env
...
async def _standalone_send(
    ...
    (``xiuqo cron``). ``TEAMS_SERVICE_URL`` is allowlisted and ``chat_id`` charset-checked
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    cwd-discovered ``.env`` at import, mutating process-global ``os.environ``. Xiuqo owns dotenv loading.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Xiuqo"}))
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        headers = {"User-Agent": "Mozilla/5.0 (compatible; XiuqoAgent/1.0)"}
    ...
    async def _on_card_action(
        ...
        xiuqo_action = data.get("xiuqo_action", "")
        session_key = data.get("session_key", "")
        if not xiuqo_action or not session_key:
        ...
        choice = _APPROVAL_CHOICES.get(xiuqo_action)
        if not choice:
            return self._invoke_message("Unknown action.")
    ...
    async def _send_exec_approval_prompt(self, prompt: ExecApprovalPrompt) -> SendResult:
        ...
        for label, choice, style in prompt.actions:
            ...
            actions.append(ExecuteAction(
                title=label, verb="xiuqo_approve",
                data={**btn_data_base, "xiuqo_action": self._EA_CARD_ACTIONS[choice]}, **kw))
...
_SETUP_INTRO = (  # "" → blank line
    ...
    '  teams app create --name "Xiuqo" --endpoint "https://<tunnel>/api/messages"', "",
...
def interactive_setup() -> None:
    ...
    from xiuqo_cli.cli_output import prompt, prompt_yes_no, print_info, print_success, print_warning
    ...
    print()
    print_success("Teams configuration saved to ~/.xiuqo/.env")
...
def _install_hint() -> str:
    ...
    ``venv_pip=True`` targets the real Xiuqo venv, sidestepping PEP 668 on Ubuntu 24.04."""
    try:
