...
(enabled/policy/allow_from) falling back independently. Config ~/.xiuqo/feishu_comment_rules.json (mtime-cached,
hot-reload); pairing store ~/.xiuqo/feishu_comment_pairing.json."""
...
def _rules_file() -> Path:
    ...
    it, else live profile-scoped XIUQO_HOME — the multiplexed gateway serves every profile from
...
def _main() -> int:
    try:
        __import__("xiuqo_cli.env_loader", fromlist=["load_hermes_dotenv"]).load_hermes_dotenv()
    except Exception:
        pass
