...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    async def send(self, chat_id: str, content: str, reply_to: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> SendResult:
        ...
        payload = {"msgtype": "markdown", "markdown": {"title": "Xiuqo", "text": self._normalize_markdown(content[: self.MAX_MESSAGE_LENGTH])}}
        try:
...
def interactive_setup() -> None:
    ...
    from xiuqo_cli.cli_output import prompt, print_header, print_success, print_warning
    ...
    if prompt_choice("Choose setup method", choices, default=0) == 0:
        try:
            from xiuqo_cli.dingtalk_auth import dingtalk_qr_auth
