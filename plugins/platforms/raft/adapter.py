...
def _make_activity_event(*, hook_event_name: str, session_id: Any, status: str = "ok", tool_name: Any = None,
    ...
    event: Dict[str, Any] = {"schema": ACTIVITY_EVENT_SCHEMA, "eventId": f"xiuqo-{uuid.uuid4()}",
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # No payload["schema"] gate: the bridge owns schema evolution; Xiuqo only checks content-free.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Accept Raft wake hints without interrupting an active Xiuqo turn."""
        if event.internal:
...
def interactive_setup() -> None:
    """``xiuqo gateway setup`` flow: persists ``RAFT_PROFILE`` to the Xiuqo env file.
    CLI helpers are lazy-imported so the plugin stays importable in gateway runtime and tests."""
    from xiuqo_cli.cli_output import print_header, print_info, print_success, print_warning, prompt
    ...
    if declines_reconfigure("Raft", "Reconfigure Raft?", "RAFT_PROFILE"):
        ...
        return
    for line in ("Connect Xiuqo to Raft as an external agent.", "Create the External Agent in Raft first, then run:",
