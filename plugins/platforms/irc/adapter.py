"""IRC Platform Adapter for Xiuqo Agent — stdlib asyncio only, zero external dependencies.

config.yaml ``gateway.platforms.irc.extra`` keys: server, port (6697), nickname (xiuqo-bot), channel,
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = _env_or_extra(extra, "IRC_NICKNAME", "nickname", "xiuqo-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Xiuqo Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            with contextlib.suppress(Exception):
                await self._send_raw("QUIT :Xiuqo Agent shutting down")
...
def interactive_setup() -> None:
    """`xiuqo gateway setup` flow (lazy xiuqo_cli imports keep the plugin importable outside the CLI)."""
    ...
    if declines_reconfigure("IRC", "Reconfigure IRC?", "IRC_SERVER"):
        return
    info("Connect Xiuqo to an IRC network. Uses Python stdlib — no extra packages needed.",
    ...
    for label, env, what in (("Bot nickname (e.g. xiuqo-bot)", "IRC_NICKNAME", "Nickname"),
                             ("Channel to join (e.g. #xiuqo — comma-separate for multiple)", "IRC_CHANNEL", "Channel")):
    ...
    print()
    print_success("IRC configuration saved to ~/.xiuqo/.env")
    print_info("Restart the gateway for changes to take effect: xiuqo gateway restart")
...
async def _sa_register(conn: _StandaloneConn, nick_base: str, server_password: str) -> Optional[Dict[str, Any]]:
    ...
    await conn.raw(f"NICK {standalone_nick}")
    await conn.raw(f"USER {standalone_nick} 0 * :Xiuqo Agent (cron)")
...
async def _standalone_send(pconfig, chat_id: str, message: str, *, thread_id: Optional[str] = None,
    ...
    nick_base = _env_or_extra(extra, "IRC_NICKNAME", "nickname", "xiuqo-bot").rstrip("_0123456789-")[:24] or "xiuqo-bot"
