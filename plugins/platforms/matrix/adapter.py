...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Xiuqo ``/command`` text."""
...
from xiuqo_constants import get_hermes_dir as _get_hermes_dir
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    typed_command_prefix = "!"  # clients reserve typed "/" for local commands; "!command" always reaches Xiuqo
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(os.getenv("XIUQO_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("XIUQO_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
        ...
        if self._access_token:
            ...
            try:
                resp = await client.login(
                    identifier=self._user_id, password=self._password, device_name="Xiuqo Agent",
    ...
    async def _connect_setup_e2ee(self, client: Any, api: Any, state_store: Any) -> bool:
        ...
        try:
            ...
            _acct_id = self._user_id or "xiuqo"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        words, or "Xiuqo Agent" would become "Agent"."""
        if not body:
            return ""
        if self._user_id:
...
def interactive_setup() -> None:
    ...
    from xiuqo_cli.cli_output import prompt, prompt_yes_no, print_header, print_info, print_success, print_warning
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        for line in ("📬 Home Room: where Xiuqo delivers cron job results and notifications.",
...
def _is_connected(config) -> bool:
    """Connected = homeserver + token (or password). Reads via xiuqo_cli.gateway.get_env_value so
