...
# the heartbeat loop to trigger its own recovery path. Refs: xux123171-rgb/xiuqo#59614
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("XIUQO_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "XIUQO_TELEGRAM_TEXT_BATCH_DELAY_SECONDS", 0.3, min_value=0.08, max_value=2.0)
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "XIUQO_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS", 1.0, min_value=self._text_batch_delay_seconds, max_value=4.0)
    ...
    def _dm_topic_fallback(metadata: Optional[Dict[str, Any]]) -> bool:
        """True for Xiuqo private-chat topic lanes (``telegram_dm_topic_reply_fallback``)."""
    ...
    def _thread_kwargs_for_send(
        ...
        metadata; Xiuqo private-chat topic lanes are marked ``telegram_dm_topic_reply_fallback``. Anchor-less synthetic sends
        prefer the Xiuqo topic's ``message_thread_id`` (the native DM-topic id renders in a different chat lane).
        ...
        restart) prefer the Xiuqo topic's ``message_thread_id`` so they stay in the active topic lane
        (#87051); ``direct_messages_topic_id`` is only used when no topic thread resolves, since the native
        DM-topic id does not match the Xiuqo topic lane and can render the message in a different chat
        lane.
        """
        ...
        if fallback and reply_to_mode != "off":
            ...
            if reply_to_message_id is None:
                # Anchor-less synthetic send: prefer the Xiuqo topic thread id (see docstring).
                # Anchor-less synthetic sends (loop wakeups, watch notifications, restart-resumed
                # follow-ups) must stay in the active topic lane: prefer the Xiuqo topic thread id when it
    ...
    def _prune_stale_dm_topic_binding(self, chat_id: Any, thread_id: Any, *, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        Rows are namespaced by profile: the send's ``xiuqo_profile`` wins over the adapter's stamp.
        ...
        binding, so the send's ``xiuqo_profile`` metadata wins over the adapter's own profile stamp;
        ...
        if db is None or not hasattr(db, "delete_telegram_topic_binding"):
            return
        try:
            profile_name = (metadata or {}).get("xiuqo_profile") or getattr(self, "_hermes_profile_name", None) or "default"
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        message = (
            ...
            "or another process is using the same bot token. To recover: ensure no other Xiuqo or OpenClaw instance is running "
            "with this token, then restart the gateway with 'xiuqo gateway restart'."
    ...
    async def _register_command_menu(self) -> None:
        ...
        from xiuqo_cli.commands_platforms import telegram_menu_commands, telegram_menu_max_commands
        if not self._bot:
            return
    ...
    async def _build_ptb_requests(self) -> tuple:
        ...
        request_kwargs = {
            "connection_pool_size": env_int("XIUQO_TELEGRAM_HTTP_POOL_SIZE", 512),
            "pool_timeout": env_float("XIUQO_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
            "connect_timeout": env_float("XIUQO_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
            "read_timeout": env_float("XIUQO_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
            "write_timeout": env_float("XIUQO_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
        ...
        disable_fallback = os.getenv("XIUQO_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"}
        fallback_ips = [] if disable_fallback else self._fallback_ips()
        if not fallback_ips and not disable_fallback:
            discovery_timeout = self._env_float_clamped("XIUQO_TELEGRAM_FALLBACK_DISCOVERY_TIMEOUT", 5.0, min_value=0.0)
    ...
    async def _initialize_app_with_retries(self, builder) -> None:
        ...
        _max_connect = 8
        _init_timeout = env_float("XIUQO_TELEGRAM_INIT_TIMEOUT", 30.0)  # per attempt
            ...
            try:
                if asyncio.get_running_loop().time() >= _total_deadline:
                    raise OSError(
                        ...
                        f"Check network connectivity to api.telegram.org or set XIUQO_TELEGRAM_HTTP_CONNECT_TIMEOUT / "
                        f"XIUQO_TELEGRAM_INIT_TIMEOUT to a lower value.")
                ...
                if _attempt >= _max_connect - 1:
                    raise OSError(
                        ...
                        f"or set XIUQO_TELEGRAM_HTTP_CONNECT_TIMEOUT to a lower value.")
    ...
    async def _start_webhook_mode(self, webhook_url: str, *, is_reconnect: bool) -> None:
        ...
        if not webhook_secret:
            raise RuntimeError(
                ...
                "https://github.com/xux123171-rgb/xiuqo/security/advisories/GHSA-3vpc-7q5r-276h.\n\n"
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        try:
            from xiuqo_cli.models_catalog_static import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(self, query, data: str, chat_id: str) -> None:
        ...
        if data.startswith("mp:"):  # provider selected: show model buttons (page 0)
            ...
            try:
                from xiuqo_cli.model_selection_guards import combined_selection_warning
            ...
            group_id = data[4:]
            try:
                from xiuqo_cli.models_catalog_static import PROVIDER_GROUPS
    ...
    # `gt:<verb>` -> (script in ~/.xiuqo/scripts/gmail-triage/, extra-args, success-label, is_state). The callback
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # Server-side MessageEntity values are authoritative: raw substrings like "foo@xiuqo_bot.example"
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # counted as incoming unread in the Xiuqo inbox (#52363). Otherwise a BotFather rename leaves the
    ...
    def _reload_dm_topics_from_config(self) -> None:
        ...
        try:
            from xiuqo_cli.config import load_config_readonly  # canonical loader: managed overlay + ${VAR}
...
# _PLATFORMS["telegram"] static dict in xiuqo_cli/{setup,gateway}.py, and the _send_telegram dispatch in
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("XIUQO_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
...
def interactive_setup() -> None:
    ...
    from xiuqo_cli import setup as _setup_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `xiuqo setup` to install Telegram support.", setup_fn=interactive_setup, apply_yaml_config_fn=_apply_yaml_config,
