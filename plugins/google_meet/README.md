...
Let the xiuqo agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where xiuqo runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `xiuqo meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `xiuqo meet` CLI |
...
| `cli.py` | `xiuqo meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$XIUQO_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `xiuqo meet node {run,list,approve,remove,status,ping}` |
...
xiuqo meet install                                      # pip + Chromium
xiuqo meet setup                                        # preflight
xiuqo meet auth                                         # optional
xiuqo meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
xiuqo meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.xiuqo/.env
xiuqo meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
xiuqo meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
xiuqo meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.xiuqo/.env
xiuqo meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, xiuqo will **not** switch your system audio input automatically — the
...
xiuqo plugins enable google_meet
xiuqo meet node run --display-name my-mac --host 0.0.0.0 --port 18789
