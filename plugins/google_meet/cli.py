"""CLI commands for the google_meet plugin (``xiuqo meet <subcommand>``).
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``xiuqo meet`` argparse tree (called at plugin load time)."""
    ...
    p["join"].add_argument("--guest-name", default="Xiuqo Agent")
...
_DISPATCH = {
    ...
    "stop": lambda a: _print_result(pm.stop(reason="xiuqo meet stop")),
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if not sub:
        print("usage: xiuqo meet {setup,auth,join,status,transcript,say,stop,node}")
        return 2
...
def _cmd_setup() -> int:
    ...
    print("  google auth    : " + (f"ok ({auth_path})" if auth_path.is_file() else "not saved — run: xiuqo meet auth"))
    print()
    all_ok = system_ok and pw_ok and chromium_ok
    print("ready. Join a meeting:  xiuqo meet join https://meet.google.com/abc-defg-hij" if all_ok
    ...
    if not realtime:
        ...
        if system == "Linux":
            ...
            print("\n  NOTE: macOS does not auto-route audio. Open\n    System Settings → Sound → "
                  ...
                  "xiuqo will not switch your default input for you.")
    print("\ndone. verify with: xiuqo meet setup")
    return 0
...
def _cmd_auth() -> int:
    ...
    try:
        ...
        return 1
    print("saved. you can now run: xiuqo meet join <url>")
    return 0
...
if __name__ == "__main__":  # pragma: no cover
    parser = argparse.ArgumentParser(prog="xiuqo meet")
    register_cli(parser)
