...
```bash
xiuqo plugins enable google_meet
xiuqo meet install                 # pip deps + Chromium (transcribe only)
xiuqo meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
xiuqo meet auth                    # optional; skips guest-lobby wait
xiuqo meet setup                   # preflight checks
```

`xiuqo meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or XIUQO_MEET_REALTIME_KEY in ~/.xiuqo/.env
```
...
pip install playwright websockets && python -m playwright install chromium
xiuqo plugins enable google_meet
xiuqo meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
xiuqo meet node approve my-mac ws://<mac-ip>:18789 <token>
xiuqo meet node ping my-mac                   # confirm reachable
```

Run `xiuqo meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Xiuqo agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `xiuqo meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `XIUQO_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `xiuqo_meet_src`). |
...
Local:
```
$XIUQO_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$XIUQO_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `xiuqo meet node approve`.
