"""`xiuqo meet node ...` subcommand tree, wired under the ``xiuqo meet`` parser."""
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    run.add_argument("--display-name", default="xiuqo-meet-node")
...
def _cmd_run(args: argparse.Namespace, reg: NodeRegistry) -> int:
    ...
    print(f"[meet-node] display_name={server.display_name}\n"
          ...
          f"             xiuqo meet node approve <name> ws://<host>:{args.port} {token}")
    try:
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``xiuqo meet node ...``; returns a process exit code."""
