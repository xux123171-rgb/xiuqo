...
One active meeting at a time, recorded in ``$XIUQO_HOME/workspace/meetings/.active.json``
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "Xiuqo Agent", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "XIUQO_MEET_URL": url, "XIUQO_MEET_OUT_DIR": str(out),
           "XIUQO_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "XIUQO_MEET_HEADED"),
        (auth_state, "XIUQO_MEET_AUTH_STATE"),
        (duration, "XIUQO_MEET_DURATION"),
        (mode, "XIUQO_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "XIUQO_MEET_REALTIME_MODEL"),
        (realtime_voice, "XIUQO_MEET_REALTIME_VOICE"),
        (realtime_instructions, "XIUQO_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("XIUQO_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
        env["XIUQO_MEET_REALTIME_KEY"] = realtime_api_key
