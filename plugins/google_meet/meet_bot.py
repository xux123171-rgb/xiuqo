...
Standalone subprocess spawned by ``process_manager.py``; configured via ``XIUQO_MEET_*`` env,
status + transcript written under ``$XIUQO_MEET_OUT_DIR`` (filesystem is the only IPC).
...
Debug: ``XIUQO_MEET_URL=... XIUQO_MEET_OUT_DIR=/tmp/x XIUQO_MEET_HEADED=1 \\
...
def _start_pcm_pump(rt: dict, bridge_info: dict, pcm_path: Path, state: "_BotState") -> None:
    ...
    if platform_tag == "linux":
        cmd = ["paplay", "--raw", "--rate=24000", "--format=s16le", "--channels=1",
               f"--device={target or 'xiuqo_meet_sink'}", str(pcm_path)]
...
def _setup_realtime(rt: dict, api_key: str, state: _BotState) -> None:
    ...
    if not api_key:
        state.set(error="realtime mode requested but no API key in XIUQO_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
        rt["enabled"] = False
        return
    try:
...
_BotConfig = SimpleNamespace  # everything the bot reads from ``XIUQO_MEET_*`` env vars


def _config_from_env() -> _BotConfig:
    env = os.environ.get
    out_raw = env("XIUQO_MEET_OUT_DIR", "").strip()
    return _BotConfig(
        url=env("XIUQO_MEET_URL", "").strip(),
        out_dir=Path(out_raw) if out_raw else None,
        headed=env("XIUQO_MEET_HEADED", "").lower() in {"1", "true", "yes"},
        auth_state=env("XIUQO_MEET_AUTH_STATE", "").strip(),
        guest_name=env("XIUQO_MEET_GUEST_NAME", "Xiuqo Agent"),
        duration_s=_parse_duration(env("XIUQO_MEET_DURATION", "")),
        realtime=env("XIUQO_MEET_MODE", "transcribe").strip().lower() == "realtime",
        # XIUQO_MEET_REALTIME_KEY is resolved by process_manager.start() via the parent's
        # profile secret scope; OPENAI_API_KEY only serves standalone `python -m` runs.
        realtime_api_key=env("XIUQO_MEET_REALTIME_KEY") or env("OPENAI_API_KEY", ""),
        realtime_model=env("XIUQO_MEET_REALTIME_MODEL", "gpt-realtime"),
        realtime_voice=env("XIUQO_MEET_REALTIME_VOICE", "alloy"),
        realtime_instructions=env("XIUQO_MEET_REALTIME_INSTRUCTIONS", ""),
        lobby_timeout=float(env("XIUQO_MEET_LOBBY_TIMEOUT", "300")))
...
def run_bot() -> int:
    ...
    if not _is_safe_meet_url(cfg.url):
        sys.stderr.write("google_meet bot: refusing to launch — XIUQO_MEET_URL must be a "
                         "meet.google.com URL. got: %r\n" % cfg.url)
        return 2
    if cfg.out_dir is None:
        sys.stderr.write("google_meet bot: XIUQO_MEET_OUT_DIR is required\n")
        return 2
