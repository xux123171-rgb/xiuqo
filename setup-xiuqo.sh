...
# 5. Symlinks the 'xiuqo' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}☤ Xiuqo Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/xiuqo-uv-install.$$.log")"
...
if is_termux; then
    # Xiuqo currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Xiuqo requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink xiuqo into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up xiuqo command..."

XIUQO_BIN="$SCRIPT_DIR/venv/bin/xiuqo"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$XIUQO_BIN" "$COMMAND_LINK_DIR/xiuqo"
echo -e "${GREEN}✓${NC} Symlinked xiuqo → $COMMAND_LINK_DISPLAY_DIR/xiuqo"
...
# ============================================================================
# Seed bundled skills into ~/.xiuqo/skills/
# ============================================================================

XIUQO_SKILLS_DIR="${XIUQO_HOME:-$HOME/.xiuqo}/skills"
mkdir -p "$XIUQO_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.xiuqo/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$XIUQO_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  xiuqo status        # Check configuration"
if is_termux; then
    echo "  xiuqo gateway       # Run gateway in foreground"
else
    echo "  xiuqo gateway install # Install gateway service (messaging + cron)"
fi
echo "  xiuqo cron list     # View scheduled jobs"
echo "  xiuqo doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m xiuqo_cli.main setup
fi
