...
Plain mixin for ``xiuqo_state.SessionDB`` (no ``__init__``/state of its own).
Must never import xiuqo_state (cycle); shared constants live in xiuqo_state_common.
"""
...
from xiuqo_state_fts import _drop_orphan_fts_shadow_tables
from xiuqo_state_holders import _read_proc_argv
...
# peer (a supervised service on the same XIUQO_HOME), not a transient one worth waiting out (#106393).
...
class SessionSchemaMixin:
    ...
    def _defer_stale_fts_for_holders(self, cursor: sqlite3.Cursor, foreign_holders) -> bool:
        ...
        if futile:
            logger.error(
                ...
                "own retry admits the rebuild within %.0fs of the holder leaving. `xiuqo doctor` shows this.",
            ...
            logger.error(
                ...
                "processes (this process's own retry then rebuilds), or run `xiuqo sessions optimize-storage` "
                "with every holder stopped. `xiuqo doctor` reports this degraded state.", attempts, foreign_holders,
            )
        logger.warning(
    ...
    def _trigram_tokenizer_available(self, cursor: sqlite3.Cursor) -> bool:
        ...
        probe = "temp.xiuqo_fts5_trigram_probe"
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        with contextlib.suppress(Exception):  # missing/corrupt cache → recompute below
            # Late import: resolves a test-patched xiuqo_constants.get_hermes_home.
            from xiuqo_constants import get_hermes_home as _home
    ...
    def _run_data_migrations(self, cursor: sqlite3.Cursor, current_version: int, fts5_available: bool) -> None:
        ...
        # install only gets a flag; `xiuqo sessions optimize-storage` performs it. The FTS
        ...
        # as before, and we only record a flag advertising that the optimization is available. `xiuqo
