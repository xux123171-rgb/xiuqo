"""Windows UTF-8 bootstrap for Xiuqo entry points (no-op on POSIX).
...
point (``xiuqo``, ``xiuqo``, ``xiuqo-acp``, ``gateway.run``, ``batch_runner``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``xiuqo_cli._subprocess_compat.suppress_platform_ver_console`` for callers
    that never import ``xiuqo_cli.main``; double application is harmless.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Xiuqo modules.

    Xiuqo ships top-level modules with common names (``utils``, ``proxy``, ``ui``); a
    ...
    dropped and the Xiuqo root is *relocated* to the front, not merely inserted when
    ...
    """
    root = src_root or os.environ.get("XIUQO_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    """Put the durable lazy-install dir (``XIUQO_LAZY_INSTALL_TARGET``) on ``sys.path``.
    ...
    """
    if not os.environ.get("XIUQO_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points only need ``import xiuqo_bootstrap`` first.
