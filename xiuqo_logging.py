"""Centralized logging setup for Xiuqo Agent.
...
# in Xiuqo (TUI, gateway, hy_memory, MCP servers, CLI commands all log) —
...
from xiuqo_constants import get_config_path, get_hermes_home, mkdir_under_hermes_home
...
def _install_session_record_factory() -> None:
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.xiuqo_home = str(get_hermes_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.xiuqo_home = ""  # type: ignore[attr-defined]
        return record
...
# Logger name prefixes per component; used by _ComponentFilter and ``xiuqo logs --component``.
COMPONENT_PREFIXES = {
    ...
    "gateway": ("gateway", "xiuqo_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("xiuqo_cli", "cli"),
    "cron": ("cron",),
    "gui": ("xiuqo_cli.web_server", "xiuqo_cli.pty_bridge", "tui_gateway", "uvicorn"),
}


def setup_logging(
    *,
    xiuqo_home: Optional[Path] = None,
    ...
    """Configure the Xiuqo logging subsystem; returns the ``logs/`` directory.
    ...
    home = xiuqo_home or get_hermes_home()
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from xiuqo_cli.config import is_managed
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their Xiuqo home.
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "xiuqo_home", "")
        try:
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    if listener is None:
        return
    t = threading.Thread(target=lambda: _quietly(listener.stop), name="xiuqo-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _read_logging_config():
    ...
    try:
        ...
        # can pin logging.*) so this reuses xiuqo_cli.main's early parse (one config.yaml parse
        # per process); fall back to a direct parse for bare xiuqo_logging consumers.
        try:
