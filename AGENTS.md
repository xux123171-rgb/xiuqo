# Xiuqo Agent - Development Guide

Instructions for AI coding assistants and developers working on the xiuqo codebase.
...
Xiuqo is a personal AI agent that runs the same agent core across a CLI, a messaging
...
Read the balance right: Xiuqo ships a **lot**. Most merges are bug fixes to reported
...
- **Expand reach at the edges.** New adapters, channels, providers, models, desktop/TUI/
  ...
  the existing setup/config UX (`xiuqo tools`, `xiuqo setup`, auto-install) rather than
...
- **E2E validation, not just green unit mocks.** Anything touching resolution chains, config
  ...
  real path with real imports against a temp `XIUQO_HOME`. Mocks hide integration bugs.
...
- **New `XIUQO_*` env vars for non-secret config.** `.env` is for secrets only. Behavioral
...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `xiuqo tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.xiuqo/plugins/` or pip
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `xiuqo <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`xiuqo webhook`, `xiuqo cron`, `xiuqo tools`).
...
4. **Plugin** — third-party/niche/user-specific; lives in `~/.xiuqo/plugins/` or a pip
...
drive a locally spawned backend, one over SSH, one behind URL + token, or Xiuqo Cloud, and
only the first two carry `XIUQO_DESKTOP=1`. An env-keyed gate is a silent no-op on the other
topologies — the tool is stripped from the schema while the platform hint tells the model it
is "inside the Xiuqo desktop app". The pattern:

- **The toolset is the surface gate.** Keep such tools off `_XIUQO_CORE_TOOLS` and in a named
...
- **Ask which identity you mean.** `XIUQO_DESKTOP=1` legitimately means "this backend was
  spawned by the app" (cron ticker, web-dist handling). It does NOT mean "a GUI is watching";
  the embedded terminal pane (`xiuqo --tui` against that backend) is the counterexample.
...
```
`scripts/run_tests.sh` probes `.venv`, then `venv`, then `$HOME/.xiuqo/xiuqo/venv`
...
├── toolsets.py           # TOOLSETS dict, _XIUQO_CORE_TOOLS
├── cli.py                # HermesCLI (REPL, slash dispatch) + xiuqo_cli/cli_*_mixin.py
├── xiuqo_state.py       # SessionDB facade; xiuqo_state_*.py siblings
├── xiuqo_constants.py   # get_hermes_home(), display_hermes_home() — profile-aware paths
├── xiuqo_logging.py     # agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # turn_*.py loop phases, providers, memory, compression, prompt builder
├── xiuqo_cli/           # CLI subcommands, setup, config, plugins loader, skins, updater
...
├── ui-tui/               # Ink (React) terminal UI — `xiuqo --tui`
...
**User state:** `~/.xiuqo/config.yaml` (settings), `~/.xiuqo/.env` (secrets only),
`~/.xiuqo/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_hermes_home()`. Browse logs with `xiuqo logs [--follow] [--level] [--session]`.
...
families: `xiuqo_state.py` (21), `gateway/run.py` (15), `tools/mcp_tool.py` (15),
`xiuqo_cli/kanban.py` (14), `xiuqo_cli/web_server.py` (13 + 24 routers), `xiuqo_cli/auth.py`
(12), `tools/browser_tool.py` (11), `cli.py` (12 `xiuqo_cli/cli_*_mixin.py`), `run_agent.py`
...
- **Compat pointers are OFF LIMITS in-tree.** Old import paths kept alive for external plugins
  ...
  `-W error::xiuqo_cli.plugin_compat.HermesPluginCompatWarning` catches them in the suite.
...
- **Never infer process identity from argv substrings** (`"serve" in cmdline`) — the bug class
  ...
  `xiuqo_cli.update_cmd._hermes_holder_subcommand`; flag sets are DERIVED from the parser
  (`_holder_value_flags()`), never hand-written; match FULL cmdlines and truncate only for
  display. Details: `xiuqo_cli/AGENTS.md`.
- **Never hardcode `~/.xiuqo`.** `get_hermes_home()` for code paths, `display_hermes_home()`
  for user-facing text (both from `xiuqo_constants`). Hardcoding breaks profiles (5 bugs in
  PR #3575). Module-level constants are fine — they cache after `_apply_profile_override()`
  sets `XIUQO_HOME`. Profile operations themselves are HOME-anchored
  (`_get_profiles_root()` = `Path.home()/.xiuqo/profiles`) so `xiuqo -p x profile list`
  sees all profiles — intentional, not a bug.
- **Argparse alias dispatch:** `add_parser("list", aliases=["ls"])` sets `dest` to the literal
  the user typed (`"ls"`). Dispatch must accept both (caught PTY-testing `xiuqo webhook ls`).
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `XIUQO_HOME` first.
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `XIUQO_HOME` → temp dir, and per-file subprocess
...
- **Flake policy:** a failing FILE is retried once in a fresh subprocess (`--file-retries`;
  `XIUQO_TEST_FILE_RETRIES=0` disables). Pass-on-retry is green but printed under `⚠ FLAKY`
...
- **Placement mirrors the source tree.** A test lives in `tests/<top-level source dir>/` (`tests/xiuqo_cli/`,
  `tests/agent/`, `tests/xiuqo_state/`, `tests/gateway/relay/`, ...); installer/updater script tests
  under `tests/scripts/{install,desktop_update}/`. Only tests of root-level modules (`batch_runner`,
  `utils`, `xiuqo_constants`, packaging) sit directly in `tests/`. No issue numbers in filenames —
...
- **Tests must not write to `~/.xiuqo/`.** The autouse `_isolate_hermes_home` fixture in
  `tests/conftest.py` redirects `XIUQO_HOME`; never hardcode `~/.xiuqo/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_hermes_home()` stay
  in the temp dir (pattern: `tests/xiuqo_cli/test_profiles.py`):
  ```python
  @pytest.fixture
  def profile_env(tmp_path, monkeypatch):
      home = tmp_path / ".xiuqo"; home.mkdir()
      monkeypatch.setattr(Path, "home", lambda: tmp_path)
      monkeypatch.setenv("XIUQO_HOME", str(home))
      return home
  ```
  Tests that `patch.object(Path, "home", ...)` must ALSO set `XIUQO_HOME` — code reads the
  env var, not `Path.home()/.xiuqo`.
...
`tests/xiuqo_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cli.py`, `xiuqo_cli/`, `main.py` | `xiuqo_cli/AGENTS.md` | CLI mixins, `_SLASH_DISPATCH`, slash registry, config system + loaders, skins, `xiuqo update` pipeline, profiles / multiplex |
...
| `plugins/`, `xiuqo_cli/plugins*.py` | `plugins/AGENTS.md` | Plugin kinds, native compat contract, in-tree policy, Sep-2026 compat window |
| `tui_gateway/`, `ui-tui/` | `tui_gateway/AGENTS.md` | Process model, JSON-RPC transport, key surfaces, slash flow, dev commands |
| `web/`, `xiuqo_cli/web_routers/` | `web/AGENTS.md` | Dashboard embeds the real TUI; what React may and may not rebuild |
...
| `cron/`, kanban (`xiuqo_cli/kanban*.py`, `tools/kanban_tools.py`, `plugins/kanban/`) | `cron/AGENTS.md` | Scheduler invariants, job fields, kanban board/dispatcher |
