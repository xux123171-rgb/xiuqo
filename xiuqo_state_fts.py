...
from xiuqo_state_common import (FTS_CJK_STALE_KEY, FTS_STALE_KEY, _FTS_CJK_TRIGGERS, _FTS_TRIGGERS,
    routed_sessions_setting)
from xiuqo_state_errors import is_fts_scoped_corruption_error

# caplog tests pin the "xiuqo_state" logger name.
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("XIUQO_FTS5_CJK_SO")
...
def _cjk_fts_config_enabled() -> bool:
    ...
    value = routed_sessions_setting("cjk_fts", "XIUQO_CJK_FTS")
...
class SessionFtsSetupMixin:
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `xiuqo update` to rebuild the venv with a "
                ...
                if cjk_present:
                    ...
                    if live:
                        ...
                        logger.warning(
                            ...
                            "run `xiuqo sessions optimize-storage` on a host "
    ...
    def _is_fts_write_corruption_error(exc: sqlite3.DatabaseError) -> bool:
        ...
        gateway transcript retry: see :func:`xiuqo_state_errors.is_fts_scoped_corruption_error`."""
