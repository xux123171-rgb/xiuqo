...
storage discipline as the other indexes). On a populated database, run
    ...
    xiuqo sessions optimize-storage

to backfill it; new messages are indexed live either way. Set
`sessions.cjk_fts: false` in `~/.xiuqo/config.yaml` to disable. Override
the .so location with `XIUQO_FTS5_CJK_SO`.
