...
Run after changing any ``desktop=`` value or alias in ``xiuqo_cli/commands.py``;
``tests/xiuqo_cli/test_desktop_slash_registry.py`` fails until the committed
...
def render() -> str:
    ...
    from xiuqo_cli.commands import desktop_surface_registry
