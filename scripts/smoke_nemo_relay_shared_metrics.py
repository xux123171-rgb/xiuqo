"""Run a real Xiuqo CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_hermes_executable(xiuqo_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "xiuqo",
        Path(".venv") / "Scripts" / "xiuqo.exe",
    ):
        candidate = xiuqo_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("xiuqo")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Xiuqo executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Xiuqo source checkout containing .venv/bin/xiuqo",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated XIUQO_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["xiuqo.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['xiuqo.client.active']}"
        )
    [model] = by_name["xiuqo.model_route.count"]
    expected_model = {
        "name": "xiuqo.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['xiuqo.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["xiuqo.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['xiuqo.task_run.started']}"
        )
    [terminal] = by_name["xiuqo.task_run.finished"]
    ...
    [tool] = by_name["xiuqo.tool_call.count"]
    ...
    lifecycle = by_name["xiuqo.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["xiuqo.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Xiuqo development environment requires jsonschema"
        ) from exc
    ...
    if metrics["xiuqo.client.active"] != [
        {
            "name": "xiuqo.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['xiuqo.client.active']}"
        )
    [model] = metrics["xiuqo.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['xiuqo.model_route.count']}"
        )
    [terminal] = metrics["xiuqo.task_run.finished"]
    ...
    [tool] = metrics["xiuqo.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["xiuqo.skill.lifecycle.count"]
    ...
    loads = metrics["xiuqo.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    xiuqo_repo = args.xiuqo_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    xiuqo = _resolve_hermes_executable(xiuqo_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="xiuqo-relay-shared-metrics-"))
    home = root / "xiuqo-home"
    ...
    (root / "xiuqo.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "xiuqo.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Xiuqo exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Xiuqo model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Xiuqo did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Xiuqo did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "xiuqo.shared_metrics.v2.schema.json",
    )

    print("Xiuqo -> NeMo Relay shared-metrics smoke test passed")
