...
run_py() {
  ...
  if [ "$with_var" = yes ]; then
    PYTHONPATH="$CAP_DIR${PYTHONPATH:+:$PYTHONPATH}" \
      XIUQO_E2E_CAPTURE_LAUNCH="$WORK/spec.json" python3 "$@"
  else
...
run_py yes -c '
import subprocess
r = subprocess.run(["npm", "exec", "--", "electron", "."], cwd="/tmp", env={"XIUQO_DESKTOP_CWD": "/tmp", "PATH": "/usr/bin"})
...
assert spec["env"]["XIUQO_DESKTOP_CWD"] == "/tmp", "env= kwarg not captured"
...
run_py yes -c '
import subprocess
exe = "/x/apps/desktop/release/linux-unpacked/Xiuqo"
...
run_py yes -c '
import subprocess
r = subprocess.run(["C:\\x\\apps\\desktop\\release\\win-unpacked\\Xiuqo.exe"], env={})
