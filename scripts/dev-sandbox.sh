#!/usr/bin/env bash
# Run a Xiuqo instance in an isolated sandbox — separate XIUQO_HOME,
...
# .xiuqo-sandbox/ in the worktree git root).
#
# Usage:
#   scripts/dev-sandbox.sh python -m xiuqo_cli.main
#   scripts/dev-sandbox.sh xiuqo desktop
#   scripts/dev-sandbox.sh electron .
#   scripts/dev-sandbox.sh -- npm run dev   # from apps/desktop/
#   scripts/dev-sandbox.sh --persistent xiuqo desktop
#   scripts/dev-sandbox.sh --persistent -- npm run dev
#
# Seed the sandbox XIUQO_HOME from an existing directory (e.g. your main
# ~/.xiuqo) so config, sessions, skills, etc. are pre-populated:
#   scripts/dev-sandbox.sh --from ~/.xiuqo xiuqo desktop
#
# Override the app name (default: HermesSandbox):
#   XIUQO_DEV_SANDBOX_NAME=Staging scripts/dev-sandbox.sh xiuqo desktop
#
# Override the persistent sandbox dir name (default: .xiuqo-sandbox):
#   XIUQO_DEV_SANDBOX_DIR=.staging-sandbox scripts/dev-sandbox.sh --persistent xiuqo desktop
...
Run a Xiuqo instance in an isolated sandbox.

Options:
  --persistent    Keep the sandbox dir across restarts (under the worktree
                  git root, in .xiuqo-sandbox/). Without this flag the
                  sandbox is a temp dir that is removed on exit.
  --from DIR      Copy DIR into the sandbox XIUQO_HOME as the starting
                  point (config, sessions, skills, etc.).
                  Ignored if the sandbox XIUQO_HOME already has content
                  (e.g. reusing a --persistent sandbox) to avoid clobbering.
  --delete        Delete the existing persistent sandbox in .xiuqo-sandbox.
  -h, --help      Show this help message.

Environment:
  XIUQO_DEV_SANDBOX_NAME  Override the app name (default: HermesSandbox)
  XIUQO_DEV_SANDBOX_DIR   Override the persistent dir name (default: .xiuqo-sandbox)

Examples:
  dev-sandbox.sh xiuqo desktop
  dev-sandbox.sh --persistent xiuqo desktop
  dev-sandbox.sh --from ~/.xiuqo xiuqo desktop
...
SANDBOX_DIR_NAME="${XIUQO_DEV_SANDBOX_DIR:-.xiuqo-sandbox}"
...
SANDBOX_NAME="${XIUQO_DEV_SANDBOX_NAME:-$DEFAULT_SANDBOX_NAME}"

if [ "$PERSISTENT" = true ]; then
  SANDBOX_ROOT="$PERSISTENT_SANDBOX_ROOT"
else
  SANDBOX_ROOT="$(mktemp -d -t xiuqo-sandbox.XXXXXX)"
fi

export XIUQO_HOME="$SANDBOX_ROOT/xiuqo-home"
export XIUQO_DESKTOP_USER_DATA_DIR="$SANDBOX_ROOT/user-data"
export XIUQO_DESKTOP_APP_NAME="$SANDBOX_NAME"

mkdir -p "$XIUQO_HOME" "$XIUQO_DESKTOP_USER_DATA_DIR"

if [ -n "$SEED_DIR" ]; then
  # Only seed when the sandbox XIUQO_HOME is empty — avoids clobbering an
  # existing persistent sandbox on re-run.
  if [ -z "$(ls -A "$XIUQO_HOME" 2>/dev/null)" ]; then
    echo "[sandbox] seeding XIUQO_HOME from $SEED_DIR" >&2
    cp -a "$SEED_DIR/." "$XIUQO_HOME/"
  else
    echo "[sandbox] --from ignored: $XIUQO_HOME already has content" >&2
  fi
fi

echo "[sandbox] XIUQO_HOME=$XIUQO_HOME" >&2
echo "[sandbox] userData=$XIUQO_DESKTOP_USER_DATA_DIR" >&2
echo "[sandbox] appName=$XIUQO_DESKTOP_APP_NAME" >&2
if [ "$PERSISTENT" = true ]; then
