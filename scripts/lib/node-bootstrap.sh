...
#   2. ~/.xiuqo/node/ from a prior Xiuqo-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.xiuqo/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$XIUQO_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   XIUQO_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   XIUQO_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   XIUQO_HOME               (default: $HOME/.xiuqo)
# ============================================================================

XIUQO_NODE_MIN_VERSION="${XIUQO_NODE_MIN_VERSION:-20}"
XIUQO_NODE_TARGET_MAJOR="${XIUQO_NODE_TARGET_MAJOR:-22}"
XIUQO_HOME="${XIUQO_HOME:-$HOME/.xiuqo}"
XIUQO_NODE_AVAILABLE=false
...
# Redirect a Xiuqo-managed Node's `npm install -g` to the command link dir
# (already on PATH) instead of the default $XIUQO_HOME/node/bin, which is off
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$XIUQO_HOME/node/etc/npmrc"
}
...
_nb_npm_range() {
    if [ -n "${XIUQO_NPM_TARGET_RANGE:-}" ]; then
        printf '%s\n' "$XIUQO_NPM_TARGET_RANGE"
        return 0
    fi
...
# The Python side recovers through xiuqo_cli/npm_engine.py; the installer path
...
_nb_ensure_bundled_npm_range() {
    local npm_bin="$XIUQO_HOME/node/bin/npm"
    [ -x "$npm_bin" ] || return 0
    ...
    _nb_warn "Fix manually: npm install -g --prefix \"$XIUQO_HOME/node\" npm@\"$range\""
    return 1
}
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$XIUQO_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $XIUQO_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$XIUQO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$XIUQO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $XIUQO_NODE_TARGET_MAJOR..."
    proto install node "$XIUQO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $XIUQO_NODE_TARGET_MAJOR..."
    nvm install "$XIUQO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$XIUQO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${XIUQO_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${XIUQO_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${XIUQO_NODE_TARGET_MAJOR}.x/"
    local tarball=""
    # `tar xf` needs the xz binary for .tar.xz; minimal images ship tar without it (#11197).
    if command -v xz >/dev/null 2>&1; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${XIUQO_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${XIUQO_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $XIUQO_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $XIUQO_HOME/node/..."
    ...
    mv "$extracted" "$XIUQO_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    # XIUQO_NODE_SKIP_LINKS=1: the caller only wants the private managed tree
    ...
    if [ "${XIUQO_NODE_SKIP_LINKS:-0}" != "1" ]; then
        mkdir -p "$_link_dir"
        ln -sf "$XIUQO_HOME/node/bin/node" "$_link_dir/node"
        ln -sf "$XIUQO_HOME/node/bin/npm"  "$_link_dir/npm"
        ln -sf "$XIUQO_HOME/node/bin/npx"  "$_link_dir/npx"
    fi
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $XIUQO_HOME/node/"
...
# ---------------------------------------------------------------------------
# Heal a broken Xiuqo-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
_nb_managed_tool_broken() {
    ...
    local probe
    for probe in \
        "$XIUQO_HOME/node/bin/$tool" \
        "$XIUQO_HOME/node/${tool}.exe" \
        "$XIUQO_HOME/node/$tool"; do
...
# The managed node runs but is below XIUQO_NODE_TARGET_MAJOR — an old tree
...
_nb_managed_node_outdated() {
    ...
    for probe in "$XIUQO_HOME/node/bin/node" "$XIUQO_HOME/node/node"; do
        [ -x "$probe" ] || continue
        ver="$("$probe" --version 2>/dev/null)" || return 1
        major="${ver#v}"; major="${major%%.*}"
        case "$major" in ''|*[!0-9]*) return 1 ;; esac
        [ "$major" -lt "$XIUQO_NODE_TARGET_MAJOR" ] && return 0
        return 1
    done
    return 1
}
...
# absent. Used by xiuqo_constants.find_hermes_node_executable() and safe
# to call from install reruns.
heal_managed_node() {
    [ -d "$XIUQO_HOME/node" ] || return 1
    if ! _nb_managed_node_needs_heal; then
        return 0
    fi
    _nb_log "Xiuqo-managed Node is broken — redownloading to $XIUQO_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if _nb_have_modern_node; then
        ...
        XIUQO_NODE_AVAILABLE=true
        return 0
    fi

    if [ -x "$XIUQO_HOME/node/bin/node" ]; then
        export PATH="$XIUQO_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Xiuqo-managed)"
            XIUQO_NODE_AVAILABLE=true
    ...
    _nb_try_fnm   && { XIUQO_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { XIUQO_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { XIUQO_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { XIUQO_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { XIUQO_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { XIUQO_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $XIUQO_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
