...
When Xiuqo runs in TUI mode, the gateway child process communicates with
...
# ``get_hermes_home() / "plugins"`` (user) and ``./.xiuqo/plugins/`` (project,
# gated behind ``XIUQO_ENABLE_PROJECT_PLUGINS``) — see
# ``xiuqo_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``xiuqo_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import xiuqo_constants (this script
    ...
    # ./.xiuqo/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_hermes_home() / "plugins"]
    if os.environ.get("XIUQO_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".xiuqo" / "plugins")
