#!/usr/bin/env bash
# Canonical test runner for xiuqo. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.xiuqo/...)
#
# Usage:
...
# (XIUQO_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.xiuqo/xiuqo/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.xiuqo/xiuqo/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${XIUQO_PYTHON:-}" ] && [ -x "$XIUQO_PYTHON" ] \
    && "$XIUQO_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: XIUQO_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `xiuqo` binary rather
  # than the devShell hook.
  PYTHON="$XIUQO_PYTHON"
  echo "▶ no local venv — using Nix dev venv via XIUQO_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and XIUQO_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.xiuqo/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.xiuqo"
...
#
#   * XIUQO_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * XIUQO_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     XIUQO_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# XIUQO_RUN_SLOW_PET_TESTS / XIUQO_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no XIUQO_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in XIUQO_TEST_IMAGE XIUQO_TEST_WORKERS XIUQO_TEST_PATHS \
  XIUQO_TEST_FILE_TIMEOUT XIUQO_TEST_FILE_RETRIES XIUQO_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${XIUQO_RUN_SLOW_PET_TESTS:+XIUQO_RUN_SLOW_PET_TESTS="$XIUQO_RUN_SLOW_PET_TESTS"} \
  ${XIUQO_E2E_BROWSER:+XIUQO_E2E_BROWSER="$XIUQO_E2E_BROWSER"} \
