# Behavioral test for install.ps1's xiuqo launcher staging (PR #92092,
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'xiuqo-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'xiuqo'
    ...
    Assert-True $missingThrew 'missing xiuqo.exe fails the launcher stage'
    ...
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'xiuqo.exe'), $hermesV1)
    ...
    Assert-BytesEqual $hermesV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'xiuqo.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'xiuqo-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'xiuqo.exe'), $hermesV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'xiuqo-acp.exe'), $acp)
    Install-HermesCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $hermesV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'xiuqo.exe'))) `
        'installer refreshes an existing Xiuqo launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'xiuqo-acp.exe'))) `
    ...
    Install-HermesCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'xiuqo.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'xiuqo.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'xiuqo.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'xiuqo.exe')) -and $cmdBody.Contains('%*')) `
