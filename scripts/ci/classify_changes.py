...
# tree generates llms.txt — the index every LLM (Xiuqo included, via the
# xiuqo skill) reads to learn what Xiuqo can do — and
...
_PY_RELEVANT_CONTRACT_FILES = {
    ...
    # tests/xiuqo_cli/test_desktop_slash_registry.py
...
_MCP_CATALOG_FILES = {"xiuqo_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Xiuqo-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``xiuqo serve`` backend, the Docker image)
