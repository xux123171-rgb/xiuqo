...
WITHOUT installing xiuqo.

NOTE: this script intentionally duplicates the schema rules instead of
importing ``xiuqo_cli`` — the whole point is the no-install requirement for
cheap cross-repo CI use. The runtime twin of this schema lives in
``xiuqo_cli/plugin_catalog.py``; if the contract changes there, update the
