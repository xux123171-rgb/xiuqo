# ============================================================================
# Xiuqo Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$HermesHome = $(if ($env:XIUQO_HOME) { $env:XIUQO_HOME } else { "$env:LOCALAPPDATA\xiuqo" }),
    [string]$InstallDir = $(if ($env:XIUQO_HOME) { "$env:XIUQO_HOME\xiuqo" } else { "$env:LOCALAPPDATA\xiuqo\xiuqo" }),
    ...
    # builds apps/desktop into a launchable Xiuqo.exe.
    #
    # Why opt-in:
    #   * Xiuqo-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Xiuqo.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Xiuqo.exe
    ...
    #     `xiuqo desktop` already builds on demand.
...
# XIUQO_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[xiuqo] $Message")
}
...
if ($PSBoundParameters.ContainsKey('HermesHome')) {
    ...
    $HermesHome = ConvertTo-LongPath $(
        if ($env:XIUQO_HOME) { $env:XIUQO_HOME } else { "$env:LOCALAPPDATA\xiuqo" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:XIUQO_HOME) { "$env:XIUQO_HOME\xiuqo" } else { "$env:LOCALAPPDATA\xiuqo\xiuqo" }
    )
}
...
$RepoUrlSsh = "git@github.com:xux123171-rgb/xiuqo.git"
$RepoUrlHttps = "https://github.com/xux123171-rgb/xiuqo.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Xiuqo Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Install-Uv {
    # Xiuqo owns its own uv at $HermesHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (xiuqo_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `xiuqo update` stay in sync.
    ...
    try {
        ...
        # (xiuqo_cli/managed_uv.py looks only at $HermesHome\bin\uv.exe).
...
# Put the Xiuqo-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone xiuqo-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Xiuqo-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Xiuqo-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("xiuqo-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and XIUQO_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Xiuqo belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # xiuqo_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".xiuqo-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Xiuqo-managed interpreter
    ...
    #
    # Under Xiuqo-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Xiuqo-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Xiuqo setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Xiuqo uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\xiuqo\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Xiuqo needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Xiuqo copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\xiuqo\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``XIUQO_GIT_BASH_PATH`` (User scope) so Xiuqo can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Xiuqo-managed PortableGit install instead..."
    }
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Xiuqo features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Xiuqo needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``XIUQO_GIT_BASH_PATH`` (User env scope) so Xiuqo can find it even before
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("XIUQO_GIT_BASH_PATH", $candidate, "User")
            $env:XIUQO_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set XIUQO_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Xiuqo may not find Git Bash."
    Write-Info "If needed, set XIUQO_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Xiuqo-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Xiuqo requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Xiuqo requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Xiuqo-managed Node.js installation instead..."
    ...
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        ...
        Write-Success "Node.js $version found (Xiuqo-managed)"
    ...
    Write-Info "Installing Xiuqo-managed Node.js $NodeVersion LTS..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\xiuqo-node-extract"
            ...
            if ($extractedDir) {
                ...
                if (Test-Path "$HermesHome\node") {
                    try {
                        ...
                        Write-Warn "Xiuqo-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\xiuqo-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `xiuqo
                    # update` path (#4735).
                    ...
                    $stashName = "xiuqo-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. xiuqo-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``xiuqo update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Xiuqo behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running xiuqo processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/xux123171-rgb/xiuqo/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/xux123171-rgb/xiuqo/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/xux123171-rgb/xiuqo/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\xiuqo-$zipLabel.zip"
                $extractPath = "$env:TEMP\xiuqo-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `xiuqo update` (see the notes at the shared clone-path
    ...
    # next `xiuqo update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Xiuqo-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Xiuqo-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running xiuqo process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running xiuqo processes before recreating venv..."
            ...
            # The launcher CLI (xiuqo.exe) plus its child tree.
            & taskkill /F /T /IM xiuqo.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM xiuqo.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m xiuqo_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not xiuqo.exe.
            ...
            # Xiuqo children re-exec through .xiuqo-runtime, so killing only
            ...
            # user (see xiuqo_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Xiuqo " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `xiuqo update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Xiuqo (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `xiuqo.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install xiuqo package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and xiuqo.exe broken with
    ...
    # here, not 30 seconds later when the user runs `xiuqo`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Xiuqo processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Xiuqo processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Xiuqo and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register xiuqo.exe in dist-info/RECORD but fail to
        ...
        # `xiuqo` command while xiuqo.exe / xiuqo-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m xiuqo_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `xiuqo dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\xiuqo_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `xiuqo dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `xiuqo dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: xiuqo_cli/web_server.py"
        }
    }
...
function Install-HermesCommandLaunchers {
    ...
    # Expose ONLY the xiuqo launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring xiuqo.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "xiuqo.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the xiuqo command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # xiuqo_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("xiuqo", "xiuqo-acp")) {
    ...
    $requiredExe = Join-Path $Destination "xiuqo.exe"
    $requiredCmd = Join-Path $Destination "xiuqo.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the xiuqo command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up xiuqo command..."
    
    if ($NoVenv) {
        $hermesBin = "$InstallDir"
    } else {
        # $HermesHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `xiuqo update`'s autostash
        ...
        # installer staged under xiuqo\bin. No git operation can ever
    ...
    #   xiuqo\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The xiuqo\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept xiuqo via $hermesBin)"
        }
    }
    ...
    # Set XIUQO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\xiuqo instead
    # of the Unix default ~/.xiuqo
    $currentHermesHome = [Environment]::GetEnvironmentVariable("XIUQO_HOME", "User")
    if (-not $currentHermesHome -or $currentHermesHome -ne $HermesHome) {
        [Environment]::SetEnvironmentVariable("XIUQO_HOME", $HermesHome, "User")
        Write-Success "Set XIUQO_HOME=$HermesHome"
    }
    $env:XIUQO_HOME = $HermesHome
    
    # Update current session
    $env:Path = "$hermesBin;$env:Path"
    
    Write-Success "xiuqo command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.xiuqo-bootstrap-complete which tells the Xiuqo
    ...
    $markerPath = Join-Path $InstallDir ".xiuqo-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the XIUQO_HOME directory structure ($HermesHome, default %LOCALAPPDATA%\xiuqo)
    ...
    # (the default PS5 behaviour), and Xiuqo's prompt-injection scanner
    ...
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in xiuqo_cli/default_soul.py. The runtime
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Xiuqo-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'xiuqo setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\xiuqo-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\xiuqo-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\xiuqo-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `xiuqo tools`
...
function Install-BrowserUseCli {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Xiuqo' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'xiuqo tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `xiuqo
...
# paths still lazy-install via install_cua_driver() (xiuqo_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `xiuqo computer-use install` runs. Bounded
        ...
        # Xiuqo' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'xiuqo tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: xiuqo computer-use install"
            }
        } else {
...
# the final `electron` -> `Xiuqo` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Xiuqo.exe. Only called from
    ...
    # resolves apps/desktop/release/win-unpacked/Xiuqo.exe directly,
    ...
    # enforces the supported Node lines and prepends the Xiuqo-managed Node to
    ...
    # The --dir mode produces an unpacked Xiuqo.exe in
    ...
    # are NOT trying to work around). The Xiuqo icon + product name are
    # stamped onto Xiuqo.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\xiuqo-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Xiuqo.exe",
        "$desktopDir\release\win-arm64-unpacked\Xiuqo.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Xiuqo.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Xiuqo icon + identity are stamped onto Xiuqo.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, xiuqo#38216).
    ...
    #    Xiuqo.exe. We deliberately do NOT point them at `xiuqo desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Xiuqo.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Xiuqo.lnk')
        )
        ...
        # with the Xiuqo icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.xiuqo/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'xiuqo setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m xiuqo_cli.main setup
    } else {
        python -m xiuqo_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $hermesCmd = "$InstallDir\venv\Scripts\xiuqo.exe"
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'xiuqo whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: xiuqo gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: xiuqo gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: xiuqo gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host ""
    Write-Host "   xiuqo              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   xiuqo setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   xiuqo config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   xiuqo config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   xiuqo gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   xiuqo update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Xiuqo repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.xiuqo-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (Xiuqo-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Xiuqo to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/xux123171-rgb/xiuqo/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
