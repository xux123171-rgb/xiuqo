...
# Python-driven tool session (e.g. Xiuqo terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:xux123171-rgb/xiuqo.git"
REPO_URL_HTTPS="https://github.com/xux123171-rgb/xiuqo.git"
XIUQO_HOME="${XIUQO_HOME:-$HOME/.xiuqo}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${XIUQO_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$XIUQO_INSTALL_DIR"
...
#   code at /usr/local/lib/xiuqo, command at /usr/local/bin/xiuqo,
#   data still at /root/.xiuqo (XIUQO_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$XIUQO_HOME/.no-bundled-skills so future"
            echo "                   'xiuqo update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Xiuqo.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.xiuqo/xiuqo"
            echo "                   default (root, Linux): /usr/local/lib/xiuqo"
            echo "  --xiuqo-home PATH  Data directory (default: ~/.xiuqo, or \$XIUQO_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Xiuqo installs the code under"
            echo "  /usr/local/lib/xiuqo and links the command into"
            echo "  /usr/local/bin/xiuqo (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$XIUQO_HOME"
            echo "  (default /root/.xiuqo).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$XIUQO_HOME/xiuqo are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `xiuqo update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Xiuqo-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Xiuqo Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install xiuqo command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `xiuqo` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $XIUQO_HOME/xiuqo
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $XIUQO_HOME/xiuqo
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/xiuqo
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $XIUQO_HOME/xiuqo — then preserve it)
#
# Always no-op when the user set --dir or $XIUQO_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in XIUQO_HOME.
    if is_termux; then
        INSTALL_DIR="$XIUQO_HOME/xiuqo"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$XIUQO_HOME/xiuqo/.git" ]; then
            INSTALL_DIR="$XIUQO_HOME/xiuqo"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/xiuqo)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/xiuqo"
        ...
        # /usr/local/bin/xiuqo wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/xiuqo"
        log_info "  Data:    $XIUQO_HOME (unchanged)"
...
# Point a Xiuqo-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $XIUQO_HOME/node/bin — which is NOT on
...
# Xiuqo's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Xiuqo-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$XIUQO_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$XIUQO_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$XIUQO_HOME/node/etc/npmrc"
}

get_hermes_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/xiuqo" ]; then
        echo "$link_dir/xiuqo"
    else
        echo "xiuqo"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/xux123171-rgb/xiuqo/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Xiuqo owns its own uv at $XIUQO_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (xiuqo_cli/managed_uv.py) looks in the same
    # place, so install.sh and `xiuqo update` stay in sync.
    local _managed_uv="$XIUQO_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $XIUQO_HOME/bin ..."
    mkdir -p "$XIUQO_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/xiuqo-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/xiuqo-uv-installer.$$.sh")"
    ...
    # directly into $XIUQO_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$XIUQO_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Xiuqo currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; Xiuqo requires Python >=3.11,<3.14"
...
# replaced with the Xiuqo-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Xiuqo-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Xiuqo-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Xiuqo-managed Node from a previous run over a too-old system one.
    if [ -x "$XIUQO_HOME/node/bin/node" ] && [ -x "$XIUQO_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$XIUQO_HOME/node/bin/node" --version)"; then
        export PATH="$XIUQO_HOME/node/bin:$PATH"
        log_success "Node.js $("$XIUQO_HOME/node/bin/node" --version) found (Xiuqo-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Xiuqo-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Xiuqo requires Node 22.22+, 24.11+, or 26+) — installing Xiuqo-managed Node $NODE_VERSION..."
...
# Download and adopt one Node release line (e.g. 26) into ~/.xiuqo/node/.
#
...
install_node_line() {
    ...
    log_info "Extracting to ~/.xiuqo/node/..."
    ...
    # Place into ~/.xiuqo/node/ and symlink binaries into the same bin dir
    # the xiuqo command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$XIUQO_HOME/node"
    mkdir -p "$XIUQO_HOME"
    mv "$extracted_dir" "$XIUQO_HOME/node"
    rm -rf "$tmp_dir"
    ...
    ln -sf "$XIUQO_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$XIUQO_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$XIUQO_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    if ! installed_ver=$("$XIUQO_HOME/node/bin/node" --version 2>&1); then
    ...
    log_success "Node.js $installed_ver installed to ~/.xiuqo/node/"
    HAS_NODE=true
    return 0
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Xiuqo install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Xiuqo Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `xiuqo update` path
                # (#4735).
                ...
                local stash_name
                stash_name="xiuqo-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``xiuqo update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Xiuqo behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up xiuqo command..."

    if [ "$USE_VENV" = true ]; then
        XIUQO_BIN="$INSTALL_DIR/venv/bin/python"
        XIUQO_ENTRYPOINT="$INSTALL_DIR/xiuqo"
    else
        XIUQO_BIN="$(which xiuqo 2>/dev/null || echo "")"
        if [ -z "$XIUQO_BIN" ]; then
            log_warn "xiuqo not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$XIUQO_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$XIUQO_ENTRYPOINT" ]; }; then
        log_warn "Xiuqo launcher prerequisites not found"
    ...
    # Create a user-facing shim for the xiuqo command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $XIUQO_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$XIUQO_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/xiuqo"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/xiuqo" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$XIUQO_BIN" "$XIUQO_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/xiuqo" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/xiuqo"
    log_success "Installed xiuqo launcher → $command_link_display_dir/xiuqo"

    # Also expose `xiuqo`. The `xiuqo` console script declared in
...
unset PYTHONPATH
unset PYTHONHOME
exec "$XIUQO_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/xiuqo" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$XIUQO_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/xiuqo"
    log_success "Installed xiuqo launcher → $command_link_display_dir/xiuqo"

    # Also expose `xiuqo-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `xiuqo-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Xiuqo as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/xiuqo-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/xiuqo-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$XIUQO_BIN" "$XIUQO_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/xiuqo-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/xiuqo-acp"
    log_success "Installed xiuqo-acp launcher → $command_link_display_dir/xiuqo-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "xiuqo command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v xiuqo' \
                >/dev/null 2>&1; then
        ...
        log_info "xiuqo not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Xiuqo Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Xiuqo Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Xiuqo Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so xiuqo works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.xiuqo directory structure (config at top level, code in subdir)
    mkdir -p "$XIUQO_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.xiuqo/.env (top level, easy to find)
    if [ ! -f "$XIUQO_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$XIUQO_HOME/.env"
            log_success "Created ~/.xiuqo/.env from template"
        else
            touch "$XIUQO_HOME/.env"
            log_success "Created ~/.xiuqo/.env"
        fi
    else
        log_info "~/.xiuqo/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$XIUQO_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.xiuqo/config.yaml (top level, easy to find)
    if [ ! -f "$XIUQO_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$XIUQO_HOME/config.yaml"
            log_success "Created ~/.xiuqo/config.yaml from template"
        fi
    else
        log_info "~/.xiuqo/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in xiuqo_cli/default_soul.py — the
    ...
    if [ ! -f "$XIUQO_HOME/SOUL.md" ]; then
        cat > "$XIUQO_HOME/SOUL.md" << 'SOUL_EOF'
You are Xiuqo Agent, built by Nous Research. Be direct: match the length of your reply to the weight of the ask — a one-line question gets a one-line answer, and finished work gets a short report of what changed, what's verified, and what's left, never a replay of the process. No filler ("Great question," "I'd be happy to"), no restating the request back, no re-summarizing what you already said, no narrating tool calls the user can see. Plain claims over adjectives; when unsure, say so plainly. Agree because it's right, not because the user said it. Depth is earned — give it when the user asks for detail, teaches, or the stakes demand it, not by default.
SOUL_EOF
        log_success "Created ~/.xiuqo/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.xiuqo/"

    # Seed bundled skills into ~/.xiuqo/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `xiuqo update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'xiuqo update'." \
            > "$XIUQO_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $XIUQO_HOME/.no-bundled-skills"
        log_info "  Future 'xiuqo update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.xiuqo/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.xiuqo/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$XIUQO_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$XIUQO_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.xiuqo/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Xiuqo Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Xiuqo will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Xiuqo Agent browser tools — explicit browser override."
...
# install — the same closure `xiuqo update` installs
# (xiuqo_cli/main.py::_update_node_dependencies). Prebuilt/partial checkouts
...
install_node_deps() {
    ...
    # Keep the checkout clean so `xiuqo update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `xiuqo tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$XIUQO_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'xiuqo tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `xiuqo tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `xiuqo computer-use install` on their own).
    ...
    # Same upstream installer `xiuqo computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Xiuqo install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Xiuqo'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'xiuqo tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: xiuqo computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'xiuqo setup' after install."
        return 0
    fi
    ...
    # Run xiuqo setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m xiuqo_cli.main setup < /dev/tty
    else
        python -m xiuqo_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$XIUQO_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Xiuqo to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$XIUQO_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'xiuqo whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                XIUQO_CMD="$(get_hermes_command_path)"
                $XIUQO_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'xiuqo whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'xiuqo gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $XIUQO_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $XIUQO_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: xiuqo gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: xiuqo gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $XIUQO_CMD gateway > "$XIUQO_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.xiuqo/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: xiuqo gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: xiuqo gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.xiuqo-bootstrap-complete, which tells the Xiuqo
    ...
    local marker_path="$INSTALL_DIR/.xiuqo-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $XIUQO_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $XIUQO_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $XIUQO_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}xiuqo${NC}              Start chatting"
    echo -e "   ${GREEN}xiuqo setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}xiuqo config${NC}       View/edit configuration"
    echo -e "   ${GREEN}xiuqo config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}xiuqo gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}xiuqo update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'xiuqo' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'xiuqo' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'xiuqo' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$XIUQO_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$XIUQO_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$XIUQO_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    # which every consumer (tools/browser_tool.py, `xiuqo update`'s npx
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$XIUQO_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Xiuqo` rename
...
# _purge_electron_build_cache() used by `xiuqo desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the Xiuqo-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `xiuqo desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Xiuqo" ]; then
            app="$desktop_dir/release/linux-unpacked/Xiuqo"
        elif [ -x "$desktop_dir/release/linux-unpacked/xiuqo" ]; then
            app="$desktop_dir/release/linux-unpacked/xiuqo"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Xiuqo.app" \
            "$desktop_dir/release/mac/Xiuqo.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `xiuqo desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading XIUQO_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if XIUQO_HOME="$XIUQO_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from xiuqo_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `xiuqo update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Xiuqo-managed Node
            # provisioned during prerequisites/node-deps (at $XIUQO_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $XIUQO_HOME.
    # $XIUQO_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'xiuqo update' on this host install.
