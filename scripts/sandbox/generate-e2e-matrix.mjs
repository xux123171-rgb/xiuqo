 ...
 *   The artifact published on the website right now -- Xiuqo-Setup.exe has
 ...
 * @typedef {InstallMethod | 'xiuqo-update' | 'open-app-update' | 'xiuqo-desktop-app-update'} UpdateMethod
 ...
 *   desktop-installer install; xiuqo-desktop-app-update starts the app
 *   via `xiuqo desktop`, which every install method provides on every
...
export const SPEC = {
  windows: {
    install: [
      // irm https://xiuqo.nousresearch.com/install.ps1 | iex
      { method: 'installer-script' },
      // The same one-liner with -IncludeDesktop: builds Xiuqo.exe AND
      ...
      // Website Xiuqo-Setup.exe, clicked through the GUI.
    ...
    update: [
      ...
      // Same button, app launched via `xiuqo desktop`.
      { method: 'xiuqo-desktop-app-update' },
    ],
  },
  macos: {
    install: [
      { method: 'installer-script' },
      { method: 'installer-script+desktop' },
      // The published Xiuqo-Setup.dmg from the website, mounted and run.
      { method: 'desktop-installer', versions: ['latest'] },
    ],
    update: [
      { method: 'installer-script' },
  ...
  linux: {
    ...
    update: [
      ...
      // linux, so there is no open-app-update; `xiuqo desktop` is always
...
export function methodNeedsDesktop(m) {
  return m.startsWith('desktop-installer') || m === 'installer-script+desktop' ||
    m === 'open-app-update' || m === 'xiuqo-desktop-app-update';
}
