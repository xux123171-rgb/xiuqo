...
# (%XIUQO_HOME%\xiuqo-setup.exe). That binary has no self-update path --
...
#
# This script lives in the repo checkout, so EVERY `xiuqo update` refreshes
...
#     -InstallRoot <path>   repo checkout (XIUQO_HOME\xiuqo)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Xiuqo.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .xiuqo-update-in-progress in place (tests)
#
...
# .xiuqo-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim XIUQO_HOME\.xiuqo-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining XIUQO_UPDATE_STARTED_AT from the Desktop hand-off.
# xiuqo_cli/update_lock.py's ancestry rule lets our
# `xiuqo update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Xiuqo.exe pid.
try {
...
$MarkerPath = Join-Path $HermesHome ".xiuqo-update-in-progress"
$LogDir = Join-Path $HermesHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $HermesHome ".xiuqo-update-result.json"
$script:Ui = $null
$script:UiStage = "Xiuqo will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale xiuqo-update-ui-* leftovers from interrupted
    ...
    # update marker (.xiuqo-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "xiuqo-update-ui-*" -ErrorAction SilentlyContinue |
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("xiuqo-update-ui-{0}" -f $PID)
                $browserArgs = @(
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `xiuqo debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"xiuqo debug share`" in a terminal to send a report."
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Xiuqo.exe before writing the replacement,
    ...
    # DO NOT spawn Xiuqo.exe as our child: Electron/Chromium calls
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Xiuqo process.
        try {
...
# closes it. `xiuqo update` deliberately runs its build steps with stdout
# inherited (xiuqo_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:XIUQO_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:XIUQO_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:XIUQO_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:XIUQO_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `xiuqo update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (xiuqo_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:XIUQO_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:XIUQO_UPDATE_PROGRESS_LOG
}
...
function Invoke-HermesStep([string]$Exe, [string[]]$HermesArgs, [string]$Tag) {
    ...
    # afterwards, where `xiuqo debug share` picks it up.
    #
    ...
    # .xiuqo-update-result.json, clearing .xiuqo-update-in-progress,
    ...
    # Xiuqo" until the user killed something by hand. Losing the tail of a
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `xiuqo update` looks for 40+
...
# the page can be QA'd in any browser; XIUQO_SELFTEST_FAIL=1 exercises the
# error state, XIUQO_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:XIUQO_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:XIUQO_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:XIUQO_SELFTEST_FAIL) {
...
# The #90455 deadlock needs no update, no checkout and no Xiuqo install to
...
#            update log, the shape of every real `xiuqo update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:XIUQO_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:XIUQO_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:XIUQO_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:XIUQO_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "xiuqo-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "xiuqo-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "xiuqo-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "xiuqo-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "xiuqo-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "xiuqo-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "xiuqo-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "xiuqo-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `xiuqo update` is this shape -- the Electron/vite build
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:XIUQO_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    # Desktop launches us from XIUQO_HOME; pin the process directory to the
    ...
    Publish-UiProgress "Waiting for Xiuqo to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Xiuqo window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Xiuqo fully and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    Publish-UiProgress "Preparing Xiuqo files"
    $shim = Join-Path $InstallRoot "venv\Scripts\xiuqo.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Xiuqo install open (venv\Scripts\xiuqo.exe locked after 20s). Nothing was changed. Close other Xiuqo windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the xiuqo.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\xiuqo.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_hermes_exe must first rename the running xiuqo.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that xiuqo.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Xiuqo window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Xiuqo.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m xiuqo_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Xiuqo installer or `xiuqo doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "xiuqo_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Xiuqo windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m xiuqo_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed xiuqo predates --keep-stash; running without it"
        }
    } catch {
    ...
    Write-HandoffLog "xiuqo update exit code: $($res.Code)"
    ...
    # `xiuqo update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "xiuqo update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-HermesStep $pythonExe @("-m", "xiuqo_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        $verifyCode = "import xiuqo_cli.main; from xiuqo_cli.desktop_update_verify import verify_windows_desktop_update; verify_windows_desktop_update()"
        $verify = Invoke-HermesStep $pythonExe @("-c", $verifyCode) "verify"
        if ($verify.Code -ne 0) {
            $finalCode = 8
            $finalMsg = "The updated Xiuqo runtime or Desktop build failed verification. Repair the installation and review antivirus quarantine before retrying."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    }
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `xiuqo desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `xiuqo debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Xiuqo is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Xiuqo was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Xiuqo."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Xiuqo"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen Xiuqo to finish (it could not restart itself)."
