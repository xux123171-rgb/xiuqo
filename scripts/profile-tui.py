#!/usr/bin/env python3
"""Drive the Xiuqo TUI under XIUQO_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.xiuqo/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with XIUQO_TUI_RESUME set so it
bypasses the xiuqo_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  XIUQO_PERF_LOG     (default ~/.xiuqo/perf.log)
  XIUQO_PERF_NODE    (default node from $PATH)
  XIUQO_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests XIUQO_DEV_PERF wiring is broken).
"""
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("XIUQO_HOME") or "").strip()
        return Path(val) if val else Path.home() / ".xiuqo"

DEFAULT_TUI_DIR = Path(
    os.environ.get("XIUQO_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("XIUQO_PERF_LOG", str(get_hermes_home() / "perf.log")))
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — XIUQO_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["XIUQO_DEV_PERF"] = "1"
    env["XIUQO_DEV_PERF_MS"] = str(args.threshold_ms)
    env["XIUQO_DEV_PERF_LOG"] = str(log)
    env["XIUQO_TUI_RESUME"] = sid
    ...
    node = os.environ.get("XIUQO_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="XIUQO_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "xiuqo-ink" / "src"
