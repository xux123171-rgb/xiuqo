"""Shared constants for Xiuqo Agent.
...
_UNSET = object()
_XIUQO_HOME_OVERRIDE: ContextVar[str | object] = ContextVar("_XIUQO_HOME_OVERRIDE", default=_UNSET)
...
def set_hermes_home_override(path: str | Path | None) -> Token:
    """Set a context-local Xiuqo home override and return its reset token.

    Deliberately does not mutate ``os.environ`` (shared by every thread in the process).
    """
    value: str | object = _UNSET if path is None else str(path)
    return _XIUQO_HOME_OVERRIDE.set(value)


def reset_hermes_home_override(token: Token) -> None:
    """Restore the previous context-local Xiuqo home override."""
    _XIUQO_HOME_OVERRIDE.reset(token)


def get_hermes_home_override() -> str | None:
    """Return the active context-local Xiuqo home override, if any."""
    override = _XIUQO_HOME_OVERRIDE.get()
    return str(override) if override is not _UNSET and override else None


def _get_platform_default_hermes_home() -> Path:
    """Return the platform-native default Xiuqo home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "xiuqo"
    return Path.home() / ".xiuqo"


def sudo_invoker_default_home() -> Path | None:
    """The invoking user's native ``~/.xiuqo`` when this process is root under ``sudo``, else None.

    sudo strips XIUQO_HOME and sets HOME=/root, so the process's own default is root's; the profile
    ...
    try:
        return Path(pwd.getpwnam(sudo_user).pw_dir) / ".xiuqo"
    except KeyError:  # SUDO_USER not in passwd (chroot/container)
        return None


def _warn_profile_fallback_once() -> None:
    """Warn once when XIUQO_HOME is unset but a non-default profile is sticky-active (wrong fallback)."""
    ...
    if active and active != "default":
        ...
        msg = (
            f"[XIUQO_HOME fallback] XIUQO_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass XIUQO_HOME explicitly "
            f"(see issue #18594)."
        )
...
def get_hermes_home() -> Path:
    """Xiuqo home: context-local override → ``XIUQO_HOME`` env var → platform default."""
    override = get_hermes_home_override()
    if override:
        return Path(override)
    if not os.environ.get("XIUQO_HOME", "").strip():
...
def xiuqo_home_key(path: str | Path | None = None) -> str:
    """Stable registry key for a Xiuqo home/profile dir.
...
def get_process_hermes_home() -> Path:
    """Xiuqo home of the running process, ignoring task overrides.
...
# get_default_hermes_root() memo keyed on (native home, XIUQO_HOME) so it stays
# fresh when a test or plugin mutates XIUQO_HOME; saves ~80us/call at 31+ sites.
_default_hermes_root_memo: "tuple[str, str, Path] | None" = None


def get_default_hermes_root() -> Path:
    """Root Xiuqo dir for profile-level ops: ``<root>`` when ``XIUQO_HOME=<root>/profiles/<name>``."""
    global _default_hermes_root_memo
    native_home = _get_platform_default_hermes_home()
    env_home = os.environ.get("XIUQO_HOME", "")
    ...
    if env_home:
        ...
        try:
            env_path.resolve().relative_to(native_home.resolve())  # under ~/.xiuqo (normal or profile mode)
        except ValueError:  # Docker/custom root: <root>/profiles/<name> -> <root>, else XIUQO_HOME itself
...
# Files marking a real Xiuqo home; arbitrary dirs with a ``profiles`` segment lack them.
_XIUQO_HOME_MARKERS = ("config.yaml", ".env", "state.db")


def _is_hermes_profiles_root(profiles_dir: Path) -> bool:
    """True when *profiles_dir* is provably ``<xiuqo-home>/profiles``.

    Accepts the classic ``~/.xiuqo`` layout, a root carrying Xiuqo-home marker files, a
    ...
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir() or any(
            (root / marker).exists() for marker in _XIUQO_HOME_MARKERS
        ):
            return True
    except OSError:
        pass
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    Requires ``<name>`` not to start with ``.`` and the ``profiles`` parent to be a real Xiuqo home;
    ...
    for candidate in (current, *current.parents):
        if (candidate.parent.name == "profiles" and not candidate.name.startswith(".")
            ...
            return candidate
        if candidate.name == ".xiuqo":  # default home: a coincidental profiles/ ancestor is not a root
            return None
    return None
...
def profile_name_for_home(path: str | Path | None) -> str | None:
    ...
    The default home is the Xiuqo root itself, so its basename is an installation detail (``.xiuqo``
    on POSIX and commonly ``xiuqo`` on Windows), not the profile id ``default``.
    """
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    return _packaged_dir("XIUQO_OPTIONAL_SKILLS", default, "optional-skills")


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers (``XIUQO_OPTIONAL_MCPS``)."""
    return _packaged_dir("XIUQO_OPTIONAL_MCPS", default, "optional-mcps")


def get_bundled_skills_dir(default: Path | None = None) -> Path:
    """Return the bundled skills directory, honoring package-manager wrappers (``XIUQO_BUNDLED_SKILLS``)."""
    return _packaged_dir("XIUQO_BUNDLED_SKILLS", default, "skills")


def get_hermes_dir(new_subpath: str, old_name: str, *, home: Path | None = None) -> Path:
    """Resolve a Xiuqo subdirectory, honouring a populated legacy ``<old_name>/`` (no migration).
...
def iter_hermes_node_dirs(home: Path | None = None) -> list[Path]:
    """Xiuqo-managed Node dirs in lookup order; both Windows and POSIX shapes so migrated installs work.
...
def _version_probe_ok(path: str) -> bool:
    """True when ``<path> --version`` exits 0 under the Xiuqo-managed Node PATH."""
    result = _run_version_probe([path, "--version"], env=with_hermes_node_path())
    return result is not None and result.returncode == 0


_XIUQO_NODE_TARGET_MAJOR = int(os.environ.get("XIUQO_NODE_TARGET_MAJOR", "22"))
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/xiuqo_constants.py). Used by secure_parent_dir() to
# skip chmod on the install dir — chmodding it 0700 breaks xiuqo-user traversal in Docker (UID 10000). See
# #25821, #93050.
...
def xiuqo_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Xiuqo-managed node/npm/npx shim exists on disk."""
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Xiuqo-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `xiuqo update` afterwards).", flush=True,
    )
...
def _stage_windows_node_zip(home: Path, node_arch: str) -> Path | None:
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_XIUQO_NODE_TARGET_MAJOR}.x/"
    index_bytes = _fetch_url(index_url, 60)
    if index_bytes is None:
        return None
    pattern = rf"node-v{_XIUQO_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip"
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%XIUQO_HOME%\\node`` on Windows.
    ...
    (images are mapped with ``FILE_SHARE_DELETE`` — the same mechanism as the xiuqo.exe quarantine); when
...
def _run_node_bootstrap(func: str, *, timeout: int, **extra_env: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "XIUQO_HOME": str(get_hermes_home()), **extra_env},
...
def bootstrap_hermes_managed_node() -> str | None:
    """Install a Xiuqo-managed Node tree under ``$XIUQO_HOME/node`` and return its npm path.

    Xiuqo never modifies a user-owned toolchain (system, nvm, brew, Nix) that fails ``engines``.
    """
    ...
    if sys.platform == "win32":
        ...
        # XIUQO_NODE_SKIP_LINKS=1 keeps node/npm/npx out of ~/.local/bin: never shadow the user toolchain.
        ok = _run_node_bootstrap("_nb_install_bundled_node", timeout=600, XIUQO_NODE_SKIP_LINKS="1")
    if not ok:
        return None
...
def heal_hermes_managed_node() -> bool:
    """Redownload Xiuqo-managed Node when the tree exists but is broken; at most once per process.
    ...
    if _managed_node_heal_attempted or not xiuqo_managed_node_tree_present():
        return False
    if sys.platform == "win32":
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    for candidate in _iter_managed_node_candidates(_candidate_node_command_names("node"), home):
        ...
        if "-" in version:
            return True
        return major < _XIUQO_NODE_TARGET_MAJOR
    return False


def find_hermes_node_executable(command: str) -> str | None:
    """Xiuqo-managed Node/npm path, healing broken/outdated trees; heal failure still returns old Node."""
...
def find_node_executable(command: str) -> str | None:
    ...
    if managed:
        return managed
    if xiuqo_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_hermes_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Xiuqo-managed Node directories prepended to PATH."""
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    local ``node_modules/agent-browser/bin/...`` binary, which then disappears on the next ``xiuqo update``
...
def display_hermes_home() -> str:
    """User-facing ``~/`` display string for XIUQO_HOME (``~/.xiuqo/profiles/coder``)."""
    home = get_hermes_home()
    try:  # as_posix(): str() on Windows yields chimeras like ~/AppData\Local\xiuqo/skills/
...
def profile_cli_selector() -> str:
    """``-p <name> `` (trailing space) pinning copy-pasteable ``xiuqo ...`` guidance to the
    active NAMED profile, else ``""``: a bare ``xiuqo`` follows the sticky ``active_profile``
    file, which can name a different database than the one that failed (#105887). A custom
    home outside the profile tree has no selector (only XIUQO_HOME names it)."""
    name = profile_name_for_home(get_hermes_home())
    return f"-p {name} " if name and name != "default" else ""


def secure_parent_dir(path: Path) -> None:
    """Chmod ``0o700`` on *path*'s parent, refusing ``/`` and top-level dirs (misresolved XIUQO_HOME)."""
    parent = path.parent.resolve()
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree: chmod 0700 breaks xiuqo-user traversal in Docker (UID 10000).
    # A credential file here means XIUQO_HOME misresolved; surface it (caused production lockouts).
    # See #25821, #93050.
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        ...
        logging.getLogger(__name__).warning(
            ...
            "xiuqo install directory (%s). Credential files are "
            "normally stored under the xiuqo home directory instead.", parent, _INSTALL_ROOT,
        )
        return
    with contextlib.suppress(OSError):
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{XIUQO_HOME}/home`` when the profile-home directory exists."""
    xiuqo_home = get_hermes_home_override() or (env or {}).get("XIUQO_HOME") or os.getenv("XIUQO_HOME")
    if not xiuqo_home:
        return None
    profile_home = os.path.join(xiuqo_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates = [_env_get(env, "XIUQO_REAL_HOME"), _env_get(env, "HOME")]
    with contextlib.suppress(Exception):
        import pwd
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """The OS user's real home, avoiding the Xiuqo profile HOME.

    ``HOME`` belongs to the OS account and external CLIs keeping credentials under ``~``; a parent
    already running with ``HOME={XIUQO_HOME}/home`` is repaired back when possible.
    """
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    ``{XIUQO_HOME}/home``; ``real``: always real HOME; ``profile``: always the profile home.
    """
    env = env or {}
    profile_home = _profile_home_path(env)
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Xiuqo' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["XIUQO_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Map a Windows-host cwd (drive path or ``\\\\wsl.localhost\\`` UNC) to POSIX when Xiuqo runs in WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under XIUQO_HOME."""
    return get_hermes_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under XIUQO_HOME."""
    return get_hermes_home() / "skills"


def get_env_path() -> Path:
    """Return the path to the ``.env`` file under XIUQO_HOME."""
...
def openrouter_variant_base(model_id: str) -> str | None:
    ...
    Lives here rather than in ``xiuqo_cli.models`` so the metadata layer
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    Canonical helper for venv layout. This was open-coded in seven places across four ``xiuqo_cli`` modules
    ...
    lived 2400 lines away in another function. A few sites outside ``xiuqo_cli``
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* ships with Xiuqo (exact first segment; ``startswith`` would claim ``agentops``)."""
