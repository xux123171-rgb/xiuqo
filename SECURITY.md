# Xiuqo Agent Security Policy

This document describes Xiuqo Agent's trust model, names the one
...
- Environment details (`xiuqo --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Xiuqo Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Xiuqo Agent's documentation
...
Xiuqo Agent supports two OS-level isolation postures. They address
...
Xiuqo Agent supports this in two ways:

- **Xiuqo Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Xiuqo Agent's in-process heuristics
...
Xiuqo Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Xiuqo Agent
itself. Bugs in Xiuqo Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Xiuqo Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Xiuqo Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Xiuqo Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  xiuqo-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Xiuqo Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Xiuqo Agent. Bugs in Xiuqo
...
- Xiuqo Agent includes supply-chain guards for MCP server
