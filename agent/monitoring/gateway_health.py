...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "xiuqo.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("xiuqo.gateway.up", int(bool(gateway_running))),
        metric("xiuqo.gateway.active_agents", active_agents),
        metric("xiuqo.gateway.busy", int(bool(busy))),
        metric("xiuqo.gateway.drainable", int(bool(drainable))),
        metric("xiuqo.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("xiuqo.gateway.state", 1, **{"xiuqo.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"xiuqo.platform": str(platform), "xiuqo.platform.state": state}
        metrics.append(metric("xiuqo.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("xiuqo.platform.degraded", int(is_degraded), **pattrs, **{"xiuqo.error_code": error_code}))
        if is_degraded:
