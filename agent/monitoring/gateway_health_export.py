...
_OBSERVABLE_METRIC_NAMES = (
    "xiuqo.gateway.up", "xiuqo.gateway.state", "xiuqo.gateway.active_agents", "xiuqo.gateway.busy",
    "xiuqo.gateway.drainable", "xiuqo.gateway.restart_requested", "xiuqo.gateway.background_work",
    "xiuqo.gateway.background_delegations", "xiuqo.platform.up", "xiuqo.platform.degraded",
    "xiuqo.cron.scheduler.heartbeat_age_seconds", "xiuqo.cron.scheduler.last_success_age_seconds",
    "xiuqo.cron.scheduler.catch_up_occurrences", "xiuqo.cron.jobs.enabled", "xiuqo.cron.jobs.running",
    "xiuqo.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="xiuqo-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("xiuqo.gateway.background_work", _read_background_work_count),
            ("xiuqo.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("xiuqo.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("xiuqo.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'xiuqo[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="xiuqo-gateway-health-export", daemon=True)
            thread.start()
