...
The dispatcher sets ``XIUQO_TURN_AUTHOR`` on the recipient's one-shot subprocess only. A cached
...
TURN_AUTHOR_ENV = "XIUQO_TURN_AUTHOR"
...
def turn_author_from_env(environ: Mapping[str, str] = os.environ) -> Optional[Dict[str, Any]]:
    """The author the dispatcher placed in ``XIUQO_TURN_AUTHOR``, or None."""
...
def take_turn_author_from_env(environ: MutableMapping[str, str] = os.environ) -> Optional[Dict[str, Any]]:
    """Read and remove ``XIUQO_TURN_AUTHOR`` so subprocesses started during the turn do not inherit it."""
