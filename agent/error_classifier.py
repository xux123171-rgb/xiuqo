...
# Parameters Xiuqo sends on SOME routes only → hosts where that is deliberate.
...
def _plugin_verdict(c: _Ctx) -> Optional[Verdict]:
    ...
    try:
        from xiuqo_cli.plugins import get_plugin_error_classification
...
def _nous_welcome_tier(c: _Ctx) -> Optional[Verdict]:
    ...
    """
    from xiuqo_cli.anon_auth import (
