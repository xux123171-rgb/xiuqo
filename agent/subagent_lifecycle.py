"""Public, plugin-safe lifecycle API for delegated Xiuqo subagents: immutable contracts, not ``AIAgent``
...
_EXECUTOR = _DaemonExecutor(max_workers=8, thread_name_prefix="xiuqo-lifecycle")
_SECRET = secrets.token_bytes(32)
_ACTIVE_PARENT_AGENT: contextvars.ContextVar[Any] = contextvars.ContextVar("xiuqo_subagent_lifecycle_parent", default=None)
...
_REQUEST_REJECTIONS: tuple[tuple[Callable[[Any], bool], str], ...] = (
    ...
    (lambda r: r.working_directory is not None,
     "working_directory is not supported because Xiuqo delegates use isolated task environments."),
    (lambda r: bool(r.blocked_tools),
     "Per-tool blocking is not supported; use allowed_toolsets. Xiuqo always blocks unsafe child tools."),
)
...
class SubagentLifecycleService:
    ...
    def launch(self, request: SubagentLaunchRequest) -> SubagentHandle:
        ...
        if parent is None:
            raise SubagentLifecycleError("No active Xiuqo parent session is available.")
        ...
        if not subagent_id:
            raise SubagentLifecycleError("Xiuqo failed to assign a child identity.")
        created = time.time()
        handle = SubagentHandle(
