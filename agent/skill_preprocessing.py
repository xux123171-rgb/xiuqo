"""Shared SKILL.md preprocessing helpers: ``${XIUQO_*}`` template tokens and
...
from xiuqo_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags

logger = logging.getLogger(__name__)

# ${XIUQO_SKILL_DIR} / ${XIUQO_SESSION_ID} tokens. Unresolvable ones (e.g. no
# session) are left as-is so the author can spot them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(XIUQO_SKILL_DIR|XIUQO_SESSION_ID)\}")
...
def substitute_template_vars(content: str, skill_dir: Path | None, session_id: str | None) -> str:
    """Replace ${XIUQO_SKILL_DIR} / ${XIUQO_SESSION_ID}; tokens without a value stay in place."""
    if not content:
        return content
    values = {
        "XIUQO_SKILL_DIR": str(skill_dir) if skill_dir else None,
        "XIUQO_SESSION_ID": str(session_id) if session_id else None,
    }
