...
- Follows the repo's "default frictionless, 0600 files OK" policy: the key
  file and vault file are created 0600 under ``<XIUQO_HOME>/vault/``.
...
# browser fill targets (agent/vault_login_classifier.py); the Desktop Add dialog and `xiuqo vault add`
...
class VaultStore:
    """Encrypted, profile-scoped vault under ``<XIUQO_HOME>/vault/``."""
    ...
    def _ensure_dir(self) -> None:
        ...
        # group-share mode and XIUQO_UID/GID ownership) rather than a
    ...
    def _locked(self):
        ...
        `xiuqo vault add` and a TUI slash worker all write the same ``vault.json.enc``; two unlocked
