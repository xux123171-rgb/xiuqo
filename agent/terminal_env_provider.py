...
class TerminalEnvironmentProvider(ProviderBase):
    ...
    * ``cache_path_base`` — where auto-synced ``~/.xiuqo/cache`` files land inside the backend
      (``"~/.xiuqo"``, ``"/root/.xiuqo"``), or ``None`` when host paths remain correct.
    ...
    * ``session_isolated_when_nonpersistent`` — non-persistent mode gives each session its own
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``xiuqo setup`` after selection (the wizard persists ``terminal.backend`` itself)."""
        return []

    def post_setup(self) -> None:
        """Optional interactive hook run by ``xiuqo setup`` after selection (prompt for tokens, install SDKs)."""

    def doctor_checks(self) -> List[Tuple[bool, str, str]]:
        """``xiuqo doctor`` rows ``(ok, label, detail)``; default reflects :meth:`is_available`."""
        try:
