"""Unified removal contract for every credential source Xiuqo reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``xiuqo auth remove <provider> <N>`` must make the entry stay gone across
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.xiuqo/.env; hint when the shell exports it."""
    from xiuqo_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like xiuqo_cli/config.py: a BOM-sensitive read would
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Xiuqo.",
            f"  The pool entry is now suppressed — Xiuqo will ignore "
            f"{env_var} until you run `xiuqo auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    """~/.xiuqo/.anthropic_oauth.json is ours — delete it outright."""
    ...
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Xiuqo Anthropic OAuth credentials")
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `xiuqo model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `xiuqo auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `xiuqo auth add copilot` to re-enable if needed.",
    ])
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `xiuqo auth add anthropic` to re-enable if needed.",
        ),
        description="~/.claude/.credentials.json",
    ),
    RemovalStep(
        provider="anthropic", source_id="xiuqo_pkce",
        remove_fn=_remove_hermes_pkce,
        description="~/.xiuqo/.anthropic_oauth.json",
    ),
    RemovalStep(
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `xiuqo auth add qwen-oauth` to re-enable if needed.",
        ),
