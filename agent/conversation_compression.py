...
# Defaults for the in-agent progress-aware wrap; mirror xiuqo_cli.config.DEFAULT_CONFIG["compression"] keys.
...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    Only the exact old ``xiuqo_state.SessionDB`` class (hot-reload skew) may fail open; proxies, lookalikes,
...
def _rebind_session_context(session_id: str) -> None:
    ...
    with contextlib.suppress(Exception):
        from xiuqo_logging import set_session_context
...
def _lower_threshold_to_aux_context(
    ...
    if threshold_suggestion_viable:
        ...
        msg += (
            ...
            f"Xiuqo's small-context floor and output reservation would recompute the trigger to "
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "Run `xiuqo setup` or set OPENROUTER_API_KEY."
                )
            agent._compression_warning = msg
            agent._emit_status(msg)
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Xiuqo "
        ...
        if _lock_lookup_error is not None:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "in-memory module after an update; restart the process (or `xiuqo update`) to resync.",
                    _lock_sid,
                )
...
def _parent_deliberately_ended(session_db: Any, session_id: str) -> bool:
    ...
    if not callable(reader):
        return False
    try:
        from xiuqo_state_common import is_automatic_end_reason
...
def _carry_session_state_to_child(agent: Any, old_session_id: str, old_title: Any) -> None:
    ...
    with _swallow('Could not migrate goal on compression: %s'):
        ...
        from xiuqo_cli.goals import migrate_goal_to_session
        migrate_goal_to_session(old_session_id, agent.session_id, reason="compression")
    with _swallow('Could not migrate heartbeat on compression: %s'):
        from xiuqo_cli.heartbeat import migrate_heartbeat_to_session
...
def _publish_rotated_compaction(
    ...
    # publish also COALESCEs from the parent row for threads lacking XIUQO_HOME.
    ...
    agent._session_db.publish_compression_child(
        ...
        source=agent.platform or os.environ.get("XIUQO_SESSION_SOURCE", "cli"), model=agent.model,
...
def compress_context(
    ...
    # compression.codex_app_server_auto). Memory handoff is Xiuqo-only: no native
...
def _compress_context_via_codex_app_server(
    ...
    Xiuqo' transcript is left unchanged."""
    ...
    if auto_mode not in {"native", "xiuqo", "off"}:
        auto_mode = "native"
    skip_reason = None
    if not force and auto_mode != "xiuqo":
...
def _shrink_data_url(url: str, *, max_dimension: int, resize_fn: Any) -> tuple:
    ...
    try:
        ...
        tmp = tempfile.NamedTemporaryFile(
            prefix="xiuqo_shrink_", suffix=_IMAGE_SUFFIX_BY_MIME.get(mime, ".jpg"), delete=False
        )
        try:
            tmp.write(raw)
            tmp.close()
