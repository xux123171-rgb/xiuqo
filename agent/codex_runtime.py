...
def _record_codex_app_server_usage(agent, turn, messages=None) -> dict[str, Any]:
    """Translate Codex app-server token usage into Xiuqo accounting. Prompt bucket = uncached + cached
    ...
    estimate grows monotonically and xiuqo-mode fires thread compaction on tiny threads (#100381)."""
...
def _record_codex_app_server_compaction(agent, turn, *, approx_tokens: int | None = None, force: bool = False) -> bool:
    ...
    if compressor is not None:
        ...
        # Codex owns this summary: a prior Xiuqo deterministic-fallback flag must not leak into it.
...
# --- Codex app-server → Xiuqo UI bridge -------------------------------------
# The app-server bypasses the Xiuqo tool loop, so the bridge translates JSON-RPC notifications
# into the callbacks the standard runtime fires (tool_progress_callback, _fire_stream_delta, ...).

# Item types that project to a Xiuqo tool_call (keep in sync with agent/transports/codex_event_projector.py
...
# Internal MCP server wrapping Xiuqo' native tools: its inner dispatch has no tool_progress_callback, so the
# codex-level mcpToolCall IS the display event and the mcp.xiuqo-tools.* prefix is stripped (users see Xiuqo tools).
_INTERNAL_MCP_SERVER = "xiuqo-tools"
...
def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Xiuqo tool name for a codex item (mirrors CodexEventProjector)."""
...
def _ensure_codex_session(agent) -> None:
    ...
    # Approval callback: Xiuqo' standard prompt flow when a CLI thread installed one.
    ...
    # explicit approval bypass (approvals.mode: off, /yolo, --yolo, XIUQO_YOLO_MODE) hands policy to codex's sandbox.
    ...
    # Xiuqo' gateway UI callbacks (tool_progress_callback, _fire_stream_delta,
...
def _bypass_sdk_request_transform(stream_kwargs: dict) -> dict:
    ...
    request without the walk. XIUQO_CODEX_SDK_TRANSFORM=1 disables."""
    if os.environ.get("XIUQO_CODEX_SDK_TRANSFORM", "").strip().lower() in {"1", "true", "yes", "on"}:
