...
_PERSISTENCE_CAUSE_EXPLANATIONS: Dict[str, str] = {
    ...
    "turn_lease": (
        "the turn was stopped because another Xiuqo process "
    ...
    "locked": (
        ...
        "(another Xiuqo process was writing to the state "
    ...
    "replaced": (
        ...
        "`xiuqo doctor --fix` or in-place FTS repair — stop "
    ...
    "deleted_wal": (
        "the turn was stopped because a live Xiuqo process held a retired "
        ...
        "Check the logs for whether Xiuqo captured the retired generation, "
        "then read the adjacent state.db.retired-wal-*/manifest.json. If "
        "manifest.main.mode is `copied`, inspect that artifact with `xiuqo "
    ...
    "corrupt": (
        ...
        "1. Run `xiuqo {profile_arg}doctor --fix`\n"
        "2. Stop the gateway, then recover with:\n"
        "   xiuqo {profile_arg}sessions recover --source {db_path} --inspect-only\n"
        "   (if it reports recoverable) xiuqo {profile_arg}sessions recover "
    ...
    "fts_index": (
        ...
        "recovery tools or restore a backup. Run `xiuqo {profile_arg}doctor --fix` "
        "(or restart Xiuqo, which repairs the index on open), then "
...
_PERSISTENCE_DEFAULT_EXPLANATION = (
    ...
    "Check the state database health (`xiuqo doctor`), then "
    "send your message again."
)
...
def _display_flag_enabled(agent, *, env_var: str, config_key: str, cache_attr: str) -> bool:
    ...
    and tests patch it at ``xiuqo_cli.config``). Any failure → True (safe default: on)."""
    try:
...
class TurnExplainersMixin:
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # Xiuqo-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        """``display.file_mutation_verifier`` / ``XIUQO_FILE_MUTATION_VERIFIER`` (a patchable seam)."""
        return _display_flag_enabled(
            self, env_var="XIUQO_FILE_MUTATION_VERIFIER", config_key="file_mutation_verifier",
            cache_attr="_file_mutation_verifier_enabled_cache",
        )

    def _turn_completion_explainer_enabled(self) -> bool:
        """``display.turn_completion_explainer`` / ``XIUQO_TURN_COMPLETION_EXPLAINER``."""
        return _display_flag_enabled(
            self, env_var="XIUQO_TURN_COMPLETION_EXPLAINER", config_key="turn_completion_explainer",
    ...
    def _format_turn_completion_explanation(
        ...
        if body is None and reason == "session_persistence_failed":
            ...
            if persistence_cause in ("corrupt", "fts_index"):
                ...
                # the process default, and a bare `xiuqo` follows active_profile (#105887).
