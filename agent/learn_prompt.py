...
``skill_manage`` per the Xiuqo authoring standards; large prose sources get the knowledge-base layout (lean SKILL.md
...
Follow the Xiuqo skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Xiuqo`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Xiuqo.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.xiuqo.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through Xiuqo tools.
...
Xiuqo-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference Xiuqo tools by name in backticks: `terminal`, `read_file`,
