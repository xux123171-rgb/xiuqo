...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in xiuqo_state.py; kept local so importing
# xiuqo_state (module-level DEFAULT_DB_PATH) is not forced at load time.
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Xiuqo tool use: model=%s provider=%s base_url=%s "
    ...
    return (
        f"Ollama loaded `{model}` with only {runtime_ctx:,} tokens of runtime context, but Xiuqo "
        ...
        "again. A known-good starting point is 65,536 tokens. In Xiuqo config, set "
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            return "\n".join([
                ...
                "Retry with a fresh credential state: `xiuqo auth reset anthropic`. Until that "
...
def _decode_inline_moa_turn(user_message, persist_user_message):
    ...
    try:
        from xiuqo_cli.moa_config import decode_moa_turn
...
def _run_conversation_turn(
    ...
    # Adopt ~/.xiuqo/.env credential/base-url edits made since the last turn — a
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'PARTIAL_STREAM_STUB_ID': ('xiuqo_constants', 'PARTIAL_STREAM_STUB_ID'),
