...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: ``XIUQO_VERIFY_ON_STOP`` env var, then ``agent.verify_on_stop``
    ...
    """
    env = os.environ.get("XIUQO_VERIFY_ON_STOP")
    if env is not None:
...
def _workspace_has_runnable_recipe(root: Any) -> bool:
    """Whether ``xiuqo verify`` has a runtime recipe here: a saved
    ``.xiuqo/environment.json`` or a statically detected recipe with a start
    ...
    if verify_commands:
        ...
        if has_recipe:
            command_instruction += (
                ...
                "start + readiness), prefer `xiuqo verify --json` — a passing "
                "run records verification evidence for this workspace."
            )
    elif has_recipe:
        command_instruction = (
            "No canonical test/lint/build command was detected, but the "
            "project has a runnable verification recipe. Run `xiuqo verify "
        ...
        command_instruction = (
            ...
            "`tempfile` path with a `xiuqo-verify-` filename prefix, run it "
