"""Core NeMo Relay adapters for physical Xiuqo provider attempts."""
...
def _relay_operation_name(provider_name: str, metadata: dict[str, Any] | None) -> str:
    """Return Relay's canonical operation name when Xiuqo knows the API mode."""
...
def _relay_metadata(provider_name: str, metadata: dict[str, Any] | None) -> dict[str, Any]:
    ...
    relay_metadata.setdefault("xiuqo.provider", provider_name)
...
class _ManagedAttempt:
    ...
    def run_callback(self, callback: Callable[..., Any], *args: Any) -> Any:
        """Run a Xiuqo callback in a fresh copy of the captured context.
        ...
        def guarded() -> Any:
            # See #77244.
            # See #77244.
            # Xiuqo-side callbacks run while the native pipeline drives this stream; nested relay calls
...
def _current_session_id() -> str | None:
    """Return the inherited Xiuqo turn's session id, or None outside a live turn."""
...
def execute(
    ...
    ``session_id`` defaults to the inherited Xiuqo turn's session (unmanaged when there is none)."""
...
# Run under the inherited Xiuqo turn when present (callers that do not know a session id).
...
def stream_current(
    ...
    """Run a provider stream under the inherited Xiuqo turn when present.
...
class AnthropicStreamAccumulator:
    ...
    def response(self, base: Any = None) -> Any:
        """Return the attribute-shaped response consumed by Xiuqo."""
        ...
        with turn.logical_llm_lock:
            ...
            if handle is None:
                ...
                handle = turn.logical_llm_calls[request_id] = runtime.run_in_session(
                    ...
                    metadata=relay_runtime.runtime_metadata(runtime.runtime_id, **{"xiuqo.call_role": call_role}),
                )
...
def _complete_logical(
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning("Xiuqo Relay logical LLM finalization failed", exc_info=True)
            return
        with turn.logical_llm_lock:
