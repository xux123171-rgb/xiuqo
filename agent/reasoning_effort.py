...
Xiuqo' internal effort ladder (``VALID_REASONING_EFFORTS`` plus ``none``) is wider than any
...
# disable can be clamped when a provider publishes it as a level. ``ultra`` is Xiuqo-internal
...
def is_astra_model(model: Optional[str]) -> bool:
    """``gpt-6-astra`` or its Xiuqo-side ``-900k`` picker alias, with or without a ``vendor/`` prefix.
