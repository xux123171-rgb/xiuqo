...
_GLOBAL_ENV_EXACT = frozenset({
    # Xiuqo runtime / deployment
    "XIUQO_HOME", "XIUQO_PROFILE", "XIUQO_GATEWAY_LOCK_DIR",
    "XIUQO_MAX_ITERATIONS", "XIUQO_API_TIMEOUT",
    "XIUQO_REDACT_SECRETS", "XIUQO_NOUS_TIMEOUT_SECONDS",
    "_XIUQO_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "XIUQO_KANBAN_DB", "XIUQO_KANBAN_WORKSPACES_ROOT", "XIUQO_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "XIUQO_KANBAN_",
    "XIUQO_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Xiuqo writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    """THE ``.env`` tokenizer: every reader (profile scope, ``xiuqo_cli.config.load_env``, the dashboard
...
def build_profile_secret_scope(xiuqo_home: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(xiuqo_home) / ".env")
    try:
        from xiuqo_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(xiuqo_home))
    except Exception:
        external_secrets = {}
