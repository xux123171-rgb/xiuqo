...
def scoped_aws_session_kwargs() -> Dict[str, str]:
    ...
    Under a XIUQO_HOME override the process env holds the LAUNCH profile's ``AWS_*`` (or nothing), so
...
def _require_boto3():
    ...
    try:
        ...
        raise ImportError(
            ...
            "Or install Xiuqo with Bedrock support: pip install -e '.[bedrock]'"
        )
    try:
...
def _cached_client(cache: Dict[str, Any], service: str, region: str):
    ...
    if get_hermes_home_override() is None:
        ...
        return cache[region]
    key = (xiuqo_home_key(), service, region)
...
def invalidate_runtime_client(region: str) -> bool:
    ...
    if get_hermes_home_override() is not None:
        return _bedrock_clients_by_home.pop((xiuqo_home_key(), "bedrock-runtime", region), None) is not None
...
def discover_bedrock_models(region: str, provider_filter: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    ...
    if get_hermes_home_override() is not None:
        cache_key = f"{xiuqo_home_key()}|{cache_key}"
