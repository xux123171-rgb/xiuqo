...
try:
    import xiuqo_cli as _xiuqo_cli

    _XIUQO_VERSION = str(_xiuqo_cli.__version__)
except Exception:
    _XIUQO_VERSION = "0.0.0"
_API_CLIENT = f"xiuqo/{_XIUQO_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Xiuqo typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.xiuqo/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.xiuqo/.env (get one at https://aistudio.google.com/app/apikey), or run `xiuqo setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Xiuqo retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No XIUQO_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
