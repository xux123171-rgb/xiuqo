...
from xiuqo_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from xiuqo_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
...
def _provider_preferences_for_agent(agent) -> Dict[str, Any]:
    ...
    with contextlib.suppress(Exception):
        ...
        from xiuqo_constants import resolve_per_model_provider_routing
...
def _check_stale_giveup(agent) -> None:
    ...
    _giveup = env_int("XIUQO_STREAM_STALE_GIVEUP", 5)
...
def _configured_stale_base(agent) -> float:
    """Per-provider ``stale_timeout_seconds`` config, else XIUQO_STREAM_STALE_TIMEOUT (180s)."""
    cfg = get_provider_stale_timeout(agent.provider, agent.model)
    return cfg if cfg is not None else env_float("XIUQO_STREAM_STALE_TIMEOUT", 180.0)
...
def _bedrock_converse_call(api_kwargs: dict, *, stream: bool, on_stream_denied=None):
    """Pop the Xiuqo routing keys and call ``converse`` / ``converse_stream`` (boto3
...
def _managed_local_load_notice(agent, api_kwargs: dict) -> "Optional[str]":
    ...
    try:
        ...
        from xiuqo_cli.local_runtime.load_progress import get_loading_progress, get_prefill_progress
...
def _resolve_nonstream_watchdogs(agent, api_kwargs: dict) -> _NonStreamWatchdogs:
    ...
    XIUQO_CODEX_TTFB_TIMEOUT_SECONDS,
    XIUQO_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each),
    XIUQO_CODEX_TTFB_DISABLE_ABOVE_TOKENS / XIUQO_CODEX_TTFB_STRICT,
    XIUQO_CODEX_TTFB_MAX_SECONDS, XIUQO_CODEX_HARD_TIMEOUT_SECONDS.
    """
    ...
    if codex and openai_codex_backend:
        ...
        hard_timeout = env_float("XIUQO_CODEX_HARD_TIMEOUT_SECONDS", 1500.0)
        if hard_timeout > 0:
    ...
    ttfb_enabled = codex
    ttfb_timeout = env_float("XIUQO_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if ttfb_timeout <= 0:
        ttfb_enabled = False
        ...
        disable_above = env_float("XIUQO_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 10_000.0)
        strict = os.environ.get("XIUQO_CODEX_TTFB_STRICT", "").strip().lower() in {"1", "true", "yes", "on"}
        if not strict and disable_above > 0 and est_tokens >= disable_above and ttfb_timeout < idle_default:
            logger.info("Scaling openai-codex no-event TTFB watchdog from %.0fs to %.0fs "
                ...
                "Set XIUQO_CODEX_TTFB_STRICT=1 to keep the smaller cutoff.", ttfb_timeout, idle_default,
                f"{est_tokens:,}", disable_above)
            ttfb_timeout = idle_default
        ttfb_cap = env_float("XIUQO_CODEX_TTFB_MAX_SECONDS", 120.0)
        if ttfb_cap > 0 and ttfb_timeout > ttfb_cap:
            logger.info("Capping openai-codex no-event TTFB timeout from %.0fs to %.0fs "
                "(context=~%s tokens). Set XIUQO_CODEX_TTFB_MAX_SECONDS to tune.", ttfb_timeout, ttfb_cap,
                f"{est_tokens:,}")
    ...
    idle_explicit = env_float("XIUQO_CODEX_EVENT_STALE_TIMEOUT_SECONDS", -1.0) != -1.0
    idle_timeout = env_float("XIUQO_CODEX_EVENT_STALE_TIMEOUT_SECONDS", idle_default)
...
def _build_chat_completions_kwargs(agent, api_messages, tools_for_api, reasoning_config, request_overrides, cache_scope_id):
    ...
    _qwen_meta = {"sessionId": agent.session_id or "xiuqo", "promptId": str(uuid.uuid4())} if _is_qwen else None
    _profile = None
    with contextlib.suppress(Exception):
        from providers import get_provider_profile
...
def _assistant_content_for_storage(agent, assistant_message):
    ...
    # history / state.db / gateway delivery (no-op with XIUQO_REDACT_SECRETS off).
...
def _is_anthropic_wire_url(url: str) -> bool:
    ...
    from xiuqo_cli.providers import host_mandated_api_mode
...
def _iteration_summary_api_messages(agent, messages: list) -> list:
    ...
    for msg in messages:
        ...
        # every Xiuqo-internal underscore-prefixed scaffolding key.
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    if getattr(agent, "suppress_status_output", False):
        ...
        # Strict machine-readable mode (xiuqo chat -Q, oneshot, background review): keep diagnostics out of
...
class _BedrockStream:
    ...
    def _on_stale(self, stale_elapsed: float) -> None:
        ...
        self.last_event = time.time()
        # Raises RuntimeError past XIUQO_STREAM_STALE_GIVEUP; otherwise end
...
class _StreamingCall(StreamingWaitMonitor):
    ...
    def _stream_timeouts(self) -> tuple[float, float, float]:
        ...
        ``request_timeout_seconds`` wins over XIUQO_API_TIMEOUT (1800s) and
        XIUQO_STREAM_READ_TIMEOUT (120s); connect/pool cover the handshake, not
        inference: 30s, or capped at 60s when configured."""
        cfg = get_provider_request_timeout(self.agent.provider, self.agent.model)
        base = cfg if cfg is not None else env_float("XIUQO_API_TIMEOUT", 1800.0)
        if cfg is not None:
            return base, cfg, min(base, 60.0)
        read = env_float("XIUQO_STREAM_READ_TIMEOUT", 120.0)
    ...
    def _call_chat_completions(self, stream_attempt_id: int):
        ...
        if self.agent.provider == "moa":
            # Xiuqo interrupts the managed stream; Relay alone closes the provider stream.
    ...
    def _call(self):
        _max_stream_retries = env_int("XIUQO_STREAM_RETRIES", 2)
        # The one stream_options compatibility retry (#9705) is not a network retry and must not
        # consume the transient budget: on the last attempt (or XIUQO_STREAM_RETRIES=0) the
    ...
    def _resolve_stale_timeout(self) -> None:
        ...
        XIUQO_LOCAL_STREAM_STALE_TIMEOUT — an infinite one stalled sessions on a
        ...
        if base == 180.0 and self.agent.base_url and is_local_endpoint(self.agent.base_url):
            ...
            self._stream_stale_timeout = env_float("XIUQO_LOCAL_STREAM_STALE_TIMEOUT", _local_default)
