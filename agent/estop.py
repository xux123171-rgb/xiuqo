...
``xiuqo pause`` writes a sentinel at ``$XIUQO_HOME/ESTOP``; ``xiuqo resume``
...
(fail safe, e.g. ``touch ~/.xiuqo/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.xiuqo).
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `xiuqo pause`."""
...
def _candidate_sentinel_paths() -> list:
    ...
    gateway (XIUQO_HOME=~/.xiuqo/profiles/<n>) must still honor an operator's ~/.xiuqo/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Xiuqo is paused{tag}. New work is on hold; run `xiuqo resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `xiuqo resume` (%s)",
