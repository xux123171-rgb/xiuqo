"""Anthropic Messages API adapter: client construction + the Messages call for Xiuqo's
...
# Xiuqo effort -> Anthropic adaptive-thinking effort (output_config.effort). 4.7+ exposes
...
# Anthropic's OAuth billing classifier fingerprints certain Xiuqo tool schemas/prose as a
...
def _attribution_headers() -> Dict[str, str]:
    ...
    return {
        "HTTP-Referer": "https://hermes-agent.nousresearch.com", "X-Title": "Xiuqo Agent",
...
def _base_client_kwargs(base_url, timeout) -> tuple[str, Dict[str, Any]]:
    ...
    xiuqo's outer loop (``max_retries=0``): the SDK default of 2 uses its own backoff that ignores
...
def _new_sdk_client(sdk, kwargs: Dict[str, Any], headers: Dict[str, str]):
    ...
    ANTHROPIC_AUTH_TOKEN in the environment (both loaded from ~/.xiuqo/.env) and then sends dual
...
def build_anthropic_bedrock_client(region: str):
    ...
    return sdk.AnthropicBedrock(
        ...
        max_retries=0,  # retry belongs to xiuqo's outer loop (honors Retry-After)
...
def _normalize_to_mcp_wire(name: str) -> str:
    ...
    Xiuqo tools (``read_file``) and native MCP tools registered as ``mcp_<server>_<tool>`` must
...
_OAUTH_SYSTEM_REPLACEMENTS = (
    ("Xiuqo Agent", "Claude Code"), ("Xiuqo agent", "Claude Code"),
    ("xiuqo", "claude-code"), ("Nous Research", "Anthropic"),
)
...
def _thinking_kwargs(reasoning_config: Dict[str, Any], model: str, effective_max_tokens: int) -> Dict[str, Any]:
    ...
    4.7+ ``thinking.display`` defaults to "omitted", hiding the reasoning Xiuqo shows in its CLI,
...
def build_anthropic_kwargs(
    ...
    # which silently hides reasoning text that Xiuqo surfaces in its CLI. We request "summarized" so the
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_hermes_home': ('xiuqo_constants', 'get_hermes_home'),
