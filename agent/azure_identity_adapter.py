...
class EntraIdentityConfig:
    """Xiuqo-managed Entra knobs; everything else (tenant, SP secret, federated token file, authority...) flows
...
def _default_chain_credential(config: EntraIdentityConfig) -> Any:
    ...
    one ``model.entra.*`` block at a time. Only Xiuqo knobs are passed as kwargs; the rest comes from ``AZURE_*``
    env vars."""
    ...
    def _inject_bearer(request: "httpx.Request") -> None:
        try:
            ...
            logger.warning("Bearer hook: Entra ID token provider returned empty (%s) "
                           ...
                           "Run `xiuqo doctor` or `az login` to recover.", exc)
