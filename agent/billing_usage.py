...
def build_usage_model(*, timeout: float = 10.0) -> UsageModel:
    """Fetch account-info and build the usage model; fail-open. ``XIUQO_DEV_CREDITS_FIXTURE`` short-circuits to a fixture."""
...
def _dev_fixture_usage_model() -> Optional[UsageModel]:
    """``XIUQO_DEV_CREDITS_FIXTURE`` -> fixture model (``free|healthy|low|topup|depleted``), else None."""
    name = (os.getenv("XIUQO_DEV_CREDITS_FIXTURE") or "").strip().lower()
