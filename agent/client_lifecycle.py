...
def _routermint_headers() -> dict:
    ...
    return {"User-Agent": f"XiuqoAgent/{_XIUQO_VERSION}"}
...
_ROUTE_DEFAULT_HEADERS = (
    ...
    ("githubcopilot.com", lambda self, url: _lazy_attr("xiuqo_cli.models", "copilot_default_headers")()),
    ...
    ("x.ai", lambda self, url: _lazy_attr("tools.xai_http", "xiuqo_xai_default_headers")()),
)
...
class ClientLifecycleMixin:
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from xiuqo_cli import auth as _auth
            resolve = (
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True, require_account: str | None = None) -> bool:
        ...
        try:
            ...
            timeout = env_float("XIUQO_NOUS_TIMEOUT_SECONDS", 15)
    ...
    def _resolve_env_credentials(self) -> Optional[tuple]:
        ...
        if pconfig and getattr(pconfig, "auth_type", "") == "api_key" and getattr(pconfig, "api_key_env_vars", ()):
            ...
            if self.provider == "actual":
                ...
                from xiuqo_cli.runtime_provider import _config_base_url_for_provider, _get_model_config
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ``~/.xiuqo/.env`` credential/base-url edits at the turn boundary (a Settings save updates ``.env``
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        if not self._is_copilot_provider():
            return False
        try:
            from xiuqo_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token, evict_cached_exchanged_token
    ...
    def _apply_client_headers_for_base_url(self, base_url: str, *, apply_user_headers: bool = True) -> None:
        ...
        if self.api_mode not in ("anthropic_messages", "bedrock_converse"):
            try:
                from xiuqo_cli.config import apply_custom_provider_extra_headers_to_client_kwargs
    ...
    def _swap_credential(self, entry) -> bool:
        ...
        from xiuqo_cli.anon_auth import route_can_serve_model
