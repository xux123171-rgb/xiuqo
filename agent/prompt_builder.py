...
def _find_hermes_md(cwd: Path) -> Optional[Path]:
    """Nearest ``.xiuqo.md`` / ``XIUQO.md`` from *cwd* up to the git root, else None."""
    ...
    for directory in [current, *current.parents] if stop_at else [current]:
        found = next((directory / n for n in (".xiuqo.md", "XIUQO.md") if (directory / n).is_file()), None)
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are Xiuqo Agent, an open-source terminal AI agent. Be direct: match the length of your reply to the weight of the ask "
...
XIUQO_AGENT_HELP_GUIDANCE = (
    # Injected only when skill_view exists AND the xiuqo skill is installed (system_prompt.py slot
    # resolution). No "when the two differ" clause: docs-are-authoritative already carries the precedence.
    "You run on Xiuqo Agent. When the user needs help with Xiuqo itself — configuring, "
    ...
    "authoritative reference and always holds the latest, most up-to-date information. The `xiuqo` "
    "skill has the actual commands and proven workflows — load it with skill_view(name='xiuqo') "
    "before configuring, modifying, or troubleshooting Xiuqo so you don't guess or invent workarounds."
)

# Variant for sessions without the skills toolset (e.g. Blank Slate): naming skill_view() there would dangle.
XIUQO_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on Xiuqo Agent. When the user needs help with Xiuqo itself — configuring, "
    "setting up, using, extending, or troubleshooting it — or when you need to understand your own features, "
...
KANBAN_GUIDANCE = (
    ...
    "You have been assigned ONE task from the shared board at `~/.xiuqo/kanban.db`. Your task id is in "
    "`$XIUQO_KANBAN_TASK`; your workspace is `$XIUQO_KANBAN_WORKSPACE`. The `kanban_*` tools in your schema are your "
    ...
    "2. **Work inside the workspace.** `cd $XIUQO_KANBAN_WORKSPACE` before any file operations. The workspace is "
    ...
    "- **Workspace.** `cd $XIUQO_KANBAN_WORKSPACE` first. For a `worktree` kind with no `.git`, `git worktree add "
    "<path> ${XIUQO_KANBAN_BRANCH:-wt/$XIUQO_KANBAN_TASK}` from the main repo, then cd there. For a project-linked "
    "task the workspace is a fresh `<repo>/.worktrees/<task-id>` and `$XIUQO_KANBAN_BRANCH` a deterministic "
    ...
    "sits in `ready` forever). Ground every assignee in a real profile (`xiuqo profile list`, or ask the user), and "
    "express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n\n"
    "- Do not shell out to `xiuqo kanban <verb>` for board operations. Use the `kanban_*` tools — they work across "
...
# cost that compounds over a long conversation. The xiuqo runtime already executes a batch of tool
...
# to xiuqo's Python prompt-assembly architecture.
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Xiuqo appends their message to the end of a tool result, wrapped exactly as:\n"
    f"{STEER_MARKER_OPEN}\n<their message>\n{STEER_MARKER_CLOSE}\n"
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    gated = (
        (True,
         "[Note: this message came from HUD mode — a small floating Xiuqo "
         ...
         "rather than anything inside Xiuqo. read_window_below identifies that app."),
        (True,
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Xiuqo terminal UI (TUI). Files: there is no attachment channel and MEDIA:/path tags "
    ...
    "desktop": (
        ...
        "You are chatting inside the Xiuqo desktop app, a graphical chat surface. Markdown renders with full GitHub "
        ...
        "or it measures full-bleed. Widgets talk back: data-xiuqo-send=\"prompt\" on any clickable element (or "
        "window.xiuqo.send(\"prompt\")) sends that prompt as a hidden user turn — answer it by updating the widget's "
        "file, not with prose."
    ),
    "sms": (
...
def _run_backend_probe(env_type: str, terminal_tool) -> str:
    ...
    env = _create_environment(
        ...
        # snapshot. A normal SSHEnvironment would upload the whole ~/.xiuqo tree just to run `uname`,
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    cache_key = (xiuqo_home_key(), env_type, _tenv_read("TERMINAL_CWD", ""))
...
def _remote_backend_hint(backend: str) -> str:
    ...
    if probe:
        return lead + (
            f"this {backend} environment — NOT on the machine where Xiuqo itself is running. The host OS, "
            f"home, and cwd of the Xiuqo process are irrelevant; only the following backend state matters:\n{probe}"
        )
    description = (
    ...
    return lead + (
        f"{description} — NOT on the machine where Xiuqo itself runs. The backend probe didn't respond at "
...
def _embedder_environment_hint() -> str:
    """Embedder-supplied environment description: XIUQO_ENVIRONMENT_HINT (container ENV)
    wins over config.yaml ``agent.environment_hint``. Read once at prompt-build time."""
    return (os.getenv("XIUQO_ENVIRONMENT_HINT") or "").strip() or str(
...
RUNTIME_ENVIRONMENT_HEADING = "# Xiuqo runtime environment"
RUNTIME_ENVIRONMENT_END = "<!-- End Xiuqo runtime environment -->"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("XIUQO_PLATFORM") or os.environ.get("XIUQO_SESSION_PLATFORM")
    if platform:
        return platform
    get_session_env = getattr(sys.modules.get("gateway.session_context"), "get_session_env", None)
    try:
        return (get_session_env("XIUQO_SESSION_PLATFORM") if get_session_env else "") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    ``skills_dir_override`` makes home resolution EXPLICIT: a build thread that never bound the XIUQO_HOME
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None) -> Optional[str]:
    """SOUL.md from XIUQO_HOME (identity slot #1), or None.

    Callers must pass ``skip_soul=True`` to ``build_context_files_prompt`` so it isn't injected twice.
    ``home_override`` pins the profile home (a thread that lost the XIUQO_HOME ContextVar reads the wrong one).

    ``home_override`` scopes the read to an explicit profile home (the agent knows its own home from its
    session_db path). Without it, resolution is ambient — which on a thread that lost the XIUQO_HOME
    ...
    """
    try:
        from xiuqo_cli.config import ensure_hermes_home
        ensure_hermes_home()
    except Exception as e:
        logger.debug("Could not ensure XIUQO_HOME before loading SOUL.md: %s", e)
...
def _load_hermes_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.xiuqo.md / XIUQO.md — nearest match walking up to the git root."""
    xiuqo_md_path = _find_hermes_md(cwd_path)
    content = _read_context_file(xiuqo_md_path) if xiuqo_md_path else ""
    if not content:
        return ""
    label = str(xiuqo_md_path.relative_to(cwd_path)) if xiuqo_md_path.is_relative_to(cwd_path) else xiuqo_md_path.name
    return _context_section(_strip_yaml_frontmatter(content), label, ".xiuqo.md", xiuqo_md_path, context_length)
...
def build_context_files_prompt(
    ...
    Only ONE project context type loads, first found wins: .xiuqo.md/XIUQO.md (walk to git root) →
    AGENTS.md chain (git root → cwd) → CLAUDE.md (cwd) → .cursorrules + .cursor/rules/*.mdc (cwd). SOUL.md
    from XIUQO_HOME is independent and always included unless *skip_soul* (already the identity slot).
    """
    cwd_path = Path(cwd if cwd is not None else os.getcwd()).resolve()
    # A FALLBACK-picked cwd inside the Xiuqo install tree must not gain system-prompt authority (the desktop
    # default would load this repo's contributor AGENTS.md). An explicit cwd is honored verbatim.
    # An explicitly configured cwd is honored verbatim — the Xiuqo tree is a legitimate workspace when the
    # user deliberately points a session at it — and CLI-style surfaces pass
    # allow_install_tree_fallback=True because their launch dir IS the user's shell cwd (developing Xiuqo
    # in-tree). See #64590.
    from agent.runtime_cwd import _is_install_tree
    if cwd is None and not allow_install_tree_fallback and _is_install_tree(cwd_path):
        logger.warning(
            "skipping project-context discovery: working-directory resolution fell back to the Xiuqo "
