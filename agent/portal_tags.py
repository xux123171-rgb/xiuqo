...
Every Xiuqo request to the Nous Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=xiuqo", "client=xiuqo-client-v<__version__>"]``.
The version is read live from ``xiuqo_cli.__version__`` — do NOT pre-compute it
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("xiuqo_affinity_scope", default=None)
...
def xiuqo_client_tag() -> str:
    """``client=xiuqo-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if xiuqo_cli is unimportable)."""
    try:
        from xiuqo_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=xiuqo-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=xiuqo", xiuqo_client_tag()]
