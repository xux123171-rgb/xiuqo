...
def ctx_bound(fn: Callable[..., Any]) -> Callable[..., Any]:
    ...
    is a ContextVar-scoped XIUQO_HOME override plus the per-turn secret scope; a worker started
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include ``xiuqo_home`` (profile-scoped storage; never hardcode
        ``~/.xiuqo``) and ``platform``; may include ``agent_context`` ("primary" |
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        """Setup fields for ``xiuqo memory setup`` ([] if none): ``key``, ``description``,
    ...
    def backup_paths(self) -> List[str]:
        """Absolute paths of provider state OUTSIDE XIUQO_HOME for ``xiuqo backup``/``import``
