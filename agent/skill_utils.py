...
def _detect_kanban() -> bool:
    ...
    if os.getenv("XIUQO_KANBAN_TASK") or os.getenv("XIUQO_KANBAN_BOARD"):
        try:
...
def _load_raw_config() -> Dict[str, Any]:
    """Read config.yaml with an mtime+size keyed cache (no xiuqo_cli.config import)."""
...
def _home_relative(p: Path) -> Path:
    """Anchor a relative config path at XIUQO_HOME; absolute paths pass through."""
    from xiuqo_constants import get_hermes_home
    return p if p.is_absolute() else get_hermes_home() / p


# Never disableable: `xiuqo` is the agent's own operating manual and the
# system prompt points at it unconditionally.
ESSENTIAL_SKILLS: frozenset = frozenset({"xiuqo"})


def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    """Disabled skill names from config.yaml: global list ∪ platform list
    (*platform* defaults to ``XIUQO_PLATFORM`` / ``XIUQO_SESSION_PLATFORM``)."""
    skills_cfg = _skills_cfg()
    if skills_cfg is None:
        return set()
    from gateway.session_context import get_session_env
    resolved_platform = platform or os.getenv("XIUQO_PLATFORM") or get_session_env("XIUQO_SESSION_PLATFORM")
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``xiuqo config set`` stores lists as quoted JSON/Python-literal strings;
...
def get_external_skills_dirs() -> List[Path]:
    ...
    are ``~``/``${VAR}`` expanded, relative to XIUQO_HOME; the local skills dir is skipped."""
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_skill_create_dir() -> Optional[Path]:
    ...
    relative to XIUQO_HOME; a value equal to the local skills dir counts as unset."""
...
def get_all_skills_dirs() -> List[Path]:
    """Skill dirs: local ``~/.xiuqo/skills/`` first, then create_dir, then external.
...
# Project-local skills (<root>/.xiuqo/skills, <root>/.agents/skills; root = nearest
...
PROJECT_SKILLS_SUBDIRS = (os.path.join(".xiuqo", "skills"), os.path.join(".agents", "skills"))
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    (XIUQO_HOME itself may live inside a git checkout)."""
    local_skills = get_skills_dir().resolve()
    dirs: List[Path] = []
...
# cached under XIUQO_HOME, never inside the repo); "dangerous" excludes the
# skill from index, list, view and slash commands ("caution" loads, as on the hub).

# ── Project skill quarantine (scan-time injection defense) ──────────────── Trust (`xiuqo skills trust`)
...
# high-confidence findings only. The scan cache lives under XIUQO_HOME, never inside the repo (we don't
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    # (which follows the live profile-scoped XIUQO_HOME), so normalization
    ...
    # ~/.xiuqo/skills/<name> may be a symlink to a checkout elsewhere, and
...
def _hermes_metadata(frontmatter: Dict[str, Any]) -> Dict[str, Any]:
    """``metadata.xiuqo`` mapping from frontmatter, or ``{}`` when malformed."""
    metadata = frontmatter.get("metadata")
    xiuqo = metadata.get("xiuqo") if isinstance(metadata, dict) else None
    return xiuqo if isinstance(xiuqo, dict) else {}
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    xiuqo = _hermes_metadata(frontmatter)
    return {key: xiuqo.get(key, []) for key in _CONDITION_KEYS}


def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract ``metadata.xiuqo.config`` declarations (key/description/default/prompt).
...
def _expand_skill_config_path(value: str) -> str:
    """Expand ``~`` / ``$HOME`` against the HOME Xiuqo injects into tool subprocesses.

    Skill config defaults describe paths the agent hands to tools, so in a container where the
    control process HOME (``/opt/data``) differs from the tool HOME (``{XIUQO_HOME}/home``) a
