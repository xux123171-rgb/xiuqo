...
_XAI_CLIENT_WEB_SEARCH_ALIAS = "xiuqo_web_search"
...
# (hermes_<name>), map back in normalize_response so Xiuqo dispatch is unaffected. See #85589.
...
def _alias_wire_tools(response_tools: Any, params: dict[str, Any], is_xai_responses: bool) -> tuple[Any, dict[str, str]]:
    ...
    swaps it 1:1 for the built-in, client mode keeps Xiuqo dispatch under an alias.
    """
    ...
    # xAI server-side web search vs Xiuqo web providers. grok models on xAI's /v1/responses surface have a
    ...
    # or resolved): keep Xiuqo dispatch so ``web.backend`` / ``web.search_backend`` is honored, but rename
    # the wire tool to ``xiuqo_web_search`` so Grok cannot hijack the name. The alias is mapped back to
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        kwargs = {
            # ``-900k`` picker variants are Xiuqo-side aliases; the backend knows only the base slug.
