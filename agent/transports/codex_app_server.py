...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # codex needs LLM provider creds but must not receive Tier-1 Xiuqo secrets (gateway/GitHub/infra tokens).
        ...
        # `os.environ.copy()` also handed it every Tier-1 Xiuqo secret — gateway bot tokens, GitHub auth,
        ...
        spawn_env = xiuqo_subprocess_env(inherit_credentials=True)
        if env:
        ...
        # Native shell children remain unowned. Only Xiuqo' managed MCP tool
        ...
        owned_task = os.environ.get("XIUQO_KANBAN_TASK") and is_dispatcher_owned_worker_context()
        if owned_task:
            for key in (*KANBAN_ENV_KEYS, "XIUQO_KANBAN_DB", "XIUQO_KANBAN_BOARD"):
                if key in os.environ:
                    cmd += ["-c", f"mcp_servers.xiuqo-mcp.env.{key}={json.dumps(os.environ[key])}"]
            cmd += ["-c", f'mcp_servers.xiuqo-mcp.env.{DELEGATED_CHILD_ENV_MARKER}=""']
        ...
        if owned_task:
            kanban_db = spawn_env.get("XIUQO_KANBAN_DB")
            default_root = os.path.join(spawn_env.get("XIUQO_HOME", os.path.expanduser("~/.xiuqo")), "kanban")
            kanban_root = os.path.dirname(kanban_db) if kanban_db else spawn_env.get("XIUQO_KANBAN_ROOT", default_root)
            cmd += [
    ...
    def initialize(
        self, client_name: str = "xiuqo", client_title: str = "Xiuqo Agent",
