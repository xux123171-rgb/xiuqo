...
from agent.prompt_builder import (
    ...
    XIUQO_AGENT_HELP_GUIDANCE, XIUQO_AGENT_HELP_GUIDANCE_NO_SKILLS, KANBAN_GUIDANCE,
...
from xiuqo_constants import get_default_hermes_root, get_hermes_home
from utils import is_truthy_value

logger = logging.getLogger(__name__)
_PLUGIN_SECTION_FRAME_RE = re.compile(
    r"^## Plugin Context: (?P<id>[a-z0-9][a-z0-9._-]{0,127})\n<!-- xiuqo-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    """Append the desktop embedded-terminal clarifier when ``XIUQO_DESKTOP_TERMINAL``
    is set (only the desktop's TUI PTY, never the chat backend). Idempotent."""
    if not hint or _TUI_EMBEDDED_PANE_CLARIFIER in hint or not is_truthy_value(os.getenv("XIUQO_DESKTOP_TERMINAL")):
        return hint
...
def _active_profile_name(agent: Any, ambient) -> str:
    ...
    failure. Ambient resolution misreports on threads that lost the XIUQO_HOME
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        try:
            from xiuqo_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from xiuqo_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    A bound XIUQO_HOME ContextVar override wins (the gateway multiplexes
...
def _active_profile_line(agent: Any) -> str:
    """Name the running profile so the agent doesn't conflate ``~/.xiuqo/skills``
    (default) with ``~/.xiuqo/profiles/<active>/skills``.  Resolved from the
    ...
    if active_profile == "default":
        ...
        return (
            "Active Xiuqo profile: default. Other profiles (if any) live "
    ...
    return (
        f"Active Xiuqo profile: {active_profile}. This session reads "
...
def _timestamp_line(agent: Any) -> str:
    ...
    from xiuqo_time import get_timezone as _hermes_tz, now as _hermes_now
    now = _hermes_now()
...
def _context_files_part(agent: Any, ctx_len: Optional[int], soul_loaded: bool) -> List[str]:
    ...
    fallback they really are so the guard can reject Xiuqo's bundled AGENTS.md."""
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # xiuqo skill installed, so the variant is chosen after the skills
    # index is built; this slot holds its position.
    _help_guidance_slot = len(stable_parts)
    stable_parts.append(XIUQO_AGENT_HELP_GUIDANCE_NO_SKILLS)
    stable_parts.extend(_guidance_parts(agent))
    skills_prompt = _skills_prompt(agent)
    # Skill-pointer variant requires BOTH skill_view AND the xiuqo skill
    # in the rendered index (pure string check — inherits the index's stability).
    if "skill_view" in (agent.valid_tool_names or set()) and "- xiuqo:" in skills_prompt:
        stable_parts[_help_guidance_slot] = XIUQO_AGENT_HELP_GUIDANCE
