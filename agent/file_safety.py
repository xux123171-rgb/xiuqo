...
def _constants_path(getter_name: str) -> Path:
    """Call ``xiuqo_constants.<getter_name>()`` (local import avoids cycles); ``~/.xiuqo`` on any failure."""
    try:
        import xiuqo_constants

        return getattr(xiuqo_constants, getter_name)()
    except Exception:
        return Path(os.path.expanduser("~/.xiuqo"))


def _hermes_home_path() -> Path:
    """Active XIUQO_HOME (profile-aware). Tests monkeypatch this name."""
    return _constants_path("get_hermes_home")


def _hermes_root_path() -> Path:
    """Xiuqo root dir (parent of any profile, never per-profile)."""
    return _constants_path("get_default_hermes_root")


def _hermes_dirs() -> list[Path]:
    """Resolved active XIUQO_HOME and global root, deduplicated.

    Both are checked so credential stores at <root>/... stay guarded when
    running under a profile (XIUQO_HOME = <root>/profiles/<name>).
    """
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    xiuqo_files = (".env", ".anthropic_oauth.json", os.path.join("cache", "bws_cache.enc.json"))
    paths = [
        *(os.path.join(home, *f) for f in home_files),
        *(str(base / f) for f in xiuqo_files for base in (_hermes_home_path(), _hermes_root_path())),
...
def get_safe_write_roots() -> set[str]:
    """Resolved XIUQO_WRITE_SAFE_ROOT paths (``os.pathsep``-separated list)."""
    roots: set[str] = set()
    for path in filter(None, os.getenv("XIUQO_WRITE_SAFE_ROOT", "").split(os.pathsep)):
...
# XIUQO_HOME / root subpaths that the agent's generic file tools must not
...
_XIUQO_PROTECTED_SUBPATHS = ("state.db", "sessions", "mcp-tokens", "pairing")
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    for base in _hermes_dirs():
        for sub in _XIUQO_PROTECTED_SUBPATHS:
            with suppress(Exception):
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside XIUQO_WRITE_SAFE_ROOT "
...
# Exact-file credential stores under XIUQO_HOME / <root>. The agent never
...
# Directory-prefix read denies under XIUQO_HOME / <root>: (subdir, message for
...
_READ_DENIED_DIRS = (
    ("mcp-tokens",
     "is the Xiuqo MCP token directory and cannot be read directly.",
     "is a Xiuqo MCP token file and cannot be read directly."),
    ("browser-profile",
     "is the Xiuqo real-profile browser snapshot directory (copied cookies/logins) and cannot be read directly.",
     "is inside the Xiuqo real-profile browser snapshot (copied cookies/logins) and cannot be read directly."),
    # vault.key + vault.json.enc sit side by side; key + ciphertext = plaintext, so the whole dir is one credential.
    ("vault",
     "is the Xiuqo credential vault directory and cannot be read directly (secrets are filled server-side by browser_vault_fill).",
     "is inside the Xiuqo credential vault (encrypted secrets + local key) and cannot be read directly (browser_vault_fill resolves them server-side)."),
)


def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Xiuqo path.

    Blocked: internal skill-hub caches (prompt-injection carriers), credential
    stores under XIUQO_HOME and the global root (exact files, plus anything
    ...
    xiuqo_dirs = _hermes_dirs()
    reason = None
    if any(_is_under(resolved, hd / "skills" / ".hub") for hd in xiuqo_dirs):
        reason = (
            "is an internal Xiuqo cache file and cannot be read directly to prevent "
            "prompt injection. Use the skills_list or skill_view tools instead."
        )
    elif any(resolved in _resolve_each(hd / name for hd in xiuqo_dirs) for name in _CREDENTIAL_FILE_NAMES):
        reason = (
            "is a Xiuqo credential store and cannot be read directly. Provider tools "
            "consume these credentials through internal channels." + _DID_SUFFIX
        )
    else:
        for subdir, dir_msg, file_msg in _READ_DENIED_DIRS:
            for blocked_dir in _resolve_each(hd / subdir for hd in xiuqo_dirs):
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Xiuqo read (see ``get_read_block_error``).
...
def _resolve_active_profile_name() -> str:
    """Active profile name from XIUQO_HOME: ``~/.xiuqo`` -> ``"default"``,
    ``~/.xiuqo/profiles/X`` -> ``"X"``; ``"default"`` on any resolution failure."""
    try:
...
#   <XIUQO_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.xiuqo/...
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Xiuqo state: ``None``
    for non-mirror paths, else ``target_path`` (resolved), ``mirror_root`` (the
    ``…/home/.xiuqo`` prefix) and ``inner_path`` (what the agent meant on the host)."""
    target = _resolve_target(path)
    parts = target.parts if target is not None else ()
    # Need at least: sandboxes / <backend> / <task> / home / .xiuqo / <thing>; inner_idx = the .xiuqo part.
    inner_idx = next(
        (i + 4 for i, part in enumerate(parts)
         if part == "sandboxes" and i + 5 < len(parts) and parts[i + 3] == "home" and parts[i + 4] == ".xiuqo"),
        None,
    )
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "Writes here land on a copy that the host Xiuqo process never reads — the "
        "authoritative file is likely {inner_path!r} under the real XIUQO_HOME.",
...
def classify_container_mirror_target(path: str, mirror_prefix: str | None = None) -> Optional[dict]:
    ...
    the bind mount strips the ``sandboxes/`` prefix (the agent sees plain ``/root/.xiuqo/…``),
...
def get_container_mirror_warning(path: str, mirror_prefix: str | None = None) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "the container's bind-mounted home — a per-task mirror that the host Xiuqo "
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Xiuqo scope, or is inside
