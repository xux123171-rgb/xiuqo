"""Lightweight i18n for Xiuqo' static user-facing strings (approval prompts, a few gateway replies).
...
Language resolution: explicit ``lang=`` > ``XIUQO_LANGUAGE`` > ``display.language`` > ``en``.
"""
...
def _locales_dir() -> Path:
    """Locale dir: ``XIUQO_BUNDLED_LOCALES`` (sealed packaging, e.g. Nix) if it exists, else ``<repo-root>/locales``.

    The source path is returned even when missing so ``_load_catalog`` can log
    the path it looked at rather than raise.
    """
    override = os.getenv("XIUQO_BUNDLED_LOCALES", "").strip()
    if override and Path(override).is_dir():
        return Path(override)
    if override:
        logger.warning(
            "XIUQO_BUNDLED_LOCALES points to a non-directory path (%s); "
...
@lru_cache(maxsize=8)
def _config_language_cached(xiuqo_home: str) -> str | None:
...
def get_language() -> str:
    """Resolve the active language using env > config > default order. ``XIUQO_LANGUAGE`` is a
    ...
    try:
        env_lang = get_secret("XIUQO_LANGUAGE")
    except UnscopedSecretError:
        env_lang = os.environ.get("XIUQO_LANGUAGE")  # unscoped default-profile path: environ IS its own value
