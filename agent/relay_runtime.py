"""Profile-scoped NeMo Relay runtimes owned by the Xiuqo agent core."""
...
from xiuqo_cli.relay_plugin_cutover import (RELAY_PLUGINS_CONFIG_ENV, configured_legacy_relay_env_vars)

logger = logging.getLogger(__name__)

SESSION_SCOPE = "xiuqo.session"
TURN_SCOPE = "xiuqo.turn"
LOGICAL_LLM_SCOPE = "xiuqo.logical_llm_call"
RUNTIME_SCHEMA_KEY = "xiuqo.relay.schema_version"
RUNTIME_SCHEMA_VERSION = "xiuqo.relay.runtime.v1"
RUNTIME_INSTANCE_KEY = "xiuqo.relay.runtime_instance"
RELAY_PLUGINS_EXECUTION_CONSUMER = "xiuqo.nemo_relay.plugins"
...
def runtime_metadata(runtime_id: str, **extra: Any) -> dict[str, Any]:
    """Return the scope metadata that stamps every Xiuqo-owned Relay scope."""
...
@dataclass
class RelaySession:
    """One isolated Relay scope stack owned by a Xiuqo session."""
    ...
    def run_in_session(self, session: RelaySession, callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        ...
        with self._lock:
            if self._runtime is None:
                raise RuntimeError("Xiuqo Relay operation lease is released")
...
class _ProcessRelayPluginConfiguration:
    ...
    def __init__(self) -> None:
        ...
        self._relay: Any = None  # set while a Xiuqo-owned configuration is active
        ...
        with self._lock:
            if not self._owners:
                ...
                if self._state is _RelayPluginConfigurationState.ACTIVE:
                    logger.info(
                        "Relay plugins are active process-wide and apply to all profiles hosted by this Xiuqo process."
                    )
    ...
    def _activate(self, relay: Any) -> _RelayPluginConfigurationState:
        try:
            ...
            logger.warning("Xiuqo Relay plugin initialization failed: %s", exc, exc_info=True)
    ...
    def _preflight(self, relay: Any) -> _RelayPluginConfigurationState | None:
        ...
        if self._relay is not None and not self._clear_active():
            logger.warning(
                "Xiuqo Relay plugin cleanup is still pending; refusing to replace the process-global configuration"
            )
            return _RelayPluginConfigurationState.FAILED
        try:
            ...
            logger.warning(
                "Xiuqo could not determine whether a process-global Relay plugin configuration is already "
                "active; refusing to replace it", exc_info=True,
            )
            return _RelayPluginConfigurationState.FAILED
        if existing_report is not None:
            logger.warning(
                "A process-global Relay plugin configuration is already active outside Xiuqo native ownership; "
                "leaving it unchanged and disabling Xiuqo-managed Relay middleware for this process"
            )
    ...
    def _initialize(self, relay: Any) -> bool:
        ...
        if dynamic_plugins:
            try:
                ...
                raise RuntimeError("Xiuqo Relay dynamic plugin activation failed") from exc
    ...
    def _clear_active(self) -> bool:
        ...
        for what, step in (
            ...
            try:
                ...
                logger.warning("Xiuqo Relay plugin %s failed", what, exc_info=True)
                return False
...
class RelayRuntime:
    ...
    def managed_execution_enabled(self) -> bool:
        """Return whether a Xiuqo-managed consumer needs the Relay pipeline."""
        with self._execution_consumers_lock:
    ...
    def rotate_session_scope(self, session: RelaySession, *, reason: str) -> None:
        ...
        with session.lock:
            ...
            try:
                self.run_in_session(
                    session, self.relay.scope.pop, old_handle, output={"xiuqo.session.segment_reason": reason},
                    metadata=runtime_metadata(self.runtime_id), timeout=_SCOPE_OP_TIMEOUT,
                )
            except Exception:
                logger.warning(
                    "Xiuqo Relay segment close failed (session=%s segment=%d); abandoning the old segment span",
                    session.session_id, session.segment - 1, exc_info=True,
                )
            scope_metadata = runtime_metadata(
                self.runtime_id, **{"xiuqo.session.segment": session.segment, "xiuqo.session.segment_reason": reason},
            )
            try:
                self._open_session_scope(session, scope_metadata, resolve_parent=False)
            except Exception:
                logger.warning(
                    "Xiuqo Relay segment open failed (session=%s segment=%d); keeping the prior scope handle",
    ...
    def get_session(self, session_id: str) -> RelaySession | None:
        """Return an active Xiuqo Relay session without creating one."""
        with self._sessions_lock:
    ...
    def _session_context(self, session: RelaySession, *, allow_closing: bool) -> contextvars.Context:
        ...
        with session.lock:
            if session.closing and not allow_closing:
                raise RuntimeError("Xiuqo Relay session is closing")
            if session.context is None or session.handle is None:
                raise RuntimeError("Xiuqo Relay session context is unavailable")
    ...
    def _begin_operation(self) -> None:
        ...
        with self._sessions_lock:
            if self._closing:
                raise RuntimeError("Xiuqo Relay runtime is shutting down")
    ...
    def apply_tool_request_intercepts(self, *, session_id: str, tool_name: str, args: dict[str, Any]) -> dict[str, Any]:
        """Apply Relay request rewriting before Xiuqo authorizes a tool call."""
    ...
    def _pop_with_drain(
        ...
        for _ in range(drain_limit):
            ...
            if session_root is not None and _same_handle(top, session_root) and handle is not session_root:
                break
            try:
                orphan_output = {"outcome": "cancelled", "xiuqo.orphan_drain": True}
                pop_relay_scope(self.relay, top, output=orphan_output, metadata=metadata)
                drained += 1
            except Exception:
                logger.warning("Xiuqo Relay orphaned scope drain failed", exc_info=True)
                break
        if drained:
            logger.warning("Xiuqo Relay drained %d orphaned scope(s) before closing %s", drained, handle)
        try:
            pop_relay_scope(self.relay, handle, output=output, metadata=metadata)
            return None
    ...
    def _close_session(self, event: dict[str, Any]) -> None:
        ...
        with self._sessions_lock:
            ...
            self._forget_subagent(session_id)
        if failure:
            logger.warning("Xiuqo Relay session %s closed with errors: %s", session_id, failure)
    ...
    def shutdown(self) -> None:
        ...
        thread = threading.Thread(
            ...
            name=f"xiuqo-nemo-relay-shutdown-{self.runtime_id[:8]}", daemon=True,
        )
        try:
            thread.start()
        except Exception:
            ...
            logger.warning("Xiuqo Relay deferred shutdown could not start", exc_info=True)
    ...
    def _finish_shutdown(self) -> None:
        try:
            ...
            with self._sessions_lock:
                self._shutdown_started = False
            logger.warning("Xiuqo Relay shutdown failed", exc_info=True)
            return
        with self._sessions_lock:
...
class RelayHostRegistry:
    """Own exactly one Relay host for each canonical Xiuqo profile."""
    ...
    def for_profile(self, profile_key: str | None = None, *, create: bool = True) -> RelayHost | None:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Xiuqo Relay runtime initialization failed", exc_info=True)
...
class RelayTurnContext:
    """Runtime-only context for one Xiuqo turn or top-level task."""
...
_CURRENT_TURN: contextvars.ContextVar[RelayTurnContext | None] = contextvars.ContextVar(
    "xiuqo_relay_turn", default=None
)

# >0 while the native pipeline is mid-dispatch of a Xiuqo tool/LLM callback. Nested managed
...
_MANAGED_CALLBACK_DEPTH: contextvars.ContextVar[int] = contextvars.ContextVar(
    "xiuqo_relay_managed_callback_depth", default=0
)
...
def _warn_on_error(what: str, callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
    """Run fail-open telemetry work: log ``Xiuqo Relay <what> failed`` and return None on error."""
    try:
        return callback(*args, **kwargs)
    except Exception:
        logger.warning("Xiuqo Relay %s failed", what, exc_info=True)
        return None
...
class RelaySessionCoordinator:
    """Own semantic conversation and turn lifetimes for Xiuqo core."""
    ...
    def _prepare_session(self, host: RelayRuntime, context: dict[str, Any]) -> None:
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning("Xiuqo Relay session initializer failed: %s", name, exc_info=True)
    ...
    def _open_conversation_session(self, host: RelayRuntime, context: dict[str, Any]) -> RelaySession | None:
        ...
        metadata = {"xiuqo.execution_surface": context["platform"] or "unknown"}
    ...
    def begin_turn(
        ...
        *,
        ...
        if lease.released:
            raise RuntimeError("Xiuqo Relay conversation lease is released")
        ...
        with self._active_turns_lock:
            if self._active_turns.get(key):
                ...
                logger.warning(
                    "Skipping Relay instrumentation for concurrent Xiuqo turn %s in session %s",
        ...
        if host is not None:
            ...
            turn_metadata.update(
                runtime_metadata(
                    host.runtime_id,
                    **{"xiuqo.execution_surface": lease.platform or "unknown"},
                )
            )
    ...
    def _close_turn_scope(self, host: RelayRuntime, turn: RelayTurnContext, *, outcome: str) -> None:
        ...
        if failure:
            logger.warning("Xiuqo Relay turn finalization failed: %s", failure)
    ...
    def _finish_logical_calls(turn: RelayTurnContext, *, outcome: str) -> None:
        ...
        while logical_calls:
            ...
            logger.warning("Xiuqo Relay logical LLM finalization failed: %s", failure)
            break
...
def resolve_execution_context(session_id: str) -> tuple[RelayRuntime | None, RelaySession | None, Any]:
    ...
    if _MANAGED_CALLBACK_DEPTH.get() > 0 or not relay_instrumentation_enabled():
        ...
        # ``tools.execute`` pipeline is mid-dispatch of a Xiuqo tool). Nested managed execution here is
...
def apply_tool_request_intercepts(*, session_id: str, tool_name: str, args: dict[str, Any]) -> dict[str, Any]:
    """Return Relay-rewritten arguments at Xiuqo's authorization boundary."""
...
def get_runtime(*, create: bool = True, profile_key: str | None = None) -> RelayRuntime | None:
    """Return the Relay host for the active Xiuqo profile."""
...
def _configured_plugin_inputs(relay: Any) -> tuple[dict[str, Any], list[Any]] | None:
    ...
    try:
        ...
        if "dynamic_plugins" in config:
            raise ValueError("Xiuqo [[dynamic_plugins]] records are unsupported; use Relay [[plugins.dynamic]] records")
        ...
        raise _RelayPluginConfigurationLoadError(
            f"Xiuqo Relay plugin configuration could not be loaded from {config_path}; continuing without Relay plugins"
        ) from exc
...
def _resolve_plugin_awaitable(value: Any) -> Any:
    ...
    return _run_on_daemon_thread(lambda: asyncio.run(value), name="xiuqo-nemo-relay-plugin-lifecycle")
...
def emit_mark(
    ...
    *,
    ...
    """Emit a fail-open Relay mark under a Xiuqo session."""
    runtime = get_runtime(create=False)
    if runtime is None:
        return False
    try:
        ...
        logger.warning("Xiuqo Relay mark failed: %s", name, exc_info=True)
        return False

def ensure_session(*, session_id: str, **context: Any) -> RelaySession | None:
    """Create or return the shared Relay session used by Xiuqo core."""
    runtime = get_runtime()
    if runtime is None:
        return None
    try:
        ...
        logger.warning("Xiuqo Relay session initialization failed", exc_info=True)
        return None
...
def run_in_session(
    ...
    **kwargs: Any,
    ...
    """Run a scope, LLM, or tool API against a shared Xiuqo session."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Xiuqo Relay runtime is unavailable")
    session = runtime.get_session(session_id)
    if session is None:
        session = runtime.ensure_session({"session_id": session_id})
    if session is None:
        raise RuntimeError("Xiuqo Relay session is unavailable")
...
async def run_in_session_async(
    ...
    **kwargs: Any,
    ...
    """Await a Relay operation inside a shared Xiuqo session context."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Xiuqo Relay runtime is unavailable")
    session = runtime.get_session(session_id)
