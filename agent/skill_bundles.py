...
YAML files in ``<XIUQO_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``XIUQO_BUNDLES_DIR`` override (tests) or ``<XIUQO_HOME>/skill-bundles``."""
    override = os.environ.get("XIUQO_BUNDLES_DIR")
...
# File-level CRUD — used by `xiuqo bundles`.
