...
# `export XIUQO_REDACT_SECRETS=false`) cannot disable redaction mid-session.
...
# to this env var in xiuqo_cli/main.py, gateway/run.py, and cli.py) or `XIUQO_REDACT_SECRETS=false` in
# ~/.xiuqo/.env. An opt-out warning is logged at gateway and CLI startup so operators see the downgrade —
# see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("XIUQO_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}

# Routed multiplex profiles: the import-time snapshot above is the LAUNCH profile's policy. A profile
# served under a XIUQO_HOME override resolves its own ``security.redact_secrets`` (its ``.env``
# value first, like the standalone bridge in xiuqo_cli/main.py), cached per home so the hot path
...
def _redact_enabled() -> bool:
    ...
    try:
        ...
        raw = scope.get("XIUQO_REDACT_SECRETS") if scope else None
        if raw is None:
...
def mask_secret(value: str, *, head: int = 4, tail: int = 4, floor: int = 12,
    ...
    """Mask a secret for display (``xiuqo config`` / ``status`` / ``dump``):
