...
INSPECTION_STAMP_ATTR = "data-xiuqo-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-xiuqo-vault-slot", nonce + ":" + index));
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-xiuqo-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-xiuqo-vault-slot]").forEach((n) => n.removeAttribute("data-xiuqo-vault-slot"));
