...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    ``PSES_BUNDLE_PATH`` env, then ``<XIUQO_HOME>/lsp/PowerShellEditorServices``."""
...
_PSES_MISSING_MSG = (
    ...
    "lsp.servers.powershell.command to the bundle path or unzip it to <XIUQO_HOME>/lsp/PowerShellEditorServices."
)
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    session_dir = xiuqo_lsp_session_dir()
    inner = (
        ...
        f"-HostName Xiuqo -HostProfileId xiuqo -HostVersion 1.0.0 -Stdio -LogLevel Normal"
    )
    return SpawnSpec(
