"""Language Server Protocol (LSP) integration for Xiuqo Agent.
...
# Routed multiplex profiles (XIUQO_HOME override) each get their own service: ``lsp.*`` config
...
def get_service() -> Optional[LSPService]:
    ...
    without it every ``xiuqo chat`` exit leaks pyright processes for a few seconds
