...
``ANTHROPIC_API_KEY``, Xiuqo-owned OAuth grants in the ``auth.json`` credential
pool, then ``~/.claude/.credentials.json`` / macOS Keychain as a borrowed fallback.
``~/.xiuqo/.anthropic_oauth.json`` (Xiuqo PKCE) and
...
_SPENT_ROTATION_SIDECAR_COMMENT = (
    ...
    "consumed server-side but never durably committed. Written by Xiuqo so sibling "
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    return source_path.with_name(source_path.name + ".xiuqo-spent-rotations.json")
...
_SINGLETON_SOURCE_PATHS = {
    "claude_code": lambda: claude_code_credentials_path(), "xiuqo_pkce": lambda: _get_hermes_oauth_file()
}
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        from xiuqo_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float
        refresh_timeout_seconds = env_float("XIUQO_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20)
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    """Prefer refreshable Claude Code creds over a static env OAuth token: Xiuqo historically persisted setup tokens
...
def _resolve_anthropic_pool_token(*, skip_borrowed: bool = False) -> Optional[str]:
    ...
    refresh=False`` (never ``select()``) so diagnostic call sites (account_usage, ``xiuqo models``) never mutate
...
# ── Xiuqo-native PKCE OAuth flow (~/.xiuqo/.anthropic_oauth.json); mirrors Claude Code / pi-ai / OpenCode ──
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Xiuqo-native OAuth PKCE flow and return credential state."""
    import webbrowser
    ...
    print("\n".join([
        "", "Authorize Xiuqo with your Claude Pro/Max subscription.", "",
    ...
    try:
        from xiuqo_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Xiuqo-managed OAuth credentials from ~/.xiuqo/.anthropic_oauth.json."""
    data = _load_json_if_exists(_get_hermes_oauth_file(), "Xiuqo OAuth credentials")
...
def _write_hermes_oauth_credentials(
    ...
    """Commit refreshed xiuqo_pkce tokens to ~/.xiuqo/.anthropic_oauth.json (``CredentialPersistError`` on failure).
    ``target`` lets a named profile commit a grant it BORROWED from the global root back to the ROOT singleton
    instead of forking a copy under its own XIUQO_HOME; without this write-through the next ``load_pool()``
