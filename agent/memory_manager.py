...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        for raw_schema in schemas:
            ...
            if tool_name in _XIUQO_CORE_TOOLS:
                logger.warning(
    ...
    def get_all_tool_schemas(self) -> List[Dict[str, Any]]:
        ...
        def _collect(provider: MemoryProvider) -> None:
            for raw_schema in provider.get_tool_schemas():
                ...
                elif schema["name"] not in _XIUQO_CORE_TOOLS and schema["name"] not in seen:
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        """Initialize all providers, injecting ``xiuqo_home`` so they resolve profile-scoped paths."""
        if "xiuqo_home" not in kwargs:
            from xiuqo_constants import get_hermes_home
            kwargs["xiuqo_home"] = str(get_hermes_home())
