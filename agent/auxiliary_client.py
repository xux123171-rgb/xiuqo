...
from xiuqo_constants import OPENROUTER_BASE_URL, xiuqo_home_key
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_verify``, ``XIUQO_CA_BUNDLE`` / ``SSL_CERT_FILE``); any failure → httpx default (``True``)."""
    try:
        from agent.ssl_verify import resolve_httpx_verify
        from xiuqo_cli.config import get_custom_provider_tls_settings, load_config_readonly
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `xiuqo update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    with contextlib.suppress(Exception):
        from xiuqo_cli.models import OPENCODE_ZEN_FREE_KEYLESS_PLACEHOLDER, opencode_zen_free_headers
    ...
    # Xiuqo owns aux retry/fallback policy; the SDK default (max_retries=2) would triple
    # wall time on a hung endpoint before Xiuqo sees one failure.
    # Xiuqo owns auxiliary retry + provider/model fallback policy (the same-provider transient retry in
    ...
    # so a 120s timeout can stall ~360s before Xiuqo sees a single failure (issue #54465). Disable
    # SDK-internal retries by default and let Xiuqo control the budget; explicit callers can still override
    # via kwargs.
...
def _run_protected_sync_provider_call(callback: Callable[[dict[str, Any]], Any], kwargs: dict[str, Any]) -> Any:
    ...
    threading.Thread(
        target=provider_context.run, args=(_provider_worker,), name="xiuqo-protected-aux-provider",
        daemon=True).start()
    while True:
...
def _client_declares(client_obj: Any, flag: str) -> bool:
    ...
    not isinstance, so out-of-tree clients can opt out of wrappers unimported (cf. SUPPORTS_XIUQO_TOOL_CALLS)."""
    try:
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        ...
        from xiuqo_cli.models_pricing import fetch_models_with_pricing
        ...
        if not api_key and is_nous:
            ...
            try:
                from xiuqo_cli.models_pricing import _resolve_nous_pricing_credentials
        ...
        _nous_kwargs = {}
        if is_nous:
            from xiuqo_cli.models_pricing import _NOUS_CATALOG_TTL_SECONDS
    ...
    if is_nous:
        ...
        try:
            from xiuqo_cli.models_pricing import nous_policy_allowed_ids, restrict_to_nous_policy
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    try:
        from xiuqo_cli.config import cfg_get, load_config
        _cfg = load_config()
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Precedence env > config > default: ``XIUQO_OPENROUTER_CACHE`` overrides
    ``openrouter.response_cache``; ``XIUQO_OPENROUTER_CACHE_TTL`` (1-86400 s) overrides
    ...
    if or_config is None:
        try:
            ...
            or_config = {}
    env_cache = os.environ.get("XIUQO_OPENROUTER_CACHE", "").strip().lower()
    if not (env_cache in {"1", "true", "yes", "on"} if env_cache else or_config.get("response_cache", False)):
        return headers
    headers["X-OpenRouter-Cache"] = "true"
    env_ttl = os.environ.get("XIUQO_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
...
_NVIDIA_NIM_CLOUD_HEADERS = {"X-BILLING-INVOKE-ORIGIN": "XiuqoAgent"}
...
from xiuqo_cli import __version__ as _XIUQO_VERSION

_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://hermes-agent.nousresearch.com",
    "X-Title": "Xiuqo Agent",
    "User-Agent": f"XiuqoAgent/{_XIUQO_VERSION}",
}

# Nous Portal attribution extra_body. Tags come from agent.portal_tags so the client= marker
# tracks xiuqo_cli.__version__ — never inline a literal here.
...
def _auth_json_path():
    ...
    from xiuqo_cli.auth import _auth_file_path
...
def _to_openai_base_url(base_url: str) -> str:
    ...
    if base_url_hostname(url) == "api.actual.inc":
        from xiuqo_cli.auth import normalize_actual_base_url
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from xiuqo_cli.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("XIUQO_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
class _CodexCompletionsAdapter:
    ...
    def _build_responses_kwargs(self, kwargs: Dict[str, Any]) -> Tuple[Dict[str, Any], str, Any]:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Codex only knows the base slug; strip the Xiuqo ``-900k`` picker suffix.
    ...
    def create(self, **kwargs) -> Any:
        from xiuqo_cli.providers import is_actual_route
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        # (already TRANSLATED to native fields — raw would 400 on strict gateways) and ``_`` Xiuqo plumbing.
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``xiuqo_cli.runtime_provider._detect_api_mode_for_url`` so aux and main agree: any
...
def _maybe_wrap_anthropic(
    ...
    """
    # Anthropic/Bedrock/Codex wrappers, plus any client declaring XIUQO_SKIP_TRANSPORT_WRAP
    ...
    if (
        ...
        or _client_declares(client_obj, "XIUQO_SKIP_TRANSPORT_WRAP")
    ):
        return client_obj
...
def _read_nous_auth() -> Optional[dict]:
    """Nous provider state dict from the credential pool or ~/.xiuqo/auth.json; None when not active with tokens."""
...
def _nous_api_key(provider: dict) -> str:
    ...
    from xiuqo_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from xiuqo_cli.auth import _agent_key_is_usable
        pool = load_pool("nous")
    except Exception as exc:
...
def _resolve_nous_runtime_api(
    *, force_refresh: bool = False, stale_access_token: Optional[str] = None
    ...
    if pooled is not None:
        return pooled
    try:
        from xiuqo_cli.auth import resolve_nous_runtime_credentials
        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("XIUQO_NOUS_TIMEOUT_SECONDS", 15),
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    """
    try:
        from xiuqo_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL, _xai_validate_inference_base_url
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                base_url = _xai_validate_inference_base_url(
                    _url(_scoped_key_env("XIUQO_XAI_BASE_URL"))
    ...
    try:
        from xiuqo_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    if pool_present:
        ...
        if token:
            return token
    try:
        from xiuqo_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from xiuqo_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            with contextlib.suppress(ImportError):
                from xiuqo_cli.auth import is_provider_explicitly_configured
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            from xiuqo_cli.models import copilot_default_headers
...
def _endpoint_default_headers(
    ...
    if base_url_host_matches(base_url, "api.kimi.com"):
        ...
        from xiuqo_cli.copilot_auth import copilot_request_headers
        ...
        from tools.xai_http import xiuqo_xai_default_headers
        headers = dict(xiuqo_xai_default_headers())
    else:
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    try:
        from xiuqo_cli.config import cfg_get, load_config_readonly
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if runtime is None and not nous:
        logger.warning("Auxiliary Nous client unavailable: no Nous authentication found (run: xiuqo auth).")
    ...
    from xiuqo_cli.anon_auth import GUEST_MODEL, route_is_welcome_host
    ...
    if not _aux_probe_active():
        try:
            from xiuqo_cli.models import get_nous_recommended_aux_model
...
def _read_main_field(field: str, *, readonly: bool, lower: bool = False) -> str:
    ...
    with contextlib.suppress(Exception):
        from xiuqo_cli import config as _cfg_mod
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        from xiuqo_cli.moa_config import resolve_moa_preset
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `xiuqo setup` or `xiuqo model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key, base_url=base_url, default_headers=xiuqo_xai_default_headers()
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *, model: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    try:
        from xiuqo_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from xiuqo_cli.nous_account import get_nous_portal_account_info
...
def _refresh_copilot_credentials() -> bool:
    from xiuqo_cli.copilot_auth import _jwt_cache, _token_fingerprint, exchange_copilot_token, resolve_copilot_token
...
def _refresh_codex_credentials() -> bool:
    from xiuqo_cli.auth import resolve_codex_runtime_credentials
...
def _refresh_nous_credentials() -> bool:
    ...
    return _creds_have_api_key(resolve_nous_runtime_credentials(
        timeout_seconds=env_float("XIUQO_NOUS_TIMEOUT_SECONDS", 15), force_refresh=True
    ))
...
def _warn_stale_openai_base_url(runtime_provider: str) -> None:
    ...
    ~/.xiuqo/.env value after `xiuqo model` poisons routing)."""
    ...
    if (_env_base and _cfg_provider and _cfg_provider != "custom" and not _cfg_provider.startswith("custom:")):
        logger.warning(
            ...
            "Run: xiuqo model to reconfigure, or remove "
            "OPENAI_BASE_URL from ~/.xiuqo/.env",
...
def _discovery_chain_allowed(main_provider: str, task: Optional[str] = None) -> bool:
    ...
    logger.warning(
        ...
        "configured — refusing to guess another logged-in provider. Re-authenticate (`xiuqo model`) "
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if _client_declares(sync_client, "XIUQO_SKIP_ASYNC_WRAP"):
        return sync_client, model
    ...
    # Xiuqo owns the auxiliary retry/timeout budget; disable SDK-internal retries.
    # See #54465.
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from xiuqo_cli.model_normalize import normalize_model_for_provider
...
def _is_actual_auxiliary_route(req: _ResolveRequest, base_url: str) -> bool:
    from xiuqo_cli.auth import normalize_actual_base_url
    from xiuqo_cli.providers import is_actual_route
    from xiuqo_cli.route_identity import normalize_route_base_url
...
def _resolve_nous_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if client is None:
        logger.warning("resolve_provider_client: nous requested but Nous Portal not configured (run: xiuqo auth)")
        return None, None
    ...
    from xiuqo_cli.providers import nous_api_mode
...
def _resolve_openai_codex_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if not model:
        ...
        return None, None
    no_token_msg = "resolve_provider_client: openai-codex requested but no Codex OAuth token found (run: xiuqo model)"
    if req.raw_codex:
...
def _resolve_xai_oauth_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default,
                          ...
                          "OAuth token found (run: xiuqo model -> xAI Grok OAuth — SuperGrok / Premium+)")
...
def _resolve_azure_foundry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default_model,
                          ...
                          "runtime resolution failed (run: xiuqo doctor for diagnostics)")
...
def _resolve_api_key_branch(req: _ResolveRequest, pconfig: Any, resolve_creds: Callable) -> _ResolveResult:
    ...
    try:
        from xiuqo_cli.models import opencode_zen_free_runtime as _oc_free_rt
    ...
    if provider == "copilot" and final_model and not req.raw_codex:
        with contextlib.suppress(ImportError):
            from xiuqo_cli.models import _should_use_copilot_responses_api
...
def _resolve_registry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    try:
        ...
        logger.debug("xiuqo_cli.auth not available for provider %s", provider)
        return None, None
...
def _client_cache_key(
    ...
    return (xiuqo_home_key(), provider, async_mode, base_url or "", api_key_key, api_mode or "", runtime_key, is_vision, task_key, pool_hint, model_key)
...
def _acquire_sync_aux_semaphore(task: Optional[str]) -> Optional[threading.BoundedSemaphore]:
    ...
    if limit is None:
        return None
    from xiuqo_constants import xiuqo_home_key
    return _cached_semaphore(_aux_sync_semaphores, (xiuqo_home_key(), task), limit, threading.BoundedSemaphore)
...
def _acquire_async_aux_semaphore(task: Optional[str]):
    ...
    try:
        ...
        return None
    from xiuqo_constants import xiuqo_home_key
    return _cached_semaphore(_aux_async_semaphores, (xiuqo_home_key(), task, id(loop)), limit, asyncio.Semaphore)
...
def _managed_local_netloc() -> str:
    ...
    if now - ts < _MANAGED_LOCAL_STATE_TTL_S:
        return cached
    try:
        from xiuqo_cli.local_runtime.supervisor import state_path
...
def _is_managed_local_endpoint(base_url: Optional[str]) -> bool:
    """True when *base_url* targets the llama-server this Xiuqo manages."""
    if not base_url:
        return False
...
def _resolve_call_client(
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is None:
                    raise RuntimeError(
                        ...
                        f"a different provider with `xiuqo model`.")
    ...
    if client is None:
        raise RuntimeError(f"No LLM provider configured for task={task} "
                           f"provider={resolved_provider}. Run: xiuqo setup")
