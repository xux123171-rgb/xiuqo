...
# long"). Codex message ids can run 400+ chars; Xiuqo ``msg_...`` ids stay under the cap.
...
def _chat_messages_to_responses_input(
    ...
    decision was wrong — xAI explicitly relies on Xiuqo threading encrypted reasoning back across turns for
    ...
    Xiuqo' local history is never truncated by native compaction, so the full conversation is still on the
    wire.
    """
