"""OpenAI-shape bridge shared by Xiuqo' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Xiuqo' tool schemas travel INTO the
...
read/edit/execute tools forwards only Xiuqo' agent-level tools, since re-offering overlapping ones
makes Xiuqo redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``xiuqo_projected_messages``)
    Xiuqo reads off the ``create`` result; a plain list would drop them on the stream path."""
