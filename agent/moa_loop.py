...
def _moa_privacy_mode(moa_raw: Any) -> str:
    ...
    from xiuqo_cli.moa_config import coerce_privacy_filter
...
def _slot_runtime(slot: dict[str, Any]) -> dict[str, Any]:
    ...
    # xiuqo_home_key() in the key: the resolved api_key/base_url are per-profile, and under a
    ...
    cache_key = (xiuqo_home_key(), provider, model)
    now = time.monotonic()
...
def aggregate_moa_context(
    *, user_prompt: str, api_messages: list[dict[str, Any]], reference_models: list[dict[str, Any]],
    ...
    synth_prompt = (
        ...
        "Xiuqo agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Xiuqo agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
