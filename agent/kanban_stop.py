...
Xiuqo treats that as a clean exit → ``rc=0`` → dispatcher ``protocol_violation``.
...
def kanban_stop_nudge_enabled() -> bool:
    """On when ``XIUQO_KANBAN_TASK`` is set, unless ``XIUQO_KANBAN_STOP_NUDGE`` disables it."""
    if (os.environ.get("XIUQO_KANBAN_STOP_NUDGE") or "").strip().lower() in {"0", "false", "no", "off"}:
        return False
    return bool((os.environ.get("XIUQO_KANBAN_TASK") or "").strip())
...
def build_kanban_stop_nudge(
    *,
    ...
    tid = (task_id or os.environ.get("XIUQO_KANBAN_TASK") or "").strip() or "this task"
    return (
        "[System: You are a Xiuqo kanban worker. A plain-text reply is NOT a "
