...
a cache problem must never block Xiuqo startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_hermes_dotenv()``, else ``$XIUQO_HOME``/``~/.xiuqo``."""
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<xiuqo_home>/cache/<basename>``::
