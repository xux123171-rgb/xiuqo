...
:func:`apply_all` via ``xiuqo_cli.env_loader``; plugins register through
...
def _validate_source(source: SecretSource) -> Optional[str]:
    ...
    if source.api_version != SECRET_SOURCE_API_VERSION:
        return (f"Ignoring secret source '{name}': built against secret-source API "
                f"v{source.api_version}, this Xiuqo speaks v{SECRET_SOURCE_API_VERSION}")
...
def _merged(scope: Optional[str]) -> Dict[str, SecretSource]:
    ...
    merged.update(_SCOPED_SOURCES.get(scope or xiuqo_home_key(), {}))
    return merged
...
def list_plugin_sources() -> List[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(xiuqo_home_key(), {}))
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    """Active profile name (``~/.xiuqo/profiles/<name>``); "" for the default profile."""
...
def apply_all(secrets_cfg: dict, home_path: Path,
    ...
    enabled = _ordered_enabled_sources(secrets_cfg, scope=xiuqo_home_key(home_path))
    if not enabled:
        return report
