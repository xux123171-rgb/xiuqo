...
Pulls API keys from BSM at startup so they need not live in ``~/.xiuqo/.env``.
``bws`` is auto-installed into ``<xiuqo_home>/bin/bws`` (one pinned version,
...
# <xiuqo_home>/cache/bws_cache.json holds only secret VALUES (never the access
...
_ENCRYPTED_CACHE_INFO = b"xiuqo-bws-encrypted-cache-v1"
...
def _hermes_bin_dir() -> Path:
    """Where Xiuqo stores its managed binaries. Profile-aware."""
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    """Managed ``<xiuqo_home>/bin/bws`` first, then PATH, then optional auto-install."""
...
def install_bws(*, force: bool = False) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="xiuqo-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
class BitwardenSource(SecretSource):
    ...
    _AUTH_HINT = (
        "Run `xiuqo secrets bitwarden token` to paste a fresh access "
        "token (create one in the Bitwarden web app: Secrets Manager → "
        "Machine accounts → Access tokens).  Wrong region?  Re-run "
        "`xiuqo secrets bitwarden setup` and pick EU/self-hosted."
    )
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        if not access_token:
            return result.fail(f"secrets.bitwarden.enabled is true but {access_token_env} is "
                               "not set.  Run `xiuqo secrets bitwarden setup`.", ErrorKind.NOT_CONFIGURED)
        project_id = str(cfg.get("project_id") or "")
        if not project_id:
            return result.fail("secrets.bitwarden.project_id is empty.  Run `xiuqo secrets bitwarden setup`.",
        ...
        if binary is None:
            return result.fail("bws binary not available and auto-install is disabled.  "
                               "Run `xiuqo secrets bitwarden setup` to install.", ErrorKind.BINARY_MISSING)
    ...
    *,
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `xiuqo secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `xiuqo secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `xiuqo secrets bitwarden setup` to install."
        )
        return result
