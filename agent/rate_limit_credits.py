...
class RateLimitCreditsMixin:
    ...
    def _capture_nous_model_switch(self, http_response: Any) -> None:
        ...
        calls by ``xiuqo_cli.anon_auth.apply_model_switch``. Fail-open."""
        headers = _response_headers(http_response)
        if not headers:
            return
        try:
            from xiuqo_cli.anon_auth import note_model_switch
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        depletion-notice bug cannot vanish silently. XIUQO_DEV_CREDITS_FIXTURE injects a chosen state instead.
        """
        try:
        ...
        if fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset XIUQO_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        dev = is_truthy_value(os.environ.get("XIUQO_DEV_CREDITS"))
        ...
        if dev:
            # XIUQO_DEV_CREDITS streams each capture to agent.log (`xiuqo logs -f`, grep 'credits ▸').
