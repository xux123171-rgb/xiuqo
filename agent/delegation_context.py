...
A Xiuqo process may itself be a Kanban dispatcher worker with XIUQO_KANBAN_* in
...
_DELEGATED_CHILD_CONTEXT: ContextVar[bool] = ContextVar("xiuqo_delegated_child_context", default=False)
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar("xiuqo_non_dispatcher_owned_context", default=False)

DELEGATED_CHILD_ENV_MARKER = "XIUQO_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "XIUQO_KANBAN_TASK", "XIUQO_KANBAN_RUN_ID", "XIUQO_KANBAN_CLAIM_LOCK",
    "XIUQO_KANBAN_GOAL_MODE", "XIUQO_KANBAN_GOAL_MAX_TURNS",
)
...
def is_dispatcher_owned_worker_context() -> bool:
    """The single predicate every ``XIUQO_KANBAN_*`` identity gate should use."""
...
def delegated_child_subprocess_env(
    ...
    """
    if not (is_delegated_child_process_context() or os.environ.get("XIUQO_KANBAN_TASK")
            or (env and (env.get("XIUQO_KANBAN_TASK") or env.get(DELEGATED_CHILD_ENV_MARKER)))):
