...
Shared by CLI ``xiuqo journey``, the TUI ``/journey`` overlay and the desktop.
Deleting a skill *archives* it (``xiuqo curator restore`` recovers it);
...
def _delete_skill(name: str) -> dict[str, Any]:
    ...
    if skill_usage.get_record(name).get("pinned"):
        return {"ok": False, "message": f"'{name}' is pinned — unpin it first (xiuqo curator unpin {name})"}
    ok, message = skill_usage.archive_skill(name)
    if ok:
        _clear_skill_cache()
    return {"ok": ok, "message": f"archived '{name}' — restore with: xiuqo curator restore {name}" if ok else message}
