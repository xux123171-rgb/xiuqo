...
thread refreshes) → disk cache (~/.xiuqo/models_dev_cache.json, any age) →
...
from xiuqo_constants import openrouter_variant_base
...
# Xiuqo provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # opencode-free is Zen-hosted (xiuqo_cli/models.py) and models.dev's "opencode" catalog lists
    ...
    # Xiuqo provider is "meta-ai"; both aliases are needed or muse-spark-*
...
# Reverse mapping: models.dev id → Xiuqo ids (built lazily; many-to-one).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_hermes_ids(mdev_id: str) -> List[str]:
    """Return the Xiuqo provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        _MODELS_DEV_TO_PROVIDER = {}
        for xiuqo_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            _MODELS_DEV_TO_PROVIDER.setdefault(mapped, []).append(xiuqo_id)
...
def _get_provider_models(provider: str, *, allow_network: bool = False) -> Optional[Dict[str, Any]]:
    """Resolve a Xiuqo provider ID to its models dict, or None if unknown.
...
# models the catalog does not know and never displace catalog data. Provider keys accept the Xiuqo
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    """Override section for *provider* (keyed by Xiuqo OR models.dev id), or None."""
    overrides = _load_model_overrides()
    provider_key = (provider or "").strip()
    if not overrides or not provider_key:
        return None
    # Forward (Xiuqo → models.dev id) and reverse (caller passed a models.dev id, config keyed by Xiuqo id) aliases.
...
def get_provider_info(provider_id: str, *, allow_network: bool = True) -> Optional[ProviderInfo]:
    """Provider metadata by Xiuqo or models.dev ID, or None if not cataloged. ``allow_network`` defaults to True (interactive setup)."""
...
def get_model_info(provider_id: str, model_id: str, *, allow_network: bool = False) -> Optional[ModelInfo]:
    """Full model metadata by Xiuqo or models.dev provider ID (exact match, then case-insensitive), or
