...
from xiuqo_cli.timeouts import get_provider_request_timeout
...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'xiuqo memory status'. Note: "
        "systemd/gateway services do not inherit ~/.xiuqo/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    with suppress(Exception):
        from xiuqo_cli.providers import XIUQO_OVERLAYS, get_provider
        overlay = XIUQO_OVERLAYS.get(provider)
    ...
    with suppress(Exception):
        ...
        from xiuqo_cli.models import normalize_provider as normalize_model_provider
        from xiuqo_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: xiuqo config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    """Per-profile marker path (``$XIUQO_HOME`` is profile-scoped; not a config key)."""
...
def _resolve_api_mode(agent, api_mode, provider_name, base_url):
    ...
    if is_actual_route(agent.provider, base_url):
        ...
        try:
            ...
            from xiuqo_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
...
def _finalize_routing(agent, api_mode, credential_pool):
    ...
    with suppress(Exception):
        from xiuqo_cli.model_normalize import (
    ...
    from xiuqo_cli.anon_auth import pin_model_for_route
...
_TURN_STATE: Dict[str, Any] = {
    ...
    # Credits tracking (dev-only, XIUQO_DEV_CREDITS) from x-nous-credits-* headers; session
...
def _init_prompt_cache_config(agent):
    ...
    agent._cache_ttl = "5m"
    with suppress(Exception):
        from xiuqo_cli.config import load_config_readonly as _load_pc_cfg
...
def _setup_logging(agent):
    ...
    from xiuqo_logging import setup_logging, setup_verbose_logging
    setup_logging(xiuqo_home=_ra()._hermes_home)
    ...
    # would starve the root file handlers. Noise reduction belongs in xiuqo_logging.
...
def _init_anthropic_client(agent, api_key, base_url, _provider_timeout):
    ...
    if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
        try:
            from xiuqo_cli.auth import build_minimax_oauth_token_provider
...
def _routed_client_kwargs(agent, fallback_model, _provider_timeout) -> Dict[str, Any]:
    ...
    if _routed_client is not None:
        from xiuqo_cli.providers import is_actual_route, normalize_provider
    ...
    if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
        ...
        raise RuntimeError(
            ...
            f"variable, or switch to a different provider with `xiuqo model`."
        )
    raise RuntimeError(
        "No LLM provider configured. Run `xiuqo model` to "
        "select a provider, or run `xiuqo setup` for first-time "
        "configuration."
    )
...
_HOST_DEFAULT_HEADERS: List[tuple[str, Callable[[Any, str], Dict[str, str]]]] = [
    ...
    ("githubcopilot.com", _lazy_headers("xiuqo_cli.models", "copilot_default_headers")),
    ...
    ("x.ai", _lazy_headers("tools.xai_http", "xiuqo_xai_default_headers")),
]
...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    # A multiplexed gateway may have switched XIUQO_HOME since model_tools was imported;
    ...
    # Kanban guidance is session-static (kanban_show iff XIUQO_KANBAN_TASK); resolve once.
...
def _publish_session_id(session_id: str) -> None:
    ...
    try:
        ...
        if not delegated_child:
            os.environ["XIUQO_SESSION_ID"] = session_id
...
def _init_session_state(agent, session_id, session_db, parent_session_id, reasoning_config, max_tokens,
    ...
    # ~/.xiuqo/sessions/ — kept unconditionally for request_dump_*.json debug breadcrumbs.
    ...
    # Process-scoped --yolo is persisted so `xiuqo --resume` restores the bypass
...
def _compression_codex_settings(cfg: Dict[str, Any]) -> tuple[str, bool, Optional[int]]:
    ...
    if app_server_auto not in {"native", "xiuqo", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, xiuqo, off.",
            app_server_auto,
        )
...
def _configured_default_base_url(_agent_cfg, _model_cfg, _custom_providers) -> str:
    ...
    if _norm in _RUNTIME_FIRST_PROVIDER_IDS:
        ...
        with suppress(Exception):
            from xiuqo_cli.auth import resolve_provider as resolve_auth_provider
...
def _select_context_engine(_agent_cfg):
    ...
    if _selected_engine is None:
        try:
            from xiuqo_cli.plugins import get_plugin_context_engine
...
def _enforce_minimum_context(agent):
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Xiuqo Agent.  Choose a model with at least "
...
def _warn_nonagentic_hermes_model(agent):
    ...
    if agent.quiet_mode or (agent.platform or "cli") == "cli":
        return
    with suppress(Exception):
        from xiuqo_cli.model_switch import _check_hermes_model_warning
    ...
    if agent.context_compressor:
        try:
            agent.context_compressor.on_session_start(
                agent.session_id, xiuqo_home=str(get_hermes_home()),
...
def init_agent(
    ...
    Non-obvious parameters:
      ...
      skip_context_files: skip SOUL.md/.xiuqo.md/AGENTS.md/CLAUDE.md/.cursorrules injection;
        load_soul_identity keeps ~/.xiuqo/SOUL.md as identity regardless.
    """
    _install_safe_stdio()
    ...
    try:
        from xiuqo_cli.config import load_config_readonly as _load_agent_config
