# Themes / Skins — Author a Xiuqo Color Theme

Author a Xiuqo **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`xiuqo_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.xiuqo/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Xiuqo itself.
...
- Write access to the Xiuqo home dir — `~/.xiuqo` by default, or `$XIUQO_HOME`
  / the active profile's dir. Skins live in `<xiuqo-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `xiuqo config set`).
...
3. `write_file` it to `<xiuqo-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `xiuqo config set display.skin default` (or they can `/skin default`).
...
```
xiuqo skin set <key> <hex>      # e.g. xiuqo skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `xiuqo skin set background "#08201f"` changes
only the background; `xiuqo skin use <name>` / `xiuqo skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.xiuqo`** when a profile is active — resolve the real home
  from `$XIUQO_HOME` first, falling back to `~/.xiuqo`.
...
- **Never hand-edit `config.yaml` to activate.** Use `xiuqo config set
...
- **You apply it, not the user.** `xiuqo config set display.skin <name>` is
...
- `read_file` the written `<xiuqo-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `xiuqo config get display.skin` and confirm it reports `<name>`.
