...
### Tool not available
1. `xiuqo tools` — check if toolset is enabled for your platform
2. Some tools need env vars (check `.env`)
3. `/reset` after enabling tools

### Model/provider issues
1. `xiuqo doctor` — check config and dependencies
2. `xiuqo auth` — re-authenticate OAuth providers (or `xiuqo auth add <provider>`)
3. Check `.env` has the right API key
4. **Copilot 403**: `gh auth login` tokens do NOT work for Copilot API. You must use the Copilot-specific OAuth device code flow via `xiuqo model` → GitHub Copilot.
...
### Skills not showing
1. `xiuqo skills list` — verify installed
2. `xiuqo skills config` — check platform enablement
3. Load explicitly: `xiuqo -s name` (or the skill's own `/<name>` slash command)

### Gateway issues
Check logs first:
```bash
grep -i "failed to send\|error" ~/.xiuqo/logs/gateway.log | tail -20
```

Common gateway problems:
- **Gateway dies on SSH logout**: Enable linger: `sudo loginctl enable-linger $USER`
- **Gateway dies on WSL2 close**: WSL2 requires `systemd=true` in `/etc/wsl.conf` for systemd services to work. Without it, gateway falls back to `nohup` (dies when session closes).
- **Gateway crash loop**: Reset the failed state: `systemctl --user reset-failed xiuqo-gateway`
...
```bash
xiuqo config set auxiliary.vision.provider <your_provider>
xiuqo config set auxiliary.vision.model <model_name>
```
