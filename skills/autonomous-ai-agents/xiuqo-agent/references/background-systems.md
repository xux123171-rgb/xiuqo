...
the `cronjob` tool, the `xiuqo cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `xiuqo curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `xiuqo curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.xiuqo/skills/.usage.json` holds
...
Users drive it via `xiuqo kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `XIUQO_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `XIUQO_KANBAN_BOARD` pinned in env); tenant is a soft namespace
