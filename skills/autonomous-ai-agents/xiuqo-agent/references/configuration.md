...
Edit with `xiuqo config edit` or `xiuqo config set section.key value`.
...
`xiuqo config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `xiuqo tools` (interactive) or `xiuqo tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_XIUQO_CORE_TOOLS` is the default bundle most platforms inherit).
