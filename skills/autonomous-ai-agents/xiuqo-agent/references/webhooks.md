...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Xiuqo agent runs by POSTing events to a URL.
...
All management is via the `xiuqo webhook` CLI command:

### Create a subscription
```bash
xiuqo webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.xiuqo/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.xiuqo/scripts/` (path traversal is blocked).

```bash
xiuqo webhook subscribe todoist-xiuqo \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-xiuqo-label.py" \
...
```bash
xiuqo webhook remove <name>
```

### Test a subscription
```bash
xiuqo webhook test <name>
xiuqo webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: new issues
```bash
xiuqo webhook subscribe github-issues \
  --events "issues" \
...
### GitHub: PR reviews
```bash
xiuqo webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
xiuqo webhook subscribe stripe-payments \
...
```bash
xiuqo webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
xiuqo webhook subscribe alerts \
...
```bash
xiuqo webhook subscribe antenna-matches \
  --deliver telegram \
...
- Subscriptions persist to `~/.xiuqo/webhook_subscriptions.json`

## How It Works

1. `xiuqo webhook subscribe` writes to `~/.xiuqo/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status xiuqo-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.xiuqo/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `xiuqo webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `xiuqo webhook test <name>` to verify the route works.
