...
├── xiuqo_state.py       # SQLite session store
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch
├── xiuqo_cli/           # CLI subcommands, config, setup, commands
...
Config: `~/.xiuqo/config.yaml` (settings), `~/.xiuqo/.env` (API keys) — both under `$XIUQO_HOME` when it is set.
...
`_XIUQO_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_hermes_home()` for paths,
never hardcode `~/.xiuqo`. For custom/local-only tools, write a plugin in
`~/.xiuqo/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `xiuqo_cli/commands.py`
...
- Tests auto-redirect `XIUQO_HOME` to temp dirs — never touch real `~/.xiuqo/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/xiuqo_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_hermes_home()` from `xiuqo_constants` for all paths (profile-safe)
