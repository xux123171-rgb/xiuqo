...
Set via `xiuqo model` (picker) or `xiuqo setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `xiuqo auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `xiuqo auth add openai-codex` |
| qwen-oauth | OAuth | `xiuqo auth add qwen-oauth` |
| minimax-oauth | OAuth | `xiuqo auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`xiuqo auth`).
Fallback chain when the primary fails: `xiuqo fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`xiuqo_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   xiuqo config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
