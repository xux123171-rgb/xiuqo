...
Write plugins for the Xiuqo desktop app: statusbar items, layout panes,
...
system (`~/.xiuqo/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$XIUQO_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$XIUQO_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.xiuqo-package.json` marker) — the ONLY root the renderer loads from — so
...
- The Xiuqo desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$XIUQO_HOME/desktop-plugins/` (usually
  `~/.xiuqo/desktop-plugins/`).

## How to Run

1. Create `$XIUQO_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.xiuqo/...` by default, or `~/.xiuqo/profiles/<profile>/...` under a
...
The ONLY import surface is `@xiuqo/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Xiuqo
  ...
  (`xiuqo://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.xiuqo/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@xiuqo/plugin-sdk`, `react`, and
