...
## Layer 1 — Is this thing a Xiuqo plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Xiuqo process**. Its LLM calls go through Xiuqo's already-configured provider. | Already uses Portal if user's Xiuqo is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Xiuqo. That LLM call is already authenticated
via Xiuqo's provider config. The plugin's own server auth (e.g.
...
What Xiuqo Agent has that *feels* like OAuth — `xiuqo login --provider nous`
opening a browser, user signs in, token lands in `~/.xiuqo/auth.json` — is a
**Xiuqo-specific browser flow**. Under the hood it produces a credential
Xiuqo uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Xiuqo's existing Portal credential out of `~/.xiuqo/auth.json`.
...
rides on the credential Xiuqo already holds.

Where this could live in Xiuqo:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Xiuqo itself.
...
1. First decide: Xiuqo plugin (runs inside Xiuqo) or separate app? If plugin,
   it already uses Portal via Xiuqo's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Xiuqo agent itself") is fully supported. Xiuqo treats agent
...
pitch in the Tool Gateway announcement is specifically about Xiuqo Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Xiuqo is configured to use Portal as its provider. It is
