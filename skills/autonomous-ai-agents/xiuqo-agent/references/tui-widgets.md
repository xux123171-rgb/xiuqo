...
Author widget apps for the Xiuqo TUI (`xiuqo --tui`): glanceable ambient
...
- The TUI must be in use (`xiuqo --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.xiuqo/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.xiuqo/logs/tui_gateway_crash.log` if a widget never appears.
