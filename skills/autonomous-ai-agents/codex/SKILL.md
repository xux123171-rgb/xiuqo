...
metadata:
  xiuqo:
    ...
    related_skills: [claude-code, xiuqo]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Xiuqo terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Xiuqo itself, `model.provider: openai-codex` uses Xiuqo-managed Codex
OAuth from `~/.xiuqo/auth.json` after `xiuqo auth add openai-codex`. For the
...
## Xiuqo Gateway Caveat

When invoking the Codex CLI from a Xiuqo gateway/service context (for example,
