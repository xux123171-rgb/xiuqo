#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Xiuqo Agent.
...
TOKEN_PATH = XIUQO_HOME / "google_token.json"
CLIENT_SECRET_PATH = XIUQO_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = XIUQO_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Xiuqo profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Xiuqo Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Xiuqo Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Xiuqo home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Xiuqo")
