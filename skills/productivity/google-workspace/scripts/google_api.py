#!/usr/bin/env python3
"""Google Workspace API CLI for Xiuqo Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Xiuqo-facing JSON contract and falls back to the Python client
...
def _gws_binary() -> str | None:
    override = os.getenv("XIUQO_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Xiuqo Agent")
