...
4. Store the token in `${XIUQO_HOME:-~/.xiuqo}/.env` (or via `xiuqo setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Xiuqo. Pipe through `python -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
- **`AIRTABLE_API_KEY` flows from `${XIUQO_HOME:-~/.xiuqo}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
