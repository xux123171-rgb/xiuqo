#!/usr/bin/env bash
# GitHub environment detection helper for Xiuqo Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _hermes_env="${XIUQO_HOME:-$HOME/.xiuqo}/.env"; [ -f "$_hermes_env" ] && grep -q "^GITHUB_TOKEN=" "$_hermes_env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_hermes_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    if [ -n "$GITHUB_TOKEN" ]; then
        GH_AUTH_METHOD="curl"
    fi
elif [ -f "$HOME/.git-credentials" ]; then
    GITHUB_TOKEN=$(uv run python3 "${XIUQO_HOME:-$HOME/.xiuqo}/skills/github/github-auth/scripts/git-credential-token.py")
    if [ -n "$GITHUB_TOKEN" ]; then
        GH_AUTH_METHOD="curl"
    fi
fi
