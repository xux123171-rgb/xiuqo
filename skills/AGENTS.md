...
- **`optional-skills/`** — heavier/niche skills shipped but NOT active; installed via
  `xiuqo skills install official/<category>/<skill>` (adapter `tools/skills_hub_official.py`
...
`[linux, macos]`, ...), `metadata.xiuqo.tags`, `metadata.xiuqo.category`,
`metadata.xiuqo.related_skills`, `metadata.xiuqo.config` (config.yaml settings the skill needs —
stored under `skills.config.<key>`, prompted during setup, injected at load). Top-level `tags:` /
`category:` are accepted and mirrored from `metadata.xiuqo.*` by the loader.
...
2. **Prose references native Xiuqo tools or the MCP servers the skill expects, in backticks**
...
4. **`author` credits the human first.** External contributor's real name + GitHub handle first,
   "Xiuqo Agent" second. A commit authored as "Xiuqo Agent" (they drafted with Xiuqo) is replaced
...
in the `xiuqo-dev` skill.

## Curator (skill lifecycle)

Background maintenance that tracks usage on agent-created skills and auto-archives stale ones;
archives go to `~/.xiuqo/skills/.archive/` and are restorable. Core `agent/curator.py` (review
loop, auto-transitions, LLM review prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots);
CLI `xiuqo_cli/curator.py` → `xiuqo curator status|run|pause|resume|pin|unpin|archive|restore|
prune|backup|rollback`; telemetry `tools/skill_usage.py` owns `~/.xiuqo/skills/.usage.json`
