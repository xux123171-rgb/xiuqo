...
Ledger location: `$XIUQO_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `XIUQO_CITATION_LEDGER`.

## How to Run

```bash
S=~/.xiuqo/skills/research/grounded-citations/scripts/sources.py
...
`xiuqo skills install official/social-media/reddit-reading` or
`xiuqo skills install official/research/rss-feeds` before using them.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `XIUQO_CITATION_LEDGER`) if their
