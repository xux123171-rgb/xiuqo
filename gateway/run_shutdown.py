...
# exit (bounded), then spawns ``xiuqo gateway restart``.
...
class GatewayShutdownMixin:
    ...
    def _pause_failed_platform(self, platform, *, reason: str = "") -> None:
        ...
        logger.warning(
            ...
            "resume %s` to retry, or `xiuqo gateway restart` to restart the gateway.",
    ...
    async def _finalize_session_off_loop(self, *, session_id: Any, platform: str, reason: str, **extra: Any) -> None:
        """Run xiuqo_cli.lifecycle.finalize_session off-loop, bounded; on timeout the worker is left alone."""
    ...
    def _restart_watcher_env() -> dict:
        """Watcher env minus ``_XIUQO_GATEWAY`` (else the CLI's self-restart guard refuses; gateway stays down)."""
        from tools.environments.local import build_subprocess_env
        watcher_env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
        watcher_env.pop("_XIUQO_GATEWAY", None)
        return watcher_env

    @staticmethod
    def _spawn_windows_restart_watcher(xiuqo_cmd: list, current_pid: int, restart_after_s: float) -> None:
        ...
        # it owns one hidden console, inherited by the `xiuqo gateway restart` child, so nothing flashes.
        # See #54220, #56747.
        ...
        watcher_argv = [
            ...
            str(current_pid), str(restart_after_s), *xiuqo_cmd, "gateway", "restart",
        ]
    ...
    async def _launch_detached_restart_command(self) -> None:
        ...
        if not xiuqo_cmd:
            logger.error("Could not locate xiuqo binary for detached /restart")
            return
        ...
        if sys.platform == "win32":
            GatewayShutdownMixin._spawn_windows_restart_watcher(xiuqo_cmd, current_pid, restart_after_s)
            return
        cmd = " ".join(shlex.quote(part) for part in xiuqo_cmd)
        shell_cmd = (
    ...
    def _wedged_agent_count(self) -> int:
        ...
        timeout = _float_env("XIUQO_AGENT_TIMEOUT", 1800)
        if timeout <= 0:
            return 0
    ...
    def _describe_active_work(self) -> list:
        ...
        (``xiuqo update``, ``xiuqo gateway status``) can name it instead of printing a bare count.
    ...
    def _start_systemd_watchdog(self) -> bool:
        ...
        watchdog.ready("Xiuqo Gateway running")
        return True
    ...
    def _stop_quiesce_and_close_session_dbs(self, timeout: float, ctx: "GatewayShutdownMixin._StopContext") -> None:
        ...
        def _close_shared() -> None:
            ...
            # (#90837).
            from xiuqo_state_registry import close_all
            closed = close_all()
            if closed:
