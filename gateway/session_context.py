"""Session-scoped context variables for the Xiuqo gateway.

Replaces the old ``os.environ``-based ``XIUQO_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "XIUQO_SESSION_PLATFORM", "XIUQO_SESSION_SOURCE", "XIUQO_SESSION_CHAT_ID",
    "XIUQO_SESSION_CHAT_TYPE", "XIUQO_SESSION_CHAT_NAME", "XIUQO_SESSION_THREAD_ID",
    "XIUQO_SESSION_USER_ID", "XIUQO_SESSION_USER_ID_ALT", "XIUQO_SESSION_USER_NAME",
    "XIUQO_SESSION_SCOPE_ID", "XIUQO_SESSION_KEY", "XIUQO_SESSION_ID",
    "XIUQO_UI_SESSION_ID", "XIUQO_SESSION_MESSAGE_ID", "XIUQO_SESSION_PROFILE",
    "XIUQO_BROWSER_CONTROL_PRINCIPAL", "XIUQO_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "XIUQO_CRON_SESSION", "XIUQO_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("XIUQO_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("XIUQO_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("XIUQO_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("XIUQO_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("XIUQO_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``XIUQO_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``XIUQO_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds XIUQO_SESSION_PLATFORM, CLI/TUI/
# desktop bind XIUQO_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``XIUQO_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("XIUQO_PLATFORM") or get_session_env("XIUQO_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("XIUQO_SESSION_SOURCE", ""))
...
def declare_stateless_channel() -> None:
    ...
    See xux123171-rgb/xiuqo#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``XIUQO_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
