...
SCALE_TO_ZERO_ENV = "XIUQO_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(xiuqo_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under XIUQO_HOME."""
    if xiuqo_home is None:
        from xiuqo_constants import get_hermes_home
        xiuqo_home = get_hermes_home()
    return Path(xiuqo_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
