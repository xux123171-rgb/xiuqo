...
class SessionTranscriptMixin:
        ...
        for dropped in remaining:
            if _spool_dropped(session_id, dropped) is None:
                logger.warning(
                    ...
                    exc_info=True)
        try:
            from xiuqo_state import divert_session_transcript_jsonl
    ...
    def _append_to_transcript_serialized(self, session_id: str, message: Dict[str, Any]) -> None:
        ...
        while True:
            try:
                ...
                from xiuqo_state import StateDbCorruptError, StateDbReplacedError
    ...
    def rewind_session(
        ...
        if not db:
            return None
        from xiuqo_state_rewind import RewindTargetUnavailableError
        with self._get_transcript_drain_lock():
            try:
