...
Hooks live in ~/.xiuqo/hooks/<name>/ with HOOK.yaml (name, description, events) and
...
def _load_hook_dir(hook_dir: Path) -> Optional[tuple]:
    ...
    module_name = f"xiuqo_hook_{hook_name}"
...
class ProfileHookRegistries:
    """``HookRegistry`` per served profile home, picked at emit time from the active XIUQO_HOME.
    ...
    first emit, i.e. inside that profile's scope (handler imports see its XIUQO_HOME); multiplexing off
    ...
    def _active(self) -> HookRegistry:
        key = xiuqo_home_key(get_hermes_home())
