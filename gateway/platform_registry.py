...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``xiuqo setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``xiuqo gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved XIUQO_HOME and overlay the
        ...
        # `xiuqo` invocation, so the import happens only when a lookup asks for it.
