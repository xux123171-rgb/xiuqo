...
class GatewayGoalsMixin:
    ...
    def _goal_max_turns_from_config(self) -> int:
        ...
        top-level ``goals`` block is only reachable via xiuqo_cli.config.load_config()."""
        try:
            goals_cfg = (
    ...
    async def _warm_goals_session_db(self, label: str) -> None:
        ...
        try:
            from xiuqo_cli.goals import _get_session_db as _warm_goals_db
    ...
    async def _post_turn_loop_completion(
        ...
        # contextvars: a bare executor hop drops the profile XIUQO_HOME override and secret scope,
    ...
    async def _loop_wakeup_fire_one(
        ...
        from xiuqo_cli.loops import LoopManager, goal_blocks_loop_tick
        ...
        # fired. The context-preserving executor keeps the profile XIUQO_HOME override under multiplex.
    ...
    async def _loop_wakeup_watcher(self, interval: float = 15.0) -> None:
        ...
        async def _scan_one_store(profile_name: Optional[str]) -> None:
            from xiuqo_cli.loops import list_active_loops
