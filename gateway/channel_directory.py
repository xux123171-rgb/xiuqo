...
Built on gateway startup, refreshed every 5 min, saved to ~/.xiuqo/channel_directory.json.
...
def format_directory_for_display(platforms: Optional[Dict[str, Any]] = None) -> str:
    ...
    ``platforms`` overrides the on-disk directory (``xiuqo send --list`` merges in
