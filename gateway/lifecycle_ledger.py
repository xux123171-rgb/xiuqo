...
``<XIUQO_HOME>/state/gateway.lifecycle.json`` closes the gap:
...
def _process_hermes_home() -> Path:
    """XIUQO_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<XIUQO_HOME>/state/gateway.lifecycle.json``."""
...
def _report_unclean_exit(evidence: Dict[str, Any], home: Optional[Path]) -> None:
    ...
    if verdict not in ("ok", "absent"):
        logger.error(
            ...
            "missing until it is repaired. Run `xiuqo doctor`.",
            verdict,
        )
