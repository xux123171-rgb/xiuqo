...
from xiuqo_cli.config import atomic_config_write, cfg_get
...
_BUSY_MODE_BEHAVIOR = {
    "queue": ("queues for next turn", "Messages will be queued for the next turn while Xiuqo is busy."),
    "steer": ("steers into current run (after next tool call)",
              "Messages will be steered into the current run (after the next tool call)."),
    "interrupt": ("interrupts current run", "Messages will interrupt the current run while Xiuqo is busy."),
}
...
def _spawn_detached_update(xiuqo_cmd, output_path, exit_code_path) -> None:
    """Spawn ``xiuqo update --gateway`` detached so it survives the gateway restart it may trigger.
    ...
    this interpreter (not venv\\Scripts\\xiuqo.exe — that shim holds its own file open, and the
    ...
    if sys.platform == "win32":
        ...
        subprocess.Popen(
            [sys.executable, "-c", _WINDOWS_UPDATE_HELPER, str(output_path), str(exit_code_path),
             sys.executable, "-m", "xiuqo_cli.main", "update", "--gateway"],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, **windows_detach_popen_kwargs())
        return
    xiuqo_cmd_str = " ".join(shlex.quote(part) for part in xiuqo_cmd)
    update_cmd = (
        f"PYTHONUNBUFFERED=1 {xiuqo_cmd_str} update --gateway"
...
class GatewaySlashCommandsMixin(
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """The prefix users can always type to reach Xiuqo commands (adapter ``typed_command_prefix``,
    ...
    async def _handle_platform_command(self, event: MessageEvent) -> str:
        ...
        if action == "pause":
            ...
            return f"✓ {name} paused. Resume with `/platform resume {name}` or `xiuqo gateway restart` to reset."
        if not queued:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Xiuqo Agent version."""
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        # (NOT `xiuqo skills diff <name>`, which diffs a bundled skill against its stock version).
        if sub == "diff" and len(out) > 3000:
            pending_id = args[1] if len(args) > 1 else "<id>"
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.xiuqo/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_busy_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /busy — control what happens when messaging while Xiuqo is working."""
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return ("No skill bundles installed.\nCreate one on the host with:\n"
                    "  `xiuqo bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        protect privacy; ``xiuqo debug share`` from the CLI does full uploads."""
        from xiuqo_cli.debug import (_GATEWAY_PRIVACY_NOTICE, _best_effort_sweep_expired_pastes,
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update — spawn ``xiuqo update`` detached (``setsid``) so it survives the gateway
        ...
        if src.platform not in self._UPDATE_ALLOWED_PLATFORMS:
            try:
                ...
                return t("gateway.update.platform_not_messaging")
        if is_managed():
            return f"✗ {format_managed_message('update Xiuqo Agent')}"
        if not (Path(__file__).parent.parent.resolve() / '.git').exists():
            return t("gateway.update.not_git_repo")
        xiuqo_cmd = _resolve_hermes_bin()
        if not xiuqo_cmd:
            return t("gateway.update.xiuqo_cmd_not_found")
        ...
        try:
            _spawn_detached_update(xiuqo_cmd, output_path, exit_code_path)
        except Exception as e:
