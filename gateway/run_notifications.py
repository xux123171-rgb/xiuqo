...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``xiuqo update --gateway`` and its watcher exchange under XIUQO_HOME."""
    ...
    async def _watch_update_progress(
        ...
        """Watch ``xiuqo update --gateway``, streaming output + forwarding prompts.
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        "✅ Xiuqo update finished." if exit_code == 0
                        else "❌ Xiuqo update failed (exit code {}).".format(exit_code)
                    )
        ...
        if not paths.exit_code.exists():
            ...
            await _flush_buffer()
            with suppress(Exception):
                await target.send("❌ Xiuqo update timed out after 30 minutes.")
            self._clear_update_markers(paths, session_key)
        ...
        try:
            ...
            if chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    status = "✅ Xiuqo update finished." if exit_code == 0 else "❌ Xiuqo update failed."
                    msg = f"{status}\n\n```\n{output}\n```"
                else:
                    msg = (
                        "✅ Xiuqo update finished successfully." if exit_code == 0 else
                        "❌ Xiuqo update failed. Check the gateway logs or run `xiuqo update` manually for details."
                    )
    ...
    async def _send_home_channel_startup_notifications(
        ...
        message = "♻️ Gateway online — Xiuqo is back and ready."
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        if getattr(self, "_session_db_handle_cache", None) is not None:
            ...
            if not error:
                ...
                return
        from xiuqo_constants import get_default_hermes_root, profile_cli_selector
        from xiuqo_state import _default_db_path, classify_persistence_error, format_session_db_unavailable
        cause = classify_persistence_error(error)
        # Copy-pasteable, so name the real store and pin the profile: a bare `xiuqo` follows
        ...
        if cause == "corrupt":
            ...
            message = (
                ...
                f"1. Run `xiuqo {profile_arg}doctor --fix`\n"
                "2. Stop the gateway, then recover with:\n"
                f"   xiuqo {profile_arg}sessions recover --source {db_path} "
                "--inspect-only\n"
                f"   (if it reports recoverable) xiuqo {profile_arg}sessions recover "
                ...
                f"Run `xiuqo {profile_arg}doctor` for sanitized diagnostics."
            )
            ...
            message = (
                ...
                f"it is repaired: run `xiuqo {profile_arg}doctor --fix`, then restart the gateway. Do not run "
                "recovery tools or restore a backup unless `xiuqo doctor` confirms damage."
            )
        else:
            message = (
                f"⚠️ Session database unavailable — messages may not be persisted. "
                f"{format_session_db_unavailable()}\nRun `xiuqo doctor` for diagnostics."
            )
    ...
    async def _inject_watch_notification(
        ...
        if not source:
            # API-server sessions bind the RAW X-Xiuqo-Session-Id key, not a structured ``agent:...`` key.
