"""Gateway runtime status helpers: PID/lock/marker files under ``{XIUQO_HOME}`` (one set per
...
from xiuqo_constants import _get_platform_default_hermes_home, get_hermes_home
...
def _get_process_hermes_home() -> Path:
    """Launch-home XIUQO_HOME for identity files (PID, lock, status, markers):
    ``get_hermes_home()`` honors the per-session ``_XIUQO_HOME_OVERRIDE`` and would misroute
    them."""
...
def _canonical_hermes_home(path: Path | str) -> Path:
    """Stable absolute XIUQO_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_hermes_home(left: Path | str, right: Path | str) -> bool:
    """Compare XIUQO_HOME paths with the host platform's case semantics."""
...
def recorded_gateway_home_conflicts(
    ...
    """True when a persisted gateway record names a DIFFERENT XIUQO_HOME (cross-profile kill guard:
    profile B's stop must never SIGTERM profile A). ``expected_home`` overrides the comparison base.
    Legacy records without ``xiuqo_home`` prove nothing -> False; a comparison failure fails
    closed -> True."""
    recorded_home = record.get("xiuqo_home") if isinstance(record, dict) else None
...
# Mirrors xiuqo_cli.profiles._PROFILE_ID_RE -- duplicated so gateway identity code
# stays import-light (xiuqo_constants + stdlib only).
...
def _profile_label_for_home(home: Path | str) -> Optional[str]:
    ...
    default_homes = (xiuqo_constants.get_default_hermes_root, _get_platform_default_hermes_home)
...
def scoped_lock_owner_label(record: Optional[dict[str, Any]]) -> Optional[str]:
    ...
    field stamped by :func:`acquire_scoped_lock`, else inferred from ``xiuqo_home`` (old locks)."""
    if not isinstance(record, dict):
        return None
    profile = record.get("profile")
    if isinstance(profile, str) and _PROFILE_LABEL_RE.match(profile.strip()):
        return profile.strip()
    home = record.get("xiuqo_home")
...
def _get_lock_dir() -> Path:
    """Machine-local dir for token-scoped gateway locks; ``XIUQO_GATEWAY_LOCK_DIR`` overrides."""
    override = os.getenv("XIUQO_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "xiuqo" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Xiuqo gateway lifecycle subcommand from a command line, or None. No loose substring matches
    (``"gateway" in cmdline`` also matched ``gateway status`` / ``python -m tui_gateway``): needs a
    Xiuqo entrypoint plus the ``gateway`` subcommand, or a gateway-dedicated entrypoint. Tokenizes
    ...
    if any(t == "gateway/run.py" or t.endswith("/gateway/run.py") for t in tokens):
        return "run"
    if any(b in ("xiuqo-gateway", "xiuqo-gateway.exe") for b in basenames):
        return "run"
    joined = " ".join(tokens)
    if "xiuqo_cli.main" not in joined and "xiuqo_cli/main.py" not in joined and not any(
        b in ("xiuqo", "xiuqo.exe") for b in basenames
    ):
        return None
    ...
    for i, token in enumerate(filtered):
        if token == "gateway":
            # Bare `xiuqo gateway` defaults to `run`.
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    Xiuqo-owned records / cleanup scans only; ``looks_like_gateway_command_line`` stays strict."""
    return _gateway_command_subcommand(command) in {"run", "restart"}


def _looks_like_gateway_process(pid: int) -> bool:
    """True when the live PID still looks like the Xiuqo gateway."""
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    ``xiuqo_cli.gateway._matches_current_profile``): a stale state file can record a PID recycled
    onto ANOTHER profile's live gateway. Named profiles carry ``-p``/``--profile <name>`` or
    ``XIUQO_HOME=`` on argv; the default gateway runs bare. Separators normalized."""
    command_lc = command.lower().replace("\\", "/")
    profile_name = _profile_name_for_home(profile_home)
    home_lc = str(profile_home).lower().replace("\\", "/")
    if profile_name is not None and profile_name != "default":
        return profile_flag_value(command_lc) == profile_name.lower() or f"xiuqo_home={home_lc}" in command_lc
    # Default profile: accept unless argv names another profile or a conflicting explicit
    # XIUQO_HOME= (its absence is not disqualifying -- XIUQO_HOME usually arrives via the env).
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    return not ("xiuqo_home=" in command_lc and f"xiuqo_home={home_lc}" not in command_lc)
...
def _build_pid_record() -> dict:
    return {
        ...
        "xiuqo_home": str(_canonical_hermes_home(_get_process_hermes_home())),
    }
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    Lazy import keeps ``gateway.status`` free of ``xiuqo_cli`` at import time. Never raises.

    A gateway keeps serving the module versions it imported at startup, so stamping the identity into
    ``gateway_state.json`` lets `xiuqo update` (and the dashboard) prove whether a running gateway actually
...
def _pid_record_belongs_to_current_profile(record: Optional[dict[str, Any]]) -> bool:
    """True when the record's ``xiuqo_home`` matches the current process (legacy records: True);
    another XIUQO_HOME's record must be ignored or the default gateway assumes its identity."""
    if not isinstance(record, dict):
        return False
    record_home = record.get("xiuqo_home")
...
def multiplexer_liveness_for_profile(profile_dir: Path) -> Optional[tuple[int, dict[str, Any]]]:
    ...
    dashboard ladder reports it stopped while ``xiuqo -p X status`` says running — the two must agree.
    """
    name = _profile_name_for_home(Path(profile_dir))
    if not name:
        return None
    from xiuqo_cli.gateway import named_profile_served_by_running_multiplexer
    from xiuqo_cli.gateway_multiplex_served import live_default_gateway_pid
...
def resolve_gateway_liveness(
    *, profile_dir: Optional[Path] = None, runtime: Any = _UNSET,
    ...
    # the multiplexer IS its gateway (mirrors `xiuqo -p X status` / `gateway list`). Unscoped, the
    # question is about the process's OWN home — which is a named profile inside a pooled
    # `xiuqo --profile X serve` (the Desktop's per-profile backend answers its REST without
...
def get_runtime_status_running_pid(
    ...
    if pid is None:
        return None
    # Active-profile context: the record's xiuqo_home must match this process so a stale record
...
def acquire_scoped_lock(
    ...
    # does not say WHICH profile). Omitted when not inferable; readers fall back to xiuqo_home.
...
# token. ``xiuqo.service`` + ``xiuqo- gateway.service``). See #5646.
...
def _get_takeover_marker_path(xiuqo_home: Optional[Path] = None) -> Path:
    """Takeover marker path; ``xiuqo_home`` is given only for a verified cross-home handoff."""
    home = _canonical_hermes_home(xiuqo_home or _get_process_hermes_home())
...
def _pid_marker_names_self(target_pid: int, target_start_time: Any) -> bool:
    ...
    ``xiuqo gateway stop`` as an unexpected exit revived by the service manager."""
...
def _consume_pid_marker_for_self(path: Path, *, ttl_s: int) -> bool:
    ...
    # cross-XIUQO_HOME --replace while ignoring a marker accidentally written into another
...
def _validated_scoped_lock_gateway_owner(record: dict[str, Any]) -> Optional[tuple[int, int, Path]]:
    ...
    raw_home = record.get("xiuqo_home")
    if (
    ...
    pid_record_home = pid_record.get("xiuqo_home")
    if (
