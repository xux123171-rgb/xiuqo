...
class GatewayTurnMixin:
    ...
    def _resolve_session_agent_runtime(
        ...
        # Provider resolved but no model.default (`xiuqo auth add` without `xiuqo model`): use the
    ...
    def _resolve_turn_agent_config(self, user_message: str, model: str, runtime_kwargs: dict) -> dict:
        ...
        from xiuqo_cli.models import resolve_fast_mode_overrides
    ...
    async def _hmwa_acquire_turn_lease(self, _quick_key, run_generation, session_entry, _session_env_tokens):
        ...
        try:
            _lease_token = await _lease_registry.acquire(
                ...
                timeout=_float_env("XIUQO_TURN_LEASE_TIMEOUT", DEFAULT_LEASE_WAIT),
            )
    ...
    async def _hmwa_hygiene_settings(self, source, session_key):
        ...
        try:
            ...
            if hs.config_context_length is not None:
                try:
                    from xiuqo_cli.route_identity import should_clear_context_pin_async
    ...
    async def _hmwa_hygiene_detached_attempt(
        ...
        try:
            ...
            attempt.future = loop.run_in_executor(
                ...
                # secret scope / XIUQO_HOME override live in ContextVars, and a bare run_in_executor worker
        ...
        if not home_env:
            with suppress(Exception):
                ...
                if prof and prof != "default" and _lgc().get_home_channel(source.platform):
                    home_env = "set"
        if not home_env:
            # Slack routes every command through the parent `/xiuqo`; bare `/sethome` would fail.
            sethome_cmd = "/xiuqo sethome" if source.platform == Platform.SLACK else "/sethome"
            await self._deliver_platform_notice(
                source, f"📬 No home channel is set for {platform_name.title()}. "
                f"A home channel is where Xiuqo delivers cron job results and cross-platform "
    ...
    def _hmwa_apply_message_timestamp(self, event, message_text):
        ...
        try:
            from xiuqo_time import get_timezone as _get_evt_tz
    ...
    async def _execute_mcp_reload(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # a browser OAuth flow either: an expired token parks with a `xiuqo mcp login` hint.
    ...
    async def _run_agent_via_proxy(
        ...
        """Forward the message to a remote Xiuqo API server instead of running a local AIAgent.
        ...
        # OpenAI chat format. The remote keeps continuity via X-Xiuqo-Session-Id; send the current
        ...
        body = {"model": "xiuqo", "messages": api_messages, "stream": True}
    ...
    def _run_agent_display_settings(self, source: SessionSource) -> "GatewayRunner._RunAgentDisplay":
        ...
        # Tool progress mode; XIUQO_TOOL_PROGRESS_MODE wins only when the config never set it.
        _resolved_tp = resolve_display_setting(user_config, platform_key, "tool_progress")
        _env_tp = os.getenv("XIUQO_TOOL_PROGRESS_MODE")
        ...
        # "log" mode: tool calls go to ~/.xiuqo/logs/tool_calls.log instead of the chat. Gateway-only.
    ...
    async def _run_agent_write_tool_log(self, log_queue: Any) -> None:
        ...
        tool_logger = logging.getLogger(f"xiuqo.tool_calls.{id(log_queue)}")
    ...
    def _run_agent_start_turn_worker(self, turn_ctx: TurnContext, run_sync: Callable[[], Any]) -> "GatewayRunner._RunAgentWorker":
        ...
        *Inactivity* timeout (agent.gateway_timeout / XIUQO_AGENT_TIMEOUT, env wins; 0 = unlimited),
        ...
        _agent_timeout, _agent_warning = (
            v if v > 0 else None
            for v in (_float_env("XIUQO_AGENT_TIMEOUT", 1800), _float_env("XIUQO_AGENT_TIMEOUT_WARNING", 900))
        )
    ...
    def _run_agent_evict_on_fallback(self, turn_ctx: TurnContext) -> None:
        ...
        with suppress(Exception):
            from xiuqo_cli.model_normalize import _AGGREGATOR_PROVIDERS, normalize_model_for_provider
    ...
    async def _run_agent_drain_pending(
        ...
        if pending and pending.strip().startswith("/"):
            ...
            if _pending_cmd_word:
                with suppress(Exception):
                    from xiuqo_cli.commands import resolve_command as _rc_pending
    ...
    async def _run_agent_notify_long_running(
        ...
        Interval: agent.gateway_notify_interval / XIUQO_AGENT_NOTIFY_INTERVAL (default 180s; 0 or
        ...
        _NOTIFY_INTERVAL = _float_env("XIUQO_AGENT_NOTIFY_INTERVAL", 180)
