...
``gateway.<key>`` form (what ``xiuqo config set gateway.<key>`` produces) is consulted only when the
...
#
# Top-level settings are also accepted nested under ``gateway:`` (what ``xiuqo config set
...
def read_yaml_layers(home: Path) -> dict:
    ...
    # xiuqo_cli.config.load_config, so managed quick_commands / stt would otherwise be ignored).
