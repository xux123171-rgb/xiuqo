...
boundary. POSIX: ``$XIUQO_HOME/gateway.sock`` (or a temp-dir socket + ``gateway.sock.path`` pointer
file when the home path exceeds ``sun_path``); Windows: named pipe ``\\\\.\\pipe\\xiuqo-gateway-<hash>``.
...
def windows_pipe_name(home: Path) -> str:
    """Per-XIUQO_HOME named pipe path (Windows transport)."""
    return rf"\\.\pipe\xiuqo-gateway-{_home_hash(home)}"
...
def _fallback_socket_path(home: Path) -> Path:
    ...
    name = f"xiuqo-gw-{_home_hash(home)}.sock"
...
def _detect_supervisor() -> str:
    ...
    if env.get("INVOCATION_ID"):
        return "systemd"
    if sys.platform == "darwin" and (env.get("XPC_SERVICE_NAME", "").startswith("ai.xiuqo")
                                     or env.get("LAUNCHD_SOCKET")):
        return "launchd"
    if env.get("XIUQO_DESKTOP_MANAGED"):
        return "desktop"
...
def build_identify_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        **{k: record.get(k) for k in ("kind", "pid", "start_time", "xiuqo_home")},
        "profile": _profile_label_for_home(record.get("xiuqo_home") or ""),
