...
(`xiuqo_cli/AGENTS.md`). Each adapter picks a base toolset (Telegram → `"messaging"`).
...
`if canonical == ...` chains. Registry + adding a command: `xiuqo_cli/AGENTS.md`.
...
duplicate-final incident (NS-658 canary ledger, xiuqo#85796 / gateway-gateway#210); violating any
...
(or `XIUQO_BACKGROUND_NOTIFICATIONS`): `concise` (default; one line, failures append an output
...
`/login` is registered in `xiuqo_cli/commands.py` with `busy_policy="dispatch"` and
...
`xiuqo serve` (control plane, desktop-spawned child) dies with the app — by design. The messaging
