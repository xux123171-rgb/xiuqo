...
All hooks write atomic JSON payloads under ``<xiuqo_home>/pending_messages/``:
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active XIUQO_HOME."""
...
def spool_dropped_transcript_message(session_id: str, message: Dict[str, Any]) -> Optional[Path]:
    ...
    message under ``<xiuqo_home>/pending_messages/``), so a runtime cap rotation no longer silently
