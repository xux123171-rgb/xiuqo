...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: xiuqo gateway restart"
        ),
    )
...
class GatewayModelCommandsMixin:
    ...
    async def _perform_model_switch(
        ...
        try:
            from xiuqo_cli.context_switch_guard import enrich_model_switch_warnings_for_gateway
    ...
    async def _record_model_switch(
        ...
        from xiuqo_cli.model_switch import format_model_for_display
    ...
    async def _model_switch_confirmation(
        ...
        from gateway.run import _load_gateway_config
        from xiuqo_cli.model_switch import format_model_for_display, resolve_display_context_length_async
    ...
    async def _send_model_picker(self, event: MessageEvent, source, adapter, session_key: str, listing_kwargs: dict, on_model_selected) -> bool:
        ...
        from xiuqo_cli.model_switch_providers import list_picker_providers
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        ctx = _ModelSwitchContext(
            ...
            # the caller's). user_id is critical for the fallback lookup path (xiuqo_state.py:1994-2009)
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        """Handle /personality — list or set a personality (xiuqo_cli.personality owns the state)."""
        from gateway.run import _load_gateway_config
