...
``<XIUQO_HOME>/state/gateway.heartbeat`` so supervisors can tell "process alive" from "loop
...
def _process_hermes_home() -> Path:
    """XIUQO_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_tick_socket_path(home: Optional[Path] = None, pid: Optional[int] = None) -> Path:
    """``<XIUQO_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a stale node from a
