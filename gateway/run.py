...
# xiuqo_bootstrap must be the very first import (UTF-8 stdio on Windows; no-op on POSIX).
try:
    import xiuqo_bootstrap  # noqa: F401
except ModuleNotFoundError:
    pass  # a partial ``xiuqo update`` can leave the bootstrap unregistered; only Windows UTF-8 stdio suffers
...
from xiuqo_cli.config import _is_ssh_remote_tilde_cwd, cfg_get
from xiuqo_cli.fallback_config import get_fallback_chain
...
async def run_codex_hygiene_compaction(
    ...
    * Evicting the cached live agent afterwards destroys the only real context: the next turn spawns an
    EMPTY thread and the model starts blank while Xiuqo still mirrors a full history (abrupt amnesia — the
    user-facing damage documented on #73503).
    """
    mode = str(auto_mode or "native").lower()
    if mode not in {"native", "xiuqo", "off"}:
        mode = "native"
    if mode != "xiuqo":
    ...
    # copy_context carries profile secret scope / XIUQO_HOME override (executors don't propagate ContextVars).
...
def _ensure_windows_gateway_venv_imports() -> None:
    """Make detached Windows gateway runs see the Xiuqo venv packages.
...
def _telegramize_command_mentions(text: str, platform: Any) -> str:
    ...
    from xiuqo_cli.commands_platforms import _sanitize_telegram_name
...
def _coerce_gateway_timestamp(value: Any) -> Optional[float]:
    ...
    if isinstance(value, (int, float)):
        # Some platform events use milliseconds; Xiuqo state rows use seconds.
...
def _startup_restore_drain_timeout_secs() -> float:
    ...
    """
    return _float_env("XIUQO_STARTUP_RESTORE_DRAIN_TIMEOUT", _STARTUP_RESTORE_DRAIN_TIMEOUT_SECS_DEFAULT)
...
def _startup_warmup_timeout_secs() -> float:
    ...
    return _float_env("XIUQO_STARTUP_WARMUP_TIMEOUT", _STARTUP_WARMUP_TIMEOUT_SECS_DEFAULT)
...
def _build_gateway_agent_history(
    ...
    from xiuqo_time import get_timezone as _get_msg_tz
...
os.environ["_XIUQO_GATEWAY"] = "1"

_ensure_ssl_certs()

sys.path.insert(0, str(Path(__file__).parent.parent))

from xiuqo_constants import get_hermes_home, get_hermes_home_override
_hermes_home = get_hermes_home()

# Load ~/.xiuqo/.env first: user-managed env files must override stale shell exports on restart.
from xiuqo_cli.env_loader import load_hermes_dotenv
_env_path = _hermes_home / '.env'
load_hermes_dotenv(xiuqo_home=_hermes_home, project_env=Path(__file__).resolve().parents[1] / '.env')


def _reload_runtime_env_preserving_config_authority() -> None:
    """Reload .env per turn for rotated keys while config.yaml stays authoritative for budgets (else a
    stale XIUQO_MAX_ITERATIONS wins). Multiplex never reloads .env globally: secrets come from the
    ...
    if not is_multiplex_active():
        load_hermes_dotenv(
            xiuqo_home=_hermes_home, project_env=Path(__file__).resolve().parents[1] / '.env')
...
def _bridge_max_turns_from_config(home: "Path") -> None:
    ...
    xiuqo_state reads the routed profile's ``sessions.*`` from its own config under scope."""
...
def _current_max_iterations() -> int:
    ...
    ``int()`` crash. A routed profile (XIUQO_HOME override, multiplexed turns) reads ITS
    ``agent.max_turns`` straight from config: the ``XIUQO_MAX_ITERATIONS`` bridge is one process-wide
    slot holding the launch profile's value, so every secondary would inherit the default's budget."""
    _reload_runtime_env_preserving_config_authority()
    from xiuqo_cli.config import resolve_turn_limit as _resolve_turn_limit
    ...
    return _resolve_turn_limit(os.getenv("XIUQO_MAX_ITERATIONS"))
...
def _multiplex_profile_homes(config: object) -> list[tuple[str, "Path"]]:
    ...
    from xiuqo_cli.profiles import profiles_to_serve
...
def _cron_tick_profile_homes(config: object) -> list[tuple[str, "Path"]]:
    ...
    XIUQO_HOME). Adapter startup already skips ``active``."""
    from xiuqo_cli.profiles import get_active_profile_name, get_profile_dir
...
def _load_profile_secret_scope(profile_home: "Path") -> dict:
    ...
    from xiuqo_constants import set_hermes_home_override, reset_hermes_home_override
    # Caller already hydrated external sources off-loop (#99519).
    from agent.secret_scope import build_profile_secret_scope
    from xiuqo_cli.env_loader import hydrate_profile_secret_sources
...
async def _discover_gateway_mcp_tools(config: object) -> None:
    ...
    actionable ``xiuqo mcp login`` warning instead of opening an authorize tab.
    """
...
def _platform_has_bot_credential(platform: "Platform", platform_config: "PlatformConfig") -> bool:
    ...
    # ~/.xiuqo/.env, so an env fallback would report "has credential" for every Matrix config on the box —
...
from xiuqo_cli.config_defaults import DEFAULT_CONFIG as _DEFAULT_CONFIG
os.environ["XIUQO_TURN_LEASE_TIMEOUT"] = str(_DEFAULT_CONFIG["agent"]["gateway_turn_lease_timeout"])
...
_AGENT_ENV_BRIDGE = {
    "gateway_timeout": "XIUQO_AGENT_TIMEOUT",
    "gateway_turn_lease_timeout": "XIUQO_TURN_LEASE_TIMEOUT",
    "gateway_timeout_warning": "XIUQO_AGENT_TIMEOUT_WARNING",
    "gateway_notify_interval": "XIUQO_AGENT_NOTIFY_INTERVAL",
    "session_stall_timeout": "XIUQO_SESSION_STALL_TIMEOUT",
    # Internal bridge only — config.yaml (agent.reconnect_attention_after) is the documented setting.
    "reconnect_attention_after": "XIUQO_RECONNECT_ATTENTION_AFTER_SECONDS",
    "restart_drain_timeout": "XIUQO_RESTART_DRAIN_TIMEOUT",
    "cron_drain_timeout": "XIUQO_CRON_DRAIN_TIMEOUT",
    "gateway_auto_continue_freshness": "XIUQO_AUTO_CONTINUE_FRESHNESS",
    "gateway_startup_restore_drain_timeout": "XIUQO_STARTUP_RESTORE_DRAIN_TIMEOUT",
    "gateway_startup_warmup_timeout": "XIUQO_STARTUP_WARMUP_TIMEOUT"}
# config-authoritative knobs for the session-search index (env stays the cross-process carrier).
_SESSIONS_ENV_BRIDGE = {"cjk_fts": "XIUQO_CJK_FTS", "search_slow_ms": "XIUQO_SEARCH_SLOW_MS"}
_DISPLAY_ENV_BRIDGE = {
    "busy_input_mode": "XIUQO_GATEWAY_BUSY_INPUT_MODE",
    "busy_text_mode": "XIUQO_GATEWAY_BUSY_TEXT_MODE",
    "busy_ack_enabled": "XIUQO_GATEWAY_BUSY_ACK_ENABLED"}
...
def _bridge_max_turns_to_env(agent_cfg: Any) -> None:
    ...
    if raw is not None:
        os.environ["XIUQO_MAX_ITERATIONS"] = str(raw)
    elif "XIUQO_MAX_ITERATIONS" in os.environ:
        del os.environ["XIUQO_MAX_ITERATIONS"]
...
def _bridge_auxiliary_config_to_env(_auxiliary_cfg: dict) -> None:
    ...
    try:
        from xiuqo_cli.plugins import get_plugin_auxiliary_tasks
...
def _bridge_config_to_env(_cfg: dict) -> None:
    ...
    # (e.g. XIUQO_MAX_ITERATIONS=60 written by an old `xiuqo setup` run) silently shadow the user's
    ...
    if (isinstance(_display_cfg, dict) and "busy_steer_ack_enabled" in _display_cfg
            and "XIUQO_GATEWAY_BUSY_STEER_ACK_ENABLED" not in os.environ):
        os.environ["XIUQO_GATEWAY_BUSY_STEER_ACK_ENABLED"] = str(_display_cfg["busy_steer_ack_enabled"])
    _tz_cfg = _cfg.get("timezone", "")
    if _tz_cfg and isinstance(_tz_cfg, str):
        os.environ["XIUQO_TIMEZONE"] = _tz_cfg.strip()
    _security_cfg = _cfg.get("security", {})
    if isinstance(_security_cfg, dict) and _security_cfg.get("redact_secrets") is not None:
        os.environ["XIUQO_REDACT_SECRETS"] = str(_security_cfg["redact_secrets"]).lower()
    # Media policy uses the shared bridge so standalone entrypoints (`xiuqo cron run`) match.
    ...
    if isinstance(_gateway_cfg, dict):
        ...
        if _trust_recent_seconds is not None:
            os.environ["XIUQO_MEDIA_TRUST_RECENT_SECONDS"] = str(_trust_recent_seconds)
        ...
        if ("platform_connect_timeout" in _gateway_cfg
                and not os.environ.get("XIUQO_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()):
            os.environ["XIUQO_GATEWAY_PLATFORM_CONNECT_TIMEOUT"] = str(_gateway_cfg["platform_connect_timeout"])
...
def _load_bridge_config(config_path: Path) -> dict:
    ...
    from xiuqo_cli.config_effective import load_user_config_effective
...
if _config_path.exists():
    try:
        ...
        print(
            ...
            "your current config.yaml. Run `xiuqo doctor` to investigate.",
            file=sys.stderr)

# IPv4 preference must apply before any HTTP clients are created.
try:
    from xiuqo_constants import apply_ipv4_preference
...
try:
    from xiuqo_cli.config import print_config_warnings
...
os.environ["XIUQO_QUIET"] = "1"  # gateway runs quiet: no debug output, cwd used directly

# XIUQO_EXEC_ASK is set in start_gateway(), NOT at import: CLI tools importing this module must not
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    from xiuqo_cli.runtime_provider import (
        resolve_runtime_provider, format_runtime_provider_error, _get_model_config)
    from xiuqo_cli.auth import AuthError, is_rate_limited_auth_error
...
def _resolve_gateway_model_context(model: Optional[str] = None) -> _GatewayModelContext:
    ...
    def _read_config() -> None:
        ...
        try:
            from xiuqo_cli.config import get_compatible_custom_providers
    ...
    def _pin_still_applies() -> bool:
        ...
        from xiuqo_cli.route_identity import should_clear_context_pin
    ...
    def _custom_ctx() -> Optional[int]:
        from xiuqo_cli.config import get_custom_provider_context_length
...
def _resolve_runtime_agent_kwargs_for_provider(provider: str) -> dict:
    ...
    from xiuqo_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error
    try:
...
def _deep_merge_request_overrides(base: Optional[dict], override: Optional[dict]) -> dict:
    ...
    from xiuqo_cli.config import _deep_merge
...
def _try_resolve_fallback_provider() -> dict | None:
    ...
    from xiuqo_cli.runtime_provider import resolve_runtime_provider
    try:
        ...
        for entry in fb_list:
            try:
                from xiuqo_cli.fallback_config import resolve_entry_api_key
                runtime = resolve_runtime_provider(
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if slug == normalized and declared_name in disabled:
                    return (
                        ...
                        f"Enable it with: `xiuqo skills config`")

        # Check optional skills (shipped with repo but not installed)
        from xiuqo_constants import get_optional_skills_dir
        ...
        if optional_dir.exists():
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                return (
                    ...
                    f"Install it with: `xiuqo skills install {install_path}`")
    except Exception:
        pass
    return None
...
def _gateway_config_home() -> Path:
    """Return the Xiuqo home that gateway config reads should use."""
    override = get_hermes_home_override()
...
def _checkpoint_agent_kwargs(config: dict | None) -> dict:
    ...
    if isinstance(cp_cfg, bool):
        ...
        cp_cfg = {}
    from xiuqo_cli.config import DEFAULT_CONFIG
...
def _resolve_hermes_bin() -> Optional[list[str]]:
    """Xiuqo update command argv: ``xiuqo`` on PATH, else ``python -m xiuqo_cli.main``, else None."""
    import shutil
    xiuqo_bin = shutil.which("xiuqo")
    if xiuqo_bin:
        return [xiuqo_bin]
    try:
        import importlib.util
        if importlib.util.find_spec("xiuqo_cli") is not None:
            return [sys.executable, "-m", "xiuqo_cli.main"]
    except Exception:
        pass
    return None
...
_RECONNECT_ATTENTION_AFTER_SECONDS = _float_env("XIUQO_RECONNECT_ATTENTION_AFTER_SECONDS", 7200)
...
_BUILTIN_ADAPTERS: dict[Platform, tuple[str, str, str, str]] = {
    Platform.WHATSAPP_CLOUD: ("whatsapp_cloud", "WhatsAppCloudAdapter", "check_whatsapp_cloud_requirements",
                              "WhatsApp Cloud: aiohttp/httpx missing — reinstall xiuqo"),
...
class GatewayRunner(
    ...
    def _init_startup_checks(self) -> None:
        ...
        try:
            from xiuqo_cli.config import load_config as _load_full_config
    ...
    def _init_session_db(self) -> None:
        ...
        try:
            ...
            # WARNING (not DEBUG) so it lands in errors.log; else an NFS XIUQO_HOME silently loses /resume etc.
        ...
        try:
            ...
            if _ckpt_cfg.get("auto_prune", False):
                ...
                # unmounted share); orphan cleanup is only via explicit `xiuqo checkpoints prune`.
    ...
    def _open_session_db_for_active_scope(self, raise_on_error: bool = False) -> Any:
        ...
        ``SessionDB()`` reads the context-local XIUQO_HOME; one handle cached per path. Construction
        ...
        resolves ``_default_db_path()`` at call time through the context-local XIUQO_HOME override
        ...
        #88235 broadcast.
        """
        from xiuqo_state import AsyncSessionDB, _default_db_path
        from xiuqo_state_registry import acquire
        from gateway.session_db_recovery import RecoverableHandleCache
    ...
    def close_all_session_db_handles(self) -> None:
        ...
        def _close(db) -> None:
            ...
            # instead (#90837).
            from xiuqo_state_registry import release_or_close
            try:
    ...
    def _warn_if_docker_media_delivery_is_risky(self) -> None:
        ...
        logger.warning(
            ...
            "output mount (for example '/home/user/.xiuqo/cache/documents:/output') is configured. "
    ...
    def _update_runtime_status(self, gateway_state: Optional[str] = None, exit_reason: Optional[str] = None) -> None:
        # ``active_work`` names each unit only while draining — that is when an observer (``xiuqo
    ...
    def _thread_metadata_for_source(
        ...
        if profile and metadata is not None:
            ...
            metadata["xiuqo_profile"] = profile
        return metadata
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=10, thread_name_prefix="xiuqo-gateway")
    ...
    def _resolve_profile_home_for_source(self, source: SessionSource) -> "Path":
        """Resolve which profile's XIUQO_HOME serves this source: ``source.profile``, then
        ``_profile_name_for_source`` (sources bypassing ``build_source``), then the active profile."""
        from gateway.profile_routing import ProfileRouteRejected
        from xiuqo_cli.profiles import get_active_profile_name, get_profile_dir, profile_exists
        ...
        try:
            ...
            if explicit_profile and not profile_exists(name):
                logger.warning(
                    ...
                    "falling back to global XIUQO_HOME",
            ...
            logger.warning(
                ...
                "falling back to global XIUQO_HOME: %s",
...
def _run_planned_stop_watcher(
    ...
    ``add_signal_handler``, so ``xiuqo gateway stop`` would never drain). Runs everywhere; on POSIX
    ...
    standard signal-driven shutdown path never runs when ``xiuqo gateway stop`` signals the gateway. The
...
def _housekeeping_paste_sweep() -> None:
    from xiuqo_cli.debug import _sweep_expired_pastes
...
def _housekeeping_auto_archive() -> None:
    ...
    from xiuqo_cli.config import load_config as _load_full_config
    from xiuqo_state_registry import acquire, release_or_close
    _sess_cfg = (_load_full_config().get("sessions") or {})
    if _sess_cfg.get("auto_archive", False):
        _adb = acquire()
        try:
...
def _housekeeping_deferred_fts_retry() -> None:
    ...
    from xiuqo_state_registry import borrow_live_shared_session_dbs
...
def _housekeeping_memory_trim() -> None:
    ...
    from xiuqo_cli.mem_trim import trim_memory
...
def _replace_target_belongs_to_other_profile(existing_pid: int) -> bool:
    ...
    by exact PID + start-time; live argv can never PROVE ownership (no XIUQO_HOME), it is only a
    ...
    # (no service supervisor — e.g. `xiuqo gateway run` on Windows) is at risk: its PID never joins the
    ...
    try:
        ...
        if _get_process_start_time(existing_pid) != recorded_start:
            return refuse("pid record start-time does not match the live process %s (stale/PID-reuse record).",
                          existing_pid)
        recorded_home = record.get("xiuqo_home")
        if not isinstance(recorded_home, str) or not recorded_home.strip():
            return refuse("pid record predates xiuqo_home stampings; ownership of PID %s unprovable.",
                          existing_pid)
        if not _same_hermes_home(recorded_home, our_home):
            return refuse("pid record belongs to a different XIUQO_HOME (%s, ours %s). Remove the stale PID "
                          "record or stop the owning profile explicitly.", recorded_home, our_home,
                          level=logging.ERROR)
        # Argv never proves ownership; an explicit contradicting --profile / XIUQO_HOME= still refuses.
        ...
        if live_cmdline and _looks_like_profile_conflict_from_cmdline(live_cmdline, our_home):
            return refuse("target PID %s command line explicitly advertises a different profile than "
                          "XIUQO_HOME %s.", existing_pid, our_home, level=logging.ERROR)
        return False
    except Exception:
...
def _looks_like_profile_conflict_from_cmdline(command: str, our_home) -> bool:
    ...
    def _env_home_value() -> Optional[str]:
        """XIUQO_HOME=<path> env-style assignment on the argv, token-exact."""
        prefix = "XIUQO_HOME="
    ...
    for flag in ("--profile", "-p"):
        ...
        if profile_name is None or profile_name == "default" or value != profile_name:
            return True
    home_value = _flag_value("--xiuqo-home") or _env_home_value()
...
async def _start_gateway_replace_existing_instance(existing_pid: int, replace: bool) -> bool:
    """Handle a live gateway PID under this XIUQO_HOME: replace it (``--replace``) or refuse.
    ...
    if not replace:
        xiuqo_home = str(get_hermes_home())
        logger.error(
            "Another gateway instance is already running (PID %d, XIUQO_HOME=%s). "
            "Use 'xiuqo gateway restart' to replace it, or 'xiuqo gateway stop' first.",
            existing_pid, xiuqo_home)
        print(
            f"\n❌ Gateway already running (PID {existing_pid}).\n"
            f"   Use 'xiuqo gateway restart' to replace it,\n"
            f"   or 'xiuqo gateway stop' to kill it first.\n"
            f"   Or use 'xiuqo gateway run --replace' to auto-replace.\n")
        return False
    ...
    if _replace_target_belongs_to_other_profile(existing_pid):
        ...
        logger.error(
            ...
            "to this profile's gateway (XIUQO_HOME %s). Remove the "
...
def _start_gateway_configure_logging(verbosity: Optional[int]) -> None:
    ...
    from xiuqo_logging import setup_logging, _safe_stderr
    setup_logging(xiuqo_home=_hermes_home, mode="gateway")

    def _security_audit() -> None:
        # Warn-on-load, never blocks: surfaces root / weak-SSH / unauthenticated-listener exposure.
        from xiuqo_cli.security_audit_startup import log_startup_security_warnings

        def _raw_cfg():
            from xiuqo_cli.config import read_raw_config
            return read_raw_config()

        log_startup_security_warnings(xiuqo_home=_hermes_home, config=_best_effort(_raw_cfg))
...
async def _start_gateway_start_control_socket(runner):
    ...
    try:
        ...
        # becomes the authoritative gateway for its XIUQO_HOME, so from here on "does a socket answer?" is
        ...
        def _pause_for_update_handler() -> dict:
            try:
                from xiuqo_cli.gateway import _get_restart_drain_timeout
        ...
        def _rescan_profiles_handler() -> dict:
            """``xiuqo profile create/delete`` asks the multiplexer to reconcile ``profiles/`` now
    ...
    if not isinstance(cron_provider, InProcessCronScheduler):
        ...
        if not _has_api_server:
            logger.warning(
                ...
                "the gateway through its supervisor (`xiuqo gateway "
...
async def _start_gateway_shutdown_tail(
    ...
    def _stop_keepalive() -> None:
        from xiuqo_cli.nous_auth_keepalive import stop_nous_auth_keepalive
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Duplicate-instance guard scoped to XIUQO_HOME; distinct-home multi-profile setups coexist.
    ...
    # Windows has no add_signal_handler, so `xiuqo gateway stop`'s SIGTERM would never drain; poll the
    # planned-stop marker (written BEFORE the kill) instead. Runs everywhere so masked-SIGTERM drains.
    # Windows fallback: asyncio.add_signal_handler raises NotImplementedError on Windows, so `xiuqo gateway
    ...
    # sessions are silently lost across restarts (issue #33778). The fix is a marker-polling thread: `xiuqo
...
def _guard_corrupt_user_config() -> None:
    ...
    named. Same policy and escape hatch (``XIUQO_IGNORE_USER_CONFIG=1``) as ``xiuqo_cli/main.py``."""
    from xiuqo_cli.config import InvalidUserConfigError, require_parseable_user_config
...
def main():
    ...
    # Advertise the harness to children (mirrors _advertise_agent_env in xiuqo_cli/main.py, inlined to
    # avoid its startup side effects). Value must equal registry id ``xiuqo`` exactly.
    ...
    def _arm_watchdog() -> None:
        ...
        from xiuqo_startup_watchdog import arm_startup_watchdog
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Xiuqo Gateway - Multi-platform messaging")
...
def _exit_after_graceful_shutdown(exit_code: int) -> None:
    ...
    def _drain_logs() -> None:
        ...
        from xiuqo_logging import drain_log_queue
