...
failed approvals, chmod 0600 data files, codes never logged. Storage: ~/.xiuqo/pairing/
"""
...
# path would freeze the XIUQO_HOME/profile context at gateway boot, ignoring later
# context-local overrides, so the gateway and ``xiuqo pairing`` CLI wrote different
...
# computing the path eagerly freezes it to whatever XIUQO_HOME/profile context existed at that exact import
# moment for the rest of the process's lifetime -- even if a context-local override (see
# xiuqo_constants.set_hermes_home_override) is established afterward. A freshly-started, short-lived
# process (e.g. the ``xiuqo pairing`` CLI) re-imports this module later with the final environment already
...
# (xux123171-rgb/xiuqo#93449). ``_default_pairing_dir()`` below resolves this fresh on every call in
...
def _write_allowlist_env(env_var: str, ids: list) -> None:
    ...
    with contextlib.suppress(Exception):
        from xiuqo_cli.config import save_env_value, remove_env_value
...
def _load_json_file(path: Path) -> dict:
    ...
    try:
        ...
        try:
            ...
            # exec` runs as root and writes the file, then the gateway process — running as `xiuqo` after
        ...
        logger.warning(
            ...
            "If you ran `docker exec <container> xiuqo pairing approve ...` as root, "
            "re-run with `docker exec -u xiuqo <container> ...` and "
            "chown the existing file to the xiuqo user, or restart the "
...
class PairingStore:
    ...
    profile's XIUQO_HOME exactly as ``xiuqo -p <name> pairing ...`` does, so multiplex
