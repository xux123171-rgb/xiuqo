...
class GatewayAdapterLifecycleMixin:
    ...
    def _adapter_disconnect_timeout_secs(self) -> float:
        ...
        override = self._env_timeout_override("XIUQO_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT")
    ...
    def _platform_connect_timeout_secs(self, platform=None, *, initial: bool = False) -> float:
        ...
        override = self._env_timeout_override("XIUQO_GATEWAY_PLATFORM_CONNECT_TIMEOUT")
    ...
    def _flag_reconnect_needs_attention(self, platform, info: dict, now: float) -> None:
        ...
        logger.warning(
            ...
            "credentials, missing intents, broken sidecar). Check `xiuqo status` / `/platform list`.",
    ...
    async def _start_secondary_profile_adapters(self) -> int:
        ...
        Each profile connects under its own XIUQO_HOME + secret scope; credential/listener collisions
    ...
    def _record_served_profiles(self, active: str, profile_homes) -> None:
        ...
        than "has a connected adapter") for `xiuqo status`; seed per-profile PairingStores."""
    ...
    def _adapter_credential_fingerprint(adapter: Any) -> Optional[str]:
        ...
        for obj, attr in candidates:
            ...
            if isinstance(val, str) and val.strip():
                import hashlib
                return hashlib.sha256(("xiuqo-mux:" + val.strip()).encode("utf-8")).hexdigest()[:16]
        return None
