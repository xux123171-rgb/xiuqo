...
class TurnRunner:
    ...
    async def _task_card_publish(self, st) -> None:
        ...
        if not st.native_failed:
            result = await st.adapter.send_native_task_card_progress(
                chat_id=ctx.source.chat_id, tasks=st.visible_tasks(), title="Xiuqo is working",
    ...
    def _skip_context_files(self, platform_key) -> bool:
        ...
        # ``xiuqo gateway setup`` writes ``gateway.platforms`` as a LIST of enabled platform names,
    ...
    def run_sync(self):
        ...
        — never os.environ["XIUQO_SESSION_KEY"], which would misroute approvals across sessions.
        """
        ...
        # tool worker threads. We deliberately do NOT write os.environ["XIUQO_SESSION_KEY"] here:
        ...
        # *subprocess* exports XIUQO_SESSION_KEY (from its own --session-key argv, a separate process) — so
