...
def _prefix_names_served_profile(profile: str) -> bool:
    ...
    try:
        from xiuqo_cli.profiles import profile_matches_home
...
_STATIC_FEATURE_FLAGS = {
    ...
    "session_continuity_header": "X-Xiuqo-Session-Id",
    "session_key_header": "X-Xiuqo-Session-Key"}
...
_BROWSER_CONTROL_WS_PROTOCOL = "xiuqo-browser-control-v1"
_BROWSER_CONTROL_TICKET_PROTOCOL_PREFIX = "xiuqo-browser-control-ticket."
...
def _hermes_version() -> str:
    """Canonical Xiuqo version: ``xiuqo_cli.__version__`` (dist-info can be stale on
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    from xiuqo_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    try:
...
def _request_agent_overrides(
    ...
    The virtual model (``xiuqo``) means "gateway default". A bare ``model`` without
    ``provider`` is honored only when ``allow_bare_model`` (generic clients hardcode "gpt-4o";
    OpenAI-compatible handlers pass the ``direct_model_requests`` opt-in, Xiuqo-native
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        from xiuqo_state_wal import apply_wal_with_fallback
...
_CORS_HEADERS = {
    ...
    "Access-Control-Allow-Headers": "Authorization, Content-Type, Idempotency-Key, X-Xiuqo-Session-Id"}
_SECURITY_HEADERS = {
...
def _derive_chat_session_id(system_prompt: Optional[str], first_user_message: str) -> str:
    ...
    turns of an Open WebUI-style conversation), so one Xiuqo session/sandbox is reused."""
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    """aiohttp server routing OpenAI-format requests through xiuqo's AIAgent."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # executing model. Requests that send an explicit ``provider`` — and the Xiuqo-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        """Advertised /v1/models name: explicit override > active profile name > "xiuqo"
        (precedence owned by ``xiuqo_cli.model_switch.resolve_effective_model``)."""
        from xiuqo_cli.model_switch import resolve_effective_model
        profile_name = ""
        with suppress(Exception):
        ...
        return resolve_effective_model(explicit, profile_name, "xiuqo")
    ...
    def _expected_api_key(self) -> str:
        ...
        try:
            ...
            from xiuqo_cli.auth import has_usable_secret
    ...
    def _declared_conversation_session(self, gateway_session_key: Optional[str]) -> Optional[str]:
        """Resolve the live session a client declared with ``X-Xiuqo-Session-Key`` (the key
    ...
    def _parse_session_key_header(
        ...
        """Validate ``X-Xiuqo-Session-Key`` (per-channel memory scope) -> ``(key_or_None, None)``
        or ``(None, error)``. Requires API-key auth so a client can't guess another scope."""
        raw = request.headers.get("X-Xiuqo-Session-Key", "").strip()
        if not raw:
            return None, None
        if not self._api_key:
            logger.warning(
                "X-Xiuqo-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping.")
            return None, _error_response(
                "X-Xiuqo-Session-Key requires API key authentication. "
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        "xiuqo" upstream as a provider model id 400s)."""
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "xiuqo_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text not in allowed:
            return "api_server"
        return "xiuqo_browser" if text == "browser" else text
    ...
    def _recover_or_record_model(self, model: str, runtime_kwargs: Dict[str, Any], gateway_session_key) -> str:
        ...
        # No model.default but a provider resolved (e.g. `xiuqo auth add` without `xiuqo model`).
        if not model and runtime_kwargs.get("provider"):
            with suppress(Exception):
                from xiuqo_cli.models import get_default_model_for_provider
    ...
    def _select_agent_runtime(
        ...
        # xiuqo_cli.model_switch.resolve_effective_model.
        from xiuqo_cli.model_switch import resolve_effective_model
        if session_override:
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "xiuqo", "version": _hermes_version()})
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "status": readiness["status"], "readiness": readiness, "platform": "xiuqo",
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — xiuqo plus configured model_routes aliases (alias + resolved
        ...
        def _model(mid: str, root: str, parent) -> Dict[str, Any]:
            return {"id": mid, "object": "model", "created": now, "owned_by": "xiuqo", "permission": [],
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        ...
        try:
            from xiuqo_cli.inventory import build_model_options_payload, load_picker_context
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<XIUQO_HOME>/plugin-data/.../artifacts``-style controlled root), so artifacts never escape the
        ...
        try:
            ...
            # Unscoped fallback (tests/manual wiring): controlled root under the Xiuqo home.
            try:
                from xiuqo_state import get_hermes_home
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Xiuqo sessions."""
        db = await self._ensure_session_db_async()
        if db is None:
        ...
        # Exact-title lookup (`xiuqo peer dm` -> canonical "Bot Chat"). include_hidden is honored
        ...
        if title_filter and not sessions:
            # A canonical Bot Chat auto-archived by the orphan reaper would make `xiuqo peer dm`
            ...
            try:
                ...
                # which would fail `xiuqo peer dm` resolution and mint transient sessions — same accident
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Xiuqo session row. Existence check, insert and
        ...
        # would persist "xiuqo" and later send it to the provider literally.
        ...
        return web.json_response({"object": "xiuqo.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "xiuqo.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "xiuqo.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "xiuqo.session", "session": self._session_response(fork)}, status=201)
    ...
    def _session_headers(session_id: str, gateway_session_key: Optional[str]) -> Dict[str, str]:
        """``X-Xiuqo-Session-Id`` (+ ``X-Xiuqo-Session-Key`` when declared) response headers."""
        headers = {"X-Xiuqo-Session-Id": session_id}
        if gateway_session_key:
            headers["X-Xiuqo-Session-Key"] = gateway_session_key
        return headers
    ...
    def _effective_turn_runtime(self, runtime_request: Dict[str, Any], result: Any, usage: Any) -> Dict[str, Any]:
        ...
        return self._sanitize_runtime_metadata(
            ...
            # gateway/status readers resolve process-level paths and do NOT follow the XIUQO_HOME
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"object": "xiuqo.session.chat.completion",
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        runtime = self._sanitize_runtime_metadata(
            ...
            model_lock="accepted")
        return web.json_response(
            {"object": "xiuqo.session.model_lock", "session_id": session_id, "runtime": runtime})
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        # Optional transient per-run context (standalone `xiuqo cron run` /
    ...
    def _bind_api_server_session(
        *, chat_id: str = "", session_key: str = "", session_id: str = "", profile: str = "",
        ...
        reach ``XIUQO_SESSION_PROFILE``: the persistent-Docker container key is derived from it, so an
