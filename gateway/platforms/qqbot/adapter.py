...
class QQAdapter(OwnAccessPolicyMixin, BasePlatformAdapter):
    ...
    async def _send_identify(self) -> None:
        ...
        payload = {"op": 2, "d": {
            ...
            "shard": [0, 1],
            "properties": {"$os": "macOS", "$browser": "xiuqo", "$device": "xiuqo"}}}
    ...
    async def _default_interaction_dispatch(self, event: InteractionEvent) -> None:
        ...
        ``~/.xiuqo/.update_response``; anything else is ignored at DEBUG."""
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``.update_response``, polled by the detached ``xiuqo update --gateway`` watcher."""
        try:
    ...
    async def send_update_prompt(
        ...
        are written to ``~/.xiuqo/.update_response`` by the interaction callback."""
