...
def _get_hermes_version() -> str:
    """Return the xiuqo package version, or 'dev' if unavailable."""
    try:
...
def build_user_agent() -> str:
    """``QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; Xiuqo/<xiuqo_version>)``."""
    v = sys.version_info
    return (f"QQBotAdapter/{QQBOT_VERSION} (Python/{v.major}.{v.minor}.{v.micro}; "
            f"{platform.system().lower()}; Xiuqo/{_get_hermes_version()})")
