...
class WhatsAppBehaviorMixin(OwnAccessPolicyMixin):
    ...
    DEFAULT_REPLY_PREFIX: str = "☤ *Xiuqo Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    /opt/xiuqo) is mirrored to XIUQO_HOME so npm install works."""
    import shutil
    from xiuqo_constants import get_hermes_home
    install_bridge = Path(__file__).resolve().parents[2] / "scripts" / "whatsapp-bridge"
    xiuqo_home_bridge = get_hermes_home() / "scripts" / "whatsapp-bridge"
    try:
        ...
        pass
    if xiuqo_home_bridge.exists():
        return xiuqo_home_bridge
    try:
        xiuqo_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(install_bridge, xiuqo_home_bridge, dirs_exist_ok=False)
        return xiuqo_home_bridge
    except Exception:
        return install_bridge
