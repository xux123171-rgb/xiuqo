...
_ROOM_RETENTION_REQUEST_KEY = (
    RequestKey("xiuqo.room_run_retention_until", float) if RequestKey is not None
    else "xiuqo.room_run_retention_until")
...
def _set_run_status(self, run_id: str, status: str, **fields: Any) -> Dict[str, Any]:
    ...
    current.update({"object": "xiuqo.run", "run_id": run_id, "status": status, "updated_at": now})
...
async def _handle_runs(self, request: "web.Request", *, _api_server) -> "web.Response":
    ...
    # Same precedence as /v1/responses: body session_id > response chain > X-Xiuqo-Session-Key
    ...
    # a declared X-Xiuqo-Session-Key, whose persisted delivery rows must reach the next
...
def _run_agent_sync(self, run: _RunLaunch, agent, approval_notify, *, _api_server):
    ...
    with self._profile_scope(run.request_profile):
        try:
            ...
            # tools.async_delegation sees no XIUQO_SESSION_CHAT_ID and forces delegations sync.
            session_tokens = self._bind_api_server_session(
                ...
                # session id or a declared X-Xiuqo-Session-Key conversation (both load
...
async def _handle_run_approval(self, request: "web.Request", *, _api_server) -> "web.Response":
    ...
    return web.json_response({
        "object": "xiuqo.run.approval_response", "run_id": run_id, "choice": choice, **request_id_field,
        "resolved": resolved})
...
async def _handle_steer_run(self, request: "web.Request", *, _api_server) -> "web.Response":
    ...
    return web.json_response({"object": "xiuqo.run.steer", "run_id": run_id, "accepted": True})
