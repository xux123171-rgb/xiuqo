...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.xiuqo to the active profile home."""
    ...
    if raw == "~/.xiuqo" or raw.startswith("~/.xiuqo/"):
        return xiuqo_home / raw[len("~/.xiuqo/"):]
    path = Path(raw).expanduser()
    return path if path.is_absolute() else xiuqo_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script; must live under XIUQO_HOME/scripts."""
    ...
    if raw_text == "~/.xiuqo" or raw_text.startswith("~/.xiuqo/"):
