...
There are two ways to add a platform to the Xiuqo gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.xiuqo/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Xiuqo code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `xiuqo gateway status` or `get_connected_platforms()` until the SDK
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `xiuqo_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Xiuqo core.
...
"xiuqo-your-platform": {
    ...
    "tools": _XIUQO_CORE_TOOLS,
    "includes": []
},
```

And add it to the `xiuqo-gateway` composite:

```python
"xiuqo-gateway": {
    "includes": [..., "xiuqo-your-platform"]
}
```
...
## 12. Status Display (`xiuqo_cli/status.py`)
...
## 13. Gateway Setup Wizard (`xiuqo_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ xiuqo_cli/ toolsets.py \
  --include="*.py" -l | sort -u
