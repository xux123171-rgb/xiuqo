...
def build_auto_tts_output_path(platform) -> str:
    ...
    ``XIUQO_SESSION_PLATFORM`` contextvar before the post-handler auto-TTS block runs.
    ...
    audio_path = os.path.join(
        tempfile.gettempdir(), "xiuqo_voice", f"tts_reply_{uuid.uuid4().hex[:12]}.{ext}")
...
def resolve_proxy_url(
    ...
    *platform_env_var* is a per-adapter, per-profile-configurable setting (each proxy URL can
    ...
    those are OS/system-level network settings, not a per-profile Xiuqo concept."""
...
def _config_section(name: str) -> dict:
    ...
    try:
        from xiuqo_cli.config import load_config_readonly as _load_config
...
from xiuqo_constants import get_default_hermes_root, get_hermes_dir, get_hermes_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.xiuqo/.env manually.")
...
async def _cache_media_from_url(url: str, ext: str, retries: int, *, media_type: str, accept: str,
    ...
    headers = {"User-Agent": "Mozilla/5.0 (compatible; XiuqoAgent/1.0)", "Accept": accept}
...
_XIUQO_HOME = get_hermes_home()
_XIUQO_ROOT = get_default_hermes_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "XIUQO_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "XIUQO_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "XIUQO_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "XIUQO_MEDIA_DELIVERY_STRICT"
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    *(_XIUQO_HOME / "cache" / d for d in _MEDIA_DELIVERY_CACHE_SUBDIRS))
...
# Credential stores at the XIUQO_HOME root, denied per-file so skills/, logs/ and agent-written
...
def _profile_cache_roots() -> List[Path]:
    ...
    roots cover only the active XIUQO_HOME). Enumerated at check time so profiles created after
    startup count and are allowlisted BEFORE the ``/root`` denylist (XIUQO_HOME symlinked).

    ``XIUQO_HOME=/opt/data``) while the model emits a profile-scoped path silently fails delivery.
    ...
    when XIUQO_HOME is symlinked under a denied prefix and $HOME is not that prefix). See issue #31733.
    """
...
def _profile_dirs() -> List[Path]:
    ...
    try:
        return [p for p in (_XIUQO_ROOT / "profiles").iterdir() if p.is_dir()]
    except OSError:
        return []


def _credential_home_roots() -> List[Path]:
    """Every Xiuqo home whose credential stores the denylist must cover: the ACTIVE home
    (the per-turn XIUQO_HOME override under ``gateway.multiplex_profiles``), the shared root
    ...
    return list(dict.fromkeys((get_hermes_home(), _XIUQO_ROOT, *_profile_dirs())))


def _kanban_root() -> Path:
    """Kanban is root-shared across profiles by design (``kanban_db.kanban_home``)."""
    return Path(os.environ.get("XIUQO_KANBAN_HOME", "").strip() or _XIUQO_ROOT).expanduser()
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    override = os.environ.get("XIUQO_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    Credential sub-dirs (``~/.ssh``, ``~/.xiuqo/.env``) stay blocked (more-specific entries)."""
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Xiuqo cache dirs (``/root/.xiuqo/...`` in
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted cache dirs (``/root/.xiuqo/...``), persistent ``/workspace`` and ``/root``."""
    if not candidate.is_absolute():
        return None
    # In-process gateways (Desktop, `xiuqo serve`) may not have bridged terminal.* config into
    ...
    # prefixes). /root/.xiuqo/* that missed a cache mount is the container's credential surface —
    # translating it via the home mount would dodge the host denylist.
    if "/root" not in mounted and not candidate.as_posix().startswith("/root/.xiuqo"):
        mounts.extend(
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    (``XIUQO_MEDIA_DELIVERY_STRICT=1``, public bots where prompt injection must not exfiltrate
    host secrets): MUST be under a Xiuqo cache, an operator root (``XIUQO_MEDIA_ALLOW_DIRS``),
    ...
    for root in _media_delivery_allowed_roots():
        ...
        if resolved_root is not None and _path_is_within(resolved, resolved_root):
            return str(resolved)
    # Non-strict (default): anything not denylisted (/etc, /proc, ~/.ssh, Xiuqo-root secrets).
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?xiuqo\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+xiuqo[.!?\s]*$", re.IGNORECASE))
...
class BasePlatformAdapter(ABC):
    ...
    # adapters (API server). Propagated to ``XIUQO_SESSION_ASYNC_DELIVERY`` so tools never promise
    # a delivery they can't keep.
    supports_async_delivery: bool = True
    # ``send()`` chunks natively via ``truncate_message()`` -> the router skips its truncation.
    splits_long_messages: bool = False
    # Prefix users can always TYPE for Xiuqo commands ("!" where the client eats a leading "/").
    ...
    # ``xiuqo gateway migrate`` can tell "URL changes" from "this profile would be skipped" as new
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("XIUQO_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower() or "interrupt")
        self._busy_text_debounce_seconds: float = _float_env("XIUQO_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35)
        self._busy_text_hard_cap_seconds: float = _float_env("XIUQO_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0)
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        """Acquire a scoped lock for this adapter; True on success. A live cross-XIUQO_HOME
        ...
        remedy = (f" Stop that gateway first (xiuqo --profile {owner_profile} gateway stop)."
    ...
    def _wire_plugin_handlers(self, native: Any = None) -> None:
        ...
        try:
            from xiuqo_cli.plugins import get_plugin_manager
    ...
    async def _handle_message_while_active(self, event: MessageEvent, session_key: str) -> None:
        ...
        from xiuqo_cli.commands import (is_interrupt_then_dispatch, should_bypass_active_session)
    ...
    def _get_human_delay() -> float:
        """Random human-like pacing delay (s) from XIUQO_HUMAN_DELAY_MODE: "off" (default) |
        "natural" 800-2500ms | "custom" via XIUQO_HUMAN_DELAY_MIN_MS /
        XIUQO_HUMAN_DELAY_MAX_MS."""
        mode = os.getenv("XIUQO_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        lo, hi = 800, 2500
        if mode != "natural":  # custom mode tolerates malformed env vars
            lo = _or_default(lambda: int(os.getenv("XIUQO_HUMAN_DELAY_MIN_MS", str(lo))), lo)
            hi = _or_default(lambda: int(os.getenv("XIUQO_HUMAN_DELAY_MAX_MS", str(hi))), hi)
    ...
    async def _synthesize_auto_tts(self, text_content: str) -> Tuple[List[str], Optional[str]]:
        ...
        (logged, never raised). Path built platform-aware HERE: XIUQO_SESSION_PLATFORM is cleared
        post-handler."""
        paths: List[str] = []
        requested_path = None
        try:
    ...
    def build_source(
        ...
        profile is stamped on ``source.profile`` for per-profile XIUQO_HOME isolation."""
