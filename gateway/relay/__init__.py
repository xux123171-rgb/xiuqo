"""Relay/connector support package for the Xiuqo gateway.
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from xiuqo_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # prefix every reply "**Xiuqo Agent:**" and shadow the connector's
        # linked-owner fallback, which actually disambiguates. Only a customized
        # name is forwarded.
        if value == "Xiuqo Agent":
            value = ""
...
def _resolve_relay_identity_token() -> str:
    ...
    Canonical resolver shared by runtime self-provision and ``xiuqo gateway enroll``.
...
def self_provision_relay() -> bool:
    ...
    ``~/.xiuqo/.env``), so a hosted container re-provisions every boot; the
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'xiuqo'}"
