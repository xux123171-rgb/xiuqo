...
(or removes) ``{XIUQO_HOME}/.drain_request.json`` and a gateway watcher reacts;
an ACTIVE marker means ``gateway_state -> "draining"``.  Two lenient staleness
signals (either suffices): epoch mismatch (XIUQO_HOME is a durable volume on
Xiuqo Cloud, so a marker survives the restart a drain-gated action ends in and
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting XIUQO_HOME."""
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    survived a container/VM restart (XIUQO_HOME is a durable Fly volume on Xiuqo Cloud) and the lifecycle
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    machine restart on the durable XIUQO_HOME volume, or an expired same-epoch orphan (#85433), is ignored
