...
schedule via the `cronjob` tool; users via `xiuqo cron list|add|edit|pause|resume|run|remove` or
...
- File lock `~/.xiuqo/cron/.tick.lock` prevents duplicate ticks across processes.
...
- The cron ticker runs in the desktop-spawned backend when `XIUQO_DESKTOP=1` — that env var means
...
Durable SQLite-backed board letting multiple profiles/workers collaborate. Users: `xiuqo kanban
...
- **CLI:** `xiuqo_cli/kanban.py` facade + 14 `kanban_*.py` siblings (`boards`, `db`, `db_connect`,
...
- **Toolset:** `tools/kanban_tools.py` — `kanban_show, kanban_complete, kanban_request_review,
  ...
  `kanban` (`xiuqo tools enable kanban --platform <p>`; default-off, in `CONFIGURABLE_TOOLSETS`) get
...
- **Dispatcher:** long-lived loop (default 60s) that reclaims stale claims, promotes ready tasks,
  ...
  (`kanban.dispatch_in_gateway: true`). Standalone: `plugins/kanban/systemd/xiuqo-kanban-dispatcher.service`.
...
Isolation: **board** is the hard boundary — workers get `XIUQO_KANBAN_BOARD` pinned in their env and
...
`tests/cron/`, `tests/xiuqo_cli/test_kanban*.py`, `tests/tools/test_kanban*.py`. Schedule parsing
