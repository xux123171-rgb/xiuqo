...
A file lock (~/.xiuqo/cron/.tick.lock) keeps overlapping processes to one tick at a time.
"""
...
# `xiuqo update`) otherwise fail with ModuleNotFoundError for xiuqo_time et al.
...
from xiuqo_time import now as _hermes_now
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `xiuqo fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `xiuqo cron pause {job_ref}` "
...
class CronTickYielded(RuntimeError):
    ...
    open). Raised, not returned, so ``record_ticker_error`` sees it and ``xiuqo cron status``
    isn't green.
    """
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    # (interrupted `xiuqo update`, manual git pull) serves MIXED modules — old entries frozen in
    ...
    if provider_reachable and re.search(
        ...
        if skew is not None:
            ...
            message += (
                ...
                "`xiuqo gateway restart` to fix it."
            )
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy: avoid heavy xiuqo_cli import at module load; shares MCP-membership with gateway/CLI
    from xiuqo_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    try:
        from xiuqo_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _resolve_job_reasoning_config(job: dict, cfg: dict, model: str) -> dict | None:
    ...
    from xiuqo_constants import parse_reasoning_effort, resolve_reasoning_config
...
def get_running_job_details() -> list[dict]:
    ...
    runs). The drain wait publishes this so ``xiuqo update`` can say WHICH job it is waiting on."""
    now = time.time()
    with _running_lock:
        return [
...
def _inflight_min_allowance_minutes() -> float:
    ...
    with contextlib.suppress(Exception):
        ...
        if _cfg_val is not None:
            ...
            if val > 0:
                return val
    raw = cron_env_setting("XIUQO_CRON_INFLIGHT_MAX_MINUTES").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid XIUQO_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
def _cron_inactivity_seconds() -> float:
    """Parse XIUQO_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600. Shared by the
    ...
    raw = cron_env_setting("XIUQO_CRON_TIMEOUT").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid XIUQO_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    A cron tick can fire while the gateway is tearing down — SIGTERM from ``xiuqo update`` / ``xiuqo
...
def _get_hermes_home() -> Path:
    """Xiuqo home at call time (honouring the test override). Cron is per-profile: never freeze
    ...
    resolving the active XIUQO_HOME at call time means a profile's jobs are stored AND executed under that
...
def _get_lock_paths() -> tuple[Path, Path]:
    ...
    xiuqo_home = _get_hermes_home()
    lock_dir = xiuqo_home / "cron"
...
def _run_no_agent_job(
    ...
    try:
        ...
        load_hermes_dotenv(xiuqo_home=_get_hermes_home())
    except Exception:
...
def _snapshot_pin(job: dict, axis: str, current: str, job_id: str) -> str:
    ...
    if snapshot and current and snapshot.lower() != current.lower():
        logger.info(
            ...
            "`xiuqo cron resnap %s` adopts the new default (stays unpinned), "
            "`xiuqo cron edit %s --%s <value>` or cron.%s in config.yaml pins it.",
...
def _load_cron_job_config(job: dict, job_id: str, job_name: str) -> _CronJobConfig:
    ...
    creation snapshot > XIUQO_MODEL > config ``model:``. Re-read every tick (no cache) so
    ``xiuqo cron edit --model`` applies next tick."""
    model = job.get("model") or cron_env_setting("XIUQO_MODEL") or ""
            ...
            if not job.get("model"):
                if _cron_default_model:
                    ...
                    _, _global_model = resolve_cron_model_drift_defaults(
                        _cfg, environ={"XIUQO_MODEL": cron_env_setting("XIUQO_MODEL")})
    ...
    if not (isinstance(model, str) and model.strip()):
        raise RuntimeError(
            ...
            f"XIUQO_MODEL={cron_env_setting('XIUQO_MODEL')!r}, "
            "config.yaml model.default missing or empty). "
            f"Set a per-job model via "
            f"`xiuqo cron edit {job_id} --model <name>` or set a "
            "default with `xiuqo model <name>`."
        )
...
def _load_prefill_messages(cfg: dict, job_id: str) -> Optional[list]:
    ...
    prefill_file = (
        cron_env_setting("XIUQO_PREFILL_MESSAGES_FILE")
...
def _resolve_job_runtime(job: dict, job_id: str, jc: _CronJobConfig) -> tuple[dict, str]:
    ...
    try:
        # Do NOT pass XIUQO_INFERENCE_PROVIDER as `requested`: it would override persisted config
...
def _open_cron_session_db(job: dict):
    """Open the SQLite session store under its own timeout (XIUQO_CRON_TIMEOUT only watches
    ...
    # handle (#96290). Bounded with its own timeout (separate from XIUQO_CRON_TIMEOUT, which only watches
...
def _run_agent_with_watchdog(
    ...
    watchdog: default 600s, override XIUQO_CRON_TIMEOUT, 0 = unlimited."""
...
def _final_response_from_result(result: dict, job_id: str, job_name: str, AIAgent) -> str:
    ...
    if final_response.strip() and turn_exit_reason:
        ...
        _explainer_variants = []
        try:
            from xiuqo_state_errors import PERSISTENCE_ERROR_CAUSES as _causes
        except Exception:
...
def _finalize_cron_session(session_db, agent, job_id: str, job_name: str, cron_session_id: str) -> None:
    ...
    try:
        ...
        with contextlib.suppress((Exception, KeyboardInterrupt)):
            ...
            # XIUQO_SESSION_KEY is not in the CLI environment, so the key resolves empty here. Since #64240
    ...
    # vocabulary in xiuqo_state's session_lifecycle_statuses docstring — keep the tuple below in sync when
...
def _prepare_job_prompt(
    ...
    # user never chose. no_agent jobs are exempt. Escape hatch: XIUQO_IGNORE_USER_CONFIG=1.
...
_CRON_DELIVERY_VARS = (
    "XIUQO_CRON_AUTO_DELIVER_PLATFORM",
    "XIUQO_CRON_AUTO_DELIVER_CHAT_ID",
    "XIUQO_CRON_AUTO_DELIVER_THREAD_ID")
...
class _CronRunScope:
    ...
    XIUQO_SESSION_* are deliberately NOT seeded from job["origin"]: it is delivery metadata, not
    a sender, and terminal/tts/skills/send_message tools would act as if the origin user were
    driving the agent. Delivery reads job["origin"] / XIUQO_CRON_AUTO_DELIVER_* directly.
    """
    ...
    def __init__(self, job: dict, job_id: str, execution_id: Optional[str]):
        ...
        self._ctx_tokens = set_session_vars(
            ...
            # We clear the XIUQO_SESSION_* routing keys just below, so an async delegation's completion
            ...
            # XIUQO_SESSION_KEY, which risks routing a cron subagent's output into an unrelated user chat.)
        ...
        self._cron_session_var = _VAR_MAP["XIUQO_CRON_SESSION"]
    ...
    def enter(self) -> None:
        ...
        # XIUQO_KANBAN_TASK in env, and an unrelated job could close the worker's task. Must be a
...
def _reload_dotenv_and_publish_delivery_target(job: dict) -> None:
    ...
    from xiuqo_cli.env_loader import load_hermes_dotenv, reset_secret_source_cache
    ...
    if delivery_target:
        _VAR_MAP["XIUQO_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
        _VAR_MAP["XIUQO_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
        _VAR_MAP["XIUQO_CRON_AUTO_DELIVER_THREAD_ID"].set(
...
def run_one_job(
    ...
    external_owner = os.environ.get("_XIUQO_CRON_EXTERNAL_WORKER") == execution_id
...
def _launch_external_cron_worker(job: dict) -> bool:
    ...
    try:
        worker_env = strip_launch_profile_env(build_subprocess_env(
            ...
            extra={"XIUQO_HOME": str(profile_home)},
        ))
    finally:
    ...
    try:
        with use_cron_store(profile_home):
            ...
            try:
                ...
                return False
            old_external_execution = os.environ.get("_XIUQO_CRON_EXTERNAL_WORKER")
            os.environ["_XIUQO_CRON_EXTERNAL_WORKER"] = execution_id
            try:
                return run_one_job(job, adapters=None, loop=None, verbose=False)
            finally:
                if old_external_execution is None:
                    os.environ.pop("_XIUQO_CRON_EXTERNAL_WORKER", None)
                else:
                    os.environ["_XIUQO_CRON_EXTERNAL_WORKER"] = old_external_execution
    finally:
        reset_secret_scope(secret_token)
...
def _worktree_maintenance_repos() -> List[str]:
    """Repos whose ``.worktrees/`` to keep pruned: the xiuqo checkout plus job workdir repo roots,
    filtered to those that actually have a ``.worktrees/`` dir."""
    repos: set = set()

    # Xiuqo source checkout (git installs only; wheel installs have no .git).
    with contextlib.suppress(Exception):
...
def _maybe_run_worktree_maintenance() -> None:
    ...
    git. Same conservative pruner as ``xiuqo -w`` startup (dirty/unpushed/locked trees untouched).
...
def _maybe_reap_dead_owners() -> None:
    ...
    # recovery previously ran only at scheduler STARTUP. A one-shot `xiuqo cron run` that claimed a job and
...
def _resolve_max_parallel_workers() -> Optional[int]:
    """Max workers: env > config.yaml > unbounded (XIUQO_CRON_MAX_PARALLEL=1 restores serial)."""
    try:
        _env_par = cron_env_setting("XIUQO_CRON_MAX_PARALLEL").strip()
        if _env_par:
            return int(_env_par) or None
    except (ValueError, TypeError):
        logger.warning("Invalid XIUQO_CRON_MAX_PARALLEL value; defaulting to unbounded")
    with contextlib.suppress(Exception):
        _ucfg = load_config() or {}
...
def tick(
    ...
    try:
        # `xiuqo pause` ESTOP: skip dispatch, never touch in-flight runs; check_paused logs once.
...
if __name__ == "__main__":
    if "--external-worker-file" in sys.argv:
        ...
        try:
            from xiuqo_logging import setup_logging

            setup_logging(xiuqo_home=_get_hermes_home(), mode="cron")
        except Exception:
            pass
        raise SystemExit(
