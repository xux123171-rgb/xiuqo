...
(``xiuqo cron notepad <job_id> set ...``) via the terminal tool; no model tool is added.
"""
...
def _connect() -> sqlite3.Connection:
    ...
    # ``xiuqo_cli.sqlite_util`` / ``cron.jobs`` cached, so new names must be resolved at call time,
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`xiuqo cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `xiuqo cron notepad {job_id} delete "
