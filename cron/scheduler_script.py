...
def _get_script_timeout() -> int:
    ...
    resolved = _timeout_from_env_or_config(
        "XIUQO_CRON_SCRIPT_TIMEOUT", "script_timeout_seconds", _positive_int,
        "cron script timeout",
    )
...
def _get_media_send_timeout() -> int:
    """Per-attachment media-send timeout: XIUQO_CRON_MEDIA_SEND_TIMEOUT env, then
    ``cron.media_send_timeout_seconds``, then 300s (long TTS audio can exceed a 30s window)."""
    resolved = _timeout_from_env_or_config(
        "XIUQO_CRON_MEDIA_SEND_TIMEOUT", "media_send_timeout_seconds", _positive_int,
...
def _get_session_db_timeout() -> float:
    """Bound on run_job's SessionDB init: XIUQO_CRON_SESSION_DB_TIMEOUT env, then
    ...
    resolved = _timeout_from_env_or_config(
        "XIUQO_CRON_SESSION_DB_TIMEOUT", "session_db_timeout_seconds", float,
...
def _resolve_script_path(script_path: str) -> tuple[Optional[Path], Optional[str]]:
    ...
    inside XIUQO_HOME/scripts/ (relative, absolute and ``~`` paths are all validated — path
    ...
    # Traversal / absolute-path / symlink escape guard — MUST stay inside XIUQO_HOME/scripts/.
    try:
...
def _run_job_script(
    ...
    Args: script_path: Path to the script. Relative paths are resolved against XIUQO_HOME/scripts/.
