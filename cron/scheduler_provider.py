...
def _existing_profile_homes(profile_homes: list) -> list:
    ...
    back on disk and in ``xiuqo profile list`` (#47368). Filtering on directory existence leaves a deleted
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        # the process-global XIUQO_HOME (the default profile) is ticked. Heartbeats and recovery are also
        # scoped per profile so `xiuqo cron status` reflects liveness for every profile independently.
        ...
        # Heartbeat before the first sleep so `xiuqo cron status` sees a live ticker immediately.
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # Persist the reason so `xiuqo cron status` (separate process) shows WHY.
