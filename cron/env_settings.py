"""Profile-scoped reads of the ``XIUQO_*`` tuning settings cron honours from ``.env``.

A standalone ``xiuqo -p X gateway run`` loads X's ``.env`` into ``os.environ``, so a bare
``os.getenv("XIUQO_CRON_TIMEOUT")`` is X's value. Under ``gateway.multiplex_profiles`` the same
