...
def _open_continuable_cron_thread(job: dict, adapter, chat_id: str, loop) -> Optional[str]:
    ...
    if not callable(create_thread) or loop is None:
        return None
    thread_name = f"Xiuqo — {job.get('name') or job.get('id', 'cron')}"
    try:
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from xiuqo_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    if xiuqo_bin:
        ...
        try:
            ...
            found = _ilu.find_spec("xiuqo_cli") is not None
        except Exception:
            found = False
        if not found:
            return "bot-chat delivery failed: xiuqo CLI not resolvable"
        argv = [sys.executable, "-m", "xiuqo_cli.main"]
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution; this scheduler's XIUQO_HOME must not shadow it.
        env.pop("XIUQO_HOME", None)
    else:
        # Multiplex workers carry the profile in a ContextVar, not os.environ.
        env["XIUQO_HOME"] = str(source_home)

    query_file = None
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="xiuqo-cron-botchat-", delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _deliver_result(
    ...
    # job's own attempt: a worker's script may dispatch another job in-process (`xiuqo cron run`),
    # and that nested delivery must not be keyed under the outer execution id.
    external_execution = os.environ.get("_XIUQO_CRON_EXTERNAL_WORKER", "")
    ...
    # persisted as ``last_delivery_unverified`` so `xiuqo cron list` shows it.
    ...
    # at boot; standalone runs (`xiuqo cron run`) did not, silently dropping files. Idempotent.
