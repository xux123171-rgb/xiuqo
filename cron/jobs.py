"""Cron job storage: ~/.xiuqo/cron/jobs.json; output in
~/.xiuqo/cron/output/{job_id}/{timestamp}.md"""
...
# and run them under the ticker's XIUQO_HOME, leaking config/credentials/skills across profiles.
# Each profile owns its own cron store under its own XIUQO_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same XIUQO_HOME — so a job authored in profile `coder` lives in
# `~/.xiuqo/profiles/coder/cron/jobs.json` and executes with `coder`'s `.env`, `config.yaml`, and skills.
...
XIUQO_DIR = get_hermes_home().resolve()
...
CRON_DIR = XIUQO_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat: touched every ticker loop so `xiuqo cron status` can tell the ticker THREAD is alive,
# not just the gateway PROCESS; success = last tick that completed WITHOUT raising.
# The gateway process and the (separate) ``xiuqo cron status`` process share it so status can tell whether
...
# threshold in `xiuqo cron status` (xiuqo_cli/cron.py), so they never drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    get_hermes_home(), so re-pointing XIUQO_HOME after import uses ITS OWN store rather than the
...
# Fallback stale-recovery window for a one-shot's running-claim when XIUQO_CRON_TIMEOUT=0
...
def _oneshot_run_claim_ttl_seconds() -> float:
    """One-shot running-claim TTL from ``XIUQO_CRON_TIMEOUT``: unset/invalid → 600s → 1800s;
...
def _secure_dir(path: Path):
    ...
    container/XIUQO_HOME_MODE rules as the rest of XIUQO_HOME (#10757)."""
    from xiuqo_cli.config import _secure_dir as _shared_secure_dir
    _shared_secure_dir(path)


def _secure_file(path: Path):
    """Owner-only (0600) via the shared helper (managed/container skip included)."""
    from xiuqo_cli.config import _secure_file as _shared_secure_file
...
def _is_named_profile_path(path: Path) -> bool:
    """True if *path* is under ``<xiuqo_home>/profiles/<name>/`` (default/custom homes are not).
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            # Naive timestamps become aware in the CONFIGURED Xiuqo timezone (not server-local):
            # the due-check compares against xiuqo_time.now().
...
def _ensure_aware(dt: datetime) -> datetime:
    """Aware datetime in the configured Xiuqo timezone. Legacy naive values are read as
    ...
    if kind == "cron":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall xiuqo or run 'pip install croniter' in your runtime env.",
                expr)
            return None
...
# --- Ticker heartbeat (liveness signal for `xiuqo cron status`) ---
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    tick* marker. We track two distinct signals so `xiuqo cron status` can tell a thread that is merely
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    critical under multiplex_profiles where ``xiuqo cron status`` must report per-profile liveness
    (#69377).
    """
    ...
    *, provider: Any, model: Any, base_url: Any, no_agent: Any,
    ...
    if normalized_provider is None:
        with contextlib.suppress(Exception):
            ...
            # Delegate all rate-limit / 5xx retry to xiuqo's outer conversation loop, which honors
...
def trigger_job(job_id: str, extra_prompt: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    if is_terminal_job(job):
        ...
        raise ValueError(
            ...
            f"Create a new occurrence with 'xiuqo cron resume {name} "
...
def _claim_owner_is_dead(claim: Dict[str, Any]) -> bool:
    ...
    ``_machine_id()`` stamps ``host:pid[:token]``; a foreign host, an explicit XIUQO_MACHINE_ID,
...
def _claim_is_live(claim: Any, now: datetime, ttl_seconds: float) -> bool:
    ...
    after the TTL (a killed ``xiuqo cron run`` otherwise blocks the next manual run for the
...
def _machine_id() -> str:
    ...
    fresh-claim check): ``XIUQO_MACHINE_ID`` if set, else hostname:pid."""
    explicit = os.getenv("XIUQO_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _oneshot_dispatch_limit_reached(job: Dict[str, Any], scan: _DueScan) -> bool:
    ...
    if job.get("last_run_at") is not None:
        ...
        logger.warning(
            ...
            "'xiuqo cron resume <job> --run-now' (#93524).",
...
def _evaluate_due_job(job: Dict[str, Any], scan: _DueScan, run_claim_ttl: float) -> bool:
    ...
    if kind == "once":
        ...
        # same XIUQO_HOME must not re-dispatch it while in flight, and advancing next_run_at by a
...
# the disk.
# Unlike the quick-snapshot store (`xiuqo_cli.backup`, capped at 20) it had no retention, so a
