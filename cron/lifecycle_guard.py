...
A cron job that restarts/stops the gateway from inside the gateway (``xiuqo gateway restart``,
``launchctl kickstart ai.xiuqo.gateway``, ``systemctl restart xiuqo-gateway``) kills the process,
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `xiuqo gateway` ops. `start` is excluded: starting from inside a
    # gateway is benign and a job may legitimately start a sibling profile. The lookbehind keeps
    # `xiuqo` from being a path component or word tail (`/docs/xiuqo gateway restart-notes.md`)
    # while every real command position (text start, whitespace, `;`/`&`/`|`, `$(`, backtick,
    # U+FFFD) still matches.
    # See #77173.
    r"(?:(?<![/\w.\-])xiuqo\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops anchored on a xiuqo-gateway label so unrelated xiuqo services stay
    ...
    # -l ai.xiuqo.gateway-<suffix> -- <helper-script>` (or `launchctl bootstrap gui/<uid> <plist>`) creates
    ...
    # Branch C: systemctl ops on a xiuqo-gateway unit.
...
# split across continuation lines (e.g. `launchctl submit \` / `  -l ai.xiuqo.gateway-... \` / `  -- ...`,
...
# Branch A2: `xiuqo -p <profile> gateway restart|stop` (also `--profile <name>` /
...
# `xiuqo -p venus gateway restart` operates on a sibling profile's gateway and is a legitimate fleet
...
# `$label` next to the verb. These verbs act on an EXISTING job, so the xiuqo-gateway label anchor
...
_XIUQO_GATEWAY_LABEL_RE = re.compile(r"(?i)\bhermes[.\-]?gateway\b")
...
def _current_profile_name() -> Optional[str]:
    """Profile running the guard: ``XIUQO_PROFILE_NAME``/``XIUQO_PROFILE`` env first, then
    ``xiuqo_cli.profiles.get_active_profile_name`` (from ``XIUQO_HOME``); ``None`` if neither."""
    for env_name in ("XIUQO_PROFILE_NAME", "XIUQO_PROFILE"):
...
def _contains_launchctl_gateway_lifecycle(normalized_text: str) -> bool:
    ...
    return bool(_LAUNCHCTL_LIFECYCLE_VERBS_RE.search(normalized_text)) and bool(
        _XIUQO_GATEWAY_LABEL_RE.search(normalized_text)
    )
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    ``xiuqo``/``gateway`` token) — this function is the single choke point
    ...
    # <<'EOF'`) are masked before scanning (#88336): a runbook line "a human can run: xiuqo gateway
    ...
    # Profile-flag form (#78028): `xiuqo -p <profile> gateway restart|stop` bypasses Branch A because the
    # selector sits between `xiuqo` and `gateway`. It is only the same foot-gun when the named profile IS
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    ``<XIUQO_HOME>/scripts/``) so the guard scans the file that will actually run."""
    ...
    try:
        ...
        # get_hermes_home() falls back to Path.home(), which raises when neither XIUQO_HOME nor
...
def check_gateway_lifecycle(prompt: Optional[str], script: Optional[str] = None) -> None:
    ...
    if script:
        ...
        if resolved_script is not None and _on_cloud_path(resolved_script):
            raise GatewayLifecycleBlocked(
                ...
                "(e.g. ~/.xiuqo/scripts/) and recreate the job."
            )
    ...
    if python_script:
        ...
        # The direct command regex below still scans the full text, so a literal `xiuqo gateway restart`
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `xiuqo gateway restart` from a shell outside "
