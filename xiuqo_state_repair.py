"""state.db repair, backup and writability preflight (split from xiuqo_state).

Patchable helpers are looked up as module globals at call time, so tests patch ``xiuqo_state_repair.<name>``.
"""
...
from xiuqo_startup_watchdog import report_startup_progress
...
# Log-record parity with the origin module (caplog tests pin "xiuqo_state").
logger = logging.getLogger("xiuqo_state")
...
_MANUAL_RECOVER_HINT = ("Free disk space, then retry (or recover manually with "
                        "`xiuqo sessions recover --source {db_path} --inspect-only` first).")
...
def _read_offline(db_path: Path, what: str, reader) -> Optional[str]:
    """Run *reader()* under ``xiuqo_cli.sqlite_safe_read.offline_file_access``.
    ...
    without xiuqo_cli have no tracked connections."""
    try:
...
def _claim_repair_attempt(db_path: Path) -> bool:
    ...
    from xiuqo_state import _repair_attempt_lock, _repair_attempted_paths
...
def _try_lock_nonblocking(handle) -> None:
    ...
    from xiuqo_state import _IS_WINDOWS
    if _IS_WINDOWS:
...
def _cross_process_repair_lock(db_path: Path):
    ...
    #36644).
    """
    from xiuqo_state import _IS_WINDOWS, _REPAIR_LOCK_TIMEOUT_SECONDS
...
def _repair_snapshot_timeout_seconds(source_path: Path) -> float:
    ...
    from xiuqo_state import _REPAIR_LOCK_TIMEOUT_SECONDS
    source_bytes = 0
...
def _persistent_repair_exhausted_error(db_path: Path) -> str:
    """The stable operator-facing diagnostic for an exhausted repair budget. The ``xiuqo`` commands
    carry the profile selector: a bare ``xiuqo`` follows ``active_profile`` (#105887)."""
    ...
    return (f"automatic repair has already failed {_MAX_PERSISTENT_REPAIR_ATTEMPTS} times on this exact file — the "
            ...
            f"required: restore a backup, or salvage with `xiuqo {profile_arg}sessions recover --source {db_path} "
            f"--inspect-only`, then (if it reports recoverable) `xiuqo {profile_arg}sessions recover --source {db_path} "
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    ``close()`` a descriptor and cancel that connection's POSIX advisory locks (``xiuqo_cli.sqlite_safe_read``) —
    ...
    `xiuqo sessions recover --source <db> --inspect-only`, never at a raw sqlite3 shell on the live file.

    See #69603.
    """
    with contextlib.suppress(ImportError):  # scaffold/embed installs without xiuqo_cli track no connections
...
def preflight_db_writability(db_path: Path, *, db_label: str = "state.db") -> None:
    ...
    "fix" (deleting the ``-wal``) loses committed transactions. ``chmod u+rw`` repair only inside the Xiuqo home
    tree (Xiuqo owns those files; ``chmod`` fails on files the user doesn't own, bounding the repair exactly);
    ...
    for p, is_dir in [(db_path.parent, True), *((p, False) for p in (db_path, *sidecars) if p.is_file())]:
        ...
        raise sqlite3.OperationalError(
            f"{db_label} is not writable: {'directory' if is_dir else 'file'} {p} is read-only for this user. Xiuqo "
...
def _reapply_durability_barriers(conn: sqlite3.Connection) -> bool:
    ...
    from xiuqo_state_wal import _apply_macos_checkpoint_barrier, _enforce_macos_synchronous_full
    try:
...
def apply_durability_barriers(conn: sqlite3.Connection) -> bool:
    ...
    from xiuqo_state_wal import _apply_synchronous_pragma
    ok = _reapply_durability_barriers(conn)
    with contextlib.suppress(Exception):
        from xiuqo_cli.config import cfg_get, load_config_readonly  # local: avoids an import cycle
...
# Xiuqo-owned FTS5 objects: the virtual tables and their shadow b-trees. Full-matched, so a
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    See #50502.
    """
    from xiuqo_state import SessionDB, load_fts5_cjk_extension
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    (``xiuqo_state_holders``) fails closed on deleted-WAL-generation, uninspectable, or unknown holders."""
    ...
    with _cross_process_repair_lock(db_path) as holding_lock:
        ...
        # exclusive guard in the locked routine excludes writers through promotion and sees DELETE-mode readers too.
            ...
            _repair_skip(report, "skipped", "a live writer still holds state.db; skipped schema surgery to avoid tearing "
                         "b-tree pages under a concurrent writer. Stop the gateway (xiuqo gateway stop) and retry.")
        else:
...
def _probe_journal_mode_for_repair(db_path: Path) -> Optional[str]:
    ...
    from xiuqo_state_wal import _on_disk_journal_mode
    try:
...
def _strategy_rebuild_fts(conn: sqlite3.Connection) -> None:
    ...
    from xiuqo_state import load_fts5_cjk_extension
