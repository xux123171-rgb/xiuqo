...
timeout. Config is env-only (``XIUQO_STARTUP_WATCHDOG=0``,
``XIUQO_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "XIUQO_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "XIUQO_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (xiuqo_cli.main / xiuqo_cli.gateway /
...
def _process_hermes_home() -> Path:
    """XIUQO_HOME for diagnostic files — stdlib-only replica of the xiuqo_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<XIUQO_HOME>/logs/gateway-startup-watchdog.log``."""
...
def startup_watchdog_disabled() -> bool:
    """True when ``XIUQO_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``XIUQO_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
