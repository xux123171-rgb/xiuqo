...
Xiuqo Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``XIUQO_NIX_BUILD=1`` on the
Xiuqo package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("XIUQO_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for xiuqo is not supported.\n"
    "Xiuqo is distributed via the shell installer, Docker image, or Nix.\n"
    ...
    "the Xiuqo Nix derivation sets XIUQO_NIX_BUILD=1. If it does, file a bug."
)
...
# Root single-file modules (``run_agent``, ``xiuqo_state``, ``toolsets``...)
