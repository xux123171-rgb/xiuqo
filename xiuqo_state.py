#!/usr/bin/env python3
"""SQLite state store for Xiuqo Agent: session metadata, message history, model
...
from xiuqo_constants import get_hermes_home, mkdir_under_hermes_home
from typing import Any, Callable, Dict, Iterator, List, Optional, Tuple, TypeVar, cast

from xiuqo_state_common import escape_like as _escape_like, stat_db_file_identity as _stat_db_file_identity
from xiuqo_state_errors import (
...
from xiuqo_state_readpool import _READ_POOL_MAX, _proc_fd_targets, _read_budget_for
from xiuqo_state_sessions import SessionSessionsMixin
from xiuqo_state_fts import SessionFtsSetupMixin, load_fts5_cjk_extension
from xiuqo_state_portability import SessionPortabilityMixin
from xiuqo_state_telegram import SessionTelegramTopicsMixin
from xiuqo_state_schema import SessionSchemaMixin
import xiuqo_state_holders as _state_holders
from xiuqo_state_dbfile import (
...
from xiuqo_state_messages import SessionMessagesMixin
from xiuqo_state_rewind import SessionRewindMixin
from xiuqo_state_wal import (
    _WAL_INCOMPAT_MARKERS, _on_disk_journal_mode, apply_database_pragmas, apply_wal_with_fallback,
)
from xiuqo_state_repair import _claim_repair_attempt, preflight_db_writability, repair_state_db_schema
from xiuqo_state_titles import SessionTitlesMixin
from xiuqo_state_usage import SessionUsageMixin
from xiuqo_state_maintenance import SessionMaintenanceMixin
from xiuqo_state_gateway import SessionGatewayMixin
from xiuqo_state_compression import SessionCompressionMixin
from xiuqo_state_search import SessionSearchMixin
...
def _default_db_path() -> Path:
    ...
    ``get_hermes_home()`` is resolved fresh (a runtime XIUQO_HOME redirect works regardless of import)."""
    return DEFAULT_DB_PATH if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH else get_hermes_home() / "state.db"


# Live-DB guard knobs live HERE (not in xiuqo_state_guard): the hermetic conftest monkeypatches
# ``xiuqo_state._STATE_DB_GUARD_BYPASS`` (``@pytest.mark.live_system_guard_bypass`` escape hatch)
# and ``_EXTRA_DENY_ROOTS`` (the pre-sandbox root, so custom-XIUQO_HOME deployments are covered).
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    ``XIUQO_HOME`` together, which is precisely the state in which it writes to production (#82770).
    """
    ...
    for root in roots:
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Xiuqo root {root}). "
                "Tests must run against a temporary XIUQO_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "XIUQO_HOME. If this test genuinely needs the live database, mark it with "
...
def _secure_state_db_files(db_path: Path, *, create_main: bool = False) -> None:
    ...
    (gateway, desktop ``xiuqo serve``) keep using the deleted inodes.
    """
    if os.name == "nt":
        return
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    """Append pending messages to XIUQO_HOME/sessions/<id>.jsonl (state.db was replaced under a
...
# connection per resolved path via xiuqo_state_registry.acquire(); one-shots use SessionDB() + close().
...
# ── Process-wide shared SessionDB registry (#90837) ── lives in xiuqo_state_registry.py (acquire /
# release / close_all / release_or_close). Long-lived in-process callers (gateway, tui_gateway, cron,
# in-process tools) share ONE writer connection per resolved path via xiuqo_state_registry.acquire(); CLI
...
class SessionDB(
    ...
    # SQLite's deterministic busy handler convoys under many xiuqo processes: keep its
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        # Opened via xiuqo_state_registry.acquire(): close() releases a refcount instead.
        # Set True when this instance is opened via xiuqo_state_registry.acquire(). Makes close() a no-op so the
    ...
    def _open_writer(self) -> None:
        ...
        try:
            ...
            if not repair_state_db_schema(self.db_path).get("repaired"):
                raise
            self._connect_and_init_with_lock_patience()
        # FTS optimization is OPT-IN (`xiuqo db optimize`); no background worker races session lifecycle.
    ...
    def _handle_quarantine_if_invalid(self, already_locked: bool = False) -> None:
        ...
        msg = (
            ...
            f"Restore from {self.db_path.parent / 'state-snapshots'} via `xiuqo snapshot list` / "
            f"`xiuqo snapshot restore <id>` if available, or salvage the preserved bytes with "
            f"`xiuqo sessions recover --source {qpath or self.db_path}`. "
            ...
            try:
                ...
                if isinstance(exc, sqlite3.OperationalError):
                    if "locked" in err_msg or "busy" in err_msg:
                        ...
                        raise sqlite3.OperationalError(
                            f"database is locked (another Xiuqo process held the "
    ...
    def _halt_db_corrupt(self, exc: BaseException) -> None:
        ...
        logger.error(
            ...
            "gateway and run `xiuqo sessions recover --source %s --inspect-only`.", self.db_path, exc,
            self.db_path,
        )
    ...
    def _settle_lost_generation_locked(self) -> bool:
        ...
        through xiuqo_state_registry.release_or_close, which swallows the error, so an interpreter
    ...
    def _foreign_state_db_holders(self) -> List[Tuple[int, str]]:
        """Foreign processes holding this DB or its WAL sidecars (see xiuqo_state_holders)."""
    ...
    def close(self):
        ...
        When this instance is shared (opened via ``xiuqo_state_registry.acquire``), ``close()`` RELEASES one
        ...
        if self._shared_registry_owned:
            from xiuqo_state_registry import release
            release(self)
            return
            ...
            if self._conn:
                ...
                if quarantine_reason is not None:
                    logger.warning(
                        ...
                        "before restarting, then run `xiuqo sessions recover --source %s --inspect-only`.",
...
_PLUGIN_COMPAT_LAZY = {
    'AUTO_VACUUM_MIN_FREELIST_RATIO': ('xiuqo_state_common', 'AUTO_VACUUM_MIN_FREELIST_RATIO'),
    'ActivityProvenance': ('agent.session_activity', 'ActivityProvenance'),
    'CompressionSessionBusyError': ('xiuqo_state_errors', 'CompressionSessionBusyError'),
    'CompressionSessionClosedError': ('xiuqo_state_errors', 'CompressionSessionClosedError'),
    'DEFERRED_INDEX_SQL': ('xiuqo_state_common', 'DEFERRED_INDEX_SQL'),
    'FTS_CJK_STALE_KEY': ('xiuqo_state_common', 'FTS_CJK_STALE_KEY'),
    'FTS_CJK_TABLE_SQL': ('xiuqo_state_fts', 'FTS_CJK_TABLE_SQL'),
    'FTS_CJK_TRIGGER_SQL': ('xiuqo_state_fts', 'FTS_CJK_TRIGGER_SQL'),
    'FTS_REBUILD_DEFERRAL_KEY': ('xiuqo_state_common', 'FTS_REBUILD_DEFERRAL_KEY'),
    'FTS_SQL': ('xiuqo_state_common', 'FTS_SQL'),
    'FTS_STALE_KEY': ('xiuqo_state_common', 'FTS_STALE_KEY'),
    'FTS_STORAGE_VERSION': ('xiuqo_state_common', 'FTS_STORAGE_VERSION'),
    'FTS_TRIGRAM_SQL': ('xiuqo_state_common', 'FTS_TRIGRAM_SQL'),
    'LEGACY_FTS_SQL': ('xiuqo_state_common', 'LEGACY_FTS_SQL'),
    'LEGACY_FTS_TRIGRAM_SQL': ('xiuqo_state_common', 'LEGACY_FTS_TRIGRAM_SQL'),
    'MAX_FTS5_QUERY_CHARS': ('xiuqo_state_common', 'MAX_FTS5_QUERY_CHARS'),
    'PERSISTENCE_ERROR_CAUSES': ('xiuqo_state_errors', 'PERSISTENCE_ERROR_CAUSES'),
    'SCHEMA_SQL': ('xiuqo_state_common', 'SCHEMA_SQL'),
    'SCHEMA_VERSION': ('xiuqo_state_common', 'SCHEMA_VERSION'),
    'SESSION_STATUS_COMPLETE': ('xiuqo_state_sessions', 'SESSION_STATUS_COMPLETE'),
    'SESSION_STATUS_EMPTY': ('xiuqo_state_sessions', 'SESSION_STATUS_EMPTY'),
    'SESSION_STATUS_ERROR': ('xiuqo_state_sessions', 'SESSION_STATUS_ERROR'),
    'SESSION_STATUS_INTERRUPTED': ('xiuqo_state_sessions', 'SESSION_STATUS_INTERRUPTED'),
    ...
    'SessionTurnLeaseLostError': ('xiuqo_state_errors', 'SessionTurnLeaseLostError'),
    'WalUnsupportedError': ('xiuqo_state_wal', 'WalUnsupportedError'),
    'apply_durability_barriers': ('xiuqo_state_repair', 'apply_durability_barriers'),
    'classify_session_status': ('xiuqo_state_sessions', 'classify_session_status'),
    'collect_state_db_stats': ('xiuqo_state_dbfile', 'collect_state_db_stats'),
    'count_db_holders': ('xiuqo_state_dbfile', 'count_db_holders'),
    'describe_skill_invocation': ('agent.skill_commands', 'describe_skill_invocation'),
    'fts5_cjk_so_path': ('xiuqo_state_fts', 'fts5_cjk_so_path'),
    'is_advisory_lock_contention': ('xiuqo_state_common', 'is_advisory_lock_contention'),
    'is_automatic_end_reason': ('xiuqo_state_common', 'is_automatic_end_reason'),
    'is_disk_full_error': ('xiuqo_state_errors', 'is_disk_full_error'),
    'is_sqlite_wal_reset_vulnerable': ('xiuqo_state_wal', 'is_sqlite_wal_reset_vulnerable'),
    'is_transient_sqlite_error': ('xiuqo_state_errors', 'is_transient_sqlite_error'),
    'iter_deleted_sqlite_sidecar_holders': ('xiuqo_state_dbfile', 'iter_deleted_sqlite_sidecar_holders'),
    'release_or_close': ('xiuqo_state_registry', 'release_or_close'),
    'report_startup_progress': ('xiuqo_startup_watchdog', 'report_startup_progress'),
    'resolve_journal_mode': ('xiuqo_state_wal', 'resolve_journal_mode'),
    'resolve_synchronous_level': ('xiuqo_state_wal', 'resolve_synchronous_level'),
    'sanitize_context': ('agent.memory_manager', 'sanitize_context'),
    'sqlite_source_id': ('xiuqo_state_wal', 'sqlite_source_id'),
    'workspace_key': ('xiuqo_state_sessions', 'workspace_key'),
}
