...
Thank you for contributing to Xiuqo Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Xiuqo to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo xux123171-rgb/xiuqo "<your terms>"
  gh search prs --repo xux123171-rgb/xiuqo --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/xux123171-rgb/xiuqo/issues?q=) · [PRs (all states)](https://github.com/xux123171-rgb/xiuqo/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Xiuqo itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Xiuqo install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `xiuqo skills browse` (labeled "official") and install it with `xiuqo skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `xiuqo skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.xiuqo/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(xiuqo_home, config)` for setup-wizard integration
...
- Integrate with `xiuqo memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Xiuqo ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.xiuqo/plugins/`, project `.xiuqo/plugins/`, or a pip entry point) — see [Build a Xiuqo Plugin](https://hermes-agent.nousresearch.com/docs/guides/build-a-xiuqo-plugin)
...
The installer creates the Xiuqo venv, wires the `xiuqo` command, stamps the
install method for `xiuqo update`, and clones the full git project into
`$XIUQO_HOME/xiuqo` (usually `~/.xiuqo/xiuqo`). That keeps your
development environment on the same layout the CLI, updater, lazy dependency
installer, gateway, and docs assume.

```bash
curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash
cd "${XIUQO_HOME:-$HOME/.xiuqo}/xiuqo"
...
Use this only if you intentionally do not want Xiuqo' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `xiuqo` entrypoint from this venv; running the
system `python3 -m xiuqo_cli.main` can pick up unrelated system Python
packages.
...
```bash
git clone https://github.com/xux123171-rgb/xiuqo.git
cd xiuqo

# Create venv with Python 3.11, OUTSIDE the source tree
uv venv ~/.xiuqo/venvs/xiuqo-dev --python 3.11
export VIRTUAL_ENV="$HOME/.xiuqo/venvs/xiuqo-dev"
...
```bash
mkdir -p ~/.xiuqo/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.xiuqo/config.yaml
touch ~/.xiuqo/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.xiuqo/.env
```

### Run

```bash
# The standard installer already put `xiuqo` on PATH.
xiuqo doctor
xiuqo chat -q "Hello"
```

If you used the manual clone fallback, run `./xiuqo` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/xiuqo" ~/.local/bin/xiuqo
```
...
├── cli.py                    # HermesCLI class — interactive CLI orchestrator (~4.6k LOC + xiuqo_cli/cli_*_mixin.py)
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (xiuqo-cli, xiuqo-telegram, etc.)
├── xiuqo_state.py           # SessionDB facade (~1.4k LOC); implementation in xiuqo_state_*.py (21 siblings) — FTS5 search, session titles
...
│
├── xiuqo_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.xiuqo/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (hermes-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.xiuqo/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.xiuqo/`)

| Path | Purpose |
|------|---------|
| `~/.xiuqo/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.xiuqo/.env` | API keys and secrets |
| `~/.xiuqo/auth.json` | OAuth credentials (Nous Portal) |
| `~/.xiuqo/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.xiuqo/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.xiuqo/state.db` | SQLite session database |
| `~/.xiuqo/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and explicit `/save` exports. Automatic per-session JSON snapshots are no longer written; state.db is canonical. |
| `~/.xiuqo/cron/` | Scheduled job data |
| `~/.xiuqo/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`xiuqo_state.py`) with full-text search and unique session titles. Automatic per-session JSON snapshots have been removed. Existing files are left untouched; use `/save json` or `xiuqo sessions export` for an explicit export.
...
(for example `_XIUQO_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.xiuqo`:
...
The user may skip setup and keep loading the skill. Xiuqo only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `xiuqo setup` or update `~/.xiuqo/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Xiuqo tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Xiuqo Agent" is the secondary collaborator. If the contributor's commit shows "Xiuqo Agent" as author because they used Xiuqo to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Xiuqo tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `xiuqo --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Xiuqo uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.xiuqo/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `xiuqo_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `xiuqo_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Xiuqo runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the xiuqo wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `xiuqo_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `xiuqo_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `xiuqo_cli/gateway_windows.py::
...
Xiuqo has terminal access. Security matters.
...
2. **Test manually**: Run `xiuqo` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/xux123171-rgb/xiuqo/issues)
- Include: OS, Python version, Xiuqo version (`xiuqo --version`), full error traceback
