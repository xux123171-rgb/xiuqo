...
_XIUQO_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `xiuqo-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _XIUQO_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_XIUQO_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _XIUQO_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `xiuqo tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``xiuqo tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _XIUQO_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (XIUQO_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _XIUQO_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in xiuqo_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full Xiuqo toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. xiuqo-acp is the
    ...
    "xiuqo-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors xiuqo-cli; `xiuqo tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "xiuqo-cron": _bundle("Default cron toolset - same core tools as xiuqo-cli; gated by `xiuqo tools`"),
    "xiuqo-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "xiuqo-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "xiuqo-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "xiuqo-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "xiuqo-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "xiuqo-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "xiuqo-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "xiuqo-email": _bundle("Email bot toolset - interact with Xiuqo via email (IMAP/SMTP)"),
    "xiuqo-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "xiuqo-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "xiuqo-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "xiuqo-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "xiuqo-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "xiuqo-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "xiuqo-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "xiuqo-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "xiuqo-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _XIUQO_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "xiuqo-sms": _bundle("SMS bot toolset - interact with Xiuqo via SMS (Twilio)"),
    "xiuqo-webhook": _ts("Webhook toolset - receive and process external webhook events", _XIUQO_WEBHOOK_SAFE_TOOLS),
    "xiuqo-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "xiuqo-telegram", "xiuqo-discord", "xiuqo-whatsapp", "xiuqo-slack",
            "xiuqo-signal", "xiuqo-bluebubbles", "xiuqo-homeassistant", "xiuqo-email",
            "xiuqo-sms", "xiuqo-mattermost", "xiuqo-matrix", "xiuqo-dingtalk",
            "xiuqo-feishu", "xiuqo-wecom", "xiuqo-wecom-callback", "xiuqo-weixin",
            "xiuqo-qqbot", "xiuqo-webhook", "xiuqo-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _XIUQO_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only xiuqo-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_XIUQO_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `xiuqo-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("xiuqo-"):
        return []
    platform_name = name[len("xiuqo-"):]
    try:
