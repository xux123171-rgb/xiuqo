...
from xiuqo_constants import OPENROUTER_BASE_URL, get_hermes_home
from agent.retry_utils import jittered_backoff
from xiuqo_cli.env_loader import load_hermes_dotenv

# Load .env from XIUQO_HOME first, then project root as a dev fallback.
load_hermes_dotenv(xiuqo_home=get_hermes_home(), project_env=Path(__file__).parent / ".env")
...
class TrajectoryCompressor:
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                raise RuntimeError(f"Provider '{provider}' is not configured. Check your API key or run: xiuqo setup")
