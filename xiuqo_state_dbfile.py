"""state.db file-level health helpers, split out of ``xiuqo_state.py``.
...
holder-process classification.  Helpers that xiuqo_state itself imports and
calls (``_connect_tracked_db`` & co) are looked up lazily from ``xiuqo_state`` at
call time, so tests that monkeypatch ``xiuqo_state.<name>`` keep intercepting.
"""
...
# shared lock (see xiuqo_cli/sqlite_safe_read.py) so another process can treat this writer as
...
def _read_sqlite_application_id(db_path: Path) -> "Optional[int]":
    ...
    from xiuqo_state_errors import _STATE_DB_APPLICATION_ID_OFFSET
...
def refuse_deleted_wal_generation(db_path) -> None:
    ...
    *before* ``sqlite3.connect`` so a second opener cannot mint a replacement WAL inode."""
    from xiuqo_state import DeletedWalGenerationError
    from xiuqo_state_errors import _DELETED_WAL_GENERATION_MSG
...
# pair that `xiuqo sessions recover --source <dir>/state.db` can open. Above it only the 100-byte
    ...
    try:
        ...
        manifest: Dict[str, Any] = {
            ...
            "note": ("Frames in the captured WAL were committed by the retired generation. Whether they "
                     ...
                     "the copied image with `xiuqo sessions recover --inspect-only` first."),
        }
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    without xiuqo_cli); a real connection failure must propagate — a silent untracked retry
    would disable the guard for that connection."""
    try:
        from xiuqo_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug("xiuqo_cli.sqlite_safe_read unavailable; opening %s untracked "
                     "(byte-probe guard inactive in this install)", path)
        return sqlite3.connect(str(path), **kwargs)
    # Open through THIS module's sqlite3.connect so tests patching xiuqo_state.sqlite3.connect keep control.
...
def is_zeroed_state_db(path: Path, *, probe_bytes: int = 100, force: bool = False) -> bool:
    ...
    ``xiuqo_cli.backup.is_zeroed_sqlite_file``; this copy keeps SessionDB openable without the CLI
    ...
    with quarantine_cross_process_lock(path) as acquired:
        if not acquired:
            logger.error("quarantine lock for %s not acquired within 5s — refusing to "
                         ...
                         "state-snapshots via `xiuqo snapshot list` / `xiuqo snapshot restore <id>`.",
                         path)
            return None
        return _do_quarantine()
...
def collect_state_db_stats(db_path: Path) -> Dict[str, Any]:
    ...
    from xiuqo_state import _connect_tracked_db
