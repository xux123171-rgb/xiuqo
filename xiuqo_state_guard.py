...
_STATE_DB_GUARD_BYPASS_ENV = "XIUQO_STATE_DB_GUARD_BYPASS"


def _real_platform_state_root() -> Optional[Path]:
    """The REAL platform-default Xiuqo root. Avoids ``Path.home()`` /
    ``xiuqo_constants`` (tests monkeypatch Path.home to a tempdir); ``expanduser``
    reads HOME/passwd, which the conftest never rewrites."""
    try:
        home = Path(os.path.expanduser("~"))
        if sys.platform == "win32":
            base = os.environ.get("LOCALAPPDATA", "").strip()
            root = Path(base) / "xiuqo" if base else home / "AppData" / "Local" / "xiuqo"
        else:
            root = home / ".xiuqo"
        return root.resolve()
    except Exception:
...
#: Exported by the hermetic conftest alongside the XIUQO_HOME redirect. Unlike
...
# : Env marker exported by the hermetic test conftest at the same moment it : redirects ``XIUQO_HOME`` to
...
# under Xiuqo test : isolation", and it inherits into subprocess children by default — so a : child that
# received the patched ``XIUQO_HOME`` also received the marker, : and a child that resolves a production DB
# while carrying it is, by : definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "XIUQO_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    rebuilt env loses PYTEST_* and the XIUQO_HOME redirect together, ancestry
    ...
    loses at the same moment it loses the ``XIUQO_HOME`` redirect: that child aims at the production DB
