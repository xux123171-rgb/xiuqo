#!/bin/sh
# shellcheck shell=sh
# /opt/xiuqo/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching XIUQO_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `xiuqo` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `xiuqo <args>` (subcommand passthrough)
#
# Drop to xiuqo via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${XIUQO_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${XIUQO_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export XIUQO_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset XIUQO_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid xiuqo "$@"; exec "$@"; }
...
# an arbitrary non-root, non-xiuqo UID, the bootstrap was skipped and the
# baked image dirs (owned by the xiuqo build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u xiuqo)" ]; then
    cat >&2 <<EOF
[xiuqo] ERROR: container started with --user $cur_uid (an arbitrary, non-xiuqo UID) — not supported.
...
# to the xiuqo user's home before dropping privileges so libraries that
...
_hermes_orig_cwd="${XIUQO_ORIG_CWD:-$PWD}"
...
# the user's command so `xiuqo chat` starts in the Docker -w
