...
# Xiuqo WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: XIUQO_TINI_SHIM_TARGET overrides the `/init` path (and
# XIUQO_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${XIUQO_TINI_SHIM_TARGET:-/init}"
WRAPPER="${XIUQO_TINI_SHIM_WRAPPER:-/opt/xiuqo/docker/main-wrapper.sh}"
