...
#
# Wired into the image as /etc/cont-init.d/01-xiuqo-setup by the
...
XIUQO_HOME="${XIUQO_HOME:-/opt/data}"
INSTALL_DIR="/opt/xiuqo"

# Drop to xiuqo via s6-setuidgid, but skip it when already non-root.
as_hermes() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid xiuqo "$@"; }
...
# starts non-root. The baked install tree under /opt/xiuqo is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $XIUQO_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass XIUQO_UID/XIUQO_GID — or the PUID/PGID aliases — which the
...
# `id -u xiuqo` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# xiuqo UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u xiuqo)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-xiuqo UID).
...
and the baked /opt/xiuqo install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e XIUQO_UID=\$(id -u) -e XIUQO_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the xiuqo user to that UID/GID at boot and chowns the data
...
# --- Bootstrap XIUQO_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid xiuqo mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom XIUQO_HOME paths whose parents only
# root can create (e.g. `XIUQO_HOME=/home/xiuqo/.xiuqo` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for XIUQO_UID/XIUQO_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  XIUQO_UID/
# XIUQO_GID still win when both are set.  See #15290, salvages #25872.
XIUQO_UID="${XIUQO_UID:-${PUID:-}}"
XIUQO_GID="${XIUQO_GID:-${PGID:-}}"

if [ -n "${XIUQO_UID:-}" ] && validate_uid_gid "$XIUQO_UID" && [ "$XIUQO_UID" != "$(id -u xiuqo)" ]; then
    echo "[stage2] Changing xiuqo UID to $XIUQO_UID"
    usermod -u "$XIUQO_UID" xiuqo
fi
if [ -n "${XIUQO_GID:-}" ] && validate_uid_gid "$XIUQO_GID" && [ "$XIUQO_GID" != "$(id -g xiuqo)" ]; then
    echo "[stage2] Changing xiuqo GID to $XIUQO_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$XIUQO_GID" xiuqo 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised xiuqo user (UID 10000)
...
# xiuqo process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes xiuqo. Idempotent across container
...
#   - xiuqo is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G xiuqo 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] xiuqo already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" xiuqo 2>/dev/null; then
        echo "[stage2] Added xiuqo to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group xiuqo failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When XIUQO_UID is remapped or the top-level $XIUQO_HOME isn't owned by
# the runtime xiuqo UID, restore ownership to xiuqo — but ONLY for the
# directories xiuqo actually writes to. The full $XIUQO_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of xiuqo-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_hermes_uid=$(id -u xiuqo)
...
chown_hermes_tree() {
    ...
    chown -R xiuqo:xiuqo "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_hermes_owner() {
    target="$1"
    find "$target" \( ! -user xiuqo -o ! -group xiuqo \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$XIUQO_HOME" 2>/dev/null)" != "$actual_hermes_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $XIUQO_HOME (targeted) to xiuqo ($actual_hermes_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $XIUQO_HOME: chown the directory itself (not its contents)
    # so xiuqo can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$XIUQO_HOME"; then
        :
    else
        chown xiuqo:xiuqo "$XIUQO_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $XIUQO_HOME failed (rootless container?) — continuing"
    fi
    # Xiuqo-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by xiuqo (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$XIUQO_HOME/$sub" ] && tree_has_non_hermes_owner "$XIUQO_HOME/$sub"; then
            chown_hermes_tree "$XIUQO_HOME/$sub"
        fi
    done
fi
...
# xiuqo user. Hosted/container instances keep mutable state under
# $XIUQO_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# XIUQO_DISABLE_LAZY_INSTALLS=1. Keeping /opt/xiuqo root-owned and
...
# $XIUQO_HOME/lazy-packages dir on the data volume (Dockerfile sets
# XIUQO_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to xiuqo
...
# Always reset ownership of $XIUQO_HOME/profiles to xiuqo on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> xiuqo …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as xiuqo and walks
...
if [ -d "$XIUQO_HOME/profiles" ] && tree_has_non_hermes_owner "$XIUQO_HOME/profiles"; then
    chown_hermes_tree "$XIUQO_HOME/profiles"
fi

# Always reset ownership of $XIUQO_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged xiuqo runtime even
...
# gate as profiles/).
if [ -d "$XIUQO_HOME/cron" ] && tree_has_non_hermes_owner "$XIUQO_HOME/cron"; then
    chown_hermes_tree "$XIUQO_HOME/cron"
fi

# Always ensure logs/gateways is xiuqo-owned (#45258). Formerly healed by
...
# top-level $XIUQO_HOME is mis-owned, so a warm volume with xiuqo-owned
# XIUQO_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid xiuqo mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as xiuqo.
if [ -d "$XIUQO_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$XIUQO_HOME/logs/gateways"; then
        :
    else
        chown xiuqo:xiuqo "$XIUQO_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# xiuqo pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged xiuqo gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $XIUQO_HOME is
...
# and cron/.
if [ -d "$XIUQO_HOME/platforms/pairing" ] && tree_has_non_hermes_owner "$XIUQO_HOME/platforms/pairing"; then
    chown_hermes_tree "$XIUQO_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$XIUQO_HOME/pairing" ] && tree_has_non_hermes_owner "$XIUQO_HOME/pairing"; then
    chown_hermes_tree "$XIUQO_HOME/pairing"
fi

# Reset ownership of xiuqo-owned top-level state files on every boot.
# The targeted data-volume chown above only covers xiuqo-owned
# *subdirectories*; loose state files living directly under $XIUQO_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> xiuqo …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged xiuqo runtime then hits
...
# sweep so host-owned files in a bind-mounted $XIUQO_HOME are never
...
# entries of xiuqo_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$XIUQO_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$XIUQO_HOME/$f"; then
            :
        else
            chown xiuqo:xiuqo "$XIUQO_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the xiuqo runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$XIUQO_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$XIUQO_HOME/config.yaml"; then
        :
    else
        chown xiuqo:xiuqo "$XIUQO_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$XIUQO_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as xiuqo user ---
# Run as xiuqo via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $XIUQO_HOME values
...
as_hermes mkdir -p \
    ...
    "$XIUQO_HOME/sessions" \
    "$XIUQO_HOME/logs" \
    "$XIUQO_HOME/logs/gateways" \
    "$XIUQO_HOME/hooks" \
    "$XIUQO_HOME/memories" \
    "$XIUQO_HOME/skills" \
    "$XIUQO_HOME/skins" \
    "$XIUQO_HOME/plans" \
    "$XIUQO_HOME/workspace" \
    "$XIUQO_HOME/home" \
    "$XIUQO_HOME/pairing" \
    "$XIUQO_HOME/platforms/pairing" \
    "$XIUQO_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/xiuqo/.install_method (see Dockerfile), NOT written here into
# $XIUQO_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $XIUQO_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.xiuqo:/opt/data) and sometimes shared with a
...
# 'xiuqo update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $XIUQO_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/xiuqo stamp, so deleting the data-dir copy is safe).
if [ -f "$XIUQO_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$XIUQO_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$XIUQO_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$XIUQO_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$XIUQO_HOME/$dest"; then
            :
        else
            as_hermes cp "$INSTALL_DIR/$src" "$XIUQO_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/xiuqo/.env.example being present
...
# not generate one. Xiuqo loads $XIUQO_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$XIUQO_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$XIUQO_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $XIUQO_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$XIUQO_HOME/.env" ] && ! refuse_symlinked_path "clean" "$XIUQO_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$XIUQO_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$XIUQO_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$XIUQO_HOME/.env"; then
        :
    else
        if [ ! -f "$XIUQO_HOME/.env" ]; then
            ...
            (umask 077 && as_hermes touch "$XIUQO_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$XIUQO_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$XIUQO_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$XIUQO_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $XIUQO_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $XIUQO_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$XIUQO_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$XIUQO_HOME/.env"; then
        :
    else
        chown xiuqo:xiuqo "$XIUQO_HOME/.env" 2>/dev/null || true
        chmod 600 "$XIUQO_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `xiuqo` user — without this it gets EACCES on every
...
# xiuqo group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp xiuqo /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted xiuqo group access to the Fly Machines API socket"
    else
...
# $XIUQO_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `xiuqo update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set XIUQO_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$XIUQO_HOME/config.yaml" ]; then
    s6-setuidgid xiuqo "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$XIUQO_HOME/auth.json" ] && [ -n "${XIUQO_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$XIUQO_HOME/auth.json"; then
        :
    else
        printf '%s' "$XIUQO_AUTH_JSON_BOOTSTRAP" > "$XIUQO_HOME/auth.json"
        chown xiuqo:xiuqo "$XIUQO_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$XIUQO_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via XIUQO_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$XIUQO_HOME/auth.json" ] && [ -n "${XIUQO_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$XIUQO_HOME/auth.json"; then
        :
    else
        s6-setuidgid xiuqo "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$XIUQO_HOME/auth.json" \
...
# can set XIUQO_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# XIUQO_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$XIUQO_HOME/gateway_state.json" ] && \
        [ "${XIUQO_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$XIUQO_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$XIUQO_HOME/gateway_state.json"
        chown xiuqo:xiuqo "$XIUQO_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$XIUQO_HOME/gateway_state.json"
    fi
fi
...
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/xiuqo/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Xiuqo spawns for the
...
# main-wrapper.sh propagates it into the supervised ``xiuqo`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-xiuqo, dashboard, etc.).
