"""Shared constants and helpers for the SessionDB family of modules.  Lives outside xiuqo_state so
...
def routed_sessions_setting(key: str, env_var: str) -> Any:
    ...
    carrier CLI/cron children read). Under a multiplexer a routed turn runs with a XIUQO_HOME override
...
# 'new_session' (xiuqo_cli/cli_session_mixin.py) without a reset child row.  A compression rotation must
...
# optimized via ``xiuqo sessions optimize-storage``. A legacy DB sits at
...
--
-- A host-declared conversation key (X-Xiuqo-Session-Key / build_session_key)
...
# and must not serve reads until `xiuqo sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `xiuqo db optimize`. This is
...
# claims progress under SQLite transaction authority).  Mirrors `xiuqo_state_repair._cross_process_repair_lock`:
...
# `<db>.repair.lock` (offline schema surgery, minutes in VACUUM).  Lives here: mixins cannot import xiuqo_state.

# ── Cross-process full-FTS-rebuild admission (single authority) ────────────── Several independent Xiuqo
# processes routinely share one state.db (gateway service, the Desktop app's `xiuqo serve` backend,
...
# authority and is intentionally multi-process. Semantics mirror `xiuqo_state_repair._cross_process_repair_lock`
...
# indeterminate liveness still defers. It lives here (not xiuqo_state) because the search/schema mixins
# cannot import xiuqo_state (cycle). The lock file is `<db>.fts_rebuild.lock`, distinct from
...
def _describe_lock_holder(record) -> str:
    ...
    if not isinstance(record, dict) or "pid" not in record:
        return "unknown (no holder record; pre-fix writer or non-Xiuqo)"
    age = ""
